...
## Follow-up tools (2026-09-26)

- `parity_analysis.py` now replaces IrRep's plain-metric orthogonality message (single-precision normalisation plus the PAW S metric; see `../workflow.md`) with `independent_wavefunction_check`: a float64 inversion matrix rebuilt from the WAVECAR, which must be unitary and ±1 on bands 1..N (1e-6), have opposite-parity overlaps ≤1e-6 and per-band inversion residuals ≤1e-5 over bands 1..N+2, and reproduce IrRep's odd-state counts. All other IrRep warnings still fail closed.
- `tr_evidence.py CAMPAIGN_ROOT` writes `<structure>/tr_evidence.json`: magnetisation density and one-centre PAW magnetisation from `scf/CHGCAR`, TRIM Kramers splitting from `trim/vaspout.h5`, and E(k) vs E(−k) on the SCF grid for structures without inversion. It characterises the converged state only.
- `spin_screen.py` / `spin_screen.pbs`: collinear ISPIN=2 SCFs (vasp_std) seeded with ferromagnetic, layer-alternating and, for odd Be layer counts, inversion-odd moments; `spin_screen/spin_screen_summary.json` reports whether every seed collapsed below 1e-3 μB.
- `refine_gap_targeted.py` / `refine_gap_targeted.pbs`: adaptive fixed-density SOC zoom into explicit direct-gap basins (`--center k1,k2,halfwidth`), 9×9 patches re-centred on each basin minimum and shrunk 4× when the minimum is interior; stages `gap_refine_3a…` and `gap_refine_3_summary.json` with a heuristic local slope bound.
