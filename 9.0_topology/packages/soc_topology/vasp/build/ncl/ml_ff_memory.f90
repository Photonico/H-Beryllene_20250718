...
! interactions will be executed or not.
            LOGICAL, INTENT(IN)   :: LMLONLY_FF ! Variable that specifies whether the training will be 1._q or not.
...
! Initialization of variables
            ...
            MEM1%RMEM_SINV=0.0_q
            MEM1%RMEM_SINV_ALL=0.0_q
            MEM1%RMEM_PS=0.0_q
...
! Calculate the maximum number of atom per element
            ...
            MNEIB_EWST=0
            NG        =0
! Calculate local size of MITYP_FF
            MITYP_LOCAL=REAL(MITYP_FF,q)/REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)
...
! Estimate the number of atoms within the cutoff radius
            ...
            ENDIF
! Set size of local arrays used in 1
            IF(LMLMB_FF) THEN
               ...
               CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_CMAT_FFM%DESC,PAR_SUP_HANDLE%DESC_CMAT_FFM%M, &
                    PAR_SUP_HANDLE%DESC_CMAT_FFM%N,PAR_SUP_HANDLE%DESC_CMAT_FFM%MB, &
                    PAR_SUP_HANDLE%DESC_CMAT_FFM%NB,PAR_SUP_HANDLE%DESC_CMAT_FFM%IRSRC, &
                    PAR_SUP_HANDLE%DESC_CMAT_FFM%ICSRC,PAR_SUP_HANDLE%CONTEXT_WORLD%ICTXT, &
                    PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW,PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL,  &
                    PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL, &
                    PAR_SUP_HANDLE%DESC_CMAT_FFM%LLD, &
...
! Optional reduced descriptor
               IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                  PAR_SUP_HANDLE%DESC_PS_FFM_RED%M=FFM%RV%MMVAR2_RED
                  PAR_SUP_HANDLE%DESC_PS_FFM_RED%N=MB_FFM
                  PAR_SUP_HANDLE%DESC_PS_FFM_RED%MB=NBLOCK_SCALAPACK
                  PAR_SUP_HANDLE%DESC_PS_FFM_RED%NB=NBLOCK_SCALAPACK
                  PAR_SUP_HANDLE%DESC_PS_FFM_RED%IRSRC=0
                  PAR_SUP_HANDLE%DESC_PS_FFM_RED%ICSRC=0
                  CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_PS_FFM_RED%DESC, &
                       PAR_SUP_HANDLE%DESC_PS_FFM_RED%M,PAR_SUP_HANDLE%DESC_PS_FFM_RED%N, &
                       PAR_SUP_HANDLE%DESC_PS_FFM_RED%MB,PAR_SUP_HANDLE%DESC_PS_FFM_RED%NB, &
                       PAR_SUP_HANDLE%DESC_PS_FFM_RED%IRSRC,PAR_SUP_HANDLE%DESC_PS_FFM_RED%ICSRC, &
                       PAR_SUP_HANDLE%CONTEXT_WORLD%ICTXT,PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW, &
                       PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL,  &
                       PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL, &
                       PAR_SUP_HANDLE%DESC_PS_FFM_RED%LLD, &
                       PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M,PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_N)
               ENDIF
               IF (FF%ISTART.EQ.2 .AND. FF%LFAST) THEN
                  PAR_SUP_HANDLE%DESC_PS_NEW_FFM%M=1
                  PAR_SUP_HANDLE%DESC_PS_NEW_FFM%N=1
               ELSE
...
! For many-body interaction force field variables that are stored statically
            IF(LMLMB_FF) THEN
               IF(W1_FFM.GT.0.0_q) THEN
                  IF (ISTART_FF.EQ.2 .AND. FF%LFAST) THEN
!                     MEM0%RMEM(2)=MEM0%RMEM(2)+MITYP_LOCAL*REAL(MB_FFM,q)*REAL(MTYP_FF,q)*8.0_q/1.0E6_q ! SOAP_C001
!                     MEM0%RMEM(2)=MEM0%RMEM(2)+MITYP_LOCAL*REAL(MB_FFM,q)*REAL(MTYP_FF,q)*8.0_q/1.0E6_q ! SOAP_C002
                     MEM0%RMEM(11)=MEM0%RMEM(11) + 2.0_q*MITYP_LOCAL*REAL(MB_FFM,q)* &
                                   REAL(MTYP_FF,q)*8.0_q/1.0E6_q ! SOAP_C001 SOAP_C002
                  ENDIF
               ENDIF
               IF(W2_FFM.GT.0.0_q) THEN
                  IF (ISTART_FF.EQ.2 .AND. FF%LFAST) THEN
!                     MEM0%RMEM(2)=MEM0%RMEM(2)+MITYP_LOCAL*REAL(MB_FFM,q)*REAL(MTYP_FF,q)*8.0_q/1.0E6_q ! SOAP_PS1
!                     MEM0%RMEM(2)=MEM0%RMEM(2)+MITYP_LOCAL*REAL(MB_FFM,q)*REAL(MTYP_FF,q)*8.0_q/1.0E6_q ! SOAP_PS2
                     MEM0%RMEM(11)=MEM0%RMEM(11) + 2.0_q*MITYP_LOCAL*REAL(MB_FFM,q)* &
                                   REAL(MTYP_FF,q)*8.0_q/1.0E6_q ! SOAP_PS1 SOAP_PS2
                  ENDIF
               ENDIF
               IF(W2_FFM.GT.0.0_q) THEN
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MMVAR2_FFM,q)*(REAL((LMAX2_FFM+1),q)**2)*REAL(MRB2_FFM,q)* &
                               REAL(MTYP_FF,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! INDEX_MAP_TO_PS
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MMVAR2_FFM,q)*(REAL((LMAX2_FFM+1),q)**2)*REAL(MRB2_FFM,q)* &
                               REAL(MTYP_FF,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! INDEX_MAP_TO_PS_B
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MMVAR2_FFM,q)*(REAL((LMAX2_FFM+1),q)**2)*REAL(MRB2_FFM,q)* &
                               REAL(MTYP_FF,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! INDEX_PAIR_HELP2
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MMVAR2_FFM,q)*(REAL((LMAX2_FFM+1),q)**2)*REAL(MRB2_FFM,q)* &
                               REAL(MTYP_FF,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! INDEX_PAIR_HELP2_B
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MMVAR2_FFM,q)*(REAL((LMAX2_FFM+1),q)**2)*REAL(MRB2_FFM,q)* &
                               REAL(MTYP_FF,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! PREFACTOR_HELP2
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MMVAR2_FFM,q)*(REAL((LMAX2_FFM+1),q)**2)*REAL(MRB2_FFM,q)* &
                               REAL(MTYP_FF,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! PREFACTOR_HELP2_B
               ENDIF
...
# 673
                  ...
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_M,q)* &
                               REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! PS
                  MEM0%RMEM_PS=MEM0%RMEM_PS+REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_M,q)* &
                               REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! PS
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_PS_NEW_FFM%LOC_M,q)* &
                               REAL(PAR_SUP_HANDLE%DESC_PS_NEW_FFM%LOC_N,q)*REAL(MTYP_FF,q)*REAL(MCONF_NEW_FF,q)*8/1.0E6_q ! PS_NEW
! Optional reduced descriptor
                  IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                     MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M,q)* &
                                  REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! PS
                     MEM0%RMEM_PS=MEM0%RMEM_PS+REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M,q)* &
                                  REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E6_q ! PS
                  ENDIF
               ENDIF
               IF (FF%ISTART.EQ.2 .AND. FF%LFAST) THEN
...
# 725
               ...
               MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MB_FFM,q)*REAL(MB_FFM,q)*REAL(MTYP_FF,q)*8/1000000_q ! SINV_ALL
               MEM0%RMEM_SINV_ALL=MEM0%RMEM_SINV_ALL+REAL(MB_FFM,q)*REAL(MB_FFM,q)* &
                                  REAL(MTYP_FF,q)*8/1000000_q ! SINV_ALL
...
!MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(MB_FFM,q)*REAL(MB_FFM,q)*REAL(MTYP_FF,q)*8/1000000_q

               MEM0%RMEM_SINV=MEM0%RMEM_SINV+REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,q) * &
                              REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q
# 745
               ...
               IF (ISTART_FF.NE.2) THEN
                  ...
                  IF (FF%IALGO_LINREG.GT.1) THEN
                     MEM0%RMEM(2)=MEM0%RMEM(2)+2.0_q*REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_M,q) * &
                                  REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! FMAT
                     MEM0%RMEM_FMAT=MEM0%RMEM_FMAT+2.0_q*REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_M,q) * &
                                    REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! FMAT
                  ELSE
                     MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_M,q) * &
                                  REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! FMAT
                     MEM0%RMEM_FMAT=MEM0%RMEM_FMAT+REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_M,q) * &
                                    REAL(PAR_SUP_HANDLE%DESC_FMAT_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! FMAT
                  ENDIF
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_FMAT_NEW_FFM%LOC_M,q) * &
                               REAL(PAR_SUP_HANDLE%DESC_FMAT_NEW_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! FMAT_NEW
               ENDIF

               MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,q) * &
                            REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! SOAP
               MEM0%RMEM_SOAP=MEM0%RMEM_SOAP+REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,q) * &
                              REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! SOAP
               IF (ISTART_FF.NE.2) THEN
                  MEM0%RMEM(2)=MEM0%RMEM(2)+12.0_q*REAL(PAR_SUP_HANDLE%DESC_WMAT_FFM%LOC_M,q) * &
                               REAL(PAR_SUP_HANDLE%DESC_WMAT_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! FFM%WMAT
               ELSE
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_WMAT_FFM%LOC_M,q) * &
                               REAL(PAR_SUP_HANDLE%DESC_WMAT_FFM%LOC_N,q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! FFM%WMAT
               ENDIF

               IF(ISTART_FF.NE.2) THEN
                  MEM0%RMEM(2)=MEM0%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_YMAT_FFM%LOC_M,q) * &
                               REAL(PAR_SUP_HANDLE%DESC_YMAT_FFM%LOC_N,q)*8/1.0E+06_q ! FFM%YMAT
               ENDIF
...
! For temporary variables necessary for many-body prediction
            IF(LMLMB_FF) THEN
               ...
               RMEM_TMP=RMEM_TMP+REAL(PAR_SUP_HANDLE%DESC_BE_ENERGY%LOC_M,q)* &
                        REAL(PAR_SUP_HANDLE%DESC_BE_ENERGY%LOC_N,q)*8/1.0E+06_q ! BE_ENERGY declared in MANYBODY
               RMEM_TMP=RMEM_TMP+REAL(PAR_SUP_HANDLE%DESC_BE_FORCE%LOC_M,q)* &
                        REAL(PAR_SUP_HANDLE%DESC_BE_FORCE%LOC_N,q)*8/1.0E+06_q ! BE_FORCE declared in MANYBODY
               RMEM_TMP=RMEM_TMP+REAL(PAR_SUP_HANDLE%DESC_BE_STRESS%LOC_M,q)* &
                        REAL(PAR_SUP_HANDLE%DESC_BE_STRESS%LOC_N,q)*8/1.0E+06_q ! BE_STRESS declared in MANYBODY
               RMEM_TMP=RMEM_TMP+REAL(PAR_SUP_HANDLE%DESC_SOAP%LOC_M,q)* &
                        REAL(PAR_SUP_HANDLE%DESC_SOAP%LOC_N,q)*8/1.0E+06_q ! SOAP1_HELP2
               RMEM_TMP=RMEM_TMP+REAL(PAR_SUP_HANDLE%DESC_SOAP%LOC_M,q)* &
                        REAL(PAR_SUP_HANDLE%DESC_SOAP%LOC_N,q)*8/1.0E+06_q ! SOAP2_HELP2
               RMEM_TMP=RMEM_TMP+REAL(PAR_SUP_HANDLE%DESC_ZMAT%LOC_M,q)* &
                        REAL(PAR_SUP_HANDLE%DESC_ZMAT%LOC_N,q)*8/1.0E+06_q ! ZMAT declared in MANYBODY
# 1213

               IF(W1_FFM.GT.0.0_q) THEN 
                  IF (ISTART_FF.EQ.2 .AND. FF%LFAST) THEN
                     RMEM_TMP=RMEM_TMP+REAL(MMVAR1_FFM,q)*REAL(MTYP_FF,q)*MITYP_LOCAL*8/1.0E6_q ! L_MAT1
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM1,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! PRODUCT_L_DPDC1
                     RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! SOAP_C001
                     RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! SOAP_C002
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM1,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! CLM1
                     RMEM_TMP=RMEM_TMP+REAL(MMVAR1_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! C00
                     RMEM_TMP=RMEM_TMP+MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! RNORM_C00
                     IF (LCOUPLE_FFM) THEN
                        RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM1,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! CLM_COUPLE_HELP1_MANYBODY
                        RMEM_TMP=RMEM_TMP+REAL(MMVAR1_FFM,q)*REAL(MTYP_FF,q)*MITYP_LOCAL*8/1.0E6_q ! C00_COUPLE
                     ENDIF
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM1,q)*REAL((MNEIB_EST1+1),q)* &
                              REAL(MTYP_FF,q)*3*MITYP_LOCAL*8/1.0E6_q ! DCLM1 in MANYBODY in ml_ff_ff2.F
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM1,q)*REAL(MTYP_FF,q)* &
                              REAL(MTYP_FF,q)*3*MITYP_LOCAL*8/1.0E6_q ! DCLM1_HEAD in MANYBODY in ml_ff_ff2.F
                  ELSE
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM1,q)*REAL((MNEIB_EST1+1),q)* &
                              REAL(MTYP_FF,q)*3*8/1.0E6_q ! DCLM1 declared in MANYBODY
                  ENDIF
                  RMEM_TMP=RMEM_TMP+REAL(MMVAR1_FFM,q)*REAL(CEILING(REAL(MITYP_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*REAL(MTYP_FF,q)*8/1.0E6_q ! C00 declared in MANYBODY
                  IF (LCOUPLE_FFM) THEN
                     RMEM_TMP=RMEM_TMP+REAL(MMVAR1_FFM,q)*REAL(CEILING(REAL(MITYP_FF,q)/ &
                                        REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*REAL(MTYP_FF,q)* &
                                        8/1.0E6_q ! C00_COUPLE declared in MANYBODY
                  ENDIF 
                  ...
                  RMEM_TMP=RMEM_TMP+REAL((MNEIB_EST1+1),q)*REAL(MTYP_FF,q)*REAL(CEILING(REAL(MIONS_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)/1.0E6_q ! LNIONS_EST1 declared in MANYBODY
                  RMEM_TMP=RMEM_TMP+REAL(MTYP_FF,q)*REAL(CEILING(REAL(MIONS_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)/1.0E+06_q ! NNEIB_EST1 declared in MANYBODY
                  ...
                  RMEM_TMP=RMEM_TMP+REAL((MNEIB_EST1+1),q)*REAL(MTYP_FF,q)*REAL(CEILING(REAL(MIONS_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*3*8/1.0E6_q ! XYZ_EST1 declared in MANYBODY
               ENDIF
               IF(W2_FFM.GT.0.0E0_q) THEN
                  IF (ISTART_FF.EQ.2 .AND. FF%LFAST) THEN
                     RMEM_TMP=RMEM_TMP+REAL(MMVAR2_FFM,q)*REAL(MTYP_FF,q)*MITYP_LOCAL*8/1.0E6_q ! L_MAT2
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM2,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! PRODUCT_L_DPDC2
                     RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! SOAP_PS1
                     RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! SOAP_PS2
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM2,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! CLM2
                     RMEM_TMP=RMEM_TMP+REAL(MMVAR2_FFM,q)*REAL(MTYP_FF,q)*MITYP_LOCAL*8/1.0E6_q ! PS
                     RMEM_TMP=RMEM_TMP+REAL((LMAX2_FFM+1)**2*MTYP_FF*MRB2_FFM,q)*REAL((MNEIB_EST2+1),q)* &
                              REAL(MTYP_FF,q)*3*MITYP_LOCAL*8/1.0E6_q ! DCLM2 in MANYBODY in ml_ff_ff2.F
                     RMEM_TMP=RMEM_TMP+REAL((LMAX2_FFM+1)**2*MTYP_FF*MRB2_FFM,q)*REAL(MTYP_FF,q)* &
                              REAL(MTYP_FF,q)*3*MITYP_LOCAL*8/1.0E6_q ! DCLM2_HEAD in MANYBODY in ml_ff_ff2.F
                     IF (LCOUPLE_FFM) THEN
                        RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM2,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! CLM_COUPLE_HELP2_MANYBODY
                        RMEM_TMP=RMEM_TMP+REAL(MMVAR2_FFM,q)*REAL(MTYP_FF,q)*MITYP_LOCAL*8/1.0E6_q ! PS_COUPLE
                     ENDIF
                     RMEM_TMP=RMEM_TMP+MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! RNORM_PS
                  ELSE
                     RMEM_TMP=RMEM_TMP+REAL(MVAR_CLM2,q)*REAL((MNEIB_EST2+1),q)* &
                              REAL(MTYP_FF,q)*3*8/1.0E6_q ! DCLM2 declared in MANYBODY
                  ENDIF
                  ...
                  RMEM_TMP=RMEM_TMP+REAL((MNEIB_EST2+1),q)*REAL(MTYP_FF,q)*REAL(CEILING(REAL(MIONS_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)/1.0E6_q ! LNIONS_EST2 declared in MANYBODY
                  RMEM_TMP=RMEM_TMP+REAL(MTYP_FF,q)*REAL(CEILING(REAL(MIONS_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)/1.0E+06_q ! NNEIB_EST2 declared in MANYBODY
                  RMEM_TMP=RMEM_TMP+REAL(MMVAR2_FFM,q)*REAL(CEILING(REAL(MITYP_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*REAL(MTYP_FF,q)*8/1.0E+06_q ! PS declared in MANYBODY
                  IF (LCOUPLE_FFM) THEN
                     RMEM_TMP=RMEM_TMP+REAL(MMVAR2_FFM,q)*REAL(CEILING(REAL(MITYP_FF,q)/ &
                              REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*REAL(MTYP_FF,q)*8/1.0E6_q ! PS_COUPLE declared in MANYBODY
                  ENDIF
                  ...
                  RMEM_TMP=RMEM_TMP+REAL((MNEIB_EST2+1),q)*REAL(MTYP_FF,q)*REAL(CEILING(REAL(MIONS_FF,q)/ &
                           REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*3*8/1.0E6_q ! XYZ_EST2 declared in MANYBODY
               ENDIF
               IF (ISTART_FF.EQ.2 .AND. FF%LFAST) THEN
                  RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q !SOAP1 in ml_ff_ff2.F
                  RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q !SOAP2 in ml_ff_ff2.F
                  RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q !SOAP3 in ml_ff_ff2.F
                  IF (LCOUPLE_FFM) THEN
                     RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! SOAP1_COUPLE
                     RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! SOAP2_COUPLE
                  ENDIF
                  RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q !SOAP_ALL1 in ml_ff_ff2.F
                  RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q !SOAP_ALL2 in ml_ff_ff2.F
                  RMEM_TMP=RMEM_TMP+MITYP_LOCAL*REAL(MTYP_FF,q)*8/1.0E6_q ! RNORM_ALL
               ENDIF
               RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*REAL(CEILING(REAL(MITYP_FF,q)/ &
                        REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*8/1.0E+06_q ! SOAP1 declared in MANYBODY
               RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*REAL(CEILING(REAL(MITYP_FF,q)/ &
                        REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*8/1.0E+06_q ! SOAP2 declared in MANYBODY
               ...
               RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*REAL(CEILING(REAL(MIONS_FF,q)/ &
                        REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*3*8/1.0E6_q ! FORCE_HELP1 declared in MANYBODY
               RMEM_TMP=RMEM_TMP+REAL(MB_FFM,q)*REAL(MTYP_FF,q)*9*8/1.0E6_q ! STRESS_HELP declared in MANYBODY
               RMEM_TMP=RMEM_TMP+REAL(CEILING(REAL(MITYP_FF,q)/ &
                        REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q)),q)*8/1.0E6_q ! VEC_LOC_ENERGY declared in MANYBODY
...
! Flushing of output needs to be 1._q for memory estimate, since code can crash later when running out of memory
! and stuff in the output buffer is lost
           FLUSH(LOGF%IU)
