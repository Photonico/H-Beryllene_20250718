...
! in PAW
    ...
    INTERFACE CALC_RHOLM
      MODULE PROCEDURE CALC_RHOLM_REAL
      MODULE PROCEDURE CALC_RHOLM_CPLX
    END INTERFACE

    INTERFACE CALC_RHOLM_FOCK
      MODULE PROCEDURE CALC_RHOLM_FOCK_REAL
      MODULE PROCEDURE CALC_RHOLM_FOCK_CPLX
    END INTERFACE

    INTERFACE CALC_DLLMM
      MODULE PROCEDURE CALC_DLLMM_REAL
      MODULE PROCEDURE CALC_DLLMM_CPLX
    END INTERFACE

    INTERFACE CALC_DLLMM_FOCK
      MODULE PROCEDURE CALC_DLLMM_FOCK_REAL
      MODULE PROCEDURE CALC_DLLMM_FOCK_CPLX
    END INTERFACE
...
!*******************************************************************
    ...
    SUBROUTINE CALC_RHOLM_REAL( LYMAX, RHOLLMM, RHOLM, P)
      USE pseudo
      USE constant
      USE asa
      IMPLICIT NONE
...
# 320
      ...
      END SUBROUTINE CALC_RHOLM_REAL

!*******************************************************************
!
!> this routine does the same thing as CALC_RHOLM with
!> RHOLM being defined as COMPLEX(q) and not REAL(q)
!
!*******************************************************************


    SUBROUTINE CALC_RHOLM_CPLX( LYMAX, RHOLLMM, RHOLM, P)
      USE pseudo
      USE constant
      USE asa
      IMPLICIT NONE
      TYPE (potcar) P
      COMPLEX(q) RHOLLMM(:,:)   !< net augmentation charge
      COMPLEX(q) RHOLM(:)    !< local charge for each L,M
      INTEGER LYMAX          !< maximum L
! local varible
      INTEGER LMYMAX,CH1,CH2,LL,LLP,LM,LMP,LMINDX,ISTART,IEND,IC,M,MP
      REAL(q) FAKT
...
! loop over all channels (l,epsilon)
      LM=1
      DO CH1=1,P%LMAX
! we have to go over all channels not just the upper triangle
! this removes any complex contributions to the (1._q,0._q)-centre occupancies
      LMP=1
      DO CH2=1,P%LMAX
...
# 392
      ...
      END SUBROUTINE CALC_RHOLM_CPLX
...
!*******************************************************************
    ...
    SUBROUTINE CALC_RHOLM_FOCK_REAL( LYMAX, RHOLLMM, RHOLM, P, NAE)
      USE pseudo
      USE constant
      USE asa
      IMPLICIT NONE
...
# 474
    ...
    END SUBROUTINE CALC_RHOLM_FOCK_REAL
...
!*******************************************************************
    ...
    SUBROUTINE CALC_RHOLM_FOCK_CPLX( LYMAX, RHOLLMM, RHOLM, P, NAE)
      USE pseudo
      USE constant
      USE asa
      IMPLICIT NONE
...
# 555
    ...
    END SUBROUTINE CALC_RHOLM_FOCK_CPLX
...
!*******************************************************************
    ...
    SUBROUTINE CALC_DLLMM_REAL( DLLMM, DLM, P)
      USE pseudo
      USE asa
      IMPLICIT NONE
      TYPE (potcar) P
...
! fill symmetric components (CH2 is >= CH1)
      ...
      LM =LM +2*LL +1
      ENDDO
    END SUBROUTINE CALC_DLLMM_REAL


!****************** SUBROUTINE CALC_DLLMM_CPLX *********************
!
!> this routine does the same thing as CALC_DLLMM_REAL with
!> DLM being defined as COMPLEX(q) and not REAL(q)
!
!*******************************************************************

    SUBROUTINE CALC_DLLMM_CPLX( DLLMM, DLM, P)
      USE pseudo
      USE asa
      IMPLICIT NONE
      TYPE (potcar) P
...
! loop over all channels (l,epsilon)
      LM=1
      DO CH1=1,P%LMAX
! go over all channels not just the upper triangle (not certain whether this makes a difference here)
      LMP=1
      DO CH2=1,P%LMAX
...
! of the corresponding charge)
      ...
      LM =LM +2*LL +1
      ENDDO
    END SUBROUTINE CALC_DLLMM_CPLX


!************************* SUBROUTINE CALC_DLLMM_FOCK_REAL ************
!
!> this routine does the same thing as CALC_DLLMM_REAL
!> but uses QPAW_FOCK instead of QPAW
!
!**********************************************************************

    SUBROUTINE CALC_DLLMM_FOCK_REAL( DLLMM, DLM, P, NAE )
      USE pseudo
      USE asa
      IMPLICIT NONE
      TYPE (potcar) P
...
! fill symmetric components (CH2 is >= CH1)
      ...
      LM =LM +2*LL +1
      ENDDO
    END SUBROUTINE CALC_DLLMM_FOCK_REAL


!************************* SUBROUTINE CALC_DLLMM_FOCK_CPLX ************
!
!> this routine does the same thing as CALC_DLLMM_FOCK_REAL with
!> DLM being defined as COMPLEX(q) and not REAL(q)
!
!**********************************************************************

    SUBROUTINE CALC_DLLMM_FOCK_CPLX( DLLMM, DLM, P, NAE )
      USE pseudo
      USE asa
      IMPLICIT NONE
      TYPE (potcar) P
...
! loop over all channels (l,epsilon)
      LM=1
      DO CH1=1,P%LMAX
! go over all channels not just the upper triangle (not certain whether this makes a difference)
      LMP=1
      DO CH2=1,P%LMAX
...
! of the corresponding charge)
      ...
      LM =LM +2*LL +1
      ENDDO
    END SUBROUTINE CALC_DLLMM_FOCK_CPLX
...
!> and
!> ~~~
!> \int Q(L)(r) r^(2+l) dr is integrating to 1
!> ~~~
!>  the "pseudopotential strength" is then given by
!> ~~~
...
! local varible
      ...
      DO LM=1,P%LMMAX
         WRITE(*,'(20(F14.10,1X))') (AIMAG(DLLMM(LMP,LM)),LMP=1,MIN(10,P%LMMAX))
      ENDDO
...
!
!  SET_RHO_PAW
!> store the elements of the augmentation
...
!=======================================================================
      ...
      ion: DO NI=1,T_INFO%NIONS
         ...
         IF (NIP==0) THEN
            CALL vtutor%bug("SET_RHO_PAW: internal error: ion not local", "paw_base.F", 1815)
         ENDIF
...
! retrieve elements which are mixed
         ...
         IF (IBASE > ISIZE+1) THEN
            CALL vtutor%bug("internal error SET_RHO_PAW: insufficient space", "paw_base.F", 1835)
         ENDIF
      ENDDO ion
# 1843
...
!
!  RETRIVE_RHO_PAW
!> retrive the elements of the augmentation
!> occupancies CRHODE from RHOLM
!>
!> this is essentially the inverse of SET_RHO_PAW
...
! local variables
      ...
      COMPLEX(q) COCC(LMDIM,LMDIM,WDES%NCDIJ),COCC_IM(LMDIM,LMDIM)
      TYPE (potcar),POINTER:: PP
      INTEGER NT,NI,NIP,ISP,ITMP
      INTEGER ISIZE,IBASE,IADD
      INTEGER, EXTERNAL :: MAXL1
...
!=======================================================================
      ...
      ion: DO NI=1,T_INFO%NIONS
         ...
         NIP=NI_LOCAL(NI, WDES%COMM_INB) ! local storage index
         IF (NIP==0) THEN
            CALL vtutor%bug("SET_RHO_PAW: internal error: ion not local", "paw_base.F", 1911)
         ENDIF
...
!            CALL DUMP_DLLMM(CRHODE(:,:,NIP,ISP),PP)
         ...
         IBASE=IBASE+IADD
         IF (IBASE > ISIZE+1) THEN
            CALL vtutor%bug("internal error SET_RHO_PAW: insufficient space", "paw_base.F", 1952)
         ENDIF
         CRHODE(:,:,NIP,1:WDES%NCDIJ)=COCC(:,:,1:WDES%NCDIJ)
