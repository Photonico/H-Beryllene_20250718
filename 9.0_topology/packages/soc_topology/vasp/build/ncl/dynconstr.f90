...
!c parameters read in from           INCAR
        ...
        REAL(q) :: CSVR_PERIOD_GL=0._q       ! CSVR_PERIOD
...
!c PENALTY_READER, FBIAS_READER, HARMONIC_SPRING_READER
          ...
          ALLOCATE(FMP_GL%ACTIVETYPE(T_INFO%NTYP))
          FMP_GL%ACTIVETYPE=.TRUE.

          CALL DYN_READER(IO,g_io,MDALGO_GL,ANDERSEN_PROB_GL,CSVR_PERIOD_GL,&
          &    SCALING_GL,NSUBSYS_GL,TSUBSYS_GL,PSUBSYS_GL,GAMMA_GL, &
          &    GAMMA_L_GL,T_INFO%NIONS,T_INFO%NTYP,DYN%ISIF,CNEXP_GL,&
          &    TILAMBDA_GL,TIDOT_GL,LTIDOT_GL,AMASS_GL,LATTICE_CONSTRAINTS_GL,LATTICE_DOF_GL,NHC,DYN%POTIM,FMP_GL)

!c allocated arrays for the use with the Mueller-Plathe method
          IF (FMP_GL%ISUSED) THEN
            ALLOCATE(FMP_GL%SLIMITS(FMP_GL%SLAB_NUMBER), FMP_GL%TSLAB(FMP_GL%SLAB_NUMBER), &
            & FMP_GL%NATSLAB(FMP_GL%SLAB_NUMBER), &
            & FMP_GL%TAT(T_INFO%NIONS),FMP_GL%ATID(T_INFO%NIONS),&
            & FMP_GL%HOTTEST(T_INFO%NTYP), FMP_GL%COOLEST(T_INFO%NTYP), &
            & FMP_GL%THOTTEST(T_INFO%NTYP), FMP_GL%TCOOLEST(T_INFO%NTYP))          

            DO i=1,FMP_GL%SLAB_NUMBER
              FMP_GL%SLIMITS(i)=i*1./FMP_GL%SLAB_NUMBER
            ENDDO  

            FMP_GL%INDEXH=INT(FMP_GL%SLAB_NUMBER/2)+1
            FMP_GL%LINIT=.TRUE.
          ENDIF 
...
!DYN%POSIOC=DYN%POSION
          ...
          IF (counter==1) THEN
!c make sure the initialization has been 1._q
...
!c Andersen thermostat (direct metaDynamics)
            CASE(1)
              ...
              &      hspring_K_GL,hspring_force_GL,fbias_R0_GL,fbias_D_GL,fbias_A_GL,TEIN,fmp_GL)
...
!c Langevin dynamics, velocity verlet (to be replaced by leap-frog)
            CASE(3)
              ...
              ELSE 
                ...
                &      ICOORDINATES_GL%iconst8,hspring_R0_GL,hspring_K_GL,hspring_force_GL,fbias_R0_GL,fbias_D_GL,fbias_A_GL,TEIN,fmp_GL)
              ENDIF
            CASE(4)
            ...
            CASE(5)
              CALL STEP_CSVR(counter,DYN,ICOORDINATES_GL,hills_GL,penalty_GL,T_INFO,INFO,LATT_CUR, &
              &      EKIN,EPS,ES,DISMAX,NDEGREES_OF_FREEDOM,IO,EPOT,CSVR_PERIOD_GL,&
              &      g_io,ICOORDINATES_GL%iconst0,ICOORDINATES_GL%iconst2,ICOORDINATES_GL%iconst4,&
              &      ICOORDINATES_GL%iconst5,ICOORDINATES_GL%iconst8,hspring_R0_GL, & 
              &      hspring_K_GL,hspring_force_GL,fbias_R0_GL,fbias_D_GL,fbias_A_GL,TEIN,fmp_GL)
...
!c and POSION the current (1._q,0._q)
!DYN%POSIOC=DYN%POSION
!IF (IO%IU6>=0) THEN
...
!write(g_io%REPORT,FMT='(3X,A22,X,I6)')    '            SCALING = ' , SCALING
        ...
        SUBROUTINE DYN_READER(IO,g_io,MDALGO,ANDERSEN_PROB,CSVR_PERIOD,&
        &  SCALING,NSUBSYS,TSUBSYS,PSUBSYS,GAMM,&
        &  GAMMA_L,NIONS,NTYP,ISIF,CNEXP,TILAMBDA,TIDOT,LTIDOT,AMASS,LATTICE_CONSTRAINTS, &
        &  LATTICE_DOF,NHC,POTIM,FMP)
!c read general input parameters for MD
          ...
          LOGICAL :: LOPEN,LDUM
          REAL(q) :: RDUM,ANDERSEN_PROB, CSVR_PERIOD
          COMPLEX(q) :: CDUM
          CHARACTER*1 :: CHARAC
          INTEGER :: NSUBSYS(3)
          REAL(q) :: TSUBSYS(3),PSUBSYS(3)
...
!c basic sanity test of MDALGO setting
          ...
          &   (MDALGO==2) .OR. (MDALGO==20) .OR. (MDALGO==3) .OR. (MDALGO==4) .OR. &
          &   (MDALGO==5))) THEN
...
!c CSVR_PERIOD - period of velocity scaling thermostant of Bussi et al.
          IF (MDALGO==5) THEN
            CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'CSVR_PERIOD', CSVR_PERIOD, IERR, WRITEXMLINCAR)
            IF (IERR==3) CSVR_PERIOD = 0.
            IF (CSVR_PERIOD <0._q) CSVR_PERIOD =0._q
          ENDIF
...
!c read in the number of slabs used in Mueller-Plathe method
            CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'FMP_SNUMBER', FMP%SLAB_NUMBER, IERR, WRITEXMLINCAR)
            IF (IERR==0) THEN
              FMP%ISUSED=.TRUE.
              IF ((FMP%SLAB_NUMBER<2) .OR. (MOD(FMP%SLAB_NUMBER,2) /= 0)) THEN
                CALL vtutor%error("FMP_SNUMBER  must an even number")
              ENDIF
...
!c direction of head flow (parallel with (1._q,0._q) of three lattice vectors defined in POSCAR)
              CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'FMP_DIRECTION', FMP%DIRECTION, IERR, WRITEXMLINCAR)
              IF (IERR==3) FMP%DIRECTION=3
              IF ((FMP%DIRECTION<1) .OR.(FMP%DIRECTION>3)) THEN
                CALL vtutor%error("FMP_DIRECTION must an integer from the range between 1 and 3")
              ENDIF


!c period of between swappings (in terms of MD steps)
              CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'FMP_PERIOD', FMP%SWAP_PERIOD, IERR, WRITEXMLINCAR)
              IF (IERR==3) FMP%SWAP_PERIOD=10
              IF (FMP%SWAP_PERIOD<1) FMP%SWAP_PERIOD=10

!c number of swappings performed at the time
              CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'FMP_SWAPNUM', FMP%SWAP_NUM, IERR, WRITEXMLINCAR)
              IF (IERR==3) FMP%SWAP_NUM=1
              IF (FMP%SWAP_NUM<0) FMP%SWAP_NUM=1

!c which atomic species are to be swapped and which not
              CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'FMP_ACTIVE', FMP%ACTIVETYPE, NTYP, IERR, WRITEXMLINCAR)
              IF (IERR==3) FMP%ACTIVETYPE=.TRUE.
...
!c write the input parameter summary into OUTCAR
              ...
              IF (MDALGO==5) THEN
                write(IO%IU6,FMT='(3X,A22,X,F8.3)')         '        CSVR_PERIOD = ',CSVR_PERIOD
              ENDIF
...
!c write the input parameter summary into REPORT
              ...
              IF ((MDALGO==1) .OR. (MDALGO==3)) THEN
                IF (FMP%ISUSED) THEN
                  write(g_io%REPORT,FMT='(3X,A22,X,I3)')     '      FMP_SNUMBER = ', FMP%SLAB_NUMBER
                  write(g_io%REPORT,FMT='(3X,A22,X,I1)')     '    FMP_DIRECTION = ', FMP%DIRECTION
                  write(g_io%REPORT,FMT='(3X,A22,X,I6)')     '       FMP_PERIOD = ', FMP%SWAP_PERIOD
                  write(g_io%REPORT,FMT='(3X,A22,X,I4)')     '      FMP_SWAPNUM = ', FMP%SWAP_NUM
                  write(g_io%REPORT,ADVANCE='NO',FMT='(3X,A22)')  '       FMP_ACTIVE = '
                  DO i=1,NTYP
                    write(g_io%REPORT,ADVANCE='NO',FMT='(X,L1)') FMP%ACTIVETYPE(i)
                  ENDDO
              ...
              IF (MDALGO==5) THEN
                write(g_io%REPORT,FMT='(3X,A22,X,F8.3)') '        CSVR_PERIOD = ',CSVR_PERIOD
              ENDIF
...
!c scaling of step taken in (1._q,0._q) iteration in the SHAKE algorithm
...
!write(g_io%REPORT,FMT='(3X,A4,6(X,E17.8E2))') 'e_b>', ETOTAL,EPOT,EKIN,ESPRING,ECONST,EPS,ES
            ...
            write(g_io%REPORT,FMT='(3X,A4,5(X,E17.8E2))') 'e_b>', ETOTAL,EPOT,EKIN,EPS,ES
...
!c identify number of constrained (non-redundant) degrees of freedom
        ...
        SUBROUTINE STEP_LANGEVIN(counter,DYN,ICOORDINATES,hills,penalty,T_INFO,INFO,LATT_CUR, &
          ...
          &      hspring_R0,hspring_K,hspring_force,fbias_R0,fbias_D,fbias_A,TEIN,fmp)
...
!c the Mueller-Plathe method
          IF (FMP%ISUSED) THEN
            ...
            FMP%TAT(:)=0.
            FMP%ATID=0
!FMP%HOTTEST=0;FMP%COOLEST=0
!FMP%THOTTEST=0.;FMP%TCOOLEST=1e12


!c update instantaneous T of each atom
            CALL TEMPERATURE_ATOMS(T_INFO%NIONS,T_INFO%NTYP,T_INFO%ITYP,T_INFO%POMASS,&
                       & DYN%POTIM,LATT_CUR%A,DYN%VEL,FMP%TAT)

!c determine slabs each atom belons to and
!c number of atoms and T of each slab
            DO i=1,T_INFO%NIONS
              dummy=MODULO(DYN%POSIOC(FMP%DIRECTION,i),1._q)
              DO j=1,FMP%SLAB_NUMBER
                IF (dummy .LT. FMP%SLIMITS(j)) THEN
                  FMP%ATID(i) = j
                  FMP%TSLAB(j) = FMP%TSLAB(j) + FMP%TAT(i)
                  FMP%NATSLAB(j) = FMP%NATSLAB(j) + 1
                  EXIT
                ENDIF
              ENDDO
            ENDDO
            ...
            DO j=1,FMP%SLAB_NUMBER
              IF (FMP%NATSLAB(j)>0) THEN
                FMP%TSLAB(j) = FMP%TSLAB(j)/FMP%NATSLAB(j)
              ENDIF
            ENDDO
            ...
            IF (MOD(counter,DYN%NBLOCK)==0) THEN
              IF (IO%IU6>0) THEN
                ...
                DO i=1,T_INFO%NIONS
                  write(g_io%REPORT,ADVANCE='YES',FMT='(3X,A5,X,I7,X,I7,X,F9.3)') &
                  & 'tat> ',i,FMP%ATID(i) , FMP%TAT(i)
                ENDDO
                ...
                DO j=1,FMP%SLAB_NUMBER
                  write(g_io%REPORT,ADVANCE='YES',FMT='(3X,A5,X,I7,X,I7,X,F9.3)') &
                  & 'tsl> ',j, FMP%NATSLAB(j) , FMP%TSLAB(j)
                ENDDO
...
!c be of the same type
            IF (MOD(counter,FMP%SWAP_PERIOD)==0) THEN
              IF (MOD(counter,DYN%NBLOCK)==0) THEN
                IF (IO%IU6>0) THEN
                  write(g_io%REPORT,ADVANCE='YES',FMT='(X)')
                ENDIF 
              ENDIF
              DO ii=1,FMP%SWAP_NUM
...
!c and vice versa
                DO i=1,T_INFO%NIONS
                  j=T_INFO%ITYP(i)
                  IF (FMP%ATID(i)==FMP%INDEXC) THEN
                    dummy=FMP%TAT(i)
                    IF (dummy>FMP%THOTTEST(j)) THEN
                      FMP%HOTTEST(j) = i
                      FMP%THOTTEST(j)=dummy
                    ENDIF
                  ELSE IF (FMP%ATID(i)==FMP%INDEXH) THEN
                    dummy=FMP%TAT(i)
                    IF (dummy<FMP%TCOOLEST(j)) THEN
                      FMP%COOLEST(j) = i
                      FMP%TCOOLEST(j)=dummy
                    ENDIF
                  ENDIF
                ENDDO
...
!c the biggest impact?
                ...
                jj=0
                DO i=1,T_INFO%NTYP
                  IF ((FMP%ACTIVETYPE(i)) .AND. (FMP%HOTTEST(i)>0) .AND. (FMP%COOLEST(i)>0)) THEN
                    dummy2=FMP%THOTTEST(i) - FMP%TCOOLEST(i)
!write(*,*) "swp",i,dummy2
...
!dummy2=dummy
                ...
                IF (jj>0) THEN
                  IF (MOD(counter,DYN%NBLOCK)==0) THEN
                    IF (IO%IU6>0) THEN
                      write(g_io%REPORT,ADVANCE='YES',FMT='(3X,A21,X,I7,X,I7,X,F12.3,X,F12.3)') &
                      & 'fmp> swapping atoms: ', FMP%HOTTEST(jj),FMP%COOLEST(jj),FMP%THOTTEST(jj), &
                      &  FMP%TCOOLEST(jj)
                    ENDIF
                  ENDIF

!c now swap
                  VEL_TMP(:,1) = DYN%VEL(:,FMP%HOTTEST(jj))
                  DYN%VEL(:,FMP%HOTTEST(jj)) = DYN%VEL(:,FMP%COOLEST(jj))
                  DYN%VEL(:,FMP%COOLEST(jj)) = VEL_TMP(:,1)
...
!c compute ((1._q,0._q) half of)acceleration
          FACT=(DYN%POTIM**2)*EVTOJ/AMTOKG *1E-10_q
...
!c convert NHC momenta in SI to velocities per fs
        ...
        SUBROUTINE STEP_ANDERSEN(counter,DYN,ICOORDINATES,hills,penalty,T_INFO,INFO,LATT_CUR, &
          &      EKIN,EPS,ES,DISMAX,NDEGREES_OF_FREEDOM,IO,EPOT,ANDERSEN_PROB,&
          &      g_io,iconst0,iconst2,iconst4,iconst5,iconst8,hspring_R0, & 
          &      hspring_K,hspring_force,fbias_R0,fbias_D,fbias_A,TEIN,fmp)
...
!c now swap
                  ...
                  VEL_TMP(:,1) = VEL_init(:,FMP%HOTTEST(jj))
                  VEL_init(:,FMP%HOTTEST(jj)) = VEL_init(:,FMP%COOLEST(jj))
                  VEL_init(:,FMP%COOLEST(jj)) = VEL_TMP(:,1)
...
!IF (g_io%REPORT>=0) THEN
        ...
        SUBROUTINE STEP_CSVR(counter,DYN,ICOORDINATES,hills,penalty,T_INFO,INFO,LATT_CUR, &
          &      EKIN,EPS,ES,DISMAX,NDEGREES_OF_FREEDOM,IO,EPOT,CSVR_PERIOD,&
          &      g_io,iconst0,iconst2,iconst4,iconst5,iconst8,hspring_R0, & 
          &      hspring_K,hspring_force,fbias_R0,fbias_D,fbias_A,TEIN,fmp)
!c velocity scaling thermostat by Bussi et al.
          TYPE(coordstructure) :: ICOORDINATES     !c working and reference coords.
          TYPE(dynamics) :: DYN
          TYPE(type_info) :: T_INFO
          TYPE (info_struct) INFO
          ...
          REAL(q) :: C_FORCE(3,T_INFO%NIONS)
          REAL(q) :: EKIN,EPS,ES,DISMAX,TEMPER,EPOT,T_old,T_new,ECONST,TEIN
          INTEGER :: i,j,ii,jj,NDEGREES_OF_FREEDOM
          INTEGER :: counter
          INTEGER :: iconst0,iconst2,iconst4,iconst5,iconst8
          REAL(q) :: dummy,dummy2,BMP,CSVR_PERIOD
          REAL(q) :: R1, sumRi2,alpha
          REAL(q) :: hills_accel(3,T_INFO%NIONS),penalty_accel(3,T_INFO%NIONS),hspring_accel(3,T_INFO%NIONS)
          REAL(q) :: hills_accel_L(3,3),penalty_accel_L(3,3),hspring_accel_L(3,3)
          REAL(q) :: fbias_accel(3,T_INFO%NIONS),fbias_accel_L(3,3)
          REAL(q) :: AMASS,C_FORCE_L(3,3)
!LOGICAL :: LSCALE,LCMASS
          ...
          REAL(q) :: TOTAL_MASS
          INTEGER,SAVE :: Ltxyz(3)
          REAL,SAVE :: expMINUSdtOVERtau, SQRTexpMINUSdtOVERtau

!c determine if coordinates for some direction
!c are frozen
          IF (counter==1) THEN
            expMINUSdtOVERtau = 0._q
            SQRTexpMINUSdtOVERtau = 0._q

            IF (CSVR_PERIOD>0.) THEN
              expMINUSdtOVERtau = EXP(-DYN%POTIM/CSVR_PERIOD)
              SQRTexpMINUSdtOVERtau = expMINUSdtOVERtau**0.5
            ENDIF 
...
!c move a half step in momentum space
          DYN%VEL=DYN%VEL+(DYN%D2C-hills_accel-penalty_accel-hspring_accel)

          IF (CSVR_PERIOD > 0.) THEN
!c determine temperature and rescale velocity
            CALL KINETIC_E(EKIN,T_INFO%NIONS,T_INFO%NTYP,T_INFO%ITYP,T_INFO%POMASS,&
                       & DYN%POTIM,LATT_CUR%A,DYN%VEL)

            TEIN=2*EKIN/BOLKEV/NDEGREES_OF_FREEDOM

            dummy=  DYN%TEMP/TEIN/NDEGREES_OF_FREEDOM
            R1=boltzmann_distribution(0._q,1._q)
            sumRi2=0.

            DO i=2,NDEGREES_OF_FREEDOM
              sumRi2=sumRi2 + (boltzmann_distribution(0._q,1._q))**2
            ENDDO

            alpha= expMINUSdtOVERtau +  & 
            &   dummy * (1. - expMINUSdtOVERtau) * (R1*R1 + sumRi2)
            alpha=alpha +  2 * SQRTexpMINUSdtOVERtau * &
            &  (dummy * (1. - expMINUSdtOVERtau))**0.5 * R1
!alpha = DYN%TEMP/ TEIN
            alpha=alpha**0.5

            DYN%VEL=DYN%VEL * alpha
          ENDIF
...
!c based on eq. 13a-13e in Bond et al., J. Comp. Phys. 151, 114 (1999)
          TYPE(coordstructure) :: ICOORDINATES     !c working and reference coords.
          TYPE(dynamics) :: DYN
          TYPE(type_info) :: T_INFO
          TYPE (info_struct) INFO
...
!DYN%VEL=(1._q-DYN%SNOSE(2)/2._q)*DYN%VEL+DYN%D2C-hills_accel-penalty_accel
!VEL_tmp=DYN%VEL
!!!
...
!c elements for redundant coordinates are (0._q,0._q)
          TYPE(type_info) :: T_INFO
          TYPE(coordstructure) :: ICOORDINATES 
          INTEGER :: IBDIM,i
