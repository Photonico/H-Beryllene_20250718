...
! once when the delta kick is 1._q, and once when the dipole is calculated (both
...
!--------------------------------------------------
  ...
  SUBROUTINE CALCULATE_TIME_PROPAGATION ( &
          ...
          T_INFO, DYN, INFO, XC, IO, KPOINTS, SYMM, &
...
! maybe by updating only selected terms in the Hamiltonian this could be 1._q ???
...
! update charge and then potential
       ...
       CALL SET_KINEDEN (GRID,GRID_SOFT,GRIDC,SOFT_TO_C,LATT_CUR,SYMM, &
         XC%LTAU,T_INFO%NIONS,WT0,WDES,KINEDEN)
...
! c_trans_prop contains all coefficients for (1._q,0._q) direction IDIR
!===============================================================
...
!=======================================================================
   ...
   IF (NSIM_LOCAL*NCPU /= NSIM) THEN
      CALL vtutor%bug("internal ERROR in EDWAV: DETERMINE_HPHI is not correct " // str(NSIM), "time_propagation.F", 980)
   ENDIF
...
! because of parallel version maybe a little bit more needs to be calculated
         ...
         DO NN=1,NBANDSK,NSIM_LOCAL
            ...
            CALL FOCK_ACC(GRID, LMDIM, LATT_CUR, W,   &
                    ...
                    WHPHI%CPTWFP(:,NN:,NK,ISP), P, CQIJ(1,1,1,1), CDCHF, LSYMGRAD=LSYMGRAD)
         ENDDO
      ENDDO
      ENDDO
      IF (LSYMGRAD) THEN
...
! TODO here we calculate too much, CHAM is only required up to NBAND_LAST_FILLED
      ...
      CALL M_sum_z(WDES%COMM_KIN,CHAM(1,NPOS_RED),NB_TOT*NSTRIP_RED)
...
! subtract sum_n2 | phi_n2> < phi_n2 | H_local  | phi_n1 >
      CALL ZGEMM('N', 'N',   WDES1%NPL_RED , NSTRIP_RED, NBAND_LAST_FILLED , -(1._q,0._q), &
     &               WA%CW_RED(1,1),  WDES1%NRPLWV_RED, CHAM(1,NPOS_RED), NB_TOT,  &
...
! subtract sum_n2  Q_ij  < p_j| phi_n2 > < phi_n2 |  H_local | phi_n1 > from orbital character
      IF (W%WDES%LOVERL) THEN
         CALL ZGEMM('N', 'N',  WDES1%NPRO_RED, NSTRIP_RED, NBAND_LAST_FILLED, -(1._q,0._q), &
...
! gradient should be orthogonal to present wavefunction
      ...
      IF (ABS(DORT)>1E-4) THEN
         CALL vtutor%bug("EDWAV: internal error, the gradient is not orthogonal " // str(NK) // " " &
            // str(NP) // " " // str(DORT), "time_propagation.F", 1230)
      ENDIF
!-----------------------------------------------------------------------
! move CF to WSEARCH
!-----------------------------------------------------------------------
...
!----------------------------------------------------------------------
      ...
      DO M=1,WDES1%NPL
         WHPHI%CPTWFP(M,N,NK,ISP)=WSEARCH%CPTWFP(M)*W%FERWE(N,NK,ISP)
      ENDDO
...
! gradient distributed over plane wave coefficients
        CALL REDIS_PW  (WDES1, NBANDS, WHPHI%CPTWFP   (1,1,NK,ISP))
      ENDIF
!=======================================================================
   ENDDO kpoint
   ENDDO spin
...
! work array
   ...
   INTEGER :: NSIM_                ! number of bands 1._q currently
   COMPLEX(q) :: CDCHF             ! double counting correction from HF
!=======================================================================
! number of bands treated simultaneously this must be a multiple of NCPU
!=======================================================================
...
! use W0 for evaluation of fock exchange
               CALL FOCK_ACC(GRID, LMDIM, LATT_CUR, W0,   &
                    NONLR_S, NONL_S, NK, ISP, NN, NNMAX-NN+1, &
                    WHPHI%CPTWFP(:,NN:,NK,ISP), P, CQIJ(1,1,1,1), CDCHF, W1, LSYMGRAD=LSYMGRAD)
            ELSE
               CALL FOCK_ACC(GRID, LMDIM, LATT_CUR, W,   &
                    NONLR_S, NONL_S, NK, ISP, NN, NNMAX-NN+1, &
                    WHPHI%CPTWFP(:,NN:,NK,ISP), P, CQIJ(1,1,1,1), CDCHF, LSYMGRAD=LSYMGRAD)
...
!-----------------------------------------------------------------------
      DO N=NN,NNMAX
         NP=N-NN+1  ! local index
         IF (LHFCALC) THEN
            WHPHI%CPTWFP(:,N,NK,ISP)=WH%CPTWFP(:,NP)+WHPHI%CPTWFP(:,N,NK,ISP)
         ELSE
            WHPHI%CPTWFP(:,N,NK,ISP)=WH%CPTWFP(:,NP)
         ENDIF
...
! so we need to conjugate here (alas why not change CDER above,however, never change a running code)
                C_OCC_UNOCC(NCV13)=CONJG(CDER_BETWEEN_STATE(IDIR))*AUTOA/(2._q*HSQDTM)*(0.0_q,1.0_q)
                IF (LWEIGHT) THEN
    ...
    CALL M_sum_z(W%WDES%COMM_KINTER, C_OCC_UNOCC, SIZE(C_OCC_UNOCC))
...
! local variables
    ...
    REAL(q) :: WEIGHT
    INTEGER NK1_IN_KPOINTS_FULL_ORIG
    COMPLEX(q),ALLOCATABLE,TARGET::  CHAM(:,:)      ! matrix of transition propabilities
    TYPE (wavedes1)    WDES1
    ...
    DO K1=1,W0%WDES%NKPTS
       ...
       WT0%CPTWFP(:,:,K1,ISP)   =W0%CPTWFP(:,:,K1,ISP)
...
! since unoccupied states are not used, not really that important I guess
                CHAM(NB1_INTO_TOT , NB3_INTO_TOT) =-CONJG(C_OCC_UNOCC(NCV13))*DELTA
             ENDIF
          ENDDO  ! enddo for N3-loop
       ENDDO     ! enddo for N1-loop
...
! and normalize the vectors to (1._q,0._q)
! this will yield orthogonal vectors to second order
!       DO N1=1,W0%WDES%NB_TOT
!          CHAM(:,N1)=CHAM(:,N1)/SQRT(REAL(DOT_PRODUCT(CHAM(:,N1),CONJG(CHAM(:,N1))),q))
!       ENDDO
...
! also we need only the occupied orbitals and not the unoccupied ones
       CALL LINCOM('F',WT0%CPTWFP(:,:,K1,ISP),WT0%CPROJ(:,:,K1,ISP),CHAM(1,1), &
            ...
            WT0%CPTWFP(:,:,K1,ISP),WT0%CPROJ(:,:,K1,ISP))
...
!    local
    ...
    TYPE (wavedes1)    WDES1
    INTEGER ISP, K1
    TYPE (wavefuna)    WA             ! pointer towards (1._q,0._q) set of orbitals in W
    TYPE (wavefuna)    WHPHIA           ! pointer towards (1._q,0._q) set of orbitals in WHPHI
...
! currently we calculate the entire matrix
       CALL ORTH2( &
            WA%CPTWFP(1,1), WHPHIA%CPTWFP(1,1), WA%CPROJ(1,1),WA%CPROJ(1,1), &   ! the CPROJ part is not used here
            W0%WDES%NB_TOT, 1, W0%WDES%NB_TOT, WDES1%NPL_RED, 0, WDES1%NRPLWV_RED, WDES1%NPROD_RED, CHAM(1,1))

       CALL M_sum_z(W0%WDES%COMM_KIN,CHAM(1,1),W0%WDES%NB_TOT*W0%WDES%NB_TOT)
...
! occupied orbital N1
    ...
    CALL M_sum_z(W0%WDES%COMM_KINTER, H_OCC_UNOCC, SIZE(H_OCC_UNOCC))
...
! weight is the derivative of the function above
          ...
          DO J=1,NELMGW_ACTUAL
             ...
             EPSILON(I,DIR,DIR)=EPSILON(I,DIR,DIR)-2.0_q*AIMAG(SCALAR(J,DIR))*EXPFAC*TIMESTEP_V(1,J)*WEIGHT
          ENDDO
!epsilon(i,dir)=cone+norm*epsilon(i,dir)
          EPSILON(I,DIR,DIR)=NORM*CONJG(EPSILON(I,DIR,DIR))
          IF (DIR<4) EPSILON(I,DIR,DIR)=(1.0_q,0.0_q)+EPSILON(I,DIR,DIR)
...
!dump dielectric function to XML file
    ...
    PREFIX = 'time-propagation'
    CALL VH5_WRITE_DIELECTRIC_ENERGIES(IH5OUTFILEID, DOMEGA, NEDOS, PREFIX)
