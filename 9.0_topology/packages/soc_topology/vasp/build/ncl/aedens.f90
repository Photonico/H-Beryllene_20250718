...
# 2 "aedens.F" 2 
  ...
  INTERFACE
      SUBROUTINE AUGCHG(WDES, GRID_SOFT,GRIDC_,GRIDUS,C_TO_US, &
        ...
        LMDIM,CRHODE, CHTOT_,CHDEN, IRDMAX ,LPSEUDO, LCORE, IU0)
      USE prec
      USE base
      USE charge
      USE pseudo
...
! read relevant stuff from INCAR
      ...
      LAECHG=.FALSE.     
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LAECHG', LAECHG, IERR, WRITEXMLINCAR)

      ENCUTAE=-1     
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'ENCUTAE', ENCUTAE, IERR, WRITEXMLINCAR)
...
!************************ SUBROUTINE AUGCHG  ***************************
!
!> Construct all-electron densities in
!> real space.
!>
...
!> reciprocal space.
!>
!> If ``LPSEUDO=.TRUE.``: AUGCHG adds the all-electron (1._q,0._q)-center charge
!> density minus the pseudo (1._q,0._q)-center charge density in the PAW
!> spheres to the pseudo charge density on the plane wave grid (CHDEN).
!> The result is stored in CHTOT, in reciprocal space.
!>
!> If ``LPSEUDO= .FALSE.``: AUGCHG adds the all-electron (1._q,0._q)-center charge
!> density minus the pseudo (1._q,0._q)-center charge density and minus the
...
! QIJ = 0.0_q ! Breaks ivdw_CF_ddsc (gnu), multiplied as a whole around line 527.
...
!-----------------------------------------------------------------------
      ...
      RHO_SOFT=RHO0(GRID_SOFT , CHDEN(1,ISP))
      IF( IU0>=0 ) WRITE(*,*) "augmentation electrons", RHO_AUG
      IF( IU0>=0 ) WRITE(*,*) "soft         electrons", RHO_SOFT

      CALL ADD_GRID(GRIDC_,GRID_SOFT,SOFT_TO_C,CHDEN(1,ISP),CHTOT_(1,ISP))

      CALL SETUNB_COMPAT(CHTOT_(1,ISP),GRIDC_)
      RHO_AUG =RHO0(GRIDC_, CHTOT_(1,ISP))
      IF( IU0>=0 ) WRITE(*,*) "total        electrons", RHO_AUG
      ENDDO spin
