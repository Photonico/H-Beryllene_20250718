...
!*************************************************************************************
  SUBROUTINE RND_ORBITALS_MP2(LATT_CUR, W, LMDIM, CQIJ, NONLR_S, NONL_S, P, TAU, TAUWEIGHT, WGW, &
    ...
    TYPE (wavespin) ::  W
    INTEGER :: LMDIM
    COMPLEX(q) CQIJ (LMDIM,LMDIM,W%WDES%NIONS,W%WDES%NCDIJ) !< not used
...
! set root node as the output node
    ...
    CALL M_sum_i(W%WDES%COMM, ROOT_NODE, 1)
...
! copy original orbitals into the occupied wave function array
    WOCC%CPTWFP(:,1:WOCC%WDES%NBANDS,:,:)    = W%CPTWFP(:,1:WOCC%WDES%NBANDS,:,:)
...
! occupied
        ...
        CALL REDIS_PW(WDESK, WOCC%WDES%NBANDS, WOCC%CPTWFP(1, 1, NK, I))
        CALL REDIS_PROJ(WDESK, WOCC%WDES%NBANDS, WOCC%CPROJ(1, 1, NK, I))
! unoccupied
        CALL SETWDES(W%WDES, WDESK, NK)            
        CALL REDIS_PW(WDESK, W%WDES%NBANDS, W%CPTWFP(1, 1, NK, I))
...
!   (2) the second argument of INIT_RND_GENERATOR is the same for all nodes in COMM_KIN
    ...
    CALL M_sum_i(W%WDES%COMM, SYS_CLOCK, 1)
...
! dump memory usage
            ...
            CALL MPI_GATHER(DMP2_SAMPLES, SIZE(DMP2_SAMPLES), MPI_double_complex, DMP2_SAMPLES_GATH, SIZE(DMP2_SAMPLES), MPI_double_complex, ROOT_NODE-1, W%WDES%COMM%MPI_COMM, IERR)
            IF(IERR > 0 .AND. IU0>0) THEN
              CALL vtutor%error("ERROR in RND_ORBITALS_MP2: MPI_GATHER failed")
            ENDIF

            CALL MPI_GATHER(XMP2_SAMPLES, SIZE(XMP2_SAMPLES), MPI_double_complex, XMP2_SAMPLES_GATH, SIZE(XMP2_SAMPLES), MPI_double_complex, ROOT_NODE-1, W%WDES%COMM%MPI_COMM, IERR)
            IF(IERR > 0 .AND. IU0>0) THEN
              CALL vtutor%error("ERROR in RND_ORBITALS_MP2: MPI_GATHER failed")
            ENDIF
...
! check if accuracy is reached
            ...
            ELSE
! all other mpi ranks should have (0._q,0._q) otherwise M_sum_i produces nonsense
              REACHED_DI_ACCURACY = 0
              REACHED_EX_ACCURACY = 0
            ENDIF

            CALL M_sum_i(W%WDES%COMM, REACHED_DI_ACCURACY, 1)
            CALL M_sum_i(W%WDES%COMM, REACHED_EX_ACCURACY, 1)
...
! local
    ...
    INTEGER ITHETA, I, A
    INTEGER :: ISP, K1, K2
    COMPLEX(q), ALLOCATABLE :: RND_MAT_OCC1(:,:), RND_MAT_VIRT1(:,:) ! random matrices
    COMPLEX(q), ALLOCATABLE :: RND_MAT_OCC2(:,:), RND_MAT_VIRT2(:,:) ! random matrices
...
! allocate memory for the random matrices
    ...
    CALL REGISTER_ALLOCATE(16.0_q /  (2*SIZE(RND_MAT_OCC1)+2*SIZE(RND_MAT_VIRT1)), "rand_mat")
...
! calculate random occupied orbitals |kappa> and |lambda>: WRNDI%CPTWFP = WI%CPTWFP * RND_MAT_OCC
    CALL ZGEMM('N', 'N',  WDESK1%NPL_RED, WRNDOCC1%WDES%NB_TOT, WDESK1%NB_TOT, (1._q,0._q), WI%CW_RED(1,1), &
                WDESK1%NRPLWV_RED, RND_MAT_OCC1, NB_OCC, (0._q,0._q), WRNDI1%CW_RED(1,1),  WDESK1%NRPLWV_RED)
    CALL ZGEMM('N', 'N', WDESK1%NPRO_RED, WRNDOCC1%WDES%NB_TOT, WDESK1%NB_TOT, (1._q,0._q), WI%CPROJ_RED(1,1), &
               WDESK1%NPROD_RED, RND_MAT_OCC1, NB_OCC, (0._q,0._q), WRNDI1%CPROJ_RED(1,1), WDESK1%NPROD_RED)

    CALL ZGEMM('N', 'N',  WDESK1%NPL_RED, WRNDOCC2%WDES%NB_TOT, WDESK1%NB_TOT, (1._q,0._q), WI%CW_RED(1,1), &
                WDESK1%NRPLWV_RED, RND_MAT_OCC2, NB_OCC, (0._q,0._q), WRNDI2%CW_RED(1,1),  WDESK1%NRPLWV_RED)
    CALL ZGEMM('N', 'N', WDESK1%NPRO_RED, WRNDOCC2%WDES%NB_TOT, WDESK1%NB_TOT, (1._q,0._q), WI%CPROJ_RED(1,1), &
               WDESK1%NPROD_RED, RND_MAT_OCC2, NB_OCC, (0._q,0._q), WRNDI2%CPROJ_RED(1,1), WDESK1%NPROD_RED)

! calculate random virtual orbitals |alpha> and |beta>: WRNDA%CPTWFP = WA%CPTWFP * RND_MAT_VIRT
    CALL ZGEMM('N', 'N',  WDESK2%NPL_RED, WRNDVIRT1%WDES%NB_TOT, WDESK2%NB_TOT, (1._q,0._q), WA%CW_RED(1,1), &
                WDESK2%NRPLWV_RED, RND_MAT_VIRT1, NB_ALL, (0._q,0._q), WRNDA1%CW_RED(1,1),  WDESK2%NRPLWV_RED)
    CALL ZGEMM('N', 'N', WDESK2%NPRO_RED, WRNDVIRT1%WDES%NB_TOT, WDESK2%NB_TOT, (1._q,0._q), WA%CPROJ_RED(1,1), &
               WDESK2%NPROD_RED, RND_MAT_VIRT1, NB_ALL, (0._q,0._q), WRNDA1%CPROJ_RED(1,1), WDESK2%NPROD_RED)

    CALL ZGEMM('N', 'N',  WDESK2%NPL_RED, WRNDVIRT2%WDES%NB_TOT, WDESK2%NB_TOT, (1._q,0._q), WA%CW_RED(1,1), &
                WDESK2%NRPLWV_RED, RND_MAT_VIRT2, NB_ALL, (0._q,0._q), WRNDA2%CW_RED(1,1),  WDESK2%NRPLWV_RED)
    CALL ZGEMM('N', 'N', WDESK2%NPRO_RED, WRNDVIRT2%WDES%NB_TOT, WDESK2%NB_TOT, (1._q,0._q), WA%CPROJ_RED(1,1), &
               WDESK2%NPROD_RED, RND_MAT_VIRT2, NB_ALL, (0._q,0._q), WRNDA2%CPROJ_RED(1,1), WDESK2%NPROD_RED)
...
! do the same for the stochastic virtuals with the lower number of NBANDS (if desired)
    IF( NB_LOW > 0) THEN
      ...
      CALL ZGEMM('N', 'N',  WDESK2%NPL_RED, WRNDVIRT1_LOW%WDES%NB_TOT, NB_LOW, (1._q,0._q), WA%CW_RED(1,1), &
                  WDESK2%NRPLWV_RED, RND_MAT_VIRT1, NB_ALL, (0._q,0._q), WRNDA1_LOW%CW_RED(1,1),  WDESK2%NRPLWV_RED)
      CALL ZGEMM('N', 'N', WDESK2%NPRO_RED, WRNDVIRT1_LOW%WDES%NB_TOT, NB_LOW, (1._q,0._q), WA%CPROJ_RED(1,1), &
                 WDESK2%NPROD_RED, RND_MAT_VIRT1, NB_ALL, (0._q,0._q), WRNDA1_LOW%CPROJ_RED(1,1), WDESK2%NPROD_RED)
      CALL ZGEMM('N', 'N',  WDESK2%NPL_RED, WRNDVIRT2_LOW%WDES%NB_TOT, NB_LOW, (1._q,0._q), WA%CW_RED(1,1), &
                  WDESK2%NRPLWV_RED, RND_MAT_VIRT2, NB_ALL, (0._q,0._q), WRNDA2_LOW%CW_RED(1,1),  WDESK2%NRPLWV_RED)
      CALL ZGEMM('N', 'N', WDESK2%NPRO_RED, WRNDVIRT2_LOW%WDES%NB_TOT, NB_LOW, (1._q,0._q), WA%CPROJ_RED(1,1), &
                 WDESK2%NPROD_RED, RND_MAT_VIRT2, NB_ALL, (0._q,0._q), WRNDA2_LOW%CPROJ_RED(1,1), WDESK2%NPROD_RED)
# 1115
...
! local
    ...
    TYPE (wavedes1), TARGET :: WDESK             ! for gamma only we need only (1._q,0._q) wavedes1
    INTEGER :: K, ISP
    INTEGER :: N, T, T1, T2
    REAL(q) :: FSG
    REAL(q) :: POTFAK(WGWQ%NPL, N_MP2_ENCUTS)
    COMPLEX(q), ALLOCATABLE :: GWORK(:,:)              ! work array for overlap density
...
! allocate work arrays and overlap densities
    ALLOCATE( GWORK( GRIDHF%MPLWV, NCORREL_THETA), CRHOLM(AUG_DES%NPROD*WHF%WDES%NRSPINORS, NCORREL_THETA), &
