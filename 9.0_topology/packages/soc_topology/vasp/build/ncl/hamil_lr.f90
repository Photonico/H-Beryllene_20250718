...
!$ACC ENTER DATA CREATE(WDES1,W0_1(:),WTMP(:),CF,CPROJ,DS,ORTH,FNORM) 
      LD=WDES%NRPLWV
...
!$ACC KERNELS PRESENT(CRHODE,WXI) 
      CRHODE=0
      WXI%CPTWFP=0
      WXI%CPROJ=0
!$ACC END KERNELS
...
! not very clean but these descriptors use same phase factors as NONLR_S
        NONLR_ION%NK=NK
        NONLR_IOND%NK=NK
!$ACC UPDATE DEVICE(NONLR_ION%NK,NONLR_IOND%NK) 
      ELSE
...
# 255
           ...
           CALL M_sum_2(WDES%COMM_INB, FNORM_, ORTH_)

           FNORM(NP)=FNORM_ ; ORTH(NP)=ORTH_

           CALL ZCOPY(WDES%NPROD,WTMP(NP)%CPROJ(1),1,WXI%CPROJ(1,N,NK,ISP),1)
!!!$ACC KERNELS PRESENT(WXI,W0_1(NP),DS(NP)) 
!!           WXI%CPROJ(:,N,NK,ISP)=WTMP(NP)%CPROJ
!$ACC KERNELS PRESENT(WXI,W0_1,DS) 
           WXI%CELEN(N,NK,ISP)=W0_1(NP)%CELEN
           WXI%FERWE(N,NK,ISP)=DS(NP)
!$ACC END KERNELS

!!!$ACC EXIT DATA DELETE(WTMP(NP)%CPTWFP,WTMP(NP)%CPROJ,WTMP(NP)%WDES1) 
# 287
...
!$ACC UPDATE SELF(W0_1(NP)%CELEN,WXI%CELEN(N,NK,ISP),WXI%FERWE(N,NK,ISP)) IF(OFFLOAD_ON)
...
!! ACC_ASYNC_Q=ACC_ASYNC_ASYNC
      ...
      ENDDO
!$ACC EXIT DATA DELETE(W0_1(:),WTMP(:),CF,CPROJ,DS,ORTH,FNORM) 
      DEALLOCATE(CF, CPROJ)
...
!$ACC END KERNELS
               ...
               IF (LSUBTRACT) THEN
!$ACC KERNELS PRESENT(EVALUE1,W0_1) 
...
!$ACC KERNELS PRESENT(CH) 
               CH(:,NP)=0
!$ACC END KERNELS
!!! test_
!!!$ACC ENTER DATA COPYIN(EVALUE0_(NP)) 
!!! test_
...
!$ACC ENTER DATA COPYIN(EVALUE0(NP) ) 
...
!$ACC EXIT DATA DELETE(EVALUE0(NP) ) 
...
!$ACC EXIT DATA DELETE(EVALUE0_(NP)) 
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CRHODE,W0_1,WTMP) 
                        DO L=1,LMMAXC
!DIR$ IVDEP
!OCL NOVREC
                        DO LP=1,LMMAXC
...
!$ACC EXIT DATA DELETE(EVALUE1,CTMP,CWORK1) 
      DEALLOCATE(CWORK1)
