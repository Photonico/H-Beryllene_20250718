...
# 2 "constrmag.F" 2 
      ...
      REAL(q), PRIVATE, PARAMETER :: TINY=1E-5_q, MAGMIN=1E-8_q
...
!=======================================================================
      ...
      INTEGER IU0,IU5,IERR
      INTEGER NMCONSTR,NI,N
      LOGICAL :: LOPEN,LDUM
      REAL(q) :: MNORM
      REAL(q) :: ALPHA,BETA
      REAL(q) :: QR,M_x,M_y
      ...
      I_CONSTRAINED_M=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'I_CONSTRAINED_M', I_CONSTRAINED_M, IERR, WRITEXMLINCAR)
! if I_CONSTRAINED_M<>0 we also need M_CONSTR and (possibly) LAMBDA
      IF (I_CONSTRAINED_M>0) THEN
         ...
         M_CONSTR=0
! ... get constraints
         IF (I_CONSTRAINED_M==1 .OR. I_CONSTRAINED_M==2 .OR. I_CONSTRAINED_M==4) THEN
! constraints are read in as vectors
            CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'M_CONSTR', AM_CONSTR, NMCONSTR, IERR, WRITEXMLINCAR)

            DO NI=1,NIONS
               M_CONSTR(1,NI)=AM_CONSTR(3*(NI-1)+1)
               M_CONSTR(2,NI)=AM_CONSTR(3*(NI-1)+2)
               M_CONSTR(3,NI)=AM_CONSTR(3*(NI-1)+3)
               IF (I_CONSTRAINED_M==1 .OR. I_CONSTRAINED_M==4) THEN
...
! constraints are read in as angles
            CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'M_CONSTR', AM_CONSTR, 2*NIONS, IERR, WRITEXMLINCAR)
...
! if I_CONSTRAINED_M<>0 we also need M_CONSTR and (possibly) LAMBDA
      ...
      CALL XML_INCAR_V('M_CONSTR','F',IDUM,M_CONSTR,CDUM,LDUM,CHARAC,NIONS_*3)
      ...
      INTEGER N1, N2, N3, N1P, N2P, N3P, NI, NCOL
      INTEGER IND, INDMAX, ISP
      REAL(q) F1, F2, F3, D1, D2, D3, X1, X2, X3, XC, YC, ZC, D, QR, BJ
...
!=======================================================================
      IF (I_CONSTRAINED_M==1.OR.I_CONSTRAINED_M==2.OR.I_CONSTRAINED_M==3.OR.I_CONSTRAINED_M==4) THEN
!=======================================================================
      NIS=1
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("STEPFNC_ALLOC: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // &
           str(N1P+1) // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "constrmag.F", 236)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
...
!***********************************************************************
      ...
      INTEGER NIS, NT, NI, NCOL
      INTEGER ISP, IND
      REAL(q) RINPL, QR, SUM, SUM_WEIGHTED
...
!***********************************************************************
      ...
      INTEGER I, ISP, IND, NT, NIS, NI
      REAL(q) MW_,MW_X,MW_Y,MW_IN_M_CONSTR,QR,MWM
...
! M_x
      ...
      IF (I_CONSTRAINED_M==4) THEN
         IF(MWM > MAGMIN) THEN
           MW_X= MW(1,NI)/MWM - M_CONSTR(1,NI)
           MW_Y= MW(2,NI)/MWM - M_CONSTR(2,NI)
         ELSE
           MW_X=0._q
           MW_Y=0._q
         ENDIF
      ...
      DO IND=1,NLIMAX(NI)
         ...
         ELSEIF (I_CONSTRAINED_M==4) THEN
           CVTOT(NLI(IND,NI),ISP)=CVTOT(NLI(IND,NI),ISP)+ &
              LAMBDA*WEIGHT(IND,NI)*(MW_X*COS(QR)-MW_Y*SIN(QR))
         ENDIF
      ENDDO
! M_y
      CASE(3)
      ...
      DO IND=1,NLIMAX(NI)
         ...
         ELSEIF (I_CONSTRAINED_M==4) THEN
           CVTOT(NLI(IND,NI),ISP)=CVTOT(NLI(IND,NI),ISP)+ &
              LAMBDA*WEIGHT(IND,NI)*(MW_Y*COS(QR)+MW_X*SIN(QR))
         ENDIF
      ENDDO
! M_z
      CASE(4)
...
!***********************************************************************
      ...
      INTEGER I, NI
      REAL(q) E_CONSTRAINT,MW_(3),MW_IN_M_CONSTR,MWM
...
! Add penalty energy
      ...
      IF (I_CONSTRAINED_M==4) THEN
!E_CONSTRAINT= 0._q
         E_PENALTY=E_PENALTY+LAMBDA*(MWM-MW_IN_M_CONSTR)
         E_EXPECT=E_EXPECT+LAMBDA*(MWM-MW_IN_M_CONSTR)
      ENDIF
      ENDDO ions
...
!***********************************************************************
      ...
      INTEGER IU, NI, I
      REAL(q) MW_(3),MW_IN_M_CONSTR,MWM
      ...
      IF (LONG) THEN
         ...
         DO NI=1,NIONS
            ...
            IF (I_CONSTRAINED_M==1.OR.I_CONSTRAINED_M==3) THEN
               DO I=1,3
                  MW_(I)=2*(MW(I,NI)-M_CONSTR(I,NI)*MW_IN_M_CONSTR)
               ENDDO
            ENDIF

            IF (I_CONSTRAINED_M==2) THEN
               DO I=1,3
                  MW_(I)=2*(MW(I,NI)-M_CONSTR(I,NI))
               ENDDO
            ENDIF

            IF (I_CONSTRAINED_M==4) THEN
               MWM=SQRT(MW(1,NI)*MW(1,NI)+MW(2,NI)*MW(2,NI)+MW(3,NI)*MW(3,NI))
               IF (MWM > MAGMIN) THEN
                  DO I=1,3
                     MW_(I)=MW(I,NI)/MWM-M_CONSTR(I,NI)
                  ENDDO
               ELSE
                  MW_=0._q
               ENDIF
      ...
      USE lattice
      USE constant
      USE tutor, ONLY: vtutor, isWarning, newArgument, SpinSpiral
      IMPLICIT NONE
      TYPE (wavedes) WDES
     ...
     &       (WDES%QSPIRAL(3)/2)*LATT_CUR%B(3,3))
      ...
      CALL vtutor%write(isWarning, SpinSpiral, newArgument(rval = RTUT))
