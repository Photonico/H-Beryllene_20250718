...
module scpc
   ...
   use pot_struct_def
   use pot_electrostat, only: potion, pothar
...
!> Apply the SCPC to the potential
   subroutine scpc_apply(gridc, latt_cur, coulomb_pot, chtot, p, t_info, cstrf, &
      cvtot, escpc)
      ...
      CALL M_bcast_i(gridc%comm, scpc_set%ntarget, 1)
      CALL M_bcast_l(gridc%comm, scpc_set%active, 1)
      CALL M_barrier(gridc%comm)
      ...
      if (ncount >= scpc_set%ntarget) then
         ...
         call scpc_build_model(gridc, p, latt_cur, t_info, coulomb_pot, chtot, cstrf)
...
! reduce grid
         ...
         call scpc_mltscale(w1, nx, ny, nz, dnx, dny, dnz, &
                            w2, mx, my, mz, dmx, dmy, dmz)
! setting the average potential to (0._q,0._q)
         r2 = sum(w2) / dble(scpc_set%mxyz)
         w2 = w2 - r2
...
!
   ...
   subroutine scpc_build_diel(gridc)
      ...
      IF (NODE_ME==IONODE) THEN
         ...
         if ((scpc_set%zlow == 0._q).and.(scpc_set%zhig == 1._q)) then
            scpc_set%meps = scpc_set%diel
            scpc_set%mdeps = 0._q
            scpc_set%isbulk = .true.
         else
            beta = 1._q / scpc_set%broad
            t0 = scpc_set%zlow*scpc_set%clat
            t1 = scpc_set%zhig*scpc_set%clat
            t2 = 1._q / scpc_set%diel
            z  = 0.0_q
            do iz = 1, scpc_set%mgz
               r0 = 1._q + erf((z - t0) * beta)
               r1 = 1._q + erf((t1 - z) * beta)
               r2 = c0 * (t2 - 1._q) * r0 * r1
               r2 = 1._q + r2
               scpc_set%meps(:,:,iz) = 1._q / r2
               z = z + scpc_set%dmz
            end do
         ...
         if ((scpc_set%prtz).and.(scpc_set%zlow /= 0._q).and.(scpc_set%zhig /= 1._q)) then
   ...
   subroutine scpc_build_model(gridc, p, latt_cur, t_info, coulomb_pot, chtot, cstrf)
      type(grid_3d), intent(in) :: gridc
      type(potcar), intent(in) :: p(:)
      type(latt), intent(in) :: latt_cur
      type(type_info), intent(in) :: t_info
      ...
      call scpc_get_vhar(gridc, p, latt_cur, t_info, coulomb_pot, chtot, cstrf, scpc_set%cvhar)
...
!scpc_set%mchg = mixing*scpc_set%mchg + (1.0_q-mixing)*rhosave
   ...
   subroutine scpc_get_vhar(gridc, p, latt_cur, t_info, coulomb_pot, chtot, cstrf, cvhar)
      type(grid_3d), intent(in) :: gridc
      type(latt), intent(in) :: latt_cur
      type(potcar), intent(in) :: p(:)
      type(type_info), intent(in) :: t_info
...
! hartree potential
      cwork = 0.0_q
      call pothar(gridc, latt_cur, coulomb_pot, chtot, cwork, rdum)
      cvhar = cwork

! local ionic potential
      cwork = 0.0_q
      call potion(gridc, p, latt_cur, t_info, coulomb_pot, cwork, cstrf, rdum)
      cvhar = cvhar + cwork
...
!write(iout,'(/,2X,a,f8.4,a)') "Alignment term", ealig, " Ev"
      ...
      CALL M_barrier(gridc%comm)
      CALL M_bcast_d(gridc%comm, scpc_set%ecor_tot, 1)
