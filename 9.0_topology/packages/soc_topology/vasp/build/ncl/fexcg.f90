...
!! USE moffload
      ...
      COMPLEX(q) DMUWORK(NPMU,NCDIJ)
      COMPLEX(q), ALLOCATABLE :: DWORK5(:),DWORK5S(:),TAUC(:)
      COMPLEX(q), POINTER :: DWGRAD(:,:) => NULL()
      COMPLEX(q), POINTER :: DHTOT(:,:) => NULL()
      COMPLEX(q), POINTER :: DWORK(:,:) => NULL()
      COMPLEX(q), POINTER :: DKINEDEN(:,:) => NULL()
...
!$ACC ENTER DATA CREATE(CWGRAD,DWORKG,DWORK1,DWORK2,DWORK3,DVC,DWORK4,DWORK5,DWORK6,DWORK7) 

      IF (XC%LSPIN_VDW) THEN
         ALLOCATE(DWORK4S(NP4),DWORK5S(NPVDW),DWORK6S(NPVDW),DWORK7S(NPVDW))
!$ACC ENTER DATA CREATE(DWORK4S,DWORK5S,DWORK6S,DWORK7S) 
      ENDIF

      IF (XC%LTAU) THEN
         ALLOCATE(CKINEDEN(GRIDC%MPLWV,NCDIJ),TAUC(NP1))
!$ACC ENTER DATA CREATE(KINEDEN,CKINEDEN,TAUC) 
!$ACC KERNELS PRESENT(CKINEDEN,TAUC) 
...
!$ACC ENTER DATA CREATE(LAPLACIAN) 
!$ACC KERNELS PRESENT(LAPLACIAN) 
         LAPLACIAN=0._q
!$ACC END KERNELS
      ENDIF
...
!$ACC ENTER DATA CREATE(DWORKL,DENSHESS) 
!$ACC KERNELS PRESENT(DWORKL,DENSHESS) 
         DWORKL=0
         DENSHESS=0
!$ACC END KERNELS
      ENDIF
...
! set CWORK to total real charge in reciprocal space
 ...
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK,DENCOR,DHTOT,RINPL,LATT_CUR,NCDIJ,ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK,DENCOR,DHTOT,LATT_CUR) 
         DO I=1,GRIDC%RL%NP
...
! Add core charge density to pseudo charge density
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK,DENCOR,DHTOT,RINPL,LATT_CUR,ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK,DENCOR,DHTOT,LATT_CUR) 
            DO I=1,GRIDC%RL%NP
 ...
 !$OMP END PARALLEL DO
         ELSE
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK,DHTOT,RINPL,LATT_CUR,ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK,DHTOT,LATT_CUR) 
            DO I=1,GRIDC%RL%NP
...
!=======================================================================
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,CWGRAD,CWORK,ISP)
!$ACC PARALLEL LOOP PRESENT(CWGRAD,CWORK) 
      DO  I=1,GRIDC%RC%NP
...
!OCL NOVREC
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GX) SHARED(GRIDC,LATT_CUR,CWORK,ISP)
...
! grad_x in real space:
      ...
      CALL OPSYNC(DWORK(1,ISP),CWORK(1,ISP),GRIDC%NPLWV)
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK1,DWORK,ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK1,DWORK) 
      DO I=1,GRIDC%RL%NP
...
! y-component:
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,CWORK,CWGRAD,ISP)
!$ACC PARALLEL LOOP PRESENT(CWGRAD,CWORK) 
      DO  I=1,GRIDC%RC%NP
...
!OCL NOVREC
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GY) SHARED(GRIDC,LATT_CUR,CWORK,ISP)
...
! grad_y in real space:
      ...
      CALL OPSYNC(DWORK(1,ISP),CWORK(1,ISP),GRIDC%NPLWV)
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK2,DWORK,ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK2,DWORK) 
      DO I=1,GRIDC%RL%NP
...
!OCL NOVREC
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GZ) SHARED(GRIDC,LATT_CUR,CWORK,ISP)
...
! grad_z in real space:
      ...
      CALL OPSYNC(DWORK(1,ISP),CWORK(1,ISP),GRIDC%NPLWV)
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK3,DWORK,ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK3,DWORK) 
      DO I=1,GRIDC%RL%NP
...
! indices for density hessian: 1 = xx, 2 = xy, 3 = xz, 4 = yy, 5 = yz, 6 = zz
          ...
          DO IDIR=1,3
            DO JDIR = IDIR,3
              IDH = IDH + 1
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GI, GJ) SHARED(GRIDC,LATT_CUR,CWORK,CWGRAD,ISP,IDIR,JDIR)
!$ACC PARALLEL LOOP PRESENT(GRIDC,LATT_CUR,CWGRAD,CWORK) PRIVATE(N1,N2,N3,NC,GI,GJ) 
              DO I=1,GRIDC%RC%NP
                N1= MOD((I-1),GRIDC%RC%NROW) +1
                NC= (I-1)/GRIDC%RC%NROW+1
                N2= GRIDC%RC%I2(NC)
 ...
 !$OMP END PARALLEL DO
              ...
              CALL OPSYNC(DWORK(1,ISP),CWORK(1,ISP),GRIDC%NPLWV)
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DENSHESS,DWORK,ISP,IDH)
!$ACC PARALLEL LOOP PRESENT(DENSHESS,DWORK) 
              DO I=1,GRIDC%RL%NP
 ...
 !$OMP END PARALLEL DO
          ...
          ENDDO
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,LAPLACIAN,DENSHESS,ISP)
!$ACC PARALLEL LOOP PRESENT(LAPLACIAN,DENSHESS) 
          DO I=1,GRIDC%RL%NP
 ...
 !$OMP END PARALLEL DO
        ELSE
          ...
          DO IDIR=1,3
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GG) SHARED(GRIDC,LATT_CUR,CWORK,CWGRAD,ISP,IDIR)
!$ACC PARALLEL LOOP PRESENT(GRIDC,CWORK,LATT_CUR,CWGRAD) PRIVATE(N1,N2,N3,NC,GG) 
...
!write(*,*)'xcgrad:cwgrad ', CWORK4(I), CWGRAD(I), G1, G2, CITPI
 ...
 !$OMP END PARALLEL DO
            ...
            CALL OPSYNC(DWORK(1,ISP),CWORK(1,ISP),GRIDC%NPLWV)
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,LAPLACIAN,DWORK,ISP)
!$ACC PARALLEL LOOP PRESENT(LAPLACIAN,DWORK) 
            DO I=1,GRIDC%RL%NP
...
!$ACC KERNELS PRESENT(DWORK,DVC,DWORKG) 
...
!$ACC END KERNELS
 ...
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP SHARED(GRIDC,NCDIJ,CHGMIN,NABMIN,LAPMIN,TAUMIN,IVDW,XDM,DMUWORK,DVXCFAC,DHTOT,DENCOR,LATT_CUR,XC,EVTOH,DWORK1,DWORK2,DWORK3,DWORK4,DWORK4S,DWORK7,DWORK7S,TAUC,DKINEDEN,DWORK,DWORKG,DVC,LAPLACIAN,DWORKL) &
 !$OMP PRIVATE(I,MAG_NORM,MAG_NORM_I,RHO1,RHO2,NABMAG,ABSNABUP,ABSNABDW,ABSNAB,TAU_NORM,TAU_DOT_MDIR,TAUU,TAUD,LAP_NORM,LAP_DOT_MDIR,LAPLUP,LAPLDW,EXCL,DEXC1,DEXC2,DVXC1,DVXC2,DVC_,DLAPU,DLAPD,MUUP,MUDW,RHO,TXDM) &
 !$OMP FIRSTPRIVATE(CVMBJ) &
 !$OMP REDUCTION(+:EXC,EDCMU)
...
!$ACC FIRSTPRIVATE(CVMBJ) REDUCTION(+:EXC,EDCMU) 
      DO I=1,GRIDC%RL%NP
...
!
 ...
 !$OMP END PARALLEL DO
      ...
      IF (XC%LLAP) THEN
!$ACC EXIT DATA DELETE(LAPLACIAN) 
         DEALLOCATE(LAPLACIAN)
      ENDIF
!! CALL VTUTOR_OFFLOAD_CHECK()
...
!$ACC END KERNELS
            ENDIF
         ELSEIF ((NCDIJ==2).OR.(NCDIJ==4)) THEN
!$ACC KERNELS PRESENT(DWORK5,DWORK6,DWORK5S,DWORK6S) 
            DWORK5=0._q
            DWORK6=0._q
            IF (XC%LSPIN_VDW) THEN
               DWORK5S=0._q
...
!$ACC END KERNELS
            ...
            IF (XC%LSPIN_VDW) THEN
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK,DWORK5,DWORK5S,DWORK1,DWORK2,DWORK3)
!$ACC PARALLEL PRESENT(DWORK,DWORK5,DWORK5S,DWORK1,DWORK2,DWORK3,DWORKG,DWORK6,DWORK6S) 
!$ACC LOOP GANG VECTOR
 ...
 !$OMP END PARALLEL DO
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORKG,DWORK6,DWORK6S)
!$ACC LOOP GANG VECTOR
               DO I=1,GRIDC%RL%NP
...
!$ACC END PARALLEL
            ELSE
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORK,DWORK5,DWORK1,DWORK2,DWORK3)
!$ACC PARALLEL PRESENT(DWORK,DWORK5,DWORK1,DWORK2,DWORK3,DWORKG,DWORK6) 
!$ACC LOOP GANG VECTOR
               DO I=1,GRIDC%RL%NP
                  DWORK(I,1)=DWORK(I,1)+DWORK5(I)/ MAX(SQRT(DWORK1(I,1)*DWORK1(I,1)+DWORK2(I,1)*DWORK2(I,1) &
              &             +DWORK3(I,1)*DWORK3(I,1)),1.E-10_q)
 ...
 !$OMP END PARALLEL DO
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC,DWORKG,DWORK6)
!$ACC LOOP GANG VECTOR
               DO I=1,GRIDC%RL%NP
                  DWORKG(I,1)=DWORKG(I,1)+DWORK6(I)
...
!=======================================================================
      ...
      IF (NCDIJ==1) THEN
         IF (XC%LDOMETAGGA) THEN
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORK1, DWORK2, DWORK3, DWORK, DVC) &
 !$OMP REDUCTION(+: SIF11, SIF22, SIF33, SIF12, SIF23, SIF31)
...
!$ACC REDUCTION(+:SIF11,SIF22,SIF33,SIF12,SIF23,SIF31) 
 ...
 !$OMP END PARALLEL DO
         ELSE
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORK1, DWORK2, DWORK3, DWORK) &
 !$OMP REDUCTION(+: SIF11, SIF22, SIF33, SIF12, SIF23, SIF31)
...
!$ACC REDUCTION(+:SIF11,SIF22,SIF33,SIF12,SIF23,SIF31) 
 ...
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORK1, DWORK2, DWORK3, DWORK, DVC, ISP) &
 !$OMP REDUCTION(+: SIF11, SIF22, SIF33, SIF12, SIF23, SIF31)
         DO I=1,GRIDC%RL%NP
...
! waith here for device offloading since the addition is 1._q on the host
...
!=======================================================================
      ...
      IF (NCDIJ==1) THEN
         IF (XC%LDOMETAGGA) THEN
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I,ANAB1U,ANAB2U,ANAB3U,ANAB1D,ANAB2D,ANAB3D) SHARED(GRIDC,DWORK1,DWORK2,DWORK3,DWORK,DVC)
!$ACC PARALLEL LOOP PRESENT(DWORK1,DWORK2,DWORK3,DWORK,DVC) &
!$ACC& PRIVATE(ANAB1U,ANAB2U,ANAB3U,ANAB1D,ANAB2D,ANAB3D) 
            DO I=1,GRIDC%RL%NP
...
!OCL NOVREC
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORK, DWORK1, ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK,DWORK1) 
      DO I=1,GRIDC%RL%NP
 ...
 !$OMP END PARALLEL DO
      ...
      CALL TRUNC_HIGH_FREQU(LATT_CUR, GRIDC, CWORK(1,ISP))
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GX) SHARED(GRIDC, LATT_CUR, CWGRAD, CWORK, ISP)
...
! y-component:
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORK, DWORK2, ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK,DWORK2) 
      DO I=1,GRIDC%RL%NP
 ...
 !$OMP END PARALLEL DO
      ...
      CALL TRUNC_HIGH_FREQU(LATT_CUR, GRIDC, CWORK(1,ISP))
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GY) SHARED(GRIDC, LATT_CUR, CWGRAD, CWORK, ISP)
...
! z-component:
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORK, DWORK3, ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK,DWORK3) 
      DO I=1,GRIDC%RL%NP
 ...
 !$OMP END PARALLEL DO
      ...
      CALL TRUNC_HIGH_FREQU(LATT_CUR, GRIDC, CWORK(1,ISP))
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GZ) SHARED(GRIDC, LATT_CUR, CWGRAD, CWORK, ISP)
...
!$ACC REDUCTION(+:SIFLAP11,SIFLAP22,SIFLAP33,SIFLAP12,SIFLAP23,SIFLAP31) 
        SLST: DO ISP=1,MIN(NCDIJ,2)
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORKL, DENSHESS, ISP) &
 !$OMP REDUCTION(+: SIFLAP11, SIFLAP22, SIFLAP33, SIFLAP12, SIFLAP23, SIFLAP31)
          DO I=1,GRIDC%RL%NP
...
!=======================================================================
 ...
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I) SHARED(GRIDC, DWORK, DWORKL, ISP)
!$ACC PARALLEL LOOP PRESENT(DWORK,DWORKL) 
          DO I=1,GRIDC%RL%NP
 ...
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, N1, NC, N2, N3, GX, GY, GZ, GG) SHARED(GRIDC,LATT_CUR,CWORK,ISP)
!$ACC PARALLEL LOOP PRESENT(GRIDC,LATT_CUR,CWORK) PRIVATE(N1,N2,N3,NC,GX,GY,GZ,GG) 
          DO I=1,GRIDC%RC%NP
             N1= MOD((I-1),GRIDC%RC%NROW) +1
             NC= (I-1)/GRIDC%RC%NROW+1
             N2= GRIDC%RC%I2(NC)
...
! nonspin-polarized case
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, RHO, VXC) SHARED(GRIDC, DWORK4, DWORKG, DWGRAD, RINPL, DWORK, DWORKL, LATT_CUR, DHTOT, XC) &
 !$OMP REDUCTION(-:XCENCC, XCENC, CVZERO)
!$ACC PARALLEL LOOP PRESENT(DWORK4,DWORKG,DWGRAD,DWORK,LATT_CUR,XC,DHTOT,DWORKL) &
!$ACC& REDUCTION(+:CVZERO,XCENCC,XCENC) PRIVATE(RHO,VXC) 
         DO I=1,GRIDC%RL%NP
...
! Spin-polarized case
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, RHO1, RHO2, VXC1, VXC2, VXC) SHARED(GRIDC, DENCOR, NCDIJ, DWORKG, DWGRAD, RINPL, DWORK, DWORKL, LATT_CUR, DHTOT, XC, CHGMIN) &
 !$OMP REDUCTION(-:XCENCC, XCENC, CVZERO)
!$ACC PARALLEL LOOP PRESENT(DHTOT,DENCOR,LATT_CUR,XC,DWORKG,DWGRAD,DWORK,DWORKL) &
!$ACC& REDUCTION(+:CVZERO,XCENCC,XCENC) PRIVATE(RHO1,RHO2,VXC1,VXC2,VXC) 
         DO I=1,GRIDC%RL%NP
...
! non-collinear case
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP PRIVATE(I, MAG_NORM, RHO1, RHO2, VXC1, VXC2, VXC) SHARED(GRIDC, DENCOR, DWORKG, DWGRAD, RINPL, DWORK, DWORKL, LATT_CUR, DHTOT, XC, CHGMIN) &
 !$OMP REDUCTION(-:XCENCC, XCENC, CVZERO)
!$ACC PARALLEL LOOP PRESENT(DHTOT,DENCOR,LATT_CUR,XC,DWORKG,DWGRAD,DWORK,DWORKL) &
!$ACC& REDUCTION(+:CVZERO,XCENCC,XCENC) PRIVATE(MAG_NORM,RHO1,RHO2,VXC1,VXC2,VXC) 
         DO I=1,GRIDC%RL%NP
!
! | m |
!
...
!
 ...
 !$OMP END PARALLEL DO
      ENDIF
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!$ACC EXIT DATA DELETE(DWORKL,DENSHESS) 
      ...
      IF (.NOT.XC%LMU) THEN
         CALL M_sum_2(GRIDC%COMM, EXC, XCENC)
      ELSE
         CALL M_sum_3(GRIDC%COMM, EXC, XCENC, EDCMU)
      ENDIF
...
!$ACC EXIT DATA DELETE(CWGRAD,DWORK,DWORKG,DWORK1,DWORK2,DWORK3,DVC,DWORK4,DWORK5,DWORK6,DWORK7) 
      DEALLOCATE(CWGRAD,DWORKG,DWORK1,DWORK2,DWORK3,DVC,DWORK4,DWORK5,DWORK6,DWORK7)

      IF (XC%LSPIN_VDW) THEN
!$ACC EXIT DATA DELETE(DWORK4S,DWORK5S,DWORK6S,DWORK7S) 
         DEALLOCATE(DWORK4S,DWORK5S,DWORK6S,DWORK7S)
      ENDIF

      IF (XC%LTAU) THEN
!$ACC EXIT DATA DELETE(KINEDEN,CKINEDEN,TAUC) 
