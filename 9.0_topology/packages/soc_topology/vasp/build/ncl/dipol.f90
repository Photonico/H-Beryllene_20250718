...
!
!> Contains routines for calculating the total mono/dipol and quadrupol
...
!only DIP
      INTEGER IDIPCO                 !< direction (0 no dipol corrections)
        ...
        REAL(q) :: EFIELD
        LOGICAL :: VACPOTAV          !< If T, the averaged vacuum potential is reported
        REAL(q) :: VACPOTFLAT        !< Expected max field in vacuum to determine a vacuum potential
        REAL(q) :: DENMIN            !< Minimum density averaged along the planar direction
        REAL(q),POINTER :: POTLIN(:)          !< 1-D average of the potential
        REAL(q),POINTER :: DIST_ALONG_IDIPOL(:)  !< Distance along the IDIPOL direction
    END TYPE
...
!*************************** FIELD_READER ****************************
!
!> Reads the INCAR file to determine whether the user
...
!**********************************************************************
    ...
    SUBROUTINE FIELD_READER(T_INFO, P, LATT_CUR, NELECT, IU0,IU5,IU6)
      ...
      USE vaspxml
      USE reader_tags
      USE tutor, ONLY: vtutor, isError, isAlert, DipolEpsilon, DipolCubic
...
!
      ...
      DIP%EFIELD=0
      DIP%VACPOTAV = .FALSE.
      DIP%VACPOTFLAT = 1E-1_q

      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDIPOL', DIP%LCOR_DIP, IERR, WRITEXMLINCAR)
      
      IF (DIP%LCOR_DIP) DIP%IDIPCO=4

      DIP%LMONO=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LMONO', DIP%LMONO, IERR, WRITEXMLINCAR)
...
!
      ...
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'IDIPOL', DIP%IDIPCO, IERR, WRITEXMLINCAR)

      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'EPSILON', DIP%EPSILON, IERR, WRITEXMLINCAR)

      IF (DIP%EPSILON/=1 .AND. DIP%LCOR_DIP) THEN
         CALL vtutor%write(isAlert, DipolEpsilon)
      ENDIF

!
! position
!
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'DIPOL', DIP%POSCEN, 3, IERR, WRITEXMLINCAR)

      IF (DIP%LCOR_DIP .AND. DIP%IDIPCO>0 .AND. DIP%IDIPCO<=3) THEN
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'EFIELD', DIP%EFIELD, IERR, WRITEXMLINCAR)
         ENDIF
!
! compute vacuum potentials by averaging over a range of potentials
!
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LVACPOTAV', DIP%VACPOTAV, IERR, WRITEXMLINCAR)
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'VACPOTFLAT', DIP%VACPOTFLAT, IERR, WRITEXMLINCAR)
      ...
      IF (DIP%LCOR_DIP .AND. ABS(NELECT-ZION)>1E-10) THEN
         IF (LATT_CUR%ANORM(1) /=  LATT_CUR%ANORM(2) .OR. LATT_CUR%ANORM(1) /= LATT_CUR%ANORM(3) .OR. &
            ...
            CALL vtutor%write(isError, DipolCubic)
         ENDIF
      ENDIF
...
!***************************** WRITE_EFIELD ***************************
!
!>   Writes the electric field parameters to the
!>   OUTCAR file
!
!**********************************************************************
    ...
    SUBROUTINE WRITE_EFIELD(IU6)
      ...
      IF (IU6>=0) THEN
         WRITE(IU6,7225) DIP%LMONO,DIP%LCOR_DIP,DIP%IDIPCO,DIP%EPSILON, DIP%VACPOTAV, DIP%VACPOTFLAT
 ...
 7225 FORMAT( &
             ...
             '   LVACPOTAV  = ',L6,  '  vacuum potentials using an averaging scheme for the charge density' / &
             '   VACPOTFLAT = ',F10.7,' required flatness to determine vacuum potential')
      
  100 FORMAT('   EFIELD = ',F10.7,' eV/A    external electric force field' /)
       
    END SUBROUTINE WRITE_EFIELD

!************************** XML_WRITE_EFIELD **************************
!
!>   Writes the electric field parameters to the XML file
!>   if required
!
!**********************************************************************
    ...
    SUBROUTINE XML_WRITE_EFIELD
      ...
      CALL XML_INCAR('LVACPOTAV','L',IDUM,RDUM,CDUM,DIP%VACPOTAV,CHARAC,1)
      CALL XML_CLOSE_TAG
    END SUBROUTINE XML_WRITE_EFIELD

!************************ SUBROUTINE  WRITE_DIP *************************
!
!>  Writes dipole information
!
!************************************************************************
...
!
!>  Writes vacuum level to the OUTCAR file
!>  Informs the user if there is variation in the region that is supposed
!>  to have a flat 1-D averaged potential.
!
!************************************************************************
      ...
      IF (DIP%DENMIN > 1E-1_q) THEN
         CALL vtutor%alert("The minimum charge density times volume &
         &of the cell along the axis of the dipole correction is &
         &larger 1E-1, which could mean that &
         &your workfunction is not accurate as there is no field free &
         &region in your cell. Please consider either increasing the &
         &size of your cell along the dipole correction (vacuum dimension) &
         &or perhaps increasing the precision of your calculation.")
      END IF

      IF (ANY(DIP%VACUUM == -1.0)) THEN
         CALL vtutor%alert("Did not find any points to average over, which means &
         &that no vacuum field-free region was found. Please increase the size of &
         & your cell in the dimension of the dipole correction to obtain accurate &
         & workfunction values.")
         RETURN
      END IF
...
!
!>  Resets the mixer in the dipol routine CDIPOL
!
!************************************************************************
...
!
!> Calculates the dipolmoment and the
...
!***********************************************************************
    ...
    SUBROUTINE POINT_CHARGE_DIPOL(T_INFO,LATT_CUR,ZVAL,POSCEN, &
         DIP_DIRECT,QUAD,NO_CB_ATOMS_OPT)
...
!> If true (default), do not include atoms at the cell boundaries.
      LOGICAL, OPTIONAL, INTENT(IN) :: NO_CB_ATOMS_OPT
      LOGICAL                       :: NO_CB_ATOMS
      ...
      NO_CB_ATOMS = .TRUE.
      IF (PRESENT(NO_CB_ATOMS_OPT)) NO_CB_ATOMS = NO_CB_ATOMS_OPT
...
!> Get a rough estimate of the field in the region. If the potential is linear
!> in the region we are interested in (often the case for a calculation with
!> enough vacuum) then this will give the right field. Otherwise it will only
!> be an estimate.
   SUBROUTINE ESTIMATE_FIELD_IN_REGION(GRIDC, LATT_CUR, IDIPCO, POT, IND, MAXFIELD)
      use prec, only: q
      ...
      type(grid_3d) :: GRIDC
      TYPE (latt)        LATT_CUR
      INTEGER :: IDIPCO       ! Direction of the dipole correction
      REAL(q) :: POT(:)       ! Region of the potential we are interested in
      INTEGER, INTENT(IN) :: IND(:)
      REAL(q) :: MAXFIELD     ! Maximum possible field in this region
      INTEGER :: NTOT         ! Total number of grid points in the direction of IDIP
      REAL(q) :: MAX_DIFF_POT, MAX_DISTANCE  ! Max potential diff and distance

      NTOT = GRIDC%NGPTAR(IDIPCO)
      MAX_DIFF_POT = MAXVAL(POT(IND)) - MINVAL(POT(IND)) 
      MAX_DISTANCE = LATT_CUR%ANORM(IDIPCO) * SIZE(POT(IND), kind=q) / NTOT 
      MAXFIELD = MAX_DIFF_POT / MAX_DISTANCE
   END SUBROUTINE ESTIMATE_FIELD_IN_REGION

!**********************************************************************
!
!> Wraps the index respecting periodic boundary conditions
!
!**********************************************************************
   SUBROUTINE WRAP_INDEX_1D(INDEX, NTOT)
      INTEGER :: INDEX, NTOT
      INDEX = MOD(INDEX+NTOT-1,NTOT) + 1
   END SUBROUTINE WRAP_INDEX_1D 

!**********************************************************************
!
!> Calculates the indices which are within two indices respecting
!> periodic boundary conditions. Note that this routine only works
!> in (1._q,0._q) dimension (needed for the dipole correction).
!
!**********************************************************************
   SUBROUTINE INDEX_BOUNDARY(IND1,IND2,NTOT,IND_RANGE)
      INTEGER, INTENT(IN) :: IND1, IND2, NTOT
      INTEGER, ALLOCATABLE, INTENT(OUT) :: IND_RANGE(:)
      INTEGER :: IND_DIFF, IND_HIGH, IND_LOW
      INTEGER :: i
      IND_HIGH = MAX(IND1, IND2)
      IND_LOW = MIN(IND1, IND2)
      IND_DIFF = ABS(IND2 - IND1)
      IF (IND_DIFF <= NTOT - IND_DIFF) THEN
         ALLOCATE(IND_RANGE(IND_DIFF+1))
         DO i = IND_LOW, IND_HIGH
            IND_RANGE(i-IND_LOW+1) = i
         END DO
      ELSE
         ALLOCATE(IND_RANGE(NTOT-IND_DIFF+1))
         DO i = IND_HIGH, NTOT
            IND_RANGE(i-IND_HIGH+1) = i
         END DO
         DO i = 1, IND_LOW
            IND_RANGE(NTOT-IND_HIGH+1+i) = i
         END DO
      END IF
   END SUBROUTINE INDEX_BOUNDARY
...
! local
      ...
      COMPLEX(q) :: CVTOT(GRIDC%MPLWV,NCDIJ)
! work array
      REAL(q),ALLOCATABLE :: DENLIN(:), POTLIN(:)
      INTEGER,PARAMETER :: NWIN=3
      INTEGER,SAVE :: LASTMIN=-1000000
      REAL(q) :: WINDOW(-NWIN:NWIN)=(/0.1,0.2,0.3,0.4,0.3,0.2,0.1/)
      INTEGER, ALLOCATABLE :: IND_VACPOT_TOP(:), IND_VACPOT_BOT(:) ! Indices of the vacuum potential at the top and bottom
...
!=======================================================================
      ...
      IF (LINIT) THEN
         ...
         ALLOCATE(DIP%POTLIN(MAX(GRIDC%NGX, GRIDC%NGY, GRIDC%NGZ)))
         LINIT=.FALSE.
      ENDIF
...
!=======================================================================
      ...
      ALLOCATE(POTLIN(MAX(GRIDC%NGX,GRIDC%NGY,GRIDC%NGZ)) )
...
!=======================================================================
      IF (ABS(NELECT-ZION)>1E-10) THEN
         ...
         CALL EWALD_MONO(GRIDC, EMONO , NELECT-ZION, LATT_CUR )
...
! field
!=======================================================================
      CALL EWALD_DIPOL(GRIDC, EDIPOL, EF, DIP%DIPOLC, LATT_CUR, IDIPCO)
...
! convert from direct to cartesian
  ...
  ENDIF correct
      ...
      IF (IDIPCO < 4) THEN
! The vacuum potential is decided by starting from the
! index with minimum density and then moving up and down
! "width" indices.
         CALL AVERAG(GRIDC,IDIPCO,CVTOT(1,1),POTLIN,.FALSE.)
         DIP%POTLIN = POTLIN
         INDTOPVACUUM = MOD(INDMIN-INT(WIDTH)+NOUT-1,NOUT) + 1
         INDBOTVACUUM = MOD(INDMIN+INT(WIDTH)+NOUT-1,NOUT) + 1
         DIP%DENMIN = DENMIN

         IF (DIP%VACPOTAV .eqv. .FALSE. .and. ANY(DIP%VACUUM < 0.0_q)) THEN
! Conventionally, only the potential at the above determined
! indices are reported as the vacuum potential.
            DIP%VACUUM(1)=POTLIN(INDTOPVACUUM)
            DIP%VACUUM(2)=POTLIN(INDBOTVACUUM)
         ELSE
! VACPOTAV = True
! Use averaging based on the density and the field in the
! vacuum region to decide is a vacuum potential is written
            FACTOR = MAXVAL(DENLIN) 
! Strategy: Starting from FACTOR (max charge density anywhere
! in the cell in 1-D), decrease by a factor of 2 each time
! till we find a point on the grid where the field between it
! and the minimum density point is small enough that it can be
! considered flat (or have no field in that region).
            DO WHILE (FACTOR > DENMIN)
! Find the indices of the FACTOR-densities on both the
! top and bottom of the cell.
               INDTOPVACUUM_MAX = INDTOPVACUUM
               DO WHILE (DENLIN(INDTOPVACUUM_MAX) < FACTOR)
                  INDTOPVACUUM_MAX = INDTOPVACUUM_MAX - 1
                  CALL WRAP_INDEX_1D(INDTOPVACUUM_MAX,NOUT)
               END DO
               INDBOTVACUUM_MAX = INDBOTVACUUM
               DO WHILE (DENLIN(INDBOTVACUUM_MAX) < FACTOR)
                  INDBOTVACUUM_MAX = INDBOTVACUUM_MAX + 1
                  CALL WRAP_INDEX_1D(INDBOTVACUUM_MAX,NOUT)
               END DO
! Get the indices of the potentials at the top and bottom
! corresponding to the indicies of the top and bottom
! of the cell determined right above.
               CALL INDEX_BOUNDARY(INDTOPVACUUM_MAX,INDTOPVACUUM,NOUT,IND_VACPOT_TOP)
               CALL INDEX_BOUNDARY(INDBOTVACUUM_MAX,INDBOTVACUUM,NOUT,IND_VACPOT_BOT)
! Estimate the field in the vacuum region for this chosen
! slice of the potential. If this estimated field is small
! enough, then report the vacuum potentials.
               CALL ESTIMATE_FIELD_IN_REGION(GRIDC,LATT_CUR,IDIPCO,POTLIN,&
               &IND_VACPOT_TOP,EFIELD_TOP)
               CALL ESTIMATE_FIELD_IN_REGION(GRIDC,LATT_CUR,IDIPCO,POTLIN,&
               &IND_VACPOT_BOT,EFIELD_BOT)
               IF (EFIELD_BOT <= DIP%VACPOTFLAT .and. EFIELD_TOP <= DIP%VACPOTFLAT) THEN
                  IF (SIZE(IND_VACPOT_TOP) > 1 .and. SIZE(IND_VACPOT_BOT) > 1) THEN
! Make sure that there is more than (1._q,0._q) point considered
! in the averaging, otherwise there is no average and this
! procedure has failed.
                     DIP%VACUUM(1) = SUM(POTLIN(IND_VACPOT_TOP)) / SIZE(IND_VACPOT_TOP)
                     DIP%VACUUM(2) = SUM(POTLIN(IND_VACPOT_BOT)) / SIZE(IND_VACPOT_BOT) 
                  ENDIF
               END IF
               IF (DENMIN == 0.0_q) THEN
! If the minimum density is exactly (0._q,0._q) no point continuin
! the do loop
                  FACTOR = 0.0_q
               ELSE
! Damping strategy to find the best charge density is just to
! divide the factor by 2 until it reaches convergence
                  FACTOR = FACTOR / 2.0_q
               END IF
            END DO
         ENDIF
...
!************************ SUBROUTINE AVERAG  ****************************
!
!>  Calculates the average (DENLIN) of an array (COMPLEX(q) DWORK1)
...
!
!> Calculates energy corrections (and electric field corrections)
!> due to dipol:
...
!***********************************************************************
      ...
      SUBROUTINE EWALD_DIPOL(GRIDC, EDIPOL, EF, DIPOLC, LATT_CUR, IDIPCO)
      USE prec
      USE lattice
      USE ebs, only: fewald_lib
...
! fire up EWALD to get energy of these charges using periodic boundary cond.
      CALL FEWALD_LIB( &
           ...
           NIONS,NIONS,ITYP,NITYP,GRIDC%COMM)
...
!
!> Calculates energy corrections for net charged system
!>
...
!***********************************************************************
      ...
      SUBROUTINE EWALD_MONO(GRIDC, ENERGY, Z, LATT_CUR )
      USE prec
      USE lattice
      USE ebs, only: fewald_lib
      use mgrid_struct_def, only: grid_3d
...
!************************ SUBROUTINE CHGION ****************************
!
!> Calculates the chargedensity which corresponds
!> to the pseudopotential formfactor
!> i.e. core-charge
!> @note subroutine is no longer used
!
!***********************************************************************
