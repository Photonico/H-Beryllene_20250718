...
!**********************************************************************
SUBROUTINE GAUSS_LEGENDRE(XX1,XX2,X,W,N,EPS)
 USE prec
 USE constant
...
!local variables
 ...
 MY_EPS = 1e-9; IF (PRESENT(EPS)) MY_EPS = EPS
 X1=REAL(XX1,q)
 X2=REAL(XX2,q)

IF (N==0) THEN
 CALL vtutor%error("Gauss Legendre integration of zeroth order is a bad idea")
ENDIF
...
SUBROUTINE GAUSS_LAGUERRE(X,W,N,ALF)
 ...
 USE tutor, ONLY: vtutor
  ...
  CALL vtutor%bug("internal Error in VASP: TOO MANY ITERATIONS IN GAULAG", "gauss_quad.F", 191)
...
SUBROUTINE GAUSSJAC(X,W,N,ALF,BET)
 ...
 CALL vtutor%error("TOO MANY ITERATIONS IN GAUJAC")
...
SUBROUTINE GAUSSC_FROM_RECURRENCE(N,A,B,M,X,W)
 ...
 USE tutor, ONLY: vtutor
  ...
  IF (SIZE(DIAG)/=SIZE(A)) THEN
   CALL vtutor%error("Error in GAUSSC_FROM_RECURRENCE, size of DIAG and A differ: " // str(SIZE(DIAG)) &
      // " " // str(SIZE(A)))
  ENDIF

  IF ((SIZE(OFFDIAG)+1)/=SIZE(B)) THEN
   CALL vtutor%error("Error in GAUSSC_FROM_RECURRENCE, size of OFFDIAG and B incompatible: " // &
      str(SIZE(OFFDIAG)) // " " // str(SIZE(B)))
  ENDIF
  ...
  DO I=1,N-1
    IF(B(I)<0) THEN
     CALL vtutor%error("Error in GAUSSC_FROM_RECURRENCE, B<0 for I= " // str(I) // " " // str(B(I)))
    ELSE
...
!solve the symmetric tridiagonal eigenvalue problem
   ...
   IF(INF/=0)THEN
    CALL vtutor%error("ERROR in GAUSSC_FROM_RECURRENCE: DSTEV failed " // str(INF))
   ENDIF
 ...
 SUBROUTINE CALC_RECURR_COF(N,ANU,AP,BP,A,B)
  ...
  USE tutor, ONLY: vtutor
   ...
   IF (SIZE(ALPHA)/=SIZE(AP)) THEN
    CALL vtutor%error("ERROR in CALC_RECURR_COF: Arrays AP and ALPHA have different size: " // &
       str(SIZE(AP)) // " " // str(SIZE(ALPHA)))
   ENDIF

   IF (SIZE(BETA)/=SIZE(BP)) THEN
    CALL vtutor%error("ERROR in CALC_RECURR_COF: Arrays BP and BETA have different size: " // &
       str(SIZE(BP)) // " " // str(SIZE(BETA)))
   ENDIF

   IF (SIZE(NU)/=SIZE(ANU)) THEN
    CALL vtutor%error("ERROR in CALC_RECURR_COF: Arrays ANU and NU have different size: " // &
       str(SIZE(ANU)) // " " // str(SIZE(NU)))
   ENDIF

   IF (SIZE(AA)/=SIZE(A)) THEN
    CALL vtutor%error("ERROR in CALC_RECURR_COF: Arrays AA and A have different size: " // &
       str(SIZE(AA)) // " " // str(SIZE(A)))
   ENDIF
 
   IF (SIZE(BB)/=SIZE(B)) THEN
    CALL vtutor%error("ERROR in CALC_RECURR_COF: Arrays BB and B have different size: " // &
       str(SIZE(BB)) // " " // str(SIZE(B)))
   ENDIF
...
!---------------------------------------------------------------------------------
     IF ( ABS(XMAX-XMIN)<1E-6 ) THEN
       CALL vtutor%error("Error in CALCULATE_WEIGHTS_AND_ZEROS: XMIN too small for TYP= " // str(TYP))
     ENDIF
...
!CALL GAUSSLAGI_MOMENT(ANU,2*N,GRIDV, WA, WB, ALPHA)
...
! generalized moments
!**********************************************************************
SUBROUTINE GAUSSLAGI_MOMENT(NU,N,GRIDT, WA, WB, ALF)
 USE prec
 USE string, ONLY: str
 USE tutor, ONLY: vtutor
 IMPLICIT NONE
...
!local variables
 ...
 REAL(q), ALLOCATABLE :: MOM(:)
  ...
  IF (SIZE(MOM)/=SIZE(NU)) THEN
   CALL vtutor%error("Error in GAUSSLAGI_MOMENT, MOM and NU sizes differ: " // str(SIZE(MOM)) // " "&
       // str(SIZE(NU)))
  ENDIF
...
!***********************************************************************
 SUBROUTINE QSIMP_MOMENT(F,AA,BB,WA,WB,GRIDTYP,NU,N,CHOOSE,ALPHA,BETA)
  ...
  ALLOCATE(MOM(0:N)); MOM=0._q
  IF (SIZE(MOM)/=SIZE(NU)) THEN
   CALL vtutor%error("Error in QSIMP_MOMENT, MOM and NU sizes differ: " // str(SIZE(MOM)) // " " // &
      str(SIZE(NU)))
  ENDIF
...
!--------------------------------------------------------------
  ...
  CALL vtutor%error("ERROR in SIMP_MOMENT: Too many steps " // str(J))
...
SUBROUTINE MIDPOINT_MOMENT1_0INF(F,AA,BB,WAA,WBB,GRIDT,S,N,M,ALPHA)
 ...
 IF (ABS(AA)>1E-6 .OR. ABS(AA-BB)<1E-6 ) THEN
  CALL vtutor%error("Error in MIDPOINT_MOMENT1_0INF: AA/=0 or |AA-BB|=0 " // str(AA) // " " // str(BB))
 ENDIF
...
!change the integration interval
  ...
  IF (ABS(AA)<1E-6.OR.ABS(BB)<1E-6.OR.ABS(AA-BB)<1E-6) THEN
   CALL vtutor%error("Error in MIDINF_MOMENT0 " // str(AA) // " " // str(BB) // " " // str(ABS(AA-BB)))
  ENDIF
...
SUBROUTINE MIDINF_MOMENT1(F,AA,BB,WAA,WBB,GRIDT,S,N,M,ALPHA)
 ...
 USE tutor, ONLY: vtutor
  ...
  IF (ABS(AA)<1E-6.OR.ABS(BB)<1E-6.OR.ABS(AA-BB)<1E-6) THEN
   CALL vtutor%error("Error in MIDINF_MOMENT1 " // str(AA) // " " // str(BB) // " " // str(ABS(AA-BB)))
  ENDIF
...
SUBROUTINE MIDINF_MOMENT2(F,AA,BB,WAA,WBB,GRIDT,S,N,M,ALPHA,BETA)
 ...
 USE tutor, ONLY: vtutor
  ...
  IF (ABS(AA)<1E-6.OR.ABS(BB)<1E-6.OR.ABS(AA-BB)<1E-6) THEN
    CALL vtutor%error("Error in MIDINF_MOMENT2 " // str(AA) // " " // str(BB) // " " // str(ABS(AA-BB)))
  ENDIF
...
!***********************************************************************
 SUBROUTINE QROMB_MOMENT(F,AA,BB,WA,WB,GRIDT,NU,N,ALPHA,BETA)
  ...
  ALLOCATE(MOM(0:N)); MOM=0._q
  IF (SIZE(MOM)/=SIZE(NU)) THEN
   CALL vtutor%error("Error in QROMB_MOMENT, MOM and NU sizes differ: " // str(SIZE(MOM)) // " " // &
      str(SIZE(NU)))
  ENDIF
...
10 IF (INFO==0) THEN 
    CALL vtutor%error("Error in QROMB_MOMENT, integration not converged \n Possible reasons: &
       &Singularity, vanishing Moment, highly oscillatory integral")
   ELSE
...
!***********************************************************************
 SUBROUTINE QROMB_IMPROPER_MOMENT(F,A,B,WA,WB,GRIDT,NU,N,CHOOSE,ALPHA,BETA)
  ...
  ALLOCATE(MOM(0:N)); MOM=0._q
  IF (SIZE(MOM)/=SIZE(NU)) THEN
   CALL vtutor%error("Error in QROMB_I_MOMENT, MOM and NU sizes differ: " // str(SIZE(MOM)) // " " // &
      str(SIZE(NU)))
  ENDIF
...
10 IF (INFO==0) THEN 
    CALL vtutor%error("Error in QROMB_IMP_MOMENT, integration not converged \n Possible reasons: &
       &Singularity, vanishing Moment, highly oscillatory integral")
   ELSE
    MOM(I)=SS                             !moment calculated
    WRITE(*,*)I,J,MOM(I)
...
!-----------------------------------------------------------------------
 ...
 IF ( ABS( XMAX-XMIN )<1E-6 ) THEN
   CALL vtutor%error("ERROR in PLEGMON: XMIN and XMAX too close! " // str(XMIN) // " " // str(XMAX))
 ENDIF
...
FUNCTION PHERM(X,N,XMIN,XMAX)
  ...
  IF (N<-1) THEN
   CALL vtutor%error("Hermite polynomials not defined for negative order")
  ENDIF
! set H_-1=0
...
FUNCTION PHERMMON(X,N,XMIN,XMAX)
  ...
  IF ( ABS(XMIN-XMAX)<1E-6 ) THEN
   CALL vtutor%error("Error PHERMMON: XMIN and XMAX too close! " // str(XMIN) // " " // str(XMAX))
  ENDIF
...
!shift
  IF ( ABS(XMIN-XMAX)<1E-6 ) THEN
    CALL vtutor%error("Error in PTMON: XMIN,XMAX too close! " // str(XMIN) // " " // str(XMAX))
  ENDIF
...
!shift
   IF ( ABS(XMIN-XMAX)<1E-6 ) THEN
     CALL vtutor%error("Error in PUMON: XMIN,XMAX too close! " // str(XMIN) // " " // str(XMAX))
   ENDIF
