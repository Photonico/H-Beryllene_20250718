...
!
!> Performs electronic minimization in VASP
!>
!> @ref openacc
!
!**********************************************************************
...
!! USE moffload
      ...
      USE full_kpoints
      USE constant
      USE setexm, ONLY : PUSH_XC_TYPE,POP_XC_TYPE,SETUP_LDA_XC
...
! solvation__
      ...
      USE electron_common, ONLY: testBreakCondition, checkAbort
      USE bandgap_tools, ONLY: ANALYZE_AND_WRITE_BANDGAP

      USE fock_dbl, ONLY : FOCK_ALL_DBLBUF


      USE vhdf5, ONLY: IH5OUTFILEID, vh5_write_eigenval, vh5_write_efermi, vh5_write_energies_hist_step
...
! local
      REAL(q) :: TOTENL=0
      REAL(q) :: DESUM1,DESUM(INFO%NELM),ECONV
      INTEGER :: IONODE, NODE_ME, IERROR
...
! local l-projected wavefunction characters (not really used here)
      ...
      INTEGER N,ISP,ICONJU,IROT,ICEL,I,II,IRDMAA, &
              IERR,IDUM,ICOUEV,ICOUEV2,NN,NORDER,IERRBR,L,LP, &
              NCOUNT,NELM,NI,NSIM
      INTEGER :: IFLAG=-1
      REAL(q) BTRIAL,RDUM,RMS,ORT,TOTEN2,RMS2,RMST, &
              WEIGHT,BETATO,DESUM2,ETHR,RMSC,RMSP,DE2H
...
! to make timing more sensefull syncronize now
 ...
 140  FORMAT (5X, //, &
     ...
     ! 'electron entered'
      ...
      CALL INIT_CORE_REL(WDES,XC,CRHODE,IO%IU0,IO%IU5,IO%IU6)
...
! add density matrix or Hamiltonian from GAMMA file or vaspgamma.h5
...
! Set all HF exchange contributions to (0._q,0._q)
...
! reset the xc tables
      ...
      IF (LFOCK_ACE() .AND. (INFO%LDAVID.OR.INFO%LDAVID_FULL) .AND. &
          (ABS(INFO%NELMDL)<3.OR.N>=ABS(INFO%NELMDL))) THEN
...
!        CALL FOCK_ACE_ACC_PAW(WDES,W,LATT_CUR,NONL_S,NONLR_S)
...
! diagonalize back to our current (1._q,0._q)-electron Hamiltonian
         IFLAG=3
         CALL EDDIAG(HAMILTONIAN,GRID,LATT_CUR,NONLR_S,NONL_S,W,WDES,SYMM, &
              LMDIM,CDIJ,CQIJ, IFLAG,SV,T_INFO,P,IO%IU0,E%EXHF,EXHF_ACFDT=E%EXHF_ACFDT)
         CALL STOP_TIMING("G",IO%IU6,"EDDIAG",XMLTAG="diag")
...
! RMM-DIIS algorithm
!
        IF (MOD(INFO%IALGO_COMPAT,2)==1) THEN
        ...
        ELSE
           CALL EDDRMM(HAMILTONIAN,GRID,INFO,LATT_CUR,NONLR_S,NONL_S,W,WDES, &
                LMDIM,CDIJ,CQIJ, RMS,DESUM1,ICOUEV, SV, IO%IU6,IO%IU0, &
                N < ABS(INFO%NELMDL)-ABS(INFO%NELMDL)/4, FBREAK_MEAN )
        ENDIF
...
!
        ...
        CALL EDDAV(HAMILTONIAN, P, GRID,INFO,LATT_CUR,NONLR_S,NONL_S,W,WDES, NSIM, &
             ...
             LDELAY=.FALSE.,  LSUBROTI=INFO%LDIAG, LEMPTY=.FALSE., LHF= .NOT. (ABS(INFO%NELMDL) >=3 .AND. N < ABS(INFO%NELMDL)),  & 
             EXHF_ACFDT= E%EXHF_ACFDT)
! require recalculation of potential in next step (then with HF contribution)
        IF ( ABS(INFO%NELMDL) >=3 .AND. N+1 == ABS(INFO%NELMDL) .AND.  LHFCALC .AND. .NOT. INFO%LCHCON) INFO%LPOTOK=.FALSE.
...
! and reorthogonalize
      ...
      ELSE IF (INFO%LDAVID_FULL) THEN
!
! Davidson algorithm (non-blocked)
!

!@TODO: the choice of convergence threshold needs to be evaluated and optimized

! DESUM2 is a rough estimate of the expected change in the total energy:
! the minimum of the change in the total energy in the preceding step, DESUM(N-1),
! and the expected change in the electrostatic energy in the current step, DE2H.
! It will be used by EDDAV_FULL to adjust its orbital energy convergence criterion
         DESUM2 = 0 ; IF (N>1) DESUM2 = MIN( ABS(DESUM(N-1)), ABS(DE2H) )

! On entry ETHR is an upper-bound for the orbital energy convergence criterion in EDDAV_FULL,
! on exit it is the actual value of this criterion used in the step that just finished.
! Setting ETHR = 1E10 in the first electronic refinement step will make sure that the
! orbital energy convergence criterion will only ever decrease in subsequent steps:
!!         IF (N==1) ETHR = 1E10
! To disable the use of ETHR, just set it to 1E10 in every step: EDDAV_FULL can then
! essentially freely choose the orbital energy convergence criterion:
         ETHR = 1E10

         CALL EDDAV_FULL(HAMILTONIAN, SV, CDIJ, CQIJ, NONLR_S, NONL_S, INFO, LATT_CUR, WDES, W, &
              DESUM2, ETHR, RMS, DESUM1, E%EXHF, ICOUEV, IO%IU0, IO%IU6, &
              LDELAY = N<=ABS(INFO%NELMDL), LRESIDUAL = N>ABS(INFO%NELMDL), &
              LHF = .NOT. (ABS(INFO%NELMDL)>=3 .AND. N<ABS(INFO%NELMDL)) )

! If the next step will switch on the Fock contributions then we require an update of the local potential
         IF (ABS(INFO%NELMDL)>=3 .AND. N+1==ABS(INFO%NELMDL) .AND. LHFCALC .AND. .NOT.INFO%LCHCON) INFO%LPOTOK=.FALSE.

         CALL STOP_TIMING("G",IO%IU6,"EDDAVF",XMLTAG="fda")
...
!=======================================================================
      ...
      ! "densta is ok"
!=======================================================================
! calculate free-energy and bandstructure-energy
...
!=======================================================================
      ...
      TOTEN=E%EBANDSTR+E%DENC+E%XCENC+E%TEWEN+E%PSCENC+E%EENTROPY+E%PAWPS &
           +E%PAWAE+INFO%EALLAT+E%EXHF+ECORE()+Ediel_sol+E%ESCPC+E%EPLUGINS
...
30303 FORMAT('DAVI:',I3,'   ',E20.12,'   ',E12.5,'   ',E12.5, &
     &       I6,'  ',E10.3)
40303 FORMAT('FDA: ',I3,'   ',E20.12,'   ',E12.5,'   ',E12.5, &
     &       I6,'  ',E10.3)
      ...
      ELSE IF (INFO%LDAVID_FULL) THEN
        WRITE(17, 40303,ADVANCE='NO')  N,TOTEN,DESUM(N),DESUM1,ICOUEV,RMS
        IF (IO%IU0>=0) &
        WRITE(IO%IU0,40303,ADVANCE='NO')  N,TOTEN,DESUM(N),DESUM1,ICOUEV,RMS
...
!=======================================================================
      CALL testBreakCondition(DESUM(N), DESUM1, N, MIX%IMIX == 0, INFO, LABORT_WITHOUT_CONV)

      CALL VH5_WRITE_OSZICAR_HIST_STEP(IH5INTERMEDIATEGROUP_ID, GRP_HISTORY, &
           [REAL(N,q), TOTEN, DESUM(N), DESUM1, REAL(ICOUEV,q), RMS, RMSC])
...
! ======================================================================
      IF (INFO%LABORT .AND. ACCURATE_CORE_LEVEL_SHIFTS()) THEN
         ...
         CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, &
              ...
              E, LASPH=INFO%LASPH , LCOREL=.TRUE. )
...
! write out additionally energy eigenvalues and fermi level
         CALL VH5_WRITE_EIGENVAL(IH5OUTFILEID, WDES, W, KPOINTS, KPOINTS_FULL)
         CALL VH5_WRITE_EFERMI(IH5OUTFILEID, EFERMI)
 ...
 2202   FORMAT(' E-fermi : ', F8.4,'     XC(G=0): ',F8.4, &
     &         '     alpha+bet :',F8.4/)
        IF (HAMILTONIAN%COULOMB_POT%KERNEL_TRUNCATE%ACTIVE) THEN
            WRITE(IO%IU6,2203) REAL(E%POTHARZERO, KIND=q), REAL(E%POTIONZERO, KIND=q)
        END IF
 2203   FORMAT(' POTHAR(G=0): ', F15.5, '    POTION(G=0): ',F15.5/)

        CALL ANALYZE_AND_WRITE_BANDGAP(IO%IU6, W, INFO%NELECT, KPOINTS%BANDGAP)
...
!=======================================================================
      ...
      CALL VH5_WRITE_OSZICAR_END_ELMIN_STEP(IH5INTERMEDIATEGROUP_ID, &
                                            GRP_HISTORY, INFO%LABORT)
...
! local
      ...
      IF (IU6>=0) THEN
        ...
        DO ISP=1,WDES%ISPIN
        WRITE(IU6,'(A,F20.10)') ' Fermi energy: ', W%EFERMI( ISP )
