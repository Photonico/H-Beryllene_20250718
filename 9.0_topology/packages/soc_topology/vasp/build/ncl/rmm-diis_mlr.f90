...
! check dimensions
         ...
         &   .OR.SIZE(SW0STORE,3)/=WDES%NKPTS.OR.SIZE(SW0STORE,4)/=WDES%ISPIN) THEN
             CALL vtutor%error("ERROR: LINEAR_RESPONSE_DIIS: SW0STORE incorrectly dimensioned.")
          ENDIF
          LCALCSW0=.FALSE.
       ELSE
...
# 677
       ...
       i2: DO NP=1,NSIM
          ...
          CALL ZCOPY(WDES%NRPLWV,WACC%CPTWFP(1,NP),1, WTMPA%CPTWFP(1,NP), 1)
...
!   <phi(1)|xi> + c.c + <phi(1)| H(0)-e(0) S(0)|phi(1)>
          ...
          CALL M_sum_3(WDES%COMM_INB, FNORM_, ORTH, EVAR_)
...
!     B(n1) =   - <R_n1 | xi >= - < phi_n1 | ( H - e S) | xi >
          ...
          IF (ABS(CHAM(1,1))<1E-15) THEN
             ...
             CALL vtutor%bug("internal ERROR: LINEAR_RESPONSE_DIIS matrix is zero, try to call with &
                ...
                1:ITER)) // " " // str(B(1:ITER)), "rmm-diis_mlr.F", 799)
             ENDIF
          ENDIF
