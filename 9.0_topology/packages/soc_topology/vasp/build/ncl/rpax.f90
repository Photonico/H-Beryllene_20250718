...
# 20
      ...
      COMPLEX(q)      , ALLOCATABLE, PUBLIC :: FTOD_OC(:,:,:), FTOD_OC_AB(:,:,:,:,:,:), FTOD_OC_AI(:,:,:,:,:,:)
      COMPLEX(q) , ALLOCATABLE, PUBLIC :: FTOD_OC_IA(:,:,:,:,:,:)
      COMPLEX(q) , ALLOCATABLE, PUBLIC :: FTOD_OC_IJ(:,:,:,:,:,:)
      COMPLEX(q) , ALLOCATABLE, PUBLIC ::  OC_IJ_TMP(:,:,:,:), PW_IJ_TMP(:,:,:,:)
      COMPLEX(q) , ALLOCATABLE, PUBLIC ::  OC_IA_TMP(:,:,:,:), PW_IA_TMP(:,:,:,:), OC_IA_TMP2(:,:,:,:), PW_IA_TMP2(:,:,:,:)
      COMPLEX(q) , ALLOCATABLE, PUBLIC ::  OC_AI_TMP(:,:,:,:), PW_AI_TMP(:,:,:,:)
      COMPLEX(q) , ALLOCATABLE, PUBLIC :: TE4O(:,:), VVOO(:,:,:,:), OVOV(:,:,:,:), JLIK(:,:,:,:), &
         KLIJ(:,:,:,:), OOVV(:,:,:,:), VV(:,:), VOVO(:,:,:,:), &
         OOOV(:,:,:,:), VV2(:,:), C_NV(:,:), CHI_GAI(:,:,:,:,:,:), AIG(:,:,:), GAI_TMP(:,:,:,:)
      COMPLEX(qs) , ALLOCATABLE, PUBLIC :: T2(:,:,:,:,:,:), T2_N_TMP(:,:,:,:,:), &
         OOVV_S(:,:,:,:), VVOO_S(:,:,:,:), &
         T2_TMP(:,:,:,:), VOVO2_S(:,:,:,:), &
         VOVO_S(:,:,:,:), VOVO3_S(:,:,:,:), &
         OOVV2_S(:,:,:,:), VV_S(:,:), VV2_S(:,:), &
...
!T1=(0.0_qs,0.0_qs)
!T1_N=(0.0_qs,0.0_qs)
         E_TOT_CCSD=(0._q,0._q)
         E_TOT_CCSD_X=(0._q,0._q)
...
!#ifdef gammareal
!         desc_FTOD_PW(9) = MAX(1, desc_FTOD_PW(9)) ! leading dimension of local array
!         desc_FTOD_PW(5) =  desc_FTOD_PW(5)
!         desc_FTOD_PW(3) =  desc_FTOD_PW(3)
!#else
...
!> which is required by the 1 routines
!***********************************************************************
...
!         T2AMPLITUDES=(0._q,0._q)
!         T2AMPLITUDES_new=(0._q,0._q)
...
!***********************************************************************
      ...
      subroutine APPEND_CDER_TO_FTOD(WDES,W,LATT_CUR,dir)
         ...
         COMPLEX(q) :: CDER_BETWEEN_STATE_IA(3),CDER_BETWEEN_STATE_AI(3)
         real(qs) :: ediff_th
         ...
         MKQ=KPOINT_IN_FULL_GRID(W%WDES%VKPT(:,KPTS_MKPTS(mki))-W%WDES%VKPT(:,KPTS_MKPTS(mki)),KPOINTS_FULL) !just to find the gamma-point
            do mni=1,VBMAX
               do mna=1,(PROCS*WDES%NBANDS-VBMAX)
                  ...
                  CDER_BETWEEN_STATE_AI(:)=(0._q,0._q)
...
!                 (1._q,0._q), TE4O(1,1),(1))
         ...
         TE4O(1,1)=CONJG(TE4O(1,1)*KPOINTS_FULL%WTKPT(1)*KPOINTS_FULL%WTKPT(1))
         CALL M_sum_z(WGW%COMM_INTER, TE4O(1,1), 1)
...
!            DO KQ=1,WDES%NKPTS
               ...
               DO NB=1,(PROCS*WDES%NBANDS-VBMAX)
                  CALL CONTR_FTOD(NGVECTOR,NHVECTOR, &
                   ...
                   VBMAX,OOVV(1,1,NA,NB),(0._q,0._q))
               ENDDO
               ENDDO
...
!                  ENDIF
               ...
               DO NB=1,(PROCS*WDES%NBANDS-VBMAX)
                  CALL CONTR_FTOD(NGVECTOR,NHVECTOR, &
                   ...
                   VBMAX,OOVV(1,1,NA,NB),(1._q,0._q))
               ENDDO
               ENDDO
...
!                  T2_MTMP(1,1,1,1,1), (PROCS*WDES%NBANDS-VBMAX),0,PROCS_KPTS(KA))
      ...
      COMPLEX(q) :: XY2E4ORB,PW_XY,OC_XY, &
              PW_XY_,OC_XY_, beta
      integer :: time_array1(8),time_array2(8),ems
!#ifdef gammareal
      call date_and_time (values=time_array1)
!#else
            CALL ZGEMM('C','n',(LNXY),(LNXY_),&
...
!                  VBMAX*(WDES%NB_TOT-VBMAX),VOVO(1,1,1,1),(0._q,0._q)) !bjai

!		CALL CONTR_FTOD(NGVECTOR,NHVECTOR, &
!                  PW_IA_TMP(1,1,1,KQ),OC_IA_TMP(1,1,1,KQ), &
!                  VBMAX*(WDES%NB_TOT-VBMAX), &
!                  CHI_GAI(1,1,1,KI,KA,2),FTOD_OC_AI(1,1,NA,KQ_,KA,2), &
!                  VBMAX*(WDES%NB_TOT-VBMAX),VOVO(1,1,1,1),(1._q,0._q)) !bjai
...
!            DO KQ=1,WDES%NKPTS
               ...
               CALL CONTR_FTOD(NGVECTOR,NHVECTOR, &
                ...
                (PROCS*WDES%NBANDS-VBMAX)*VBMAX,OVOV(1,1,1,1),(0._q,0._q))
               CALL SORT_O1V1O2V2_V2V1O2O1(WDES,OVOV, &
                 VVOO)
...
!if (ME==0) WRITE(*,*)'TETS(ab)=',TETS
!if (ME==0) WRITE(*,*)'TET2S(T2)=',TET2S
!if (iteration==9) T2(23:(PROCS*WDES%NBANDS-VBMAX),10:(PROCS*WDES%NBANDS-VBMAX),:,:,:,:,:)=(0._q,0._q)
!if (iteration==9) T2_N(23:(PROCS*WDES%NBANDS-VBMAX),10:(PROCS*WDES%NBANDS-VBMAX),:,:,:,:,:)=(0._q,0._q)
