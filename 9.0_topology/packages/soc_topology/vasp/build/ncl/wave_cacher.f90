...
!> free entries in this block (-1 signifies not allocated)
     INTEGER:: NFREE
!> how often is entry used
     INTEGER, POINTER:: ICNT(:)
!> wavefunction in real space
     COMPLEX(q), POINTER:: CR(:,:)
!> projector (complex)
     COMPLEX(q)      ,POINTER:: CPROJ(:,:)
...
!> integer indicating in which block a wavefunction is store
     INTEGER, POINTER:: IBLK(:,:,:)
!> integer indicating where in the block a wavefunction is store
     INTEGER, POINTER:: IPOS(:,:,:)
!> NBLK wave_cache_block arrays
     TYPE( wave_cache_block), POINTER:: C(:)
!> cache for phase factor at each k-point
     COMPLEX(q), POINTER:: CPHASE(:,:)
!> phase factor required or not
     LOGICAL, POINTER ::  LPHASE(:)

! this handle stores simga_c phi_n
!
!> lowest  band entered in FH
     INTEGER:: NBLOW
!> highest band entered in FH
     INTEGER:: NBHIGH
     INTEGER:: NB_PAR !
     TYPE(fock_handle), POINTER:: FH(:)
  END TYPE wave_cache

  REAL(q), SAVE :: MAX_MEMORY_CACHER = 0.0_q

!> matrix to store search direction for subspace rotation
  COMPLEX(q), ALLOCATABLE:: CHF(:,:,:,:)
...
!>
!> @details @ref openmp :
!> the wave_cache::fh (fock::fock_handle) allocated with dimension
!> mopenmp::omp_nthreads. To reduce the memory demands, only some parts
!> of WCACHE\%FH(2:omp_nthreads) are allocated.
!
!***********************************************************************
...
! local
    ...
    IF (NBLK*NBLK < WDES%NBANDS* WDES%NKPTS * WDES%ISPIN) THEN
       CALL vtutor%bug("internal error in ALLOCATE_CACHER: cache allocation failed", "wave_cacher.F", 94)
    ENDIF
...
!$  DO N=2,omp_nthreads
!$     CALL ALLOCATE_FOCK_HANDLE_NOACC(WCACHE%FH(N),LMDIM)
!$  ENDDO
...
!>
!> @details @ref openmp :
!> the wave_cache::fh (fock::fock_handle) was allocated with dimension
!> mopenmp::omp_nthreads. To reduce the memory demands, only some parts
!> of WCACHE\%FH(2:omp_nthreads) were allocated.
!
!***********************************************************************
...
!$  DO N=2,omp_nthreads
!$     CALL DEALLOCATE_FOCK_HANDLE_NOACC(WCACHE%FH(N))
!$  ENDDO
...
!>
!> @details @ref openmp :
!> adding a wave function to the cache structure is 1._q inside
!> a CRITICAL region (wcacher). This ensures the integrity of the
!> cache structure.
!
!***********************************************************************
  ...
  SUBROUTINE STORE_PHASE(WCACHE, NK,  LPHASE, CPHASE)
    ...
    IF (LPHASE) THEN
       IF ( SIZE(WCACHE%CPHASE,1) /= SIZE(CPHASE)) THEN
          CALL vtutor%bug("internal error in STORE_PHASE: size changed : " // &
             str(SIZE(WCACHE%CPHASE,1)) // " " // str(SIZE(CPHASE)), "wave_cacher.F", 277)
       ENDIF
...
! search for an emtpy slot in that block
       IF (WCACHE%C(IBLK)%NFREE>0) THEN
          ...
          ENDDO
          CALL vtutor%bug("internal error in FIND_NEW_CACHER: NFREE was not correct, no free line", "wave_cacher.F", 351)
...
! allocate initialize and return
       ...
       ENDIF
    ENDDO
    CALL vtutor%bug("internal error in FIND_NEW_CACHER: no free block", "wave_cacher.F", 371)
...
!>
!> @details @ref openmp :
!> removing a wave function from the cache structure is 1._q inside
!> a CRITICAL region (wcacher). This ensures the integrity of the
!> cache structure.
!
!***********************************************************************
...
!$OMP CRITICAL (wcacher)
    ...
    IF (IBLK==0 .OR. IPOS==0 ) THEN
       CALL vtutor%error("error in REMOVE_CACHER: could not find wavefunction " // str(NB) // " " // &
          str(NK) // " " // str(ISP))
    ELSE
       IF ( WCACHE%C(IBLK)%ICNT(IPOS)<=0) THEN
          CALL vtutor%error("error in REMOVE_CACHER: reference count is zero " // str(IBLK) // " " // &
             str(IPOS))
       ENDIF
    ...
    DO ISP=1,ISPIN
       DO K=1,NKPTS
          DO NB=1,NBANDS
             ...
             IF (IBLK/=0 .OR. IPOS/=0 ) THEN
                CALL vtutor%error("error in ISEMPTY_CACHER: cache structure is not emtpy " // str(NB) &
                   // " " // str(K) // " " // str(ISP))
             ENDIF
          ENDDO
       ENDDO
...
! search for an emtpy slot in that block
       IF (WCACHE%C(IBLK)%NFREE>0 .AND. WCACHE%C(IBLK)%NFREE/=WCACHE%NBLK ) THEN
          CALL vtutor%error("error in ISEMPTY_CACHER: cache blocks are dusty " // str(IBLK) // " " // &
             str(WCACHE%C(IBLK)%NFREE) // " " // str(WCACHE%NBLK))
       ELSE IF (WCACHE%C(IBLK)%NFREE>0) THEN
          DO IPOS=1,WCACHE%NBLK
             IF (WCACHE%C(IBLK)%ICNT(IPOS)/=0) THEN
                CALL vtutor%error("error in ISEMPTY_CACHER: cache block is dusty " // str(IBLK) // "&
                   & " // str(IPOS) // " " // str(WCACHE%C(IBLK)%ICNT(IPOS)))
             ENDIF
          ENDDO          
       ENDIF
    ENDDO
...
!>
!> @details @ref openmp :
!> this subroutine is called from within a parallel region. Each thread
!> has its own workspace, WCACHE\%FH(omp_get_thread_num). All threads,
!> however, store the results into WCACHE\%FH(1)\%CXI and \%CKAPPA.
!> The integrity of these arrays is ensured by means of OMP locks:
!> WCACHE\%FH(1)\%OMP_LCK1 and \%OMP_LCK2, respectively.
!
!***********************************************************************
...
!> sum_G CONJG(W_1, 2(G)) CHG(G)
    COMPLEX(q):: Z
    INTEGER:: NB1, NK1, NB2, NK2, ISP
!> weight use for the addition to XI
    REAL(q):: W_INTER(2)
! local
    TYPE (wavedes1) :: WDES1
    LOGICAL :: LPHASE
...
!!$OMP CRITICAL (wcacher)
    IF (WCACHE%NBLOW==-1) THEN
       IF (NB1 > SIZE(WCACHE%FH(1)%CXI,2)) THEN
          CALL vtutor%bug("internal error in STORE_GW_ACC: band index exceeds FH dimension " // &
             str(NB1) // " " // str(SIZE(WCACHE%FH(1)%CXI,2)), "wave_cacher.F", 594)
       ENDIF
       NB1_STORE=NB1
    ELSE
       IF (NB1 < (WCACHE%NBLOW-1)*WCACHE%NB_PAR+1 ) THEN
          CALL vtutor%bug("internal error in STORE_GW_ACC: band index below NBLOW " // str(NB1) // &
             " " // str(WCACHE%NBLOW), "wave_cacher.F", 600)
       ENDIF

       IF (NB1 > WCACHE%NBHIGH*WCACHE%NB_PAR ) THEN
          CALL vtutor%bug("internal error in STORE_GW_ACC: band index exceeds NBHIGH " // str(NB1) &
             // " " // str(WCACHE%NBHIGH), "wave_cacher.F", 605)
       ENDIF
    ...
    IF ((GRIDHF%LREAL  .NEQV.  WDESQ%GRID%LREAL) .OR. (GRIDHF%REAL2CPLX  .NEQV. WDESQ%GRID%REAL2CPLX)) THEN
       CALL vtutor%bug("internal error in STORE_GW_ACC: incompatible grids " // str(GRIDHF%LREAL) &
          // " " // str(WDESQ%GRID%LREAL) // " " // str(GRIDHF%REAL2CPLX) // " " // &
          str(WDESQ%GRID%REAL2CPLX), "wave_cacher.F", 613)
    ENDIF
...
!$  CALL OMP_SET_LOCK(WCACHE%FH(1)%OMP_LCK1(NB1_STORE))
...
!$  CALL OMP_UNSET_LOCK(WCACHE%FH(1)%OMP_LCK1(NB1_STORE))
...
!$     CALL OMP_SET_LOCK(WCACHE%FH(1)%OMP_LCK2(NB1_STORE))
...
!$     CALL OMP_UNSET_LOCK(WCACHE%FH(1)%OMP_LCK2(NB1_STORE))
    ENDIF
!!$OMP END CRITICAL (wcacher)
...
!    IF (WDES1%DO_REDIS) THEN  ! why is this if here
      CALL M_sum_z(WDES1%COMM_INTER,WCACHE%FH(1)%CXI(1,1,INDEX),SIZE(WCACHE%FH(1)%CXI,1)*SIZE(WCACHE%FH(1)%CXI,2))
      CALL M_sum_z(WDES1%COMM_INTER,WCACHE%FH(1)%CKAPPA(1,1,INDEX),SIZE(WCACHE%FH(1)%CKAPPA,1)*SIZE(WCACHE%FH(1)%CKAPPA,2))
!    ENDIF
...
! global band index, N_ is the local band index
       ...
       IF (N_STORE> SIZE(W%CPTWFP,2)) THEN
          CALL vtutor%bug("internal error in STORE_GW_ACC_FINAL: N_STORE is out of range " // &
             str(N_STORE) // " " // str(SIZE(W%CPTWFP,2)) // " " // str(WCACHE%NBLOW) // " " // &
             str(WCACHE%NBHIGH), "wave_cacher.F", 752)
       ENDIF
...
! add CKAPPA to CXI (full acceleration on band N now in CXI to W%CPTWFP
       IF (W%WDES%LOVERL) THEN
          IF (NONLR_S%LREAL) THEN
             ...
             ENDDO
          ELSE
             CALL VNLAC0(NONL_S, WDES1, WCACHE%FH(1)%CKAPPA(1,N,INDEX), W%CPTWFP(1, N_STORE, NK1, ISP))
          ENDIF
       ENDIF
       DO ISPINOR=0,WDES1%NRSPINORS-1
...
!> use COHSEX
    LOGICAL LCOHSEX
!> determine natural orbitals for GW type calculations (see below)
    LOGICAL LGWNO
    TYPE (kpoints_struct) KPOINTS
!> minimum occupancy for empty state
    REAL(q) WEIMIN
!> Fermi-level
    REAL(q) EFERMI
    INTEGER IU0, IU6
...
!> avoid cutting through degenerate eigenstates
...
! CDCHF = (0.0_q, 0.0_q) ! Breaks bulk_GaAs_scGW0_ALGO=D_sym (gnu), passed uninitialized to FOCK_ACC, is only updated there.
...
! force full fock contribution
    ...
    CALL PROCESS_INCAR(.TRUE., IU0, 99, 'LKOTANI', LKOTANI, IERR, WRITEXMLINCAR)
    CALL PROCESS_INCAR(.TRUE., IU0, 99, 'LDYSON', LDYSON, IERR, WRITEXMLINCAR)
    IF (LDYSON) THEN
       LKOTANI=.TRUE.
       LADDHERM=.TRUE.
    ENDIF
    ...
    CALL PROCESS_INCAR(.TRUE., IU0, 99, 'LADDHERM', LADDHERM, IERR, WRITEXMLINCAR)
...
! compose into upper and lower triangular matrix
             ...
             IF (IFAIL/=0) THEN
                CALL vtutor%error("error in EDDIAG_GW: ZGETRF returns " // str(IFAIL))
             ENDIF
! invert matrix
             CALL ZGETRI( NBANDSGW_K(NK,ISP), COVL(1,1), SIZE(COVL,1), IPIV, & 
                   CWRK, SIZE(CWRK), IFAIL )
             IF (IFAIL/=0) THEN
                CALL vtutor%error("error in EDDIAG_GW: ZGETRI returns " // str(IFAIL))
             ENDIF
...
# 1230
                ...
                IF (IFAIL/=0) THEN
                   CALL vtutor%error("LAPACK: Routine ZPOTRF failed! " // str(IFAIL))
                ENDIF
...
# 1240
                ...
                IF (IFAIL/=0) THEN
                   CALL vtutor%error("LAPACK: Routine ZTRTRI failed! " // str(IFAIL))
                ENDIF
...
! force diagonal to be real
                ...
                DO  NP=N,NBANDSGW_K(NK,ISP)
                   ...
                   CHAM(NP,N,NK,ISP)=(CHAM(NP,N,NK,ISP)*MAX(W%FERTOT(N,NK,ISP),1.E-2_q) &
                              +CONJG(CHAM(N,NP,NK,ISP))*MAX(W%FERTOT(NP,NK,ISP),1.E-2_q))/ &
...
!=======================================================================
    ...
    IF (HFEIG(N2)<1E-8) THEN
      IF( IU6>=0 ) WRITE(IU6,'(A,I5,F10.5)') ' negative eigenvalue (try to increase CSHIFT) ',N2,HFEIG(N2)
      IF( IU6>=0 ) WRITE(IU6,'(20F6.3)') CUNI(1:NBANDS,N2)
    ENDIF
    ENDDO
