...
! were 1._q by Tomas Bucko  - most importantly,
...
!tb  beg
      ...
      real(q) :: s6_d3_gl,s8_d3_gl,rs6_d3_gl
      real(q) :: rs8_d3_gl,alp_d3_gl,rthr_d3_gl,rthr2_d3_gl
      integer :: version_d3_gl
      logical :: linit_d3_gl=.FALSE.
      character*10 :: method_d3_gl
...
!       !subroutine vdw_forces_D3(IO,LATT_CUR,XC,DYN,T_INFO,TSIF,TIFOR,TOTEN,
!     . ! ELEM,historycounter)
      subroutine vdw_forces_D3(IO,LATT_CUR,XC,DYN,T_INFO,TSIF,TIFOR,TOTEN, &
      &                        ELEM,IVDW)
!      !tb end
...
!c at this state, the vdwd3 module must be initialized
!c stop otherwise
      IF (.NOT. linit_d3_gl) THEN
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      SUBROUTINE readparam(ivdw,IO,XC)
      USE base
      USE setexm
      USE fock
      USE constant
...
!Local Variables
      ...
      IF (LINIT_D3_GL .EQV. .TRUE.) RETURN
...
!          END SELECT
!       ! hybrid functionals
!       ELSE IF (ALDAX==0.75 .AND. HFSCREEN==0.0) THEN
!          select case (LEXCH)
!          case (8) ! pbe
!             method='pbe0'
!          case default
...
!       ENDIF
      ...
      IF (version==2) THEN
        ...
        IF (IERR==0) THEN
           ...
           ELSE
              CALL vtutor%alert('VDW_S6 is not reasonable. Taking default.')
           ENDIF
        ENDIF
      ...
      ELSE IF (version.eq.3) THEN !version=3
        ...
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'VDW_SR', RDUM, IERR, WRITEXMLINCAR)
        IF (IERR==0) THEN
           IF ((RDUM>0.0).AND.(RDUM<10.0)) THEN
              rs6=RDUM
              user_defined = user_defined // '\nVDW_SR = ' // str(rs6)
           ELSE
              CALL vtutor%alert('VDW_SR is not reasonable. Taking default.')
           ENDIF
        ENDIF
...
!Reading from INCAR: VDW_RADIUS
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'VDW_RADIUS', crit_vdw, IERR, WRITEXMLINCAR)
      IF (IERR==0.and.crit_vdw.gt.0) THEN
         user_defined = user_defined // '\nVDW_RADIUS = ' // str(crit_vdw)
         crit_vdw=(crit_vdw/autoa)**2
      END IF
!      IF (IERR==3) crit_vdw=50._q

!Reading from INCAR: CN_RADIUS
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'VDW_CNRADIUS', crit_cn, IERR, WRITEXMLINCAR)
      IF (IERR==0.and.crit_cn.gt.0) THEN
         user_defined = user_defined // '\nVDW_CNRADIUS = ' // str(crit_cn)
         crit_cn=(crit_cn/autoa)**2
      END IF
      IF (crit_cn <0._q) THEN
         crit_cn=1600.0_q
         CALL vtutor%alert("VDW_CNRADIUS is not reasonable. Taking default.")
      END IF
      
      CALL CLOSE_INCAR_IF_FOUND(IO%IU5)

      IF (TRIM(method) == 'unknown') THEN
         ...
         ELSE
            ...
            CASE (2)
               user_defined = "VDW_S6"
            CASE (3)
               user_defined = "VDW_S8 and VDW_SR"
            CASE (4)
               user_defined = "VDW_A1, VDW_A2, and VDW_S8"
            CASE DEFAULT
               user_defined = "the required parameters"
            END SELECT
            CALL vtutor%error("Unsupported xc functional, please specify " // &
               user_defined // " in the INCAR file.")
         END IF
      ELSE IF (LEN_TRIM(user_defined) > 0) THEN
         CALL vtutor%alert("Specifying the vdW parameters" // user_defined // &
            "\nin the INCAR file will overwrite the defaults for " // TRIM(method) // &
      ...
      if (IO%IU6.ge.0) then
        ...
        write(IO%IU6,   '('' VDW_RADIUS   ='',f10.4,'' A'')') sqrt(crit_vdw)*autoa
        write(IO%IU6,   '('' VDW_CNRADIUS ='',f10.4,'' A'')') sqrt(crit_cn)*autoa
      endif
...
!     printed out. (Default=True)
      logical grad,echo
! switches between D2, D3((0._q,0._q)) and D3(BJ)
...
!c how many different C6 for (1._q,0._q) element
...
!c JM: rs18=a2
      ...
      select case (func)
         ...
         case ("tpss")
              ...
              s18 =1.9435
              rs18=4.4752
         case ("scan") !c see Brandenburg, PRB 94, 115144 (2016)
              rs6 =0.538
              s18 =0.
              rs18=5.420
         case ("hf")
...
!c special HF/DFT with eBSSE correction
         ...
         case ("hcth120")
              ...
              rs18=4.3359
         case DEFAULT
              call vtutor%bug("version 4: No match for " // trim(func), "subdftd3.F", 2891)
      end select
      endif
...
!c default def2-QZVP (almost basis set limit)
      select case (func)
         ...
         case ("hcth120")
              ...
              s18=1.206
         case DEFAULT
              call vtutor%bug("version 3: No match for " // trim(func), "subdftd3.F", 3055)
      end select
      endif !version =3
!c DFT-D2
      if(version.eq.2)then
...
!C     *****************************************************************
      ...
      call vtutor%error('program stopped due to: ' // s)
