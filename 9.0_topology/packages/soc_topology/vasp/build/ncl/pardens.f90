...
MODULE pardens
  ...
  USE vhdf5, ONLY : vh5_init_parcharge_output, vh5_write_to_parcharge_array, IH5OUTFILEID
...
! k-point or not
    ...
    INTEGER :: kpts_list(wdes%nkpts) ! list of kpts to calculate charge density for
    INTEGER :: bands_list(wdes%nb_tot)  ! list of bands to calculate charge density for
    INTEGER, ALLOCATABLE :: tkl(:), tbl(:) ! trimmed lists for contributing bands and kpts.
...
! counters and such stuff
    INTEGER  :: i,nk, nb, ik, ib, bc, kc, isp, ni
...
! mpi version requires to know which node does the io
      ...
      IF (WDES%COMM_KINTER%NCPU.NE.1) THEN
         CALL vtutor%error("PARCHG: KPAR>1 not implemented, sorry.")
      END IF
    ...
    ltest1: IF (ltest) THEN
       IF (lop6) THEN
          ...
          SELECT CASE (nbmodus)
             ...
             CASE DEFAULT
                CALL WFORCE(io%iu6)
                CALL vtutor%error("VALUE NOT KNOWN, SO I WILL STOP")
             ...
             IF (io%lparchgh5) WRITE(TIU6,'(A)') "Redirect all PARCHG output to hdf5 file."
          ...
          SELECT CASE (nbmodus)
             ...
             IF (io%lparchgh5) WRITE(TIU0,'(A)') "Redirect all PARCHG output to hdf5 file."
...
! first make a proper starting status for the files CHG and CHGCAR
       ...
       ELSE
          ...
          IF ((iuchgcar <= -1) .OR. (iuchgcar >= 99)) THEN  ! no I/O unit free
             IF (io%lopen) CALL WFORCE(io%iu6)
             CALL err_handle(1,"CHGCAR","NONE","U")
          END IF
...
! before starting the whole stuff: search for a free I/O unit:
      IF (NODE_ME==IONODE) THEN
       ...
       IF ((iuout <= -1) .OR. (iuout >= 99) ) THEN
          IF (io%lopen) CALL WFORCE(io%iu6)
          CALL err_handle(1,"PARCHG.nb.nk","NONE","U")
       END IF
...
! merge everything:                          3           PARCHG
      ...
      bands_list=0
       ...
       CASE (0) sep  ! seperate everything
         ...
         IF (io%lparchgh5) THEN
!first figure out how many bands and kpts we have to allocate:
            kc = 1
            bc = 1
            kpts_list = 0
            bands_list = 0

            kpoints00: DO ik=1,nkpoint      ! loop over all selected kpoints
               nk=kpuse(ik)
               ...
               IF (nbmodus <= -2) THEN
                  ...
                  DO ib=1,wdes%nb_tot
                     ...
                     DO isp=1,info%ispin
                        ...
                        IF ((eint(1) <=  eigv) .AND. &
                              (eigv <= eint(2))) THEN
                           IF (.NOT. any(bands_list == ib)) THEN
                              bands_list(bc) = ib
                              bc = bc + 1
                           END IF
                           IF (.NOT. any(kpts_list == nk)) THEN
                              kpts_list(kc) = nk
                              kc = kc + 1
                           END IF
                        END IF
                  ...
                  END DO
               ELSE
                  bands_list(1:nband) = nbuse(1:nband)
                  bc = nband+1
                  kpts_list = kpuse
                  kc = nkpoint+1
               END IF
...
! allocate arrays to store all used bands and kpoints
            ALLOCATE(tkl(kc-1), tbl(bc-1))
            tkl = kpts_list(1:kc-1)
            tbl = bands_list(1:bc-1)
! init the parcharge_h5 output. In this case, in all likelihood, we will have empty combinations of kpts and bands!
            CALL vh5_init_parcharge_output(IH5OUTFILEID, gridc, info%ispin, tbl, tkl)
         END IF
...
! --- now output to the file PARCHG.$nb.$nk or hdf5 output
                IF (io%lparchgh5) THEN

! Find the correct kpoints index to write to
                  kc = 0
                  DO i = 1, size(tkl)
                     IF (tkl(i) == nk) THEN
                        kc = i
                     ...
                     END IF
                  END DO
! Find the correct band index to write to
                  bc = 0
                  DO i = 1, size(tbl)
                     IF (tbl(i) == nb) THEN
                        bc = i
                     ...
                     END IF
                  END DO
                  CALL vh5_write_to_parcharge_array(IH5OUTFILEID, gridc, chtot, bc, kc)
# 475
...
! and write to file PARCHG.$nb,ALLK or hdf5 output
                IF (io%lparchgh5) THEN

                  CALL vh5_write_to_parcharge_array(IH5OUTFILEID, gridc, chtot, ib, 1)
# 533
          ...
          ELSE ! now that is a little bit more difficult, because of dispersion
            ...
            IF (io%lparchgh5) THEN
!first figure out how many bands we have to allocate:

               bc = 1
               bands_list = 0

               bands_check: DO ib=1,wdes%nb_tot

                  DO isp = 1,info%ispin
                     w%fertot(:,:,isp) = 0.
                  END DO

                  kpoints_check: DO ik=1,nkpoint
                     ...
                     END DO
                  END DO kpoints_check
               END DO bands_check
! allocate arrays to store all used bands
               ALLOCATE(tbl(bc-1))
               ALLOCATE(tkl(1))
               tbl = bands_list(1:bc-1)
               tkl = (/0/)
! init the parcharge_h5 output.
               CALL vh5_init_parcharge_output(IH5OUTFILEID, gridc, info%ispin, tbl, tkl)
            END IF
...
! and write to file "PARCHG.nb.ALLK" or hdf5
                   IF (io%lparchgh5) THEN

                      CALL vh5_write_to_parcharge_array(IH5OUTFILEID, gridc, chtot, bc, 1)
                      bc = bc + 1
# 629
...
! and write it to the file PARCHG.ALLB.$nk or hdf5
             IF (io%lparchgh5) THEN

                CALL vh5_write_to_parcharge_array(IH5OUTFILEID, gridc, chtot, 1, ik)
# 718
...
! copy charge density to second channel
          ...
          ENDIF
! write to the PARCHG file or hdf5
          IF (io%lparchgh5) THEN

            ALLOCATE(tkl(1), tbl(1))
            tbl = (/0/)
            tkl = (/0/) 
            CALL vh5_init_parcharge_output(IH5OUTFILEID, gridc, info%ispin, tbl, tkl)
            CALL vh5_write_to_parcharge_array(IH5OUTFILEID, gridc, chtot, 1, 1)
# 855
...
! as this is a tool to analyse a allready cenverged system: CALL M_stop('VASP aborting ...'); stop the
...
!
    SUBROUTINE read_pard()
      ...
      USE vhdf5_base, ONLY: incar_found, hdf5_found
...
! first get status of the input file:
      ...
      IF ((.NOT.incar_found) .and. (.not.hdf5_found)) THEN
         IF (io%lopen) CALL WFORCE(io%iu6)
         CALL vtutor%error("Can't open input file " // filein)
      END IF

      CALL open_incar_if_found(iupar, lopen)
# 944


! check for noncollinear calculation for which partial charges are not yet implemented
      lnoncollinear = .FALSE.
      CALL process_incar(lopen, io%iu0, iupar, 'LNONCOLLINEAR', lnoncollinear, ierr1, lcontinue=.true.)
      IF(ierr1/=0) CALL process_incar(lopen, iupar, iupar, 'LSORBIT', lnoncollinear, ierr1)

      IF (lnoncollinear) THEN
         CALL vtutor%error("Partial charge densities are not yet implemented for noncollinear calculations.")
      END IF



! try to find band index
      CALL process_incar(lopen, io%iu0, iupar, 'IBAND', nbuse, wdes%nb_tot, ierr1, writexmlincar, lcontinue=.true., foundnumber=ndat)
      IF(ierr1/=0) CALL process_incar(lopen, io%iu0, iupar, 'IBAND', nbuse, MAX(1,ndat), ierr1, writexmlincar, foundnumber=ndat)
      ...
      CASE (0)        ! IBAND found
         ...
         nband = nbmodus
! check if any band is larger than wdes%nb_tot
         IF (ANY(nbuse(1:nband) > wdes%nb_tot)) THEN
            CALL vtutor%error("One or more band numbers passed in IBAND are larger than the total number &
            &of bands in WAVECAR. Please adjust your input and restart.")
         END IF

! read nbmodus and look for a consitents
         CALL process_incar(lopen, io%iu0, iupar, 'NBMOD', idummy, 1, ierr2, writexmlincar, foundnumber=ndat)
         ...
         CASE DEFAULT err_nbmod1
            IF (io%lopen) CALL WFORCE(io%iu6)
            CALL err_handle(ierr3,filein,"NBMOD","I")
      ...
      CASE (3)        ! IBAND not found, search for other information
         ...
         CASE (0)  ! nbmod found
            ...
            CASE (-3) nbmod      ! read eint w.r.t. fermi energy
               nbmodus = -3
               CALL process_incar(lopen, io%iu0, iupar, 'EINT', eint, 2, ierr3, writexmlincar, lcontinue=.true., foundnumber=ndat)
               IF(ierr3/=0) CALL process_incar(lopen, io%iu0, iupar, 'EINT', eint, 1, ierr3, writexmlincar, foundnumber=ndat)
...
! first proff: are there two values ?
                  ...
                  ELSE
                     ...
                     IF(lop6) THEN
                        ...
                        WRITE(TIU6,'(A,A,A,2F8.3)') &
                             "Energy range w.r.t. Fermi energy from ",filein,": ", &
                             eint
               ...
               CASE DEFAULT eint1
                  IF (io%lopen) CALL WFORCE(io%iu6)
                  CALL err_handle(ierr3,filein,"EINT","F")
               END SELECT eint1
      ...
      CASE DEFAULT iband
         ...
         IF (io%lopen) CALL WFORCE(io%iu6)
         CALL err_handle(ierr1,filein,"IBAND","I")
      END SELECT iband

! reading k modus
      CALL process_incar(lopen, io%iu0, iupar, 'KPUSE', kpuse, wdes%nkpts, ierr1, writexmlincar, lcontinue=.true., foundnumber=ndat)
      IF(ierr1/=0) CALL process_incar(lopen, io%iu0, iupar, 'KPUSE', kpuse, MAX(1,ndat), ierr1, writexmlincar, foundnumber=ndat)

      kmod: SELECT CASE (ierr1)

      CASE (0) kmod ! kpoints found
! check if any kpoint is larger than wdes%nkpts
         IF (ANY(kpuse(1:ndat) > wdes%nkpts)) THEN
            CALL vtutor%error("One or more kpoints passed in KPUSE are larger than the total number &
            &of kpoints in WAVECAR. Please adjust your input and restart.")
         END IF
         IF(lop6) THEN
      ...
      CASE DEFAULT kmod
         IF (io%lopen) CALL WFORCE(io%iu6)
         CALL err_handle(ierr1,filein,"KPUSE","I")

      END SELECT kmod

      CALL process_incar(lopen, io%iu0, iupar, 'LSEPK', ldummy, 1, ierr1, writexmlincar, foundnumber=ndat)
      ...
      CASE DEFAULT sepk
         IF (io%lopen) CALL WFORCE(io%iu6)
         CALL err_handle(ierr1,filein,"LSEPK","L")
      END SELECT sepk

      CALL process_incar(lopen, io%iu0, iupar, 'LSEPB', ldummy, 1, ierr1, writexmlincar, foundnumber=ndat)
      ...
      CASE DEFAULT sepb
         IF (io%lopen) CALL WFORCE(io%iu6)
         CALL err_handle(ierr1,filein,"LSEPB","L")
      END SELECT sepb
...
! for security: check if eint(1) < eint(2)
      ...
      IF (nbmodus <= -2) THEN
         ...
         END IF
      END IF
      CALL close_incar_if_found(iupar)
...
!
    ...
    SUBROUTINE err_handle(ierr,file,keyword,type)
      ...
      CASE (1)
         CALL vtutor%error(msg // "Didn't find free I/O unit to open file " // file // ".")

      CASE (2)
         CALL vtutor%error(msg // "Can't open file " // file // ".")

      CASE (3)
         CALL vtutor%error(msg // "Didn't find tag " // keyword // ".")

      CASE (4)
         CALL vtutor%error(msg // "Invalid data typ for tag " // keyword // "." // strtype // " recommended.")

      CASE (5)
         CALL vtutor%error(msg // "Error reading/parsing data list for tag " // keyword // ". &
             &Check format! Data type recommended: " // strtype)

      CASE (6)
         CALL vtutor%error(msg // "Cannot open scratch file for conversion of data for tag" // keyword // ".")

      CASE DEFAULT
         CALL vtutor%error(msg // "Unknown error code !")
