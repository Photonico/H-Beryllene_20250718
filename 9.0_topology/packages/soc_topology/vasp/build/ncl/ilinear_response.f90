...
!*********************************************************************
  ...
  SUBROUTINE LR_MAIN( &
          ...
          T_INFO,T_INFO_0,DYN,INFO,XC,IO,MIX,KPOINTS,SYMM,GRID,GRID_SOFT, &
    ...
    USE pot_struct_def 
    USE pot_electrostat, only: pothar
    IMPLICIT NONE
!=======================================================================
!  structures
!=======================================================================
    ...
    REAL(q) TOTEN, EFERMI, EFERMI0, EFERMI1
...
# 143
    ...
    REAL(q) :: CSHIFT=0.010_q            ! complex shift
...
! belong to different symmetry classes
...
! when shifts are used
    ...
    REAL(q) :: RMST(INFO%NELM)           ! magnitude of charge density residuum
    ...
    INTEGER ISP
    REAL(q)             :: POTIM
    REAL(q)             :: RMSC,RMSP,WEIGHT,STEP,STEP_W,DE2H ! mixer
    REAL(q)             :: Z(3)
    INTEGER             :: IERRBR
    REAL(q)             :: SCALE=0.02_q
...
! forces and stress tensors
    ...
    REAL(q)   FBREAK_MEAN
! potentially (1._q,0._q) might default CSHIFT to LCSHIFT/5 (LCSHIFT defaults to 1.0)
!    CSHIFT=LCSHIFT/5
...
!=======================================================================
    ...
    INFO%LCHCON=.FALSE.
    FBREAK_MEAN=0.0        ! does not apply FBREAK_MEAN in first iteration
...
!  and derivatives of non local strength (CDIJ1)
!
!=======================================================================
       CALL START_TIMING("G")
...
! positive displacement
       ...
       CALL POT_DER4( KINEDEN, HAMILTONIAN, WDES, GRID, GRIDC, GRIDUS, GRID_SOFT, C_TO_US, SOFT_TO_C, &
            INFO, XC, T_INFO, P,  LATT_CUR, E1, E2, CSTRF, CSTRF1, CSTRF2, CSTRF3, CSTRF4, &
            CHDEN, CHTOT, CHTOT1, CHTOTL, DENCOR, CVTOT1, CWORK, SV1, SV2, & 
...
!       CALL POTXC2(GRIDC,LATT_CUR,XC,CHTOT,DENCOR,CHTOT1,CWORK,E2%XCENC)
!       WRITE(*,*) E2%XCENC,E1%XCENC,E2%XCENC-E1%XCENC
! Hartree and LDA contribution
       CALL POTHAR(GRIDC,LATT_CUR,HAMILTONIAN%COULOMB_POT,CHTOT1-CHTOT2,CWORK,E1%DENC)
       CALL POTXC2(GRIDC,LATT_CUR,XC,CHTOT,DENCOR,CHTOT1-CHTOT2,CWORK,E1%XCENC)
...
!  the first order change of the energy e(1) is returned in WXI%CELEN
!  the first order change of the norm <phi(0)|S(1)|phi(0)> in WXI%FERWE
!
!=======================================================================
       ...
       CALL LINEAR_RESPONSE_DIIS(GRID,INFO,LATT_CUR,NONLR_S,NONL_S,W1,WXI,W0,WDES, &
            LMDIM,CDIJ,CQIJ, RMS(N),DESUM,ICOUEV, SV, CSHIFT, IO%IU6,IO%IU0, LRESET=(N==1), FBREAK_MEAN_IN= FBREAK_MEAN)

  CALL MRG_CEL(WDES,W1)
!!  CALL MRG_CELTOT(WDES,W1)
...
!$ACC UPDATE SELF(W1%CELTOT,W1%FERTOT,WXI%CELTOT,WXI%FERTOT) 
!$ACC UPDATE SELF(W1%CELEN,W1%FERWE,WXI%CELEN,WXI%FERWE) 
...
! normalize phi(1) by adding -1/2*<phi(0)| S(1) |phi(0)> |phi(0)>
...
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ALLOW)
...
!  broyden mixing ... :
             CALL BRMIX(KINEDEN,GRIDB,GRIDC,IO,MIX,XC,B_TO_C, &
                  ...
                  DE2H,RMST(N),RMSC,RMSP,WEIGHT,.TRUE.,IERRBR)
             MIX%LRESET=.FALSE.
          ELSE
!  simple mixing ... :
             RMST(N)=0
             CALL MIX_SIMPLE(GRIDC,MIX,WDES%NCDIJ, CHTOT1,CHTOTL, &
                  N_MIX_PAW, RHOLM1, RHOLM_LAST, LATT_CUR%B, LATT_CUR%OMEGA, DE2H, RMST(N))
          ENDIF
          CALL STOP_TIMING("G",IO%IU6,"MIXING")
          ! "mixing is ok"

          IF (IO%IU0>=0) THEN
             WRITE(IO%IU0,300) RMST(N)*SCALE
             WRITE(17,300) RMST(N)*SCALE
          ENDIF
300       FORMAT('   ',E10.3)
       ELSE
          IF (IO%IU0>=0) THEN
...
! rms stable for four steps, ok stop, no way to improve
       IF(N>RESOLVE_DEG_NTIMES+4) THEN
          
! scf: stagnation of charge density mixer for 3 steps
          IF (MIX%IMIX/=0) THEN
               IF (ABS((RMST(N)-RMST(N-1))/MIN(RMST(N),RMST(N-1),RMST(N-2),RMST(N-3)))<0.2_q &
             .AND. ABS((RMST(N)-RMST(N-2))/MIN(RMST(N),RMST(N-1),RMST(N-2),RMST(N-3)))<0.2_q & 
             .AND. ABS((RMST(N)-RMST(N-3))/MIN(RMST(N),RMST(N-1),RMST(N-2),RMST(N-3)))<0.2_q) THEN
                  INFO%LABORT=.TRUE.
               ENDIF
          ELSE IF (ABS((RMS(N)-RMS(N-1))/RMS(N))<0.1_q .AND. ABS((RMS(N)-RMS(N-2))/RMS(N))<0.1_q .AND. ABS((RMS(N)-RMS(N-3))/RMS(N))<0.1_q) THEN
! no scf: stagnation of eigenvalue solver for 3 steps
               INFO%LABORT=.TRUE.
          ENDIF
       ENDIF
!-----do not stop before minimum number of iterations is reached
       IF(MIX%IMIX/=0 .AND. N>=RESOLVE_DEG_NTIMES .AND. ABS(RMST(N))*SCALE <1E-8)  INFO%LABORT=.TRUE.
...
! mixing related output
      IF ( INFO%LMIX .AND. MIX%IMIX==4 ) THEN
        ...
        IF (ABS(RMST(N)-RMSC)/RMST(N)> 0.1_q) THEN
...
!$ACC UPDATE DEVICE(W1%CELTOT,W1%FERTOT,WXI%FERTOT) ASYNC(ACC_ASYNC_Q) IF(OFFLOAD_ALLOW)
!$ACC UPDATE DEVICE(W1%CELEN,W1%FERWE,WXI%FERWE) ASYNC(ACC_ASYNC_Q) IF(OFFLOAD_ALLOW)
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ALLOW)
...
!-----------------------------------------------------------------------
    CALL RPA_ZERO_POT( CVTOT1, CDIJ1 )

    IF ( IO%IU0>=0) THEN
       WRITE(*,'(A,E14.5)') ' first order change of Fermi-energy',EFERMI1
    ENDIF
...
!! 
!$ACC KERNELS PRESENT(W0,W1,WXI) 
       WXI%CPTWFP    =W0%CPTWFP    +STEP_W* W1%CPTWFP
       WXI%CPROJ =W0%CPROJ +STEP_W* W1%CPROJ
       WXI%CELTOT=W0%CELTOT+STEP_W* W1%CELTOT
       WXI%FERTOT=W0%FERTOT+STEP_W* W1%FERTOT
!$ACC END KERNELS
!! 
!$ACC UPDATE SELF(WXI%CPTWFP,WXI%CPROJ,WXI%CELTOT,WXI%FERTOT) ASYNC(ACC_ASYNC_Q) IF(OFFLOAD_ALLOW)

       CHTOT2 =CHTOT +CHTOT1*STEP_W
       CTMP   =CRHODE+CRHODE1*STEP_W
       RHOLM_LAST=RHOLM  +RHOLM1*STEP_W
       ...
       IF (.NOT. INFO%LREAL) CALL PHASE(WDES,NONL_S,0)
       ...
       IF (LRPAFORCE) THEN
          CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
               INFO,XC,P,T_INFO,E1,LATT_CUR, &
               CHDEN,CHTOT2,CSTRF1,CVTOT1,DENCOR,SV1,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)

! subtract from potential the change of the Fermi-energy: CVTOT1=CVTOT1-STEP_W*EFERMI1
         CALL RL_ADD_CONSTANT( CVTOT1(1,1), GRIDC, WDES%NCDIJ, -EFERMI1*STEP_W)
         ...
         CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, &
              ...
              E1, LASPH=INFO%LASPH, LCOREL= .FALSE.  )
! store the first derivative of the total self-consistent Kohn-Sham potential for this displacement
!        CALL RPA_ADD_TO_POT( CVTOT1, CDIJ1, E1%EENTROPY*STEP_W, WEIGHT) ! entropy change from this routine
         CALL RPA_ADD_TO_POT( CVTOT1, CDIJ1, 0.0_q, WEIGHT) ! entropy change from chi_super.F
       ENDIF
...
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ALLOW)
    ...
    CALL RPA_FORCE_CHARGE( W0%WDES, T_INFO, GRIDC, ION, IDIR )
...
! \sum_j beta_j* (<p_j | d phi_nk/d R_i> + <d p_j/d R_i | phi_nk >)
    ...
    EFERMI=EFERMI0  ! restore Fermi level
...
!=======================================================================
!
!> Small helper routine to add a scalar to say CVTOT
!
!=======================================================================

  SUBROUTINE RL_ADD_CONSTANT( C, GRID, ISPIN, EFERMI)
    USE mgrid
    IMPLICIT NONE
    TYPE (grid_3d)     GRID       !< grid for wavefunctions
    INTEGER :: ISPIN              !< spin degrees of freedom (1,2 or 4)
    COMPLEX(q) :: C(GRID%MPLWV, ISPIN)
    REAL(q) :: EFERMI
! local
    INTEGER :: N
    DO N=1, ISPIN
       C(1:GRID%RL%NP, N)=C(1:GRID%RL%NP, N)+EFERMI
    ENDDO
  END SUBROUTINE RL_ADD_CONSTANT
