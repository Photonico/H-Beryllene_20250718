...
!> a previous implementation present in VASP 1._q by Gerald Jordan
...
!> base type to perform wannier interpolation
  TYPE wannier_interpolator
     ...
     COMPLEX(q), ALLOCATABLE :: U(:,:,:,:)              !< wannier rotation matrices
     COMPLEX(q), ALLOCATABLE :: HAM(:,:,:,:)            !< Hamiltonian on coarse grid
     COMPLEX(q), ALLOCATABLE :: CDER(:,:,:,:,:)         !< Optical matrix elements on coarse grid in the Wannier basis
...
!************************************************************************
  SUBROUTINE HAM_DIAG(NBANDS,NKPTS,NSPIN,HAM,EIG)
    ...
    COMPLEX(q),INTENT(IN) :: HAM(NBANDS,NBANDS,NKPTS,NSPIN)
...
!COMPLEX(q), ALLOCATABLE :: DHDK_KPATH(:,:,:,:,:)
    ...
    COMPLEX(q), ALLOCATABLE :: CHAM_KPATH(:,:,:,:)
    COMPLEX(qs), ALLOCATABLE :: CHAMS_KPATH(:,:,:,:,:)
    COMPLEX(qs) :: C(WANN_INTERP%NW,WANN_INTERP%NW), D(WANN_INTERP%NW,WANN_INTERP%NW), U(WANN_INTERP%NW,WANN_INTERP%NW)
...
# 746
    ...
    COMPLEX(q) :: HAM(:,:,:,:) !< Hamiltonian in reciprocal space
...
!> @brief Transform from real space (R) to reciprocal space (K) using slow FFT
  SUBROUTINE FOURIER_INTERPOL_RTOK(HAMR,VKPT_INT,HAM_INT,LAT,DHDK_INT)
    ...
    COMPLEX(q) :: HAM_INT(:,:,:,:) !< Interpolated Hamiltonian
    TYPE (latt) :: LAT !< lattice type
    COMPLEX(q), OPTIONAL :: DHDK_INT(:,:,:,:,:) !< Calculate derivatives
! local variables
