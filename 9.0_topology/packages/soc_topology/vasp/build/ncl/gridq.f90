...
!***********************************************************************
  ...
  TYPE gridquant
     COMPLEX(q), POINTER :: C(:,:) => NULL()   ! pointer to the multidimensional gridquantity
     REAL(q), POINTER    :: R(:,:) => NULL()   ! pointer to the same storage position (real)
     COMPLEX(q), POINTER      :: RG(:,:) => NULL()  ! pointer to the same storage position (rgrid)
     TYPE (grid_3d), POINTER :: GRID => NULL()
...
!=======================================================================
  ...
  SUBROUTINE ALLOCATE_GRID_QUANTITY( GQ, GRID, NCDIJ)
    USE base, ONLY: CAST_COMPLEX_TO_REAL, CAST_COMPLEX_TO_RGRID
    TYPE (gridquant) GQ
...
! alias the real pointer
    CALL CAST_COMPLEX_TO_REAL(GQ%C, GQ%R)
    CALL CAST_COMPLEX_TO_RGRID(GQ%C, GQ%RG)
...
!! USE moffload
    ...
    IF (.NOT.GQ%WAS_ALLOCATED) THEN
       CALL vtutor%bug("internal error in DEALLOCATE_GRID_QUANTITY: gridquant not properly allocated", "gridq.F", 258)
    ENDIF
    IF (ASSOCIATED(GQ%RG)) THEN
!$ACC EXIT DATA DELETE(GQ%RG) IF(OFFLOAD_ON)
       NULLIFY(GQ%RG)
    ENDIF
    IF (ASSOCIATED(GQ%C)) THEN
!$ACC EXIT DATA DELETE(GQ%C) IF(OFFLOAD_ON)
       DEALLOCATE(GQ%C)
    ENDIF
!$ACC EXIT DATA DELETE(GQ) IF(OFFLOAD_ON)
...
!=======================================================================
  ...
  SUBROUTINE ALLOCATE_GRID_QUANTITY_FORCE_RL( GQ, GRID, GRID2, NCDIJ)
    USE base, ONLY: CAST_COMPLEX_TO_REAL
!! USE moffload
    ...
    GQ%NCDIJ=NCDIJ
    ALLOCATE( GQ%C(GRID%MPLWV, NCDIJ))
!$ACC ENTER DATA CREATE(GQ,GQ%C) IF(OFFLOAD_ON)
! alias the real pointer
    CALL CAST_COMPLEX_TO_REAL(GQ%C, GQ%R)
# 298

    GQ%RG=>GQ%C
!$ACC ENTER DATA COPYIN(GQ%RG) IF(OFFLOAD_ON)
...
!=======================================================================
  ...
  SUBROUTINE SUBINDEX_GQ(GQR, GQ, I1, I2)
    ...
    IF (I1<1) THEN
       CALL vtutor%bug("internal error in SUBINDEX_GQ: first index too small " // str(I1), "gridq.F", 354)
    ENDIF
    IF (I2>GQ%NCDIJ) THEN
       CALL vtutor%bug("internal error in SUBINDEX_GQ: second index too large " // str(I2), "gridq.F", 357)
    ENDIF
    GQR%GRID => GQ%GRID
...
!=======================================================================
  ...
  SUBROUTINE CHECK_INITIALISED_GQ( GQ)
    ...
    IF (.NOT. GQ%IS_INITIALISED) THEN
       CALL vtutor%bug("internal error in CHECK_INITIALISED_GQ: a grid quantity is not initialised", "gridq.F", 388)
    ENDIF
...
!=======================================================================
  ...
  SUBROUTINE CHECK_ASSOCIATED_GQ( GQ)
    ...
    IF (.NOT. ASSOCIATED(GQ%GRID)) THEN
       CALL vtutor%bug("internal error in CHECK_INITIALISED_GQ: grid pointer is not associated", "gridq.F", 402)
    ENDIF
    IF (.NOT. ASSOCIATED(GQ%C)) THEN
       CALL vtutor%bug("internal error in CHECK_INITIALISED_GQ: array is not associated", "gridq.F", 405)
    ENDIF
...
!=======================================================================
  ...
  SUBROUTINE CHECK_CONFORM_GQ( GQ1, GQ2)
    ...
    IF (GQ1%REALSPACE .NEQV. GQ2%REALSPACE) THEN
       CALL vtutor%bug("internal error in CHECK_CONFORM: presentation not conform " // &
          str(GQ1%REALSPACE) // " " // str(GQ2%REALSPACE), "gridq.F", 422)
    ENDIF
    IF (GQ1%REALSPACE) THEN
       IF (GQ1%REAL_IN_REALSPACE .NEQV. GQ2%REAL_IN_REALSPACE) THEN
          CALL vtutor%bug("internal error in CHECK_CONFORM: real presentation not conform " // &
             str(GQ1%REAL_IN_REALSPACE) // " " // str(GQ2%REAL_IN_REALSPACE), "gridq.F", 427)
       ENDIF
       IF (GQ1%GRID%RL%NP /= GQ2%GRID%RL%NP) THEN
          CALL vtutor%bug("internal error in CHECK_CONFORM: real space not conform " // &
             str(GQ1%GRID%RL%NP) // " " // str(GQ2%GRID%RL%NP), "gridq.F", 431)
       ENDIF
    ELSE
       IF (GQ1%GRID%RC%NP /= GQ2%GRID%RC%NP) THEN
          CALL vtutor%bug("internal error in CHECK_CONFORM: reciprocal space not conform " // &
             str(GQ1%GRID%RC%NP) // " " // str(GQ2%GRID%RC%NP), "gridq.F", 436)
       ENDIF
    ENDIF
    IF (GQ1%NCDIJ /= GQ2%NCDIJ) THEN
       CALL vtutor%bug("internal error in CHECK_CONFORM: last dimensions are not conform " // &
          str(GQ1%NCDIJ) // " " // str(GQ2%NCDIJ), "gridq.F", 441)
    ENDIF
    IF (GQ1%NCDIJ/=1) THEN
       IF (GQ1%SPINOR_REPRESENTATION .NEQV. GQ2%SPINOR_REPRESENTATION) THEN
          CALL vtutor%bug("internal error in CHECK_CONFORM: spinor presentation not conform " // &
             str(GQ1%SPINOR_REPRESENTATION) // " " // str(GQ2%SPINOR_REPRESENTATION), "gridq.F", 446)
       ENDIF
...
!=======================================================================
  ...
  SUBROUTINE CHECK_SLOPPY_CONFORM_GQ( GQ1, GQ2)
    ...
    IF (GQ1%REALSPACE .NEQV. GQ2%REALSPACE) THEN
       CALL vtutor%bug("internal error in CHECK_CONFORM: presentation not conform " // &
          str(GQ1%REALSPACE) // " " // str(GQ2%REALSPACE), "gridq.F", 466)
    ENDIF
    IF (GQ1%REALSPACE) THEN
       IF (GQ1%GRID%RL%NP /= GQ2%GRID%RL%NP) THEN
          CALL vtutor%bug("internal error in CHECK_CONFORM: real space not conform " // &
             str(GQ1%GRID%RL%NP) // " " // str(GQ2%GRID%RL%NP), "gridq.F", 471)
       ENDIF
    ELSE
       IF (GQ1%GRID%RC%NP /= GQ2%GRID%RC%NP) THEN
          CALL vtutor%bug("internal error in CHECK_CONFORM: reciprocal space not conform " // &
             str(GQ1%GRID%RL%NP) // " " // str(GQ2%GRID%RL%NP), "gridq.F", 476)
       ENDIF
    ENDIF
...
! local
    ...
    IF (.NOT. GQ1%REALSPACE) THEN
       CALL vtutor%bug("internal error in SUMRL_GQ: operates in real space only GQ1 " // &
          str(GQ1%REALSPACE), "gridq.F", 540)
    ENDIF
...
!$ACC UPDATE SELF(GQ1%RG) IF_PRESENT 
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
       GQ2=GQ1
    ELSE
       IF (GQ2%GRID%RL%NP /=0) THEN
          CALL vtutor%bug("internal error in SUMRL_GQ: the destination grid contains data on nodes " &
             // "where no operation is performed", "gridq.F", 561)
       ENDIF
    ENDIF
...
! local
    ...
    IF (.NOT. GQ1%REALSPACE) THEN
       CALL vtutor%bug("internal error in BROAD_CAST_GQ: operates in real space only GQ1 " // &
          str(GQ1%REALSPACE), "gridq.F", 585)
    ENDIF
    ...
    ELSE
       IF (GQ1%GRID%RL%NP /=0) THEN
          CALL vtutor%bug("internal error in BROAD_CAST_GQ: the source grid contains data on nodes " &
             // "where no operation is performed", "gridq.F", 593)
       ENDIF
    ENDIF
...
! local
    ...
    IF ( GQ%REAL_IN_REALSPACE .NEQV. GQ%GRID%LREAL) THEN
       CALL vtutor%bug("FFT_GQ: FFT using an enforced grid","gridq.F",633)
    ENDIF
...
! local
    ...
    CALL CHECK_INITIALISED_GQ(GQ)
    IF (GQ%REALSPACE) THEN
       CALL vtutor%bug("internal error in SETUNB_GQ: operates in reciprocal space only", "gridq.F", 668)
    ENDIF
...
!=======================================================================
  ...
  FUNCTION GQ_PLUS_DP( GQ1, DP)
    ...
    IF (DP%SCALE2/=0) THEN
       CALL vtutor%bug("internal error in GQ_PLUS_DP: only operations of the kind \n GQ = GQ1 * " &
          // "SCALE + GQ2* SCALE can be coded", "gridq.F", 743)
    ENDIF
  ...
  FUNCTION DP_PLUS_GQ(  DP, GQ1)
    ...
    IF (DP%SCALE2/=0) THEN
       CALL vtutor%bug("internal error in DP_PLUS_GQ: only operations of the kind \n GQ = GQ1 * " &
          // "SCALE + GQ2* SCALE can be coded", "gridq.F", 761)
    ENDIF
  ...
  FUNCTION GQ_MINUS_DP( GQ1, DP)
    ...
    IF (DP%SCALE2/=0) THEN
       CALL vtutor%bug("internal error in GQ_MINUS_DP: only operations of the kind \n GQ = GQ1 * " &
          // "SCALE + GQ2* SCALE can be coded", "gridq.F", 779)
    ENDIF
  ...
  FUNCTION DP_MINUS_GQ(  DP, GQ1)
    ...
    IF (DP%SCALE2/=0) THEN
       CALL vtutor%bug("internal error in DP_MINUS_GQ: only operations of the kind \n GQ = GQ1 * " &
          // "SCALE + GQ2* SCALE can be coded", "gridq.F", 797)
    ENDIF
  ...
  FUNCTION DP_PLUS_DP( DP1, DP2)
    ...
    IF (DP1%SCALE2/=0 .OR. DP2%SCALE2/=0 ) THEN
       ...
       CALL vtutor%bug("internal error in DP_PLUS_DP: only operations of the kind \n GQ = GQ1 * " &
          // "SCALE + GQ2* SCALE can be coded", "gridq.F", 815)
    ENDIF
  ...
  FUNCTION DP_MINUS_DP( DP1, DP2)
    ...
    IF (DP1%SCALE2/=0 .OR. &
       ...
       CALL vtutor%bug("internal error in GQ_MINUS_DP: only operations of the kind \n GQ = GQ1 * " &
          // "SCALE + GQ2* SCALE can be coded", "gridq.F", 834)
    ENDIF
  ...
  FUNCTION DM_PLUS_DP( DM, DP)
    ...
    IF ( DM%SCALE3 /=0) THEN
       CALL vtutor%bug("internal error in DM_PLUS_DP: only operations of the kind \n GQ = GQ1 * GQ2 " &
          // "*SCALE + GQ3 * SCALE can be coded", "gridq.F", 937)
    ENDIF

    IF ( DP%SCALE2 /=0) THEN
       CALL vtutor%bug("internal error in DM_PLUS_DP: only operations of the kind \n GQ = GQ1 * GQ2 " &
          // "*SCALE + GQ3 * SCALE can be coded", "gridq.F", 942)
    ENDIF
...
!=======================================================================
  ...
  SUBROUTINE GQ_ASSIGN_DP( GQ1, DP)
    TYPE (gridquant), TARGET, INTENT(INOUT) :: GQ1
    TYPE (delayed_plus), INTENT(IN) :: DP
...
!=======================================================================
  ...
  SUBROUTINE ADD_GRID_QUANTITY( GQ1, SCALE1, GQ2, SCALE2,  GQR )
    TYPE (gridquant), TARGET, INTENT(INOUT) :: GQR
    TYPE (gridquant), TARGET, INTENT(IN)  :: GQ1, GQ2
...
! check association and initialisation status
    ...
    IF (.NOT. GQ1%REALSPACE .OR. .NOT. GQ2%REALSPACE ) THEN
       CALL vtutor%bug("internal error in CONGMUL_GRID_QUANTITY: operates in real space only GQ1, " &
          // "GQ2 " // str(GQ1%REALSPACE) // " " // str(GQ2%REALSPACE), "gridq.F", 1086)
    ENDIF
    CALL CHECK_ASSOCIATED_GQ(GQR)
...
! entire check of third quantity which might be aliased to GQR
    IF (SCALE3 /=0 ) THEN
       ...
       IF (.NOT. GQ3%REALSPACE) THEN
          CALL vtutor%bug("internal error in CONGMUL_GRID_QUANTITY: operates in real space only GQ3", "gridq.F", 1094)
       ENDIF
       GQR%REALSPACE=.TRUE.
...
! set result
    ...
    IF ( GQ1%NCDIJ*GQ2%NCDIJ /= GQR%NCDIJ ) THEN
       CALL vtutor%bug("internal error in CONJGMUL_GRID_QUANTITY: not conform " // str(GQ1%NCDIJ) &
          // " " // str(GQ2%NCDIJ) // " " // str(GQR%NCDIJ), "gridq.F", 1110)
    ENDIF
...
! check association and initialisation status
    ...
    CALL CHECK_INITIALISED_GQ(GQ2)
    IF (.NOT. GQ1%REALSPACE .OR. .NOT. GQ2%REALSPACE ) THEN
       CALL vtutor%bug("internal error in MUL_GRID_QUANTITY: operates in real space only GQ1 or GQ2", "gridq.F", 1168)
    ENDIF
    CALL CHECK_ASSOCIATED_GQ(GQR)
...
! entire check of third quantity which might be aliased to GQR
    IF (SCALE3 /=0 ) THEN
       CALL CHECK_INITIALISED_GQ(GQ3)
       IF (.NOT. GQ3%REALSPACE) THEN
          CALL vtutor%bug("internal error in MUL_GRID_QUANTITY: operates in real space only GQ3", "gridq.F", 1176)
       ENDIF
       GQR%REALSPACE=.TRUE.
       CALL CHECK_CONFORM_GQ( GQR, GQ3)
    ELSE
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C)
       DO N=1,NP
          C(N)=0
       ENDDO
!$OMP END PARALLEL DO
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,A)
       DO N=1,NP
          C(N)=A(N)
       ENDDO
!$OMP END PARALLEL DO
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,A,SCALE1)
       DO N=1,NP
          C(N)=A(N)*SCALE1
       ENDDO
!$OMP END PARALLEL DO
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,B)
       DO N=1,NP
          C(N)=C(N)+B(N)
       ENDDO
!$OMP END PARALLEL DO
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,B,SCALE2)
       DO N=1,NP
          C(N)=C(N)+B(N)*SCALE2
       ENDDO
!$OMP END PARALLEL DO
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,A,B,SCALE1)
       DO N=1,NP
          C(N)=A(N)*B(N)*SCALE1
       ENDDO
!$OMP END PARALLEL DO
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,A,B,D,SCALE1,SCALE3)
       DO N=1,NP
...
!DIR$ IVDEP
!OCL NOVREC
!$ACC PARALLEL LOOP PRESENT(A,B,C) ASYNC(ACC_ASYNC_Q) &
!$ACC IF(ACC_IS_PRESENT(A,1).AND.ACC_IS_PRESENT(B,1).AND.ACC_IS_PRESENT(C,1).AND.OFFLOAD_ON)
 !$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,A,B,SCALE1)
       DO N=1,NP
          C(N)=A(N)*B(N)*SCALE1
       ENDDO
...
!DIR$ IVDEP
!OCL NOVREC
!$ACC PARALLEL LOOP PRESENT(A,B,C,D) ASYNC(ACC_ASYNC_Q) &
!$ACC IF(ACC_IS_PRESENT(A,1).AND.ACC_IS_PRESENT(B,1).AND.ACC_IS_PRESENT(C,1).AND.ACC_IS_PRESENT(D,1).AND.OFFLOAD_ON)
 !$OMP PARALLEL DO PRIVATE(N) SHARED(NP,C,A,B,D,SCALE1,SCALE3)
       DO N=1,NP
          C(N)=A(N)*B(N)*SCALE1+ D(N)*SCALE3
       ENDDO
 !$OMP END PARALLEL DO
  ...
  SUBROUTINE REAL_CMPLX_SCALE( NP, A, SCALE, B)
    USE prec
    IMPLICIT NONE
    INTEGER :: NP
    REAL(q) :: A(NP)
...
!DIR$ IVDEP
!OCL NOVREC
!$OMP PARALLEL DO PRIVATE(N) SHARED(NP,A,B,SCALE)
    DO N=1,NP
       B(N)=A(N)*SCALE
    ENDDO
!$OMP END PARALLEL DO

  END SUBROUTINE REAL_CMPLX_SCALE


  SUBROUTINE REAL_REAL_SCALE( NP, A, SCALE, B)
    USE prec
    IMPLICIT NONE
    INTEGER :: NP
    REAL(q) :: A(NP), B(NP)
...
!$OMP END PARALLEL DO
  ...
  END SUBROUTINE REAL_REAL_SCALE
