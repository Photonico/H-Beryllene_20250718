...
! for 1 we use RANE_ION which is properly seeded
           V(1,KP)=RANG_ION(RNULL,BMP)
           V(2,KP)=RANG_ION(RNULL,BMP)
           V(3,KP)=RANG_ION(RNULL,BMP)
...
! set here center of BOX
      ...
      DO N=1,NIONS
         ...
         DO  J=1,3
            POS(J)=MODULO(X(J,N)-AMYCEN(J)+60.5,1._q)-0.5
...
!     L= r x v m (store -L in W)
      DO N=1,NIONS
         DO J=1,3
            POS(J)=MODULO(X(J,N)-AMYCEN(J)+60.5,1._q)-0.5-CENTER(J)
            TMP3(J)=V(J,N)
         ENDDO
         CALL DIRKAR(1,POS,A)
...
!*********************************************************************
      ...
      DO 50 NI=1,NIONS
        TMP(1)=MODULO(POSION(1,NI)-POSIOL(1,NI)+1.5_q,1._q)-.5_q
        TMP(2)=MODULO(POSION(2,NI)-POSIOL(2,NI)+1.5_q,1._q)-.5_q
        TMP(3)=MODULO(POSION(3,NI)-POSIOL(3,NI)+1.5_q,1._q)-.5_q
        CALL DIRKAR(1,TMP,A)
        ED1=ED1 &
...
!----- keep ions in unit cell (Robin Hirschl)
         POSION(1,NI)= MODULO(POSION(1,NI),1._q)
         POSION(2,NI)= MODULO(POSION(2,NI),1._q)
         POSION(3,NI)= MODULO(POSION(3,NI),1._q)
...
!    previous step was trial step than give long output
   ...
   25 FORMAT(' opt : ',F8.4,'  next Energy=',F13.6, &
     &       ' (dE=',E10.3,')')
!    do not make another call to ZBRENT if required accuracy was reached
...
!*********************************************************************
      ...
      SUBROUTINE PCDAT_PACO(WEIGHT,T_INFO,PACO,AC,XC,COMM)
      USE base
      USE prec
      USE lattice
      USE poscar
      USE mpimy
      USE tutor, ONLY: vtutor, isError, argument
      USE string, ONLY: str

      IMPLICIT NONE
      REAL(q) :: WEIGHT          ! weight of this configuration
      TYPE (type_info) :: T_INFO
      TYPE (paco_struct)  PACO
      REAL(q) AC(3,3)            ! Bravais lattice
      REAL(q) XC(3,T_INFO%NIONS) ! positions
      TYPE (communic) :: COMM
! local
      TYPE (latt)::       LATT_CUR
      INTEGER I1MAX, I2MAX, I3MAX, IADD, I1, I2, I3, NI, NII, INDEX
      INTEGER NT, NTT,  INDEX_COMBINED, INDEX_MIN, INDEX_MAX
      REAL(q) APACO2, SCALE, D1, D2, D3, R2

      LATT_CUR%A=AC
      CALL LATTIC(LATT_CUR)

      PACO%SMEANP =PACO%SMEANP+WEIGHT
      PACO%OMEGA  =PACO%OMEGA +LATT_CUR%OMEGA *WEIGHT
!=======================================================================
!  IXMAX,Y,Z defines the maximum number of cells over which the
!  pair-correlation-function is evaluted
!=======================================================================
      I1MAX=AINT(PACO%APACO*LATT_CUR%BNORM(1)-.001_q)
      I2MAX=AINT(PACO%APACO*LATT_CUR%BNORM(2)-.001_q)
      I3MAX=AINT(PACO%APACO*LATT_CUR%BNORM(3)-.001_q)

      APACO2=PACO%APACO**2
      SCALE =PACO%NPACO/PACO%APACO
!=======================================================================
...
!=======================================================================
      ...
      DO NI=1,T_INFO%NIONS
      NT=T_INFO%ITYP(NI)
      DO NII=NI+1,T_INFO%NIONS

         IF (MOD(NII,COMM%NCPU).NE.COMM%NODE_ME-1) CYCLE

         NTT=T_INFO%ITYP(NII)
! determine flat index for lower triangular "matrix"
         INDEX_COMBINED=((2*T_INFO%NTYP-(NT-2))*(NT-1))/2+(NTT-NT)+1
         INDEX_MIN=MIN(INDEX_MIN, INDEX_COMBINED)
         INDEX_MAX=MAX(INDEX_MAX, INDEX_COMBINED)
         
         D1= I1+MODULO(XC(1,NI)-XC(1,NII),1._q)
         D2= I2+MODULO(XC(2,NI)-XC(2,NII),1._q)
         D3= I3+MODULO(XC(3,NI)-XC(3,NII),1._q)
         R2= (D1*LATT_CUR%A(1,1)+D2*LATT_CUR%A(1,2)+D3*LATT_CUR%A(1,3)) **2 &
     &        + (D1*LATT_CUR%A(2,1)+D2*LATT_CUR%A(2,2)+D3*LATT_CUR%A(2,3)) **2 &
     &        + (D1*LATT_CUR%A(3,1)+D2*LATT_CUR%A(3,2)+D3*LATT_CUR%A(3,3)) **2

         IF (R2<APACO2) THEN
            INDEX=INT(SQRT(R2)*SCALE)
            PACO%SIPACO(INDEX,INDEX_COMBINED)=PACO%SIPACO(INDEX,INDEX_COMBINED)+IADD*WEIGHT
         ENDIF
      ENDDO
      ENDDO
      ENDDO
      ENDDO
      ENDDO
      IF (INDEX_MIN<1 .OR. INDEX_MAX>UBOUND(PACO%SIPACO,2)) THEN
          CALL vtutor%bug("internal error in  PCDAT_PACO index: " // str(INDEX_MIN)  // " " // str(INDEX_MAX), &
              "dyna.F", 1346)
      ENDIF

      END SUBROUTINE PCDAT_PACO

!*********************************************************************
! final processing of PACO structure
! divide by average number of particles in sphere shell
! divide by number of samples
!*********************************************************************

      SUBROUTINE PCDAT_FINALIZE(T_INFO, PACO, COMM)
      USE base
      USE poscar
      USE constant
      USE mpimy
...
! local
      ...
      REAL(q) :: PCFAK
      INTEGER :: I, NT, NTT, INDEX_COMBINED

! divide accumulated volume by number of contributions
      PACO%OMEGA=PACO%OMEGA / PACO%SMEANP
      CALL M_sum_d(COMM, PACO%SIPACO, SIZE(PACO%SIPACO))

! index 0 total pair correlation function
      PCFAK = 1.5_q/PI/T_INFO%NIONS/T_INFO%NIONS*(PACO%OMEGA)*(PACO%NPACO/PACO%APACO)**3
      DO  I=0,PACO%NPACO-1
         PACO%SIPACO(I,0)=PCFAK*SUM(PACO%SIPACO(I,1:))/ REAL( 3*I*(I+1)+1 ,KIND=q) /PACO%SMEANP
      ENDDO

      DO NT=1,T_INFO%NTYP
      DO NTT=NT,T_INFO%NTYP
         INDEX_COMBINED=((2*T_INFO%NTYP-(NT-2))*(NT-1))/2+(NTT-NT)+1

         PCFAK = 1.5_q/PI/T_INFO%NITYP(NT)/T_INFO%NITYP(NTT)*(PACO%OMEGA)*(PACO%NPACO/PACO%APACO)**3
         IF (NT /= NTT) PCFAK=PCFAK/2
  
         DO  I=0,PACO%NPACO-1
            PACO%SIPACO(I,INDEX_COMBINED)=PCFAK*PACO%SIPACO(I,INDEX_COMBINED)/ REAL( 3*I*(I+1)+1 ,KIND=q) /PACO%SMEANP
         ENDDO
      ENDDO
      ENDDO


      END SUBROUTINE PCDAT_FINALIZE


!*********************************************************************
!  allocate the PACO structure
!  if it is already allocated, deallocate it
!*********************************************************************

      SUBROUTINE PCDAT_ALLOCATE_PACO(T_INFO, PACO)
      USE base
      USE poscar
      IMPLICIT NONE
      TYPE (type_info) :: T_INFO
      ...
      IF (ALLOCATED(PACO%SIPACO)) DEALLOCATE(PACO%SIPACO)

! entry (0._q,0._q) is reserved for total pair correlation function
      ALLOCATE(PACO%SIPACO(0:PACO%NPACO,0:T_INFO%NTYP*(T_INFO%NTYP+1)/2))
      CALL PCDAT_CLEAR_PACO( PACO)

      END SUBROUTINE PCDAT_ALLOCATE_PACO

!*********************************************************************
!  clear all counters in the PACO structure reset
!  the pair correlation array
!*********************************************************************

      SUBROUTINE PCDAT_CLEAR_PACO( PACO)
      USE base
      IMPLICIT NONE
      TYPE (paco_struct)  PACO
      ...
      END SUBROUTINE PCDAT_CLEAR_PACO
