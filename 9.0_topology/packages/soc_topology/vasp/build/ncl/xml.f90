...
!
!> this module implements the (deprecated) xml output of VASP
!>
!> the DDT can be found in vasp.xml
!> for new routines focus on implementing the HDF5 output
!
!**********************************************************************
      ...
      MODULE vaspxml
        USE prec
        IMPLICIT NONE
!> unit to which xml output is written
!> if uixml is smaller 0 no output is written
        INTEGER,SAVE ::  uixml
        LOGICAL,SAVE ::  lxml   !< perform xml output

        INTEGER, PARAMETER,PRIVATE :: maxstringlen=30, maxdepth=20
...
!=======================================================================
!
!> writes a single TAG to the xml and pops the tag to a stack
!
!=======================================================================
      CONTAINS
...
! check whether maxstringlen is sufficiently large
          ...
          IF (stackposition > maxdepth) THEN
             CALL vtutor%bug("internal ERROR in xml.F: increase maxdepth", "xml.F", 50)
          ENDIF
...
!> pop a TAG from the stack and write it to the xml file
        SUBROUTINE XML_CLOSE_TAG( tag)
          ...
          IF (stackposition < 1) THEN
             msg = "internal ERROR in xml.F: xml stack exhausted, no more tags to close"
             IF (PRESENT(tag)) msg = msg // '\nclosing tag is ' // TRIM(tag)
             CALL vtutor%bug(msg, "xml.F", 91)
          ENDIF
          ...
          IF (PRESENT(tag)) THEN
             IF (tag /= stack(stackposition)(1:l)) THEN
                CALL vtutor%bug("XML_CLOSE_TAG internal error: not a matching tag " // TRIM(tag) // &
                   " " // stack(stackposition)(1:l), "xml.F", 100)
             ENDIF
          ENDIF
          stackposition=stackposition-1
...
!> writes an single string to the the xml file, bracketing it between
!> <TAG> entity <\TAG>
        SUBROUTINE XML_TAG_STRING( tag, string)
          IMPLICIT NONE
...
!> writes an single float to the the xml file, bracketing it between
!> <TAG> entity <\TAG>
...
!> writes an single integer to the the xml file, bracketing it between
!> <TAG> entity <\TAG>
...
!> write a vector with n entries possibly add a name to the vector
!>
!> <v>  entity1 entity2 entity3  </v>
!> <v name=supplied_name >  entity1 entity2 entity3  </v>
...
!> subroutine closes all open tags and closes the xml file
      SUBROUTINE STOP_XML
        IMPLICIT NONE
        INTEGER i
...
! CALL XML_ROW_DATA( TMP(1:3),form='(G12.4)')
        ...
        IF (LPARTIAL) THEN
           ...
           DO l=1,LDIM
              ...
              CALL SPHPRO_DESCRIPTION(ni, l, lmtype)
              IF (lmtype==" err ") THEN
                 CALL vtutor%bug("internal error: SPHPRO_DESCRIPTION returns an error for NI= " // &
                    str(ni) // " L " // str(L), "xml.F", 831)
                 EXIT
              ENDIF
...
!=======================================================================
      ...
      SUBROUTINE XML_PROCAR(PAR, CELTOT, FERTOT, NB_TOT, NKPTS, LDIM , NIONS, ISPIN, TAG)
...
! local
        ...
        CHARACTER (LEN=5) :: lmtype
        CHARACTER (LEN=40) :: strcounter
        CHARACTER (LEN=80) :: PROJECTED_TAG

        PROJECTED_TAG = "projected"; IF (PRESENT(TAG)) PROJECTED_TAG = TAG
        CALL XML_TAG(TRIM(PROJECTED_TAG))
        ...
        CALL XML_CLOSE_TAG("set")
        CALL XML_CLOSE_TAG("array")
        CALL XML_CLOSE_TAG(TRIM(PROJECTED_TAG))
...
! local variables
        ...
        IF (N/=1 .AND. FORM /= "S") THEN
           CALL vtutor%bug("internal error: XML_INCAR called with a vector, use XML_INCAR_V " &
              // "instead " // TRIM(TAG), "xml.F", 1001)
        ENDIF
        IF (.NOT. lxml) RETURN
...
!=======================================================================
!
!> writes the read INCAR tags to UNIT=19 if the corresponding unit is open
!
!=======================================================================
...
! local variables
        ...
        IF (N/=1 .AND. FORM /= "S") THEN
           CALL vtutor%bug("internal error: JOBFLOW called with a vector, use XML_INCAR_V " &
              // "instead " // TRIM(TAG), "xml.F", 1133)
        ENDIF
        IF (.NOT. lxml) RETURN
...
!=======================================================================
      ...
      SUBROUTINE XML_EPSILON_W(DELTAE, EPS_REAL, EPS_IMAG, NEDOS)
        ...
        CALL XML_EPSILON_W_COMMON(DELTAE, EPS_REAL, EPS_IMAG, NEDOS)

      END SUBROUTINE XML_EPSILON_W

!> write the frequency dependent dielectric function with a comment
      SUBROUTINE XML_EPSILON_W_COMMENT(DELTAE, EPS_REAL, EPS_IMAG, NEDOS, comment)
        USE vaspxml
        USE prec
        IMPLICIT none
      ...
      END SUBROUTINE XML_EPSILON_W_COMMENT


!> write the frequency dependent dielectric function (common part)
      SUBROUTINE XML_EPSILON_W_COMMON(DELTAE, EPS_REAL, EPS_IMAG, NEDOS)
        USE vaspxml
        USE prec
        IMPLICIT none
...
! cap values to avoid ****** output
      ...
      END SUBROUTINE XML_EPSILON_W_COMMON
...
!=======================================================================
!
! XML_LANCZOS_W(N_ITER,A_COEFF,B_COEFF)
!> write the Lanczos coefficients to the vasprun.xml file
!
!=======================================================================

      SUBROUTINE XML_LANCZOS_W(N_ITER, A_COEFF, B_COEFF)
        USE vaspxml
        USE prec
        IMPLICIT none

        INTEGER    :: N_ITER                                     !> number of Lanczos iterations
        REAL(q)    :: A_COEFF(1:N_ITER,1:6,1:6), B_COEFF(1:N_ITER,1:6,1:6)   !> a and b Lanczos coefficients
! local
        INTEGER    :: N
        REAL(q)    :: TMP(6)

        CALL XML_TAG("lanczos coefficients")
        CALL XML_TAG("a coefficients")
        CALL XML_TAG("array")
        CALL XML_DIMENSION("gridpoints",1)
        CALL XML_FIELD("xx", type="float")
        CALL XML_FIELD("yy", type="float")
        ...
        CALL XML_FIELD("xy", type="float")
        CALL XML_FIELD("yz", type="float")
        CALL XML_FIELD("xz", type="float")
        CALL XML_TAG("set")

! cap values to avoid ****** output
        DO N=1,N_ITER
          TMP(1)= MIN(A_COEFF(N,1,1),99999._q)
          TMP(2)= MIN(A_COEFF(N,2,2),99999._q)
          TMP(3)= MIN(A_COEFF(N,3,3),99999._q)
          TMP(4)= MIN(A_COEFF(N,4,4),99999._q)
          TMP(5)= MIN(A_COEFF(N,6,6),99999._q)
          TMP(6)= MIN(A_COEFF(N,5,5),99999._q)
          CALL XML_ROW_DATA(TMP,FORM='(F10.4,1X)')
        ENDDO
        
        CALL XML_CLOSE_TAG("set")
        CALL XML_CLOSE_TAG("array")
        CALL XML_CLOSE_TAG("a coefficients")

        CALL XML_TAG("b coefficients")
        CALL XML_TAG("array")
        CALL XML_DIMENSION("gridpoints",1)
        CALL XML_FIELD("xx", type="float")
        CALL XML_FIELD("yy", type="float")
...
! cap values to avoid ****** output
        DO N=1,N_ITER
          TMP(1)= MIN(B_COEFF(N,1,1),99999._q)
          TMP(2)= MIN(B_COEFF(N,2,2),99999._q)
          TMP(3)= MIN(B_COEFF(N,3,3),99999._q)
          TMP(4)= MIN(B_COEFF(N,4,4),99999._q)
          TMP(5)= MIN(B_COEFF(N,6,6),99999._q)
          TMP(6)= MIN(B_COEFF(N,5,5),99999._q)
          CALL XML_ROW_DATA(TMP,FORM='(F10.4,1X)')
        ENDDO

        CALL XML_CLOSE_TAG("set")
        CALL XML_CLOSE_TAG("array")
        CALL XML_CLOSE_TAG("b coefficients")

        CALL XML_CLOSE_TAG("lanczos coefficients")
      END SUBROUTINE XML_LANCZOS_W
...
!=======================================================================
!
! XML_LINEAR_RESPONSE_E
!> write the frequency dependent linear response as function of energy
!
!=======================================================================

      SUBROUTINE XML_EPSILON_E_DIR(DELTAE, EPSILON, NEDOS, IEPSILON, COMMENT)
        USE vaspxml
        USE prec
        IMPLICIT none

        REAL(q) DELTAE
        COMPLEX(q) :: EPSILON(1:NEDOS,3)
        INTEGER NEDOS
        INTEGER :: IEPSILON
        CHARACTER (LEN=*) :: COMMENT
...
! cap values to avoid ****** output
        DO N=1,Nedos
           tmp(1)= E(N)
           DO I=DIR_MIN,DIR_MAX
              tmp(2+I-DIR_MIN)= MIN(AIMAG(EPSILON(N,I)),99999._q)
           ENDDO
...
! cap values to avoid ****** output
        DO N=1,Nedos
           tmp(1)= E(N)
           DO I=DIR_MIN,DIR_MAX
              tmp(2+I-DIR_MIN)= MIN(REAL(EPSILON(N,I),q),99999._q)
           ENDDO
           CALL XML_ROW_DATA( tmp(1:2+DIR_MAX-DIR_MIN), form='(F10.4,1X)')
        ENDDO
        CALL XML_CLOSE_TAG("set")
      ...
      END SUBROUTINE XML_EPSILON_E_DIR
...
!=======================================================================
      ...
      SUBROUTINE XML_KPOINTS_1(NKPTS, VKPT, WTKPT, NTET, IDTET, VOLWGT, NKL, KLABELS, KLABELSPOS)
        USE vaspxml
        USE prec
        IMPLICIT NONE
        ...
        INTEGER :: NKL            !< number of k-points labels
        CHARACTER(LEN=*) :: KLABELS(NKL) !< kpoints labels
        INTEGER :: KLABELSPOS(NKL)       !< positions of k-points labels
...
! kpoints labels
        IF (NKL>0) THEN
           CALL XML_KPOINTS_LABELS(NKL, KLABELS, KLABELSPOS)
        ENDIF
...
!=======================================================================
      ...
      SUBROUTINE XML_KPOINTS_3(NKPTS, VKPT, WTKPT, NINTER, NKL, KLABELS, KLABELSPOS)
        USE vaspxml
        USE prec
        IMPLICIT NONE
...
! kpoints labels
      ...
      SUBROUTINE XML_KPOINTS_LABELS(N, KLABELS, KLABELSPOS)
        use vaspxml
        INTEGER :: N
        CHARACTER(LEN=*) :: KLABELS(N)
        INTEGER :: KLABELSPOS(N)
!local variables
        CALL XML_TAG("kpoints_labels")
        DO I=1,N
            CALL XML_TAG_INT(TRIM(KLABELS(I)), KLABELSPOS(I))
        ENDDO
        CALL XML_CLOSE_TAG("kpoints_labels")
      END SUBROUTINE XML_KPOINTS_LABELS
!=======================================================================
!
! XML_BORN_CHARGES
...
!
! XML_FORCE_CONSTANTS
!>   write force constant tensors
!>
!>   forces on the ions in the primitive cell (as defined by the NIONP
!>   entries of MAP), induced by displacements of all ions in the supercell.
!
!=======================================================================
