...
!**********************************************************************
!
!> Handles MP2 using similar routines as coded in
!> local_field.F
!>
...
!> Stores the two-orbital response functions at q
!> <ij|ab> (q=k_b-k_j), FTOD_PW=(ia)
!> FTOD_PW(NGVECTOR,NBANDS,VBMAX,NKPTS,NKPTS(KQ),ISPIN)
      COMPLEX(q) , ALLOCATABLE, PRIVATE, SAVE :: FTOD_PW(:,:,:,:,:,:,:)      
!> Stores the (1._q,0._q) center part of the two-orbital response functions at q
!> <ij|ab> (q=k_b-k_j), FTOD_PW=(ia)
...
!> function that returns the number of rows or columns of the LOCAL ARRAY if you enter
...
!> Restrict band index to active space: NBANDSHIGH >= N > NFREEZE
      ...
      INTEGER :: NFREEZE
      INTEGER :: NBANDSHIGH
!> Dump pairwise energies only works for Gamma-only!
      LOGICAL :: LDUMPPAIRS
...
!local variables
         TYPE (wavespin) WHF
         INTEGER KI,KQ,KQ_,KA,KB,ISP,KJ,NBI,NBJ,ISPI,ISPJ
         LOGICAL :: qchange
         REAL(q) :: test         
...
!read MP2 routine specific input flags
         CALL MP2_READER(IO%IU5, IO%IU6, IO%IU0)
         IF (NFREEZE/=0) LFREEZE=.TRUE.
         IF ((IO%IU0>0) .and. (LFREEZE)) write(IO%IU0,*)'The first ',NFREEZE,' orbitals will be frozen in this MP2 calculation!'

         MP2_bandstructure=.FALSE.
         IF (MP2_bandstructure) THEN
            IF (SYMM%ISYM>=0) THEN
               CALL vtutor%error("CALCULATE_MP2: MP2 QP energies are not implemented for ISYM>=0. Redo &
                  &all calculations setting ISYM=-1 in the INCAR file.")
            ENDIF
            IF (WDES%ISPIN>1) THEN
               CALL vtutor%error("CALCULATE_MP2: MP2 QP energies are not implemented for ISPIN>1.")
...
# 255
                        ...
                        CALL PZGEMM('C','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                          NGVECTOR,-(1._q,0._q), FTOD_PW(1,1,NBI-NFREEZE,RKIofKI(KI),RKQofKQ(KQ),ISP,1),1,1,&
                          desc_FTOD_PW,FTOD_PW(1,1,NBJ-NFREEZE,RKIofKI(KJ),RKQofKQ(KQ),ISP,ncc),1,1,&
...
# 267
                           ...
                           CALL PZGEMM('C','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                          NHVECTOR,-(1._q,0._q), FTOD_OC(1,1,NBI-NFREEZE,RKIofKI(KI),RKQofKQ(KQ),ISP,1),1,1,&
                          desc_FTOD_OC,FTOD_OC(1,1,NBJ-NFREEZE,RKIofKI(KJ),RKQofKQ(KQ),ISP,2),1,1,&
...
# 303
                           ...
                           CALL PZGEMM('C','n',(PROCS*WDES%NBANDS),&
                             (PROCS*WDES%NBANDS),&
                             NGVECTOR,-(1._q,0._q), FTOD_PW(1,1,NBJ-NFREEZE,RKIofKI(KJ),RKQofKQ(KQ_),ISP,ncc),1,1, &
...
# 317
                              ...
                              CALL PZGEMM('C','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                               NHVECTOR,-(1._q,0._q), FTOD_OC(1,1,NBJ-NFREEZE,RKIofKI(KJ),RKQofKQ(KQ_),ISP,2),1,1,&
                               desc_FTOD_OC,FTOD_OC(1,1,NBI-NFREEZE,RKIofKI(KI),RKQofKQ(KQ_),ISP,1),1,1,&
...
# 349
                           ...
                           CALL PZGEMM('C','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                          NGVECTOR,-(1._q,0._q), FTOD_PW(1,1,NBI-NFREEZE,RKIofKI(KI),RKQofKQ(KQ),ISP,1),1,1,&
                          desc_FTOD_PW,FTOD_PW(1,1,NBJ-NFREEZE,RKIofKI(KJ),RKQofKQ(KQ),ISP-1,ncc),1,1,&
                          desc_FTOD_PW,(0._q,0._q), TWOE4ORBITAL(1,1),1,1,desc_TWOE4ORBITAL)
...
# 361
                           ...
                           CALL PZGEMM('C','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                          NHVECTOR,-(1._q,0._q), FTOD_OC(1,1,NBI-NFREEZE,RKIofKI(KI),RKQofKQ(KQ),ISP,1),1,1,&
                          desc_FTOD_OC,FTOD_OC(1,1,NBJ-NFREEZE,RKIofKI(KJ),RKQofKQ(KQ),ISP-1,2),1,1,&
                          desc_FTOD_OC,(1._q,0._q), TWOE4ORBITAL(1,1),1,1,desc_TWOE4ORBITAL)
...
!***********************************************************************
!> Use the matrices `TWOE4ORBITAL` (`<ij|ab>`) and `TWOE4ORBITAL_X` (`<ij|ba>`)
...
!***********************************************************************
!> Calculates the fourier-transformed overlap integrals `<i|-G|a>`
...
!and allocate the TWOE4ORBITAL and TWOE4ORBITAL_X matrices
         ...
         ALLOCATE(tmp_FTOD_PW(NGVECTOR,WDES%NBANDS,VBMAX-NFREEZE,ncc))
         IF (ASSOCIATED(H)) THEN
            ALLOCATE(tmp_FTOD_OC(NHVECTOR,WDES%NBANDS,VBMAX-NFREEZE,2))
         ENDIF 
...
# 1324
                        ...
                        IF (ASSOCIATED(H)) THEN
                           tmp_FTOD_OC(1:H%TOTAL_ENTRIES,NBAA+NBA-1,NBI-NFREEZE,1)=(CRHOIA(1:H%TOTAL_ENTRIES,NBAA))
                        ENDIF
...
!copy FTOD functions to FTOD_PW and FTOD_OC
                        ...
                        IF (ASSOCIATED(H)) THEN
                           tmp_FTOD_OC(1:H%TOTAL_ENTRIES,NBAA+NBA-1,NBI-NFREEZE,2)=CRHOIB(1:H%TOTAL_ENTRIES,NBAA)
                        ENDIF   
...
!***********************************************************************
!> Redistributes the fourier-transformed overlap integrals `<i|G|a>`
...
!Distribute the fourier-transformed overlap integrals
         DO NBI=1,VBMAX   
            IF ((LFREEZE) .and. (NBI<=NFREEZE)) CYCLE
            DO cc=1,ncc
               CALL PZGEMR2D(NGVECTOR,(PROCS*WDES%NBANDS),tmp_FTOD_PW(1,1,NBI-NFREEZE,cc),1,1,&
                desc_FTOD_PW_br,FTOD_PW(1,1,NBI-NFREEZE,RKIofKI(KI),RKQofKQ(KQ),ISP,cc),1,1,desc_FTOD_PW,contxt_grid)
            ENDDO
         ENDDO
...
# 1631
                  ...
                  CALL PZGEMR2D(NHVECTOR,(PROCS*WDES%NBANDS),tmp_FTOD_OC(1,1,NBI-NFREEZE,cc),1,1,&
                   desc_FTOD_OC_br,FTOD_OC(1,1,NBI-NFREEZE,RKIofKI(KI),RKQofKQ(KQ),ISP,cc),1,1,desc_FTOD_OC,contxt_grid)
...
!Blocking size for block-cyclic distribution of FTOD_PW
         ...
         ALLOCATE(FTOD_PW(FTOD_PW_rows,FTOD_PW_cols,VBMAX-NFREEZE,REALNKPTS,REALNKPTS,WDES%ISPIN,ncc))
...
!Blocking size for block-cyclic distribution of FTOD_PW
            ...
            ALLOCATE(FTOD_OC(FTOD_OC_rows,FTOD_OC_cols,VBMAX-NFREEZE,REALNKPTS,REALNKPTS,WDES%ISPIN,2))
         ENDIF
...
!Check if k-mesh is shifted from Gamma
