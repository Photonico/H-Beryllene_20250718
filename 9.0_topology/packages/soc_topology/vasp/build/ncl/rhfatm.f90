...
!***********************************************************************
!***********************************************************************
!> routines that adapt the PAW datasets
!> to be used in Hartree-Fock calculations, basically by replacing the
!> DFT frozen core by its HF counterpart.
!>
!> The main public module subroutine is rhfatm::RHFATM_SET_PP, that is called by
!> pawm::SET_PAW_ATOM_POT_RHF (the HF counterpart of pawm::SET_PAW_ATOM_POT).
...
!#define USELINEAR
     ...
     &   IS_BOUND,N_OF_CHANNEL,IR_END,IR_CORE,IR_SWITCH,IR_WMAX, &
     &   RCRCUT
...
!> Occupation of subshell nl
         REAL(q), POINTER :: OCC(:)
!> Grid points for inner and outer integration (outward ->  KI <- inward KJ expoential)
         INTEGER, POINTER :: KI(:)
         INTEGER, POINTER :: KJ(:)
...
!
!> Reads the keywords LRHFATM and MAXPWAMP from INCAR.
!>
...
! local variables
      ...
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LRHFATM', LuseRHFATM, IERR, WRITEXMLINCAR)

      IF (LuseRHFATM) THEN
            maxParWaveAmplitude=10._q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'MAXPWAMP', maxParWaveAmplitude, IERR, WRITEXMLINCAR)
      ENDIF
...
!
!> Main public subroutine of the rhfatm module.
!>
...
!> (rhfatm:DFATOM and rhfatm::HFATOM respectively). Subsequently the DFT and HF partial
!> waves are (re)calculated (in rhfatm::DF_PAR_WAVE and rhfatm::HF_PAR_WAVE).
!> N.B.: presently the HF partial waves are take to be equal to the
!> original DFT partial waves.
!> These are orthogonalized to the HF core states (rhfatom::ORTHO_PAR_WAVE).
!> Subsequently the PAW parameters DIJ and QIJ are calculated (rhfatm::SET_PP),
!> and the Hartree-Fock core-valence exchange is added to the PAW
!> strength parameters DIJ (rhfatm::HF_CORE_VALENCE_EXCH).
!> The HF atomic reference energy is stored in PP%EATOM.
!
!***********************************************************************
      SUBROUTINE RHFATM_SET_PP(PP,XC,IO)
      USE prec
      USE base
      USE pseudo
...
! Copy the core charge to the PP data structure
      ...
      CALL VH5_WRITE_RHFATOM(IH5OUTFILEID,WDF,WHF)
      ...
      SUBROUTINE VH5_WRITE_RHFATOM(FILEID,WDF,WHF,GROUP,SUBGROUP)
      USE vhdf5_base
      IMPLICIT NONE
...
!-------------------------------------------------------------
      ...
      MY_SUBGROUP = GRP_RHFATOM; IF (PRESENT(SUBGROUP)) MY_SUBGROUP = TRIM(SUBGROUP)
      call vh5_error(VH5_GROUP_OPEN_OR_CREATE(FILEID, TRIM(MY_GROUP), GROUPID),"rhfatm.F",380)
      call vh5_error(VH5_GROUP_OPEN_OR_CREATE(GROUPID, TRIM(MY_SUBGROUP), SUBGROUPID),"rhfatm.F",381)

! Write DFT WFs
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_dft_a", WDF%A),"rhfatm.F",384)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_dft_b", WDF%B),"rhfatm.F",385)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_dft_w", WDF%W),"rhfatm.F",386)
! Write DFT RHO
      CALL SIMPI(WDF%R,WDF%RHO,RHOT)
      call vh5_error(VH5_WRITE(SUBGROUPID, "rho_dft", RHOT),"rhfatm.F",389)
      call vh5_error(VH5_WRITE(SUBGROUPID, "rho_ae_dft", WDF%RHOAE),"rhfatm.F",390)
      call vh5_error(VH5_WRITE(SUBGROUPID, "rho_ps_dft", WDF%RHOPS),"rhfatm.F",391)
! Write DFT matching points
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_dft_ki", WDF%KI),"rhfatm.F",393)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_dft_kj", WDF%KJ),"rhfatm.F",394)
! Write DFT energies
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_dft_e", WDF%E),"rhfatm.F",396)
! Write DFT number of iterations
      call vh5_error(VH5_WRITE(SUBGROUPID, "dft_niter", WDF%NITER),"rhfatm.F",398)

! Write HF WFs
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_hf_a", WHF%A),"rhfatm.F",401)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_hf_b", WHF%B),"rhfatm.F",402)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_hf_w", WHF%W),"rhfatm.F",403)
! Write HF RHO
      CALL SIMPI(WHF%R,WHF%RHO,RHOT)
      call vh5_error(VH5_WRITE(SUBGROUPID, "rho_hf", RHOT),"rhfatm.F",406)
      call vh5_error(VH5_WRITE(SUBGROUPID, "rho_ae_hf", WHF%RHOAE),"rhfatm.F",407)
      call vh5_error(VH5_WRITE(SUBGROUPID, "rho_ps_hf", WHF%RHOPS),"rhfatm.F",408)
! Write HF matching points
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_hf_ki", WHF%KI),"rhfatm.F",410)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_hf_kj", WHF%KJ),"rhfatm.F",411)
! Write HF energies
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_hf_e", WHF%E),"rhfatm.F",413)
! Write HF number of iterations
      call vh5_error(VH5_WRITE(SUBGROUPID, "hf_niter", WHF%NITER),"rhfatm.F",415)

! Write HF Potential
      call vh5_error(VH5_WRITE(SUBGROUPID, "v_hf", WHF%V),"rhfatm.F",418)
      call vh5_error(VH5_WRITE(SUBGROUPID, "vx_hf", WHF%VX),"rhfatm.F",419)
! Write HF inhomogeneous part
      call vh5_error(VH5_WRITE(SUBGROUPID, "ih_hf", WHF%IH),"rhfatm.F",421)

! Write Quantum numbers
      call vh5_error(VH5_WRITE(SUBGROUPID, "r", WHF%R%R),"rhfatm.F",424)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_n", WHF%N),"rhfatm.F",425)
      call vh5_error(VH5_WRITE(SUBGROUPID, "wf_l", WHF%L),"rhfatm.F",426)

      call vh5_error(VH5_GROUP_CLOSE_WRITING(SUBGROUPID),"rhfatm.F",428)
      call vh5_error(VH5_GROUP_CLOSE_WRITING(GROUPID),"rhfatm.F",429)
      call vh5_error(VH5_FILE_FLUSH(FILEID),"rhfatm.F",430)
      END SUBROUTINE VH5_WRITE_RHFATOM
...
!
!***********************************************************************
      SUBROUTINE RHFATM_CROP_PSEUDO(P,T_INFO,LOVERL,XC,IO)
      USE prec
      USE base
      USE poscar
      USE pseudo
...
! LRELCOR: do we want to perform a relaxed core calculation?
      LRELCOR=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LRELCOR', LRELCOR, IERR, WRITEXMLINCAR)
...
! read which types are submitted to core relaxation
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LRCTYPE', LRCTYPE, T_INFO%NTYP, IERR, WRITEXMLINCAR)

! RCRHOCUT: defines the extent of the core
      ALLOCATE(RCRHOCUT(T_INFO%NTYP))
      RCRHOCUT=1E-5
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'RCRHOCUT', RCRHOCUT, T_INFO%NTYP, IERR, WRITEXMLINCAR)
...
! out divergencies and extra nodes in the AE partial waves
      LADAPTELIN=.TRUE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LADAPTELIN', LADAPTELIN, IERR, WRITEXMLINCAR)

! ESEMICORE: defines the semicore states (Default: -2 Ry)
      ESEMICORE=-27.211652
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'ESEMICORE', ESEMICORE, IERR, WRITEXMLINCAR)
...
! define the extent of the core
         RCUT=RCRCUT(RCRHOCUT(NT),ESEMICORE,PP,XC,IO)
...
!
!***********************************************************************
      SUBROUTINE RHFATM_RECALC_PSEUDO(P,XC,T_INFO,LOVERL,IO)
      USE prec
      USE base
      USE poscar
      USE pseudo
...
!
!> just a wrapper for a completely non-verbose call to DFATOM
!
!***********************************************************************
      SUBROUTINE RHFATM_DFATOM(PP,W,XC)
      USE pseudo
      USE base
      USE setexm_struct_def, ONLY : xc_info
      IMPLICIT NONE
...
!******************** FUNCTION LRHFATM *********************************
!
!> Returns .TRUE. if LRHFATM=.TRUE. was read from the INCAR
!> by RHFATM_READER.
!
!***********************************************************************
      FUNCTION LRHFATM()
...
!> decrease occupancy of the highest occupied shell
!> so as to prepare cationic state
!> @author  Tomas Bucko
...
! check if there is still some electron to be removed
       IF (sum(OCC)<1._q) THEN
         CALL vtutor%error("REMOVE_ELECTRON: ERROR, no electron available")
       ENDIF
      
       nmax=1
       emax=E(nmax)
       DO i=1,NDIM
         OCC(i)=1._q*NINT(OCC(i))
...
!> increase occupancy of the lowest available shell
!> so as to prepare anionic state
!> @author  Tomas Bucko
...
!c check if there is a free shell
       ...
       DO i=1,NDIM
         maxocc=4*L(i)+2
         IF (NINT(OCC(i))<maxocc .AND. NINT(OCC(i)) > 0) THEN
           emin=E(i)
           nmin=i
           EXIT
         ENDIF
       ENDDO
       IF (nmin==0) THEN
         CALL vtutor%error("ADD_ELECTRON: ERROR, no shell available")
       ENDIF
      
       DO i=1,NDIM
         OCC(i)=1._q*NINT(OCC(i))
         maxocc=4*L(i)+2
         IF (OCC(i)<maxocc .AND. OCC(i)>0 .AND. E(i)<emin) THEN
           emin=E(i)
           nmin=i
         ENDIF
       ENDDO
...
!
!***********************************************************************
     SUBROUTINE DFATOM2(PP,W,XC,IO,qstate,LFROZENORBITAL)
      USE prec
      USE base
      USE pseudo
      USE setexm, ONLY : PUSH_XC_TYPE, POP_XC_TYPE
      USE setexm_struct_def, ONLY : xc_info,XC_DATA_PP
      USE constant
      USE string, ONLY: str
      USE tutor, ONLY: vtutor
...
! first we try to converge without gradient corrections
      LNOGGA=.FALSE. ; IF (XC%LDOGGA.OR.XC%LDOMETAGGA) LNOGGA=.TRUE.
    
      scf: DO I=1,NSCF
         RHOP=RHO
! Get the potential
!CALL RAD_POT_DFT(RHO,INT(W%Z),W%R,V,XC)
         CALL RAD_POT_DFT2(RHO,RHO_compenzation,NINT(W%Z),W%R,V,XC,0,DOUBLEC,LGGA=.NOT.LNOGGA)
!CALL RAD_POT_DFT(RHO,NINT(W%Z),W%R,V,XC,LGGA=.NOT.LNOGGA)
...
! Switch back to HF exchange
      CALL POP_XC_TYPE(XC)
      IF (I>NSCF) THEN
         CALL vtutor%error("DFATOM2: ERROR, unable to reach convergence in " // str(NSCF) // " cycles.")
      ELSE
!          ! Recompute POTAE and POTAEC
!          CALL RAD_POT_DFT(RHO,INT(W%Z),W%R,V,XC)
!          CALL RAD_POT_DFT(W%RHOAE,INT(W%Z),W%R,W%POTAEC,XC,LXC=.FALSE.)
         TOTEN=0._q
         CALL RAD_POT_DFT2(RHO,RHO_compenzation,NINT(W%Z),W%R,V,XC,0,TOTEN)
         CALL RAD_POT_DFT2(W%RHOAE,RHO_compenzation,NINT(W%Z),W%R,W%POTAEC,XC,0,DOUBLEC,LXC=.FALSE.)
         W%POTAE=V-W%POTAEC
         DO ii=1,W%NS
...
!  write(*,*) 'test Ccharge:',qdummy,W%NSCORE
      ...
      IF ((qstate .NE. 0) .AND.(.NOT. LFROZENORBITAL)) THEN
        RHO_frozen(:)=RHO(:)
        CALL compensation_watson(W,XC,IO,RHO,RHO_cor,qstate,Rcomp,LRELAXCORE)
! CALL compensation_uniform(W,XC,IO,RHO,RHO_cor,qstate,LRELAXCORE)
      ENDIF
...
!       IF (IO%IU6>0)  write(*,*) 'alpha2',Rcomp/AUTOA**3
      ...
      SUBROUTINE compensation_watson(W,XC,IO,RHO,RHO_cor,qstate,Rcomp,LRELAXCORE)
        USE prec
        USE base
        USE pseudo
        USE setexm_struct_def, ONLY : xc_info
        ...
        REAL(q), PARAMETER :: MIX=0.2_q
        REAL(q), PARAMETER :: RHODIFF=5.0E-7
        REAL(q), PARAMETER :: RHODIFF_soft=1.0E-6
        INTEGER :: qstate
        REAL(q) :: Rcomp,Rcomp_old,qdummy, qdummy_old
        REAL(q) :: DOUBLEC,TOTEN
        LOGICAL :: LRELAXCORE
...
! Get the potential
            CALL RAD_POT_DFT3(RHO,INT(W%Z),W%R,V,XC,qdummy,Rcomp,DOUBLEC)
...
# 1054
          ...
          IF ((I>NSCF) .AND. (DRHO>RHODIFF_soft)) THEN
            CALL vtutor%error("SUBROUTINE COMPENSATION_WATSON: ERROR1, unable to reach convergence in &
               &" // str(NSCF) // " cycles.")
          ELSE
            TOTEN=0._q
        
            CALL RAD_POT_DFT3(RHO,INT(W%Z),W%R,V,XC,qdummy,Rcomp,TOTEN)
            CALL RAD_POT_DFT3(W%RHOAE,INT(W%Z),W%R,W%POTAEC,XC,qdummy,Rcomp,DOUBLEC,LXC=.FALSE.)
            DO ii=1,W%NS
              TOTEN=TOTEN+W%OCC(ii)*W%E(ii)
            ENDDO
...
!         ENDIF
      ...
      SUBROUTINE compensation_uniform(W,XC,IO,RHO,RHO_cor,qstate,LRELAXCORE)
        USE prec
        USE base
        USE pseudo
        USE setexm_struct_def, ONLY : xc_info
...
! Get the potential
            CALL RAD_POT_DFT2(RHO,RHO_compenzation,INT(W%Z),W%R,V,XC,qstate,DOUBLEC)
...
! Copy RHO to RHOP
          ...
          ENDDO scf2
          IF (I>NSCF) THEN
            CALL vtutor%error("COMPENSATION_UNIFORM: ERROR1, unable to reach convergence in " // &
               str(NSCF) // " cycles.")
          ELSE
            TOTEN=0._q
            CALL RAD_POT_DFT2(RHO,RHO_compenzation,INT(W%Z),W%R,V,XC,qstate,TOTEN)
            CALL RAD_POT_DFT2(W%RHOAE,RHO_compenzation,INT(W%Z),W%R,W%POTAEC,XC,qstate,DOUBLEC,LXC=.FALSE.)
            W%POTAE=V-W%POTAEC
            DO ii=1,W%NS
              TOTEN=TOTEN+W%OCC(ii)*W%E(ii)
            ENDDO
...
!             IF (IO%IU6>0) write(*,*) 'KUKU',Rcomp,TOTEN-1.5*FELECT*INT(W%Z)*qstate/Rcomp  !+0.6_q*FELECT*qstate**2/Rcomp  !-1.5*FELECT*INT(W%Z)*qstate/Rcomp
        ...
        IF (K>100) THEN
          CALL vtutor%error("COMPENSATION_UNIFORM: ERROR2, unable to reach convergence in " // &
             str(100) // " cycles.")
        ENDIF    
...
!******************** SUBROUTINE DFATOM ********************************
!
!> Solves the radial scalar relativistic Schroedinger equation for
!> the spherical nonspinpolarized atom.
!>
...
! local variables
      ...
      REAL(q), ALLOCATABLE :: EP(:)
      INTEGER, PARAMETER :: NSCF=1000
      REAL(q), PARAMETER :: MIX=0.1_q
      REAL(q), PARAMETER :: EDIFF=1.0E-7
      REAL(q), PARAMETER :: RHODIFF=1.0E-7
      REAL(q), PARAMETER :: DWDIFF=1.0E-7
...
! Initialize wave functions
      ...
      WP%KI = W%KI ; WP%KJ = W%KJ

# 1408


      ALLOCATE(RHO(W%R%NMAX),RHOP(W%R%NMAX),V(W%R%NMAX),EP(W%NS))
...
# 1425
      ...
      CALL DIIS_INIT(D,W%R%NMAX,10,MIX)
      CALL DIIS_APPEND(D,RHO)
      scf: DO I=1,NSCF
         CALL DIIS_SAVE(D,RHO)
         RHOP=RHO
         EP=W%E
! Get the potential
         CALL RAD_POT_DFT(RHO,NINT(W%Z),W%R,V,XC,LGGA=(XC%LDOGGA.OR.XC%LDOMETAGGA))
...
# 1444
         ...
         CALL WMIX(W,WP,MIX,DW,MIX=.FALSE.)
         CALL DIIS_APPEND(D,RHO)
         CALL DIIS_STEP(D,RHO)
         DE=MAXVAL(ABS(W%E-EP))
!CALL DIIS_EDIT(D,RHO)



!#ifdef debug
         IF (IO%IU6>0) WRITE(*,'(I3,2F22.12)') I,DE,DW
!#endif

!IF (DRHO<RHODIFF) THEN
         IF (DE<EDIFF.AND.DW<DWDIFF) THEN
! final convergence reached, exit scf
            EXIT scf
         ENDIF
! Copy W to WP
...
! Switch back to HF exchange
      ...
      IF (I>NSCF) THEN
         CALL vtutor%error("DFATOM: ERROR, unable to reach convergence in " // str(NSCF) // " cycles.")
      ELSE
! Recompute POTAE and POTAEC
         CALL RAD_POT_DFT(RHO,NINT(W%Z),W%R,V,XC)
         CALL RAD_POT_DFT(W%RHOAE,NINT(W%Z),W%R,W%POTAEC,XC,LXC=.FALSE.)
         W%POTAE=V-W%POTAEC
! Write atomic eigenvalues to OUTCAR
         CALL REPORT(PP,W,IO,"(DFT)")
# 1494
...
!******************** SUBROUTINE DFSOLVE *******************************
!
!> Wrapper for the subroutine cl::CORE_WAVE_FKT
!>
...
!> On exit the following components of the datastructure W have been set:
!>   E,A,B,W,RHOAE
!L (see the definition of TYPE atomic in the header of the module).
!
!***********************************************************************
...
! local variables
      ...
      RHO=0
      DO I=1,W%NS
         CALL CORE_WAVE_FKT(DRHO,W%E(I),W%N(I),W%L(I),V,W%R,CEILING(W%Z), &
        &   A_=W%A(:,I),B_=W%B(:,I),TOL=1.E-8_q,KI_OUT=W%KI(I),KJ_OUT=W%KJ(I))
         W%B(:,I)=W%B(:,I)*C
...
!
!> Recomputes the all-electron partial waves.
!>
...
!
!***********************************************************************
      SUBROUTINE DF_PAR_WAVE(PP,W,XC,IO)
      USE prec
      USE base
      USE pseudo
      USE radial
...
! Not found
            IF (LERR) THEN
               CALL vtutor%error("DF_PAR_WAVE: ERROR, no matching bound state: " // str(I) // " " // &
                  str(N_OF_CHANNEL(PP,I)) // " " // str(PP%LPS(I)))
            ENDIF
         ELSE
!
...
! integrate outwards at the linearization energies PP%E
            A(1)=PP%WAE(1,I)
            NERR=0 ; CALL OUTINT(PP%E(I),PP%LPS(I),W%Z,W%R,V(1:W%R%NMAX),KI,A,B,NERR,RC=W%R%REND)
...
!******************** SUBROUTINE HFATOM ********************************
!
!> Solves the radial scalar relativistic Hartree-Fock equations
...
! local variables
      TYPE(atomic) WP
      TYPE(diis_type) :: DA, DB
      INTEGER I,J,K
      REAL(q) DW,DWP,DE
      REAL(q) WN,WNA
      REAL(q) EKIN,EHAR,EHARV,EXCH,EXCHC,EIGNSM,DHAR
...
!     IF (PRESENT(W0)) W=W0
      IF (PRESENT(W0)) THEN
         ...
         W%KI = W0%KI ; W%KJ = W0%KJ
      ENDIF

      WP%A=W%A; WP%B=W%B; WP%W=W%W; WP%E=W%E
      WP%KI = W%KI ; WP%KJ = W%KJ
      CALL DIIS_INIT(DA,W%R%NMAX*W%NS,10,ALPHA)
      CALL DIIS_INIT(DB,W%R%NMAX*W%NS,10,ALPHA)
      CALL DIIS_APPEND(DA,W%A)
      CALL DIIS_APPEND(DB,W%B)
...
# 1930
     ...
     &   DXB(W%R%NMAX),TMP(W%R%NMAX),VV(W%R%NMAX))
      ...
      DW = 1e8
      scf: DO I=1,NSCF
         CALL DIIS_SAVE(DA,W%A)
         CALL DIIS_SAVE(DB,W%B)
         DWP = DW
...
! Mix potential
         CALL DIIS_APPEND(DA,W%A)
         CALL DIIS_APPEND(DB,W%B)
         CALL DIIS_STEP(DA,W%A)
         CALL DIIS_STEP(DB,W%B)
         CALL WMIX(W,WP,ALPHA,DW,MIX=.FALSE.)
...
! Copy W to WP
      ...
      IF (I>NSCF) THEN
         CALL vtutor%error("HFATOM: ERROR, unable to reach convergence in " // str(NSCF) // " cycles.")
      ELSE
...
!******************** SUBROUTINE HFSOLVE *******************************
!
!> Wrapper for the subroutine rhfatm::WSHOOT.
!>
...
# 2203
         ...
         CALL WSHOOT(W%R,W%N(I),W%L(I),REAL(CEILING(W%Z),q),VI,W%E(I),W%A(:,I),XIA,XIB,W%A(:,I),W%B(:,I),&
                  W%KI(I),W%KJ(I))
! Normalize wave function
         TMP(:)=W%A(:,I)*W%A(:,I)
         CALL SIMPI(W%R,TMP,WNA)
...
!
!> Copies the DFT all-electron and pseudized partial
...
! Compute the new partial wave
            CALL PWSHOOT(W%R,PP%LPS(I),W%Z,W%EPS(I),V,XA,XB,A,B,IRM)
! Break condition
            DW=MAXVAL(ABS(A-AP))
...
! simple mixing
         ...
         ENDDO scf
         IF (J>NSCF) THEN
            CALL vtutor%error("HF_PAR_WAVE: ERROR, partial wave " // str(I) // " not found within " &
               // str(NSCF) // " tries.")
         ENDIF
         W%WAE(:,I)=A
...
! Not found
            IF (LERR) THEN
               CALL vtutor%error("HF_PAR_WAVE: ERROR, no matching bound state: " // str(I) // " " // &
                  str(N_OF_CHANNEL(PP,I)) // " " // str(PP%LPS(I)))
            ENDIF
         ELSE
...
! Compute the new partial wave
!           CALL PWSHOOT(W%R,PP%LPS(I),W%Z,W%EPS(I),V,XA,XB,A,B,IRM)
!           NERR=0 ; CALL OUTINT(W%EPS(I),PP%LPS(I),W%Z,W%R,V,KI,A,B,NERR,RC=W%R%R(IRM+5),IHA=XA,IHB=XB)
            NERR=0 ; CALL OUTINT(-62.06_q,PP%LPS(I),W%Z,W%R,V,KI,A,B,NERR,RC=W%R%R(IRM+5),IHA=XA,IHB=XB)
! Break condition
            DW=MAXVAL(ABS(A-AP))
! test
            WRITE(*,*) J,KI,'DW=',DW
...
!
!> Orthogonalizes the partial waves stored in W0 with
...
!
!> Adds the Hartree-Fock core-valence exchange
...
!******************** SUBROUTINE WSHOOT ********************************
!> Computes solutions to the scalar relativistic Hartree-Fock
!> equations by inward and outward integration (shooting method).
!***********************************************************************
      SUBROUTINE WSHOOT(R,N,L,Z,V,E,AP,XA,XB,A,B,KI,KJ)
      USE prec
      USE radial
      USE constant
      USE string, ONLY: str
...
! outward integration of inhomogeneous equations
         NERR=0 ; CALL OUTINT(E,L,Z,R,V,KI,A,B,NERR,IHA=XA,IHB=XB)
...
! too few nodes
         IF (NODES<NNODES) THEN
            ...
            IF ((EMAX-EMIN)<(E*1E-10)) THEN
               CALL vtutor%error("WSHOOT: too few nodes: " // str(NODES) // " " // str(N) // " " // &
                  str(L) // " " // str(EMIN) // " " // str(E) // " " // str(EMAX))
            ENDIF
            CYCLE tries      
         ENDIF
...
! too many nodes
         IF (NODES>NNODES) THEN
            ...
            E=0.5*(E+EMIN)
            IF ((EMAX-EMIN)<(E*1E-10)) THEN 
               CALL vtutor%error("WSHOOT: too many nodes: " // str(NODES) // " " // str(N) // " " // &
                  str(L) // " " // str(EMIN) // " " // str(E) // " " // str(EMAX))
            ENDIF
            CYCLE tries
         ENDIF         
...
# 3422
      ...
      CALL vtutor%error("WSHOOT: nodes in inward integration: " // str(NODES) // " " // str(KI) // "&
         & " // str(KJ))
...
! outward integration of the imhomogeneous equations
         NERR=0 ; CALL OUTINT(E,L,Z,R,V,KI,ATMP,BTMP,NERR,RC=R%R(IRM+5),IHA=XA,IHB=XB)
...
# 3521
         ...
         CALL vtutor%error("PWSHOOT: ERROR, unable to match logarithmic derivatives within " // &
            str(NTRIES) // " attempts. \n " // str(NODES0) // " " // str(NODES) // " " // str(LGDR0) &
            // " " // str(LGDR) // " " // str(E) // " " // str(EMIN) // " " // str(EMAX))
      ENDIF
...
!
!***********************************************************************
      SUBROUTINE WMIX(W,WP,ALPHA,DWMAX,MIX)
      USE prec
      IMPLICIT NONE
      TYPE(atomic) W
      TYPE(atomic) WP
...
! local variables
      ...
      MY_MIX = .TRUE.; IF (PRESENT(MIX)) MY_MIX = MIX

      DW=0
      DO I=1,W%NS
         DO K=1,SIZE(W%W,1)
            DIFF = ABS(W%A(K,I)-WP%A(K,I))
            IF (DIFF>DW(I)) DW(I)=DIFF
         ENDDO
...
# 3564
         ...
         IF (MY_MIX) THEN
            W%A(:,I)=ALPHA*W%A(:,I)+(1._q-ALPHA)*WP%A(:,I)
            W%B(:,I)=ALPHA*W%B(:,I)+(1._q-ALPHA)*WP%B(:,I)
         ENDIF
...
!************************** SUBROUTINE WNORM ***************************
!> Helper routine to normalize the wavefunctions
!***********************************************************************
      SUBROUTINE WNORM(W)
      USE prec
      TYPE(atomic) W
...
! Normalize wave function
         CALL SIMPI(W%R,W%A(:,I)*W%A(:,I),WNA)
         CALL SIMPI(W%R,W%B(:,I)*W%B(:,I),WNB)
         WN = WNA+WNB/C/C
         W%A(:,I)=W%A(:,I)/SQRT(WN)
         W%B(:,I)=W%B(:,I)/SQRT(WN)
      ENDDO
      END SUBROUTINE WNORM
...
! Compute the charge density
      RHO=0
      ISTART=1;  IF (PRESENT(ILO)) ISTART=ILO
      IEND=W%NS; IF (PRESENT(IHI)) IEND=IHI
      DO I=ISTART,IEND
...
! local variables
      ...
      JSTART=1;  IF (PRESENT(JLO)) JSTART=JLO
      JEND=W%NS; IF (PRESENT(JHI)) JEND=JHI
      ...
      DO J=JSTART,JEND
         ...
         DO LL=LMIN,LMAX,2
            ...
            IF (I/=J) THEN
               SCALE=W%OCC(J)*CLEBG0(FAC,LI,LJ,LL)**2
            ELSE
               IF (LL/=0) THEN
                  SCALE=(W%OCC(J)-1)*(4*LJ+2)/(4*Lj+1)*CLEBG0(FAC,LI,LJ,LL)**2
               ELSE
                  SCALE=2
               ENDIF
            ENDIF
            ...
            XA(:,I)=XA(:,I)+SCALE*V(:)*W%A(:,J)
            XB(:,I)=XB(:,I)+SCALE*V(:)*W%B(:,J)
...
!
!***********************************************************************
      SUBROUTINE RAD_POT_DFT(RHO,Z,R,V,XC,EXC,LXC,LGGA)
...
! and double counting corrections
 ...
 lda: IF (TLDA.AND.LADDXC) THEN
         CALL RAD_LDA_XC(R,XC,TREL,0,RWRK2(:,1),RWRK1(:,:,1),VWRK(:,:,1),DEXC,DVXC,.FALSE.)
      ENDIF lda
...
! GGA if required
 ...
 gga: IF ((XC%LUSE_LIBXC.OR.XC%LDOGGA.OR.XC%LDOMETAGGA).AND.LADDXC.AND.LADDGC) THEN
         CALL RAD_GGA_XC(R,XC,TLDA,RWRK2(:,1),RWRK1(:,1,1),VWRK(:,1,1),DEXC_GGA,DVXC_GGA)
...
!> @author Tomas Bucko
!***********************************************************************
      SUBROUTINE RAD_POT_DFT2(RHO,RHO_comp,Z,R,V,XC,qstate,DOUBLEC,EXC,LXC,LGGA)
...
!  gga: IF ((XC%LUSE_LIBXC.OR.XC%LDOGGA.OR.XC%LDOMETAGGA).AND.LADDXC) THEN
!          CALL RAD_GGA_XC(R,XC,TLDA,RWRK2(:,1),RWRK1(:,1,1),VWRK(:,1,1),DEXC_GGA,DVXC_GGA)
!       ENDIF gga
...
!*********************** SUBROUTINE RAD_POT_DFT3 ************************
!> compensation charge via Watson sphere
!> @author Tomas Bucko
!*********************** SUBROUTINE RAD_POT_DFT3 ************************
      SUBROUTINE RAD_POT_DFT3(RHO,Z,R,V,XC,qdummy,Rcomp,DOUBLEC,EXC,LXC)
      USE prec
      USE constant
      USE radial
      USE setexm_struct_def, ONLY : xc_info
...
! Compute the core wave functions
      W%W=0; W%A=0; W%B=0
      DO I=1,W%NSCORE
         CALL CORE_WAVE_FKT(RHO, W%E(I), W%N(I), W%L(I), V, PP%R, CEILING(W%Z), &
...
! Consistency check
         IF (I>W%NS) THEN
            CALL vtutor%error("INITWF: ERROR, W inconsistently dimensioned: " // str(CHANNEL))
         ENDIF
! Consistency check
         IF (W%L(I)/=PP%LPS(CHANNEL)) THEN
            CALL vtutor%error("INITWF: ERROR, angular moments do not match: " // str(PP%LPS(CHANNEL)) &
               // " " // str(W%L(I)))
         ENDIF
...
! Sanity check
      IF ((I-1)/=W%NS) THEN
         CALL vtutor%error("INITWF: ERROR, did not find all wave functions: " // str(I-1) // " " // &
            str(W%NS))
      ENDIF
...
!     W%ZCORE=PP%ZCORE
      ...
      ALLOCATE(W%VX(W%R%NMAX,N),W%IH(W%R%NMAX,N),W%V(W%R%NMAX))
      ALLOCATE(W%KI(N),W%KJ(N))
...
!>
!> this routine is a derivative of start2/diff2.
!> much unnecessary indexing has been removed among other things. dale koelling
...
! convert to atomic units
      ...
      IHRA=0; IF (PRESENT(IHA)) IHRA=IHA/(2*RYTOEV)*R%R/AUTOA
      IHRB=0; IF (PRESENT(IHB)) IHRB=IHB/(2*RYTOEV)*R%R/AUTOA/C/C
...
!
!***********************************************************************
      FUNCTION RCRCUT(RHOCUT,ECUT,PP,XC,IO)
      USE base
      USE pseudo
      USE setexm_struct_def, ONLY : xc_info
      IMPLICIT NONE
