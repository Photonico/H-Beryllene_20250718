...
! _test_mani move hbar to top of module
   ...
   TYPE ELPHON_SETTINGS
      ...
      LOGICAL                    :: CALC_MATRIX_ELEMENTS = .FALSE.
      ...
      REAL(q), ALLOCATABLE       :: BZ_POINTS(:, :)                  !< Calculation at these Brillouin-(1._q,0._q) point (direct coordinates)
      ...
      LOGICAL                    :: WRITE_PHONON_DATA = .FALSE.
      LOGICAL                    :: READ_PHONON_DATA = .FALSE.
      LOGICAL                    :: FORCE_MELS_ZERO = .FALSE.
...
! COMPLEX(q), ALLOCATABLE,         SAVE :: CQIJ_INIT(:, :, :, :)
   ...
   TYPE ELPHON_INTERPOLATOR
      ...
      TYPE(WANNIER_DESCRIPTOR_T), ALLOCATABLE,  PRIVATE  :: WANNIER_DESCRIPTORS(:)
      ...
      COMPLEX(q), ALLOCATABLE,   PRIVATE  :: CDIJ_INIT(:, :, :, :)
...
!>
!> ## LELPH_ZPR
!> Calculates the (0._q,0._q)-point renormalization of the band gap
!>
!> ## LELPH_TEMP_GAP
...
!>
!> ## ELPH_BZ_POINTS
!> ZPR is evaluated at these Brillouin-(1._q,0._q) points. Points are given in
...
! Do any kind of calculation?
      CALL process_incar(INCAR, "LELPH_MELS", SETTINGS%CALC_MATRIX_ELEMENTS)
      ...
      CALL process_incar(INCAR, "LPHON_WRITE_DATA", SETTINGS%WRITE_PHONON_DATA)
      CALL process_incar(INCAR, "LPHON_READ_DATA", SETTINGS%READ_PHONON_DATA)

! Calculate the AE electron-phonon matrix element?
      CALL process_incar(INCAR, "LELPH_ALL_ELEC", SETTINGS%ALL_ELECTRON)

      CALL process_incar(INCAR, "LELPH_FORCE_MELS_ZERO", SETTINGS%FORCE_MELS_ZERO)
...
! Read Brillouin-(1._q,0._q) vectors
...
! Make sure a method is selected when we initialize
      ...
      CALL process_incar(INCAR, "PRIM_LATTICE", SETTINGS%PRIM_LATT, IERR)
...
# 348
   ...
   PURE LOGICAL FUNCTION SETTINGS_DO_PERTURB(THIS) RESULT(D)
      IMPLICIT NONE
      CLASS(ELPHON_SETTINGS), INTENT(IN)  :: THIS
      D = THIS%CALC_ZPR .OR. THIS%CALC_TEMPERATURE_GAP .OR. THIS%WRITE_DATA .OR. THIS%DO_POLAR .OR. THIS%EXTERNAL_INTERFACE .OR. THIS%CALC_MATRIX_ELEMENTS
      D = D .AND. .NOT. THIS%DO_LOAD_DATA()
   END FUNCTION
   ...
   PURE LOGICAL FUNCTION SETTINGS_DO_CALC(THIS) RESULT(D)
      IMPLICIT NONE
      CLASS(ELPHON_SETTINGS), INTENT(IN)  :: THIS
      D = THIS%CALC_BANDSTRUCTURE .OR. THIS%CALC_ZPR .OR. THIS%CALC_TEMPERATURE_GAP .OR. THIS%CALC_MATRIX_ELEMENTS
   END FUNCTION
...
!_______________________________________________________________________________
   SUBROUTINE ELPH_IO(EP_INTERPOL, SETTINGS, PRIM_CELL, T_INFO, POLAR_DATA, IO)
...
! Write data to file
      IF (SETTINGS%WRITE_DATA) THEN
         IF (SETTINGS%METHOD == METHOD_LOAD_DATA) CALL vtutor%bug( &
            "LELPH_WRITE_DATA = .TRUE. cannot be used in conjunction with ELPH_METHOD = LOAD_DATA", "elphon.F", 457)
         IF (.NOT. EP_INTERPOL%IS_INITIALIZED) CALL vtutor%bug("LELPH_WRITE_DATA but module not initialized!", "elphon.F", 458)
         IF (.NOT. EP_INTERPOL%IS_COMPLETED) CALL vtutor%bug("LELPH_WRITE_DATA but calculation not completed!", "elphon.F", 459)


! Write HDF5 data
         call vh5_error(vh5_file_create_or_overwrite('elphon_data.h5', FILE_ID),"elphon.F",463)
         CALL PRIM_CELL%WRITE_HDF5(FILE_ID, 'supercell')
         call vh5_error(vh5_write(FILE_ID, "supercell/prim_atom_masses", T_INFO%POMASS(T_INFO%ITYP(PRIM_CELL%SUPER_IDX(1, :)))),"elphon.F",465)
         CALL EP_INTERPOL%WRITE_HDF5(FILE_ID, 'elphon')
         CALL PHON_WRITE_HDF5(PRIM_CELL, FILE_ID, 'phonon')
         IF (ALLOCATED(POLAR_DATA%BORN_EFF_CHARGES)) THEN
            call vh5_error(vh5_write(FILE_ID, "polar/born_eff_charges", POLAR_DATA%BORN_EFF_CHARGES),"elphon.F",469)
            call vh5_error(vh5_write(FILE_ID, "polar/dielectric_tensor", POLAR_DATA%DIELECTRIC_TENSOR),"elphon.F",470)
         ENDIF
         call vh5_error(vh5_file_close_writing(FILE_ID),"elphon.F",472)

         IF (SETTINGS%WRITE_PHONON_DATA) THEN
            call vh5_error(vh5_file_create_or_overwrite('phonon_data.h5', FILE_ID),"elphon.F",475)
            CALL PHON_WRITE_HDF5(PRIM_CELL, FILE_ID, 'phonon')
            call vh5_error(vh5_file_close_writing(FILE_ID),"elphon.F",477)
         ENDIF
# 482
...
! Read HDF5 data
         ...
         call vh5_error(vh5_file_open_read('elphon_data.h5', FILE_ID),"elphon.F",497)
         CALL EP_INTERPOL%READ_HDF5(PRIM_CELL, FILE_ID, 'elphon')
         CALL PHON_READ_HDF5(PRIM_CELL, FILE_ID, 'phonon')
         call vh5_error(vh5_file_close(FILE_ID),"elphon.F",500)

         IF (SETTINGS%DO_CALC()) THEN
            DO SPIN_IDX = 1, SIZE(EP_INTERPOL%WANNIER_DESCRIPTORS)
               CALL EP_INTERPOL%WANNIER_DESCRIPTORS(SPIN_IDX)%CALC_MIN_IMAGES()
               CALL EP_INTERPOL%WANNIER_DESCRIPTORS(SPIN_IDX)%CALC_MIN_IMAGES_TO_ATOM()
            ENDDO
         ENDIF

         IF (SETTINGS%READ_PHONON_DATA) THEN
            call vh5_error(vh5_file_open_read('phonon_data.h5', FILE_ID),"elphon.F",510)
            CALL PHON_READ_HDF5(PRIM_CELL, FILE_ID, 'phonon')
            call vh5_error(vh5_file_close(FILE_ID),"elphon.F",512)
         ENDIF
# 517
...
! _test_mani write wannier functions to disk.
   SUBROUTINE EPI_WRITE_WANNIER_FUNCS(THIS, IO)
      ...
      CALL OUTWAV(IO, THIS%W_WAN_INIT%WDES, THIS%W_WAN_INIT, THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL%SUPER_LATT, 0._q, 'wan')
   END SUBROUTINE
...
! _test_mani read wannier functions from disk.
   SUBROUTINE EPI_READ_WANNIER_FUNCS(THIS, WDES, NONLR_S, NONL_S, IO)
      ...
      CALL INWAV_ALTERNATIVE(IO, WDES, THIS%W_WAN_INIT, WDES%GRID, THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL%SUPER_LATT, LWREAD, 'wan')

      IF (.NOT.LWREAD) CALL vtutor%error('EPI_READ_WANNIER_FUNCS: Unable to read WAVECAR.wan.')

      CALL PROALL(WDES%GRID, THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL%SUPER_LATT, NONLR_S, NONL_S, THIS%W_WAN_INIT)
   END SUBROUTINE


   SUBROUTINE WRITE_MATRIX_ELEMENTS(EP_INTERPOL, T_INFO, IO)
      ...
      CLASS(ELPHON_INTERPOLATOR), INTENT(INOUT) :: EP_INTERPOL
      TYPE(type_info), INTENT(IN) :: T_INFO
      TYPE(in_struct), INTENT(IN) :: IO

      TYPE(kpoints_struct) :: K_POINTS, Q_POINTS
      COMPLEX(q), ALLOCATABLE :: COUPLING(:, :, :)
      COMPLEX(q), ALLOCATABLE :: ALL_COUPLINGS(:, :, :, :, :, :)
      COMPLEX(q), ALLOCATABLE :: COUPLING_SLICE(:, :, :, :, :, :)
      REAL(q), ALLOCATABLE :: EV(:)
      REAL(q), ALLOCATABLE :: ALL_EV_KQ(:, :, :, :)
      REAL(q), ALLOCATABLE :: EV_KQ_SLICE(:, :, :, :)
      REAL(q), ALLOCATABLE :: EV_K(:, :, :)
      REAL(q), ALLOCATABLE :: FREQU(:, :)
      COMPLEX(q), ALLOCATABLE :: TMP(:, :)
      REAL(q) :: K_VEC(3), Q_VEC(3)
      INTEGER :: NUM_SPIN, NUM_MODES, NUM_BANDS
      INTEGER :: SPIN_IDX, K_IDX, Q_IDX
      ...
      K_POINTS%LTET = .FALSE.
      CALL RD_KPOINTS(K_POINTS, EP_INTERPOL%WANNIER_DESCRIPTORS(1)%PRIM_CELL%PRIM_LATT, &
         .FALSE., .FALSE., IO%IU6, IO%IU0, FILE_NAME = 'KPOINTS_G')
      Q_POINTS%LTET = .FALSE.
      CALL RD_KPOINTS(Q_POINTS, EP_INTERPOL%WANNIER_DESCRIPTORS(1)%PRIM_CELL%PRIM_LATT, &
         .FALSE., .FALSE., IO%IU6, IO%IU0, FILE_NAME = 'QPOINTS_G')

      NUM_BANDS = EP_INTERPOL%WANNIER_DESCRIPTORS(1)%NUM_PRIM_ORBITALS
      NUM_SPIN = SIZE(EP_INTERPOL%HAM_INIT)
      NUM_MODES = EP_INTERPOL%COUPLING_WANN(1)%NUM_MODES

      IF (COMM%NODE_ME == COMM%IONODE) THEN
         ALLOCATE(ALL_COUPLINGS(NUM_BANDS, NUM_BANDS, NUM_MODES, Q_POINTS%NKPTS, K_POINTS%NKPTS, NUM_SPIN))
         ALL_COUPLINGS = 0
         ALLOCATE(ALL_EV_KQ(NUM_BANDS, Q_POINTS%NKPTS, K_POINTS%NKPTS, NUM_SPIN))
         ALL_EV_KQ = 0
      ENDIF

      IF (K_POINTS%NKPTS > Q_POINTS%NKPTS) THEN
         CALL GET_MPI_STRIP(COMM, K_POINTS%NKPTS, K_START, K_END)
         Q_START = 1
         Q_END = Q_POINTS%NKPTS
      ELSE
         CALL GET_MPI_STRIP(COMM, Q_POINTS%NKPTS, Q_START, Q_END)
         K_START = 1
         K_END = K_POINTS%NKPTS
      ENDIF

      ALLOCATE(COUPLING_SLICE(NUM_BANDS, NUM_BANDS, NUM_MODES, Q_END - Q_START + 1, K_END - K_START + 1, NUM_SPIN))
      COUPLING_SLICE = 0
      ALLOCATE(EV_KQ_SLICE(NUM_BANDS, Q_END - Q_START + 1, K_END - K_START + 1, NUM_SPIN))
      EV_KQ_SLICE = 0

      DO SPIN_IDX = 1, NUM_SPIN
         DO K_IDX = K_START, K_END
            K_VEC = K_POINTS%VKPT(:, K_IDX)
            DO Q_IDX = Q_START, Q_END
               Q_VEC = Q_POINTS%VKPT(:, Q_IDX)

               CALL EP_INTERPOL%CALC_COUPLING(COUPLING, Q_VEC, K_VEC, SPIN_IDX, T_INFO)
               COUPLING_SLICE(:, :, :, Q_IDX - Q_START + 1, K_IDX - K_START + 1, SPIN_IDX) = COUPLING(:, :, :)

               CALL EP_INTERPOL%HAM_INIT(SPIN_IDX)%DIAG_AT_K(K_VEC + Q_VEC, EV, TMP)
               EV_KQ_SLICE(:, Q_IDX - Q_START + 1, K_IDX - K_START + 1, SPIN_IDX) = EV(:)
            ENDDO
         ENDDO
      ENDDO

      IF (COMM%NODE_ME /= COMM%IONODE) THEN
         BOUNDS = [K_START, K_END, Q_START, Q_END]
         CALL M_send_i(COMM, COMM%IONODE, BOUNDS(1), 4)
         CALL M_send_z(COMM, COMM%IONODE, COUPLING_SLICE(1, 1, 1, 1, 1, 1), SIZE(COUPLING_SLICE))
         CALL M_send_d(COMM, COMM%IONODE, EV_KQ_SLICE(1, 1, 1, 1), SIZE(EV_KQ_SLICE))
         DEALLOCATE(COUPLING_SLICE)
         DEALLOCATE(EV_KQ_SLICE)
      ELSE
         ALL_COUPLINGS(:, :, :, Q_START:Q_END, K_START:K_END, :) = COUPLING_SLICE(:, :, :, :, :, :)
         ALL_EV_KQ(:, Q_START:Q_END, K_START:K_END, :) = EV_KQ_SLICE(:, :, :, :)
         DEALLOCATE(COUPLING_SLICE)
         DEALLOCATE(EV_KQ_SLICE)

         DO NODE_IDX = 1, COMM%NCPU
            IF (NODE_IDX == COMM%IONODE) CYCLE
            CALL M_recv_i(COMM, NODE_IDX, BOUNDS, 4)
            K_START = BOUNDS(1)
            K_END = BOUNDS(2)
            Q_START = BOUNDS(3)
            Q_END = BOUNDS(4)
            ...
            ALLOCATE(COUPLING_SLICE(NUM_BANDS, NUM_BANDS, NUM_MODES, Q_END - Q_START + 1, K_END - K_START + 1, NUM_SPIN))
            CALL M_recv_z(COMM, NODE_IDX, COUPLING_SLICE(1, 1, 1, 1, 1, 1), SIZE(COUPLING_SLICE))
            ALL_COUPLINGS(:, :, :, Q_START:Q_END, K_START:K_END, :) = COUPLING_SLICE(:, :, :, :, :, :)
            DEALLOCATE(COUPLING_SLICE)

            ALLOCATE(EV_KQ_SLICE(NUM_BANDS, Q_END - Q_START + 1, K_END - K_START + 1, NUM_SPIN))
            CALL M_recv_d(COMM, NODE_IDX, EV_KQ_SLICE(1, 1, 1, 1), SIZE(EV_KQ_SLICE))
            ALL_EV_KQ(:, Q_START:Q_END, K_START:K_END, :) = EV_KQ_SLICE(:, :, :, :)
            DEALLOCATE(EV_KQ_SLICE)
         ENDDO

         ALLOCATE(EV_K(NUM_BANDS, K_POINTS%NKPTS, NUM_SPIN))
         DO SPIN_IDX = 1, NUM_SPIN
            DO K_IDX = 1, K_POINTS%NKPTS
               CALL EP_INTERPOL%HAM_INIT(SPIN_IDX)%DIAG_AT_K(K_POINTS%VKPT(:, K_IDX), EV, TMP)
               EV_K(:, K_IDX, SPIN_IDX) = EV(:)
            ENDDO
         ENDDO

         ALLOCATE(FREQU(NUM_MODES, Q_POINTS%NKPTS))
         DO Q_IDX = 1, Q_POINTS%NKPTS
            CALL PHON_CALC_PHONONS(Q_POINTS%VKPT(:, Q_IDX), EP_INTERPOL%WANNIER_DESCRIPTORS(1)%PRIM_CELL, T_INFO, EV, TMP)
            FREQU(:, Q_IDX) = EV(:)
         ENDDO
      ENDIF

      call vh5_error(vh5_file_create_or_overwrite('elphon-mels.h5', FILE_ID),"elphon.F",688)
      call vh5_error(vh5_write(FILE_ID, "k_points", K_POINTS%VKPT),"elphon.F",689)
      call vh5_error(vh5_write(FILE_ID, "q_points", Q_POINTS%VKPT),"elphon.F",690)
      call vh5_error(vh5_write(FILE_ID, "matrix_elements", ALL_COUPLINGS),"elphon.F",691)
      call vh5_error(vh5_write(FILE_ID, "electron_eigenvalues_k", EV_K),"elphon.F",692)
      call vh5_error(vh5_write(FILE_ID, "electron_eigenvalues_kq", ALL_EV_KQ),"elphon.F",693)
      call vh5_error(vh5_write(FILE_ID, "phonon_frequencies_q", FREQU),"elphon.F",694)
      call vh5_error(vh5_file_close_writing(FILE_ID),"elphon.F",695)      
   END SUBROUTINE
...
! _test_mani Serialization routine to write precalculated data to disk.
   ...
   SUBROUTINE EPI_WRITE_HDF5(THIS, LOC_ID, GROUP_PATH)
      ...
      USE vhdf5_base,   ONLY: vh5_error, &
                              vh5_write, &
                              vh5_group_open_or_create, &
                              vh5_group_close_writing
      IMPLICIT NONE
      ...
      IF (.NOT. THIS%IS_INITIALIZED) CALL vtutor%bug("Module not initialized!", "elphon.F", 716)
      IF (.NOT. THIS%IS_COMPLETED) CALL vtutor%bug("Calculation not completed!", "elphon.F", 717)

      N_SPIN = SIZE(THIS%HAM_INIT)

! Write number of spin components
      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // '/spin_count', N_SPIN),"elphon.F",722)

      DO SPIN_IDX = 1, N_SPIN
         call vh5_error(vh5_group_open_or_create(LOC_ID, GROUP_PATH // '/spin_' // str(SPIN_IDX), GROUP_ID),"elphon.F",725)
! Write the Wannier descriptors
         CALL THIS%WANNIER_DESCRIPTORS(SPIN_IDX)%WRITE_HDF5(GROUP_ID, 'wannier_descriptor')
! Write the Wannier matrices
         CALL THIS%HAM_INIT(SPIN_IDX)%WRITE_HDF5(GROUP_ID, 'initial_hamiltonian')
         CALL THIS%COUPLING_WANN(SPIN_IDX)%WRITE_HDF5(GROUP_ID, 'coupling_wannier')
         CALL THIS%OVERLAP_PERT(SPIN_IDX)%WRITE_HDF5(GROUP_ID, 'perturbed_overlap')
         call vh5_error(vh5_group_close_writing(GROUP_ID),"elphon.F",732)
      ENDDO
   END SUBROUTINE
...
! Read number of spin components
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // '/spin_count', N_SPIN),"elphon.F",757)

! Setup allocation
      ALLOCATE(THIS%WANNIER_DESCRIPTORS(N_SPIN))
      ALLOCATE(THIS%HAM_INIT(N_SPIN))
      ALLOCATE(THIS%COUPLING_WANN(N_SPIN))
      ALLOCATE(THIS%OVERLAP_PERT(N_SPIN))

      DO SPIN_IDX = 1, N_SPIN
         call vh5_error(vh5_group_open(LOC_ID, GROUP_PATH // '/spin_' // str(SPIN_IDX), GROUP_ID),"elphon.F",766)
! Write the Wannier descriptors
         CALL THIS%WANNIER_DESCRIPTORS(SPIN_IDX)%READ_HDF5(PRIM_CELL, GROUP_ID, 'wannier_descriptor')
! Write the Wannier matrices
         CALL THIS%HAM_INIT(SPIN_IDX)%READ_HDF5(THIS%WANNIER_DESCRIPTORS(SPIN_IDX), GROUP_ID, 'initial_hamiltonian')
         CALL THIS%COUPLING_WANN(SPIN_IDX)%READ_HDF5(THIS%WANNIER_DESCRIPTORS(SPIN_IDX), GROUP_ID, 'coupling_wannier')
         CALL THIS%OVERLAP_PERT(SPIN_IDX)%READ_HDF5(THIS%WANNIER_DESCRIPTORS(SPIN_IDX), GROUP_ID, 'perturbed_overlap')
         call vh5_error(vh5_group_close(GROUP_ID),"elphon.F",773)
      ENDDO
...
! _test_mani testing methods
   SUBROUTINE TEST_WANNIER_MAT_TRANS_SYMM(WANNIER_MAT, DESCRIPTOR, FILE_NAME, IO)
      ...
      COMPLEX(q),                         INTENT(IN)  :: WANNIER_MAT(:, :)
...
! CALL FFTEXT_WDES1(MY_W1%WDES1, MY_W1%CR, MY_W1%CPTWFP, .FALSE.) ! _test_mani can't use in GPU version
   ...
   SUBROUTINE IMPORT_WANNIER_FROM_PRIM(W, W_WANNIER, NUM_ORBS_IN_CELL, PRIM_CELL, LATT_CUR, NONL_S, NONLR_S, CQIJ, IO)
      ...
      NKPTS_STORE = W_WANNIER%WDES%NKPTS
      W_WANNIER%WDES%NKPTS = 1

      CALL PHASE(W_WANNIER%WDES, NONL_S, 1)

      DO SPIN_IDX = 1, W_WANNIER%WDES%ISPIN
         DO ORB_IDX = 1, NUM_ORBS_IN_CELL
            DO BAND_IDX_LOCAL = 1, WDES1%NBANDS
               ...
               IF (W_WANNIER%WDES%ISPIN == 1) THEN
                  WRITE(FILE_NAME, '(A, I5.5, A)') "wannier90_", ORB_IDX, ".xsf"
                  CALL READ_WANNIER_CENTERS("wannier90_centres.xyz", WANNIER_CENTERS)
               ELSE
                  WRITE(FILE_NAME, '(A, I1, A, I5.5, A)') "wannier90.", SPIN_IDX, "_", ORB_IDX, ".xsf"
                  CALL READ_WANNIER_CENTERS("wannier90." // str(SPIN_IDX) // "_centres.xyz", WANNIER_CENTERS)
               ENDIF
               ...
               CALL CREATE_ORBITAL_FROM_FIELD(ELEMENT(W_WANNIER, WDES1, BAND_IDX_LOCAL, SPIN_IDX), LATT_CUR, NONL_S, NONLR_S, CQIJ, FIELD, GRID_LATTICE, GRID_OFFSET)
            ENDDO
         ENDDO
      ENDDO

      W_WANNIER%WDES%NKPTS = NKPTS_STORE
...
! _test_mani TODO: general case.
   SUBROUTINE GATHER_PRIM_CELL_WAVE_FUNCTIONS(W, PRIM_CELL, LATT_CUR, WAVE_FUNC, BAND_INDICES)
      ...
      TYPE(PRIM_CELL_T),      INTENT(IN)  :: PRIM_CELL
      TYPE(latt),             INTENT(IN)  :: LATT_CUR
      COMPLEX(q), ALLOCATABLE,      INTENT(OUT) :: WAVE_FUNC(:, :)
      ...
      IF (WDES1%GRID%RL%NFAST /= 1) CALL vtutor%bug("GATHER_PRIM_CELL_WAVE_FUNCTIONS: NFAST /= 1 not implemented!", "elphon.F", 1393)

      IF (WDES1%GRID%COMM%NCPU /= 1) CALL vtutor%bug("GATHER_PRIM_CELL_WAVE_FUNCTIONS: NCPU /= 1 not implemented!", "elphon.F", 1395)
...
! Copy the relevant part of the wave function to the output array.
         WAVE_FUNC(:, I) = W1%CR(POINT_INDICES(:)) ! Imaginary part is (0._q,0._q) in gamma-only version.
      ENDDO

      CALL DELWAV(W1, .TRUE.)

      CALL M_sum_z(W%WDES%COMM, WAVE_FUNC(1, 1), NUM_POINTS * NB_INC) ! _test_mani comm?
...
! rewrite/remove
   SUBROUTINE GATHER_PRIM_CELL_WAVE_FUNCTIONS_3_27(W, PRIM_CELL, LATT_CUR, WAVE_FUNC, BAND_INDICES)
      ...
      TYPE(PRIM_CELL_T),         INTENT(IN)  :: PRIM_CELL
      TYPE(latt),                INTENT(IN)  :: LATT_CUR
      COMPLEX(q), ALLOCATABLE, TARGET, INTENT(OUT) :: WAVE_FUNC(:, :)
      INTEGER, ALLOCATABLE,      INTENT(OUT) :: BAND_INDICES(:)
      ...
      REAL(q)              :: R_IN_GRID(3)
      REAL(q)              :: R_IN_BOX(3)
      COMPLEX(q), POINTER        :: WAVE_FUNC_PTR(:, :, :, :, :, :, :)
      COMPLEX(q), POINTER  :: CR_PTR(:, :, :)
      INTEGER              :: X, Y, Z
      INTEGER              :: SHIFT_X, SHIFT_Y, SHIFT_Z
      INTEGER              :: R_VEC(3)
...
! Loop over neighbor cells
      ...
      CALL M_sum_z(W%WDES%COMM, WAVE_FUNC(1, 1), SIZE(WAVE_FUNC)) ! _test_mani comm?
...
! I don't trust the compiler to do deallocations in a smart way, so I do all the deallocations manually as soon as possible.
   SUBROUTINE CALC_WANNIER_ORBITALS_SCDM(W, NUM_ORBS_IN_CELL, W_WANNIER, T, MU, SIGMA, PRIM_CELL, NONL_S, NONLR_S, CQIJ, IO)
      ...
      TYPE(wavedes1)       :: WDES1
      TYPE(wavefun1)       :: W1
      COMPLEX(q), ALLOCATABLE    :: PSI(:, :)
      COMPLEX(q), ALLOCATABLE    :: PSI_D(:, :)
      COMPLEX(q), ALLOCATABLE    :: SOVL(:, :)
...
! Use pivot, then orthonormalization via SVD. _test_mani
      PSI_D(:, :) = CONJG(TRANSPOSE(PSI(PIVOT(:), :)))
...
! _test_mani lincom
   ...
   SUBROUTINE CALC_WANNIER_ORBITALS_FROM_PRIM(W, NUM_ORBS_IN_CELL, W_WANNIER, T, PRIM_CELL, NONL_S, NONLR_S, CQIJ, IO)
      ...
      DO SPIN_IDX = 1, W_WANNIER%WDES%ISPIN
         CALL POPULATE_SUPERCELL_WITH_ORBITALS(ELEMENTS(W_WANNIER, WDES1, SPIN_IDX), NUM_ORBS_IN_CELL, PRIM_CELL)
      ENDDO
      CALL PROALL(W_WANNIER%WDES%GRID, PRIM_CELL%SUPER_LATT, NONLR_S, NONL_S, W_WANNIER)
...
! Orthonormalization via SVD
            CALL SVD%CALC(T(:, :, K_IDX, SPIN_IDX), INFO) ! _test_mani handle errors
...
! Target cell is deduced from global band index (1)
...
!    CALL ZGEMV('N', NUM_ROWS, NUM_ROWS, (1._q,0._q), Q_MAT, NUM_ROWS, A(:, COL_IDX), 1, (0._q,0._q), BUFFER, 1)
!    A(:, COL_IDX) = BUFFER
! ENDDO
         ENDDO
      ENDDO
...
! Use rank remapping to represent 3D data
      ...
      CALL M_barrier(W_WANN%WDES%COMM)
...
! WRITE(UNIT, '(3F12.6)') 0., 0., 0.
   ...
   SUBROUTINE CALC_WANNIER_HAM_FROM_TRAFO(W_BLOCH, UK, HAM_WANNIER)
      ...
      COMPLEX(q), INTENT(IN) :: UK(:, :, :)
      COMPLEX(q), INTENT(OUT) :: HAM_WANNIER(:, :, :)
...
! Calculate the Hamiltonian matrix using a simple basis transform
         DO WAN_IDX_2 = 1, NUM_WAN
            DO WAN_IDX_1 = 1, NUM_WAN
               HAM_WANNIER(WAN_IDX_1, WAN_IDX_2, SPIN_IDX) = &
               HAM_WANNIER(WAN_IDX_1, WAN_IDX_2, SPIN_IDX) + SUM( &
                  CONJG(UK(:, WAN_IDX_1, SPIN_IDX)) * &
                  REAL(W_BLOCH%CELTOT(1 : NB_TOT, 1, SPIN_IDX), q) * &
                  UK(:, WAN_IDX_2, SPIN_IDX) &
               )
            ENDDO
         ENDDO
      ENDDO
...
! _test_mani Helper routine ATM.
   SUBROUTINE ELPH_INTERPOLATE_BANDSTRUCTURE(INTERPOL, IO)
      ...
      DO SPIN = 1, SIZE(INTERPOL%HAM_INIT)
         ASSOCIATE( &
            PC => INTERPOL%WANNIER_DESCRIPTORS(SPIN)%PRIM_CELL, &
            HAM => INTERPOL%HAM_INIT(SPIN))
            ...
            CALL WRITE_Q_DATA_TO_FILE(PATH_QUANTITY, Q_PATH%VKPT, 'debug_band_struct_' // str(SPIN), PC%PRIM_LATT%B, IO)
            DEALLOCATE(PATH_QUANTITY)
         END ASSOCIATE
      ENDDO
...
! locals
      ...
      IF (INTERPOL%IS_COMPLETED .AND. .NOT. INTERPOL%IS_INITIALIZED) CALL vtutor%bug("IS_COMPLETED but not IS_INITIALIZED", "elphon.F", 2372)

      ASSOCIATE( &
         PC => INTERPOL%WANNIER_DESCRIPTORS(1)%PRIM_CELL)

         IF (IO%IU0 >= 0) WRITE(*, *) "elphon start output"

         CALL PC%SETUP_ADDITIONAL_DATA()

         IF (SETTINGS%FORCE_MELS_ZERO) THEN
            DO SPIN_IDX = 1, SIZE(INTERPOL%HAM_INIT)
               CALL CORRECT_COUPLING_WANN(PC, T_INFO, INTERPOL%COUPLING_WANN(SPIN_IDX))
            ENDDO
         ENDIF

         IF (SETTINGS%CALC_MATRIX_ELEMENTS) THEN

            CALL WRITE_MATRIX_ELEMENTS(INTERPOL, T_INFO, IO)
         ...
         IF (SETTINGS%CALC_BANDSTRUCTURE) THEN
            ...
            IF (.NOT. INTERPOL%IS_INITIALIZED) CALL vtutor%bug( &
               "Bandstructure requested but module not initialized.", "elphon.F", 2397)
         ...
         IF (SETTINGS%CALC_ZPR) THEN
            ...
            IF (.NOT. INTERPOL%IS_COMPLETED) CALL vtutor%bug("ZPR requested but module not completed.", "elphon.F", 2407)

            CALL BRILL_ZONE%INIT(PC%PRIM_LATT)
            CALL BRILL_ZONE%SAMPLE(PC%SUPER_LATT%B, SETTINGS%BZ_SUBDIVISION)
            CALL M_bcast_d(COMM, BRILL_ZONE%QMESH(1, 1), size(BRILL_ZONE%QMESH)) ! _test_mani Improve 1 handling
         ...
         IF (SETTINGS%CALC_TEMPERATURE_GAP) THEN
            ...
            IF (.NOT. INTERPOL%IS_COMPLETED) CALL vtutor%bug("Temperature dependence requested but module not completed.", "elphon.F", 2446)
...
! _test_mani helper routine ATM.
   SUBROUTINE ELPH_TEST_COUPLING(INTERPOL, T_INFO, IO)
      ...
      DO SPIN = 1, SIZE(INTERPOL%HAM_INIT)
         CALL PLOT_COUPLING(INTERPOL%COUPLING_WANN(SPIN), INTERPOL%HAM_INIT(SPIN), [REAL(q):: 0, 0, 0], "debug_elph_element_" // str(SPIN), .TRUE., T_INFO, IO)
         CALL WRITE_COUPLING(INTERPOL%COUPLING_WANN(SPIN), INTERPOL%HAM_INIT(SPIN), [REAL(q):: 0, 0, 0], "debug_elph_mat_" // str(SPIN), T_INFO, IO, 4)
      ENDDO
   END SUBROUTINE

   subroutine CORRECT_COUPLING_WANN(PRIM_CELL, T_INFO, COUPLING_WANN)
      ...
      TYPE(PRIM_CELL_T), INTENT(IN) :: PRIM_CELL
      TYPE(type_info), INTENT(IN) :: T_INFO
      TYPE(WANNIER_MAT_MODES_T), INTENT(INOUT) :: COUPLING_WANN

      COMPLEX(q), ALLOCATABLE :: COUPLING_BLOCH(:, :, :, :, :)
      INTEGER :: NUM_ORBS, NUM_MODES, NUM_CELLS
      INTEGER :: I, J, KX, KY, KZ, K_IDX, Q_IDX
      REAL(q), ALLOCATABLE :: K_POINTS(:, :)
      REAL(q) :: K_CART(3), Q_CART(3)
      REAL(q) :: R1(3), R2(3)
      INTEGER :: K_MESH(3)
      INTEGER :: P_IDX
      ...
      COMPLEX(q), ALLOCATABLE :: MODE_TRAFO(:, :)
      COMPLEX(q), ALLOCATABLE :: PHASES(:, :, :)
      INTEGER :: K_START, K_END

      NUM_ORBS = COUPLING_WANN%DESCRIPTOR%NUM_PRIM_ORBITALS
      NUM_MODES = COUPLING_WANN%NUM_MODES
      NUM_CELLS = PRIM_CELL%NUM_CELLS

      DO I = 1, 3
         K_MESH(I) = NINT(PRIM_CELL%SUPER_TO_PRIM(I, I))
      ENDDO

      ALLOCATE(K_POINTS(3, NUM_CELLS))
      ALLOCATE(COUPLING_BLOCH(NUM_ORBS, NUM_ORBS, NUM_MODES, NUM_CELLS, NUM_CELLS))
      COUPLING_BLOCH = 0
      ALLOCATE(PHASES(NUM_CELLS, NUM_CELLS, NUM_CELLS))

      CALL GET_MPI_STRIP(COMM_WORLD, NUM_CELLS, K_START, K_END)
      ...
      DO KZ = 0, K_MESH(3) - 1
         DO KY = 0, K_MESH(2) - 1
            DO KX = 0, K_MESH(1) - 1
               K_IDX = K_IDX + 1
               K_POINTS(:, K_IDX) = [REAL(q):: KX, KY, KZ] / K_MESH
            ENDDO
         ENDDO
      ENDDO

      DO K_IDX = 1, NUM_CELLS
         K_CART = MATMUL(PRIM_CELL%PRIM_LATT%B, K_POINTS(:, K_IDX))
         DO Q_IDX = 1, NUM_CELLS
            Q_CART = MATMUL(PRIM_CELL%PRIM_LATT%B, K_POINTS(:, Q_IDX))
            DO CELL_IDX_1 = 1, NUM_CELLS
               R1 = MATMUL(PRIM_CELL%SUPER_LATT%A, PRIM_CELL%GET_POSION(CELL_IDX_1, 1) - PRIM_CELL%GET_POSION(1, 1))
               PHASES(CELL_IDX_1, Q_IDX, K_IDX) = EXP(CITPI * DOT_PRODUCT(K_CART + Q_CART, R1))
            ENDDO
         ENDDO
      ENDDO
      ...
      DO K_IDX = K_START, K_END
         DO Q_IDX = 1, NUM_CELLS
            DO CELL_IDX_2 = 1, NUM_CELLS
               DO CELL_IDX_1 = 1, NUM_CELLS
                  COUPLING_BLOCH(:, :, :, Q_IDX, K_IDX) = COUPLING_BLOCH(:, :, :, Q_IDX, K_IDX) + &
                  COUPLING_WANN%ELEMENTS(:, :, CELL_IDX_1, CELL_IDX_2, :) * &
                  CONJG(PHASES(CELL_IDX_1, Q_IDX, K_IDX)) * &
                  PHASES(CELL_IDX_2, 1, K_IDX)
               ENDDO
            ENDDO
         ENDDO
      ENDDO

      CALL M_sum_z(COMM_WORLD, COUPLING_BLOCH(1, 1, 1, 1, 1), SIZE(COUPLING_BLOCH))

      CALL PHON_CALC_PHONONS([0._q, 0._q, 0._q], PRIM_CELL, T_INFO, TMP, MODE_TRAFO)
      DO P_IDX = 1, PRIM_CELL%NUM_PRIM_ATOMS
         MODE_TRAFO(P_IDX * 3 - 2 : P_IDX * 3, :) = MODE_TRAFO(P_IDX * 3 - 2 : P_IDX * 3, :) / &
         SQRT(T_INFO%POMASS(T_INFO%ITYP(PRIM_CELL%SUPER_IDX(1, P_IDX))))
      ENDDO
      DO K_IDX = 1, NUM_CELLS
         DO ORB_IDX_2 = 1, NUM_ORBS
            DO ORB_IDX_1 = 1, NUM_ORBS
               COUPLING_BLOCH(ORB_IDX_1, ORB_IDX_2, :, 1, K_IDX) = &
               MATMUL(COUPLING_BLOCH(ORB_IDX_1, ORB_IDX_2, :, 1, K_IDX), MODE_TRAFO)
            ENDDO
         ENDDO
      ENDDO

      COUPLING_BLOCH(:, :, :3, 1, :) = 0
      ...
      DO P_IDX = 1, PRIM_CELL%NUM_PRIM_ATOMS
         ...
         SQRT(T_INFO%POMASS(T_INFO%ITYP(PRIM_CELL%SUPER_IDX(1, P_IDX))))
      ENDDO
      MODE_TRAFO = CONJG(TRANSPOSE(MODE_TRAFO))
      DO K_IDX = 1, NUM_CELLS
         DO ORB_IDX_2 = 1, NUM_ORBS
            DO ORB_IDX_1 = 1, NUM_ORBS
               COUPLING_BLOCH(ORB_IDX_1, ORB_IDX_2, :, 1, K_IDX) = &
         ...
         DO Q_IDX = 1, NUM_CELLS
            DO CELL_IDX_2 = 1, NUM_CELLS
               DO CELL_IDX_1 = 1, NUM_CELLS
                  COUPLING_WANN%ELEMENTS(:, :, CELL_IDX_1, CELL_IDX_2, :) = &
                  COUPLING_WANN%ELEMENTS(:, :, CELL_IDX_1, CELL_IDX_2, :) + &
                  COUPLING_BLOCH(:, :, :, Q_IDX, K_IDX) * &
                  PHASES(CELL_IDX_1, Q_IDX, K_IDX) * &
                  CONJG(PHASES(CELL_IDX_2, 1, K_IDX))
               ENDDO
            ENDDO
         ENDDO
      ENDDO

      CALL M_sum_z(COMM_WORLD, COUPLING_WANN%ELEMENTS(1, 1, 1, 1, 1), SIZE(COUPLING_WANN%ELEMENTS))

      COUPLING_WANN%ELEMENTS = COUPLING_WANN%ELEMENTS / (NUM_CELLS * NUM_CELLS)
   END SUBROUTINE
   ...
   SUBROUTINE PLOT_COUPLING(WANNIER_MAT, HAM_INIT, K_VEC, FILE_NAME, AVERAGE, T_INFO, IO)
      ...
      TYPE(WANNIER_MAT_MODES_T)  :: WANNIER_MAT
...
! This is only used to output a fancy plot.
            IF (AVERAGE) THEN
               ...
               CALL HAM_INIT%DIAG_AT_K(K_VEC, BAND_EV_K, TMP)
               CALL AVERAGE_DEGEN(BAND_EV_K, 2, COUPLING)
               CALL HAM_INIT%DIAG_AT_K(K_VEC + Q_PATH%VKPT(:, Q_IDX), BAND_EV_K, TMP)
               CALL AVERAGE_DEGEN(BAND_EV_K, 1, COUPLING)
               COUPLING = SQRT(ABS(COUPLING))
            ENDIF

            DO BAND_IDX = 1, NUM_BANDS
               PATH_QUANTITY(:, Q_IDX, BAND_IDX) = abs(COUPLING(BAND_IDX, BAND_IDX, :))
            ENDDO
         ENDDO

         CALL M_sum_d(COMM_WORLD, PATH_QUANTITY(1, 1, 1), SIZE(PATH_QUANTITY))
...
! Read the q-path from file.
         ...
         CALL M_sum_d(COMM_WORLD, PATH_QUANTITY(1, 1, 1, 1), SIZE(PATH_QUANTITY))
...
! This should never happen
      IF (.NOT. INTERPOL%IS_COMPLETED) &
         CALL vtutor%bug("Internal error in ELPH_CALC_COUPLING: Missing Wannier-space data!", "elphon.F", 2882)

      ASSOCIATE( &
         PC => INTERPOL%WANNIER_DESCRIPTORS(SPIN)%PRIM_CELL, &
         N_ORBS => INTERPOL%WANNIER_DESCRIPTORS(SPIN)%NUM_PRIM_ORBITALS, &
...
! Sum up the individual contributions to the energy shift from each node
         IF (PRESENT(COMM)) THEN
            CALL M_sum_z(COMM, SELF_ENERGY(1, 1), SIZE(SELF_ENERGY))
         ENDIF
...
! The matrix elements can be obtained from COUPLING_0 and COUPLING_0S by rotating with phonon modes
         ...
         E_PHON = E_PHON * HBAR
! TRANSLATIONAL_MODES = FIND_TRANSLATIONAL_MODES(MODES)
...
! This should never happen
      IF (.NOT. INTERPOL%IS_COMPLETED) &
         CALL vtutor%bug("Internal error in ELPH_CALC_COUPLING: Missing Wannier-space data!", "elphon.F", 3082)
...
! Sum up the individual contributions to the energy shift from each node
         IF (PRESENT(COMM)) THEN
            CALL M_sum_d(COMM, SELF_ENERGY(1), N_ORBS)
         ENDIF
...
! We run into potential problems at hard (negative) (0._q,0._q) (IEEE -> -Infinity)
...
!_______________________________________________________________________________
   ...
   SUBROUTINE TEST_HAM_MAT(HAM_MAT, ENERGIES, IO)
      ...
      COMPLEX(q),             INTENT(IN)  :: HAM_MAT(:, :)
      COMPLEX(q),       INTENT(IN)  :: ENERGIES(:)
      TYPE(in_struct),  INTENT(IN)  :: IO

      COMPLEX(q), ALLOCATABLE       :: EIGENVECTORS(:, :)
      INTEGER                 :: MAT_SIZE
...
# 3347
      ...
      CALL vtutor%bug("TEST_HAM_MAT: Complex not implemented!", "elphon.F", 3348)
...
!_______________________________________________________________________________
   SUBROUTINE EPI_INIT(THIS, SETTINGS, MLWF_T, W, CQIJ, CDIJ, P, PRIM_CELL, T_INFO, INFO, &
      ...
      COMPLEX(q), ALLOCATABLE :: UK(:, :, :, :)
      COMPLEX(q), ALLOCATABLE :: HAM_INIT_MAT(:, :, :)
      INTEGER           :: SPIN_IDX
...
! Rotate to Wannier functions and store them in W_WAN_INIT.
         ...
         CASE DEFAULT
            CALL vtutor%bug("No elphon method selected!", "elphon.F", 3441)
      END SELECT

! Setup Wannier-orbital descriptor.
      ALLOCATE(THIS%WANNIER_DESCRIPTORS(W%WDES%ISPIN))
      DO SPIN_IDX = 1, W%WDES%ISPIN
         CALL THIS%WANNIER_DESCRIPTORS(SPIN_IDX)%INIT( &
            THIS%W_WAN_INIT, NUM_ORBS_IN_CELL, PRIM_CELL, GRID, GRIDC, GRIDUS, &
            C_TO_US, T_INFO, P, SIZE(CQIJ, 1), INFO%LOVERL, IRDMAX, SPIN_IDX, FORCE_ATOM_CENTERED)
      ENDDO

! Calculate the unperturbed Wannier-space Hamiltonian matrix.
      ALLOCATE(HAM_INIT_MAT(SIZE(UK, 2), SIZE(UK, 2), W%WDES%ISPIN))
      CALL CALC_WANNIER_HAM_FROM_TRAFO(W, UK(:, :, 1, :), HAM_INIT_MAT) ! _test_mani maybe replace with MLWF version

! Setup the initial Hamiltonian data structure.
      ALLOCATE(THIS%HAM_INIT(W%WDES%ISPIN))
      DO SPIN_IDX = 1, W%WDES%ISPIN
         THIS%HAM_INIT(SPIN_IDX) = WANNIER_MAT_SYMM_T(THIS%WANNIER_DESCRIPTORS(SPIN_IDX), &
            HAM_INIT_MAT(1 : NUM_ORBS_IN_CELL * PRIM_CELL%NUM_CELLS, 1 : NUM_ORBS_IN_CELL, SPIN_IDX))
      ENDDO
...
! locals
      COMPLEX(q), ALLOCATABLE, TARGET, SAVE  :: HAM_DIFF(:, :, :)
      COMPLEX(q), ALLOCATABLE, SAVE          :: OVERLAP_DIFF(:, :, :)
      COMPLEX(q), ALLOCATABLE                :: TMP_MAT(:, :)
      COMPLEX(q), POINTER                    :: HAM_PTR(:, :, :, :)
      INTEGER                          :: NB_TOT
      INTEGER                          :: SPIN_IDX, NUM_SPIN
      TYPE(wavedes1)                   :: WDES1
      REAL(q), POINTER                 :: POSION_PTR(:,:)
      INTEGER                          :: LMDIM
      COMPLEX(q)                             :: FIN_DIFF_FACTOR
      INTEGER                          :: NKPTS_STORE
! Needed by EDDIAG.
      INTEGER                          :: IFLAG
...
! If nothing has been set up, stop
      IF (.NOT. THIS%IS_INITIALIZED) CALL vtutor%bug("EPI object not initialized!", "elphon.F", 3545)
...
! Check if finite difference has made progress.
      ...
      IF (CUR_DISPL /= 1 .AND. CUR_DISPL /= 2) CALL vtutor%bug( &
         "DISPL > 2 not supported! " // str(CUR_DISPL), "elphon.F", 3555)

! Set k-point count temporarily. _test_mani needs fixing
      NKPTS_STORE = THIS%W_WAN_INIT%WDES%NKPTS
      THIS%W_WAN_INIT%WDES%NKPTS = 1
...
! Allocate the perturbed Hamiltonian and S-overlap matrices.
      IF (.NOT.ALLOCATED(THIS%COUPLING_WANN)) THEN
         ...
         DO SPIN_IDX = 1, NUM_SPIN
            THIS%COUPLING_WANN(SPIN_IDX) = WANNIER_MAT_MODES_T(THIS%WANNIER_DESCRIPTORS(SPIN_IDX), GLOB_FINDIF_INFO%NUM_DOF)
            THIS%OVERLAP_PERT(SPIN_IDX) = WANNIER_MAT_MODES_T(THIS%WANNIER_DESCRIPTORS(SPIN_IDX), GLOB_FINDIF_INFO%NUM_DOF)
         ENDDO
...
! Create Wannier matrix object
      ...
      ASSOCIATE(PC => THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL)
...
! Reset NONL_S to perturbed positions.
      ...
      THIS%W_WAN_INIT%WDES%NKPTS = NKPTS_STORE
   ...
   SUBROUTINE EPI_PERTURB_WANNIER_AE(THIS, GRID, P, LATT_CUR, T_INFO, &
      ...
      COMPLEX(q),                      INTENT(IN)     :: CQIJ(:, :, :, :)
      COMPLEX(q),                      INTENT(IN)     :: CDIJ(:, :, :, :)
      COMPLEX(q),                        INTENT(IN)     :: SV(:, :) ! local potential
      TYPE(nonl_struct),            INTENT(INOUT)  :: NONL_S ! _test_mani must be mutable!
      TYPE(nonlr_struct),           INTENT(INOUT)  :: NONLR_S ! _test_mani must be mutable!
...
! locals
      COMPLEX(q), ALLOCATABLE, TARGET, SAVE  :: HAM_DIFF(:, :, :)
      COMPLEX(q), ALLOCATABLE, SAVE          :: CQIJ_PROJ_DIFF(:, :, :)
      COMPLEX(q), ALLOCATABLE, SAVE          :: CDIJ_PROJ_DIFF(:, :, :)
      COMPLEX(q), ALLOCATABLE, SAVE          :: D_CDIJ_DIFF(:, :, :)
      COMPLEX(q), ALLOCATABLE                :: TMP_MAT(:, :), TMP_MAT_2(:, :), TMP_TMP(:, :), TOTAL_MAT(:, :)
      COMPLEX(q), POINTER                    :: HAM_PTR(:, :, :, :)
      COMPLEX(q), ALLOCATABLE             :: CQIJ_PERT(:, :, :, :)
      INTEGER                          :: NB_TOT
      INTEGER                          :: SPIN_IDX, NUM_SPIN
      TYPE(wavedes1)                   :: WDES1
      REAL(q), POINTER                 :: POSION_PTR(:,:)
...
! new
      COMPLEX(q), ALLOCATABLE, TARGET  :: CPROJ_PERT(:, :, :, :)
      COMPLEX(q), POINTER              :: CPROJ_INIT_PTR(:, :, :, :)
      COMPLEX(q), ALLOCATABLE       :: CQDIJ_ZERO(:, :, :, :)

! If nothing has been set up, stop
      IF (.NOT. THIS%IS_INITIALIZED) CALL vtutor%bug("Elphon not initialized!", "elphon.F", 3753)
...
! Check if finite difference has made progress.
      ...
      IF (CUR_DISPL /= 1 .AND. CUR_DISPL /= 2) CALL vtutor%bug( &
         "DISPL > 2 not supported! " // str(CUR_DISPL), "elphon.F", 3763)
...
! ^ could be 1._q in (1._q,0._q) call but we don't use this in final implementation, just testing
...
! -P(D part)
! HAM_DIFF(1 : MAT_SIZE, 1 : MAT_SIZE, SPIN_IDX) = HAM_DIFF(1 : MAT_SIZE, 1 : MAT_SIZE, SPIN_IDX) - CDIJ_PROJ_DIFF(:, :, SPIN_IDX) - CONJG(TRANSPOSE(CDIJ_PROJ_DIFF(:, :, SPIN_IDX)))
            TOTAL_MAT = TOTAL_MAT + CDIJ_PROJ_DIFF(:, :, SPIN_IDX) + CONJG(TRANSPOSE(CDIJ_PROJ_DIFF(:, :, SPIN_IDX)))

! _test_mani
! Mul P(Q part) with eigenvalues (matmul Wannier Hamiltonian)
            CALL THIS%HAM_INIT(SPIN_IDX)%UNPACK(TMP_MAT_2)
            CALL ZGEMM( 'N', 'N', &
               MAT_SIZE, MAT_SIZE, MAT_SIZE, (1._q,0._q), TMP_MAT_2, MAT_SIZE, &
               CQIJ_PROJ_DIFF(:, :, SPIN_IDX), MAT_SIZE, (0._q,0._q), TMP_MAT, NB_TOT)
            TMP_MAT_2 = -TMP_MAT(1 : MAT_SIZE, 1 : MAT_SIZE) - CONJG(TRANSPOSE(TMP_MAT(1 : MAT_SIZE, 1 : MAT_SIZE)))
...
! Mul R with eigenvalues (matmul Wannier Hamiltonian)
            ...
            CALL ZGEMM( 'N', 'N', &
               MAT_SIZE, MAT_SIZE, MAT_SIZE, (1._q,0._q), TMP_MAT_2, MAT_SIZE, &
               TMP_MAT, NB_TOT, (0._q,0._q), TMP_TMP, MAT_SIZE)
...
! _test_mani add more after clean up
   ...
   SUBROUTINE SET_CQIJ_PERT(T_INFO, P, NUM_SPIN, ATOM_IDX, CART_IDX, CQIJ_PERT)
      ...
      COMPLEX(q), ALLOCATABLE,   INTENT(OUT) :: CQIJ_PERT(:, :, :, :)
...
! _test_mani this might cause the original wavespin object (W) to become dysfunctional
   SUBROUTINE EPI_DEALLOC(THIS)
      ...
      IF (ALLOCATED(THIS%WANNIER_DESCRIPTORS)) DEALLOCATE(THIS%WANNIER_DESCRIPTORS)
...
!>                      Brillouin (1._q,0._q) of the primitive cell
...
! This should never happen
      IF (.NOT. THIS%IS_COMPLETED) CALL vtutor%bug("EPI_CALC_COUPLING: Missing Wannier-space data!", "elphon.F", 4130)
...
! Rotate coupling with phonon eigenmodes
      ...
      CALL ROTATE_WITH_MODES(COUPLING_PTR, Q_VEC, THIS%WANNIER_DESCRIPTORS(SPIN)%PRIM_CELL, T_INFO)
   END SUBROUTINE
...
! This should never happen.
      IF (.NOT. THIS%IS_COMPLETED) CALL vtutor%bug("EPI_CALC_COUPLING_ATOMS: Missing Wannier-space data!", "elphon.F", 4179)
...
! Finally, rescale with frequency and attach correct units
      DO MODE_IDX = 1, SIZE(MODES, 1)
         IF (ABS(FREQU(MODE_IDX)) < 1E-20_q) THEN
...
!_______________________________________________________________________________
   SUBROUTINE EPI_INIT_POLAR(THIS, SETTINGS, POLAR_DATA)
      ...
      IF (.NOT. THIS%IS_COMPLETED) CALL vtutor%bug("Electron-phonon code not completed!", "elphon.F", 4424)

      ABS_G_CUTOFF = GET_G_CUTOFF(SETTINGS%POLAR_G_CUTOFF, THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL%PRIM_LATT)
      CALL THIS%POLAR_EWALD%INIT(ABS_G_CUTOFF, SETTINGS%POLAR_EWALD_THRESHOLD, POLAR_DATA)
      CALL GENERATE_EWALD_G_VECTORS(THIS%POLAR_EWALD, THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL%PRIM_LATT, THIS%EWALD_G_VECTORS)
...
!_______________________________________________________________________________
   SUBROUTINE EPI_SETUP_COUPLING_WANN_LONG(THIS, SETTINGS)
      ...
      ASSOCIATE( &
         PC => THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL, &
         N_SPIN => SIZE(THIS%COUPLING_WANN), &
         N_PRIM => THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL%NUM_PRIM_ATOMS, &
         N_CELLS => THIS%WANNIER_DESCRIPTORS(1)%PRIM_CELL%NUM_CELLS, &
         N_ORBS => THIS%WANNIER_DESCRIPTORS(1)%NUM_PRIM_ORBITALS, &
...
! Skip if Ewald suppression factor is too small
               ...
               DO P_IDX = 1, N_PRIM
                  CALL SETUP_PHASE_BUFFER(PHASE, THIS%WANNIER_DESCRIPTORS(SPIN_IDX), P_IDX, G_CART)
...
!_______________________________________________________________________________
   SUBROUTINE EPI_SUBTRACT_COUPLING_WANN_LONG(THIS, COUPLING, Q_VEC, K_VEC, SPIN)
      ...
      ASSOCIATE( &
         N_ORBS => THIS%WANNIER_DESCRIPTORS(SPIN)%NUM_PRIM_ORBITALS, &
         N_PRIM => THIS%WANNIER_DESCRIPTORS(SPIN)%PRIM_CELL%NUM_PRIM_ATOMS)
...
! Calculate the 'electronic' rotation matrices from Wannier to Bloch basis
         ...
         Q_CART = MATMUL(THIS%WANNIER_DESCRIPTORS(SPIN)%PRIM_CELL%PRIM_LATT%B, Q_VEC)

         DO P_IDX = 1, N_PRIM
! Pre-calculate phase factors
            CALL SETUP_PHASE_BUFFER(PHASE, THIS%WANNIER_DESCRIPTORS(SPIN), P_IDX, -Q_CART)
...
!_______________________________________________________________________________
   SUBROUTINE EPI_ADD_COUPLING_LONG(THIS, COUPLING, Q_VEC, K_VEC, SPIN)
      ...
      ASSOCIATE( &
         PC => THIS%WANNIER_DESCRIPTORS(SPIN)%PRIM_CELL, &
         N_ORBS => THIS%WANNIER_DESCRIPTORS(SPIN)%NUM_PRIM_ORBITALS, &
...
! |                                                                           |
! |  This routine samples the first Brillouin (1._q,0._q) using a uniform grid       |
...
! |  inside the first Brillouin (1._q,0._q) by shifting along reciprocal lattice     |
! |  vectors.                                                                 |
! |___________________________________________________________________________|
! |                                                                           |
...
! |  min_vec     - On return, point inside the first Brillouin (1._q,0._q).          |
! |___________________________________________________________________________|
...
! |                                                                           |
! |  cart_qpoint - Point in reciprocal space in Cartesian coordinates.        |
! |  min_vec     - On return, point inside the first Brillouin (1._q,0._q) of the    |
! |                stored lattice.                                            |
! |___________________________________________________________________________|
