...
!>  mixing is used
!>  \param[in] NGX,NGY,NGZ     is the reduced grid on which Broyden mixing is 1._q
!>                             corresponds to NGXB,NGYB,NGZB in main program
!>  \param[in] NGXC,NGYC,NGZC  is the full grid
!
!***********************************************************************

      SUBROUTINE BRMIX( &
         KINEDEN,GRIDB,GRIDC,IO,MIX,XC,B_TO_C, &
         NGIGA,CHTOT,CHTOTL,ISPIN,B,OMEGA, &
         N_MIX_PAW, RHOLM, RHOLM_LAST, &
         DE2H, RMST, RMS, RMSP, WEIGHT, LWARN, IERRBR, &
...
! double NCHARGE to hold tau for metaGGA's
      IF (XC%LTAU.AND.LMIX_TAU()) NCHARGE=2*NCHARGE
...
! additional terms for relaxed core methods
      ...
      IF (NP /= NDATA) THEN
         CALL vtutor%bug("internal error 1 in BRMIX " // str(NP) // " " // str(NDATA), "broyden.F", 174)
      ENDIF
...
!$OMP PRIVATE(ISP,I,N1,NC,N2,N3,FACTM,ALPHA,DEL) &
!$OMP REDUCTION(+:RMST,RMS_tau)
      DO ISP=1,ISPIN
! charge density
!$OMP DO
         DO I=1,GRIDC%RC%NP
...
!$OMP SHARED(GRIDB,SFAKT,CWRK3,CWRK4,ISP) &
!$OMP PRIVATE(I,IP,N1,NC,N2,N3,FACTM)
         DO I=1,GRIDB%RC%NP
...
! kinetic energy density
         IF (XC%LTAU.AND.LMIX_TAU()) THEN
!$OMP PARALLEL DO DEFAULT(NONE) &
!$OMP SHARED(GRIDB,SFAKT,CWRK3,CWRK4,ISP,ISPIN) &
!$OMP PRIVATE(I,IP,N1,NC,N2,N3,FACTM)
            DO I=1,GRIDB%RC%NP
...
!$OMP PARALLEL DO DEFAULT(NONE) &
!$OMP SHARED(GRIDB,ISPIN,CHP,CWRK3) &
!$OMP PRIVATE(I)
...
! initial mixing for relaxed core method
         ...
         IF (NP /= NDATA) THEN
            CALL vtutor%bug("internal error 2 in BRMIX " // str(NP) // " " // str(NDATA), "broyden.F", 352)
         ENDIF
      ENDIF
! Broyden mixing:
      IB=0
...
!OCL NOVREC
!$OMP PARALLEL DO DEFAULT(NONE) &
!$OMP SHARED(GRIDB,CWRK4,ISP,ISFAKT) &
!$OMP PRIVATE(I,IP,N1,NC,N2,N3,FACTM) &
!$OMP REDUCTION(+:SUM_)
         DO I=1,GRIDB%RC%NP
            IP= I+(ISP-1)*GRIDB%RC%NP
...
! kinetic energy density
         IF (XC%LTAU.AND.LMIX_TAU()) THEN
!$OMP PARALLEL DO DEFAULT(NONE) &
!$OMP SHARED(GRIDB,CWRK4,ISP,ISPIN,ISFAKT) &
!$OMP PRIVATE(I,IP,N1,NC,N2,N3,FACTM)
            DO I=1,GRIDB%RC%NP
               IP= I+(ISP-1+ISPIN)*GRIDB%RC%NP
               N1= MOD((I-1),GRIDB%RC%NROW) +1
...
! (1._q,0._q) center terms
      ...
      IF (NP /= NDATA) THEN
         CALL vtutor%bug("internal error 3 in BRMIX " // str(NP) // " " // str(NDATA), "broyden.F", 446)
      ENDIF
...
! We average CHTOT over the KPAR (possibly slighty) different instances ... first the summation
      CALL M_bcast_z(GRIDC%COMM_KINTER,CHTOT(1,1),SIZE(CHTOT))
      DE2H=0 ; SUM_=0
      DO ISP=1,ISPIN
!$OMP PARALLEL DO DEFAULT(NONE) &
!$OMP SHARED(GRIDC,CHTOTL,CHTOT,ISP,B) &
!$OMP PRIVATE(I,N1,NC,N2,N3,FACTM,GX,GY,GZ,GSQU) &
!$OMP REDUCTION(+:SUM_,DE2H)
         DO I=1,GRIDC%RC%NP
            N1= MOD((I-1),GRIDC%RC%NROW) +1
            NC= (I-1)/GRIDC%RC%NROW+1
            N2= GRIDC%RC%I2(NC)
            ...
            DE2H = DE2H +  CONJG((CHTOT(I,ISP)-CHTOTL(I,ISP)))*(CHTOT(I,ISP)-CHTOTL(I,ISP))/GSQU
...
! TAU needs to be averaged over KPAR instances as well
         CALL M_bcast_z(GRIDC%COMM_KINTER,KINEDEN%TAU(1,1),SIZE(KINEDEN%TAU))
         DO ISP=1,ISPIN
!$OMP PARALLEL DO DEFAULT(NONE) &
!$OMP SHARED(GRIDC,KINEDEN,ISP) &
!$OMP PRIVATE(I)
            DO I=1,GRIDC%RC%NP
...
! copy occupancies
      ...
      CALL M_bcast_d(GRIDC%COMM_KINTER,RHOLM,SIZE(RHOLM))
...
! copy (1._q,0._q)-center densities
      ...
      CALL M_sum_2(GRIDC%COMM, SUM_, DE2H)
      ! 'summed charge is',SUM_

      DE2H=DE2H*EDEPS/OMEGA
...
!***********************************************************************
!> Implementation Broyden and Pulay mixing
!>
...
!>
!> Note that we explicitly take the real part at several stages in this
!> routine, because the density is real and contributions from +G and -G
!> would cancel. But the density vector contains only the +G components
!> so that the update would lead to a spurious imaginary component.
!***********************************************************************
...
! Change of vector |X>
      ...
      SUMP=0
      DO I=1,NDIM
         SUMP=SUMP+REAL(CONJG(F(I))*FP(I))
         SUM_=SUM_  +REAL(CONJG(F(I))*F(I))
      ENDDO
...
! Norm of this vector (with integration weights/metric. included ...):
      FNORM=0._q
      DO  I=1,NDIM
        FNORM=FNORM+REAL(FP(I)*CONJG(F(I)))
      ENDDO
...
!=======================================================================
      DO J=1,ITERM1
         ...
         SUM_=0
         DO I=1,NDIM
           SUM_=SUM_+REAL(CONJG(WRK2(I))*FP(I))
         ENDDO
...
!  SUM2=<D F(j)| U^(iter-1)> = <D F(j)| G(m)-G(1) | D F^(iter-1)>
         DO I=1,NDIM
           SUM1=SUM1+REAL(CONJG(WRK2(I))*WRK1(I))
           SUM2=SUM2-REAL(CONJG(WRK2(I))*WRK1(I+NDIM))
         ENDDO
         SMAT(J,ITERM1)=SUM1
         SMAT(ITERM1,J)=SUM1
         GMAT(J,ITERM1)=SUM2
...
! SUM_=<D F(iter-1)| U^(j)>
         SUM_=0
         DO I=1,NDIM
           SUM_=SUM_+REAL(CONJG(WRK1(I))*WRK2(I))
         ENDDO
         GMAT(ITERM1,J)=-SUM_
      ENDDO
...
!************************* SUBROUTINE BRPRE ****************************
!> Setup the metric for density mixing.
!>
...
!***********************************************************************
      ...
      IF (.NOT.ASSOCIATED(root)) THEN
         CALL vtutor%error("BROYDEN: fatal error: dyn_get leaf not allocate")
         ALLOCATE(root)
         NULLIFY(root%store)
         NULLIFY(root%next)
      ENDIF
      IF (irec==1) THEN
        IF  (.NOT.ASSOCIATED(root%store)) THEN
          CALL vtutor%error("BROYDEN: fatal error: dyn_get store not allocated")
        ENDIF
        data=root%store
      ELSE
...
!> \param[in]   NP  : first physical dimension of matrices A and B
!> \param[out]  B   : inverse matrix. Make B=A for 'in place' inversion
!> \param       AUX : Auxiliary array of minimun size N*N. If A and B are different
