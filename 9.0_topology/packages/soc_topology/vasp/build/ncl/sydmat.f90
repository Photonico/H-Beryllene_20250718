...
! For all atomic species ... :
      DO IS2=1,NSP
         ISP2=IS2
         IF (TCONT) ISP2=1
         IF (TCONT.AND.(IS2>1)) IBEG2=IBEG2+NA(IS2-1)
         IBEG1=1
         DO IS1=1,NSP
            ISP1=IS1
            IF (TCONT) ISP1=1
            IF (TCONT.AND.(IS1>1)) IBEG1=IBEG1+NA(IS1-1)
! Save original DMAT:
...
!>   a given lattice.
!>
!>   In principle (1._q,0._q) has 3*N degrees of freedom for a
...
!>      ATOM_INDICES (otional) specifies atoms for which to do all
!>                   3 Cartesian displacements.
!>
!>   Output parameters:
!>   ------------------
!>
...
!***********************************************************************
     ...
     &                      WD,IW,A1,A2,A3,B1,B2,B3,IDIRD,NALL,AIDX)
      USE sym_prec
      USE msymmetry
      IMPLICIT REAL(q) (A-H,O-Z)
...
! Loop over all atoms and (lattice) directions ... :
         DO IA=ISTART,NA(IS)+ISTART-1
            DO IDIR=1,3
! Test direction in in cartesian coordinates
               VR1=VTEST(1,IDIR)
               VR2=VTEST(2,IDIR)
               VR3=VTEST(3,IDIR)
! Get "test direction" on atom in direct (fractional) coordinates ...
               V1=VR1*B1(1)+VR2*B1(2)+VR3*B1(3)
               V2=VR1*B2(1)+VR2*B2(2)+VR3*B2(3)
               V3=VR1*B3(1)+VR2*B3(2)+VR3*B3(3)
               IF (NALL > 0) THEN
                  LNEW=ANY(IA == AIDX(1:NALL))
               ELSE
! test whether direction was already generated previously, by
! orthogonalization to vectors already generated on this atom
                  CALL NEWDIR(VR1,VR2,VR3,WD(1,1,IA),IW(IA),LNEW)
! gK 07.08.2018:
! if this was a new direction use it (VR currently holds the orthogonalized direction)
! restore VRX to the original (test) direction
! this allows to use the linear response code for every conceivable case
! results are compatible to old version (uncomment to turn back to old version)
                  IF (LNEW) THEN
                     VR1=VTEST(1,IDIR)
                     VR2=VTEST(2,IDIR)
                     VR3=VTEST(3,IDIR)
                  ENDIF
               ENDIF
! store the displacement if it is a new (1._q,0._q)
               IF (LNEW) THEN
...
! orthogonalize vector (X,Y,Z) onto  all previously considered directions
      DO I=1,NOLD
         ...
         Z=Z-P*VOLD(3,I)
      ENDDO
! calculate norm of the new vector ...
      VNORM=SQRT(X*X+Y*Y+Z*Z)
      LNEW=VNORM>TINY
      IF (LNEW) THEN
! renormalize projected vector to 1
         X=X/VNORM
         Y=Y/VNORM
         Z=Z/VNORM
      ENDIF
...
! increase displacement counter on ion JA
                        IW(JA)=IW(JA)+1
                        IF (IW(JA)>3) THEN
                           CALL vtutor%bug("internal error in MKDMAT: displacement counter exceeds 3", "sydmat.F", 634)
                        ENDIF
...
! increase displacement counter on ion JA
                        IW(JA)=IW(JA)+1
                        IF (IW(JA)>3) THEN
                           CALL vtutor%bug("internal error in MKDMAT: displacement counter exceeds 3", "sydmat.F", 916)
                        ENDIF
! mark displacement on ion JA
                        AD(1,IW(JA),JA)=VR1
                        AD(2,IW(JA),JA)=VR2
