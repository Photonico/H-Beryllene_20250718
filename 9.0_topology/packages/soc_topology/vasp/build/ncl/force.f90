...
# 6 "force.F" 2 
      MODULE force
      USE prec
      USE pot_electrostat, only: pothar, potion
      CONTAINS
...
!***********************************************************************
      ...
      SUBROUTINE STRELO(GRIDC,P,T_INFO,LATT_CUR,COULOMB_POT,&
...
!$ACC ENTER DATA CREATE(CWORK1,CWORK2,EISIF,DSIF) 
!=======================================================================
! first caclulate the local potential and its derivative
!=======================================================================
      CALL POTION(GRIDC,P,LATT_CUR,T_INFO,COULOMB_POT,CWORK1,CSTRF,PSCENC)
      IF ( .not. COULOMB_POT%KERNEL_TRUNCATE%ACTIVE) THEN
          CALL GRAD_POTION(GRIDC,P,LATT_CUR,T_INFO,COULOMB_POT,CWORK2,CSTRF)
      END IF
!=======================================================================
...
!=======================================================================
!$ACC KERNELS PRESENT(EISIF,DSIF) 
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(GRIDC,LATT_CUR,CHTOT,CWORK1,CWORK2,EISIF,DSIF) &
!$ACC& PRIVATE(N2,N3,NG,FACTM,GX,GY,GZ,GSQU,GMOD,PSCD,DUM) &
!$ACC& REDUCTION(+:PSCV,DENC)  DEFAULT(none)
...
!$ACC ATOMIC UPDATE
      ...
      ENDDO row
      ENDDO col
!$ACC EXIT DATA COPYOUT(EISIF,DSIF) DELETE(CWORK1,CWORK2) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!=======================================================================
...
! work arrays
      ...
      ALLOCATE(CWORK1(GRIDC%MPLWV),CWORK2(GRIDC%MPLWV))
!$ACC ENTER DATA CREATE(CWORK1,CWORK2) 
!=======================================================================
...
!$ACC ATOMIC UPDATE
      ...
      ENDDO row
      ENDDO col
!$ACC EXIT DATA COPYOUT(HARSIF) DELETE(CWORK1,CWORK2) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
...
!-MM- changes to accommodate spin spirals
      QSP=WDES%QSPIRAL/2
!-MM- end of addition
!$ACC LOOP COLLAPSE(2) VECTOR REDUCTION(+:SIKE1,SIKE2,SIKE3,SIKE12,SIKE23,SIKE31) &
...
!$ACC ATOMIC UPDATE
      ...
      ENDDO band_k
      ENDDO spin
!$ACC EXIT DATA COPYOUT(SIKEF) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
! reduction of SIKEF
...
!***********************************************************************
     ...
     &   GRID,GRID_SOFT,GRIDC,SOFT_TO_C,XC,P,T_INFO,LATT_CUR,SYMM, &
     &   TAUSIF)
...
!! USE moffload_struct_def
      ...
      USE tau_mu, ONLY : tau_handle,SET_KINEDEN,TAUPAR
      IMPLICIT NONE
...
!$ACC ENTER DATA COPYIN(LATT_PERTURBED) 
         CALL SET_KINEDEN(GRID,GRID_SOFT,GRIDC,SOFT_TO_C,LATT_PERTURBED,SYMM,XC%LTAU,T_INFO%NIONS,W,WDES,KINEDEN)

         CALL TAUPAR(GRIDC,T_INFO,LATT_PERTURBED%B,LATT_PERTURBED%OMEGA,P,CSTRF,KINEDEN%TAUC)
!$ACC EXIT DATA DELETE(LATT_PERTURBED) 
...
! restore initial state
      ...
      CALL SET_KINEDEN(GRID,GRID_SOFT,GRIDC,SOFT_TO_C,LATT_CUR,SYMM,XC%LTAU,T_INFO%NIONS,W,WDES,KINEDEN)
...
!***********************************************************************
      ...
      SUBROUTINE CHGGRA(GRIDC,LATT_CUR,XC,COULOMB_POT,&
...
! add hartree-term
!-----------------------------------------------------------------------
!$ACC PARALLEL LOOP PRESENT(CWORK1,CHTOTL,CHTOT) 
      DO N=1,GRIDC%RC%NP
        CWORK1(N)=CHTOT(N)-CHTOTL(N)
      ENDDO
      CALL POTHAR(GRIDC,LATT_CUR,COULOMB_POT,CWORK1,CWORK2,DENC)

!$ACC PARALLEL LOOP PRESENT(CHGGA,CWORK2) 
      DO N=1,GRIDC%RC%NP
       CHGGA(N)=CHGGA(N)+CWORK2(N)
      ENDDO
!$ACC EXIT DATA DELETE(DWORK2,CWORK1,CWORK2) 
...
!$ACC ENTER DATA CREATE(WORK,EIFOR) 
...
!=======================================================================
 ...
 !$OMP SHARED(GRIDC,LATT_CUR,PSGMA2,ARGSC,P,NT,WORK,ZZ) &
 !$OMP PRIVATE(N,N1,NC,N2,N3,FACTM,GX,GY,GZ,G,I,REM,VPST)
!$ACC PARALLEL LOOP PRIVATE(N1,NC,N2,N3,FACTM,GX,GY,GZ,G,I,REM,VPST) &
!$ACC& PRESENT(GRIDC,LATT_CUR,P,WORK) 
      DO N=1,GRIDC%RC%NP
...
!$ACC& PRESENT(GRIDC,T_INFO%POSION,T_INFO%VCA,WORK,EIFOR,CHTOT) 
...
# 782
 ...
 !$OMP PARALLEL DO DEFAULT(NONE) &
 !$OMP SHARED(GRIDC,T_INFO,NI,NT,CX,G1,WORK,CHTOT) &
 !$OMP PRIVATE(NC,NGP,N2,N3,G2,G3,CE,N1P,N1,NG,FACTM,CEXPF,FOR) &
 !$OMP REDUCTION(+:FOR1,FOR2,FOR3)
...
!=======================================================================
      ...
      NIS=NIS+NIADD
      ENDDO typ
!$ACC EXIT DATA COPYOUT(EIFOR) DELETE(WORK) 
...
! work arrays
      ...
      ALLOCATE(WORK(GRIDC%RC%NP))
!$ACC ENTER DATA CREATE(WORK,HARFOR) 

!$ACC KERNELS PRESENT(HARFOR) 
...
!=======================================================================
 ...
 !$OMP PARALLEL DO DEFAULT(NONE) &
 !$OMP SHARED(GRIDC,LATT_CUR,PSGMA2,ARGSC,WORK,PRHO) &
 !$OMP PRIVATE(N,N1,NC,N2,N3,FACTM,GX,GY,GZ,G,ARG,NADDR,REM,V1,V2,V3,V4,T0,T1,T2,T3)
!$ACC PARALLEL LOOP PRIVATE(N1,NC,N2,N3,FACTM,GX,GY,GZ,G,ARG,NADDR,REM,V1,V2,V3,V4,T0,T1,T2,T3) &
!$ACC& PRESENT(GRIDC,LATT_CUR,PRHO,WORK) 
      DO N=1,GRIDC%RC%NP
        N1= MOD((N-1),GRIDC%RC%NROW) +1
        NC= (N-1)/GRIDC%RC%NROW+1
        N2= GRIDC%RC%I2(NC)
...
!$ACC& PRESENT(GRIDC,T_INFO%POSION,T_INFO%VCA,WORK,HARFOR,CHGGA) 
      ion: DO NI=NIS,NIADD+NIS-1
!=======================================================================
! initialise the force on the ion to (0._q,0._q)
!=======================================================================
...
# 1005
         ...
         G1 =T_INFO%POSION(1,NI)*(-(GRIDC%NGX/2-1))
 !$OMP PARALLEL DO DEFAULT(NONE) &
 !$OMP SHARED(GRIDC,T_INFO,NI,NT,CX,G1,WORK,CHGGA) &
 !$OMP PRIVATE(NC,NGP,N2,N3,G2,G3,CE,N1P,N1,NG,FACTM,CEXPF,FOR) &
 !$OMP REDUCTION(+:FOR1,FOR2,FOR3)
!$ACC LOOP VECTOR REDUCTION(-:FOR1,FOR2,FOR3) PRIVATE(NGP,N2,N3,G2,G3,CE,N1,NG,FACTM,CEXPF,FOR)
         DO NC=1,GRIDC%RC%NCOL
...
!=======================================================================
      ...
      NIS=NIS+NIADD
      ENDDO typ
!$ACC EXIT DATA COPYOUT(HARFOR) DELETE(WORK) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!=======================================================================
! forces are now in the reciprocal lattice transform it to
! cartesian coordinates
!=======================================================================
...
!***********************************************************************
  ...
  SUBROUTINE FORCE_AND_STRESS( &
          ...
          T_INFO,T_INFO_0,DYN,INFO,XC,IO,MIX,SYMM,GRID,GRID_SOFT, &
...
!! USE moffload
    ...
    USE hamil_struct_def
    USE setexm_struct_def, ONLY : xc_info
    USE tau_mu, ONLY : tau_handle,LMIX_TAU,FORTAU
    USE us
    USE morbitalmag
    USE classicfields
    USE vdwforcefield
...
! solvation__
    ...
    USE coulomb_cutoff_gradients, only: TRUNCATE_FORLOC
    USE pot_struct_def
    USE plugins, ONLY : PLUGINS_FORCE_AND_STRESS
    IMPLICIT NONE
!=======================================================================
!  structures
!=======================================================================
...
!  local work arrays for individual force components
    ...
    REAL(q)      RMST,RMSC,RMSP,WEIGHT,DE2H ! mixer
...
!     local contribution to force
      CALL START_TIMING("G")
!$ACC UPDATE DEVICE(CHTOT) 
      IF (HAMILTONIAN%COULOMB_POT%KERNEL_TRUNCATE%ACTIVE) THEN
          CALL TRUNCATE_FORLOC(GRIDC,P,T_INFO,LATT_CUR,HAMILTONIAN%COULOMB_POT,CHTOT(:,1),CSTRF,EIFOR)
      ELSE
...
! must restore CDIJ at this point, since FORDEP destroys it
      ...
      CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, &
           ...
           ETMP, LASPH=INFO%LASPH, LCOREL= .FALSE.  )
...
! local part
         CALL STRELO(GRIDC,P,T_INFO,LATT_CUR,HAMILTONIAN%COULOMB_POT,&
...
# 1540
         ...
         IF (XC%LMU) THEN
             ...
             &   GRID,GRID_SOFT,GRIDC,SOFT_TO_C,XC,P,T_INFO,LATT_CUR,SYMM,TAUSIF)
...
!$ACC UPDATE DEVICE(CHTOT) 
         IF (IVDW.eq.4) THEN
            ...
            CALL POTXC(KINEDEN,GRIDC,GRID_SOFT,SOFT_TO_C,INFO,XC,WDES,LATT_CUR,CHDEN,CHTOT,DENCOR,CVTOT,HAMILTONIAN%MUTOT,IVDW,XDM,GRIDC%RL%NP)
         ELSE
            ALLOCATE(XDM(1))
            CALL POTXC(KINEDEN,GRIDC,GRID_SOFT,SOFT_TO_C,INFO,XC,WDES,LATT_CUR,CHDEN,CHTOT,DENCOR,CVTOT,HAMILTONIAN%MUTOT,IVDW,XDM,1)
         ENDIF

         IF (XC%LMU) CALL FORTAU(GRIDC,P,T_INFO,LATT_CUR,HAMILTONIAN%MUTOT,TAUFOR)
...
!-----------------------------------------------------------------------
      IF (INFO%LCHCON.OR.INFO%LCORR) THEN
         ...
         CALL CHGGRA(GRIDC,LATT_CUR,XC,HAMILTONIAN%COULOMB_POT,CVTOT,CHTOT,CHTOTL,DENCOR)
...
!-----------------------------------------------------------------------
      ...
      TIFOR=EIFOR+EWIFOR+EINL+HARFOR+PARFOR+FORHF+TAUFOR+DYN%EFOR
...
!=======================================================================
! potentially modify energy, stress and forces on ions with Python
!=======================================================================
      IF (INFO%PLUGIN%FORCE_AND_STRESS) THEN
         CALL FFT3D(CHTOTL(1,1), GRIDC, 1)
         CALL PLUGINS_FORCE_AND_STRESS(INFO, T_INFO, LATT_CUR, P, GRIDC, TOTEN, TIFOR, TSIF, CHTOTL(:,1))
      END IF
...
! and the stress as well
      CALL M_bcast_d(WDES%COMM, TSIF, 9)
...
!LOGICAL,SAVE :: LFIRST=.TRUE.
         ...
         IF (LVDW) THEN
!c nothing to be 1._q here after the first call of this subroutine
...
!c Tkatchenko-Scheffler method
             IF (IVDW==2 .OR. IVDW==4 .OR. IVDW==14 .OR. (IVDW>=20 .AND. IVDW<=22) .OR. IVDW==26 .OR. IVDW==200 .OR. IVDW==210  &
...
!c use standard Hirshfeld partitioning
               IF ( IVDW==2 .OR. IVDW==14 .OR. IVDW==20 .OR. IVDW==200 .OR. IVDW==202  ) THEN
                CALL hirshfeld_standard(GRIDC, IO,DYN,T_INFO,INFO,&
               &  LATT_CUR,P,XC,  RELVOL, RELCHG, NALLOC,(CWORK1),(CWORK2),VDW_REFSTATE_GL)
               ENDIF

!c use iterative Hirshfeld-I partitioning
               IF (IVDW==21 .OR. IVDW==210 .OR. IVDW==212) THEN
                 CALL hirshfeld_iterative(GRIDC, IO,DYN,T_INFO, INFO, LATT_CUR,P,XC,  &
...
!c use iterative iterative Hirshfeld-dominant partitioning
               IF (IVDW==4) THEN 
                  CALL hirshfeld_dominant(GRIDC, IO,DYN,T_INFO,INFO,&
                       &  LATT_CUR,P,XC,RELVOL,NALLOC,(CWORK1),(CWORK2),VDW_REFSTATE_GL,WDES%NCDIJ,XDM,RELCHG,Overlap,M2,HCD)
               ENDIF
               IF (IVDW.eq.4) DEALLOCATE(XDM)

!c use iterative iterative Hirshfeld-dominant partitioning
!IF (IVDW==5) CALL hirschfeld_dominant_iterative(GRIDC, IO,DYN,T_INFO, LATT_CUR,P,XC,  &
!&    NALLOC,GRIDC%MPLWV,CWORK1,CWORK2,RELVOL,.FALSE.)
           ...
           IF (PRESENT(KPOINTS)) THEN
             CALL vdw_forces_main(GRIDC,XC,IO,LATT_CUR,DYN,T_INFO,VDWSIF,VDWFOR,VDWEN,P%ELEMENT,& 
             & RELCHG,RELVOL,VDW_REFSTATE_GL,Overlap,M2,HCD,LHESSEVDW,HESSE_VDW,KPOINTS)
           ELSE
             CALL vdw_forces_main(GRIDC,XC,IO,LATT_CUR,DYN,T_INFO,VDWSIF,VDWFOR,VDWEN,P%ELEMENT, &
             & RELCHG,RELVOL,VDW_REFSTATE_GL,Overlap,M2,HCD,LHESSEVDW,HESSE_VDW)
           ENDIF
...
!***********************************************************************
  ...
  SUBROUTINE FORCE_AND_STRESS_LJ(GRIDC,XC,IO,WDES,P,LATT_CUR,DYN,T_INFO,TSIF,TIFOR,TOTEN)
    USE mgrid
    USE base
...
! for pure Lennard-Jones we invoke the vdw routines directly
    IVDW_GL = 612
    CALL vdw_forces_main(GRIDC,XC,IO,LATT_CUR,DYN,T_INFO,TSIF,TIFOR,TOTEN, &
    & P%ELEMENT,RELCHG,RELVOL,VDW_REFSTATE_GL,Overlap,M2,HCD,LHESSEVDW,HESSE_VDW)
...
!***********************************************************************
   ...
   SUBROUTINE VCA_DER( & 
        ...
        T_INFO,INFO,XC,IO,KPOINTS,GRID,GRID_SOFT, &
      ...
      USE pp_data
      USE setexm_struct_def, ONLY : xc_info
      USE tau_mu, ONLY : tau_handle,TAUPAR
     ...
     DO NT=1,T_INFO%NTYP
        IF (T_INFO%VCA(NT)/=1 .AND. T_INFO%VCA(NT)/=0) THEN
           ...
           DO IDIS=-1,1,2
              ...
              IF (XC%LTAU) CALL TAUPAR(GRIDC,T_INFO,LATT_CUR%B,LATT_CUR%OMEGA,P,CSTRF,KINEDEN%TAUC)
