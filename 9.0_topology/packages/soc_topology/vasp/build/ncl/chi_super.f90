...
!***********************************************************************
  ...
  SUBROUTINE CALCULATE_XI_REAL( & 
          ...
          T_INFO, DYN, INFO, XC, IO, KPOINTS, SYMM, MIX, & 
    ...
    USE tutor, ONLY: vtutor, isError, isAlert, ACFDTRnotSupported, &
        ...
        TauPar, chiGG_NOmega, OmegaGridOmegaTL, newArgument, &
! OEP related
        LPOTOK  
...
! we need two descriptors, (1._q,0._q) for the time (tau) and (1._q,0._q) for the energy-frequency (nu) domain
! these two are required since in time domain the data are distributed over all cores
! and (1._q,0._q) imaginary time is usually 1._q at time
! whereas in frequency domain a subset of cores can work on (1._q,0._q) frequency
...
!!!need to figure out which (1._q,0._q) we need in the WORK state
    ...
    COMPLEX(q), POINTER       :: SIGMAO_DIAG(:)=>NULL()      ! diagonal of <i| Sigma_o(tau) | i> of self-energy
    COMPLEX(q), POINTER       :: SIGMAU_DIAG(:)=>NULL()      ! diagonal of <i| Sigma_u(tau) | i> of self-energy
    COMPLEX(q), POINTER, CONTIGUOUS :: SIGMAO_MAT(:,:,:,:)=>NULL() ! matrix <i| Sigma_o(tau) | a> stored distributed
    COMPLEX(q), POINTER, CONTIGUOUS :: SIGMAU_MAT(:,:,:,:)=>NULL() ! matrix <i| Sigma_u(tau) | a> stored distributed
    COMPLEX(q), POINTER, CONTIGUOUS :: CHAM_MAT(:,:,:,:)=>NULL()   ! Hamilton matrix T + V_local + V_x or density matrix
    COMPLEX(q), POINTER, CONTIGUOUS :: GAMMA(:,:,:,:)=>NULL()      ! density matrix for (1._q,0._q) k-point
    COMPLEX(q), POINTER, CONTIGUOUS :: CORR_MAT(:,:,:,:)=>NULL()   ! correlation contribution to density matrix
    COMPLEX(q), POINTER, CONTIGUOUS :: SIGMA_MAT(:,:,:,:)=>NULL()  ! int G_0(w) (\Sigma(w)+H_HF) G_0(w) w dw
    COMPLEX(q), POINTER, CONTIGUOUS :: GO_MAT(:,:,:,:)=>NULL()     ! matrix <i| G_o(tau) | a> stored distributed
    COMPLEX(q), POINTER, CONTIGUOUS :: GU_MAT(:,:,:,:)=>NULL()     ! matrix <i| G_u(tau) | a> stored distributed
...
# 195
    ...
    COMPLEX(q), POINTER       :: SIGMAO_TAU(:,:,:,:)=>NULL() ! diagonal of <i| Sigma_o(tau) | i> of self-energy
    COMPLEX(q), POINTER       :: SIGMAU_TAU(:,:,:,:)=>NULL() ! diagonal of <i| Sigma_u(tau) | i> of self-energy
    INTEGER         :: NTAU_ROOT
...
!    LOGICAL :: LKINETIC=.TRUE.
    ...
    TYPE (wavedes), POINTER :: WGWF=>NULL()
    TYPE (grid_3d), POINTER :: GRIDWGWF=>NULL()
    TYPE (screened_2e_handle) :: S2EF
...
!=======================================================================
! switch off symmetry
! interpolation of electronic structure factor requires thi work-around
! for RPAgamma, where only half of the full k-points are used for
...
!=======================================================================
    IF ( LESF_SPLINES ) THEN
       ...
       ALLOCATE(WGWF, GRIDWGWF)
       CALL GENERATE_RESPONSE_DESCRIPTOR( WGWF, GRIDWGWF, GRID_SOFT, LATT_CUR, INFO, IO )

! determine the q-points in IrBZ that are used for the response function
       CALL SETUP_IRZ_MAP(S2EF, WGWF, IO%IU0, IO%IU6)
!----------------------------------------------------------------------
! prepare ACFDT total energy setup
! also index k-points for electronic structure factor interpolation
! this is required for RPAgamma calculations
!----------------------------------------------------------------------
       CALL XI_ACFDT_SETUP_GG( COR, ENCUTGW, ENCUTGWSOFT)
! allocate electronic structure factor for all q-points
! also set up map from IRZ to full BZ (stored in ESF%KPOINTS_TRANS)
       NULLIFY( COR%ESF ) 
       ALLOCATE( COR%ESF( COR%NE ) ) 
       DO I = 1, COR%NE 
          CALL ALLOCATE_ELSTUFAC_HANDLE( COR%ESF(I), WGWF, COR%ENCUTGW(I), COR%ENCUTGWSOFT(I), LATT_CUR, &
               SYMM, WDES, S2EF%NUMBER_OF_NQ, T_INFO, DYN, IO ) 
       ENDDO
! get rid of the IRZ map, since only the number of q-points has been used
       CALL DEALLOCATE_IRZ_MAP(S2EF)
    ENDIF
...
!=======================================================================
    ...
    CALL GENERATE_RESPONSE_DESCRIPTOR( WGW, GRIDWGW, GRID_SOFT, LATT_CUR, INFO, IO )

!----------------------------------------------------------------------
! prepare ACFDT total energy setup
!----------------------------------------------------------------------
! allocate correlation energy arrays
    IF ( .NOT. LESF_SPLINES ) CALL XI_ACFDT_SETUP_GG( COR, ENCUTGW, ENCUTGWSOFT)
...
!we need either minimax or frequency grid + fourier matrix
    ...
    NOMEGA_GRID=NOMEGA
    IF (LHFCALC_GG ) NOMEGA_GRID=NOMEGA-1
...
! frequencies (ideally (1._q,0._q)) and works only on those
!=======================================================================
! instead of G(t)G(-t) (1._q,0._q) can also set up G(-t)G(-t)
! this has the effect that X(0)V = E_X
! currently this is supported only for NTAUPAR = 1
    IF (LHFCALC_GG) THEN
       IF (IO%IU0>0) WRITE(IO%IU0,*) 'internal WARNING in VASP: adding tau=0- as grid point, forcing NTAUPAR=1 '
...
!total number of CPUs or NOMEGAPAR
       ...
       CALL vtutor%write(isAlert, OmegaPar, newArgument( ival=[NOMEGAPAR]) )
    ENDIF
...
!total number of CPUs or NTAUPAR
       ...
       CALL vtutor%write(isAlert, TauPar, newArgument( ival=[NTAUPAR]) )
    ENDIF
...
!so the total number of CPUs has to be dividable by these numbers
       CALL vtutor%write(isError, TauParCPU, &
         newArgument( ival=[WDES%COMM_KIN%NCPU,NTAUPAR, NOMEGAPAR ] ) )
    ENDIF

    IF (NOMEGA_GRID==1) THEN
       CALL SET_IMAG_GRID_HANDLE(OMEGAMIN, OMEGATL, NOMEGA_GRID, OMEGAGRID, &
           KPOINTS%SIGMA, IMAG_GRIDS, WDES%COMM, IO)
       CALL ALLOCATE_IMAG_GRID( IMAG_GRIDS, NOMEGA_GRID )
...
! it will determine the direct MP2 correlation energy, but nonsence for RPA correlation energy
    ...
    CALL M_barrier(WDES%COMM)

    IF (IO%LOPEN) CALL WFORCE(IO%IU6)
# 625


! check imaginary grid quality for Luttinger-Ward calculations only
    IF( (LFINITE_TEMPERATURE .AND. LUTTINGER_WARD) .AND. ( LG0W0 .OR. LGW0 ) ) CALL CHECK_IMAG_GRID_QUALITY( W, IMAG_GRIDS, INFO, IO )
...
!=======================================================================
! set two green function descriptors, (1._q,0._q) for time and (1._q,0._q) for frequency domain
...
! G(nu) is distributed among all nodes in groups, each group treating (1._q,0._q) freq. point
...
! TODO: gK (1._q,0._q) could save memory if k-point rotation is 1._q inside GW routine
...
! this object is stored in CHI0
   ...
   CALL START_TIMING("GWLOOP")  ! super timer for e.g. (1._q,0._q) loop
...
! calculate convergence correction for exchange part
      IF ( LDMP1 ) FSG0 = FSG0 + KPOINTS_ORIG%WTKPT(NQ)*FSG_STORE(NQ) 
...
! convergence correction to the exact exchange
   IF ( LDMP1 ) THEN
      FSG0 = FSG0 * INFO%NELECT / WDES%RSPIN
      COR%CORRMP2DIR(:) = COR%CORRMP2DIR(:) + FSG0 
   ENDIF 
! linear regression
   CALL LIN_REG_GG(COR, E, IO)

! deallocate correlation energy array
! this routine also deallocates electronic structure factor
...
! backup Tr{ gam_DFT h_HF } to calculate Singles in GWSE
       CSINGLES = E%EBANDSTR 
       IF (IO%IU6>=0) WRITE(IO%IU6,'(" HF energy in DFT basis:",F18.8)') REAL(CSINGLES  ,q)
    ENDIF
...
! allocate space for self-energy in frequency domain, 1 distributed
...
!allocate storage for self-energies in Bloch domain
       ...
       SIGMAO_DIAG=(0._q,0._q)
       SIGMAU_DIAG=(0._q,0._q)
...
!CHI should point to current q-point
             ...
             IF ( IMAG_GRIDS%T%LDO_POINT_LOCAL ) CALL CLEAR_RESPONSE(CHI_TAU(NQ)) !(1._q,0._q) q-point at a time
             CALL SET_RESPONSE_KPOINT(CHI_TAU(NQ), WDES%VKPT(:,NQ), NQ)
...
! This should not be 1._q if LGW0=.TRUE. since the correlation contribution
...
!   (hopefully to get QP corrections via analytic continuation)
    ...
    CALL M_sum_z(GDES_MAT%COMM_INTER, CORR_MAT, SIZE(CORR_MAT))
...
! diagonal part of self-energy on imaginary time axis
    CALL M_sum_z(IMAG_GRIDS%T%COMM_BETWEEN_GROUPS, SIGMAO_TAU, SIZE(SIGMAO_TAU))
    CALL M_sum_z(IMAG_GRIDS%T%COMM_BETWEEN_GROUPS, SIGMAU_TAU, SIZE(SIGMAU_TAU))
...
# 1316
    ...
    IF (ASSOCIATED(SIGMA_MAT)) & 
        CALL M_sum_z(GDES_MAT%COMM_INTER, SIGMA_MAT, SIZE(SIGMA_MAT))
    CALL M_sum_z(GDES_MAT%COMM_INTER, MP2SUM, 1)
    CALL M_sum_z(GDES_MAT%COMM_INTER, MP2SUM2, 1)
    CALL M_sum_z(GDES_MAT%COMM_INTER, SIGMA_SIN, SIZE(SIGMA_SIN))
    CALL M_sum_z(GDES_MAT%COMM_INTER, SIGMA_COS, SIZE(SIGMA_COS))
    CALL M_sum_d(GDES_MAT%COMM_INTER, FRNL, SIZE(FRNL))
...
!> note, E%EBANDSTR still contains Tr{ gam_DFT h_HF } in G0W0R NATURALO = 2 case,
!> which is required for GWSE singles corrections, make a backup here
    CSINGLES = E%EBANDSTR 
!
! set OEP shift
!
...
! R stores the diagonal elements
       ...
       CALL M_sum_z(GDES_MAT%COMM_INTRA, R, WDES%NB_TOTK(NK1, ISP) )
...
! this is 1._q below, so just clean array here
...
!
                ...
                CALL  ADD_TO_DIAGONALE_REAL( WDES%NB_TOTK(NK1,ISP), GAMMA(:,1,1,1), RR, GDES_MAT%DESC)
!
! for gam^1 this must be 1._q manually here
!
                IF ( ASSOCIATED( SIGMA_MAT ) )  THEN
...
!#endif
          ...
          CALL M_bcast_d_from(GDES_MAT%COMM_INTER , RR(1), SIZE(RR), 1)
!#ifdef verbose
...
! for insulators (1._q,0._q) iteration should be identical to original version documented out below
...
! bcast from root group to all other groups to avoid accumulation of rounding errors
       CALL M_bcast_z_from(GDES_MAT%COMM_INTER, GAMMA(1,1,1,1), SIZE(GAMMA,1), 1)
...
!
! if Luttinger Ward is not selected compute only Galitskii-Migdal energy
!
    IF ( .NOT. LUTTINGER_WARD ) THEN
       TOTEN = TOTEGM
       E%EENTROPY = 0
    ENDIF 
    ...
    IF (.NOT. LRPAFORCE) THEN
       ...
       IF (IO%IU0>=0) WRITE(IO%IU0,'(" GWSE singles contribution: ",F18.8)') REAL(E%EBANDSTR- CSINGLES , q)
       IF (IO%IU6>=0) WRITE(IO%IU6,'(/,"  GWSE singles contribution: ",F18.8)') REAL(E%EBANDSTR - CSINGLES , q)
...
!-----------------------------------------------------------------------
! update the (1._q,0._q)-electron orbitals, and rotate Greens function & density matrix
    IF (LRPAFORCE) THEN
...
! destroy  sclapack descriptor
    ...
    IF (.NOT.IO%LH5 .AND. IO%LWAVE .AND. .NOT. ( LACFDT .OR. LCRPA ) ) THEN
       CALL OUTWAV(IO, WDES, W, LATT_CUR, W%EFERMI(1), NBANDS_DUMP = NBANDS_WAVE )
    ENDIF

    IF (IO%LH5 .AND. IO%LWAVE .AND. .NOT. ( LACFDT .OR. LCRPA )) THEN
...
!destroy imaginary grid handle
    ...
    IF ( LESF_SPLINES ) THEN
       DEALLOCATE(WGWF%VKPT, WGWF%WTKPT) 
       DEALLOCATE(WGWF, GRIDWGWF)
    ENDIF 
...
!***********************************************************************
    SUBROUTINE  REF_GF_HFHAM_CORR_DENS_MAT( CHAM_MAT, CORR_MAT, SIGMAW_MAT ) 
       COMPLEX(q), POINTER, INTENT(INOUT) :: CHAM_MAT(:,:,:,:)
       COMPLEX(q), POINTER, INTENT(INOUT) :: CORR_MAT(:,:,:,:)
...
! compute change in bandstructure energy between iterations
    DTRGHF = E%EBANDSTR
!> E%BANDSTR contains now Tr{ gam_RPA h_HF } contribution in Eq. (34) of JCP 143,102816 (2015)
!> to obtain E_c^(GWSE) (1._q,0._q) has to subtract Tr{ gam_DFT h_HF }
    CALL DUMP_DIAGRAMMATIC_ENERGIES_RPA("Energies using frozen KS orbitals", E, TOTEN, INFO, IO ) 
...
! reset Fermi-energy and (1._q,0._q)-electron occupancies to previous value
...
! mix the Fermi-level with previous (1._q,0._q), and store it as the reference value for the next iteration
...
!> routine that requires
     SUBROUTINE GENERATE_RESPONSE_DESCRIPTOR( WGW, GRIDWGW, GRID_SOFT, LATT_CUR, INFO, IO )
       USE xi, ONLY: XI_DATAKE
       IMPLICIT NONE
       TYPE (wavedes)     :: WGW        ! < result, descriptor for response function
       TYPE (grid_3d)     :: GRIDWGW    ! < grid structure for response function
       TYPE (grid_3d)     :: GRID_SOFT  ! grid for soft chargedensity
       TYPE (latt)        :: LATT_CUR
       TYPE (info_struct) :: INFO
       TYPE (in_struct)   :: IO
...
! replace kinetic energy with the Coulomb potential in the WGW%DATAKE array
      ...
      END SUBROUTINE GENERATE_RESPONSE_DESCRIPTOR
