...
! from which partial occupancies are set
   ...
   INTEGER IRDMAX
   INTEGER N_MIX_PAW
   REAL(q) BTRIAL,ORT,RMS,TOTEN,TOTEN2,DESUM1
...
! local variables
   ...
   REAL(q)    :: WSCAL,ORT1,ORT2,DE,DE1,DE2,ORTCEL,DECEL,WSUM,EFERMI_SHIFT,WEIGHT,X,DFUN,SFUN,DER
   REAL(q)    :: BCSTEP,BOPT,DORT,THETA,A1,A2,WMIN,DECEL_in,ORTCEL_in,DIFCEL
...
!=======================================================================
   CALL MRG_AUX(WDES,W)
!$ACC ENTER DATA CREATE(WSCAL) 
!$ACC KERNELS PRESENT(W,WSCAL) 
...
!=======================================================================
   ...
   IF (NSIM_LOCAL*NCPU /= NSIM) THEN
      CALL vtutor%bug("internal ERROR in EDWAV: NSIM is not correct " // str(NSIM), "rot.F", 299)
   ENDIF
...
!$ACC ENTER DATA CREATE(CPROW,CF,CFOLD,CPROF) 
...
! wave functions coefficients are redistributed in W_F
      IF (IALGO>=8) THEN
!$ACC PARALLEL LOOP GANG PRESENT(WDES1,W,WA_F) 
         DO N1=1,NB_TOT
...
!$ACC LOOP VECTOR
            ...
            ENDIF
         ENDDO
!$ACC PARALLEL LOOP GANG PRESENT(WDES,W,WA_F) COPYIN(W%FERWE(:,NK,ISP)) 
...
!=======================================================================
!$ACC KERNELS PRESENT(CF,CPROF,CFOLD) 
      CF=0
      CPROF=0
      CFOLD=0
!$ACC END KERNELS
      ...
      DO N=NN,NNMAX ! each step in the loop copies NSIM_LOCAL/NSIM bands !
         ...
         CALL ZCOPY(WDES%NRPLWV,W_G%CPTWFP(1,N,NK,ISP),1,CF(1,NP),1)

         CALL ZCOPY(WDES%NRPLWV,W_F%CPTWFP(1,N,NK,ISP),1,CFOLD(1,NP),1)
 
         CALL ZCOPY(WDES%NRPLWV,W%CPTWFP(1,N,NK,ISP),1,W1(NP)%CPTWFP(1),1)
         CALL ZCOPY(WDES%NPROD,W%CPROJ(1,N,NK,ISP),1,W1(NP)%CPROJ(1),1)
...
!$ACC KERNELS PRESENT(CF,CPROF,W1) 
            CF(:,NP)=0
            CPROF(:,NP)=0
            W1(NP)%CPTWFP=0
            W1(NP)%CR=0
...
!$ACC PARALLEL LOOP PRESENT(W,CHAM) 
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CHAM) 
         DO N1=NPOS_RED,NPOS_RED+NSTRIP_RED-1
            DO N2=1,NPOS_RED-1
...
!-----------------------------------------------------------------------
      CALL ZCOPY(WDES%NRPLWV,CF(1,NP),1,WSEARCH%CPTWFP(1),1)
...
!----------------------------------------------------------------------
      DO ISPINOR=0,WDES1%NRSPINORS-1
         CALL FFTWAV(NGVECTOR,WDES1%NINDPW(1),WSEARCH%CR(1+ISPINOR*WDES1%GRID%MPLWV),WSEARCH%CPTWFP(1+ISPINOR*NGVECTOR),GRID)
      ENDDO
      IF (LREAL) THEN
# 720
...
!$ACC UPDATE SELF(W%AUX(N,NK,ISP))  ! W%AUX(N,NK,ISP) is needed on the host (below)
...
! use local contributions only
         CB_=ZDOTC(WDES1%NPL,WSEARCH%CPTWFP(1),1,CF(1,NP),1)
...
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON) ! wait for W%AUX(N,NK,ISP)
...
!$ACC UPDATE DEVICE(W%AUX(N,NK,ISP)) 
...
! gradient should be orthogonal to present wavefunction
      DORT=ZDOTC(WDES1%NPL,W1(NP)%CPTWFP(1),1,CF(1,NP),1)
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
      ...
      IF (ABS(DORT)>1E-4) THEN
         CALL vtutor%bug("EDWAV: internal error, the gradient is not orthogonal " // str(NK) // &
            " " // str(NP) // " " // str(DORT), "rot.F", 805)
      ENDIF

! orthogonality of gradient to previous search direction
      DORT=ZDOTC(WDES1%NPL,CFOLD(1,NP),1,CF(1,NP),1)
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
      CALL M_sum_d(WDES%COMM_INB, DORT, 1)
...
! energy change for a unit step along gradient
      IF (IROT==1 .OR. IROT>=3 ) THEN
         DE=ZDOTC(WDES1%NPL,WSEARCH%CPTWFP(1),1,CF(1,NP),1)
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
...
!$ACC KERNELS PRESENT(W_G) 
         W_G%CPTWFP(:,N,NK,ISP)=0
!$ACC END KERNELS
         CALL ZCOPY(WDES1%NPL,WSEARCH%CPTWFP(1),1,W_G%CPTWFP(1,N,NK,ISP),1)
         
!$ACC KERNELS PRESENT(W_G) 
         W_G%CPROJ(:,N,NK,ISP)=0
!$ACC END KERNELS
         CALL ZCOPY(WDES%NPRO,WSEARCH%CPROJ(1),1,W_G%CPROJ(1,N,NK,ISP),1)
...
! set the gradients up to NBANDSLO_LOCAL to (0._q,0._q)
      IF (NBANDSLO_LOCAL>1) THEN
!$ACC KERNELS PRESENT(W_G) 
         W_G%CPTWFP   (:,1:NBANDSLO_LOCAL-1,NK,ISP)=0
...
!$ACC END KERNELS
         ...
         ENDDO
      ENDDO
!$ACC EXIT DATA DELETE(CPROW,CF,CFOLD,CPROF) 
...
!$ACC ENTER DATA CREATE(CHAMP) 
         CALL ZCOPY(WDES%NB_TOT*WDES%NB_TOT*WDES%NKPTS*WDES%ISPIN,CHAM(1,1,1,1),1,CHAMP(1,1,1,1),1)
...
! store diagonals for later use (update of fermi-weights)
            IF (ICEL/=0) THEN
               CALL vtutor%bug("internal error in EDWAV: for IROT=5, ICEL must be 0", "rot.F", 1005)
            ENDIF
         ELSE
! store diagonals for later use (update of fermi-weights)
...
!$ACC PARALLEL LOOP GANG REDUCTION(+:WSUM,EFERMI_SHIFT) PRESENT(WDES,W,W_F) PRIVATE(WEIGHT) 
         DO ISP=1,WDES%ISPIN
         DO NK=MYKSTART,WDES%NKPTS,MYKINCR
            WEIGHT =WDES%WTKPT(NK)*WDES%RSPIN
!$ACC LOOP VECTOR REDUCTION(+:WSUM,EFERMI_SHIFT) PRIVATE(X,DFUN,SFUN)
...
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON) ! wait for WSUM and EFERM_SHIFT
...
!$ACC PARALLEL LOOP COLLAPSE(2) GANG REDUCTION(+:DECEL,ORTCEL) PRESENT(WDES,W,W_F,W_G) PRIVATE(WEIGHT)
         DO ISP=1,WDES%ISPIN
         DO NK=MYKSTART,WDES%NKPTS,MYKINCR
            WEIGHT =WDES%WTKPT(NK)*WDES%RSPIN
!$ACC LOOP VECTOR REDUCTION(+:DECEL,ORTCEL) PRIVATE(X,DFUN,SFUN)
            DO N =1,W%WDES%NB_TOTK(NK,ISP)
! skip states below NBANDSLOW
               IF (N<WDES%NBANDSLOW) CYCLE
...
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON) ! wait for DECEL and ORTCEL
      ...
      ELSE IF (KPOINTS%SIGMA>0 .AND. ICEL == 1) THEN
         DECEL  =0
         ORTCEL =0
!$ACC PARALLEL LOOP COLLAPSE(2) GANG PRESENT(WDES,W_G,W_F,W) 
         DO ISP=1,WDES%ISPIN
         DO NK=MYKSTART,WDES%NKPTS,MYKINCR
!$ACC LOOP VECTOR
            DO N =1,W%WDES%NB_TOTK(NK,ISP)
...
! step for wavefunction
      IF (IROT==1 .OR. IROT>=3) THEN
!$ACC PARALLEL LOOP COLLAPSE(4) PRESENT(WDES,W,W_F) 
        DO ISP=1,WDES%ISPIN
        DO NK=MYKSTART,WDES%NKPTS,MYKINCR
        DO N=1,WDES%NBANDS
        DO M=1,WDES%NRPLWV
...
!$ACC PARALLEL LOOP COLLAPSE(2) GANG PRESENT(WDES,W,W_F) 
        DO ISP=1,WDES%ISPIN
        DO NK=MYKSTART,WDES%NKPTS,MYKINCR
!$ACC LOOP VECTOR
        DO N =1,W%WDES%NB_TOTK(NK,ISP)
...
!-----------------------------------------------------------------------
      IF (IROT==1 .OR. IROT>=3) THEN
!$ACC PARALLEL LOOP COLLAPSE(4) PRESENT(WDES,W_F,W_G) 
        DO ISP=1,WDES%ISPIN
        DO NK=MYKSTART,WDES%NKPTS,MYKINCR
        DO N=1,WDES%NBANDS
        DO M=1,WDES%NRPLWV
...
!$ACC PARALLEL LOOP COLLAPSE(4) PRESENT(WDES,CHF,CHAM) 
        DO ISP=1,WDES%ISPIN
        DO NK=MYKSTART,WDES%NKPTS,MYKINCR
        DO N2=1,NB_TOT
        DO N1=1,NB_TOT
      ...
      IF (KPOINTS%SIGMA>0 .AND. ICEL == 1) THEN
!$ACC PARALLEL LOOP COLLAPSE(2) GANG PRESENT(W,W_F,W_G) 
        DO ISP=1,WDES%ISPIN
        DO NK=MYKSTART,WDES%NKPTS,MYKINCR
!$ACC LOOP VECTOR
        DO N =1,W%WDES%NB_TOTK(NK,ISP)
...
!-----------------------------------------------------------------------
      BSTEP=BTRIAL
      IF (IROT==1 .OR. IROT>=3) THEN
!$ACC PARALLEL LOOP COLLAPSE(4) PRESENT(WDES,W_G,W,W_F) 
        DO ISP=1,WDES%ISPIN
        DO NK=MYKSTART,WDES%NKPTS,MYKINCR
        DO N=1,WDES%NBANDS
        DO M=1,WDES%NRPLWV
...
! update fermi-weights W%FERTOT
      ...
      IF (KPOINTS%SIGMA>0 .AND. ICEL == 1) THEN
!$ACC PARALLEL LOOP COLLAPSE(2) GANG PRESENT(W,W_F) 
         DO ISP=1,WDES%ISPIN
         DO NK=MYKSTART,WDES%NKPTS,MYKINCR
!$ACC LOOP VECTOR
         DO N =1,W%WDES%NB_TOTK(NK,ISP)
...
! store step and energy for later use
      NCOUNT=NCOUNT+1
      IF (NCOUNT > NSTEP) CALL vtutor%bug("NCOUNT exceeds array dimensions", "rot.F", 1582)
      STEP(NCOUNT)  =BSTEP
      ENRGY(NCOUNT)=TOTEN2
...
!$ACC PARALLEL LOOP COLLAPSE(4) PRESENT(WDES,W,W_G,W_F) 
        DO ISP=1,WDES%ISPIN
        DO NK=MYKSTART,WDES%NKPTS,MYKINCR
        DO N=1,WDES%NBANDS
        DO M=1,WDES%NRPLWV
...
!$ACC EXIT DATA DELETE(COVL,CUNI,CEIDB) 
...
!$ACC EXIT DATA DELETE(WSEARCH) 
!$ACC EXIT DATA DELETE(CELTOT) 
!$ACC UPDATE SELF(W_G%CPTWFP,W_G%CPROJ,W_G%FERTOT) 
   ENDIF
# 1820
...
!$ACC PARALLEL LOOP GANG VECTOR COLLAPSE(2) REDUCTION(+:DE2,ORT2) PRESENT(W,WDES,CHAM,CHAMP,W_F) &
!$ACC PRIVATE(DIFFER,CEXPRE,DIFCEL,CROT,THETA) 
         DO N2=1,WDES%NB_TOT
            DO N1=1,WDES%NB_TOT
...
! gradient times present precond. gradient
         ...
         CALL ZCOPY(WDES%NB_TOT*WDES%NB_TOT,CEIDB(1,1),1,CHAM(1,1,NK,ISP),1)
!         CALL DUMP_HAM( "rotation matrix",WDES, CHAM(1,1,NK,ISP))

      ENDDO
      ENDDO

!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON) ! wait for ORT2 and DE2
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CHF,CEIDB) PRIVATE(CH,THETA) 
      DO N1=1,NBANDS
         DO N2=1,NBANDS
            CH= CHF(N1,N2)
...
!$ACC PARALLEL LOOP PRESENT(CEIDB) 
      DO N1=1,NBANDS
         CEIDB(N1,N1)=1
      ENDDO
...
! Diagononalize the resulting Matrix (search direction)
!=======================================================================
      CALL ZCOPY(NBANDS*NBANDS,CHF(1,1),1,CUNI(1,1),1)
...
! set up the unitary transformation CEIDB for wavefunctions
!=======================================================================
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CUNI,CTMP) 
      DO N1=1,NBANDS
      DO N2=1,NBANDS
...
! take the transposed of CEIDB because MATMUL uses the transposed ...
!=======================================================================
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CTMP,CEIDB) 
      DO N1=1,NBANDS
      DO N2=1,NBANDS
         CTMP(N1,N2)=CEIDB(N2,N1)
      ENDDO
      ...
      CALL ZCOPY(NBANDS*NBANDS,CTMP(1,1),1,CEIDB(1,1),1)
...
!      depsilon_Fermi =
!     [\sum_n df_n/d(epsilon_n-epsilon_Fermi) x d epsilon_n ]/ [\sum_n df_n/d(epsilon_n-epsilon_Fermi)]
!$ACC PARALLEL LOOP COLLAPSE(2) GANG REDUCTION(+:WSUM,EFERMI_SHIFT) PRESENT(WDES,W,W_F) PRIVATE(WEIGHT) 
      DO ISP=1,WDES%ISPIN
         DO NK=MYKSTART,WDES%NKPTS,MYKINCR
            WEIGHT =WDES%WTKPT(NK)*WDES%RSPIN
!$ACC LOOP VECTOR REDUCTION(+:WSUM,EFERMI_SHIFT) PRIVATE(X,DFUN,SFUN)
...
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON) ! wait for WSUM and EFERMI_SHIFT
      CALL M_sum_d(WDES%COMM_KINTER,WSUM,1)
      CALL M_sum_d(WDES%COMM_KINTER,EFERMI_SHIFT,1)
...
!$ACC PARALLEL LOOP COLLAPSE(2) GANG REDUCTION(+:DECEL,ORTCEL) PRESENT(WDES,W,W_F,CHAM,CHF) PRIVATE(WEIGHT)
      DO ISP=1,WDES%ISPIN
         DO NK=MYKSTART,WDES%NKPTS,MYKINCR
            WEIGHT =WDES%WTKPT(NK)*WDES%RSPIN
!$ACC LOOP VECTOR REDUCTION(+:DECEL,ORTCEL) PRIVATE(X,DFUN,SFUN)
...
!     ENDIF
      ...
      DO NK=MYKSTART,WDES%NKPTS,MYKINCR
         WEIGHT =WDES%WTKPT(NK)
!$ACC PARALLEL LOOP GANG COLLAPSE(2) REDUCTION(-:DE2,ORT2) PRESENT(CEIDB,WDES,W,CHAM,CHAMP,CHF) PRIVATE(DIFFER,CGRAD,DIFCEL,CROT) 
         DO N2=1,WDES%NB_TOT
            DO N1=1,WDES%NB_TOT
              IF (N1/=N2) THEN
                 DIFFER=     (W%FERTOT(N1,NK,ISP)-W%FERTOT(N2,NK,ISP))
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(WDES,CEIDB,CHAM) 
         DO N2=1,WDES%NB_TOT
            DO N1=1,WDES%NB_TOT
               IF (N1/=N2) THEN
...
!=======================================================================
!$ACC KERNELS PRESENT(CHF) 
      CHF=CONJG(CHF)
!$ACC END KERNELS
!$ACC PARALLEL LOOP GANG PRESENT(CUNI,CHF,CELTOT) 
      DO N1=1,NBANDS
!$ACC LOOP VECTOR
         DO N2=1,NBANDS
...
!$ACC KERNELS PRESENT(CELTOT,HFEIG) 
      CELTOT=HFEIG
!$ACC END KERNELS
!$ACC EXIT DATA DELETE(HFEIG) 
!=======================================================================
! transform AUXTOT (conditioning step with for each orbital)
!=======================================================================
      IF (PRESENT(AUXTOT)) THEN
! CTMP = AUXTOT * U
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CTMP,AUXTOT,CUNI) 
         DO N1=1,NBANDS
            DO N2=1,NBANDS
...
!$ACC PARALLEL LOOP PRESENT(AUXTOT,CEIDB) 
         DO N1=1,NBANDS
...
!=======================================================================
!$ACC KERNELS PRESENT(CEIDB,CUNI,CHF) 
      CEIDB=CUNI
      CHF=CONJG(CHF)
!$ACC END KERNELS
