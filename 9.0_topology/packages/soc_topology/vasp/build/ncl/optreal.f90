...
!-----------------------------------------------------------------------
! allign QCUTP and QGAM with linear grid
! gK 30.05.2019: QCUTP must not exceed the available data
!     otherwise the optimization is sort of busted
!     also corrected some other of bound issues on arrays
!     this version has been tested with array bound checks activated
!-----------------------------------------------------------------------
      QCUTP=DELQNL*MIN(NQNL,INT(QCUTIN/DELQNL))
      IF (QCUTP>=QCUTIN) QCUTP=QCUTIN
      QGAM=DELQNL*INT(QGAMIN/DELQNL)
      ITYPE = 0
...
!     WRITE(80,*) 'next'
      DO N=1,NTEST
        ...
        ELSE
           SOLL=0
        ENDIF
! gK 30.05.2019: testing this only for N<NTEST does not seem very sensible
! but to maintain strict copatibility, leave it in place
        IF (N < NTEST) THEN
...
!        WRITE(80,'(4F14.7)') (N-1)*DELQNL,SUM-SOLL,SUMGAU-SOLL
      ENDDO
...
! check whether required accuracy was reached, but break at some point to avoid
! too large radii
      IF ( LAUTO.AND. ABS(WMAX/BETMAX)  > ABS(ROPT) .AND. RMAX < RMAXIN*2.5) THEN
         RMAX = RMAX*1.03
         CYCLE qcut_loop
      ENDIF
!-----------------------------------------------------------------------
...
! local variables
      REAL(q) :: SBESSITER !< define SBESSITER as a real function
...
! local variables
      ...
      CALL vtutor%error("ERROR: SBESSELITER : nicht konvergent")
