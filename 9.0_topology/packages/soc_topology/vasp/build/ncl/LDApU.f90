...
!***********************************************************************
      ...
      COMPLEX(q), PRIVATE, ALLOCATABLE, SAVE :: OCC_MAT_ALL(:,:,:,:)
...
!=======================================================================
      TLDA_U=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDAU', TLDA_U, IERR, WRITEXMLINCAR)
       
      TYPE_POT=2
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDAUTYPE', TYPE_POT, IERR, WRITEXMLINCAR)

      ALLOCATE(LANG_LDAPLUSU(NTYP),U(NTYP),J(NTYP))

      LANG_LDAPLUSU=2
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDAUL', LANG_LDAPLUSU, NTYP, IERR, WRITEXMLINCAR)
...
!       ENDDO
      ...
      U=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDAUU', U, NTYP, IERR, WRITEXMLINCAR)

      J=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDAUJ', J, NTYP, IERR, WRITEXMLINCAR)

      PRINTOCC=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDAUPRINT', PRINTOCC, IERR, WRITEXMLINCAR)

      LORBMOM=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LORBMOM', LORBMOM, IERR, WRITEXMLINCAR)
...
!=======================================================================
      DO ITYP=1,NTYP
         IF (.NOT.ASSOCIATED(P(ITYP)%QPAW).AND.LANG_LDAPLUSU(ITYP)/=-1) &
            CALL vtutor%write(isError, NoLDAU)
      ENDDO
...
!=======================================================================
      ...
      ALLOCATE(OVERLAP_AE_(LNMAX_,LNMAX_,NTYP))
      OVERLAP_AE_=0._q
! to signify that OVERLAP_AE_ needs to be calculated
      TFIRST=.TRUE.

      IF (PRINTOCC/=0) THEN
         ALLOCATE(OCC_MAT_ALL(LMMAX_,LMMAX_,NCDIJ,NIONS))
         OCC_MAT_ALL=0._q
      ENDIF
...
!**********************************************************************
      ...
      SUBROUTINE LDAPLUSU(LMDIM,NCDIJ,IATOM,ITYP,CRHODE,CDIJ,PP,DBLE_LDAU)
...
!===================================================================
      ...
      ALLOCATE(OCC_MAT(LMMAX,LMMAX,NCDIJ))
      OCC_MAT=0._q
...
!==============================================================
      ...
      IF (TYPE_POT == 1) THEN 
         CALL POT_TYPE1(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,PP%LPS,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
      ELSEIF (TYPE_POT == 2) THEN  
         CALL POT_TYPE2(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,PP%LPS,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
      ELSEIF (TYPE_POT == 3) THEN
         CALL POT_TYPE3(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,PP%LPS,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
      ELSEIF (TYPE_POT == 4) THEN 
         CALL POT_TYPE4(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,PP%LPS,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
      ELSEIF (TYPE_POT == 5) THEN 
         CALL POT_TYPE5(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,PP%LPS,OCC_MAT,CDIJ,DBLE_LDAU)
      ENDIF
...
!**********************************************************************
      ...
      SUBROUTINE POT_TYPE1(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,L_OF_LN,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
      ...
      COMPLEX(q) OCC_MAT(LMMAX,LMMAX,NCDIJ)
...
!**********************************************************************
      ...
      SUBROUTINE POT_TYPE4(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,L_OF_LN,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
...
!**********************************************************************
      ...
      SUBROUTINE POT_TYPE2(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,L_OF_LN,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
...
!**********************************************************************
      ...
      SUBROUTINE POT_TYPE3(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,L_OF_LN,OCC_MAT,CRHODE,CDIJ,DBLE_LDAU)
...
!**********************************************************************
      ...
      SUBROUTINE POT_TYPE5(LNMAX,LMMAX,LMDIM,NCDIJ,IATOM,ITYP,L_OF_LN,OCC_MAT,CDIJ,DBLE_LDAU)
...
!***********************************************************************
      ...
      SUBROUTINE CALC_ORBITAL_MOMENT(LMDIM,NCDIJ,IATOM,ITYP,CRHODE,PP,THETA,PHI)
      ...
      INTEGER LMDIM,NCDIJ,IATOM,ITYP
      REAL(q) THETA,PHI
      COMPLEX(q) CRHODE(LMDIM,LMDIM,NCDIJ) ! occupancy matrix
...
! Calculate the occupation matrix
      ...
      ALLOCATE(OCC_MAT(LMMAX,LMMAX,NCDIJ)) ; OCC_MAT=0._q
...
!           ORBMOM(L,I,IATOM)=ORBMOM(L,I,IATOM)+L_OP(M1,M2,I,L)*OCC_MAT(L*L+M1,L*L+M2,ISP)
      ...
      DEALLOCATE(OCC_MAT,L_OP,DUMMY)

      RETURN
      END SUBROUTINE CALC_ORBITAL_MOMENT

!***********************************************************************
!> @brief Get the variable ORBMOM of this module (LDAPLUSU_MODULE)
!
!> Performs an 1 sum of the angular momentum stored in ORBMOM and
!> reshape it before returning it in ORBITAL_MOMENT.
!
!***********************************************************************
      SUBROUTINE GET_ORBITAL_MOMENT(WDES,ORBITAL_MOMENT)
      ...
      REAL(q), INTENT(INOUT)       :: ORBITAL_MOMENT(:,:,:) !< orbital moments of shape [n_orbitals,n_ions,3]
! local
      INTEGER I, J, K
      INTEGER SHAPE_(3)

      SHAPE_ = SHAPE(ORBMOM) ! [n_orbitals,3,n_ions]
      
      DO I = 1, SHAPE_(1)
        DO J = 1, SHAPE_(2)
          DO K = 1, SHAPE_(3)
            ORBITAL_MOMENT(I,K,J) = ORBMOM(I,J,K)
          END DO
        END DO
      END DO

      CALL M_sum_d( WDES%COMM, ORBITAL_MOMENT, SIZE(ORBITAL_MOMENT))
 
      END SUBROUTINE GET_ORBITAL_MOMENT
