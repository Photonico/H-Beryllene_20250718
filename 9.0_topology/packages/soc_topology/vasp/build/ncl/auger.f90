...
!
          ...
          IF(USEDTIMERS>MAXTIMERS)THEN
             CALL vtutor%error("Maximum number of timers exceeded")
          END IF
          DO J=1,USEDTIMERS-1
             IF(TRIM(TIMER_TAG(J))==TRIM(TAG))THEN
                CALL vtutor%error("Timer tag already in use")
             END IF             
          END DO
...
!> flag to switch on calculation of Auger rates
        LAUGER=.FALSE.
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LAUGER', LAUGER, IERR, WRITEXMLINCAR)

!> calculate EEH processes
        LAUGER_EEH=.FALSE.
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LAUGER_EEH', LAUGER_EEH, IERR, WRITEXMLINCAR)

!> calculate EHH processes
        LAUGER_EHH=.FALSE.
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LAUGER_EHH', LAUGER_EHH, IERR, WRITEXMLINCAR)

!> half(!)-width of rectangular energy window function
!> i.e. (Theta_-AUGER_EWIDTH,+AUGER_EWIDTH)/(2*AUGER_EWIDTH)
        AUGER_EWIDTH=0.1_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EWIDTH', AUGER_EWIDTH, IERR, WRITEXMLINCAR)

!> Either AUGER_EDENS or AUGER_HDENS can be specified, but not both
!> density of conduction band electrons in [cm^-3]
        AUGER_EDENS=-1._q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EDENS', AUGER_EDENS, IERR, WRITEXMLINCAR)

!> density of valence band holes in [cm^-3]
        AUGER_HDENS=-1._q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_HDENS', AUGER_HDENS, IERR, WRITEXMLINCAR)
       
!> temperature for Fermi distribution in [K] (same for electrons and holes)
        AUGER_TEMP=300._q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_TEMP', AUGER_TEMP, IERR, WRITEXMLINCAR)

!> Fixed chemical potential in [eV] (in this case, AUGER_EDENS/AUGER_HDENS are ignored)
        AUGER_EFERMI=-2E30_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EFERMI', AUGER_EFERMI, IERR, WRITEXMLINCAR)

!> manual upper bound for valence band maximum (EVBM)
        AUGER_EVBHI=-2E30_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EVBHI', AUGER_EVBHI, IERR, WRITEXMLINCAR)

!> manual lower bound for conduction band minimum (ECBM)
        AUGER_ECBLO=-2E30_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_ECBLO', AUGER_ECBLO, IERR, WRITEXMLINCAR)
...
!> is larger than VBM/CBM*AUGER_OCC_FAC_EEH
        ...
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_OCC_FAC_EEH', AUGER_OCC_FAC_EEH, IERR, WRITEXMLINCAR)

!> we take into account only bands whose hole/lectron occupation
!> is larger than VBM/CBM*AUGER_OCC_FAC_EEH
        AUGER_OCC_FAC_EHH=1E-4_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_OCC_FAC_EHH', AUGER_OCC_FAC_EHH, IERR, WRITEXMLINCAR)

!> lower bound on band energy
        AUGER_EMIN_EEH=-2E30_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EMIN_EEH', AUGER_EMIN_EEH, SIZE(AUGER_EMIN_EEH), IERR, WRITEXMLINCAR)
        
!> upper bound on band energy
        AUGER_EMAX_EEH=-2E30_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EMAX_EEH', AUGER_EMAX_EEH, SIZE(AUGER_EMAX_EEH), IERR, WRITEXMLINCAR)

!> lower bound on band index
        AUGER_BMIN_EEH=-1000
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_BMIN_EEH', AUGER_BMIN_EEH, SIZE(AUGER_BMIN_EEH), IERR, WRITEXMLINCAR)
        
!> upper bound on band index
        AUGER_BMAX_EEH=-1000
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_BMAX_EEH', AUGER_BMAX_EEH, SIZE(AUGER_BMAX_EEH), IERR, WRITEXMLINCAR)

!> lower bound on band energy
        AUGER_EMIN_EHH=-2E30_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EMIN_EHH', AUGER_EMIN_EHH, SIZE(AUGER_EMIN_EHH), IERR, WRITEXMLINCAR)
        
!> upper bound on band energy
        AUGER_EMAX_EHH=-2E30_q
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_EMAX_EHH', AUGER_EMAX_EHH, SIZE(AUGER_EMAX_EHH), IERR, WRITEXMLINCAR)

!> lower bound on band index
        AUGER_BMIN_EHH=-1000
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_BMIN_EHH', AUGER_BMIN_EHH, SIZE(AUGER_BMIN_EHH), IERR, WRITEXMLINCAR)

!> upper bound on band index
        AUGER_BMAX_EHH=-1000
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'AUGER_BMAX_EHH', AUGER_BMAX_EHH, SIZE(AUGER_BMAX_EHH), IERR, WRITEXMLINCAR)
...
!> T    collect all wavefunctions at start before looping over k-points
        ...
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LAUGER_COLLECT', LAUGER_COLLECT, IERR, WRITEXMLINCAR)
...
!>        >=1    collect & use shared memory, NCSHMEM cores at each processor
        NCSHMEM=0
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'NCSHMEM', NCSHMEM, IERR, WRITEXMLINCAR)
...
!> In this case, AUGER_EWIDTH is reset automatically to the k-point spacing
        LAUGER_DHDK=.FALSE.
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LAUGER_DHDK', LAUGER_DHDK, IERR, WRITEXMLINCAR)
...
!> Otherwise, redistribution of k1,k3 and k2,k4 is separated
        LAUGER_JIT=.FALSE.
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LAUGER_JIT', LAUGER_JIT, IERR, WRITEXMLINCAR)
...
# 557
        ...
        IF(AUGER_EDENS<0 .AND. AUGER_HDENS<0.AND.AUGER_EFERMI<-1E30_q)THEN
           CALL vtutor%error("You must specify one out of AUGER_HDENS, AUGER_EDENS or AUGER_EFERMI in &
              &INCAR.")
        END IF
        IF(AUGER_EDENS>0 .AND. AUGER_HDENS>0)THEN
           CALL vtutor%error("You cannot specify both AUGER_HDENS and AUGER_EDENS in INCAR.")
        END IF
...
! Setup for using energy derivatives to determine energy width
        IF(LAUGER_DHDK)THEN
           ...
           IF(DHDK_NKPTS/=KPOINTS_IBZ%NKPTS)THEN
              CALL vtutor%error("CALCULATE_AUGER: DHDK_NKPTS /= KPOINTS_IBZ%NKPTS " // str(DHDK_NKPTS) &
                 // " " // str(KPOINTS_IBZ%NKPTS))
           END IF
           ALLOCATE(DHDK(DHDK_NB,3,DHDK_NKPTS))
           DO IK1=1,DHDK_NKPTS
              READ(199) DHDK_VKPT,DHDK(:,:,IK1)
              IF( MINVAL(ABS(DHDK_VKPT-KPOINTS_IBZ%VKPT(:,IK1)))>1E-8 )THEN
                 CALL vtutor%error("CALCULATE_AUGER: k-point changed " // str(IK1) // " " // &
                    str(DHDK_VKPT) // " " // str(KPOINTS_IBZ%VKPT(:,IK1)))
              END IF
           END DO
           CLOSE(199)
...
! CHECK
              IF( K1/=K1LIST(WDES%NB_LOW) ) THEN
                 CALL vtutor%error("SOMETHING WRONG " // str(WDES%NB_LOW) // " " // str(K1) // " " // &
                    str(K1LIST))
              END IF
!---------------------------------------------------------------------------
...
! Otherwise, START=0 > CALL M_stop('VASP aborting ...'); stop=-1 and the loops are not entered
              IF(LAUGER_EEH)THEN
...
! sanity check
           IF( (.NOT.(EV1_eeh<EVBM)) &
              ...
              CALL vtutor%error("CALCULATE_AUGER: Error in energy bounds _eeh: " // str(EV1_eeh) // &
                 " " // str(EVBM) // " " // str(ECBM) // " " // str(EC1_eeh) // " " // str(EC2_eeh) &
                 // " " // str(EC3_eeh))
           END IF
...
! sanity check
           IF( (.NOT.(EV3_ehh<EV2_ehh .AND. EV2_ehh<EVBM .AND. EV1_ehh<EVBM)) &
              ...
              CALL vtutor%error("CALCULATE_AUGER: Error in energy bounds _ehh: " // str(EV3_ehh) // &
                 " " // str(EV2_ehh) // " " // str(EV1_ehh) // " " // str(EVBM) // " " // str(ECBM) &
                 // " " // str(EC1_ehh))
           END IF
...
! is transformed corresponding to a wavevector q
        ...
        IF (NQ/=NQ_) THEN
           CALL vtutor%bug("internal error in AUGER_2E4O: q-point changed " // str(NQ) // " " // &
              str(NQ_), "auger.F", 2351)
        ENDIF
                 ...
                 DO N2=1,NSTRIP2
                    DO N4=1,NSTRIP4
                       ...
                       IF (NB4 > SIZE(M4O,4)) THEN
                          CALL vtutor%bug("internal error in AUGER_2E4O: out of bounds 4 " // &
                             str(NB4), "auger.F", 2481)
                       ENDIF

                       DO N3=1,NSTRIP3
                          NB3=NPOS3-1+N3
                          IF ( NB3> SIZE(M4O,2)) THEN
                             CALL vtutor%bug("internal error in AUGER_2E4O: out of bounds 2 " // &
                                str(NB3), "auger.F", 2488)
                          ENDIF
...
!
        ...
        IF( (D1-DENS)*(D2-DENS)>0 ) THEN
           CALL vtutor%error("DETERMINE_EFERMI: bisectioning not possible " // str(EMIN) // " " // &
              str(EMAX) // " " // str(D1) // " " // str(D2))
        END IF
...
! shift EF1
           ...
           END IF
        END DO
        CALL vtutor%error("DETERMINE_EFERMI: search for Fermi energy failed to converge")
101     RETURN
