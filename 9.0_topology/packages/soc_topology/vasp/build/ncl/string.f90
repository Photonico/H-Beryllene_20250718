...
!> Find or count all instances of (1._q,0._q) or more search patterns.
!>
...
!> module as search string, which will recognize a line ending character of
!> Unix, Mac, and Windows. It will also ignore all matches where a newline
...
! return number of elements found and optionally the indices
    ...
    pure subroutine interpret_special_characters(this, text, status_)
        ...
        character(len=*), parameter :: newline_unix = achar(10), newline_mac = achar(13), newline_dos = achar(13) // achar(10)
        logical is_newline_unix, is_newline_mac
...
! 1 newline would match second part of dos newline
        is_newline_unix = .not.status_%is_newline_dos .and. compare(newline_unix, text, status_%active_escape_with_whitespace)
        is_newline_mac = compare(newline_mac, text, status_%active_escape_with_whitespace)
        status_%is_newline_dos = compare(newline_dos, text, status_%active_escape_with_whitespace)
        status_%is_newline = is_newline_unix .or. is_newline_mac .or. status_%is_newline_dos
