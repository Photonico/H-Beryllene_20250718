...
!> The dimensions are (NBANDS,NK,ISP,NPROJ)
      COMPLEX(q), ALLOCATABLE, SAVE :: LPRJ_COVL_IAO(:,:,:,:)
...
!> when (1._q,0._q) or more local functions are of locproj::LPRJ_function::radial_type=2
...
!***********************************************************************
      ...
      SUBROUTINE LPRJ_READER(T_INFO,P,NONCOLLINEAR,IO,ISYM)
      USE base
      USE poscar
      USE pseudo
      USE full_kpoints
      ...
      INTEGER :: NUMB, LOCPROJ_SIZE, ISYM
...
!!   original order, i.e., the (1._q,0._q) in potcar::ATOMIC_E).
...
! TODO: this should be read from the input file, should be 1._q when we modify the parser
...
! Local variables
      ...
      LOGICAL :: LREALLOCATE
      REAL(q) :: KPT(3)
      COMPLEX(q),ALLOCATABLE :: LPRJ_COVL_LOCAL(:,:)
...
! allocate wavespin to store LPRJ_functions if desired
      IF (PRESENT(W_TRIALFUNC)) THEN
         ...
         W_TRIALFUNC%CPTWFP = cmplx(0.0_q,0.0_q,q)
         W_TRIALFUNC%CPROJ = (0._q,0._q)
      ENDIF
...
! Allocate WA_TRIALFUNC
            ...
            WA_TRIALFUNC%CPTWFP = cmplx(0.0_q,0.0_q,q)
            WA_TRIALFUNC%CPROJ = (0._q,0._q)
...
! if the batch of functions is not complete then add this (1._q,0._q)
...
!Reset the batch size
      ...
      CALL M_sum_z( WDES%COMM_KINTER, LPRJ_COVL, SIZE(LPRJ_COVL))
...
!> will be allocated. This is 1._q so that consecutive calls don't
...
! copy relevant (1._q,0._q) to SPLINE
...
! project onto Hydrogen-like functions (like in the 1 interface)
         CASE(3)
...
!> (band, wannier idx, kpoint, spin)
         COMPLEX(q), ALLOCATABLE, INTENT(OUT) :: UK(:, :, :, :)
...
! locals
         ...
         COMPLEX(q), ALLOCATABLE    :: SVD_V(:, :), SVD_WD(:, :)
         ...
         IF (NB_TOT < NWAN) THEN
            CALL vtutor%bug("internal error in VASP: LPRJ_CALC_WANNIER_TRAFO: NB_TOT should be " &
               // "larger or equal to NWAN", "locproj.F", 1047)
         ENDIF
...
! USE new_tutor
         ...
         COMPLEX(q),              INTENT(OUT) :: UK(:,:,:,:) ! (band, wannier idx, kpoint, spin)
...
! local
         ...
         INTEGER              :: WAN_IDX, BAND_IDX, IB
         COMPLEX(q), ALLOCATABLE    :: SVD_SIG(:), SVD_V(:, :), SVD_WD(:, :)
         INTEGER              :: INFO, LWORK
         INTEGER              :: NBANDS(2)
         COMPLEX(q)                 :: ASKWORK
         INTEGER, ALLOCATABLE :: IWORK(:)
         COMPLEX(q), ALLOCATABLE    :: WORK(:)
           ...
           CALL vtutor%bug("oops: the SVD in LPRJ_SVD_LOCALIZE is not ready for complex orbitals","locproj.F",1154)


         IF (.NOT. ALLOCATED(LPRJ_COVL)) THEN
            CALL vtutor%bug("LPRJ_COVL not allocated", "locproj.F", 1158)
         ENDIF
...
!> U_{nm}^k tranformation matrix between Bloch and Wannier functions (band, wannier idx, kpoint, spin)
        ...
        TYPE(in_struct),   INTENT(IN)  :: IO
! local
        COMPLEX(q), ALLOCATABLE :: M(:, :), U(:,:), VD(:,:), T1(:,:), T2(:,:)
        ...
        IF (NB_TOT < NWAN) THEN
         CALL vtutor%bug("internal error in VASP: LPRJ_CALC_WANNIER_TRAFO_QAO: NB_TOT should be " &
            // "larger than NWAN", "locproj.F", 1323)
         CALL vtutor%stopCode()
        ENDIF
...
! construct T1(a,m) = sum_n^NWAN U(a,n) VD(n,m) where m is from [1,NWAN-NOCC]
            ...
            T1 = 0
            CALL ZGEMM('N', 'N', NVIRT, NWAN-NOCC, NWAN, (1._q,0._q), &
                       U, NVIRT, VD, NWAN, (0._q,0._q), T1, NVIRT)
...
! VD = T1 * U
            CALL ZGEMM('C', 'N', NWAN-NOCC, NWAN, NVIRT, (1._q,0._q), &
                       T1, NVIRT, U, NVIRT, (0._q,0._q), VD, NWAN-NOCC)
...
! construct T2 = U VD
            ...
            CALL ZGEMM('N', 'N', NWAN, NWAN, NWAN, (1._q,0._q), &
                       U, NWAN, VD, NWAN, (0._q,0._q), T2, NWAN)
...
! missuse U and VD as temp/auxilary matrices
            ...
            U(:,:) = T2(NOCC+1:NWAN,:)
! VD = T1 * U
            CALL ZGEMM('N', 'N', NVIRT, NWAN, NWAN-NOCC, (1._q,0._q), &
                       T1, NVIRT, U, NWAN-NOCC, (0._q,0._q), VD, NVIRT)
...
! local
      COMPLEX(q), ALLOCATABLE      :: ROTYLM(:,:)
      COMPLEX(q), ALLOCATABLE      :: GHYBRID_ORBITAL(:)
...
!> Kohn-sham orbitals from which to build the IAOS
      ...
      COMPLEX(q), ALLOCATABLE, INTENT(IN) :: LPRJ_COVL(:,:,:,:)
!> Contains the projections of the Kohn-Sham orbitals onto IAOs
!> The dimensions are (NBANDS,NK,ISP,NPROJ)
      COMPLEX(q), ALLOCATABLE, INTENT(OUT) :: LPRJ_COVL_IAO(:,:,:,:)
...
! local
      ...
      COMPLEX(q), ALLOCATABLE           :: A(:,:)     ! overlap <mu|nu> of non-orthog. basis B2
      COMPLEX(q), ALLOCATABLE           :: S(:,:)     ! inverse overlap of non-orthog. basis B2
      COMPLEX(q), ALLOCATABLE           :: OVL_MUI(:,:,:)  ! overlap <mu|i>
      COMPLEX(q), ALLOCATABLE           :: OVL_JTMU(:,:,:) ! overlap <j^tilde|mu>
      COMPLEX(q), ALLOCATABLE           :: OVL_IAO(:,:,:) ! overlap <IAO|IAO>
      ...
      COMPLEX(q), ALLOCATABLE           :: CPROJ_RED(:,:)     ! tmp wave function array
      COMPLEX(q), ALLOCATABLE           :: CPROJ_RED_SUM(:,:) ! tmp wave function array
      TYPE(wavefuna)              :: WA
...
! gather results from all cores
      CALL M_sum_z(WDES_MU%COMM_INTER, A(1,1), SIZE(A))
...
! invert overlap matrix A, S=A^-1 (using SVD and pseudo-inverse)
      ...
      ENDIF
      S = CONJG(TRANSPOSE(SVD%V_DAGGER_MAT))
      DO MU = 1, NB2
        ...
        ENDIF
      ENDDO
      S = MATMUL(S,CONJG(TRANSPOSE(SVD%U_MAT)))
      CALL SVD%DEALLOC()
...
!                   S(:,:)                                                                    &
!                 ),                                                                          &
!                 TRANSPOSE(CONJG(LPRJ_COVL(NBLOCH_LOW(ISP):NBLOCH_HIGH(ISP), IK, ISP, :)))  &
!               )
!  TRACE = 0.0_q
!  DO I = 1, NTARGET(ISP)
...
! copy original bloch orbitals into this "target" wave function array
      DO ISP = 1, W%WDES%ISPIN
        W_TARGET%CPTWFP(:,1:NTARGET_LOCAL_MPIROUNDED(ISP),:,ISP)    = W%CPTWFP(:,NB_COPY_LOCAL_LOW(ISP):NB_COPY_LOCAL_HIGH(ISP),:,ISP)
...
! calc overlap <mu|i>
        ...
        ENDDO
! gather results from all cores
        CALL M_sum_z(WDESTARGET%COMM_INTER, OVL_MUI(:,:,ISP), SIZE(OVL_MUI(:,:,ISP)))
...
! calc <i^tilde|mu>
      ...
      ENDDO
! gather results from all cores
      CALL M_sum_z(WDESTARGET%COMM_INTER, OVL_JTMU(1,1,1), SIZE(OVL_JTMU))
      ...
      DO ISP = 1, WDESTARGET%ISPIN
        ...
        CPROJ_RED=(0._q,0._q)
        CPROJ_RED_SUM=(0._q,0._q)

        GWORK = (0._q,0._q)

! build <G|f1_mu> and store in CW_RED_SUM and CPROJ_RED_SUM
        GWORK(1:NTARGET(ISP),:) = CONJG(TRANSPOSE(OVL_MUI(:,1:NTARGET(ISP),ISP)))
        CALL SETWDES(WDESTARGET, WDESK, IK)
        WA_TARGET = ELEMENTS(W_TARGET, WDESK, ISP) 
...
! calc overlap <i|IAO> and store in LPRJ_COVL_IAO
      ...
      ENDDO
! gather results from all cores
      CALL M_sum_z(W%WDES%COMM_INTER, LPRJ_COVL_IAO(1,1,1,1), SIZE(LPRJ_COVL_IAO))
...
! calc and dump overlap matrix of IAOs
      ...
      ENDDO
! gather results from all cores
      CALL M_sum_z(W%WDES%COMM_INTER, OVL_IAO(1,1,1), SIZE(OVL_IAO))
...
! by calcualting sum_{i,j,mu} <i|mu><mu|j> while i,j is restricted to target
      DO ISP = 1, W%WDES%ISPIN
        ...
        DO MU = 1, NB2
          ...
          GWORK1(MU) = GWORK1(MU) * SUM(CONJG(LPRJ_COVL_IAO(NBLOCH_LOW(ISP):NBLOCH_HIGH(ISP),IK,ISP,MU)))
        ENDDO
...
!***********************************************************************
    SUBROUTINE LPRJ_LOEWDIN_ORTHONORMAL(W, WDES, ISPIN, NBSTART, NBSTOP, CQIJ, SOUT)
      ...
      INTEGER        :: NBSTOP(2)
      COMPLEX(q)        :: CQIJ(:,:,:,:)
      COMPLEX(q), OPTIONAL :: SOUT(:,:,:)   ! overlap matrix as optional output
! local
      INTEGER                     :: IK, ISP
      ...
      TYPE(wavefun1), ALLOCATABLE :: W1GATHER(:) ! for blocking
      TYPE(wavefuna)              :: WA
      COMPLEX(q), ALLOCATABLE           :: A(:,:)     ! overlap matrix
      COMPLEX(q), ALLOCATABLE           :: S(:,:)     ! inverse overlap
      INTEGER                     :: IERR
      REAL(q)                     :: WSCAL
      TYPE(SV_DECOMPOSER)         :: SVD
...
! gather all Ws into W1GATHER
      DO ISP = 1, ISPIN
        ...
        ENDDO
! gather results from all cores
        CALL M_sum_z(WDES%COMM_INTER, A(1,1), SIZE(A))
...
!***********************************************************************
      SUBROUTINE LPRJ_CALC_SVD(MODE, A, U, VD, SIGMA, IO)
         ...
         COMPLEX(q),                   INTENT(INOUT)  :: A(:, :) !! Content is destroyed on exit
         COMPLEX(q), ALLOCATABLE,      INTENT(OUT)    :: U(:, :)
         COMPLEX(q), ALLOCATABLE,      INTENT(OUT)    :: VD(:, :)
...
! Make sure everything is deallocated first.
         ...
         SELECT CASE (MODE)
            ...
            CASE DEFAULT
               CALL vtutor%bug("LPRJ_CALC_SVD: Mode '" // MODE // "' not implemented!", "locproj.F", 2484)
         END SELECT
...
!********************************************************************************************
      SUBROUTINE LPRJ_ASSIGN_ORBITALS_TO_SITES(LPRJ_COVL_IAO, IBLOW, IBHIGH, T_INFO, IO, UTRAFO_UP, UTRAFO_DOWN)
        ...
        USE base
        COMPLEX(q)              :: LPRJ_COVL_IAO(:,:,:,:)
        INTEGER           :: IBLOW(2)  ! for spin up&down
        INTEGER           :: IBHIGH(2) ! for spin up&down
        TYPE(type_info)   :: T_INFO
        TYPE(in_struct)   :: IO
        COMPLEX(q), OPTIONAL    :: UTRAFO_UP(:,:)
        COMPLEX(q), OPTIONAL    :: UTRAFO_DOWN(:,:)
! local
...
! get overlap matrix and apply trafo
          ...
          IF(ISP .EQ. 1) THEN
            IF(PRESENT(UTRAFO_UP)) THEN
              GOVL(IBLOW(1):IBHIGH(1),:) = MATMUL(CONJG(TRANSPOSE(UTRAFO_UP(:,:))), GOVL(IBLOW(1):IBHIGH(1),:))
            ENDIF
          ELSE
            IF(PRESENT(UTRAFO_DOWN)) THEN
              GOVL(IBLOW(2):IBHIGH(2),:) = MATMUL(CONJG(TRANSPOSE(UTRAFO_DOWN(:,:))), GOVL(IBLOW(2):IBHIGH(2),:))
            ENDIF
          ENDIF
...
!***********************************************************************
    ...
    USE vhdf5, ONLY: IH5OUTFILEID, vh5_write_locproj
...
! same site as before
               ...
               kpoints1: DO IK=1,NK
                  ...
                  DO IB=1,NB
                     ...
                     DO IFNC=IBASE,NF
                        IF (LDONOW(IFNC)) &
                        WRITE(99,'(2X,F6.3,X,F6.3)',ADVANCE='No') CONJG(LPRJ_COVL(IB,IK,ISP,IFNC))
                     ENDDO
                     WRITE(99,*)
                  ENDDO
         ...
         spin2: DO ISP=1,NS
            kpoints2: DO IK=1,NK
               ...
               bands2: DO IB=1,NB
                  ...
                  funcs2: DO IFNC=1,NF
                     WRITE(99,'(1I6,2F20.10)') IFNC,CONJG(LPRJ_COVL(IB,IK,ISP,IFNC))
                  ENDDO funcs2
                  WRITE(99,*)
               ENDDO bands2
            ENDDO kpoints2
...
# 2962
      ...
      CALL VH5_WRITE_LOCPROJ(IH5OUTFILEID,LPRJ_functions,LPRJ_COVL, W%WDES%LNONCOLLINEAR)
      CALL M_barrier(W%WDES%COMM)
...
! local
      ...
      REAL(q) DR
      REAL(q), PARAMETER :: TINYR=1.E-4_q
      COMPLEX(q), ALLOCATABLE :: DENSITY_MATRIX(:,:,:), U(:,:,:), GAMMA(:,:)
...
! now work on (1._q,0._q) site only
      U=0
      IFNC=1
      DO
         DO I=IFNC,NF
...
!! set to (0._q,0._q), the overlap operator is taken to be the identity
!! operator, S=1.
!
!***********************************************************************
...
# 3239
         ...
         ENDIF
      ENDIF
      CALL NEWWAVA(WRYLM,WDESK,NYLM); WRYLM%CPTWFP=0
...
!> local functions are stored in WRYLM%CPTWFP, and the projections onto
!> the PAW projectors in WRYLM%CPROJ.
!>
!> This is 1._q for a set of local functions with l = [LSTART,LSTOP],
...
! get me the Bessel transform of the radial function
         ...
         DO M=1,2*L+1
            DO ISPINOR_=1,WRYLM%WDES1%NRSPINORS
               DO IND=1,NPL
                  WRYLM%CPTWFP(IND+(ISPINOR_-1)*NPL,LM+M-1)=ZVR(ISPINOR_,ISPINOR)*FAKT*(CSET**L)*CFAKTX(IND)*FG(IND)*YLM(IND,LMBASE+M-1)
               ENDDO ! plane-waves
            ENDDO ! spinor
...
! PSPNL_SPLINE = 0.0_q ! Breaks mlwf_mos2_soc_locproj (intel), uninitialized values enter in loop in LPRJ_GEN_TRIALORBITALS.
...
! radial grid to Bohr here is strange.
      ...
      SELECT CASE(ITYP)
       ...
       CASE DEFAULT
         call vtutor%bug('RADIAL_HYDROGEN_FUNCTION: TYPE '//str(ITYP)//'does not exist',"locproj.F",3643)
      END SELECT
...
! Slater-type (according to Slater's rule)
      ...
      SELECT CASE(ITYP)
         ...
         CASE(6)
           NSTAR = 4.2_q
         CASE DEFAULT
           call vtutor%bug('RADIAL_SLATER_FUNCTION: TYPE '//str(ITYP)//'does not exist',"locproj.F",3679)
      END SELECT
...
! calc <r> (not to be confused with <vec r> which is (0._q,0._q))
...
!***********************************************************************
      SUBROUTINE RGRID_PARAMETERS(RSTART,REND,H)
       REAL(q) :: RSTART !< starting radius (should avoid (0._q,0._q))
...
!***********************************************************************
      SUBROUTINE SETROTYLM( XIN,ZIN,LMAX,ROTYLM )
         ...
         COMPLEX(q),INTENT(OUT) :: ROTYLM(:,:) !< Ylm rotation matrix
! local variables
         INTEGER LMMAX
         REAL(q) XN,ZN,XDOTZ
