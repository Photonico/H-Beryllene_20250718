...
# 2 "sphpro.F" 2 
      ...
      INTEGER, PARAMETER :: FINAL_STEP = 0, INNER_STEP = -1
...
! write header fo file PROOUT
      IF (LORBIT==5) THEN
         DO ISP=1,WDES%ISPIN+WDES%NRSPINORS-1
            WRITE(FILENAME,'(A,I1)') "PROOUT"
            IF (WDES%ISPIN==2.OR.WDES%NRSPINORS==2) WRITE(FILENAME,'(A,I1)') "PROOUT.",ISP
            OPEN(UNIT=IUP+ISP-1,FILE=DIR_APP(1:DIR_LEN)//FILENAME,STATUS='UNKNOWN')
         ENDDO
         DO ISP=1,WDES%ISPIN+WDES%NRSPINORS-1
...
! compute the first LTRUNC zeros of the bessel function
      ...
      DO I=1,LTRUNC
        ...
        QR=QZERO(I)
! compute the value of the bessel function BQ  = j_l(qr)
!                       and its derivative BQP = j'_l(qr)
! with QR = qr and LL = l
...
!
! new calculate phas(lm,alp) =
! sum_n < phi(alp) | beta lmn > abs(< phi(alp) | beta lmn >)
!
      LMS=0
      DO LL=0,LDIMP-1
         DO I=1,LTRUNC
...
! for the non collinear case
      ...
      IF (LORBIT==5) THEN
! we want to have real and imaginary part of < Ylm | phi_alp > where alp denotes the spinor component
! in CSUM_ABS    we have   rho(lm,alp,bet) = sum_n < phi(alp) | beta lmn >     < beta lmn | phi(bet) >
! in CSUM_PHASE  we have   phase(lm,alp)   = sum_n < phi(alp) | beta lmn > abs(< beta lmn | phi(alp) >)
!
! we can get the value of sum_n < phi(alp) | beta lmn > from
! sum_n < phi(alp) | beta lmn > = phase(lm,alp)/sqrt(rho(lm,alp,alph))
         IF (WDES%LNONCOLLINEAR) THEN
            PHAS(:,NI,NK,WDES%NB_LOW+(NB-1)*WDES%NB_PAR,1)=CSUM_PHASE(:,1)/SQRT(ABS(CSUM_ABS(:,1)))
            PHAS(:,NI,NK,WDES%NB_LOW+(NB-1)*WDES%NB_PAR,2)=CSUM_PHASE(:,2)/SQRT(ABS(CSUM_ABS(:,4)))
         ELSE
            PHAS(:,NI,NK,WDES%NB_LOW+(NB-1)*WDES%NB_PAR,1:WDES%ISPIN)=&
                CSUM_PHASE(:,1:WDES%ISPIN)/SQRT(ABS(CSUM_ABS(:,1:WDES%ISPIN)))
         ENDIF
      ENDIF
...
! compute rho^{ss'}_{ij,nk} = <\tilde{psi}^s_nk|p_j> Q_ij <p_i|\tilde{psi}^s'_nk>
         CTMP=W%CPROJ(LMIND,N,NK,ISP)*CONJG(W%CPROJ(LMIND_,N,NK,ISP))*QIJ

! accumulate rho^{ss'}_{nk} = sum_ij rho^{ss'}_{ij,nk}
         CSUM_ABS(LL*LL+M+1,II)=CSUM_ABS(LL*LL+M+1,II)+CTMP

! Store <\tilde{psi}^s_nk|p_j> Q_ij <p_i|\tilde{psi}^s_nk>
         IF(ISPINOR==ISPINOR_) THEN
            CSUM_PHASE(LL*LL+M+1,ISPINOR+ISP)=CSUM_PHASE(LL*LL+M+1,ISPINOR+ISP)+CTMP
         ENDIF
...
! Orbital moment: stop
      ...
      IF (LORBIT==5) THEN
         DO M=0,MMAX-1
            PHAS(LL*LL+M+1,NI,NK,WDES%NB_LOW+(N-1)*WDES%NB_PAR,1:WDES%NRSPINORS+WDES%ISPIN-1)=CSUM_PHASE(LL*LL+M+1,1:WDES%NRSPINORS+WDES%ISPIN-1)
         ENDDO
      ENDIF
...
!=======================================================================
      IF (LORBIT/=5) THEN
         ...
         ELSE
            CALL VH5_WRITE_PROJECTORS(IH5OUTFILEID, T_INFO, WDES, W, LORBIT, LPAR, PAR, LCHAR, PHAS)
         END IF
...
!***********************************************************************
      ...
      SUBROUTINE SPHPRO_FAST( &
          ...
          LOVERL,LMDIM,CQIJ,LPAR,LDIMP,LMDIMP,NSTEP,LORBIT,PAR, &
          EFERMI, EMIN, EMAX, SYMM, LACUMULATE, PHAS_OUT)
...
!! USE moffload
      ...
      LOGICAL, OPTIONAL :: LACUMULATE
      COMPLEX(q),ALLOCATABLE, OPTIONAL :: PHAS_OUT(:,:,:,:,:)

      LOGICAL LOVERL,LFINAL
      LOGICAL MY_LACUMULATE
      INTEGER LORBIT,NSTEP
...
! CSUM = (0.0_q, 0.0_q) ! Breaks bulk_CoO_wurzite_SOC_PBE0_nosym (gnu), used partially uninitialized in C_FLIP.

      LFINAL = NSTEP == FINAL_STEP
      MY_LACUMULATE = LFINAL; IF (PRESENT(LACUMULATE)) MY_LACUMULATE=LACUMULATE
...
!   write header fo file PROCAR and
      ...
      IF (LORBIT==14) THEN
         IF (.NOT. PRESENT(EFERMI) .OR. .NOT. PRESENT(EMIN) .OR. .NOT. PRESENT(EMAX)) THEN
            CALL vtutor%bug("internal error in SPHPRO_FAST: optional arguments are missing", "sphpro.F", 1175)
         ENDIF
      ENDIF

      IF (LFINAL.OR.PRESENT(PHAS_OUT)) THEN
         PAR=0
...
! diagonalize the AE density matrix QTOT= <psi_i | psi_j>
      ...
      IF (N_CHANNELS > NDMAT) THEN
         CALL vtutor%bug("internal error in VASP: SPHPRO_FAST does not support more then 4 channels " &
            // "per L quantum number", "sphpro.F", 1227)
      ENDIF
...
! so that the charge is simply give by projection onto
      CALL DIAG_AND_SORT( EV, N_CHANNELS, NDMAT )
      ...
      IF (LFINAL .AND. (LORBIT>=11 .AND. LORBIT<=14) .AND. LM > LPAR) THEN
         CALL vtutor%bug("internal ERROR: LPAR is too small in SPHPRO_FAST (LM) " // str(LM) // " " &
            // str(LPAR), "sphpro.F", 1242)
      ELSE IF (LFINAL .AND. LL+1 > LPAR) THEN
         CALL vtutor%bug("internal ERROR: LPAR is too small in SPHPRO_FAST (LL) " // str(LL+1) // &
            " " // str(LPAR), "sphpro.F", 1245)
      ENDIF
      ...
      DO NB=1,WDES%NB_TOT
         ...
         IF (LFINAL.OR.MY_LACUMULATE) THEN
...
! set occupancy matrix for this L quantum number to 0
         ...
         DO NB=1,WDES%NB_TOT
            ...
            DO M=1,MMAX
               LM=LL*LL+M
! project onto four sqrt(eigenvalue) eigenvectors of QTOT (possibly just (0._q,0._q))
...
! sum OCC over bands
         CALL M_sum_d( WDES%COMM_INTER, OCC, SIZE(OCC))
         CALL M_sum_d( WDES%COMM_KINTER, OCC, SIZE(OCC))

! diagonalize OCC and store largest eigenvalue, specifically OCC(:,:) VEC(:)
         CALL DIAG_OCC(NDMAT, N_CHANNELS, OCC, VEC)
...
!=======================================================================
      IF ((LORBIT>=12 .AND. LORBIT<=13) .AND. (LFINAL .OR. PRESENT(PHAS_OUT))) THEN
         DO ISP=1,WDES%ISPIN
         DO II=0,WDES%NRSPINORS-1
         DO NK=1,WDES%NKPTS
...
! hard coded to avoid another deep do loop
            ...
            VEC=0
            IF (LORBIT==12) THEN
! pick projector that yields the largest (1._q,0._q)-center all-electron density
! for this orbital
...
!-----------------------------------------------------------------------
      ...
      IF (LFINAL) THEN
        CALL M_sum_d8( WDES%COMM, PAR(1,1,1,1,1),SIZE(PAR,KIND=qi8))
        IF (LORBIT>=12 .AND. LORBIT<=14) THEN
           CALL M_sum_d8( WDES%COMM, PHAS(1,1,1,1,1),SIZE(PHAS,KIND=qi8)*2_qi8)
        ENDIF
      ENDIF
         ...
         IF (SYMM%ISYM>=0) THEN
            DO ISP=1,WDES%ISPIN
               CALL PARSYL(LDIMP-1,T_INFO%NIONS,T_INFO%NIOND,T_INFO%NTYP,T_INFO%NITYP, &
                           ION_SUM(1,1,ISP),SYMM%ROTMAP(1,1,1),SYMM%MAGROT(1,1),ISP)
            ENDDO
            IF (WDES%LNONCOLLINEAR) &
...
!=======================================================================
      IF (LORBIT==14) THEN
         ...
         CALL M_sum_i( WDES%COMM, NSET_OPTPROJ, SIZE(NSET_OPTPROJ))
         DO NT=1,T_INFO%NTYP
! TODO use of PP_POINTER here
            PP=>P(NT)
            CALL M_sum_d( WDES%COMM, PP%OPTPROJ, SIZE(PP%OPTPROJ))
            LOW=1
            DO
               LL=PP%LPS(LOW)
               DO LHI=LOW,PP%LMAX
...
!=======================================================================
      ...
      DO NI=1,T_INFO%NIONP
         IF (LORBIT==11.OR.LORBIT==12.OR.LORBIT==13.OR.LORBIT==14) THEN
            DO NL=1,LPAR
               LMTABLE(NL,NI)=LMCHAR(NL)
            ENDDO
         ELSE
...
!=======================================================================
      CALL WRITE_CONDENSED_INFORMATION(ION_SUM, IU6)

      IF (NSTEP > 0) CALL VH5_WRITE_MOMENTS_HIST_STEP( &
          IH5INTERMEDIATEGROUP_ID,GRP_HISTORY,NSTEP,ION_SUM,"spin")
...
!> Write projections to PROCAR file
      SUBROUTINE WRITE_PROCAR(IUP, W, WDES, LORBIT, LPAR, LMDIMP, PAR, PHAS, T_INFO, EMIN, EMAX)
        ...
        REAL(q) :: EMIN, EMAX
! local variables
        INTEGER :: II, ISP, NK, NL, NI, NB, M
        REAL(q) :: PARSUM, S
        REAL(q) SUMION(LPAR),SUMTOT(LPAR)

!write header of PROCAR file
        IF (LORBIT==1.OR.LORBIT==11) THEN
           WRITE(IUP,'(A)')'PROCAR lm decomposed'
        ELSE  IF (LORBIT==2.OR.LORBIT==12) THEN
           WRITE(IUP,'(A)')'PROCAR lm decomposed + phase'
        ELSE  IF (LORBIT==13) THEN
           WRITE(IUP,'(A)')'PROCAR lm decomposed + phase; optimize projector for each state'
        ELSE  IF (LORBIT==14) THEN
        ...
        DO ISP=1,WDES%ISPIN
          ...
          DO NK=1,WDES%NKPTS
            ...
            DO NB=1,WDES%NB_TOT
              ...
              WRITE(IUP,'(A3)',ADVANCE='No') "ion"
              IF (LORBIT>=11 .AND. LORBIT<=14 .OR. LORBIT==1 .OR. LORBIT==2) THEN
                 DO NL=1,LPAR
                    WRITE(IUP,'(A7)',ADVANCE='No') LMCHAR(NL)
                 ENDDO
              ELSE
...
!            WRITE(IUP,3204) NI, (REAL(PHAS(M,NI,NK,NB,ISP)*CONJG(PHAS(M,NI,NK,NB,ISP))),M=1,LMDIMP), &
 ...
 3215 FORMAT('charge',17(F6.3,1X,'      ',2X)/)
      ...
      END SUBROUTINE WRITE_PROCAR

!> Write magnetization data to output
      SUBROUTINE WRITE_CONDENSED_INFORMATION(ION_SUM, IU6)

      USE string, ONLY: PARSENEWLINE

      IMPLICIT NONE

      REAL(q), INTENT(IN) :: ION_SUM(:,:,:)
      INTEGER, INTENT(IN) :: IU6
      INTEGER ISP, ION, LDIMP, NIONS, NCDIJ
      REAL(q), ALLOCATABLE :: TOT_ION_SUM(:,:)
      CHARACTER(LEN=:), ALLOCATABLE :: TITLE, TABLE_HEAD

      IF (IU6 < 0) RETURN

      LDIMP = SIZE(ION_SUM, 1)
      NIONS = SIZE(ION_SUM, 2)
      NCDIJ = SIZE(ION_SUM, 3)
      ALLOCATE(TOT_ION_SUM(LDIMP, NCDIJ))
      TOT_ION_SUM = SUM(ION_SUM, DIM=2)

      IF (LDIMP /= 4) THEN
         TABLE_HEAD = PARSENEWLINE('# of ion       s       p       d       tot\n&
                                   &------------------------------------------')
      ELSE
         TABLE_HEAD = PARSENEWLINE('# of ion       s       p       d       f       tot\n&
                                   &--------------------------------------------------')
      END IF
      ...
      DO ISP = 1, NCDIJ
         ...
         CASE (1)
            TITLE = ' total charge'
         CASE (2)
            TITLE = ' magnetization (x)'
         CASE (3)
            TITLE = ' magnetization (y)'
         CASE (4)
            TITLE = ' magnetization (z)'
         END SELECT

         WRITE(IU6, '(A)') REPEAT(NEW_LINE('n'), 3) // TITLE // NEW_LINE('n')
         WRITE(IU6, '(A)') TABLE_HEAD
         DO ION = 1, NIONS
            WRITE(IU6, '(I5,5X,5(F8.3))') ION, ION_SUM(:,ION,ISP), SUM(ION_SUM(:,ION,ISP))
         END DO
         IF (NIONS > 1) THEN
            WRITE(IU6, '(A)') '--------------------------------------------------'
            WRITE(IU6, '(A3,7X,5(F8.3))') 'tot', TOT_ION_SUM(:,ISP), SUM(TOT_ION_SUM(:,ISP))
         END IF
      END DO

      END SUBROUTINE WRITE_CONDENSED_INFORMATION
...
!********************************************************************
     ...
     SUBROUTINE DIAG_AND_SORT(EV, N_CHANNELS, NDMAT )
       IMPLICIT NONE
       INTEGER :: N_CHANNELS, NDMAT
       REAL(q) :: EV(:,:)
! local
       REAL(q) :: R(NDMAT)
...
!*******************************************************************
!
!> Small helper routine to diagonalize an (1._q,0._q)-center occupancy
!> matrix and yield a new projector that yields (when
!> projected onto it) a maximal occupancy.
!
!********************************************************************

     SUBROUTINE DIAG_OCC( NDMAT, N_CHANNELS, OCC, VEC )
       IMPLICIT NONE
       INTEGER :: NDMAT, N_CHANNELS
...
!********************************************************************
!> @brief Compute optimal projectors in the energy interval.
!> This was originally coded inside SPHPRO_FAST however the optimal projectors
!> might be used elsewhere (for example in LOCPROJ) so we create a separate routine.
!********************************************************************
     SUBROUTINE SPHPRO_OPTPROJ( &
          P,T_INFO,W,WDES,&
          LDIMP,LMDIMP, IU6, &
          EMIN, EMAX)
      USE prec
...
! Local variables
      ...
      REAL(q) :: EV(NDMAT,NDMAT), OCC(NDMAT,NDMAT)
      INTEGER :: NSET_OPTPROJ(T_INFO%NTYP, LDIMP)
      INTEGER :: NI,NT,NIS,NB,NB_,NK,I,II,ISP
      INTEGER :: J,L,LHI,LL,LM,LMBASE,LOW,NIP,N_CHANNELS,M,MMAX
...
!=======================================================================
      ...
      IF (N_CHANNELS > NDMAT) THEN
         CALL vtutor%bug("internal error in VASP: SPHPRO_FAST does not support more then 4 channels " &
            // "per L quantum number", "sphpro.F", 2001)
      ENDIF
      EV(1:N_CHANNELS,1:N_CHANNELS)=PP%QTOT(LOW:LHI,LOW:LHI)
! determine QTOT =  O+ lambda O;  EV = lambda^(1/2) O
! so that the charge is simply give by projection onto
...
!***********************************************************************
      ...
      END SUBROUTINE SPHPRO_DESCRIPTION
