...
!***********************************************************************
!
!> Implements fast multipole corrections to the
!> non-local exchange
!>
...
!
!> Determines corrections potentials
!
!**********************************************************************
       ...
       DO N=1,10
          DO NG=1,10
             ...
             ELSE IF (.NOT.GRIDHF%REAL2CPLX) THEN
                ...
                PROJ_WEIGHT=FACTM
             ELSE 
                CALL vtutor%bug("internal error in FOCK_MULTIPOLE_CORR: currently this storage " &
                   // "layout is not supported", "fock_multipole.F", 240)
             END IF
...
!
!> Determines corrections projectors (ACFDT)
!
!**********************************************************************
...
!   should not really happen:
    IF(POTFAK0<0.0) THEN
       CALL vtutor%bug("internal error in GW_MULTIPOLE_PROJ_SETUP: PF0<0 - NEED TO THINK ABOUT THIS!", "fock_multipole.F", 881)
    ELSE
...
!=======================================================================
    ...
    IF (IFAIL/=0) THEN
       CALL vtutor%bug("internal ERROR in ROTINV(C): Call to routine ZHEEV failed! Error code was " &
          // str(IFAIL), "fock_multipole.F", 965)
    ENDIF
    
    DO N1=1,NBANDS
       IF(FB==1) THEN
          TMPDAT=CMPLX(1.0/HFEIG(N1),0.0_Q,q)
       ELSE
          TMPDAT=CMPLX(HFEIG(N1),0.0_Q,q)
       ENDIF
       IF(FA==1) THEN
          TMPDAT=SQRT(TMPDAT)
       ENDIF
...
! multiply by 4 pi e^2/ G^2
       ...
       DO N=1,10
          DO NG=1,10
             ...
             ELSE 
                CALL vtutor%bug("internal error in FOCK_MULTIPOLE_CORR: currently this storage " &
                   // "layout is not supported", "fock_multipole.F", 1212)
             END IF
          ENDDO
       ENDDO
...
!
!>  Multiplies the charge density by the potential kernel
...
# 1295
     ...
     TSUM=0
!$ACC ENTER DATA COPYIN(TSUM,P_PROJ2,PROJ_WEIGHT) 
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(TSUM,CWORK,P_PROJ2,PROJ_WEIGHT) 
     DO N=1,10
        DO NP=1,GRID%RC%NP
...
# 1307
        ...
        ENDDO
     ENDDO
!$ACC EXIT DATA COPYOUT(TSUM) DELETE(PROJ_WEIGHT) 

     POTCORRECTION=MATMUL(MQ_MAT,TSUM)

!$ACC ENTER DATA COPYIN(POTCORRECTION) 
!$ACC PARALLEL LOOP PRESENT(CWORK,POTFAK) 
     DO NP=1,GRID%RC%NP
        CWORK(NP)=POTFAK(NP)*CWORK(NP)
     ENDDO

!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CWORK,P_PROJ2,POTCORRECTION) 
     DO N=1,10
        DO NP=1,GRID%RC%NP
...
# 1327
        ...
        ENDDO
     ENDDO
!$ACC EXIT DATA DELETE(POTCORRECTION,P_PROJ2) 
