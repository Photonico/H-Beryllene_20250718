...
!***********************************************************************
!
!> Provides subroutines to evaluate the Berry-phase
...
!
!> Finds points within cutoff around ion, computes spherical harmonics
!>
...
!***********************************************************************
     ...
     &  LMYDIM,LYDIM,YLM,IRMAX,INDMAX,DISX,DISY,DISZ,XS,YS,ZS,DIST)
      ...
      IF ((LYDIM+1)**2 > LMYDIM) THEN
         CALL vtutor%bug("internal error: LMYDIM is too small " // str(LYDIM) // " " // str(LMYDIM), "elpol.F", 69)
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      IF (GRID%RL%I2(NCOL) /= N1P+1 .OR. GRID%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("internal ERROR SETYLM: " // str(N1P+1) // " " // str(N2P+1) // " " // &
           ...
           str(GRID%RL%I3(NCOL)) // " " // str(N2P), "elpol.F", 116)
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      IF (INDMAX>IRMAX) THEN
        CALL vtutor%bug("internal ERROR: DEPLE: IRDMAX must be increased to " // str(INT(INDMAX*1.1)), "elpol.F", 197)
      ENDIF
...
!************************ SUBROUTINE SETd_IJ ***************************
!
!> Calculates the dipole moment of the augmentation charges
!>
!> According to eq.2 of ref.1 (see elpol module description).
!
!***********************************************************************
# 222
...
!************************ SUBROUTINE SET_EV ****************************
!
!> Calculates the "expectation-value" (EV) term
!>
!> Appears in eq.19 of ref.1 (see elpol module description), for a specific
!> k-point, i.e.
!> ~~~
...
!PK Probably skipping non-owned k-points is sufficient
            CALL vtutor%error("SET_EV: KPAR>1 not implemented, sorry.")
         END IF
...
!
!> Calculates the product of e^iGr (or cos(Gr), or sin(Gr))
...
!>
!> These matrix elements are calculated:
!> ~~~
...
!PK All the Berry phase service routines need updating/checking too
         CALL vtutor%error("MNMAT: KPAR>1 not implemented, sorry.")
      END IF
# 902
...
!  Mind that all wavefunctions are redistributed at this point
      IF (NDONE/=NBANDS .OR. NPOS_RED+NSTRIP_RED/=NB_TOT+1 ) THEN
       CALL vtutor%bug("EDDIAG: fatal internal error (2): " // str(NDONE) // " " // str(NBANDS) // &
          " " // str(NPOS_RED+NSTRIP_RED) // " " // str(NB_TOT+1), "elpol.F", 1062)
      ENDIF
...
!
!> Reads LBERRY, IGPAR and NSTR from INCAR
!>
...
! Calculate Berry-Phase? Default -> .FALSE.
      LBERRY=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LBERRY', LBERRY, IERR, WRITEXMLINCAR)
!
      IF (LBERRY) THEN
...
!
!> Writes the parameters of the berry phase routine
!
!***********************************************************************
...
!
! Writes LBERRY, IGPAR and NPPSTR to OUTCAR
!
!***********************************************************************
...
!>   "Berry phase" contributions to the polarization (see ref.1 in elpol
!>   module description). (Type: grep '<R>' OUTCAR)
!>
...
!=======================================================================
      ...
      IF (IUNOUT>=0) THEN
         ...
         CALL VH5_WRITE_DIPOLE_MOMENT(IH5OUTFILEID, "ion", DIP_ION)
...
!
 1000 CONTINUE   ! Case IFAIL<>0
      DEALLOCATE(KPTSTR)
      IF (IFAIL < 0) THEN
          CALL vtutor%error('Error in subroutine BERRY: did not find all determinants. &
              &Matrix CMK is singular to working precision for ISTR = ' // str(ISTR) // &
              ' j = ' // str(IK-1))
      ELSE
          CALL vtutor%error('Error in subroutine BERRY: did not find all determinants. &
              &Matrix CMK is not an nxn matrix for ISTR = ' // str(ISTR) // ' j = ' // &
              str(IK-1))
      END IF

 1001 CONTINUE   ! Case IK<>NPPSTR
      DEALLOCATE(KPTSTR)
      CALL vtutor%error('Error in subroutine BERRY: problem with k-mesh. NKPTS = ' // &
          str(KPOINTS%NKPTS) // ' NSTR = ' // str(NSTR) // ' NPPSTR = ' // str(NPPSTR))
...
!************************************************************************
!
!> Read KPOINTS for calculating polarization
!>
!> Read UNIT=14: KPOINTS and generate the k-points for calculating polarization
!> in direction `IGPAR`.
!> This reader supports both manual and automatic k-point
!> generations.
!> The gambler mode A should not be selected (the generating
!> vectors must be parallel to the reciprocal lattice vectors).
!
!************************************************************************
...
!-----k-points
      ...
      IF (KPOINTS%NKPTS>KPOINTS%NKDIM) THEN
        CALL vtutor%error("ERROR in RD_KPOINTS (mkpoints.F): increase NKDIMD to " // str(KPOINTS%NKPTS))
      ENDIF
...
! Read in a given set of arbitrary k-points:
     ...
     &  WTKPT(NKP)
         ...
         IF (WSUM==0) THEN
            CALL vtutor%error("ERROR: sum of weights is zero")
         ENDIF
...
! Find all irreducible points in the first Brillouin (1._q,0._q) ... :
         CALL IBZKPT(LATT_CUR%B,BK,SHIFT,KPOINTS%NKPTS,VKPT,WTKPT, &
              KPOINTS%LTET,KPOINTS%LKMAP,KPOINTS%NTET,IDTET,KPOINTS%FBZ2IBZ,KPOINTS%INVMAP,KPOINTS%VOLWGT, &
              LATT_CUR%SCALE,LINVERSION,.FALSE.,.FALSE.,.FALSE.,IU6)
...
! set old k-points
      GOTO 70222
70111 CONTINUE
      CALL vtutor%error("Error reading file KPOINTS. Stopping execution.")
70222 CONTINUE
# 1884
