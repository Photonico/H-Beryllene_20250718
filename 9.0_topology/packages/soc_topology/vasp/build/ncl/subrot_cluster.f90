...
!>
!> interval determined by DEGENERACY_THRESHOLD
...
!> unitary rotation matrix
     COMPLEX(q),POINTER :: U(:,:)
!> commulative rotation matrix
     COMPLEX(q),POINTER :: U_COMMULATIVE(:,:)
!> pointer to next cluster
     TYPE(eigenf_cluster), POINTER :: next
  END TYPE eigenf_cluster

!> we need an array of pointers with dimension WDES%NKPTS,WDES%ISPIN
!> for structures of the type eigenf_cluster
...
!> This parameter plays a rather subtle role
...
!> For the complex version, a complex shift of the eigenvalues is implemented
!> this is more robust than the treatment based on eigenvalue degeneracy
!> for metals though we still need to rotate degenerate states at Fermi-level
!> to get correct first order response of partial occupancies
  REAL(q) :: DEGENERACY_THRESHOLD=2E-3_q

  INTEGER :: RESOLVE_DEG_NTIMES=2
!> threshhold whether orbital is partially occupied or not
  REAL(q), PARAMETER :: SUBROT_CLUSTER_EMPTY_THRESHHOLD=1E-5_q
...
!***********************************************************************
  ...
  SUBROUTINE FIND_DEG_CLUSTERS(WDES, W, DEG_CLUSTER, THRESHOLD, LDISTRIBUTED_IN, LPARTIAL_IN )
    USE wave_mpi
    USE wave
...
! local
    ...
    LPARTIAL=.FALSE.
    IF (PRESENT(LPARTIAL_IN)) THEN
       LPARTIAL=LPARTIAL_IN
    ENDIF
    ...
    IF (SIZE(DEG_CLUSTER,1) /= WDES%NKPTS) THEN
       CALL vtutor%bug("internal error in FIND_DEG_CLUSTERS: dimension one wrong " // &
          str(SIZE(DEG_CLUSTER,1)) // " " // str(WDES%NKPTS), "subrot_cluster.F", 117)
    ENDIF
    IF (SIZE(DEG_CLUSTER,2) /= WDES%ISPIN) THEN
       CALL vtutor%bug("internal error in FIND_DEG_CLUSTERS: dimension two wrong " // &
          str(SIZE(DEG_CLUSTER,2)) // " " // str(WDES%ISPIN), "subrot_cluster.F", 121)
    ENDIF
...
! cluster only partially occupied orbitals
                IF (LPARTIAL) THEN
                   IF (ABS(W%FERTOT(NB_END,NK,ISP)-1.0_q)<SUBROT_CLUSTER_EMPTY_THRESHHOLD .OR. &
                       ABS(W%FERTOT(NB_END,NK,ISP))<SUBROT_CLUSTER_EMPTY_THRESHHOLD) EXIT
                ENDIF
...
# 148
                ...
                IF (.NOT.ASSOCIATED(DEG_CLUSTER(NK,ISP)%DEG_CLUSTER)) THEN
                   CALL vtutor%bug("internal error in FIND_DEG_CLUSTERS: association failed", "subrot_cluster.F", 150)
                ENDIF
...
! work arrays for unitary transformation
    ...
    IF (NBANDS /= SIZE(CHAM,1)) THEN
       CALL vtutor%bug("internal error in SETUP_DEG_CLUSTER: CHAM is not conform", "subrot_cluster.F", 307)
    ENDIF
    DEG_CLUSTER=>DEG_CLUSTER_BASE

! find number of states in the largest degenerate subspace
    NMAX_DEG=0
    DO
       ...
       IF (NB_TOT>NMAX_DEG) NMAX_DEG=NB_TOT

! go to the next link the in linked list of degenerate clusters
...
! start again from the begining of the linked list
    DEG_CLUSTER=>DEG_CLUSTER_BASE

!$ACC ENTER DATA CREATE(U,R,CWRK,RWORK) 
    DO
       IF (.NOT.ASSOCIATED(DEG_CLUSTER)) EXIT
       NB_START=DEG_CLUSTER%BAND_START
       NB_END=DEG_CLUSTER%BAND_END
...
!$ACC PARALLEL LOOP PRESENT(U,CHAM) 
       DO N=1,NB_TOT
          DO NP=1,NB_TOT
             U(N,NP)=CHAM(NB_START+N-1,NB_START+NP-1)
          ENDDO
       ENDDO
!$ACC PARALLEL LOOP PRESENT(U,CHAM_DIAGONAL) 
       DO N=1,NB_TOT
...
!$ACC UPDATE SELF(U) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)

       IF (IFAIL/=0) THEN
          CALL vtutor%error("ERROR in SETUP_DEG_CLUSTERS: call to ZHEEV/DSYEV failed! error code was &
             &" // str(IFAIL))
       ENDIF
# 399
       ...
       IF (SIZE(DEG_CLUSTER%U,1)/=NB_TOT) THEN
          CALL vtutor%bug("internal ERROR in SETUP_DEG_CLUSTERS: incorrect allocation", "subrot_cluster.F", 402)
       ENDIF
...
!$ACC EXIT DATA DELETE(U,R,CWRK,RWORK) 
    DEALLOCATE(U)
    DEALLOCATE(CWRK)
    DEALLOCATE(RWORK)
    DEALLOCATE(R)
...
! local
    INTEGER :: NB_START, NB_END, NB_TOT
! work arrays for unitary transformation
    LOGICAL :: LZERO  ! small helper variable to determine whether entries have been zeroed
...
!***********************************************************************
  ...
  SUBROUTINE SUBROT_DEG_ALL(WDES, W, LCONJG, LCOMMULATIVE, DEG_CLUSTER )
      ...
      TYPE (wavedes)     WDES
      TYPE (wavespin)    W
!> use the conjugation of the stored matrix
      LOGICAL LCONJG
!> use the commulative matrix transform
...
!$ACC ENTER DATA COPYIN(DEG_CLUSTER%U_COMMULATIVE) 
       IF (.NOT. LCONJG) THEN
       ...
       CALL LINCOM('C',CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:),DEG_CLUSTER%U_COMMULATIVE(:,:), &
             ...
             CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:))
       ENDIF
!$ACC EXIT DATA DELETE(DEG_CLUSTER%U_COMMULATIVE) 
       ELSE
!$ACC ENTER DATA COPYIN(DEG_CLUSTER%U) 
       IF (.NOT. LCONJG) THEN
       ...
       CALL LINCOM('C',CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:),DEG_CLUSTER%U(:,:), &
             ...
             CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:))
       ENDIF
!$ACC EXIT DATA DELETE(DEG_CLUSTER%U) 
       ENDIF
