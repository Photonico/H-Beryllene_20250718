...
!****************************************************************************************************
       ...
       SUBROUTINE DEF_ELEMENT_ORDER_FF (AB_HELP,TIM,FF,FFM,TL,EATOM_NEW,NITYP_NEW,NIONS_NEW,NTYP_NEW,POMASS_NEW,TYPE_NEW,LALLOCATE_ARRAYS,ISTART_FF)
         USE TIME_CHECK
...
! Loop over all types found in the ML_AB file.
            DO INTYP=1, MTYP_OLD
...
!unit               POMASS_TMP(INTYP)=POMASS_OLD(INTYP)/MUNIT
            ...
            ENDDO
            MTYP_TMP=MTYP_OLD
! Now, loop over all types in the current VASP calculation (POSCAR).
            DO INTYP=1, NTYP_NEW
               IFLAG_LOC=1
! For every type in the POSCAR file, check if it is identical to (1._q,0._q) of the
! types in ML_AB.
               DO JNTYP=1, MTYP_OLD
! Check if type names are identical in POSCAR and ML_AB.
                  IF(TYPE_NEW(INTYP) .EQ. TYPE_OLD(JNTYP)) THEN
                     IFLAG_LOC=0
! Type names, masses and reference energies have already been read from ML_AB
! file (see loop above). However, now different reference energies may be
! provided by the user in the INCAR file for those types which are also found
! in the POSCAR file. If this is the case, update with values from INCAR tag
! ML_EATOM_REF.
                     IF (TL%GET_STATE("ML_EATOM_REF") == TL_INCAR .OR. &
                         TL%GET_STATE("ML_EATOM_REF") == TL_INCAR_ALT) THEN
                        EATOM_TMP(JNTYP) = EATOM_NEW(INTYP)
                     END IF
                     EXIT
                  ENDIF
               ENDDO
...
! Memorize the merged data
            ...
            FF%MIONS=MIONS_OLD
            FF%MTYP=MTYP_OLD
! Check if we need to update ML_EATOM_REF for some types.
            IF (TL%GET_STATE("ML_EATOM_REF") == TL_INCAR .OR. &
                TL%GET_STATE("ML_EATOM_REF") == TL_INCAR_ALT) THEN
               DO INTYP=1, NTYP_NEW
! For every type in the POSCAR file, check if it is identical to (1._q,0._q) of the
...
! Check if type names are identical in POSCAR and ML_AB.
                     IF(TYPE_NEW(INTYP) .EQ. TYPE_OLD(JNTYP)) THEN
! Overwrite ML_AB file values with updated versions.
                           EATOM_OLD(JNTYP) = EATOM_NEW(INTYP)
                        EXIT
                     ENDIF
                  ENDDO
               ENDDO
...
!****************************************************************************************************
        ...
        SUBROUTINE SET_FF_PAR_ALLOCATION_AND_SECOND_PART (AB_HELP,          &
                               TAG_LIST,         &
                               TIM,              &
                               PAR_SUP_HANDLE,   &
                               MLSHM,            &
          ...
          USE ML_FF_TAGLIST
          USE FORCE_FIELD_ADDITIONAL2, ONLY: SET_DESCRIPTOR_COMBOS
...
! Read FFCAR file.
            ...
            ENDIF
! ISTART_FF=3: Read ABCAR file, and use it as a database.
          ELSE IF(ISTART_FF.EQ.3) THEN
...
! Determination of the numbers of variables related to the angular descriptor.
            IF(FFM%W2.GT.0.0_q) THEN
               CALL SET_DESCRIPTOR_COMBOS(FFM,FF)
            ENDIF
! Stop profiling.
...
! Determination of the numbers of variables related to the angular descriptor.
        ...
        END SUBROUTINE SET_FF_PAR_ALLOCATION_AND_SECOND_PART
...
! Set sizes of arrays related to descriptors. For the angular descriptors.
            IF(FFM%W2.GT.0.0_q) THEN
               ...
               ELSE IF (FFM%DESC_TYPE.EQ.2) THEN
                  FFM%MMVAR2=FFM%MRB2*(FFM%MRB2+1)/2*(FFM%LMAX2+1)
                  FFM%MMVAR2_ALLOCATE=FFM%MRB2*(FFM%MRB2+1)/2*(FFM%LMAX2+1)
                  FFM%RV%MMVAR2_RED=FFM%RV%MRB2_RED*(FFM%RV%MRB2_RED+1)/2*(FFM%RV%LMAX2_RED+1)*FF%MTYP**2
                  FFM%RV%MMVAR2_ALLOCATE_RED=FFM%RV%MRB2_RED*(FFM%RV%MRB2_RED+1)/2*(FFM%RV%LMAX2_RED+1)*FF%MTYP_ALLOCATE**2
               ELSE IF (FFM%DESC_TYPE.EQ.3) THEN
                  FFM%MMVAR2=FFM%MRB2*(FFM%LMAX2+1)*FF%MTYP**2
                  FFM%MMVAR2_ALLOCATE=FFM%MRB2*(FFM%LMAX2+1)*FF%MTYP_ALLOCATE**2
...
!            ENDIF
            IF (FFM%LSIC .AND. FFM%DESC_TYPE>0) THEN
               CALL ml_tutor%error("ML_DESC_TYPE only available &
                    &for ML_LSIC = .TRUE.. This combination should not &
                    &happen so something is seriously wrong.")
            ENDIF
! Set size parameters for the design matrices
...
! Optionally set array sizes for reduced descriptor
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               FFM%RV%MMVAR2_ALLOCATE_RED=FFM%RV%MMVAR2_RED
            ENDIF
...
! Set sizes of arrays related to descriptors. For the angular descriptors.
             IF(FFM%W2.GT.0.0_q) THEN
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   FFM%RV%MLNRB2_RED=FFM%RV%MRB2_RED*(FFM%RV%LMAX2_RED+1)
                ENDIF
             ENDIF
...
! Local variables
          ...
          INTEGER :: L
          REAL(q) :: RMEM
          REAL(q), ALLOCATABLE :: FWIN2_HELP(:,:)
          REAL(q), ALLOCATABLE :: QGRID2_HELP(:,:)
          REAL(q), ALLOCATABLE :: RB2_HELP(:,:,:)
          REAL(q), ALLOCATABLE :: RNORM2_HELP(:,:)
          INTEGER, ALLOCATABLE :: NRB_HELP(:)
          INTEGER :: MRB_HELP
          INTEGER :: IRB_HELP
          INTEGER :: IR
...
! Construct radial basis set for the radial descriptor.
             IF(FFM%W1.GT.0.0_q) THEN
                CALL RJ_RSBESSEL (FFM%DR1,FFM%FWIN1,FFM%IWINDOW1,FFM%LMAX1,FFM%LVARTRAN1,&
                     ...
                     FFM%QGRID1,FFM%RB1,FFM%RCUT1,FFM%RG1,RMEM,FFM%RNORM1)
                MEM1%RMEM(7)=RMEM
             ENDIF
! Construct radial basis set for the angular descriptor.
             IF(FFM%W2.GT.0.0_q) THEN
                IF (FFM%BASIS_TYPE2.EQ.0) THEN
                   CALL RJ_RSBESSEL (FFM%DR2,FFM%FWIN2,FFM%IWINDOW2,FFM%LMAX2,FFM%LVARTRAN2, &
                        ...
                        FFM%QGRID2,FFM%RB2,FFM%RCUT2,FFM%RG2,RMEM,FFM%RNORM2)
                ELSEIF (FFM%BASIS_TYPE2.EQ.1) THEN
                   CALL RJ_RSBESSEL (FFM%DR2,FFM%FWIN2,FFM%IWINDOW2,FFM%LMAX2,FFM%LVARTRAN2, &
                        FFM%LWINDOW2,FFM%MRB2,FFM%NR2,FFM%NRB2,FFM%NVARTRAN2, &
                        FFM%QGRID2,FFM%RB2,FFM%RCUT2,FFM%RG2,RMEM,FFM%RNORM2)
                   CALL SK_GAUSS (FFM%DR2,FFM%LMAX2,FFM%MRB2,FFM%NR2,FFM%RCUT2,FFM%NRB2,FFM%RB2,FFM%RG2)
                ELSEIF (FFM%BASIS_TYPE2.EQ.3) THEN
                   CALL RJ_RSBESSEL (FFM%DR2,FFM%FWIN2,FFM%IWINDOW2,FFM%LMAX2,FFM%LVARTRAN2, &
                        FFM%LWINDOW2,FFM%MRB2,FFM%NR2,FFM%NRB2,FFM%NVARTRAN2, &
                        FFM%QGRID2,FFM%RB2,FFM%RCUT2,FFM%RG2,RMEM,FFM%RNORM2)
                   CALL SK_GAUSS2 (FFM%DR2,FFM%LMAX2,FFM%MRB2,FFM%NR2,FFM%RCUT2,FFM%NRB2,FFM%RB2,FFM%RG2)
                ELSEIF (FFM%BASIS_TYPE2.EQ.4 .OR. FFM%BASIS_TYPE2.EQ.5) THEN
! First call Bessel functions to obtain FFM%QGRID2
                   CALL RJ_RSBESSEL (FFM%DR2,FFM%FWIN2,FFM%IWINDOW2,FFM%LMAX2,FFM%LVARTRAN2, &
                        FFM%LWINDOW2,FFM%MRB2,FFM%NR2,FFM%NRB2,FFM%NVARTRAN2, &
                        FFM%QGRID2,FFM%RB2,FFM%RCUT2,FFM%RG2,RMEM,FFM%RNORM2)
                   MRB_HELP=FFM%MRB2_MAX
                   ALLOCATE(FWIN2_HELP(0:FFM%LMAX2,1:MRB_HELP))
                   ALLOCATE(QGRID2_HELP(1:MRB_HELP,0:FFM%LMAX2))
                   ALLOCATE(RB2_HELP(1:FFM%NR2,1:MRB_HELP,0:FFM%LMAX2))
                   ALLOCATE(RNORM2_HELP(0:FFM%LMAX2,1:MRB_HELP))
                   ALLOCATE(NRB_HELP(0:FFM%LMAX2))
                   CALL RJ_RSBESSEL (FFM%DR2,FWIN2_HELP,FFM%IWINDOW2,FFM%LMAX2,FFM%LVARTRAN2, &
                        FFM%LWINDOW2,MRB_HELP,FFM%NR2,NRB_HELP,FFM%NVARTRAN2, &
                        QGRID2_HELP,RB2_HELP,FFM%RCUT2,FFM%RG2,RMEM,RNORM2_HELP)
                   IF (FFM%BASIS_TYPE2.EQ.5) THEN
                      FFM%NRB2=FFM%MRB2
                   ENDIF
                   DO L=0, FFM%LMAX2
                      DO IR=1, FFM%NR2
                         DO IRB=1, FFM%NRB2(L)
                            FFM%RB2(IR,IRB,L)=0.0_q
                            DO IRB_HELP=IRB, NRB_HELP(L)
                               FFM%RB2(IR,IRB,L)=FFM%RB2(IR,IRB,L) + RB2_HELP(IR,IRB_HELP,L)
                            ENDDO
                         ENDDO
                      ENDDO
                   ENDDO
                   DEALLOCATE(FWIN2_HELP)
                   DEALLOCATE(QGRID2_HELP)
                   DEALLOCATE(RB2_HELP)
                   DEALLOCATE(RNORM2_HELP)
                   DEALLOCATE(NRB_HELP)
                ENDIF

                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (FFM%RV%BASIS_TYPE2_RED.EQ.0) THEN
                      CALL RJ_RSBESSEL (FFM%DR2,FFM%FWIN2,FFM%IWINDOW2,FFM%RV%LMAX2_RED,FFM%LVARTRAN2, &
                           FFM%LWINDOW2,FFM%RV%MRB2_RED,FFM%NR2,FFM%RV%NRB2_RED,FFM%NVARTRAN2, &
                           FFM%RV%QGRID2_RED,FFM%RV%RB2_RED,FFM%RCUT2,FFM%RV%RG2_RED,RMEM,FFM%RV%RNORM2_RED)
                   ELSEIF (FFM%RV%BASIS_TYPE2_RED.EQ.4 .OR. FFM%BASIS_TYPE2.EQ.5) THEN
                      CALL RJ_RSBESSEL (FFM%DR2,FFM%FWIN2,FFM%IWINDOW2,FFM%RV%LMAX2_RED,FFM%LVARTRAN2, &
                           FFM%LWINDOW2,FFM%RV%MRB2_RED,FFM%NR2,FFM%RV%NRB2_RED,FFM%NVARTRAN2, &
                           FFM%RV%QGRID2_RED,FFM%RV%RB2_RED,FFM%RCUT2,FFM%RV%RG2_RED,RMEM,FFM%RV%RNORM2_RED)
                      MRB_HELP=FFM%RV%MRB2_MAX_RED
                      ALLOCATE(FWIN2_HELP(0:FFM%RV%LMAX2_RED,1:MRB_HELP))
                      ALLOCATE(QGRID2_HELP(1:MRB_HELP,0:FFM%RV%LMAX2_RED))
                      ALLOCATE(RB2_HELP(1:FFM%NR2,1:MRB_HELP,0:FFM%RV%LMAX2_RED))
                      ALLOCATE(RNORM2_HELP(0:FFM%RV%LMAX2_RED,1:MRB_HELP))
                      ALLOCATE(NRB_HELP(0:FFM%RV%LMAX2_RED))
                      CALL RJ_RSBESSEL (FFM%DR2,FWIN2_HELP,FFM%IWINDOW2,FFM%RV%LMAX2_RED,FFM%LVARTRAN2, &
                           FFM%LWINDOW2,MRB_HELP,FFM%NR2,NRB_HELP,FFM%NVARTRAN2, &
                           QGRID2_HELP,RB2_HELP,FFM%RCUT2,FFM%RV%RG2_RED,RMEM,RNORM2_HELP)
                      IF (FFM%BASIS_TYPE2.EQ.5) THEN
                         FFM%RV%NRB2_RED=FFM%RV%MRB2_RED
                      ENDIF
                      DO L=0, FFM%RV%LMAX2_RED
                         DO IR=1, FFM%NR2
                            DO IRB=1,FFM%RV%NRB2_RED(L)
                               FFM%RV%RB2_RED(IR,IRB,L)=0.0_q
                               DO IRB_HELP=IRB, NRB_HELP(L)
                                  FFM%RV%RB2_RED(IR,IRB,L)=FFM%RV%RB2_RED(IR,IRB,L) + RB2_HELP(IR,IRB_HELP,L)
                               ENDDO
                            ENDDO
                         ENDDO
                      ENDDO
...
! For the radial descriptor.
                IF(FFM%W1.GT.0.0_q) THEN
                   CALL RAD_FUNC (FFM%DR1,FFM%DRSPL1,FFM%FNL1(1:,1:,0:,1:,INTYP),FFM%IBROAD1, &
                        ...
                        FFM%MRB1,FFM%MSPL1,FFM%NMETRIC1,FFM%NR1,FFM%NRB1,FFM%NSPL1,FFM%NVARTRAN1,FFM%QGRID1,FFM%RB1, &
                        FFM%RCUT1,FFM%RG1,RMEM,FFM%RMETRIC1,FFM%RNORM1,FFM%WION1(INTYP))
...
! For the angular descriptor.
                IF (FFM%W2.GT.0.0_q) THEN
                   CALL RAD_FUNC (FFM%DR2,FFM%DRSPL2,FFM%FNL2(1:,1:,0:,1:,INTYP),FFM%IBROAD2, &
                        ...
                        FFM%MRB2,FFM%MSPL2,FFM%NMETRIC2,FFM%NR2,FFM%NRB2,FFM%NSPL2,FFM%NVARTRAN2,FFM%QGRID2,FFM%RB2, &
                        FFM%RCUT2,FFM%RG2,RMEM,FFM%RMETRIC2,FFM%RNORM2,FFM%WION2(INTYP))
                   MEM1%RMEM(7)=MAX(MEM1%RMEM(7),RMEM)
                   IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                      CALL RAD_FUNC (FFM%DR2,FFM%DRSPL2,FFM%RV%FNL2_RED(1:,1:,0:,1:,INTYP),FFM%IBROAD2, &
                           FFM%ICUT2,FFM%RV%LMAX2_RED,FFM%LMETRIC2,FFM%LVARTRAN2, &
                           FFM%RV%MRB2_RED,FFM%MSPL2,FFM%NMETRIC2,FFM%NR2,FFM%RV%NRB2_RED,FFM%NSPL2, &
                           FFM%NVARTRAN2,FFM%RV%QGRID2_RED,FFM%RV%RB2_RED, &
                           FFM%RCUT2,FFM%RV%RG2_RED,RMEM,FFM%RMETRIC2,FFM%RV%RNORM2_RED,FFM%WION2(INTYP))
                      MEM1%RMEM(7)=MEM1%RMEM(7) + RMEM
                   ENDIF 
                ENDIF
             ENDDO
...
! For the angular descriptor.
             IF(FFM%W2.GT.0.0_q) THEN
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   FFM%RV%NLNRB2_RED=0
                   DO IRB=1, FFM%RV%MRB2_RED
                      DO L=0, FFM%RV%LMAX2_RED
                         IF (IRB .LE. FFM%RV%NRB2_RED(L)) THEN
                            FFM%RV%NLNRB2_RED=FFM%RV%NLNRB2_RED+1
                            FFM%RV%L_LNRB2_RED(FFM%RV%NLNRB2_RED)=L
                            FFM%RV%NRB_LNRB2_RED(FFM%RV%NLNRB2_RED)=IRB
                         ENDIF
                      ENDDO
                   ENDDO
                ENDIF
...
!****************************************************************************************************
        ...
        SUBROUTINE INIT_PS (ASA,TIM,PAR_SUP_HANDLE,MLSHM,MEM1,FF,FFM,A,B,LADD,LNIONS,LNTYP,MCONF,MIONS,MITYP,MTYP,NCONF,NIONS,NITYP,NTYP,POSION)
          USE NEIGHBOUR
          USE SOAP_KERNEL
          USE ML_FF_STRUCT, ONLY: ASA_STUFF, TIME, PARALLEL_SUPER,ML_SHMEM, MEMORY, FF_PAR, FFM_PAR
          IMPLICIT NONE
! Input variables
          TYPE (ASA_STUFF)      :: ASA
          TYPE (TIME)           :: TIM
                  ...
                  IF(FFM%NB(INTYP0).GT.0) THEN
                     DO IB_TMP=1, FFM%NB(INTYP0), PAR_SUP_HANDLE%COMM_WORLD%NCPU
                        ...
                        IF(IB.LE.FFM%NB(INTYP0)) THEN
                           ...
                           CALL SET_SIZE_EST_POINT (A(1:,1:,ICONF),B(1:,1:,ICONF),INIONS, &
                                LADD(1:,ICONF),LNIONS(1:,1:,ICONF),MIONS,MITYP,MNEIB_TMP,MTYP, &
...
! Initialization of CLM1
                  ...
                  IF(FFM%W2.GT.0.0_q) THEN
                     ...
                     IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                        MLSHM%CLM2_RED(1:MROW_CLM2_RED,1:FFM%NB(INTYP0))=0.0_q
                     ENDIF
                     IF(FFM%LSIC) THEN
...
! This part is parallelized by 1
                  DO IB_TMP=1, FFM%NB(INTYP0), PAR_SUP_HANDLE%COMM_WORLD%NCPU
                     IB=IB_TMP+PAR_SUP_HANDLE%COMM_WORLD%NODE_ME
                     IF(IB.LE.FFM%NB(INTYP0)) THEN
                        ICONF=FFM%LNCONF(IB,INTYP0)
...
! Configure structural data
                        ...
                        IF(FFM%W1.GT.0.0_q) THEN
                           CALL SET_EST_DATA_POINT (1,A(1:,1:,ICONF),B(1:,1:,ICONF),INIONS, &
                                LNTYP(1:,ICONF),LNIONS_EST1,MIONS,MROW_EST1,MTYP, &
                                NIONS(ICONF),NNEIB_EST1,NTYP(ICONF),FFM%RCUT1,FFM%RCUT1, &
                                FFM%RCUT2,POSION(1:,1:,ICONF),XYZ_EST1)
                        ENDIF
                        IF(FFM%W2.GT.0.0_q) THEN
                           CALL SET_EST_DATA_POINT (1,A(1:,1:,ICONF),B(1:,1:,ICONF),INIONS, &
                                LNTYP(1:,ICONF),LNIONS_EST2,MIONS,MROW_EST2,MTYP, &
                                NIONS(ICONF),NNEIB_EST2,NTYP(ICONF),FFM%RCUT2,FFM%RCUT1, &
                                FFM%RCUT2,POSION(1:,1:,ICONF),XYZ_EST2)
                        ENDIF
                        ...
                        IF(FFM%W2.GT.0.0_q) THEN
                           ...
                           IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7) THEN
                              CALL D0CLM_SOAP_NOSIC_DUAL (MLSHM%CLM2_RED(:,LOCVAR_COL_CLM2), &
                                   ...
                                   INTYP0,LADD(1:,ICONF), &
                                   FFM%LMAX2,FFM%RV%LMAX2_RED,LNIONS_EST2, &
                                   FFM%LWINDOW2,FF%MIONS,MROW_EST2,FFM%MRB2,FFM%RV%MRB2_RED, &
                                   FFM%MSPL2,MTYP, &
                                   MLSHM%MROW_CLM2_GLOBAL, &
                                   NNEIB_EST2,FFM%NRB2,FFM%RV%NRB2_RED,FFM%NSPL2,NTYP(ICONF), &
                                   FFM%RCOUPLE_IONS,RMEM_LOC3,XYZ_EST2, &
                                   ASA%YLM3I,ASA%JS,ASA%INDCG,ASA%LMAXCG,ASA%YLM3LOOKUP_TABLE)
                           ELSE 
                              CALL D0CLM_SOAP (MLSHM%CLM2(:,LOCVAR_COL_CLM2), &
                                   MLSHM%CLM_COUPLE_HELP2_MANYBODY(:,LOCVAR_COL_CLM2_COUPLE), &
                                   FFM%DRSPL2,FFM%FNL2,FFM%FWIN2, &
                                   INTYP0,LADD(1:,ICONF),.FALSE.,FFM%LFLAG_VAR,FFM%LFLAG_VAR_SIC, &
                                   FFM%LIONS_COUPLE,FFM%LMAX2,LNIONS_EST2, &
                                   FFM%LSIC,FFM%LVAR,FFM%LVAR_SIC, &
                                   FFM%LWINDOW2,FF%MIONS,MROW_EST2,FFM%MRB2,FFM%MSPL2,MTYP, &
                                   ...
                                   MLSHM%PS_SIC_COUPLE(:,LOCVAR_COL_PS_SIC_COUPLE), &
                                   FFM%RCOUPLE_IONS,RMEM_LOC3,XYZ_EST2, &
                                   ASA%YLM3I,ASA%JS,ASA%INDCG,ASA%LMAXCG,ASA%YLM3LOOKUP_TABLE)
                                   RMEM_LOC2=MAX(RMEM_LOC2,RMEM_LOC3)
                           ENDIF
                           ...
                           IF (FFM%DESC_TYPE.EQ.0) THEN
                              IF (FFM%LSIC) THEN
                                 CALL D0PS_SOAP (FFM%AFILT2, MLSHM%CLM2(:,LOCVAR_COL_CLM2), &
                                      MLSHM%CLM_COUPLE_HELP2_MANYBODY(:,LOCVAR_COL_CLM2_COUPLE), &
                                      FFM%IAFILT2, INTYP0, FFM%LAFILT2, LADD(1:,ICONF), .FALSE., &
                                      ...
                                      PS(1:,LOCVAR_COL_PS), PS_COUPLE(1:), &
                                      MLSHM%PS_SIC(:,LOCVAR_COL_PS_SIC), &
...
! Low resolution type-dependent descriptor
                              CALL D0PS_SOAP_NOSIC (FFM%AFILT2, MLSHM%CLM2_RED(:,LOCVAR_COL_CLM2), &
                                   ...
                                   FFM%RV%NLNRB2_RED, FFM%RV%NRB2_RED(0:), FFM%RV%NRB_LNRB2_RED, NTYP(ICONF), &
                                   FFM%RV%PS_UGANG_RED(1:,LOCVAR_COL_PS,1), PS_COUPLE(1:), &
                                   FFM%RV%WVAR_RED(:,:,0:,:,:,:), &
                                   FFM%RV%LMAX2_RED)
                           ELSE IF (FFM%DESC_TYPE.EQ.3) THEN
                  ...
                  DO IB_TMP=1, FFM%NB(INTYP0), PAR_SUP_HANDLE%COMM_WORLD%NCPU
                     ...
                     IF(IB.LE.FFM%NB(INTYP0)) THEN
                        ...
                        ELSE
                           LOCVAR_COL_PS=1
                        ENDIF
                        CALL NORMALIZATION_D0 (FFM,C00(1:,LOCVAR_COL_C00),C00_COUPLE(1:), &
                                               FFM%LSUPERVEC,.FALSE.,FFM%LNORM1,FFM%LNORM2, &
                                               FFM%NNVAR1,FFM%NNVAR2(INTYP0), &
                                               FFM%RV%NNVAR2_RED(INTYP0), &
                                               PS(1:,LOCVAR_COL_PS), &
                                               FFM%RV%PS_UGANG_RED(1:,LOCVAR_COL_PS,1), &
                                               PS_COUPLE(1:), &
...
! Optionally copy the calculated PS_RED to FFM%RV%PS_RED.
                     IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                        DO IB=1, FFM%NB(INTYP0)
! Copy PS to helping array PS_HELP.
                           IPROCESS=MOD(IB-1,PAR_SUP_HANDLE%COMM_WORLD%NCPU)
                           IF (PAR_SUP_HANDLE%COMM_WORLD%NODE_ME.EQ.IPROCESS) THEN
                              LOCVAR_COL=CEILING(REAL(IB,q)/REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q))
                              PS_HELP_RED(1:FFM%RV%NNVAR2_RED(INTYP0))=FFM%RV%PS_UGANG_RED(1:FFM%RV%NNVAR2_RED(INTYP0),LOCVAR_COL,1)
                           ENDIF
! Broadcast the helping array.
                           CALL M_BCAST (PAR_SUP_HANDLE%COMM_WORLD,PS_HELP_RED(1:),FFM%RV%NNVAR2_RED(INTYP0),IPROCESS)
! Copy the helping array to FFM%PS.

                           TEST_COL=MOD((IB-1)/PAR_SUP_HANDLE%DESC_PS_FFM_RED%NB,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL)
                           IF (PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL.EQ.TEST_COL) THEN
                              LOCVAR_COL=GLOBAL2LOCAL(IB,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL,PAR_SUP_HANDLE%DESC_PS_FFM_RED%NB)
# 2206

                              DO IVAR=1, FFM%RV%NNVAR2_RED(INTYP0)

                                 TEST_ROW=MOD((IVAR-1)/PAR_SUP_HANDLE%DESC_PS_FFM_RED%MB,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW)
                                 IF (PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW.NE.TEST_ROW) THEN
                                    CYCLE
                                 ENDIF
                                 LOCVAR_ROW=GLOBAL2LOCAL(IVAR,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW,PAR_SUP_HANDLE%DESC_PS_FFM_RED%MB)
# 2216

                                 FFM%RV%PS_RED(LOCVAR_ROW,LOCVAR_COL,INTYP0)=PS_HELP_RED(IVAR)
                              ENDDO
...
! Define sizes.
            ...
            IF(FFM%W2.GT.0.0_q) THEN
               ...
               MROW_CLM2_RED=(FFM%RV%LMAX2_RED+1)**2*FF%MTYP*FFM%RV%MRB2_RED
...
! Allocate arrays.
            ...
            IF(ALLOCATED(FFM%RV%PS_UGANG_RED)) THEN
               DEALLOCATE(FFM%RV%PS_UGANG_RED)
            ENDIF
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               ALLOCATE (FFM%RV%PS_UGANG_RED(1:MROW_PS_RED,1:MCOL_PS_RED,1:MTYP))
               RMEM_LOC1=RMEM_LOC1+REAL(MROW_PS_RED,q)*REAL(MCOL_PS_RED,q)*8/1.0E+06_q
            ELSE
               ALLOCATE (FFM%RV%PS_UGANG_RED(1:1,1:MCOL_PS,1:MTYP))
            ENDIF
            ...
            IF(ALLOCATED(PS_HELP_RED)) THEN
               DEALLOCATE(PS_HELP_RED)
            ENDIF
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               ALLOCATE (PS_HELP_RED(1:MROW_PS_RED))
               RMEM_LOC1=RMEM_LOC1+REAL(MROW_PS_RED,q)*8/1.0E+06_q
            ELSE
               ALLOCATE (PS_HELP_RED(1:1))
            ENDIF
...
! Collect the distributed angular descriptor data into process 0.
             IF(FFM%W1.GT.0.0_q) THEN
                ...
                CALL SCALAPACK_REDUCE_TO_ONE_CORE(PAR_SUP_HANDLE,PAR_SUP_HANDLE%DESC_C00_FFM,FFM%C00(1:,1:,INTYP0), &
                     PAR_SUP_HANDLE%CONTEXT_WORLD,FFM%NNVAR1,NB(INTYP0))
...
! Collect the distributed angular descriptor data into process 0.
                CALL SCALAPACK_REDUCE_TO_ONE_CORE(PAR_SUP_HANDLE,PAR_SUP_HANDLE%DESC_PS_FFM,FFM%PS(1:,1:,INTYP0), &
                     PAR_SUP_HANDLE%CONTEXT_WORLD,FFM%NNVAR2(INTYP0),NB(INTYP0))
! Copy the collected angular descriptors into local arrays.
                IF (PAR_SUP_HANDLE%CONTEXT_SINGLE%ME_ROW.EQ.0.AND.PAR_SUP_HANDLE%CONTEXT_SINGLE%ME_COL.EQ.0) THEN
                   DO IB=1, NB(INTYP0)
                      IB_INTYP0=IB+LHEAD(INTYP0)-1
...
! Optional distribution of reduced descriptor
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN 

                   CALL SCALAPACK_REDUCE_TO_ONE_CORE(PAR_SUP_HANDLE,PAR_SUP_HANDLE%DESC_PS_FFM_RED,FFM%RV%PS_RED(1:,1:,INTYP0), &
                        PAR_SUP_HANDLE%CONTEXT_WORLD,FFM%RV%NNVAR2_RED(INTYP0),NB(INTYP0))
                   IF (PAR_SUP_HANDLE%CONTEXT_SINGLE%ME_ROW.EQ.0.AND.PAR_SUP_HANDLE%CONTEXT_SINGLE%ME_COL.EQ.0) THEN
                      DO IB=1, NB(INTYP0)
                         IB_INTYP0=IB+LHEAD(INTYP0)-1
                         DO IVAR=1, FFM%RV%NNVAR2_RED(INTYP0)
                            FFM%RV%PS_ALL_RED(IVAR,IB_INTYP0)=PAR_SUP_HANDLE%GENERAL_SINGLE(IVAR,IB)
                         ENDDO
                      ENDDO
                   ENDIF
                   CALL FINIT_SCALAPACK_REDUCE_TO_ONE_CORE(PAR_SUP_HANDLE)
...
# 2569
                ...
                CALL M_BCAST (PAR_SUP_HANDLE%COMM_WORLD,FFM%RV%PS_ALL_RED,FFM%RV%MMVAR2_RED*NBTOT,0)
...
!****************************************************************************************************
        ...
        SUBROUTINE STORE_SINV_FFM_IN_ALL_PROCESS (PAR_SUP_HANDLE,FF,FFM)
          USE ML_FF_STRUCT, ONLY: PARALLEL_SUPER, FF_PAR, FFM_PAR, ML_IO_WRITE
          IMPLICIT NONE
! Input variables
          TYPE (PARALLEL_SUPER) :: PAR_SUP_HANDLE
...
! Set the maximum number of basis sets per element.
          NB_MAX=FFM%NB(1)
          DO INTYP0=2, FF%MTYP, 1
...
! Merge FFM%SINV.
          DO INTYP0=1, FF%MTYP
             ...
             CALL SCALAPACK_REDUCE_TO_ONE_CORE(PAR_SUP_HANDLE,PAR_SUP_HANDLE%DESC_SOAP_FFM, &
                  FFM%SINV(1:,1:,INTYP0),PAR_SUP_HANDLE%CONTEXT_WORLD, &
                  PAR_SUP_HANDLE%DESC_SOAP_FFM%M,PAR_SUP_HANDLE%DESC_SOAP_FFM%N)
! Copy the collected FFM%SINV into local arrays.
             IF (PAR_SUP_HANDLE%CONTEXT_SINGLE%ME_ROW.EQ.0.AND.PAR_SUP_HANDLE%CONTEXT_SINGLE%ME_COL.EQ.0) THEN
                DO IB=1, FFM%NB(INTYP0)
                   FFM%SINV_ALL(1:FFM%NB(INTYP0),IB,INTYP0)=PAR_SUP_HANDLE%GENERAL_SINGLE(1:FFM%NB(INTYP0),IB)
                ENDDO
             ENDIF
! Then, deallocate GENERAL_SINGLE array.
             CALL FINIT_SCALAPACK_REDUCE_TO_ONE_CORE(PAR_SUP_HANDLE)
...
# 2762
             ...
             CALL M_BCAST (PAR_SUP_HANDLE%COMM_WORLD,FFM%SINV_ALL(1:,1:,INTYP0),FFM%MB*NB_MAX,0)

          ENDDO
! Stop profiling.
          

      END SUBROUTINE STORE_SINV_FFM_IN_ALL_PROCESS
...
!****************************************************************************************************
        SUBROUTINE INIT_SOAP(TIM,PAR_SUP_HANDLE,MEM1,FF,FFM,LAPACKWORK)
          USE SOAP_KERNEL
          USE ML_FF_STRUCT, ONLY: TIME, PARALLEL_SUPER, MEMORY, FF_PAR, FFM_PAR, LAPACK_WORK
          IMPLICIT NONE
! Input variables
          TYPE (TIME)           :: TIM
          TYPE (PARALLEL_SUPER) :: PAR_SUP_HANDLE
...
!PREFAC_HELP_RED=(1.0_q - FFM%RV%RATIO_DESC) * FFM%W2
...
# 3075
                   ...
                   CALL POLY_SOAP_INIT1 (PAR_SUP_HANDLE%CONTEXT_WORLD,PAR_SUP_HANDLE%DESC_SOAP_FFM,IPOLY,FFM%LSUPERVEC, &
                        MROW_SOAP,MCOL_SOAP,1,FFM%NB(INTYP0),FFM%NB(INTYP0),FFM%NHYP2,SOAP1(1:,1:),SOAP2(1:,1:),PREFAC_HELP)
! Optional reduced part of SOAP
                   IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN

                      CALL PDGEMM('T','N',FFM%NB(INTYP0),FFM%NB(INTYP0),FFM%RV%NNVAR2_RED(INTYP0),1.0_q, &
                           FFM%RV%PS_RED(1,1,INTYP0),1,1,PAR_SUP_HANDLE%DESC_PS_FFM_RED%DESC, &
                           FFM%RV%PS_RED(1,1,INTYP0),1,1,PAR_SUP_HANDLE%DESC_PS_FFM_RED%DESC, &
                           0.0_q,SOAP1(1,1),1,1,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC)
# 3088

                      CALL POLY_SOAP_INIT1 (PAR_SUP_HANDLE%CONTEXT_WORLD,PAR_SUP_HANDLE%DESC_SOAP_FFM,1,FFM%LSUPERVEC, &
                           MROW_SOAP,MCOL_SOAP,1,FFM%NB(INTYP0),FFM%NB(INTYP0),FFM%NHYP2,SOAP1(1:,1:),SOAP2(1:,1:),PREFAC_HELP_RED)
                   ENDIF
                ENDIF
...
! Copy SOAP.
                DO IB=1, FFM%NB(INTYP0)
                   ...
                   ELSE IF(PAR_SUP_HANDLE%CONTEXT_WORLD%NDIM.EQ.2) THEN
                      CALL PDCOPY(FFM%NB(INTYP0),SOAP2(1,1),1,IB,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC,1, &
                                  FFM%SOAP(1,1,INTYP0),1,IB,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC,1)
                   ENDIF
# 3114

                ENDDO
! Compute inverse matrix of SOAP for computation of spilling factor.
! This matrix is needed to for calculation of the spilling factor. In a production run (ML_MODE=RUN) which is
! using an ML_FF file that was obtained from SVD fitting (ML_MODE=REFIT) SINV should be read from the ML_FF
! file so this part needs to be skipped (LREAD_SINV_FROM_FILE = .TRUE).
! 1) Diagonalize the SOAP matrix.
                IF (.NOT.FF%LREAD_SINV_FROM_FILE) THEN

                   
! 1-1) Calculate the necessary size of work array.
                   CALL SCALAPACK_WORK_ARRAY_SIZE_PDSYEV_INIT(PAR_SUP_HANDLE%SCAWORK)
                   CALL PDSYEV('V','U',FFM%NB(INTYP0),SOAP2(1,1),1,1,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC, &
...
! 2) Compute the inverse matrix of SOAP.
! 2-1) Compute the maximum eigenvalue.
...
! 2-2) Preparation.
                   DO IB=1, FFM%NB(INTYP0)
                      ...
                      DO JB=1,FFM%NB(INTYP0)
                         ...
                         IF(ABS(EIGENVALUE(IB)/WMAX).GT.EPS) THEN
                            U(LOCVAR_ROW,LOCVAR_COL)=EIGENVECTOR(LOCVAR_ROW,LOCVAR_COL)/EIGENVALUE(IB)
                         ELSE
                            EIGENVECTOR(LOCVAR_ROW,LOCVAR_COL)=0.0_q
                            U(LOCVAR_ROW,LOCVAR_COL)=0.0_q
                         ENDIF
                      ENDDO
# 3189
...
! Compute the inverse matrix.
                   ...
                   CALL PDGEMM('N','T',FFM%NB(INTYP0),FFM%NB(INTYP0),FFM%NB(INTYP0),1.0_q, &
                        EIGENVECTOR(1,1),1,1,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC, &
                        U(1,1),1,1,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC, &
                        0.0_q,FFM%SINV(1,1,INTYP0),1,1,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC)
...
! Allocate arrays.
           ...
           RMEM_HELP=RMEM_HELP+REAL(FFM%MB,q)*8/1.0E+06_q
           MEM1%RMEM(5)=MAX(MEM1%RMEM(5),RMEM_HELP)

           IF(ALLOCATED(EIGENVECTOR)) THEN
              DEALLOCATE(EIGENVECTOR)
           ENDIF
           ALLOCATE (EIGENVECTOR(1:MROW_SOAP,1:MCOL_SOAP))
           ...
           IF(ALLOCATED(U)) THEN
              DEALLOCATE(U)
           ENDIF
           ALLOCATE (U(1:MROW_SOAP,1:MCOL_SOAP))
           RMEM_HELP=RMEM_HELP+REAL(MROW_SOAP,q)*REAL(MCOL_SOAP,q)*8/1.0E+06_q
           MEM1%RMEM(5)=MAX(MEM1%RMEM(5),RMEM_HELP)
...
! Sparsification is 1._q for CUR decomposition of A=P P^T, where P is
! the matrix for the angular descriptors.
!****************************************************************************************************

        SUBROUTINE SPARS_DES_CUR (LAPACKWORK,PAR_SUP_HANDLE,LOGF,FF,FFM, &
                                  DESC_PS_FFM,DESC_TYPE, &
                                  LFLAG_VAR,LMAX2,LVAR, &
                                  MMVAR2,MRB2,NB,NRB2, &
                                  NNVAR2,PS,WVAR, &
                                  LFLAG_VAR_SIC,LVAR_SIC,NNVAR2_SIC)
          USE ML_FF_STRUCT, ONLY: LAPACK_WORK, PARALLEL_DESCRIPTOR, PARALLEL_SUPER, LOG_PAR, FF_PAR, FFM_PAR
          IMPLICIT NONE
! Input variables
          TYPE (LAPACK_WORK)               :: LAPACKWORK
          TYPE (PARALLEL_SUPER)            :: PAR_SUP_HANDLE
          ...
          LOGICAL,INTENT(INOUT)            :: LFLAG_VAR(:,:,0:,:,:,:)
          INTEGER,INTENT(IN)               :: LMAX2
          INTEGER,INTENT(INOUT)            :: LVAR(:,:,0:,:,:,:)
          INTEGER,INTENT(IN)               :: MMVAR2
          INTEGER,INTENT(IN)               :: MRB2
          INTEGER,INTENT(IN)               :: NB(:) !(1:FF%MTYP)
          INTEGER,INTENT(IN)               :: NRB2(0:) !(1:FF%MTYP)
          INTEGER,INTENT(INOUT)            :: NNVAR2(:)
          REAL(q),INTENT(INOUT)            :: PS(:,:,:)
          REAL(q),INTENT(INOUT)            :: WVAR(:,:,0:,:,:,:)
          LOGICAL,INTENT(INOUT)            :: LFLAG_VAR_SIC(:,:,0:,:,:)
          INTEGER,INTENT(INOUT)            :: LVAR_SIC(:,:,0:,:,:)
          INTEGER,INTENT(INOUT)            :: NNVAR2_SIC(:)
! Local variables.
          ...
          REAL(q)                          :: WRATIO_OLD
          NNVAR2_OLD = 0
! Sparsification is 1._q only for the angular descriptor.
          IF(FFM%W2.GT.0.0_q) THEN
! Start profiling.
            
! Allocate helping arrays.
             CALL ALLOCATE_SPARSDES_CUR(MMVAR2,LMAX2,MRB2)
             DO INTYP0=1, FF%MTYP
                IF(NB(INTYP0).GT.0) THEN
                   NNVAR2_OLD(INTYP0) = NNVAR2(INTYP0)
! Calculate A=PS PS^T matrix.

                   CALL PDGEMM('N','T',NNVAR2(INTYP0),NNVAR2(INTYP0),NB(INTYP0),1.0_q, &
                        PS(1,1,INTYP0),1,1,DESC_PS_FFM%DESC, &
                        PS(1,1,INTYP0),1,1,DESC_PS_FFM%DESC, &
...
! 1) Calculate the necessary size of work array.
            CALL SCALAPACK_WORK_ARRAY_SIZE_PDSYEV_INIT(PAR_SUP_HANDLE%SCAWORK)
            CALL PDSYEV('V','U',NNVAR2(INTYP0),A_SPARSDES_CUR(1,1),1,1,PAR_SUP_HANDLE%DESC_A_SPARSDES_CUR%DESC, &
...
! Check the ratio of the minumum singular value to the maximum (1._q,0._q).
! Calculate the ratio of minimum eigenvalue to maximum (1._q,0._q)
            WMAX=ABS(EIGENVALUE(1))
            WMIN=ABS(EIGENVALUE(1))
            DO IVAR=2, NNVAR2(INTYP0), 1
...
! Here, the number of remained descriptors is set to that specified by the INCAR-tag diffrently from the sparsification of the basis sets.
            NDES=INT(FFM%RDES_SPARSDES*NNVAR2(INTYP0))
            NDEL_DES=NNVAR2(INTYP0)-NDES+1
! Calculate leverage scorings as indicators of correlations with high rank eigenvalues.
            PIL(1:NNVAR2(INTYP0))=0.0_q
            V_HELP=0.0_q
! Determine the left-singular vectors.
            DO IVAR=1, FFM%NRANK_SPARSDES

               CALL PDCOPY(NNVAR2(INTYP0),V_SPARSDES_CUR,1,NNVAR2(INTYP0)-FFM%NRANK_SPARSDES+IVAR, &
                    PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%DESC,1,V_HELP,1,IVAR,PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%DESC,1)
# 3480

            ENDDO
! Calculation of the leverage scoring.

            CALL PDGEMM('N','T',NNVAR2(INTYP0),NNVAR2(INTYP0),FFM%NRANK_SPARSDES,1.0_q, &
                 V_HELP(1,1),1,1,PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%DESC, &
                 V_HELP(1,1),1,1,PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%DESC,0.0_q,U_HELP(1,1), &
                 1,1,PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%DESC)
# 3491

! Then, the leverage scorings are calculated by the same method as for local reference configurations.
            DO IVAR=1, NNVAR2(INTYP0)
               ...
               IF (PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW.NE.TEST_ROW .OR. &
                   PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL.NE.TEST_COL) THEN
                  CYCLE
               ENDIF
...
# 3506
            ...
            CALL MPI_ALLREDUCE(MPI_IN_PLACE,PIL(1),NNVAR2(INTYP0),MPI_REAL8,MPI_SUM,PAR_SUP_HANDLE%COMM_WORLD%MPI_COMM,IERR)
               ...
               DO JNTYP0=1, FF%MTYP
                  DO JJNTYP0=1, FF%MTYP
                     DO L=0, LMAX2
                        DO IRB=1, NRB2(L)
                           DO JRB=IRB, NRB2(L)
                              LVAR_HELP2(JRB,IRB,L,JJNTYP0,JNTYP0)=LVAR(JRB,IRB,L,JJNTYP0,JNTYP0,INTYP0)
                           ENDDO
                        ENDDO
                     ENDDO
                  ENDDO
            ...
            ELSE IF (DESC_TYPE.EQ.1) THEN
               DO JNTYP0=1, FF%MTYP
                  DO L=0, LMAX2
                     DO IRB=1, NRB2(L)
                        DO JRB=IRB, NRB2(L)
                           LVAR_HELP2(JRB,IRB,L,1,JNTYP0)=LVAR(JRB,IRB,L,1,JNTYP0,INTYP0)
                        ENDDO
                     ENDDO
                  ENDDO
               ENDDO
            ELSE IF (DESC_TYPE.EQ.3) THEN
               DO JNTYP0=1, FF%MTYP
                  DO JJNTYP0=1, FF%MTYP
                     DO L=0, LMAX2
                        DO IRB=1, NRB2(L)
                           LVAR_HELP2(1,IRB,L,JJNTYP0,JNTYP0)=LVAR(1,IRB,L,JJNTYP0,JNTYP0,INTYP0)
                        ENDDO
                     ENDDO
                  ENDDO
               ENDDO
...
! Copy to helping arrays.
            DO IB=1, NB(INTYP0)
               ...
               TEST_COL=MOD((IB-1)/DESC_PS_FFM%NB,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL)
               IF(PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL.EQ.TEST_COL) THEN
                  LOCVAR_COL=GLOBAL2LOCAL(IB,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL,DESC_PS_FFM%NB)
# 3621

                  DO IVAR=1, NNVAR2(INTYP0)
                     IF(LFLAG_DES(IVAR).EQ.1) THEN

                        TEST_ROW=MOD((IVAR-1)/DESC_PS_FFM%MB,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW)
                        IF (PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW.NE.TEST_ROW) THEN
                           CYCLE
                        ENDIF
                        LOCVAR_ROW=GLOBAL2LOCAL(IVAR,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW,DESC_PS_FFM%MB)
# 3632

                        PS_HELP(LVAR_HELP1(IVAR))=PS(LOCVAR_ROW,LOCVAR_COL,INTYP0)
                     ENDIF
                  ENDDO
...
# 3648
                  ...
                  PS(1:DESC_PS_FFM%LOC_M,LOCVAR_COL,INTYP0)=0.0_q
...
# 3659
                     ...
                     PS(LOCVAR_ROW,LOCVAR_COL,INTYP0)=PS_HELP(IVAR)
                  ENDDO
...
! Restructure the related lists.
            NNVAR2(INTYP0)=0
            LFLAG_VAR(1:MRB2,1:MRB2,0:LMAX2,1:FF%MTYP,1:FF%MTYP,INTYP0)=.FALSE.
            LVAR(1:MRB2,1:MRB2,0:LMAX2,1:FF%MTYP,1:FF%MTYP,INTYP0)=0
            WVAR(1:MRB2,1:MRB2,0:LMAX2,1:FF%MTYP,1:FF%MTYP,INTYP0)=0.0_q
            IF(FFM%LSIC) THEN
               NNVAR2_SIC(INTYP0)=0
               LVAR_SIC(1:MRB2,1:MRB2,0:LMAX2,1:FF%MTYP,INTYP0)=0
               LFLAG_VAR_SIC(1:MRB2,1:MRB2,0:LMAX2,1:FF%MTYP,INTYP0)=.FALSE.
            ENDIF
            IF (DESC_TYPE.EQ.0) THEN
               DO JNTYP0=1, FF%MTYP
                  DO JJNTYP0=1, FF%MTYP
                        ...
                        DO IRB=1, NRB2(L)
                           DO JRB=IRB, NRB2(L)
                              IF(LFLAG_DES(LVAR_HELP2(JRB,IRB,L,JJNTYP0,JNTYP0)).EQ.1) THEN
                                 NNVAR2(INTYP0)=NNVAR2(INTYP0)+1
                                 LVAR(JRB,IRB,L,JJNTYP0,JNTYP0,INTYP0)=NNVAR2(INTYP0)
                                 LFLAG_VAR(JRB,IRB,L,JJNTYP0,JNTYP0,INTYP0)=.TRUE.
                                 IF(JRB.EQ.IRB) THEN
                                    WVAR(JRB,IRB,L,JJNTYP0,JNTYP0,INTYP0)=1.0_q
                                 ELSE
                                    WVAR(JRB,IRB,L,JJNTYP0,JNTYP0,INTYP0)=SQRT(2.0_q)
                                 ENDIF
                                 IF(FFM%LSIC) THEN
                                    IF(JJNTYP0.EQ.JNTYP0) THEN
                                       NNVAR2_SIC(INTYP0)=NNVAR2_SIC(INTYP0)+1
                                       LVAR_SIC(JRB,IRB,L,JNTYP0,INTYP0)=NNVAR2_SIC(INTYP0)
                                       LFLAG_VAR_SIC(JRB,IRB,L,JNTYP0,INTYP0)=.TRUE.
                                    ENDIF
                                 ENDIF
                              ENDIF
                           ENDDO
                     ...
                     DO IRB=1, NRB2(L)
                        DO JRB=IRB, NRB2(L)
                           IF(LFLAG_DES(LVAR_HELP2(JRB,IRB,L,1,JNTYP0)).EQ.1) THEN
                              NNVAR2(INTYP0)=NNVAR2(INTYP0)+1
                              LVAR(JRB,IRB,L,1,JNTYP0,INTYP0)=NNVAR2(INTYP0)
                              LFLAG_VAR(JRB,IRB,L,1,JNTYP0,INTYP0)=.TRUE.
                              IF(JRB.EQ.IRB) THEN
                                 WVAR(JRB,IRB,L,1,JNTYP0,INTYP0)=1.0_q
                              ELSE
                                 WVAR(JRB,IRB,L,1,JNTYP0,INTYP0)=SQRT(2.0_q)
                              ENDIF
                           ENDIF
                        ENDDO
                     ENDDO
            ...
            ELSE IF (DESC_TYPE.EQ.3) THEN
               DO JNTYP0=1, FF%MTYP
                  DO JJNTYP0=1, FF%MTYP
                     DO L=0, LMAX2
                        DO IRB=1, NRB2(L)
                           IF (LFLAG_DES(LVAR_HELP2(1,IRB,L,JJNTYP0,JNTYP0)).EQ.1) THEN
                              NNVAR2(INTYP0)=NNVAR2(INTYP0)+1
                              LVAR(1,IRB,L,JJNTYP0,JNTYP0,INTYP0)=NNVAR2(INTYP0)
                              LFLAG_VAR(1,IRB,L,JJNTYP0,JNTYP0,INTYP0)=.TRUE.
                              DO JRB=IRB, NRB2(L)
                                 IF (JRB.EQ.IRB) THEN
                                    WVAR(JRB,IRB,L,JJNTYP0,JNTYP0,INTYP0)=1.0_q
                                 ELSE
...
! Allocate helping arrays.
          ...
          SUBROUTINE ALLOCATE_SPARSDES_CUR(INPUT_DIM,LMAX_HELP,MRB_HELP)
            IMPLICIT NONE
! Input variables
            INTEGER, INTENT(IN)              :: INPUT_DIM
            INTEGER, INTENT(IN)              :: LMAX_HELP
            INTEGER, INTENT(IN)              :: MRB_HELP
! Local variables
...
! Size for A_SPARSDES_CUR.
            PAR_SUP_HANDLE%DESC_A_SPARSDES_CUR%M=INPUT_DIM
            PAR_SUP_HANDLE%DESC_A_SPARSDES_CUR%N=INPUT_DIM
            ...
            CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_A_SPARSDES_CUR%DESC, &
                 PAR_SUP_HANDLE%DESC_A_SPARSDES_CUR%M,PAR_SUP_HANDLE%DESC_A_SPARSDES_CUR%N, &
...
! Size for V_SPARSDES_CUR.
            PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%M=INPUT_DIM
            PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%N=INPUT_DIM
            ...
            CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%DESC, &
                 PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%M,PAR_SUP_HANDLE%DESC_V_SPARSDES_CUR%N, &
...
# 3803
            ...
            IF(ALLOCATED(EIGENVALUE)) THEN
               DEALLOCATE(EIGENVALUE)
            ENDIF
            ALLOCATE (EIGENVALUE(1:INPUT_DIM))
            IF(ALLOCATED(LFLAG_DES)) THEN
               DEALLOCATE(LFLAG_DES)
            ENDIF
            ALLOCATE (LFLAG_DES(1:INPUT_DIM))
            IF(ALLOCATED(LVAR_HELP1)) THEN
               DEALLOCATE(LVAR_HELP1)
            ENDIF
            ALLOCATE (LVAR_HELP1(1:INPUT_DIM))
            IF(ALLOCATED(LVAR_HELP2)) THEN
               DEALLOCATE(LVAR_HELP2)
            ENDIF
            ALLOCATE (LVAR_HELP2(1:MRB_HELP,1:MRB_HELP,0:LMAX_HELP,1:FF%MTYP,1:FF%MTYP))
            IF(ALLOCATED(PIL)) THEN
               DEALLOCATE(PIL)
            ENDIF
            ALLOCATE (PIL(1:INPUT_DIM))
            IF(ALLOCATED(PS_HELP)) THEN
               DEALLOCATE(PS_HELP)
            ENDIF
            ALLOCATE (PS_HELP(1:INPUT_DIM))
...
! Local variables
!********************************************************************
!> Sparsification result indicator.
!>
!> Used to trigger meaningful warning/info messages about abnormal
!> sparsification routine termination.
!********************************************************************
          ENUM, BIND(C)
!> Dummy state for usage as kind function argument.
             ENUMERATOR :: SR_STATE   = 0
!> Sparsification successful, normal continuation.
             ENUMERATOR :: SR_OK      = 1
!> Sparsification successful, but remaining number of local
!> reference configurations too low for training (ML_MB_MIN).
             ENUMERATOR :: SR_LOW_LRC = 2
!> Sparsification failed, no remaining local reference
!> configurations.
             ENUMERATOR :: SR_NO_LRC  = 3
!> Sparsification failed, no remaining configurations.
             ENUMERATOR :: SR_NO_CONF = 4
          END ENUM
          ...
          INTEGER(KIND(SR_STATE))          :: SPARS_RESULT
          REAL(q)          , ALLOCATABLE   :: U(:,:)
...
! Measure the initial times
          CALL VTIME (VPUTIM0,CPUTIM0)
! If sparsification is successful, the result indicator stays in OK state.
          SPARS_RESULT = SR_OK
! Initialize the memory usage
...
! Output
          ...
          IF (SPARS_RESULT == SR_LOW_LRC) THEN
             CALL LOGLINE_MSG(FF, LOGF, MSG_INFO, "Number of local reference &
                &configurations after sparsification below ML_MB_MIN, &
                &skipping training.")
          ELSE IF (SPARS_RESULT == SR_NO_LRC) THEN
             CALL LOGLINE_MSG(FF, LOGF, MSG_WARNING, "No remaining local &
                &reference configurations after sparsification, cannot create &
                &force field.")
          ELSE IF (SPARS_RESULT == SR_NO_CONF) THEN
             CALL LOGLINE_MSG(FF, LOGF, MSG_WARNING, "No remaining structures &
                &after sparsification, cannot create force field.")
          END IF
! Deallocate
...
! If the number of remained basis set equals (0._q,0._q), the machine fails the sparsification.
! In this case, FF%ISPARS is set as .FALSE.
            FF%ISPARS=.TRUE.
             DO INTYP0_LOC=1, FF%MTYP
               IF(FFM%NB(INTYP0_LOC)-NDEL_BASIS(INTYP0_LOC).LE.0) THEN
                  FF%ISPARS=.FALSE.
                  SPARS_RESULT = SR_NO_LRC
               ENDIF
            ENDDO
...
! If any of basis sets equals (0._q,0._q), the machine cannot generate force fields.
! In this case, FF%LGENFF is changed to be .FALSE.
            DO INTYP0_LOC=1, FF%MTYP
               IF(FFM%NB(INTYP0_LOC)-NDEL_BASIS(INTYP0_LOC).LT.FFM%MB_MIN) THEN
                  FF%LGENFF=.FALSE.
! Do not overwrite SR_NO_LRC if already detected.
                  IF (SPARS_RESULT /= SR_NO_LRC) SPARS_RESULT = SR_LOW_LRC
               ENDIF
            ENDDO
! Check the necessity of the calculations of variances in training data.
! If the number of remained configuration is (0._q,0._q), the machine cannot generate force field and cannot calculate the variances.
! In this case, the calculations are skipped.
            IF(NCONF-NDEL_CONF.LE.0) THEN
               FF%LGENFF=.FALSE.
               LREBUILD=.FALSE.
               FF%LSIG=.FALSE.
               SPARS_RESULT = SR_NO_CONF
            ENDIF
...
! For both the cases of 1- and 2-dimesional 1 grids, column dimension is distributed.
! Hence, PDCOPY is necessary.

                           CALL PDCOPY(FFM%NB(INTYP0),FFM%SOAP(1,1,INTYP0),1,IB,PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC,1, &
                                FFM%SOAP(1,1,INTYP0),1,LDEL_BASIS(NDEL_BASIS3,INTYP0),PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC,1)
! In case of non-1 version, just call DCOPY.
# 4722
...
! For both the cases of 1- and 2-dimensional 1 grids, column dimension is distributed.
! Hence, PDCOPY is necessary when 1 is used.
               IF(FF%LMLMB) THEN
                        ...
                        IF(NDEL_BASIS(INTYP0) .NE. 0) THEN
                           ...
                           DO IB=FFM%NB(INTYP0), 1, -1
                              ...
                              IF(LFLAG_BASIS(IB,INTYP0) .EQ. 1) THEN
                                 ...
                                 CALL PDCOPY(FFM%NNVAR2(INTYP0),FFM%PS(1,1,INTYP0),1,IB, &
                                      PAR_SUP_HANDLE%DESC_PS_FFM%DESC,1,FFM%PS(1,1,INTYP0),1, &
                                      LDEL_BASIS(NDEL_BASIS3,INTYP0),PAR_SUP_HANDLE%DESC_PS_FFM%DESC,1)
# 5071

! Optionally the same operations for the reduced descriptor
                                 IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN

                                    CALL PDCOPY(FFM%RV%NNVAR2_RED(INTYP0),FFM%RV%PS_RED(1,1,INTYP0),1,IB, &
                                         PAR_SUP_HANDLE%DESC_PS_FFM_RED%DESC,1,FFM%RV%PS_RED(1,1,INTYP0),1, &
                                         LDEL_BASIS(NDEL_BASIS3,INTYP0),PAR_SUP_HANDLE%DESC_PS_FFM_RED%DESC,1)
# 5081
...
! Optionally the same for reduced descriptor
                              IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                                 FFM%RV%PS_RED(1:PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M,LOCVAR_COL,INTYP0)=0.0_q
                              ENDIF
# 5101
...
! as any basis set. Hence, LB for that atom /configuraton should be set as (0._q,0._q).
...
! Otherwise, set the variables as (0._q,0._q)
                        ELSE
                           FFM%LNCONF(IB,INTYP0)=0
                           FFM%LNIONS(IB,INTYP0)=0
                        ENDIF
...
! should not exist anymore. Hence, nothing is 1._q for the deleting configuration.
                        DO INIONS=1, NIONS(ICONF)
                           IB=LB(INIONS,ICONF)
                           IF(IB .NE. 0) THEN
...
!****************************************************************************************************
        ...
        SUBROUTINE INIT_FMAT (ASA,AB,PAR_SUP_HANDLE,MLSHM,FF,FFM,A,B,LADD,LNITYP,LNIONS,LNTYP, &
                   MCONF,MITYP,MIONS,MTYP,NCONF,NITYP,NIONS,NTYP,POSION,OMEGA)
          USE NEIGHBOUR
          USE SOAP_KERNEL
          USE FORCE_FIELD_ADDITIONAL2
          USE ML_FF_STRUCT, ONLY: ASA_STUFF, ABINITIO, PARALLEL_SUPER, ML_SHMEM, FF_PAR, FFM_PAR, ML_IO_WRITE
          IMPLICIT NONE
! Input variables
          TYPE (ASA_STUFF)      :: ASA
          TYPE (ABINITIO)       :: AB
...
! Local variables
          ...
          REAL(q)               :: DUMMY_1D_1(1:1)
          REAL(q)               :: DUMMY_2D_1(1:1,1:1)
          REAL(q)               :: DUMMY_2D_2(1:1,1:1)
...
! Calculate expansion coefficients
                CALL COMPUTE_D0CLM(ASA, FF, FFM, MLSHM, PAR_SUP_HANDLE, &
                                        LADD(:,ICONF), LNIONS_EST1, LNIONS_EST2, &
                                        LNTYP(:,ICONF), MROW_EST1, MROW_EST2, MTYP, &
                                        NIONS(ICONF), NNEIB_EST1, NNEIB_EST2, &
                                        NTYP(ICONF), RMEM_HELP1, RMEM_HELP2, &
                                        XYZ_EST1, XYZ_EST2, "INIT")
...
! Calculate 1-point correlation descriptor
                   CALL COMPUTE_D0C00(FFM, PAR_SUP_HANDLE, MLSHM, C00, C00_COUPLE(:,:), &
                                           INTYP0, LADD(:,ICONF), LNIONS(:,:,ICONF), &
                                           MROW_C00, MROW_C00_COUPLE, MTYP, &
                                           NITYP(:,ICONF), NTYP(ICONF), "INIT")
!! Calculate 2-point correlation descriptor
                   CALL COMPUTE_D0PS(FFM, MLSHM, PAR_SUP_HANDLE, INTYP0, &
                                          LADD(:,ICONF), LNIONS(:,:,ICONF), MROW_PS, MROW_PS_COUPLE, &
                                          MTYP, NITYP(:,ICONF), NTYP(ICONF), PS, PS_COUPLE(:,:), "INIT")
! Normalization.
                   CALL COMPUTE_NORMD0(FFM, PAR_SUP_HANDLE, C00, C00_COUPLE(:,:), INTYP0, &
                                                      LNIONS(:,:,ICONF), MROW_C00, &
                                                      MROW_C00_COUPLE, MROW_PS, MROW_PS_COUPLE, &
                                                      NITYP(:,ICONF), PS, PS_COUPLE(:,:), &
                                                      RNORM_C00, RNORM_PS, RNORM_ALL, "INIT")
! Calculate SOAP kernel.
                   CALL COMPUTE_SOAP(FF, FFM, MLSHM, PAR_SUP_HANDLE, C00, &
                                                 INTYP, INTYP0, LHEAD, LNIONS(:,:,ICONF), MCOL_SOAP, MROW_C00, &
                                                 MROW_PS, MROW_SOAP, NB_MAX, NCOL_NITYP, NIONS(ICONF), NITYP(:,ICONF), &
                                                 NTYP(ICONF), PS, SOAP1, SOAP2, "INIT", &
                                                 MROW_C00_COUPLE, MROW_PS_COUPLE, &
                                                 C00_COUPLE(:,:), PS_COUPLE(:,:), &
                                                 1, 1, &
                                                 DUMMY_2D_1, DUMMY_2D_2)
! Calculate energy matrix elements, and copy them to global arrays.
                   CALL COMPUTE_ENERGY_HELP(FF, FFM, PAR_SUP_HANDLE, ICONF, INTYP0, ENERGY_HELP(:), &
                                       LNIONS(:,:,ICONF), MROW_SOAP, NCOL_NITYP, NIONS(ICONF), &
                                       NITYP(:,ICONF), SOAP2, VEC_LOC_ENERGY, "INIT", &
                                       1, 1, DUMMY_2D_1, &
                                       DUMMY_2D_2, DUMMY_1D_1)
                ENDDO
! Calculate derivatives of SOAP kernel
! 1-parallel over KNIONS loop.
...
! Calculate derivatives of expansion coefficients.
                      CALL COMPUTE_D1CLM(ASA, FF, FFM, PAR_SUP_HANDLE, DCLM1, DCLM2, &
...
! Calculate derivatives of angular descriptor.
                      CALL COMPUTE_D1C00(FFM, MLSHM, PAR_SUP_HANDLE, DC00, DCLM1, &
                                      KNIONS, KNTYP0, LADD(:,ICONF), LNIONS_EST1, &
                                      LNIONS_EST_MAX, MCOL_DC00, MCOL_DCLM1, &
                                      MROW_DC00, MROW_DCLM1, MROW_EST1, MROW_EST_MAX, &
                                      AB%MTYP,AB%MTYP_ALLOCATE, &
                                      NNEIB_EST1, NNEIB_EST_MAX, NTYP(ICONF))
                      CALL COMPUTE_D1PS(FFM, MLSHM, PAR_SUP_HANDLE, DCLM2, &
                                     DPS, DPS_SIC, &
                                     KNIONS, KNTYP0, LADD(:,ICONF), LNIONS_EST2, LNIONS_EST_MAX, &
                                     MCOL_DCLM2, MCOL_DPS, MCOL_DPS_SIC, &
                                     MROW_EST2, MROW_EST_MAX, MROW_DCLM2, &
                                     MROW_DPS, MROW_DPS_SIC, &
                                     MTYP, AB%MTYP_ALLOCATE, NIONS(ICONF), &
                                     NNEIB_EST2, NNEIB_EST_MAX, NTYP(ICONF))
! Normalization.
                      CALL COMPUTE_NORMD1(FF, FFM, PAR_SUP_HANDLE, DC00, DPS, KNIONS, &
                                                 KNTYP0, LADD(:,ICONF), LNIONS_EST1, LNIONS_EST2, LNIONS_EST_MAX, &
                                                 LNITYP(:,ICONF), MCOL_DC00, MCOL_DPS, MCOL_RNORM_ALL, &
                                                 MCOL_RNORM_C00, MCOL_RNORM_PS, MIONS, MITYP, &
                                                 MROW_EST1, MROW_EST2, MROW_EST_MAX, MROW_DC00, &
                                                 MROW_DPS, MROW_RNORM_C00, MROW_RNORM_PS, &
                                                 NNEIB_EST1, NNEIB_EST2, NNEIB_EST_MAX, NTYP(ICONF), &
                                                 RNORM_ALL, RNORM_C00, RNORM_PS)
! Element loop.
                      DO INTYP=1, NTYP(ICONF)
                         INTYP0=LADD(INTYP,ICONF)
! Calculate derivatives of SOAP similarity kernel.
                         CALL COMPUTE_DSOAP(FFM, MLSHM, PAR_SUP_HANDLE, DC00, DPS, &
...
! Calculate force and stress components.
                         CALL COMPUTE_FORCE_STRESS(FFM, PAR_SUP_HANDLE, DSOAP21, DSOAP22, &
                                                 FORCE_HELP1, INTYP0, KNIONS, KNTYP0, &
                                                 MROW_DSOAP1, MROW_DSOAP2, &
                                                 NNEIB_EST1, NNEIB_EST2, NNEIB_EST_MAX, &
                                                 OMEGA(ICONF), STRESS_HELP, &
...
! Transfer and copy the calculated force components in design matrix into the distributed FFM%FMAT array.
                DO INTYP=1, NTYP(ICONF)
                   INTYP0=LADD(INTYP,ICONF)
                   CALL COPY_FORCE_STRESS(FFM, PAR_SUP_HANDLE, ICONF, INTYP0, &
                                                      FORCE_HELP1, FORCE_HELP2, &
                                                      NIONS(ICONF), STRESS_HELP, "INIT")
...
!ENDDO
                     ...
                     IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
!DO IB=1, NB_MAX
!   MLSHM%SOAP_PS_RED(1:NIONS(ICONF),IB)=0.0_q
!ENDDO
                        MLSHM%SOAP_PS_RED=0.0_q
                     ENDIF
                  ENDIF
               ELSE
!DO IB=1, NB_MAX
...
! Local variables
            ...
            IF(FFM%W2.GT.0.0_q) THEN
               ...
               IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                  MLSHM%CLM2_RED(1:MROW_CLM2_RED,1:NIONS(ICONF))=0.0_q
               ENDIF
               IF(FFM%LSIC) THEN
...
!HERE_SWAP
            MROW_SOAP=FFM%MB
            MCOL_SOAP=CEILING(REAL(MITYP,q)/REAL(PAR_SUP_HANDLE%COMM_WORLD%NCPU,q))
...
! Size of DCLM2.
               ...
               IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                  MROW_DCLM2_RED=(FFM%RV%LMAX2_RED+1)**2*FF%MTYP*FFM%RV%MRB2_RED
               ELSE
                  MROW_DCLM2_RED=1
               ENDIF
! Size of DPS.
...
! Then, allocate all arrays.
            ...
            IF(ALLOCATED(C00_COUPLE)) THEN
               DEALLOCATE(C00_COUPLE)
            ENDIF
            ALLOCATE (C00_COUPLE(1:MROW_C00_COUPLE,1:1))
            ...
            DCLM2=0.0_q
! Reduced descriptor
            IF(ALLOCATED(FFM%RV%DCLM2_RED)) THEN
               DEALLOCATE(FFM%RV%DCLM2_RED)
            ENDIF
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               ALLOCATE (FFM%RV%DCLM2_RED(1:3,1:MROW_DCLM2_RED,1:FF%MTYP,1:MCOL_DCLM2))
               RMEM_MB=RMEM_MB+3*REAL(MROW_DCLM2_RED,q)*8*REAL(FF%MTYP,q)*REAL(MCOL_DCLM2,q)/1.0E+06_q
               FFM%RV%DCLM2_RED=0.0_q
            ELSE
               ALLOCATE (FFM%RV%DCLM2_RED(1:1,1:1,1:FF%MTYP,1:1))
            ENDIF
...
! Allocation of reduced variable
            IF (ALLOCATED(FFM%RV%PS_UGANG_RED)) THEN
               DEALLOCATE(FFM%RV%PS_UGANG_RED)
            ENDIF
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               ALLOCATE (FFM%RV%PS_UGANG_RED(1:FFM%RV%MMVAR2_RED,1:MCOL_PS,1:MTYP))
               RMEM_MB=RMEM_MB+REAL(FFM%RV%MMVAR2_RED,q)*REAL(MCOL_PS,q)*REAL(MTYP,q)*8/1.0E+06_q
            ELSE
               ALLOCATE (FFM%RV%PS_UGANG_RED(1:1,1:MCOL_PS,1:MTYP))
            ENDIF
            IF(ALLOCATED(PS_COUPLE)) THEN
               DEALLOCATE(PS_COUPLE)
            ENDIF
            ALLOCATE (PS_COUPLE(1:MROW_PS_COUPLE,1:1))
...
! Reduced variable
            IF(ALLOCATED(FFM%RV%DPS_UGANG_RED)) THEN
               DEALLOCATE(FFM%RV%DPS_UGANG_RED)
            ENDIF
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               ALLOCATE (FFM%RV%DPS_UGANG_RED(1:FFM%RV%MMVAR2_RED,1:MCOL_DPS,0:3,1:MTYP))
               RMEM_MB=RMEM_MB+REAL(FFM%RV%MMVAR2_RED,q)*REAL(MCOL_DPS,q)*REAL(MTYP,q)*4*8/1.0E+06_q
            ELSE 
               ALLOCATE (FFM%RV%DPS_UGANG_RED(1:1,1:1,0:0,1:MTYP))
            ENDIF
            ...
            IF(ALLOCATED(FFM%RV%RNORM_PS_RED)) THEN
               DEALLOCATE(FFM%RV%RNORM_PS_RED)
            ENDIF
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               ALLOCATE (FFM%RV%RNORM_PS_RED(1:MROW_RNORM_PS,1:MCOL_RNORM_PS))
               RMEM_MB=RMEM_MB+REAL(MROW_RNORM_PS,q)*REAL(MCOL_RNORM_PS,q)*8/1.0E+06_q
            ELSE
               ALLOCATE (FFM%RV%RNORM_PS_RED(1:1,1:MCOL_RNORM_PS))
            ENDIF
            IF(ALLOCATED(LNIONS_EST1)) THEN
               DEALLOCATE(LNIONS_EST1)
            ENDIF
...
!****************************************************************************************************
        SUBROUTINE PES_FF (ASA,AB,ABN,TIM,PAR_SUP_HANDLE,MLSHM,MEM1,LOGF,FF,FFM,LDO_AB_INITIO, &
                   NCONF,NCONF_NEW,NCONF_SYS,NSTEP,&
                   NEAREST_NEIGHBOR_RADIAL, &
                   NEAREST_NEIGHBOR_ANGULAR )        
          USE ML_FF_CONSTANT
          USE ML_FF_STRUCT, ONLY: ASA_STUFF, ABINITIO, ABINITIONEW, TIME, PARALLEL_SUPER, &
                            ML_SHMEM, MEMORY, LOG_PAR, FF_PAR, FFM_PAR, ML_IO_WRITE 
          USE ml_ff_tutor, ONLY: ml_tutor
...
! Initialize the memory usage
          RMEM_HELP2=0.0_q
          CALL MANYBODY (ASA,AB,ABN,PAR_SUP_HANDLE,MLSHM,FF,FFM,C00,ENERGY_HELP,FORCE_HELP, &
               MCOL_ENERGY_HELP, &
...
! Estimate average and maximum errors in forces.
            ...
            DO INIONS=1, ABN%NIONS
               ABN%BEAV(2,ABN%NCONF)=ABN%BEAV(2,ABN%NCONF) + &
                   ABN%BE(3*(INIONS-1)+2,ABN%NCONF)+ABN%BE(3*(INIONS-1) + &
                   3,ABN%NCONF)+ABN%BE(3*(INIONS-1)+4,ABN%NCONF)
               ABN%BEMAX(2,ABN%NCONF)=MAX(ABN%BEMAX(2,ABN%NCONF), &
                   SQRT(ABN%BE(3*(INIONS-1)+2,ABN%NCONF)+ABN%BE(3 * &
                   (INIONS-1)+3,ABN%NCONF)+ABN%BE(3*(INIONS-1)+4,ABN%NCONF)))
            ENDDO
...
! If FF%LTEST=.TRUE., LDO_AB_INITIO is set as .TRUE. when MOD(NSTEP,FF%NTEST)=1.
            ...
            IF (FF%LTEST.AND.FF%NTEST.NE.0) THEN
               ...
               ENDIF
            ENDIF
! If FF%ISTART=4 no update is 1._q since learning was 1._q in the first step
...
! Allocate PS.
            ...
            ENDIF
            IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
               ALLOCATE (FFM%RV%PS_UGANG_RED(1:FFM%RV%MMVAR2_RED,1:MCOL_PS,1:ABN%MTYP_ALLOCATE)) 
               RMEM_HELP1=RMEM_HELP1+REAL(FFM%RV%MMVAR2_RED,q)*REAL(MCOL_PS,q)*REAL(ABN%MTYP_ALLOCATE,q)*8/1.0E+06_q
            ELSE
               ALLOCATE (FFM%RV%PS_UGANG_RED(1:1,1:MCOL_PS,1:ABN%MTYP_ALLOCATE)) 
            ENDIF
! Allocate PS_HELP.
            IF(FFM%W2.GT.0.0_q) THEN
               IF(ALLOCATED(PS_HELP)) THEN
...
!****************************************************************************************************
        SUBROUTINE MANYBODY (ASA,AB,ABN,PAR_SUP_HANDLE,MLSHM,FF,FFM,C00,ENERGY_HELP,FORCE_HELP, &
                   MCOL_ENERGY_HELP, &
                   MROW_C00,MROW_ENERGY_HELP,MROW_FORCE_HELP,MROW_PS,MROW_STRESS_HELP, &
...
! Allocate.
! Configure structural data.
          IF (FF%LFAST .AND. FF%ISTART.EQ.2) THEN
...
! angular variables
          ...
          ELSE
             IF(FFM%W1.GT.0.0_q) THEN
                CALL SET_SIZE_EST (PAR_SUP_HANDLE%COMM_WORLD,ABN%A(1:,1:,ABN%NCONF), &
                                   ABN%B(1:,1:,ABN%NCONF),ABN%LADD(1:),ABN%LNIONS(1:,1:), &
                                   ABN%MIONS,ABN%MITYP,MNEIB_EST1,ABN%MTYP,ABN%NIONS, &
                                   ABN%NITYP(1:),ABN%NTYP,FFM%RCUT1,ABN%POSION(1:,1:,ABN%NCONF))
             ELSE
                MNEIB_EST1=1
             ENDIF
             IF(FFM%W2.GT.0.0_q) THEN
                CALL SET_SIZE_EST (PAR_SUP_HANDLE%COMM_WORLD,ABN%A(1:,1:,ABN%NCONF), &
                                   ABN%B(1:,1:,ABN%NCONF),ABN%LADD(1:),ABN%LNIONS(1:,1:), &
                                   ABN%MIONS,ABN%MITYP,MNEIB_EST2,ABN%MTYP,ABN%NIONS, &
                                   ABN%NITYP(1:),ABN%NTYP,FFM%RCUT2,ABN%POSION(1:,1:,ABN%NCONF))
             ELSE
                MNEIB_EST2=1
             ENDIF
             CALL ALLOCATE_NEAREST_NEIGHBOR_MANYBODY
...
! Do actual calculations
          IF (FF%ISTART.EQ.2 .AND. FF%LFAST) THEN
             CALL FMAT_NEW_MB3(ASA,AB,ABN,PAR_SUP_HANDLE,MLSHM,FF,FFM,LHEAD,LNIONS_EST1,LNIONS_EST2, &
                     LNIONS_EST_MAX,MCOL_EATOM, &
                     MNEIB_EST1,MNEIB_EST2,MNEIB_EST_MAX, &
                     MROW_C00,MROW_EATOM, &
                     MROW_EST1,MROW_EST2,MROW_EST_MAX, &
                     MROW_PS,ABN%NIONS,NB_MAX,NNEIB_EST1,NNEIB_EST2,NNEIB_EST_MAX,NORMALIZATION_XYZ1,NORMALIZATION_XYZ2, &
                     XYZ_EST1,XYZ_EST2,XYZ_EST1_NORM,XYZ_EST2_NORM,XYZ_EST_MAX,EATOM_HELP, &
                     ENERGY_COUPLE_HELP,ENERGY_HELP,HEAT_HELP1,HEAT_HELP2, &
                     RMEM_HELP1,RMEM_HELP2,ABN%SF(1:ABN%NIONS,ABN%NCONF), &
                     ABN%TIFOR_MB(1:,1:,ABN%NCONF),ABN%TSIF_MB(1:,1:,ABN%NCONF))
          ...
          ELSE
! Calculate design matrix
             CALL FMAT_NEW_MB(ASA,AB,ABN,PAR_SUP_HANDLE,MLSHM,FF,FFM,LHEAD,LNIONS_EST1,LNIONS_EST2, &
                        LNIONS_EST_MAX,MCOL_EATOM,MNEIB_EST1,MNEIB_EST2,MNEIB_EST_MAX, &
                        MROW_C00,MROW_EATOM,MROW_EST1,MROW_EST2,MROW_EST_MAX, &
                        MROW_PS,ABN%NIONS,NB_MAX,NNEIB_EST1,NNEIB_EST2,NNEIB_EST_MAX,XYZ_EST1,XYZ_EST2, &
                        XYZ_EST_MAX,C00,EATOM_HELP,ENERGY_COUPLE_HELP,ENERGY_HELP,FORCE_HELP, &
                        HEAT_HELP1,HEAT_HELP2,PS,RMEM_HELP1,RMEM_HELP2, &
                        ABN%SF(1:ABN%NIONS,ABN%NCONF),STRESS_HELP)
...
! Calculate maximum and mean of spilling factor
          CALL CALCULATE_SF_MAX_AND_MEAN(FFM%LCOUPLE, FFM%LIONS_COUPLE(1:), &
                                         ABN%NIONS, ABN%SF(1:,ABN%NCONF), &
                                         ABN%SFMAX(ABN%NCONF), ABN%SFMEAN(ABN%NCONF), &
                                         ABN%SFMIN(ABN%NCONF), ABN%SFVAR(ABN%NCONF))
 
! Bayesian error estimation
! Never do it for ML_MODE=run (FF%ISTART=2) and FF%LFAST
          IF (FF%ISTART.EQ.2 .AND. FF%LFAST) THEN
          ELSE
             IF (FF%IERR.GT.0) THEN
                IF (FF%IERR_COUNTER.EQ.FF%IERR) THEN
                   CALL BAYES_ERR(ABN,PAR_SUP_HANDLE,FF,FFM, &
                              MROW_ENERGY_HELP,MROW_FORCE_HELP,MROW_STRESS_HELP, &
                              NCONF,NCONF_SYS,ABN%NIONS,ABN%NTYP, &
                              MCOL_MIONS,ENERGY_HELP,FORCE_HELP,RMEM_HELP1, &
                              STRESS_HELP,ABN%BE(1:,ABN%NCONF))
                ENDIF
             ENDIF
          ENDIF
...
!------------------------------------------------------------------------------------------------------------------
! Calculate maximum and mean of spilling factor.
! The spilling factors for each atom were previously calculated
! in FMAB_NEW_MB and FMAT_NEW_MB3
!------------------------------------------------------------------------------------------------------------------
        SUBROUTINE CALCULATE_SF_MAX_AND_MEAN(LCOUPLE, LIONS_COUPLE, NIONS, &
                                             SF,SFMAX,SFMEAN,SFMIN,SFVAR)
           IMPLICIT NONE
           LOGICAL, INTENT(IN) :: LCOUPLE
           LOGICAL, INTENT(IN) :: LIONS_COUPLE(:)
           INTEGER, INTENT(IN) :: NIONS
           REAL(q),INTENT(IN)  :: SF(:)
           REAL(q),INTENT(OUT) :: SFMAX
           REAL(q),INTENT(OUT) :: SFMEAN
           REAL(q),INTENT(OUT) :: SFMIN
           REAL(q),INTENT(OUT) :: SFVAR
! Local variables
           INTEGER             :: INIONS
! Calculations begin here
           ...
           SFMEAN = SF(1)
           DO INIONS=2, NIONS
              IF (LCOUPLE .AND. LIONS_COUPLE(INIONS)) CYCLE
              SFMAX = MAX(SFMAX,SF(INIONS))
              SFMIN = MIN(SFMIN,SF(INIONS))
              SFMEAN = SFMEAN+SF(INIONS)
           ENDDO
           SFMEAN = SFMEAN/NIONS
! Calculate variance
           SFVAR = 0.0_q
           DO INIONS=1, NIONS
              IF (LCOUPLE .AND. LIONS_COUPLE(INIONS)) CYCLE
              SFVAR = SFVAR + (SF(INIONS)-SFMEAN)**2
           ENDDO
           SFVAR=SFVAR/NIONS
        END SUBROUTINE CALCULATE_SF_MAX_AND_MEAN
...
!------------------------------------------------------------------------------------------------------------------
        SUBROUTINE FMAT_NEW_MB(ASA,AB,ABN,PAR_SUP_HANDLE,MLSHM,FF,FFM,LHEAD,LNIONS_EST1,LNIONS_EST2, &
                   LNIONS_EST_MAX,MCOL_EATOM, &
                   MNEIB_EST1,MNEIB_EST2,MNEIB_EST_MAX, &
                   MROW_C00,MROW_EATOM, &
                   MROW_EST1,MROW_EST2,MROW_EST_MAX, &
                   MROW_PS,MROW_SF,NB_MAX,NNEIB_EST1,NNEIB_EST2,NNEIB_EST_MAX, &
                   XYZ_EST1,XYZ_EST2,XYZ_EST_MAX,C00,EATOM_HELP, &
                   ENERGY_COUPLE_HELP,ENERGY_HELP,FORCE_HELP,HEAT_HELP1,HEAT_HELP2,PS, &
                   RMEM_HELP1,RMEM_HELP2,SF,STRESS_HELP)
        USE SOAP_KERNEL
        USE FORCE_FIELD_ADDITIONAL2
        USE ML_FF_STRUCT, ONLY: ASA_STUFF, ABINITIO, ABINITIONEW, PARALLEL_SUPER, ML_SHMEM, FF_PAR, FFM_PAR, ML_IO_WRITE
          IMPLICIT NONE
...
! Calculate expansion coefficients.
          CALL COMPUTE_D0CLM(ASA, FF, FFM, MLSHM, PAR_SUP_HANDLE, &
                                 ABN%LADD(1:), LNIONS_EST1, LNIONS_EST2, &
                                 ABN%LNTYP(:), MROW_EST1, MROW_EST2, ABN%MTYP, &
                                 ABN%NIONS, NNEIB_EST1, NNEIB_EST2, & 
                                 ABN%NTYP, RMEM_HELP2, RMEM_HELP3, & 
                                 XYZ_EST1, XYZ_EST2, "PRED") 
...
! Calculate 1-point correlation (radial) descriptor.
             CALL COMPUTE_D0C00(FFM, PAR_SUP_HANDLE, MLSHM, &
                                     C00, C00_COUPLE(:,:,INTYP0), &
                                     INTYP0, ABN%LADD(1:), ABN%LNIONS, &
                                     MROW_C00, MROW_C00_COUPLE, &
                                     ABN%MTYP, ABN%NITYP, &
                                     ABN%NTYP, "PRED") 
! Calculate 2-points correlation (angular) descriptor.
             CALL COMPUTE_D0PS(FFM, MLSHM, PAR_SUP_HANDLE, INTYP0, &
                                    ABN%LADD(1:), ABN%LNIONS, &
                                    MROW_PS, MROW_PS_COUPLE, &
                                    ABN%MTYP, ABN%NITYP, ABN%NTYP, &
                                    PS, PS_COUPLE(:,:,INTYP0), "PRED") 
! Normalization.
             CALL COMPUTE_NORMD0(FFM, PAR_SUP_HANDLE, C00, C00_COUPLE(:,:,INTYP0), &
                                                INTYP0, ABN%LNIONS, MROW_C00, MROW_C00_COUPLE, &
                                                MROW_PS, MROW_PS_COUPLE, ABN%NITYP, &
                                                PS, PS_COUPLE(:,:,INTYP0), &
                                                RNORM_C00, RNORM_PS, RNORM_ALL, "PRED")
! Calculate SOAP kernel.
             CALL COMPUTE_SOAP(FF, FFM, MLSHM, PAR_SUP_HANDLE, &
                                           C00, INTYP, INTYP0, LHEAD, ABN%LNIONS, &
                                           MCOL_SOAP, MROW_C00, MROW_PS, MROW_SOAP, &
                                           NB_MAX, NCOL_NITYP, ABN%NIONS, ABN%NITYP, & 
                                           ABN%NTYP, PS, SOAP1, SOAP2, &
                                           "PRED", &
                                           MROW_C00_COUPLE, MROW_PS_COUPLE, &
                                           C00_COUPLE(:,:,INTYP0), PS_COUPLE(:,:,INTYP0), &
                                           MROW_SOAP_COUPLE, MCOL_SOAP_COUPLE, &
                                           SOAP1_COUPLE, SOAP2_COUPLE)
! Calculate energy components.
             CALL COMPUTE_ENERGY_HELP(FF, FFM, PAR_SUP_HANDLE, 1, &
                                      INTYP0, ENERGY_HELP(:,1,INTYP0), &
                                      ABN%LNIONS, MROW_SOAP, NCOL_NITYP, ABN%NIONS, &
                                      ABN%NITYP, SOAP2, VEC_LOC_ENERGY, &
                                      "PRED", &
                                      MCOL_EATOM, MROW_EATOM, EATOM_HELP(:,:,INTYP0), &
                                      SOAP2_COUPLE, ENERGY_COUPLE_HELP(:,1,INTYP0))
...
! Calculate derivatives of expansion coefficients DCLM.
                ...
                CALL COMPUTE_D1CLM(ASA, FF, FFM, PAR_SUP_HANDLE, DCLM1, DCLM2, &
                                        DPS_SIC, KNIONS, KNTYP0, &
                                        ABN%LADD(:), LNIONS_EST1, LNIONS_EST2, &
                                        MCOL_DCLM1, MCOL_DCLM2, MCOL_DPS_SIC, &
                                        MROW_DCLM1, MROW_DCLM2, &
                                        MROW_DPS_SIC, MROW_EST1, MROW_EST2, &
                                        ABN%MTYP, NNEIB_EST1, NNEIB_EST2, &
                                        ABN%NTYP, RMEM_HELP2, RMEM_HELP3, &
                                        XYZ_EST1, XYZ_EST2)
! Calculate derivatives of radial descriptors DC00.
                CALL COMPUTE_D1C00(FFM, MLSHM, PAR_SUP_HANDLE, DC00, DCLM1, &
                                       KNIONS, KNTYP0, ABN%LADD(:), LNIONS_EST1, &
                                       LNIONS_EST_MAX, MCOL_DC00, MCOL_DCLM1, &
                                       MROW_DC00, MROW_DCLM1, MROW_EST1, MROW_EST_MAX, &
                                       ABN%MTYP, ABN%MTYP_ALLOCATE, &
                                       NNEIB_EST1, NNEIB_EST_MAX, ABN%NTYP)
! Calculate derivatives of angular descriptors DPS.
                CALL COMPUTE_D1PS(FFM, MLSHM, PAR_SUP_HANDLE, DCLM2, &
                                      DPS, DPS_SIC, &
                                      KNIONS, KNTYP0, ABN%LADD(:), &
                                      LNIONS_EST2, LNIONS_EST_MAX, &
                                      MCOL_DCLM2, MCOL_DPS, MCOL_DPS_SIC, &
                                      MROW_EST2, MROW_EST_MAX, MROW_DCLM2, &
                                      MROW_DPS, MROW_DPS_SIC, &
                                      ABN%MTYP, ABN%MTYP_ALLOCATE, ABN%NIONS, &
                                      NNEIB_EST2, NNEIB_EST_MAX, ABN%NTYP)
! Normalization of derivatives.
                CALL COMPUTE_NORMD1(FF, FFM, PAR_SUP_HANDLE, DC00, DPS, KNIONS, &
                                           KNTYP0, ABN%LADD, LNIONS_EST1, LNIONS_EST2, LNIONS_EST_MAX, &
                                           ABN%LNITYP, MCOL_DC00, MCOL_DPS, MCOL_RNORM_ALL, &
                                           MCOL_RNORM_C00, MCOL_RNORM_PS, FF%MIONS, ABN%MITYP, &
                                           MROW_EST1, MROW_EST2, MROW_EST_MAX, MROW_DC00, &
                                           MROW_DPS, MROW_RNORM_C00, MROW_RNORM_PS, &
                                           NNEIB_EST1, NNEIB_EST2, NNEIB_EST_MAX, ABN%NTYP, &
                                           RNORM_ALL, RNORM_C00, RNORM_PS)
...
! Size of CLM2 and DCLM2.
                ...
                MROW_CLM2_RED=(FFM%RV%LMAX2_RED+1)**2*FF%MTYP_ALLOCATE*FFM%RV%MRB2_RED
                MROW_DCLM2=(FFM%LMAX2+1)**2*FF%MTYP_ALLOCATE*FFM%MRB2
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   MROW_DCLM2_RED=(FFM%RV%LMAX2_RED+1)**2*FF%MTYP_ALLOCATE*FFM%RV%MRB2_RED
                ELSE
                   MROW_DCLM2_RED=1
                ENDIF
                MCOL_DCLM2=MNEIB_EST2+1
...
! Reduced descriptor
             ...
             ENDIF
             IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                ALLOCATE(FFM%RV%DCLM2_RED(1:3,1:MROW_DCLM2_RED,1:FF%MTYP_ALLOCATE,1:MCOL_DCLM2))
                RMEM_HELP1=RMEM_HELP1+REAL(MROW_DCLM2_RED,q)*REAL(MCOL_DCLM2,q)*REAL(FF%MTYP_ALLOCATE,q)*3*8/1.0E+06_q
             ELSE
                ALLOCATE(FFM%RV%DCLM2_RED(1:1,1:1,1:FF%MTYP_ALLOCATE,1:1))
             ENDIF
             IF(ALLOCATED(DPS)) THEN
                DEALLOCATE(DPS)
             ENDIF
...
! Reduced descriptor
             ...
             ENDIF
             IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                ALLOCATE (FFM%RV%DPS_UGANG_RED(1:FFM%RV%MMVAR2_RED,1:MCOL_DPS,0:3,1:ABN%MTYP_ALLOCATE))
                RMEM_HELP1=RMEM_HELP1+REAL(FFM%RV%MMVAR2_RED,q)*REAL(MCOL_DPS,q)*REAL(ABN%MTYP_ALLOCATE,q)*4*8/1.0E+06_q
             ELSE
                ALLOCATE (FFM%RV%DPS_UGANG_RED(1:1,1:1,0:0,1:ABN%MTYP_ALLOCATE))
             ENDIF
             IF(ALLOCATED(DPS_SIC)) THEN
                DEALLOCATE(DPS_SIC)
             ENDIF
...
! Reduced descriptor
             ...
             IF(ALLOCATED(SOAP3)) THEN
                DEALLOCATE(SOAP3)
             ENDIF
             ALLOCATE (SOAP3(1:MROW_SOAP,1:MCOL_SOAP))
             RMEM_HELP1=RMEM_HELP1+REAL(MROW_SOAP,q)*REAL(MCOL_SOAP,q)*8/1.0E+06_q
...
! Local Variables
             ...
             IF(FFM%W2.GT.0.0_q) THEN
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9)THEN
                   MLSHM%CLM2_RED(1:MROW_CLM2_RED,1:ABN%NIONS)=0.0_q
                ENDIF
                IF(FFM%LCOUPLE) THEN
...
!                   IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
!                      DO IB=1, NB_MAX
!                         MLSHM%SOAP_PS_RED(1:ABN%NIONS,IB)=0.0_q
!                      ENDDO
!                   ENDIF
!                ENDIF
!             ELSE
...
! If there is no basis sets, set spilling factor as unity.
             ...
             ELSE
                DO INITYP=1, ABN%NITYP(INTYP0)
                   ...
                   SF(INIONS)=0.0_q
                ENDDO
! Then, calculate spilling factor.
!                

                CALL DGEMM ('N','N',FFM%NB(INTYP0),NCOL_NITYP,FFM%NB(INTYP0),1.0_q,FFM%SINV_ALL(1,1,INTYP0), &
                     FFM%MB,SOAP2(1,1),MROW_SOAP,0.0_q,SOAP3(1,1),MROW_SOAP)
# 10309
...
! Calculate normalized spilling factor.
                      SF(INIONS)=MIN(1.0_q,ABS(1.0_q-DDOT(FFM%NB(INTYP0),SOAP2(1,LOCVAR_COL),1,SOAP3(1,LOCVAR_COL),1)/SF_SELF))
                   ENDIF
                ENDDO
...
! Update of Bayesian force theshold will be only 1._q if FF%ICRITERIA>0
...
! Log file output is activated here and is 1._q later.
...
! Only copies of FFM%PS and FFM%C00 are parallelized by 1
!****************************************************************************************************

        SUBROUTINE PUT_PS (TIM,PAR_SUP_HANDLE,LOGF,FF,FFM,BE,SF, &
                   LADD,LB,LNIONS,LNTYP,MCONF,MCONF_NEW,MITYP,MIONS,MIONS_NEW,MTYP, &
...
! Measure the initial times
          CALL VTIME (VPUTIM0,CPUTIM0)
! Define the error.
! Here we introduce a variable FF%SCALE_X which
! controls the extent of selected local referenc configurations.
! In the interval 0.0<FF%SCALE_X<1.0 this variable leads
! to a larger number of local reference configurations.
          IF (FF%CALGO .EQ. 0) THEN
             CERR=FF%SCALE_CTIFOR*FF%CTIFOR
          ELSE IF (FF%CALGO .EQ. 1) THEN
             CERR=FF%SCALE_CTIFOR*FF%CTIFOR*FUNIT
          ENDIF
          IF (FF%IERR .EQ. FF%IERR_COUNTER) THEN
             IF (FF%CALGO .EQ. 0) THEN
             ...
             ELSE IF (FF%CALGO .EQ. 1) THEN
                DO ICONF=1, NCONF_NEW
                   DO INIONS=1, NIONS_NEW
                      ERR(INIONS,ICONF)=SF(INIONS,ICONF)
                   ENDDO
                ENDDO
             ENDIF
          ENDIF    
             ...
             DO ICONF=1, NCONF_NEW
                ...
                DO INTYP=1, NTYP(NCONF_LOC)
                   ...
                   DO INITYP=1, NITYP(INTYP0,NCONF_LOC)
                      ...
                      LFLAG_LOC=0
                      IF (ERR(INIONS,ICONF) .GT. CERR) THEN
                         LFLAG_LOC=1
                      ENDIF
# 11748
...
! PDCOPY is necessary even for the case of 1-dimensional 1 grid.
...
! In the case of no-1 version, just call DCOPY.
# 11768
...
! we can use DCOPY for 1-dimensional 1 grid.
                DO IB=1, FFM%NB(INTYP0)
                   IF(PAR_SUP_HANDLE%CONTEXT_WORLD%NDIM.EQ.1) THEN
                      TEST_COL=MOD((IB-1)/PAR_SUP_HANDLE%DESC_SOAP_FFM%NB,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL)
                      IF(PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL.NE.TEST_COL) THEN
...
!****************************************************************************************************
        ...
        SUBROUTINE PUT_FMAT (ASA,AB,TIM,PAR_SUP_HANDLE,MLSHM,MEM1,FF,FFM,A,B,LADD,LNIONS,LNITYP,LNTYP,MCONF, &
                   MCONF_NEW,MIONS,MITYP,MTYP,NCONF,NCONF_NEW,NIONS,NITYP,NTYP,OMEGA,POSION)
          USE NEIGHBOUR
          USE SOAP_KERNEL
          USE FORCE_FIELD_ADDITIONAL2
          USE ML_FF_STRUCT, ONLY: ASA_STUFF, ABINITIO, TIME, PARALLEL_SUPER, ML_SHMEM, MEMORY, FF_PAR, FFM_PAR 
          IMPLICIT NONE
! Input variables
          TYPE (ASA_STUFF)      :: ASA
          TYPE (ABINITIO)       :: AB
...
! Calculate 1-point correlation (radial) descriptor
                  CALL COMPUTE_D0C00(FFM, PAR_SUP_HANDLE, MLSHM, C00, C00_COUPLE(:,:), &
                                           INTYP0, LADD(:,ICONF), LNIONS(:,:,ICONF), &
                                           MROW_C00, MROW_C00_COUPLE, MTYP, &
                                           NITYP(:,ICONF), NTYP(ICONF), "PUTA")
! Calculate 2-point correlation (angular) descriptor
                  CALL COMPUTE_D0PS(FFM, MLSHM, PAR_SUP_HANDLE, INTYP0, &
                                          LADD(:,ICONF), LNIONS(:,:,ICONF), MROW_PS, MROW_PS_COUPLE, &
                                          MTYP, NITYP(:,ICONF), NTYP(ICONF), PS, PS_COUPLE(:,:), "PUTA")
! Normalization
                  CALL COMPUTE_NORMD0(FFM, PAR_SUP_HANDLE, C00, C00_COUPLE(:,:), INTYP0, &
                                                      LNIONS(:,:,ICONF), MROW_C00, &
                                                      MROW_C00_COUPLE, MROW_PS, MROW_PS_COUPLE, &
                                                      NITYP(:,ICONF), PS, PS_COUPLE(:,:), &
                                                      RNORM_C00, RNORM_PS, RNORM_ALL, "PUTA")
! Calculate SOAP kernel.
                  CALL COMPUTE_SOAP(FF, FFM, MLSHM, PAR_SUP_HANDLE, C00, &
                                                 INTYP, INTYP0, LHEAD, LNIONS(:,:,ICONF), MCOL_SOAP, MROW_C00, &
                                                 MROW_PS, MROW_SOAP, NB_MAX, NCOL_NITYP, NIONS(ICONF), NITYP(:,ICONF), &
                                                 NTYP(ICONF), PS, SOAP1, SOAP2, "PUTA", &
                                                 MROW_C00_COUPLE, MROW_PS_COUPLE, &
                                                 C00_COUPLE(:,:), PS_COUPLE(:,:), &
                                                 1, 1, &
                                                 DUMMY_2D_1, DUMMY_2D_2)
! Calculate and copy energy matrix elements.
                  CALL COMPUTE_ENERGY_HELP(FF, FFM, PAR_SUP_HANDLE, ICONF, INTYP0, ENERGY_HELP(:), &
                                       LNIONS(:,:,ICONF), MROW_SOAP, NCOL_NITYP, NIONS(ICONF), &
                                       NITYP(:,ICONF), SOAP2, VEC_LOC_ENERGY, "PUTA", &
                                       1, 1, DUMMY_2D_1, &
                                       DUMMY_2D_2, DUMMY_1D_1)
               ENDDO ! End of element loop.
! Calculate derivatives of SOAP kernel
...
!                  IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
!                     DO IB=1, NB_MAX
!                        MLSHM%SOAP_PS_RED(1:FF%MIONS,IB)=0.0_q
!                     ENDDO
!                  ENDIF
!               ENDIF
!            ELSE
...
! Need to allocate as dummy for the moment
            IF (ALLOCATED(FFM%RV%DPS_UGANG_RED)) THEN
               DEALLOCATE(FFM%RV%DPS_UGANG_RED)
            ENDIF
            ALLOCATE (FFM%RV%DPS_UGANG_RED(1:1,1:1,0:0,1:AB%MTYP_ALLOCATE))
...
! Need to allocate as dummy for the moment
            IF (ALLOCATED(FFM%RV%PS_UGANG_RED)) THEN
               DEALLOCATE(FFM%RV%PS_UGANG_RED)
            ENDIF
            ALLOCATE(FFM%RV%PS_UGANG_RED(1:1,1:MCOL_PS,1:AB%MTYP_ALLOCATE))
...
! Needs to be allocated dummywise for the moment
            IF(ALLOCATED(FFM%RV%RNORM_PS_RED)) THEN
               DEALLOCATE(FFM%RV%RNORM_PS_RED)
            ENDIF
            ALLOCATE (FFM%RV%RNORM_PS_RED(1:1,1:AB%MTYP_ALLOCATE))
! Next
...
! Input variables
          ...
          IF(ALLOCATED(FFM%RV%NNVAR2_RED)) THEN
             DEALLOCATE(FFM%RV%NNVAR2_RED)
          ENDIF
          ALLOCATE (FFM%RV%NNVAR2_RED(1:FF%MTYP_ALLOCATE))
...
! Reduced 3-body descriptor PS
             IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                PAR_SUP_HANDLE%DESC_PS_FFM_RED%M=FFM%RV%MMVAR2_RED
                PAR_SUP_HANDLE%DESC_PS_FFM_RED%N=FFM%MB
...
! Covariance matrix
          IF (FF%ISTART.EQ.2 .AND. FF%LFAST) THEN
             PAR_SUP_HANDLE%DESC_CMAT_FFM%M=1
             PAR_SUP_HANDLE%DESC_CMAT_FFM%N=1
          ELSE
...
! Shared memory arrays need to be defined regardless the use of the 1.
...
! CLM1
          ...
          IF(FFM%W2.GT.0.0_q) THEN
             ...
             IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                MLSHM%MROW_CLM2_GLOBAL_RED=(FFM%RV%LMAX2_RED+1)**2*FF%MTYP*FFM%RV%MRB2_RED
                MLSHM%MCOL_CLM2_GLOBAL_RED=MLSHM%MCOL_CLM2_GLOBAL
                MLSHM%MROW_CLM2_GLOBAL_ALLOCATE_RED=(FFM%RV%LMAX2_RED+1)**2*FF%MTYP_ALLOCATE*FFM%RV%MRB2_RED
                MLSHM%MCOL_CLM2_GLOBAL_ALLOCATE_RED=MLSHM%MCOL_CLM2_GLOBAL_ALLOCATE
             ELSE
                MLSHM%MROW_CLM2_GLOBAL_RED=1
                MLSHM%MCOL_CLM2_GLOBAL_RED=1
                MLSHM%MROW_CLM2_GLOBAL_ALLOCATE_RED=1
                MLSHM%MCOL_CLM2_GLOBAL_ALLOCATE_RED=1
             ENDIF
             IF(FFM%LCOUPLE) THEN
...
! Input and output variables
          ...
          TYPE (FFM_PAR)        :: FFM
! Local variables
          INTEGER               :: CMAT_ALL_ROW
          INTEGER               :: CMAT_ALL_COL
          INTEGER               :: IERR 
# 13779
...
! Allocate
          ...
          IF(FF%LMLMB) THEN
             IF (FF%ISTART.EQ.2 .AND. FF%LFAST) THEN
                ...
                CMAT_ALL_COL = 1
             ELSE
                CMAT_ALL_ROW = FFM%MBTOT_ALLOCATE
                CMAT_ALL_COL = FFM%MBTOT_ALLOCATE
             ENDIF
...
! Allocate angular descriptor
                ...
                PAR_SUP_HANDLE%DESC_PS_FFM%IRSRC=0
                PAR_SUP_HANDLE%DESC_PS_FFM%ICSRC=0
                CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_PS_FFM%DESC, &
                     PAR_SUP_HANDLE%DESC_PS_FFM%M,PAR_SUP_HANDLE%DESC_PS_FFM%N, &
                     PAR_SUP_HANDLE%DESC_PS_FFM%MB,PAR_SUP_HANDLE%DESC_PS_FFM%NB, &
                     PAR_SUP_HANDLE%DESC_PS_FFM%IRSRC,PAR_SUP_HANDLE%DESC_PS_FFM%ICSRC, &
                     PAR_SUP_HANDLE%CONTEXT_WORLD%ICTXT,PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW, &
                     PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW, &
                     PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL, PAR_SUP_HANDLE%DESC_PS_FFM%LLD, &
                     PAR_SUP_HANDLE%DESC_PS_FFM%LOC_M,PAR_SUP_HANDLE%DESC_PS_FFM%LOC_N)
                IF(ALLOCATED(FFM%PS)) THEN
                   DEALLOCATE(FFM%PS)
                ENDIF
                ALLOCATE (FFM%PS(1:PAR_SUP_HANDLE%DESC_PS_FFM%LOC_M, &
                          1:PAR_SUP_HANDLE%DESC_PS_FFM%LOC_N,1:FF%MTYP_ALLOCATE))
                MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_M,q)* &
                             REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_N,q)* &
                             REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
                MEM1%RMEM_PS=MEM1%RMEM_PS+REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_M,q)* &
                             REAL(PAR_SUP_HANDLE%DESC_PS_FFM%LOC_N,q)* &
                             REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
                PAR_SUP_HANDLE%DESC_PS_FFM%M=FFM%MMVAR2
                PAR_SUP_HANDLE%DESC_PS_FFM%N=FFM%MB
                CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_PS_FFM%DESC, &
                     ...
                     PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL,  &
                     PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL, &
                     PAR_SUP_HANDLE%DESC_PS_FFM%LLD, &
                     PAR_SUP_HANDLE%DESC_PS_FFM%LOC_M,PAR_SUP_HANDLE%DESC_PS_FFM%LOC_N)
! Optionally allocate reduced angular descriptor
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%M=FFM%RV%MMVAR2_ALLOCATE_RED
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%N=FFM%MB_ALLOCATE
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%MB=NBLOCK_SCALAPACK
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%NB=NBLOCK_SCALAPACK
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%IRSRC=0
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%ICSRC=0
                   CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_PS_FFM_RED%DESC, &
                        ...
                        PAR_SUP_HANDLE%CONTEXT_WORLD%ICTXT,PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW, &
                        PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW, &
                        PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL, PAR_SUP_HANDLE%DESC_PS_FFM_RED%LLD, &
                        PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M,PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_N)
                   IF (ALLOCATED(FFM%RV%PS_RED)) THEN
                      DEALLOCATE(FFM%RV%PS_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%PS_RED(1:PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M, &
                             1:PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_N,1:FF%MTYP_ALLOCATE))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M,q)* &
                                REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_N,q)* &
                                REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
                   MEM1%RMEM_PS=MEM1%RMEM_PS+REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_M,q)* &
                                REAL(PAR_SUP_HANDLE%DESC_PS_FFM_RED%LOC_N,q)* &
                                REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%M=FFM%RV%MMVAR2_RED
                   PAR_SUP_HANDLE%DESC_PS_FFM_RED%N=FFM%MB
                ...
                ELSE
                   IF (ALLOCATED(FFM%RV%PS_RED)) THEN
                      DEALLOCATE(FFM%RV%PS_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%PS_RED(1:1,1:1,1:1))
                ENDIF
...
! Allocate covariance matrix
             ...
             IF (IERR.NE.0) THEN
                CALL ml_tutor%error("First initialization of covariance matrix (FFM%CMAT) failed. Exiting...")
             ENDIF
...
! FF%LMLONLY=.TRUE. and FF%ISTART=2, the design matrix is not necessary.
             IF(.NOT.FF%LMLONLY.OR.FF%ISTART.NE.2) THEN
                ...
                IF (IERR.NE.0) THEN
                   CALL ml_tutor%error("First initialization of design matrix (FFM%FMAT) failed. Game over...")
                ENDIF
...
! Allocate local SOAP matrix and its inverse
             ...
             PAR_SUP_HANDLE%DESC_SOAP_FFM%IRSRC=0
             PAR_SUP_HANDLE%DESC_SOAP_FFM%ICSRC=0
             CALL SCALAPACK_SET_DESCRIPTOR(PAR_SUP_HANDLE%DESC_SOAP_FFM%DESC, &
                  PAR_SUP_HANDLE%DESC_SOAP_FFM%M,PAR_SUP_HANDLE%DESC_SOAP_FFM%N, &
                  PAR_SUP_HANDLE%DESC_SOAP_FFM%MB,PAR_SUP_HANDLE%DESC_SOAP_FFM%NB, &
                  PAR_SUP_HANDLE%DESC_SOAP_FFM%IRSRC,PAR_SUP_HANDLE%DESC_SOAP_FFM%ICSRC, &
                  PAR_SUP_HANDLE%CONTEXT_WORLD%ICTXT,PAR_SUP_HANDLE%CONTEXT_WORLD%ME_ROW,PAR_SUP_HANDLE%CONTEXT_WORLD%ME_COL,  &
                  PAR_SUP_HANDLE%CONTEXT_WORLD%NP_ROW,PAR_SUP_HANDLE%CONTEXT_WORLD%NP_COL, PAR_SUP_HANDLE%DESC_SOAP_FFM%LLD, &
                  PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N)
             ...
             MEM1%RMEM(2)  =MEM1%RMEM(2)  +REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,q) * &
                  REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,q)*REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
             MEM1%RMEM_SOAP=MEM1%RMEM_SOAP+REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,q) * &
                  REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,q)*REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
             IF(ALLOCATED(FFM%SINV)) THEN
                DEALLOCATE(FFM%SINV)
             ENDIF
             ALLOCATE (FFM%SINV(1:PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,1:PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,1:FF%MTYP_ALLOCATE))
             MEM1%RMEM(2)  =MEM1%RMEM(2)  +REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,q) * &
                  REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,q)*REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
             MEM1%RMEM_SINV=MEM1%RMEM_SINV+REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_M,q) * &
                  REAL(PAR_SUP_HANDLE%DESC_SOAP_FFM%LOC_N,q)*REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
! Allocate regression coefficients.
...
! negligibly small.
             ...
             IF(ALLOCATED(FFM%RV%LFLAG_VAR_RED)) THEN
                DEALLOCATE(FFM%RV%LFLAG_VAR_RED)
             ENDIF
             ALLOCATE (FFM%RV%LFLAG_VAR_RED(1:FFM%RV%MRB2_RED,1:FFM%RV%MRB2_RED, &
                       0:FFM%RV%LMAX2_RED,1:FF%MTYP_ALLOCATE,1:FF%MTYP_ALLOCATE,1:FF%MTYP_ALLOCATE))
             MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%RV%MRB2_RED,q)**2*REAL((FFM%RV%LMAX2_RED+1),q)*REAL(FF%MTYP_ALLOCATE,q)**3/1.0E+06_q
             IF(ALLOCATED(FFM%LFLAG_VAR_SIC)) THEN
                DEALLOCATE(FFM%LFLAG_VAR_SIC)
             ENDIF
             IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4) THEN
                ALLOCATE (FFM%LFLAG_VAR_SIC(1:MAX(FFM%MRB2,FFM%RV%MRB2_RED),1:MAX(FFM%MRB2,FFM%RV%MRB2_RED), &
                          0:MAX(FFM%LMAX2,FFM%RV%LMAX2_RED),1:FF%MTYP_ALLOCATE,1:FF%MTYP_ALLOCATE))
             ELSE
             ...
             MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%MRB2,q)**2*REAL((FFM%LMAX2+1),q)*REAL(FF%MTYP_ALLOCATE,q)**3/1.0E+06_q
             IF(ALLOCATED(FFM%RV%LVAR_RED)) THEN
                DEALLOCATE(FFM%RV%LVAR_RED)
             ENDIF
             ALLOCATE (FFM%RV%LVAR_RED(1:FFM%RV%MRB2_RED,1:FFM%RV%MRB2_RED, &
                       0:FFM%RV%LMAX2_RED,1:FF%MTYP_ALLOCATE,1:FF%MTYP_ALLOCATE,1:FF%MTYP_ALLOCATE))
             MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%RV%MRB2_RED,q)**2*REAL((FFM%RV%LMAX2_RED+1),q)*REAL(FF%MTYP_ALLOCATE,q)**3/1.0E+06_q
...
! FFM%RV%NRB2_RED needs to be allocated even if FFM%W2 is set to 0.
             IF(ALLOCATED(FFM%RV%NRB2_RED)) THEN
                DEALLOCATE(FFM%RV%NRB2_RED)
             ENDIF
             ALLOCATE (FFM%RV%NRB2_RED(0:FFM%RV%LMAX2_RED))
             MEM1%RMEM(2)=MEM1%RMEM(2)+REAL((FFM%RV%LMAX2_RED+1),q)/1.0E+06_q
...
! Weight for each descriptor. This also needs to be allocated even when W2_FFM is set to 0.
             IF(ALLOCATED(FFM%RV%WVAR_RED)) THEN
                DEALLOCATE(FFM%RV%WVAR_RED)
             ENDIF
             ALLOCATE (FFM%RV%WVAR_RED(1:FFM%RV%MRB2_RED,1:FFM%RV%MRB2_RED, &
                       0:FFM%RV%LMAX2_RED,1:FF%MTYP_ALLOCATE,1:FF%MTYP_ALLOCATE,1:FF%MTYP_ALLOCATE))
             MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%RV%MRB2_RED,q)**2*REAL((FFM%RV%LMAX2_RED+1),q)*REAL(FF%MTYP_ALLOCATE,q)**3*8/1.0E+06_q
             IF(FFM%W1.GT.0.0_q) THEN
             ...
             IF(FFM%W2.GT.0.0_q) THEN
                ...
                MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%MSPL2,q)*5*REAL(FFM%MRB2,q)* &
                             REAL((FFM%LMAX2+1),q)*8/1.0E+06_q*REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ALLOCATED(FFM%RV%FNL2_RED)) THEN
                      DEALLOCATE(FFM%RV%FNL2_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%FNL2_RED(1:FFM%MSPL2,1:5,0:FFM%RV%LMAX2_RED,1:FFM%RV%MRB2_RED,1:FF%MTYP_ALLOCATE))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%MSPL2,q)*5*REAL(FFM%MRB2,q)* &
                                REAL((FFM%RV%LMAX2_RED+1),q)*8/1.0E+06_q*REAL(FF%MTYP_ALLOCATE,q)*8/1.0E+06_q
                ENDIF
                ...
                ENDIF
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   ALLOCATE (FFM%FWIN2(0:MAX(FFM%LMAX2,FFM%RV%LMAX2_RED),1:MAX(FFM%MRB2,FFM%RV%MRB2_RED)))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL((MAX(FFM%LMAX2,FFM%RV%LMAX2_RED)+1),q) * &
                                REAL(MAX(FFM%MRB2,FFM%RV%MRB2_RED),q)*8/1.0E+06_q
                ELSE
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ALLOCATED(FFM%RV%L_LNRB2_RED)) THEN
                      DEALLOCATE(FFM%RV%L_LNRB2_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%L_LNRB2_RED(1:FFM%RV%MLNRB2_RED))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%RV%MRB2_RED,q)*REAL((FFM%RV%LMAX2_RED+1),q)/1.0E+06_q
                ENDIF
                ...
                MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%MRB2,q)*REAL((FFM%LMAX2+1),q)/1.0E+06_q
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ALLOCATED(FFM%RV%NRB_LNRB2_RED)) THEN
                      DEALLOCATE(FFM%RV%NRB_LNRB2_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%NRB_LNRB2_RED(1:FFM%RV%MLNRB2_RED))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%RV%MRB2_RED,q)*REAL((FFM%RV%LMAX2_RED+1),q)/1.0E+06_q
                ENDIF
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ALLOCATED(FFM%RV%RB2_RED)) THEN
                      DEALLOCATE(FFM%RV%RB2_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%RB2_RED(1:FFM%NR2,1:FFM%RV%MRB2_RED,0:FFM%RV%LMAX2_RED))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%NR2,q)*REAL(FFM%RV%MRB2_RED,q)*REAL((FFM%RV%LMAX2_RED+1),q)*8/1.0E+06_q
                ENDIF
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ALLOCATED(FFM%RV%QGRID2_RED)) THEN
                      DEALLOCATE(FFM%RV%QGRID2_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%QGRID2_RED(1:FFM%RV%MRB2_RED,0:FFM%RV%LMAX2_RED))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%RV%MRB2_RED,q)*REAL((FFM%RV%LMAX2_RED+1),q)*8/1.0E+06_q
                ENDIF
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ALLOCATED(FFM%RV%RG2_RED)) THEN
                      DEALLOCATE(FFM%RV%RG2_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%RG2_RED(1:FFM%NR2))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%NR2,q)*8/1.0E+06_q
                ENDIF
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ALLOCATED(FFM%RV%RNORM2_RED)) THEN
                      DEALLOCATE(FFM%RV%RNORM2_RED)
                   ENDIF
                   ALLOCATE (FFM%RV%RNORM2_RED(0:FFM%RV%LMAX2_RED,1:FFM%RV%MRB2_RED))
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL((FFM%RV%LMAX2_RED+1),q)**2*REAL(FFM%RV%MRB2_RED,q)*8/1.0E+06_q
                ENDIF
             ENDIF
...
! Optional reduced descriptor
             IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                IF (ASSOCIATED(MLSHM%CLM2_NOSHMEM_RED)) THEN
                   DEALLOCATE(MLSHM%CLM2_NOSHMEM_RED)
                ENDIF
                ALLOCATE(MLSHM%CLM2_NOSHMEM_RED(MLSHM%MROW_CLM2_GLOBAL_ALLOCATE_RED,MLSHM%MCOL_CLM2_GLOBAL_ALLOCATE_RED))
                MLSHM%CLM2_RED => MLSHM%CLM2_NOSHMEM_RED
                MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(MLSHM%MROW_CLM2_GLOBAL_ALLOCATE_RED,q)*REAL(MLSHM%MCOL_CLM2_GLOBAL_ALLOCATE_RED,q)*8/1000000_q
             ENDIF
...
!PS_ALL
             IF(FFM%W2.GT.0.0_q) THEN
                ...
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   IF (ASSOCIATED(MLSHM%PS_ALL_NOSHMEM_RED)) THEN
                      DEALLOCATE(MLSHM%PS_ALL_NOSHMEM_RED)
                   ENDIF
                   ALLOCATE(MLSHM%PS_ALL_NOSHMEM_RED(1:FFM%RV%MMVAR2_ALLOCATE_RED,1:FFM%MBTOT_ALLOCATE))
                   FFM%RV%PS_ALL_RED  => MLSHM%PS_ALL_NOSHMEM_RED
                   MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%RV%MMVAR2_ALLOCATE_RED,q)*REAL(FFM%MBTOT_ALLOCATE,q)*8/1000000_q
                   MEM1%RMEM_PS_ALL=MEM1%RMEM_PS_ALL+REAL(FFM%RV%MMVAR2_ALLOCATE_RED,q)*REAL(FFM%MBTOT_ALLOCATE,q)*8/1000000_q
                ENDIF
             ENDIF
!PS_W_ALL
             IF(FFM%W2.GT.0.0_q) THEN
...
!SOAP_PS_RED
             IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                IF (ASSOCIATED(MLSHM%SOAP_PS_NOSHMEM_RED)) THEN
                   DEALLOCATE(MLSHM%SOAP_PS_NOSHMEM_RED)
                ENDIF
                ALLOCATE(MLSHM%SOAP_PS_NOSHMEM_RED(1:MLSHM%MROW_SOAP_PS_GLOBAL_ALLOCATE,1:MLSHM%MCOL_SOAP_PS_GLOBAL_ALLOCATE))
                MLSHM%SOAP_PS_RED => MLSHM%SOAP_PS_NOSHMEM_RED
                MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(MLSHM%MROW_SOAP_PS_GLOBAL_ALLOCATE,q)*REAL(MLSHM%MCOL_SOAP_PS_GLOBAL_ALLOCATE,q)*8/1000000_q
             ENDIF
! SOAP_ALL
...
! SINV_ALL
             IF(ASSOCIATED(MLSHM%SINV_ALL_NOSHMEM)) THEN
                DEALLOCATE(MLSHM%SINV_ALL_NOSHMEM)
             ENDIF
             ALLOCATE(MLSHM%SINV_ALL_NOSHMEM(1:FFM%MB_ALLOCATE,1:FFM%MB_ALLOCATE,1:FF%MTYP))
             FFM%SINV_ALL => MLSHM%SINV_ALL_NOSHMEM
             MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(FFM%MB_ALLOCATE,q)*REAL(FFM%MB_ALLOCATE,q)*REAL(FF%MTYP,q)*8/1000000_q
             MEM1%RMEM_SINV_ALL=MEM1%RMEM_SINV_ALL+REAL(FFM%MB_ALLOCATE,q)*REAL(FFM%MB_ALLOCATE,q)*REAL(FF%MTYP,q)*8/1000000_q
!CMAT_ALL
             IF(ASSOCIATED(MLSHM%CMAT_ALL_NOSHMEM)) THEN
                DEALLOCATE(MLSHM%CMAT_ALL_NOSHMEM)
             ENDIF
             ALLOCATE(MLSHM%CMAT_ALL_NOSHMEM(1:CMAT_ALL_ROW,1:CMAT_ALL_COL))
             FFM%CMAT_ALL => MLSHM%CMAT_ALL_NOSHMEM
             MEM1%RMEM(2)=MEM1%RMEM(2)+REAL(CMAT_ALL_ROW,q)*REAL(CMAT_ALL_COL,q)*8/1000000_q
             MEM1%RMEM_CMAT_ALL=MEM1%RMEM_CMAT_ALL+REAL(CMAT_ALL_ROW,q)*REAL(CMAT_ALL_COL,q)*8/1000000_q
...
! Initialization of variable
          ...
          IF(FF%LMLMB) THEN
             ...
             IF(FFM%W2.GT.0.0_q) THEN
                ...
                FFM%RNORM2=0.0_q
                IF (FFM%DESC_TYPE.EQ.2 .OR. FFM%DESC_TYPE.EQ.4 .OR. FFM%DESC_TYPE.EQ.7 .OR. FFM%DESC_TYPE.EQ.9) THEN
                   FFM%RV%FNL2_RED=0.0_q
                   FFM%RV%PS_RED=0.0_q
                   FFM%RV%QGRID2_RED=0.0_q
                   FFM%RV%RB2_RED=0.0_q
                   FFM%RV%RG2_RED=0.0_q
                   FFM%RV%RNORM2_RED=0.0_q
                ENDIF
             ENDIF
             FFM%CMAT=0.0_q
             IF(.NOT.FF%LMLONLY.OR.FF%ISTART.NE.2) THEN
...
! Input variables
          ...
          TYPE (FF_PAR)      :: FF
          TYPE (FFM_PAR)     :: FFM
! Deallocate stuff important for fast version of code
...
!> mpi communicator of machine learning force field
           TYPE (ML_MPI_PAR), INTENT( IN ) :: COMM_WORLD
