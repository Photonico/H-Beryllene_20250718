...
! quick return if possible
      ...
      ALLOCATE(T_INFO_PRIM%ATOMOM(3*T_INFO_PRIM%NIOND),DYN_PRIM%EFOR(3,T_INFO_PRIM%NIONS))
      T_INFO_PRIM%ATOMOM=0 ; DYN_PRIM%EFOR=0

      IF (SYM%ISYM>0) THEN
         CALL INISYM(LATT_PRIM%A,DYN_PRIM%POSION,DYN_PRIM%VEL,DYN_PRIM%EFOR,T_INFO_PRIM%LSFOR, &
      ...
      DEALLOCATE(T_INFO_PRIM%ATOMOM,DYN_PRIM%EFOR)
         ...
         DO IND=1,NGVECTOR
            ...
            IF (KPT==0) THEN
               CALL vtutor%bug("internal error in KPROJ: G vector was not assigned to k-vector " // &
                  ...
                  str(W%WDES%IGZ(IND,NK)), "k-proj.F", 265)
            ENDIF
...
! store the k-point
         ...
         IF (NKPTS_IRZ>SIZE(VKPT_IRZ,2)) THEN
            CALL vtutor%bug("IBZKPT_LIST: internal error.. VKPT_IRZ is not sufficiently large", "k-proj.F", 473)
         ENDIF
