...
!     "the ZORA equation", Thesis of E. van Lenthe.)
      DO I=1,R%NMAX
!        KSI(I)=INVMC2*(RYTOEV/(RYTOEV-0.5_q*APOT(I)/CLIGHT/CLIGHT))*DPOT(I)/R%R(I)
         KSI(I)=INVMC2*(1._q/(1._q-APOT(I)/(2._q*RYTOEV)/CLIGHT/CLIGHT/2._q)**2)*DPOT(I)/R%R(I)
      ENDDO
...
!> assuming a spin quantization axis rotated by \f$ \theta \f$
!> and \f$ \phi \f$
...
!>
!> @note be aware that the storage layout with respect to m1, m2,
