...
!>  DESUM   change in bandstructure energy
!>
!> @details @ref openacc :
...
!! USE moffload
      ...
      USE hamil
      USE constant
      USE tutor, ONLY: vtutor, isError, NPAR_IALGO8
...
!----- local work arrays
      ...
      INTEGER NITER, NK, NPL, NGVECTOR, NPRO, N, M, MM, K, KK, LMDIM
      INTEGER ICOUEV, I, ISP, IWARN, IDUMP, ISPINOR, ITER, IU0, IU6
      REAL(q) DEIT, DELAST, DECELL, DEBAND, DETEST, DECEL, DECEL0
      REAL(q) DESUM, DE_ATT, DE, RMS, SLOCAL, FAKT, EKIN, WSCAL
      REAL(q) X, X2, EVALUE, FNORM, GNORM, GNORML, GAMMA
      REAL(q) ADD, BB, THETA, DCOST, SINT
      COMPLEX(q) CPT, CSCPD, CA, CB
...
!! 
    ...
    IF (W%WDES%COMM_KIN%NCPU /= W%WDES%COMM_INB%NCPU) THEN
       CALL vtutor%write(isError, NPAR_IALGO8)
    ENDIF
...
!-----------------------------------------------------------------------
      DO ISPINOR=0,WDES%NRSPINORS-1
        CALL FFTWAV(NGVECTOR,WDES1%NINDPW(1),W1%CR(1+ISPINOR*WDES1%GRID%MPLWV),W1%CPTWFP(1+ISPINOR*NGVECTOR),GRID)
      ENDDO
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
     ...
     &    CDIJ, CQIJ, SV, ISP, WSEARCH%CPTWFP)
      ...
      CALL M_sum_2(WDES%COMM_INB, DETEST, FNORM)
