...
MODULE xi
  ...
  USE local_field
  USE bse
  USE bse_driver, only: CALCULATE_BSE_ACC
  USE wpot
  USE crpa
# 25
...
CONTAINS
  ...
  SUBROUTINE RESPONSE_SET_ENCUT(ENMAX)
    USE tutor, ONLY: vtutor, isAlert, RPAENCUTGWSOFT, newArgument
    REAL(q) :: ENMAX
...
! prevent warning to pop up if no GW algorithm is used
    IF ( ABS( ENCUTGWSOFT - ENCUTGWSOFT_OLD ) > 0.01_q .AND.  &
       ...
       CALL vtutor%write(isAlert, RPAENCUTGWSOFT, newArgument(rval = [ENCUTGWSOFT_OLD]))
    ENDIF
  ...
  SUBROUTINE WRITE_RESPONSE(IU6)
    ...
    IF (IU6>=0 .AND. (LCHI.OR. LTIME_EVOLUTION .OR.LMP2.OR.LMP2KPAR.OR.LMP2NO .OR. LFCIDUMP  .OR. LRPAX .OR.  LCCSD .OR. LBRACKETST)) THEN
       WRITE(IU6,10 )  ALGO_FROM_GW(), LALL_IN_ONE, IALL_IN_ONE, NBANDS_WAVE, LFINITE_TEMPERATURE, &
            ...
            ENCUTGW, ENCUTGWSOFT, ENCUTLF, LESF_SPLINES, ESF_CONV, ESF_NINTER, LMAXMP2, &
...
10  FORMAT(' Response functions by sum over occupied states:'  / &
         ...
         '   ESF_SPLINES         =',L6,  '    tricubic spline interpolation of RPA/ACFDT electronic structure factor'/&
         '   ESF_CONV            =',F6.1,'    convergence threshhold for tricubic spline interpolation'/&
         '   ESF_NINTER          =',I6,  '    maximum refinement factor for tricubic spline interpolation'/&
...
! local
    ...
    CALL XML_INCAR('ESF_SPLINES','L',IDUM,RDUM,CDUM,LESF_SPLINES,CHARAC,N)
    CALL XML_INCAR('ESF_CONV','F',IDUM,ESF_CONV,CDUM,LDUM,CHARAC,N)
    CALL XML_INCAR('ESF_NINTER','I',ESF_NINTER,RDUM,CDUM,LDUM,CHARAC,N)
...
!*********************************************************************
  ...
  SUBROUTINE CALCULATE_XI( &
    ...
    USE setexm, ONLY : POP_XC_TYPE, SETUP_LDA_XC
    ...
    USE tutor, ONLY: vtutor, isError, isAlert, FermiFiniteTemp, NUPDOWN, NOmega32, &
        OmegaGridOmegaTL, LSPECTRALinefficient, LPOTOK, LOPTICS_SPLINES
...
! local
    ...
    REAL(q) :: NFLOAT, NFLOAT_, FSG0, RMST, DE2H
...
!
    IF (LWFROMFILE) THEN
       ...
       IF (IO%LOPTICS) THEN
          ...
          CALL LR_OPTIC( &
               KINEDEN,HAMILTONIAN,P,WDES,NONLR_S,NONL_S,W_W,LATT_CUR,LATT_INI, &
...
!
    ...
    IF( (LR_OPTIC_DONE() .OR. IO%LOPTICS) .AND. LESF_SPLINES )THEN
       CALL vtutor%write(isAlert, LOPTICS_SPLINES )
    ENDIF
...
!  hence commented out for the time being)
    ...
    ELSE IF (.NOT. LWFROMFILE) THEN
       ...
       IF (LGW0 .AND. .NOT. LscQPGW) THEN
          ...
          W_W%FERWE => W_W%FERTOT(W_W%WDES%NB_LOW:W_W%WDES%NB_TOT:W_W%WDES%NB_PAR,1:W_W%WDES%NKPTS,1:W_W%WDES%ISPIN)
          W_W%CELEN => W_W%CELTOT(W_W%WDES%NB_LOW:W_W%WDES%NB_TOT:W_W%WDES%NB_PAR,1:W_W%WDES%NKPTS,1:W_W%WDES%ISPIN)
          W_W%CELTOT=W%CELTOT
          W_W%FERTOT=W%FERTOT
          W_W%EFERMI=W%EFERMI
       ENDIF
...
! take into account all spins
          IF (IBSE==1 .OR. IBSE==2 .OR. IBSE==3) THEN
! use the new BSE driver
             CALL CALCULATE_BSE_ACC( &
                ...
                LCORBSE, LQPBSE, LTCTC)
          ELSE
! use the old BSE driver
             CALL CALCULATE_BSE( &
                IBSE,P,WDES,NONLR_S,NONL_S,W,LATT_CUR,LATT_INI, &
                T_INFO,DYN,INFO,IO,KPOINTS,SYMM,GRID,LMDIM,CQIJ, &
                WGW, SHIFT, CELTOT_HARTREE_KINETIC, CELTOT_X, &
...
! calculate potential here
       ...
       CALL OEP_GW(HAMILTONIAN, KINEDEN, NBANDSGW, W%WDES, LATT_CUR,  NONLR_S, NONL_S, W, WACC1, WACC2, LEXX, &
            LMDIM, CDIJ, CQIJ, SV, T_INFO, P, INFO%TIME, LGW0, IO%IU0, IO%IU6, &
            KPOINTS%ISMEAR, KPOINTS%SIGMA, SYMM, .NOT. LSPECTRAL , &
            INFO, XC, WGWQ, CHI0, GRID, GRIDC, GRID_SOFT, GRIDUS, C_TO_US, SOFT_TO_C, &
            CHDEN, CHTOT, DENCOR, CVTOT, CSTRF, IRDMAX, CRHODE, N_MIX_PAW, MIX%AMIX, RHOLM )
       IF (IO%IU6>=0) WRITE(IO%IU6,130)
...
!=======================================================================
    IF (SHOULD_WRITE_POT(IO%LH5, IO%WRT_POTENTIAL) .AND. LOEP) THEN
...
!*********************************************************************
     ...
     SUBROUTINE CALCULATE_QPBSE()
       ...
       REAL(q), ALLOCATABLE :: A(:,:)                  ! local structure that allows to print (1._q,0._q) band at the time
...
! now perform a Hilbert transform to obtain self-energy from it's spectral representation
         CALL M_sum_d(WDES%COMM_KINTER, SEFROMBSE%REAL_PART(1,1,1,1), SIZE(SEFROMBSE%REAL_PART))
         CALL M_sum_d(WDES%COMM_KINTER, SEFROMBSE%IMAG_PART(1,1,1,1), SIZE(SEFROMBSE%IMAG_PART))
...
! gK: commented this out, can be 1._q by simple postprocessing from self-energy
!     IF (IO%IU6>=0 .AND. NELM==NELMGW_COUNT) THEN
!       DO ISP=1, SEFROMBSE%ISPIN
!         DO NK=1, S2E%NUMBER_OF_NQ ! SEFROMBSE%NKPTS
!           A = 0.0_q
...
! all-in-(1._q,0._q) mode selected
    ELSE
...
!**********************************************************************
  ...
  SUBROUTINE XI_DATAKE( WGW, LATT_CUR, ENCUT, ENCUTSOFT)
    USE fock
    USE constant
    USE esf, ONLY: SQUEEZED_COULOMB_KERNEL
...
! F(NOMEGA)=F(NOMEGA)+LIND_REAL(Q,CHI%OMEGA(NOMEGA))
          ...
          F=F/NINTER**3
       ELSE
          CALL vtutor%bug("internal error in XI_LIND: HFRCUT==0 not yet implemented", "chi.F", 4327)
       ENDIF
...
! CHI_WORK(1:NP,1:NP)=MATMUL(CHI_WORK(1:NP,1:NP),CHI%RESPONSEFUN(1:NP,1:NP,NOMEGA))
       ...
       ENDIF
    ENDDO
! TODO UINIT: On multiple cores only rank 0 has non-(0._q,0._q) CHI%OMEGA. The
! subroutine will be executed with arguments (IU0, 0, 0, 1, 1) producing
! a division by (0._q,0._q) in chi_base.F:4124 (Co_CRPA_proj). This is normally
...
! CELTOT_X=(0.0_q, 0.0_q) ! Breaks bulk_CoO_wurzite_SOC_G0W0_sym (gnu), may be used uninitialized in FOCK_FWD.
...
! ) for the all-in-(1._q,0._q) new RPA code, in order to perform the DFT
...
SUBROUTINE PUSH_XC_TYPE_FOR_GW(XC)
  USE prec
  USE chi_glb, ONLY : LCHI
  USE fock_glb, ONLY : LRSCOR,MODEL_GW
  USE setexm_struct_def, ONLY : xc_info
  USE setexm, ONLY : PUSH_XC_TYPE
  IMPLICIT NONE
  ...
  IF (LCHI) THEN
     MODEL_GW=0
     XC_DATA_TEMP=XC
     XC_DATA_TEMP%ALDAX=0._q
     XC_DATA_TEMP%AGGAX=0._q
     XC_DATA_TEMP%AGGAC=0._q
     XC_DATA_TEMP%AMGGAX=0._q
     XC_DATA_TEMP%AMGGAC=0._q
     XC_DATA_TEMP%AEXX=1._q
...
! correlation functional as it was supplied by the user
        XC_DATA_TEMP%ALDAC=XC%ALDAC
        CALL PUSH_XC_TYPE(XC,XC_DATA_TEMP)
     ELSE
        XC_DATA_TEMP%ALDAC=0._q
        CALL PUSH_XC_TYPE(XC,XC_DATA_TEMP)
     ENDIF
  ENDIF
