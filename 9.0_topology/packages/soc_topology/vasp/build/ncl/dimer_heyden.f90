...
!*********************************************************************
      ...
      MODULE dimer_heyden
        ...
        TYPE dimer_input
          REAL(q) :: stepsize                                 !< finite difference for translation
          REAL(q) :: minrot                                   !< minimal allowed rotation of dimer
          REAL(q) :: maxstep                                  !< maximum step length for translation
          INTEGER :: SWITCH                                   !
          INTEGER :: engine                                   !< optimization engine (1-SD,2-CG)
          INTEGER :: restartCG                                !< restart CG algorithm every **
          INTEGER :: findiff                                  !< finite differences (1-forward,2-central)
          LOGICAL :: Fletcher_Reeves                          !
          LOGICAL :: dimerdynamics                            !< if true dynamics instead of relaxation
          LOGICAL :: LIDM_SELECTIVE                           !< consider on atoms from "reaction core" in the dimer definision?
        END TYPE

        TYPE dimer_data
          REAL(q),POINTER :: R0_NDa(:),R1_NDa(:),R2_NDa(:)     !< positions
          REAL(q),POINTER :: F0_NDa(:),F1_NDa(:),F2_NDa(:)     !< forces
          REAL(q),POINTER :: F0_dagger(:)
          REAL(q),POINTER :: FN(:)                             !< net force
          REAL(q) :: EPOT0,EPOT1,EPOT2                         !< potential energiesdynMat
          REAL(q),POINTER :: N_ND(:)                           !< unit vector along R1R2
          REAL(q),POINTER :: N_T(:)                            !< direction for translation
          REAL(q),POINTER :: THETA(:)                          !< rotational axis
          REAL(q) :: delta_R                                   !< R1R0 distance
          REAL(q) :: curve                                     !< curvature in dimer direction
          REAL(q) :: curve1                                    !< curvature in dimer direction
          REAL(q) :: dcurve_psi0                               !< derivative of the rotation curvature
          REAL(q) :: dcurve_psi1                               !< derivative of the rotation curvature
          LOGICAL :: crisis
...
!INCLUDE "dimer_heyden.inc"
  ...
  1250 FORMAT(X,3(F18.16,X))
          ...
          IF (DYN%ISIF>2) THEN
              CALL vtutor%error("ERROR: only atomic relaxation is supported, change ISIF to 2, 1, or 0")
          END IF
...
!WRITE(IO%IU6,1241) "Midpoint calculations"
            ...
            IF (SUM(Dimer_Dat%N_ND**2)<1e-4) THEN
              CALL vtutor%error("ERROR: missing or invalid vector defining dimer")
            END IF
...
!> read vector from DIMERDIRECTION - no longer needed as this
!> is read directly from POSCAR
...
!c SWITCH
          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'SWITCH', Dimer_Inp%SWITCH, IERR, WRITEXMLINCAR)
...
!c finite differences (1-forward,2-central)
          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'FINDIFF', Dimer_Inp%FINDIFF, IERR, WRITEXMLINCAR)
...
!c distance between images (A)
          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'DIMER_DIST', Dimer_Dat%DELTA_R, IERR, WRITEXMLINCAR)
...
!c step size for the numerical differenciation
          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'STEP_SIZE', Dimer_Inp%STEPSIZE, IERR, WRITEXMLINCAR)
          IF (IERR==3) Dimer_Inp%stepsize=5e-3
          IF (Dimer_Inp%stepsize<0._q) Dimer_Inp%stepsize=5e-3

!c maximal step length for tranlsation
          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'STEP_MAX', Dimer_Inp%MAXSTEP, IERR, WRITEXMLINCAR)
          IF (IERR==3) Dimer_Inp%maxstep=1e-1
          IF (Dimer_Inp%maxstep<0._q) Dimer_Inp%maxstep=1e-1

!c minimal allowed rotation of dimer
          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'MINROT', Dimer_Inp%MINROT, IERR, WRITEXMLINCAR)
          IF (IERR==3) Dimer_Inp%minrot=1e-2
          Dimer_Inp%minrot=ABS(Dimer_Inp%minrot)

!c
          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'ENGINE', Dimer_Inp%ENGINE, IERR, WRITEXMLINCAR)
          IF (IERR==3) Dimer_Inp%engine=2
          IF (Dimer_Inp%engine<1 .OR. Dimer_Inp%engine>2) Dimer_Inp%engine=2

          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'RESTARTCG', Dimer_Inp%RESTARTCG, IERR, WRITEXMLINCAR)
          IF (IERR==3) Dimer_Inp%restartCG=10
          IF (Dimer_Inp%restartCG<1) Dimer_Inp%restartCG=10

          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'FLETCHER_REEVES', Dimer_Inp%FLETCHER_REEVES, IERR, WRITEXMLINCAR)
          IF (IERR==3) Dimer_Inp%FLETCHER_REEVES=.FALSE.

          CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LIDM_SELECTIVE', Dimer_Inp%LIDM_SELECTIVE, IERR, WRITEXMLINCAR)
...
!> transforms three-column format into (1._q,0._q)-column vector

        SUBROUTINE THREETOONE(N,TVECTOR,M,OVECTOR)
          USE prec
          INTEGER :: i,j,N,M         !< M>=N!!!
          REAL(q) :: TVECTOR(3,N)    !< vector in three-column format
          REAL(q) :: OVECTOR(M)   !< vector in (1._q,0._q)-column format
...
!> transforms (1._q,0._q)-column format into three-column vector
        SUBROUTINE ONETOTHREE(N,TVECTOR,OVECTOR)
          USE prec
          INTEGER :: i,j,N
          REAL(q) :: TVECTOR(3,N)   !< vector in three-column format
          REAL(q) :: OVECTOR(3*N)   !< vector in (1._q,0._q)-column format
