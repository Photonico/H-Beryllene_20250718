...
!
    ...
    NC=C%NCPU+1
    ALLOCATE(GRID%RC_IN%TBL(NA1), GRID%RC_IN%PTR(NC), GRID%RC_IN%N(NC), RCV_TBL1(NA1))
    CALL REGISTER_ALLOCATE(8._q*(SIZE(GRID%RC_IN%TBL)+SIZE(GRID%RC_IN%PTR)+SIZE(GRID%RC_IN%N)), "fftplans")

    NA2=MAX(GRID%IN%NALLOC,1)     ! can receive only number of IN-data
    ALLOCATE(GRID%RC_IN%TBLI(NA2), GRID%RC_IN%PTRI(NC), GRID%RC_IN%NI(NC))
    CALL REGISTER_ALLOCATE(8._q*(SIZE(GRID%RC_IN%TBLI)+SIZE(GRID%RC_IN%PTRI)+SIZE(GRID%RC_IN%NI)) , "fftplans")

! can send only number    of RC-data
    GRID%RC_IN%N=0
    GRID%RC_IN%NI=0

    ALLOCATE(GRID%IN_RL%TBL(NA2), GRID%IN_RL%PTR(NC), GRID%IN_RL%N(NC), RCV_TBL2(NA2))

    CALL REGISTER_ALLOCATE(8._q*(SIZE(GRID%IN_RL%TBL)+SIZE(GRID%IN_RL%PTR)+SIZE(GRID%IN_RL%N)), "fftplans")

    NA3=MAX(GRID%RL_FFT%NALLOC,1)     !  can receive onle number of RL-data
    ALLOCATE(GRID%IN_RL%TBLI(NA3), GRID%IN_RL%PTRI(NC), GRID%IN_RL%NI(NC))

    CALL REGISTER_ALLOCATE(8._q*(SIZE(GRID%IN_RL%TBLI)+SIZE(GRID%IN_RL%PTRI)+SIZE(GRID%IN_RL%NI)) , "fftplans")
...
! number of send data must be equal to data on local node
    IF (I1 /=  GRID%RC%NP) THEN
       CALL vtutor%bug("internal error 1: MAPSET " // str(I1) // " " // str(GRID%RC%NP), "fft_comm.F", 207)
    ENDIF
    IF (I2 /=  GRID%IN%NP) THEN
       CALL vtutor%bug("internal error 2: MAPSET " // str(I2) // " " // str(GRID%IN%NP), "fft_comm.F", 210)
    ENDIF
...
! in this case the send table should be simply 1,...,N
       DO I=1,GRID%RC_IN%N(NODE_ME)
          IF (GRID%RC_IN%TBL(I)/=I) THEN
             CALL vtutor%bug("internal error: MAPSET sendtable RC_IN incorrect " // str(I) // " " &
                // str(GRID%RC_IN%TBL(I)), "fft_comm.F", 226)
          ENDIF
       ENDDO
    ELSE
    ...
    IF (ISND==0) THEN
       ...
       DO I=1,GRID%IN_RL%N(NODE_ME)
          IF (GRID%IN_RL%TBL(I)/=I) THEN
             CALL vtutor%bug("internal error: MAPSET sendtable IN_RL incorrect " // str(I) // " " &
                // str(GRID%IN_RL%TBL(I)), "fft_comm.F", 241)
          ENDIF
       ENDDO
    ELSE
...
! (typically for LPLANE_WISE set to .TRUE. this is the case)
    ...
    DEALLOCATE(RECV%RC%I2, RECV%RC%I3, RECV%IN%I2, RECV%IN%I3, RECV%RL%I2, RECV%RL%I3)
    RETURN
...
!$ACC KERNELS PRESENT(XDAT) 
 !!$OMP PARALLEL DO DEFAULT(NONE) SHARED(MAP,COMM,NZERO,XDAT) PRIVATE(I)
...
!$ACC PARALLEL LOOP PRESENT(XDAT,SNDBUF,MAP%PTR,COMM%NCPU) 
 !$OMP PARALLEL DO DEFAULT(NONE) SHARED(MAP,COMM,SNDBUF,XDAT) PRIVATE(I)
...
!$ACC PARALLEL LOOP PRESENT(XDAT) 
 !$OMP PARALLEL DO DEFAULT(NONE) SHARED(NZERO,XDAT) PRIVATE(I)
       DO I=1,NZERO
          XDAT(I)=0
       ENDDO
 !$OMP END PARALLEL DO
...
!$ACC PARALLEL LOOP PRESENT(DEST,SRC,INDEX) 
 !$OMP PARALLEL DO DEFAULT(NONE) SHARED(SRC,DEST,INDEX,N) PRIVATE(I)
    DO I=1,N
       DEST(INDEX(I))=SRC(I)
    ENDDO
 !$OMP END PARALLEL DO
