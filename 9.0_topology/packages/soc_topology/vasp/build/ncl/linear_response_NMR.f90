...
! #define exact_diagonalisation
      ...
      COMPLEX(q), PRIVATE, SAVE :: QQ(3,3,2)
...
! setup (1._q,0._q)-centre partial wave matrix elements for current augmentation
...
! calculate induced field
      ...
      CALL M_sum_z(W%WDES%COMM,QQ,18)
...
! NICS_
         ...
         DO ISUSP=1,2
            ...
            IF     ((     LVGVAPPL).AND.(ISUSP.EQ.2)) THEN
               TMP(1:3,1:3)=TMP_MACRO(1:3,1:3)      ! use vGv
            ELSEIF ((.NOT.LVGVAPPL).AND.(ISUSP.EQ.1)) THEN
               TMP(1:3,1:3)=TMP_MACRO(1:3,1:3)      ! use pGv
            ENDIF
...
! because the current augmentation has to be 1._q in PS_BIOT_SAVART_DR the strengths have to be
...
! allocate before doubling the k-point grid
      ...
      IF (LVGVCALC) THEN
         CALL ALLOCW(W%WDES,WXI_1)   !vGv
         CALL ALLOCW(W%WDES,WXI_2)   !vGv
         CALL ALLOCW(W%WDES,WXI_3)   !vGv
      ENDIF
...
! WDES%DATAKE should point to the upper panel of NKPTS_ORIG k-points
      ...
      IF (LVGVCALC) THEN
         WXI_1%WDES=>WDES_kpq     !vGv
         WXI_2%WDES=>WDES_kpq     !vGv
         WXI_3%WDES=>WDES_kpq     !vGv
      ENDIF
...
!           CALL SPHER_DER(GRID,NONL_D,P,W%WDES,LATT_CUR,VDIR)
         ...
         qloop: DO IQ=-CHI_BARE_STENCIL,CHI_BARE_STENCIL
            ...
            IF (LVGVCALC) THEN
               WXI_1%CPTWFP=(0._q,0._q); WXI_1%CPROJ=(0._q,0._q)       ! vGv
               WXI_2%CPTWFP=(0._q,0._q); WXI_2%CPROJ=(0._q,0._q)       ! vGv
               WXI_3%CPTWFP=(0._q,0._q); WXI_3%CPROJ=(0._q,0._q)       ! vGv
            ENDIF
...
! let's start the linear response stuff to get the contribution | psi^(1e) >
            ...
            IF (LVGVCALC) THEN  !vGv
               ...
               CALL MLR_COMMUTATOR(W,GRID,NONL_S,NONLR_S,NONL_D,NONLR_D,LATT_CUR,LMDIM,CDIJ,CQIJ,INFO,1,WXI_1)
               IF (.NOT.INFO%LREAL) CALL SPHER_DER(GRID,NONL_D,P,W%WDES,LATT_CUR,2)
               CALL MLR_COMMUTATOR(W,GRID,NONL_S,NONLR_S,NONL_D,NONLR_D,LATT_CUR,LMDIM,CDIJ,CQIJ,INFO,2,WXI_2)
               IF (.NOT.INFO%LREAL) CALL SPHER_DER(GRID,NONL_D,P,W%WDES,LATT_CUR,3)
               CALL MLR_COMMUTATOR(W,GRID,NONL_S,NONLR_S,NONL_D,NONLR_D,LATT_CUR,LMDIM,CDIJ,CQIJ,INFO,3,WXI_3)
               IF (.NOT.INFO%LREAL) CALL SPHER_DER(GRID,NONL_D,P,W%WDES,LATT_CUR,VDIR)
            ENDIF
...
! write out (1._q,0._q) column of the tensor, should maybe go elsewhere
     ...
     &                                         B_NICS_OUT_PARA_PS(3,NI)
      ...
      CALL DEALLOCW(WXI)
      IF (LVGVCALC) THEN
         CALL DEALLOCW(WXI_1)     ! vGv
         CALL DEALLOCW(WXI_2)     ! vGv
         CALL DEALLOCW(WXI_3)     ! vGv
      ENDIF
...
!          CALL vtutor%error("MLR_B_QQ: ERROR: k-points are not properly doubled " // str(NKPTS_ORIG) &
!             // " " // str(W%WDES%NKPTS))
!       ENDIF
!
!       ! WGKPQ and WXI will live at k+q, their WDES remains the same except WDES%NKPTS, WDES%VKPT,
...
! fft to real space
                     DO ISPINOR=0,WDES1%NRSPINORS-1
                        CALL FFTWAV(NGVECTOR,WDES1%NINDPW(1),W0_1(NP)%CR(1+ISPINOR*GRID%MPLWV),W0_1(NP)%CPTWFP(1+ISPINOR*NGVECTOR),GRID)
                     ENDDO
                  ENDIF
               ENDIF
            ENDDO newband
               ...
               DO IC1=ISTART1,IEND1-1
                  ...
                  IF (LM2 <=0 .OR. LM2 > SIZE(JPAWRAD,2)) THEN
                     CALL vtutor%bug("internal error in CALC_B_MAGATOM_LR: index 2 into JPAWRAD " &
                        // "exceeds bounds", "linear_response_NMR.F", 3802)
                  ENDIF
...
!NO AUG                   CALL M_stop('VASP aborting ...'); stop
!NO AUG                ENDIF
...
!                    GC=(G1*LATT_CUR%B(IDIR,1)+G2*LATT_CUR%B(IDIR,2)+G3*LATT_CUR%B(IDIR,3))*TPI
                     ...
                     CSUM=WGKPQ%CPTWFP(I,NB,NK,ISP)*CONJG(W_1%CPTWFP(I,NB,NK,ISP))
!                    CSUM=WGKPQ%CPTWFP(I,NB,NK,ISP)*GC*CONJG(W%CPTWFP(I,NB,NK,ISP)) !*(0._q,1._q)  ! Gilles doesn't understand the i
               ...
               IF (IDIR.EQ.2) THEN
                  DO I  =1,W%WDES%NPLWKP(NK)
                     CSUM=WGKPQ%CPTWFP(I,NB,NK,ISP)*CONJG(W_2%CPTWFP(I,NB,NK,ISP))
                     CSUM = CSUM /(2*HSQDTM)   ! heuristic, but should be correct for units, see MLR_COMMUTATOR
                     NAB_QQ(IDIR)=NAB_QQ(IDIR)+CSUM*WEIGHT
                  ENDDO
               ENDIF
               IF (IDIR.EQ.3) THEN
                  DO I  =1,W%WDES%NPLWKP(NK)
                     CSUM=WGKPQ%CPTWFP(I,NB,NK,ISP)*CONJG(W_3%CPTWFP(I,NB,NK,ISP))
                     CSUM = CSUM /(2*HSQDTM)   ! heuristic, but should be correct for units, see MLR_COMMUTATOR
                     NAB_QQ(IDIR)=NAB_QQ(IDIR)+CSUM*WEIGHT
                  ENDDO
               ENDIF
            ...
            DO NB =1,W%WDES%NBANDS
               ...
               DO I  =1,W%WDES%NGVECTOR(NK)
                  ...
                  DO ISPINOR=0,W%WDES%NRSPINORS-1
                     ...
                     CSUM=WGKPQ%CPTWFP(II,NB,NK,ISP)*GC*CONJG(W%CPTWFP(II,NB,NK,ISP))  !*(0._q,1._q)  ! Gilles doesn't understand the i
                     NAB_QQ(IDIR)=NAB_QQ(IDIR)+CSUM*WEIGHT
                  ENDDO
               ENDDO
            ENDDO
