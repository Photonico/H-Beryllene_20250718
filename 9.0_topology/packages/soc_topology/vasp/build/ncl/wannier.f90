...
!> Compute wannier orbitals using the orbital rotation method of Silvestrelli
      MODULE wannier
      USE prec
      IMPLICIT NONE
...
!************************************************************************
      ...
      USE base
      USE reader_tags
      USE tutor, ONLY: vtutor, isError, NoWannier
      ...
      LWANNIER_=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LWANNIER', LWANNIER_, IERR, WRITEXMLINCAR)

      IF (LWANNIER_) THEN     
         CALL vtutor%write(isError, NoWannier)
      ENDIF
# 45
...
!************************************************************************
     ...
     &  T_INFO,LOVERL,LMDIM,d_IJ,IRDMAX,CVALUE)
      ...
      INTEGER LYDIM, LMYDIM, LYMAX, LDEP_INDEX, L, LM, LL, LMP, LP, LLP
      INTEGER ISP, ISP_INC, IND, INDMAX, INDPYL, INDYLM
      INTEGER NI, NT, NIP, M, MPLOW, MP
      REAL(q) RINPL
...
!**********************************************************************
      ...
      INTEGER NB, NP, NPRO, NPRO_, NT, NI, NIS
      INTEGER L, LP, LMMAXC, ISPINOR, ISPINOR_
...
!************************ SUBROUTINE WANNMAT ***************************
!
!> Calculates the following matrix elements
!>
...
! maybe we will need this in the future
      ...
      INTEGER NRPLWV_RED, NPROD_RED, NB_TOT, NBANDS, NSTRIP, NSTRIP_ACT, NSTRIP_RED
      INTEGER NDONE, N, NP, NPOS, NPOS_RED, NPL, NPL2, NPRO, NPRO_O, N1, N2
      INTEGER LMDIM, IRDMAX, ISPINOR
      REAL(q) RINPLW
...
!  Mind that all wavefunctions are redistributed at this point
      IF (NDONE/=NBANDS .OR. NPOS_RED+NSTRIP_RED/=NB_TOT+1 ) THEN
       CALL vtutor%bug("EDDIAG: fatal internal error (2): " // str(NDONE) // " " // str(NBANDS) // &
          " " // str(NPOS_RED+NSTRIP_RED) // " " // str(NB_TOT+1), "wannier.F", 746)
      ENDIF
...
! maybe we will need this in the future
      ...
      INTEGER NODE_ME, IONODE, NCPU
      INTEGER NRPLWV_RED, NPROD_RED, NB_TOT, NBANDS, NPL, NPRO
...
!>
!> N.B. the only difference between this subroutine and the us::SETYLM_AUG,
!> is that SETYLM_AUG_TRUNC also writes the arrays XS,YS and ZS
...
!***********************************************************************
     ...
     &  LMYDIM,LYDIM,YLM,IRMAX,INDMAX,N1L,N1H,N2L,N2H,N3L,N3H,XS,YS,ZS,DIST)
      ...
      INTEGER                    LMYDIM, LYDIM, IRMAX, INDMAX
      ...
      INTEGER N1LOW, N2LOW, N3LOW, N1HI, N2HI, N3HI, N1, N2, N3
      INTEGER IND, NADDR, NPSRNL
      REAL(q) F1, F2, F3, X1, X2, X3, D1, D2, D3, X, Y, Z, D
      REAL(q) ARGSC, ARG, PSDMAX

      IF ((LYDIM+1)**2 > LMYDIM) THEN
         CALL vtutor%bug("internal error: LMYDIM is too small " // str(LYDIM) // " " // str(LMYDIM), "wannier.F", 1472)
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      IF (INDMAX>IRMAX) THEN
        CALL vtutor%bug("internal ERROR: DEPLE: IRDMAX must be increased to " &
           // str(INT(INDMAX*1.1)), "wannier.F", 1550)
      ENDIF
...
!************************************************************************
     ...
     &   NIONS,POSION,MAP_,d_IJ)
      ...
      INTEGER LYDIM, LMYDIM, LYMAX, LDEP_INDEX, L, LMP, LP, LLP, LM, LL
      INTEGER NI, NT, N1L, N1H, N2L, N2H, N3L, N3H, M, MPLOW, MP
      INTEGER INDMAX, ISP_INC, ISP, INDYLM, IND, INDPYL, I
      REAL(q) RINPL
      INTEGER, EXTERNAL :: MAXL_AUG, MAXL1
