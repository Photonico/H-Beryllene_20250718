...
!    NSTRIP= MIN(NGLB4,16)
    ...
    IF (NGLB < NSTRIPV) THEN
       CALL vtutor%bug("internal error in CALCULATE_LOCAL_FIELD: NSTRIPV is larger than NGLB", "ump2kpar.F", 239)
    ENDIF
...
!****************** SUBROUTINE  ADD_MP2 *******************************
!
!> Add up the MP2 energies to the
...
! local variables
    ...
    IF (WHF%WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL vtutor%error("TWOELECTRON4O_ACC_MP2: KPAR>1 not implemented, sorry.")
    END IF
...
! is transformed corresponding to a wavevector q
    ...
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_MP2: q-point changed " // str(NQ) // " " &
          // str(NQ_), "ump2kpar.F", 896)
    ENDIF
...
# 1056
             ...
             NFLOAT=NFLOAT+8._q*NSTRIP1*NSTRIP2*NSTRIP3*NSTRIP4*H%TOTAL_ENTRIES
          ENDIF
!==========================================================================
...
!==========================================================================
          DO N1=1,NSTRIP1
             IF (N1 > SIZE(TWOELECTRON3O,4)) THEN
                CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_MP2: out of bounds 4 " // str(N1), "ump2kpar.F", 1068)
             ENDIF
             DO N2=1,NSTRIP2
                IF (N2 > SIZE(TWOELECTRON3O,3)) THEN
                   CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_MP2: out of bounds 3 " // &
                      str(N2), "ump2kpar.F", 1073)
                ENDIF
                NB1_INTO_TOT=NPOS1-1+N1
                NB2_INTO_TOT=NPOS2-1+N2
                DO N4=1,NSTRIP4
                   ...
                   IF (NB4 > SIZE(TWOELECTRON3O,2)) THEN
                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_MP2: out of bounds 2 " &
                         // str(NB4), "ump2kpar.F", 1083)
                   ENDIF
                   DO N3=1,NSTRIP3
                      NB3_INTO_TOT=CBMIN3_CYCLIC+(NPOS3-CBMIN3)+N3-1
                      NB3         =NB3_INTO_TOT-CBMIN+1
                      IF ( NB3> SIZE(TWOELECTRON3O,1)) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_MP2: out of bounds 1 " &
                            // str(NB3_INTO_TOT) // " " // str(NB3), "ump2kpar.F", 1090)
                      ENDIF
