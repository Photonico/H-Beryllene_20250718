...
# 2 "tau_mu.F" 2 
  ...
  MODULE tau_mu
    ...
    TYPE tau_handle
       ...
       COMPLEX(q),      POINTER, CONTIGUOUS :: TAUC(:)   ! kinetic energy density of the core electrons
    END TYPE tau_handle
...
! just a failsafe
      IF (.NOT.ASSOCIATED(KINEDEN%TAU)) THEN
         CALL vtutor%bug("SET_KINEDEN: internal ERROR, KIN_EDEN handle not allocated", "tau_mu.F", 230)
      ENDIF

      CALL TAU_PW(&
     &   GRID,GRID_SOFT,GRIDC,SOFT_TO_C,LATT_CUR,SYMM,NIOND,W,WDES,KINEDEN%TAU)

!! !$ACC UPDATE SELF(KINEDEN%TAU) IF(ASSOCIATED(KINEDEN%TAU).AND.OFFLOAD_ON) ASYNC(ACC_ASYNC_Q)
...
!$ACC PARALLEL LOOP PRESENT(P,GRIDC,CHTOT,CSTRF,CHDER)  &
...
!$ACC ENTER DATA CREATE(WORK,TAUFOR) 

      SCALE=1._q/(RYTOEV*AUTOA2)

!$ACC KERNELS PRESENT(TAUFOR) 
...
!$ACC& PRESENT(GRIDC,T_INFO,T_INFO%POSION,T_INFO%VCA,WORK,TAUFOR,MUTOT) 
...
!=======================================================================
      ...
      NIS=NIS+NIADD
      ENDDO typ
!$ACC EXIT DATA COPYOUT(TAUFOR) DELETE(WORK) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!=======================================================================
...
! local variables
      ...
      IF (NCDIJ==1) THEN
!$ACC PARALLEL LOOP REDUCTION(+:DE) PRESENT(DMUTOT,DKINEDEN,TAUC) 
         DO I=1,GRIDC%RL%NP
...
!$ACC PARALLEL LOOP REDUCTION(+:DE) PRESENT(DMUTOT,DKINEDEN,TAUC) 
         ...
         ENDDO
      ELSE
         CALL vtutor%bug("MUxTAU: internal error, NCDIJ= " // str(NCDIJ), "tau_mu.F", 685)
      ENDIF
...
# 755
      ...
      ALLOCATE(CFA(WDES%NRPLWV),CFAR(MPLWV),CKIN(MPLWV))
...
!$ACC ENTER DATA CREATE(CFA,CFB,CFAR,CFBR,CKIN) 

!$ACC KERNELS PRESENT(TAU) 
      TAU=0
!$ACC END KERNELS

!$ACC UPDATE SELF(W%FERWE) 
...
!$ACC KERNELS PRESENT(CFAR,CFBR,CKIN) 
...
!$ACC END KERNELS
      ...
      kpoints: DO NK=1,WDES%NKPTS
         ...
         bands: DO N=1,WDES%NBANDS
            ...
            dirs: DO IDIR=1,3
!$ACC KERNELS PRESENT(CFA,CFB) 
               CFA=0 ; IF (WDES%LNONCOLLINEAR) CFB=0
!$ACC END KERNELS
! loop over plane wave coefficients
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(WDES,LATT_CUR,W,CFA) PRIVATE(G1,G2,G3,GC) 
               DO I=1,NPL
...
! and for LNONCOLLINEAR=T repeat this procedure for the second spinor component
               IF (WDES%LNONCOLLINEAR) THEN
! loop over plane wave coefficients
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(WDES,LATT_CUR,W,CFB) PRIVATE(G1,G2,G3,GC) 
               DO I=1,NPL
! get k-vector of respective k-point and coefficient
                  G1=WDES%IGX(I,NK)+WDES%VKPT(1,NK)
                  G2=WDES%IGY(I,NK)+WDES%VKPT(2,NK)
...
!$ACC PARALLEL LOOP PRESENT(CKIN,CFBR,CFAR,GRID) 
               DO I=1,GRID%RL%NP
# 833
...
! rescaling
!!!$ACC KERNELS PRESENT(CKIN,GRID_SOFT) 
...
!$ACC EXIT DATA DELETE(CFA,CFB,CFAR,CFBR,CKIN) 
