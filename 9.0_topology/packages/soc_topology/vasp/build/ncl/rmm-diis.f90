...
!$ACC ENTER DATA CREATE(W_INI,WTMPA,WOPT) 
...
!! !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(NP,N,IPREC,LNEWB) SCHEDULE(dynamic) &
...
!! !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(NP,N,ITER) SCHEDULE(dynamic) &
!! !$OMP NUM_THREADS(omp_nthreads_acc)
       i1: DO NP=1,NSIM
...
!! !$OMP PARALLEL DO NUM_THREADS(omp_nthreads_acc) DEFAULT(SHARED) SCHEDULE(dynamic) &
!! !$OMP PRIVATE(NP,N,ITER,FNORM,CTMP,CHAM,N1,CWORK1,N2,R,CWORK,RWORK,IFAIL,OTRIAL,B,B_,IPIV,C,I) &
!! !$OMP REDUCTION(+:RMS,FBREAK_MEAN,ICOU_FBREAK)
       i2: DO NP=1,NSIM
...
!----------------------------------------------------------------------
!$ACC ENTER DATA CREATE(CTMP,CHAM,CWORK1) 
!$ACC KERNELS PRESENT(CTMP,CHAM) 
...
!$ACC PARALLEL LOOP PRESENT(CTMP,CWORK1) 
                   DO N2=N1,ITER
                      CTMP(N2,N1)=      (CWORK1(N2-N1+1))
                      CTMP(N1,N2)=(CONJG(CWORK1(N2-N1+1)))
                   ENDDO
                ENDDO buildh
!$ACC EXIT DATA COPYOUT(CTMP,CHAM) DELETE(CWORK1) 
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
...
!
!***********************************************************************
!$ACC ENTER DATA CREATE(CWORK1,CHAM,B) 
...
!    A(n2,n1)=    < phi_n2 |  ( H - e S) ( H - e S)  | phi_n1 >
                builda: DO N1=1,ITER
                   CALL W1_GEMV( (1._q,0._q), ELEMENTS( WOPT, NRES+N1, NRES+ITER, NP), ELEMENT( WOPT, NRES+N1, NP), (0._q,0._q), CWORK1, 1)
!$ACC PARALLEL LOOP PRESENT(CHAM,CWORK1) 
                   DO N2=N1,ITER
                      CHAM(N2,N1)=       (CWORK1(N2-N1+1))
                      CHAM(N1,N2)=(CONJG(CWORK1(N2-N1+1)))
...
! B(n1) = < phi_n1 |  ( H - e S) | phi_ini >
                CALL W1_GEMV( (1._q,0._q), ELEMENTS( WOPT, NRES+1, NRES+ITER, NP), ELEMENT( W_INI,  NP), (0._q,0._q), B(1), 1)
...
! overwrite B by X
                ...
                IF (IFAIL ==0) CALL ZGETRS('N', ITER, 1, CHAM, NRES, IPIV, B, NRES, IFAIL)
...
!$ACC WAIT(ACC_ASYNC_ASYNC) IF(OFFLOAD_ON)

       

!! !$OMP PARALLEL DO PRIVATE(NP,N,ITER,A2,WSCAL,A1,DE,DECEL) SCHEDULE(dynamic) &
!! !$OMP NUM_THREADS(omp_nthreads_acc) REDUCTION(+:DESUM,ICOUEV)
       i3: DO NP=1,NSIM
...
! stop if change of the (1._q,0._q)-electron energy is smaller than EBREAK
...
!$ACC EXIT DATA DELETE(WTMP(:),W1(:)) 
    ...
    CALL DELWAVA(WOPT)
    CALL DELWAVA(WTMPA)
!$ACC EXIT DATA DELETE(W_INI,WOPT,WTMPA) 
!$ACC EXIT DATA DELETE(SV,CDIJ,CQIJ,PRECON) 
