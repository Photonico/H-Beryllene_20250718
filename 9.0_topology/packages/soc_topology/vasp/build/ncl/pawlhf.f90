...
! copy valence partial waves to A
      ...
      ELSE IF (SIZE(COCC_ADD,1) /= LMMAX) THEN
         CALL vtutor%bug("internal error in CALC_LHF_ONE_CENTRE: SIZE(COCC_ADD) has changed " // &
            str(SIZE(COCC_ADD,1)) // " " // str(LMMAX), "pawlhf.F", 881)
      ENDIF
...
! check potential after division by charge density
      ...
      IF (ABS(HF_ENERGY_PS-SUM)>MIN_CHARGE*10) THEN
         CALL vtutor%bug("internal error in LHF_ONE_CENTRE: potential shift caused by negative " &
            // "charge density " // str(HF_ENERGY_PS) // " " // str(SUM), "pawlhf.F", 990)
      ENDIF
      ...
      IF (.NOT.USE_CORE .AND. ABS(HF_ENERGY_VAL-SUM)>MIN_CHARGE*10) THEN
         CALL vtutor%bug("internal error in LHF_ONE_CENTRE: potential shift caused by negative " &
            // "charge density " // str(HF_ENERGY_VAL) // " " // str(SUM), "pawlhf.F", 998)
      ENDIF
!-----------------------------------------------------------------------
...
! old version shift AE potential accordingly
            ...
            IF (ABS(-SUM-REAL(D_CORE,q))>=1E-6) THEN
               CALL vtutor%error("correction on strength radial grid not correct " // str(SUM) // " &
                  &" // str(REAL(D_CORE,q)))
            ENDIF
...
! check potential after shift
      ...
      IF (ABS(HF_ENERGY_VAL-HF_ENERGY_PS-SUM)>MIN_CHARGE*10) THEN
         CALL vtutor%bug("internal error in LHF_ONE_CENTRE: potential shift caused change in &
            &Hartree-Fock energy or the charge density is negative " // &
            str(HF_ENERGY_VAL-HF_ENERGY_PS) // " " // str(SUM), "pawlhf.F", 1066)
      ENDIF
!-----------------------------------------------------------------------
...
! because the pawlhf low level routines process the negative exchange potentials
      DO LM1 = 1, MIN( SIZE(DFOCK_LOCAL,1), LMMAX )
        DO LM2 = 1, MIN( SIZE(DFOCK_LOCAL,2), LMMAX )
          DFOCK_LOCAL( LM1, LM2 )=DFOCKPS(LM1,LM2)-DCORRPS(LM1,LM2)- (DFOCKAE(LM1,LM2)-DCORRAE(LM1,LM2))
          DLOCAL     ( LM1, LM2 )=DCORRPS(LM1,LM2)-           DCORRAE(LM1,LM2)
        ENDDO
      ENDDO
...
! local
      ...
      IF (SIZE(MAG) /= SIZE(CRHODE,3)) THEN
         CALL vtutor%bug("internal error in MAG_DIRECTION: size of MAG and CRHODE do not match " // &
            str(SIZE(MAG)) // " " // str(SIZE(CRHODE,3)), "pawlhf.F", 1213)
      ENDIF
...
! local
      ...
      IF (SIZE(CRHODE,3)/=4 .OR. SIZE(CRHODE_UP_DOWN,3)/=2) THEN
         CALL vtutor%bug("internal error in MAG_DIRECTION: size of CRHODE is inappropriate " // &
            str(SIZE(CRHODE,3)) // " " // str(SIZE(CRHODE_UP_DOWN,3)), "pawlhf.F", 1289)
      ENDIF
      IF (SIZE(CRHODE,1)/= SIZE(CRHODE_UP_DOWN,1)) THEN
         CALL vtutor%bug("internal error in MAG_DIRECTION: size of CRHODE and CRHODE_UP_DOWN do not " &
            // "match " // str(SIZE(CRHODE,1)) // " " // str(SIZE(CRHODE_UP_DOWN,1)), "pawlhf.F", 1293)
      ENDIF
...
! local
      ...
      IF (SIZE(CDIJ,3)/=4 .OR. SIZE(CDIJ_UP_DOWN,3)/=2) THEN
         CALL vtutor%bug("internal error in FULLMATRIX_FROM_UP_DOWN: size of CDIJ is inappropriate " &
            // str(SIZE(CDIJ,3)) // " " // str(SIZE(CDIJ_UP_DOWN,3)), "pawlhf.F", 1333)
      ENDIF
      IF (SIZE(CDIJ,1)/= SIZE(CDIJ_UP_DOWN,1)) THEN
         CALL vtutor%bug("internal error in FULLMATRIX_FROM_UP_DOWN: size of CDIJ and CDIJ_UP_DOWN " &
            // "do not match " // str(SIZE(CDIJ,1)) // " " // str(SIZE(CDIJ_UP_DOWN,1)), "pawlhf.F", 1337)
      ENDIF
...
!*******************************************************************
    ...
    SUBROUTINE LHF_TO_OEP(WDES, P , T_INFO, LOVERL, LCORE, LMDIM, &
      ...
      COMPLEX(q)  CRHODE(:,:,:,:)
! local variables
      INTEGER NT,NI,NDIM,NIP,ISP,IBASE,K,LMAX,NI_L
      TYPE (potcar), POINTER ::  PP
! variables required to store core wavefunctions
