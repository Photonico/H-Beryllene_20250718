...
!*********************************************************************
    ...
    SUBROUTINE YLM3LOOKUP_OLD(L,LP,LMIND)
      ...
      CALL vtutor%bug("internal ERROR: YLM3LK: look up of l= " // str(L) // " l'= " // str(LP) // &
         " was not possible", "asa.F", 458)
...
!*********************************************************************
    ...
    SUBROUTINE YLM3LOOKUP(L,LP,LMIND)
      ...
      IF (L>LMAXCG .OR. LP>LMAXCG) THEN
         CALL vtutor%bug("internal error in YLM3LOOKUP: L or LP > LMAXCG \n L, LP, LMAXCG " // &
            str(L) // " " // str(LP) // " " // str(LMAXCG), "asa.F", 479)
      ENDIF
...
!-----------------------------------------------------------------------
      IF ( UBOUND(YLM,2) < (LYDIM+1)**2) THEN
         CALL vtutor%bug("internal ERROR: SETYLM, insufficient L workspace", "asa.F", 516)
      ENDIF
! gfortran compiler bug workaround suggested by jF
      ITMP=UBOUND(YLM,1)
      IF ( ITMP < INDMAX) THEN
!     IF ( UBOUND(YLM,1) < INDMAX) THEN
         CALL vtutor%bug("internal ERROR: SETYLM, insufficient INDMAX workspace", "asa.F", 522)
      ENDIF
...
!-----------------------------------------------------------------------
      IF ( UBOUND(YLM,2) < (LYDIM+1)**2) THEN
         CALL vtutor%bug("internal ERROR: SETYLM_GRAD, insufficient L workspace", "asa.F", 650)
      ENDIF
! gfortran compiler bug workaround suggested by jF
      ITMP=UBOUND(YLM,1)
      IF ( ITMP < INDMAX) THEN
!     IF ( UBOUND(YLM,1) < INDMAX) THEN
         CALL vtutor%bug("internal ERROR: SETYLM_GRAD, insufficient INDMAX workspace", "asa.F", 656)
      ENDIF
...
! local
      ...
      IF (SIZE(YLM_NABLA_YLM,1) <(LYDIM+1)*(LYDIM+1) .OR. &
         ...
         CALL vtutor%bug("internal ERROR: SETYLM_NABLA_YLM, insufficient L workspace " // &
            str(SIZE(YLM_NABLA_YLM,1)) // " " // str(SIZE(YLM_NABLA_YLM,2)) // " " // &
            str(SIZE(YLM_NABLA_YLM,3)), "asa.F", 881)
      ENDIF
      ...
      IF (SIZE(YLM_X_YLM,1) <(LYDIM+1)*(LYDIM+1) .OR. &
         ...
         CALL vtutor%bug("internal ERROR: SETYLM_NABLA_YLM, insufficient L workspace " // &
            str(SIZE(YLM_X_YLM,1)) // " " // str(SIZE(YLM_X_YLM,2)) // " " // str(SIZE(YLM_X_YLM,3)), "asa.F", 888)
      ENDIF
...
! sanity check
      IF (SIZE(YLM_XIXJ_YLM,1) <(LYDIM+1)*(LYDIM+1) .OR. &
         ...
         CALL vtutor%error("SETYLM_LAPLA_YLM: ERROR: SETYLM_XIXJ_YLM, too small " // &
            str(SIZE(YLM_XIXJ_YLM,1)) // " " // str(SIZE(YLM_XIXJ_YLM,2)) // " " // &
            str(SIZE(YLM_XIXJ_YLM,3)))
      ENDIF
