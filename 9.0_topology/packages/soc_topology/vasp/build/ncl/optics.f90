...
! write OPTIC
         ...
         IF (IU>=0) THEN
            CALL OUTOPT(NABIJ,IDIR,NK,ISP,W%CELTOT,W%FERTOT,NB_TOT, &
                        INFO%ISPIN,WDES%NRSPINORS,KPOINTS%NKPTS,WDES%VKPT, &
...
!***********************************************************************
      ...
      DO I  =1,NPL
       ...
       DO NBP=1,WDES%NB_TOT
        CSUM=W%CPTWFP(I+ISPINOR*NPL,NB,NK,ISP)*GC*CONJG(W%CPTWFP(I+ISPINOR*NPL,NBP,NK,ISP))*(0._q,1._q)
...
!***********************************************************************
      ...
      block: DO IBLOCK=0,NPL-1,NBLK
       ...
       CALL ZGEMM('C', 'N', NB_TOT, NB_TOT,  ILENPL, (1._q,0._q), &
                  CBLOCK(1,1),  NBLK, W%CPTWFP(IADDPL+1+ISPINOR*NPL,1,NK,ISP), &
...
! all nonzero angular parts hard coded and selected by if statements
      IF (L > 3 .OR. LP > 3) RETURN             ! we cannot/do not handle l>3
      IF (L == LP+1 .OR. L == LP-1) THEN        ! it must hold Delta L = +/-1
         IF (L == 1 .AND. LP == 0) THEN         ! p-s transitions
            ALLOWED = .TRUE.
            ...
            ONEBYR_SCALE = -1._q
         ENDIF
         IF (L == 0 .AND. LP == 1) THEN         ! s-p transitions
            ALLOWED = .TRUE.
            ...
            ONEBYR_SCALE = 1._q
         ENDIF
         IF (L == 2 .AND. LP == 1) THEN         ! d-p transitions
            IF (MP == -1) THEN
               IF (M /=  1) ALLOWED = .TRUE.
            ...
            ONEBYR_SCALE = -2._q
         ENDIF
         IF (L == 1 .AND. LP == 2) THEN         ! p-d transitions
            IF (MP == -2) THEN
               IF (M /=  0) ALLOWED = .TRUE.
               IF (M == -1) ANGLE(1) = 1._q/SQRT(5._q)
               IF (M ==  1) ANGLE(2) = 1._q/SQRT(5._q)
            ...
            ONEBYR_SCALE = 2._q
         ENDIF
         IF (L == 3 .AND. LP == 2) THEN         ! f-d transitions
            IF (MP == -2) THEN
               IF (M /=  0 .AND. M /=  2) ALLOWED = .TRUE.
               IF (M == -3) ANGLE(1) = SQRT(3._q/14._q)
               IF (M == -2) ANGLE(3) = 1._q/SQRT(7._q)
               IF (M == -1) ANGLE(1) = -1._q/SQRT(70._q)
               IF (M ==  1) ANGLE(2) = -1._q/SQRT(70._q)
               IF (M ==  3) ANGLE(2) = -SQRT(3._q/14._q)
            ENDIF
            IF (MP == -1) THEN
               IF (M /=  1 .AND. M /=  3 .AND. M /= -3) ALLOWED = .TRUE.
               IF (M == -2) ANGLE(1) = 1._q/SQRT(7._q)
               IF (M == -1) ANGLE(3) = 2._q*SQRT(2._q/35._q)
               IF (M ==  0) ANGLE(2) = -SQRT(3._q/35._q)
               IF (M ==  2) ANGLE(2) = -1._q/SQRT(7._q)
            ENDIF
            IF (MP ==  0) THEN
               IF (M ==  0 .OR. M ==  1 .OR. M == -1) ALLOWED = .TRUE.
               IF (M == -1) ANGLE(2) = SQRT(6._q/35._q)
               IF (M ==  0) ANGLE(3) = 3._q/SQRT(35._q)
               IF (M ==  1) ANGLE(1) = SQRT(6._q/35._q)
            ENDIF
            IF (MP ==  1) THEN
               IF (M /=  3 .AND. M /= -1 .AND. M /= -3) ALLOWED = .TRUE.
               IF (M == -2) ANGLE(2) = 1._q/SQRT(7._q)
               IF (M ==  0) ANGLE(1) = -SQRT(3._q/35._q)
               IF (M ==  1) ANGLE(3) = 2._q*SQRT(2._q/35._q)
               IF (M ==  2) ANGLE(1) = 1._q/SQRT(7._q)
            ENDIF
            IF (MP ==  2) THEN
            ...
            ONEBYR_SCALE = -3._q
         ENDIF
         IF (L == 2 .AND. LP == 3) THEN         ! d-f transitions
            IF (MP == -3) THEN
               IF (M ==  2 .OR. M == -2) ALLOWED = .TRUE.
               IF (M == -2) ANGLE(1) = SQRT(3._q/14._q)
               IF (M ==  2) ANGLE(2) = SQRT(3._q/14._q)
            ENDIF
            IF (MP == -2) THEN
               IF (M /=  0 .AND. M /=  2) ALLOWED = .TRUE.
               IF (M == -2) ANGLE(3) = 1._q/SQRT(7._q)
               IF (M == -1) ANGLE(1) = 1._q/SQRT(7._q)
               IF (M ==  1) ANGLE(2) = 1._q/SQRT(7._q)
            ENDIF
            IF (MP == -1) THEN
               IF (M /=  1) ALLOWED = .TRUE.
               IF (M == -2) ANGLE(1) = -1._q/SQRT(70._q)
               IF (M == -1) ANGLE(3) = 2._q*SQRT(2._q/35._q)
               IF (M ==  0) ANGLE(2) = SQRT(6._q/35._q)
               IF (M ==  2) ANGLE(2) = 1._q/SQRT(70._q)
            ENDIF
            IF (MP ==  0) THEN
               IF (M /=  2 .AND. M /= -2) ALLOWED = .TRUE.
               IF (M == -1) ANGLE(2) = -SQRT(3._q/35._q)
               IF (M ==  0) ANGLE(3) = 3._q/SQRT(35._q)
               IF (M ==  1) ANGLE(1) = -SQRT(3._q/35._q)
            ENDIF
            IF (MP ==  1) THEN
               IF (M /= -1) ALLOWED = .TRUE.
               IF (M == -2) ANGLE(2) = -1._q/SQRT(70._q)
               IF (M ==  0) ANGLE(1) = SQRT(6._q/35._q)
               IF (M ==  1) ANGLE(3) = 2._q*SQRT(2._q/35._q)
               IF (M ==  2) ANGLE(1) = -1._q/SQRT(70._q)
            ENDIF
            IF (MP ==  2) THEN
...
! write out header of file OPTIC on I/O unit IU
      ...
      SUBROUTINE OUTOPT(NABIJ,IDIR,NK,ISP,CELEN,FERWE,NB_TOT,ISPIN,NRSPINORS, &
...
!          ETMP(N)=0.5_q*REAL(CELEN(N,NK,IS),KIND=q)/RYTOEV
          ...
          FTMP(N)=(3-MAX(ISPIN,NRSPINORS))*FERWE(N,NK,IS)
         ENDDO
...
!       EIG =0.5_q*REAL(CELEN(N,NK,ISP),KIND=q)/RYTOEV
       ...
       FEIG=(3-MAX(ISPIN,NRSPINORS))*FERWE(N,NK,ISP)
