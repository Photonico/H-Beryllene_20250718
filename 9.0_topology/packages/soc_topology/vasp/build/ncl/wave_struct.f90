...
! this must not change when the number of k-point changes
     ...
     INTEGER,POINTER :: NPLWKP(:)=>NULL()  !< number of coefficients for each k-point and band per node
     INTEGER,POINTER :: NGVECTOR(:)=>NULL()!< number of G-vectors in the basis for each k-point per node
...
! (summed over nodes, doubled in the non collinear case)
     INTEGER,POINTER :: NGVECTOR_POS(:)=>NULL()!< sum of NGVECTOR up to (but not including) the current node
     INTEGER,POINTER :: NPLWKP_TOT(:)=>NULL()  !< total number of coefficients in plane wave array at each k-points
     INTEGER,POINTER :: NB_TOTK(:,:)=>NULL()!< number of bands to be calculated for each k-point and spin
! possibly smaller than NB_TOT
     INTEGER         :: NCOL       !< number of columns
     INTEGER,POINTER,CONTIGUOUS :: PL_INDEX(:,:)=>NULL() !< index a column would have in serial version
     INTEGER,POINTER,CONTIGUOUS :: PL_COL(:,:)=>NULL()!< number of plane wave in this column
     INTEGER,POINTER ::NPRO_POS(:)=>NULL() !< for each atom, start index of entries in CPROJ in serial version
     ...
     INTEGER,POINTER,CONTIGUOUS :: NINDPW(:,:)=>NULL()!< index to the FFT box for each pw comp and k-point
     LOGICAL,POINTER,CONTIGUOUS :: LUSEINV(:)=>NULL() !< for each k-point decides whether reduced G grid can be used (compare AT_GAMMA)
...
! collinear:  NRPLWV=NGDIM, noncollinear:  NRPLWV=2*NGDIM
     INTEGER NRPLWV_RED            !< first dimension of wavefunction array after data redistribution
     INTEGER NPROD                 !< first dimension of projected wave array
     INTEGER NPRO                  !< local number of elements in projected wave array
...
!< indices: plane wave, local band index, k-point
     COMPLEX(q)      ,POINTER,CONTIGUOUS:: CPROJ(:,:,:)  !< orbital character <p_alpha| phi_n,k>
...
!
!> structure required to store (1._q,0._q) orbital (no spin no bands)
!
  TYPE wavefun1
     ...
     COMPLEX(q)      , POINTER, CONTIGUOUS :: CPROJ(:)  => NULL()  !< orbital character <p_alpha| phi>
...
!
  TYPE wavefuna
     ...
     COMPLEX(q)      , POINTER, CONTIGUOUS :: CPROJ_RED(:,:) => NULL() !< orbital character redistributed
     COMPLEX(q), POINTER, CONTIGUOUS :: CR(:,:)        => NULL() !< orbital in real space
     REAL(q),    POINTER :: FERWE(:)  !< fermi-weight for each band
     REAL(q),    POINTER :: AUX  (:)  !< auxilary
     COMPLEX(q), POINTER :: CELEN(:)  !< eigenvalues
