...
! MODULE PP_DATA
!
!> Contains all the routines needed to setup the
...
!*********************** SUBROUTINE SPAWN_PP ****************************
!
!> Determine the set of ions (on the local node) that
!> are unrelated by symmetry.
!>
!> It takes only those types into account that
...
!>
!> @param NIONS_GLOBAL : total number of ions
!> @param NIONS_LOCAL  : number of ions on local node
!> @param NIRRED_IONS  : size of the set of "irreducible" ions
!
!> @param MAP2GLOBAL   : maps a local ion id number to its global counterpart
!> @param MAP2TYPE_ORIG: maps a local ion id number to the original type
!>                designation (in accordance with the POSCAR information)
!> @param MAP2TYPE     : maps the local ion id number to the type designation
!>                in the RCP datastructure
!> @param TYPE2ION     : maps a type designation in the RCP datastructure to
...
! (N.B.: if not, there is no need to spawn)
      LSPAWN=.FALSE.     
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LRELCOR', LSPAWN, IERR, WRITEXMLINCAR)
...
! read which types are submitted to core relaxation
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LRCTYPE', LSPWNTYPE, T_INFO%NTYP, IERR, WRITEXMLINCAR)
...
!************************************************************************
      ...
      ELSE
         CALL vtutor%error("SET_PP_POINTER: Error, called with LSPAWN= " // str(LSPAWN) // "\n &
            &LSPAWN_PP_DONE= " // str(LSPAWN_PP_DONE()) // "\n ION => TYPE= " // str(NI) // " => " // &
            str(MAP2TYPE(NI)))
      ENDIF
...
! MODULE CORE_REL
!
!> Contains the subroutines that enable VASP to perform
...
!
!> Initialize the core relaxation procedure (for electronic minimization).
!>
!> It is called in ELMIN, at the start of the electronic
...
!
!************************************************************************
      SUBROUTINE INIT_CORE_REL(WDES,XC,CRHODE,IU0,IU5,IU6)
...
! calculated
      LMIMICFC=.FALSE.     
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LMIMICFC', LMIMICFC, IERR, WRITEXMLINCAR)

! LUSERWIGS: do we want to match the PS partial waves at PP%RWIGS?
! If not, they are matched at the PAW sphere boundary.
      LMATCHRW=.TRUE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LMATCHRW', LMATCHRW, IERR, WRITEXMLINCAR)
      

! RADEQ: which radial equation do we use?
! Scalar relativistic, Scalar relativistic + SOC, or Dirac.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'RADEQ', INPLIN, 40, IERR, WRITEXMLINCAR)
...
! check if the pseudopotential data has been spawned
      IF (.NOT.LSPAWN_PP_DONE()) THEN
         CALL vtutor%error("INIT_CORE_REL: pseudopotential data was not spawned \n ... exiting, sorry!")
      ENDIF
...
! check if each node holds all ions
      IF (NIONS_LOCAL/=NIONS_GLOBAL) THEN
         CALL vtutor%error("INIT_CORE_REL: distribution of ions over nodes not implemented, \n i.e.,&
            & NPAR has to equal the number of processors \n ... exiting, sorry!")
      ENDIF
...
! get dimensions of CRHODE, and perform some consistency checks
      ...
      IF (ISPIN/=1.AND.ISPIN/=2.AND.ISPIN/=4) THEN
         CALL vtutor%error("INIT_CORE_REL: CRHODE wrongly dimensioned (1): " // str(ISPIN))
      ENDIF
      IF (ISPIN/=WDES%NCDIJ) THEN
         CALL vtutor%error("INIT_CORE_REL: CRHODE wrongly dimensioned (2): " // str(ISPIN) // " " // &
            str(WDES%NCDIJ))
      ENDIF
      IF (SIZE(CRHODE,2)/=LMDIM) THEN
         CALL vtutor%error("INIT_CORE_REL: CRHODE wrongly dimensioned (3): " // str(LMDIM) // " " // &
            str(SIZE(CRHODE,2)))
      ENDIF
      IF (SIZE(CRHODE,3)/=NIONS_LOCAL) THEN
         CALL vtutor%error("INIT_CORE_REL: CRHODE wrongly dimensioned (4): " // str(NIONS_LOCAL) // &
            " " // str(SIZE(CRHODE,3)))
      ENDIF
...
! restart from a previous relaxed core calculation
         CALL vtutor%error("INIT_CORE_REL: restart from previous relaxed core calculations \n is not &
            &possible... yet")
      ELSE
...
! the core ionicity might have to be adjusted
         IF (.NOT.LMIMFC()) CALL SCCORE(PP,XC)
...
!> \f[
!>       dE = \int_\Omega \tilde{V}_{eff}(r) \hat{Q}^{00}(r) dr -
...
!*********************** SUBROUTINE CORREL ******************************
!> @details
!> Called from ELMIN to perform the relaxation of
...
! determine the AE partial waves
            CALL AE_PAR_WAVE(PP,XC,NT)
! determine the PS partial waves
            CALL PS_PAR_WAVE(PP,XC,NT)
...
!*********************** SUBROUTINE CHARGE ******************************
!
!> Calculate the onsite AE and PS charge densities
...
! is target array large enough to hold these elements?
      IF (NELEMENTS>SIZE(CHARRAY)) THEN
         CALL vtutor%bug("GLUE_ARRAY: internal error, target array too small: " // str(NELEMENTS) &
            // " " // str(SIZE(CHARRAY)), "core_rel.F", 1476)
      ENDIF
! fill array
      IBASE=1
      DO NT=1,NTYP
...
! is target array large enough to hold these elements?
      IF (NELEMENTS>SIZE(CHARRAY)) THEN
         CALL vtutor%bug("SPLIT_ARRAY: internal error, target array too small: " // str(NELEMENTS) &
            // " " // str(SIZE(CHARRAY)), "core_rel.F", 1540)
      ENDIF
! split array
      IBASE=1
      DO NT=1,NTYP
      ...
      DO NT=1,NTYP
        ...
        &           PP%LMAX*3+1+3+MAX_NUM_CORE_STATES+2
         IF (COMM_BUFF_POS+ENTRY_SIZE>COMM_BUFF_SIZE+1) THEN
            CALL vtutor%error("SYNC_PP: PP communication buffer too small (1): " // &
               str(COMM_BUFF_POS+ENTRY_SIZE-1) // " " // str(COMM_BUFF_SIZE))
         ENDIF
         IF (DO_TYPE_LOCAL(NT)) THEN
! PP parameters
...
! fill PP data structure
      ...
      DO NT=1,NTYP
        ...
        &           PP%LMAX*3+1+3+MAX_NUM_CORE_STATES+2
         IF (COMM_BUFF_POS+ENTRY_SIZE>COMM_BUFF_SIZE+1) THEN
            CALL vtutor%error("SYNC_PP: PP communication buffer too small (2): " // &
               str(COMM_BUFF_POS+ENTRY_SIZE-1) // " " // str(COMM_BUFF_SIZE))
         ENDIF
...
!>
!> @note A possible adjustment of the core eigenstate occupations
!>       used to calculate final state core level shift is taken
!>       into account!
!
!************************************************************************
      SUBROUTINE SCCORE(PP,XC)
...
! calculate core charge density
         CALL ATOM(RHO,INT(PP%ZVALF_ORIG+PP%ZCORE),PP,XC,EIG,WORK)
! quick return if possible
         IF (.NOT.LUPDATE()) THEN
...
# 2019
      ...
      CALL vtutor%error("SCCORE: solution not found within " // str(MAXITER) // " attempts: MAXDIF= &
         &" // str(MAXDIF))
      RETURN
...
! \int n_c(r) V_H[n_Z] dr
      ...
      ENDDO
! Exc[rhoc]
      CALL RAD_CORE_XC(PP%R,XC,PP%RHOAE,DEXC)
# 2123
...
!
!************************************************************************
      SUBROUTINE ATOM(RHO,Z,PP,XC,EIG,RHOC)
...
! check grid consistency
      IF (SIZE(RHO,1)<PP%R%NMAX.OR.SIZE(RHO,1)/=SIZE(RHOC)) THEN
         CALL vtutor%error("ATOM: Grid inconsistency: " // str(SIZE(RHO,1)) // " " // str(SIZE(RHOC)) &
            // " " // str(PP%R%NMAX))
      ENDIF      
! initialize
      V=0; DV=0 ;EIG=0; RHOC=0
! get effective potential
      CALL POT(RHO,Z,PP%R,V,XC)
...
!*********************** FUNCTION LMULT *********************************
!
!> Returns the occupation of the core state (N,L),
...
!
!> Recalculate the AE partial waves PP\%WAE
!>
!> It updates:
!>       -  PP\%V00_AE        : spherical component of total
!>                            onsite AE potential
...
!>
!> @note the partial waves are calculated at the energies
!>       PP\%E(:)+PP\%AVERAGEPOT(1)
!
!************************************************************************
      SUBROUTINE AE_PAR_WAVE(PP,XC,ITYP)
...
# 2490
      ...
      CALL POT(RHO,Z,PP%R,V,XC)
!     CALL POT(RHO,Z,PP%R,V,XC,add_gga=.false.)
! take the spherical component
      SCALE=1/(2*SQRT(PI))      
      V00(:)=V(:,1,1)*SCALE
! store spherical potential
...
! update PP%POTAE (spherical part of valence only AE potential)
      ...
      RHO(:,1,1)=PP%RHOAE00(:)
      V=0
      CALL POT(RHO,0,PP%R,V,XC,PP%RHOAE)
!     CALL POT(RHO,0,PP%R,V,XC,PP%RHOAE,add_gga=.false.)
...
# 3444
      ...
      SUBROUTINE PS_PAR_WAVE(PP,XC,ITYP)
...
! setup the potential
      ...
      RHO(:,1,1)=PP%RHOPS00(:)
      V=0
      CALL POT(RHO,0,PP%R,V,XC,PP%RHOPS,PP%POTPSC)
!     CALL POT(RHO,0,PP%R,V,XC,PP%RHOPS,PP%POTPSC,add_gga=.false.)
! take the spherical component
      SCALE=1/(2*SQRT(PI))      
      V00(:)=V(:,1,1)*SCALE
      AVERAGEPOT=0
...
!*********************** SUBROUTINE PSSOLVE *****************************
!
!> This subroutine solves the homogenous schroedinger-equation
!> ~~~
!> (T+V-E) |\phi_loc>  =   0
!> ~~~
!> and the inhomogenous differential equations
!> ~~~
!> (T+V-E) |\phi_i>    = |\beta_i>
!> ~~~
!************************************************************************
...
! perform some checks on the input
      IF (SIZE(V)<R%NMAX) THEN
         CALL vtutor%error("PAR_WAVE_FKT: Grid inconsistency (1): " // str(R%NMAX) // " " // str(SIZE(V)))
      ENDIF
      IF (PRESENT(INH)) THEN
         IF (SIZE(INH)<R%NMAX) THEN
            CALL vtutor%error("PAR_WAVE_FKT: Grid inconsistency (2): " // str(R%NMAX) // " " // &
               str(SIZE(INH)))
         ENDIF
      ENDIF     
...
!>
!> @param[in] RHO(:,:,:) charge density, in (charge,magnetization) format
!> @param[in] Z          nuclear charge
!> @param[in] R          grid
!> @param[in] RHOC(:)    partial core density (optional)
!> @param[in] POTC(:)    frozen core potential (optional)
!>
!> @param[out] V(:,:,:)   potential, in (charge,magnetization) format
!************************************************************************
      SUBROUTINE POT(RHO,Z,R,V,XC,RHOC,VC,EXC,ADD_GGA)
...
! in this case non spherical contributions are missing
      ...
      ADDGGA=(XC%LUSE_LIBXC.OR.XC%LDOGGA.OR.XC%LDOMETAGGA)
...
! get dimensions and perform some checks
      ...
      IF (((LMAX+1)*(LMAX+1))/=SIZE(RHO,2)) THEN
         CALL vtutor%error("POT: LMAX and the 2nd dimension of RHO do not match: " // &
            str((LMAX+1)*(LMAX+1)) // " " // str(SIZE(RHO,2)))
      ENDIF
      ...
      ISPIN=SIZE(RHO,3)
      IF (ISPIN/=1.AND.ISPIN/=2.AND.ISPIN/=4) THEN
         CALL vtutor%error("POT: ISPIN /= 1,2, or 4: " // str(ISPIN))
      ENDIF
      NMAX=SIZE(RHO,1)
      IF (NMAX<R%NMAX) THEN
         CALL vtutor%error("POT: Grid inconsistency (1): " // str(R%NMAX) // " " // str(NMAX))
      ENDIF
      IF (PRESENT(RHOC)) THEN
         IF (SIZE(RHOC)<R%NMAX) THEN
            CALL vtutor%error("POT: Grid inconsistency (2): " // str(R%NMAX) // " " // str(NMAX) // &
               " " // str(SIZE(RHOC)))
         ENDIF
      ENDIF
      IF (PRESENT(VC)) THEN
         IF (SIZE(VC)<R%NMAX) THEN
            CALL vtutor%error("POT: Grid inconsistency (3): " // str(R%NMAX) // " " // str(NMAX) // &
               " " // str(SIZE(VC)))
         ENDIF
      ENDIF
...
! add partial core charge if present
 ...
 lda: IF (TLDA) THEN
         IF (ISPIN==1) THEN
            CALL RAD_LDA_XC(R,XC,TREL,LMAX,WORK2(:,1),WORK1(:,:,1),V(:,:,1),DEXC,DVXC,.TRUE.)
         ELSE
            CALL RAD_LDA_XC_SPIN(R,XC,TREL,LMAX,WORK2,WORK1,V,DEXC,DVXC,.TRUE.)
         ENDIF
      ENDIF lda
!========================================================================
! GGA if required
...
! gga: IF (ISGGA()) THEN
  gga: IF (ADDGGA) THEN
         IF (ISPIN==1) THEN
            CALL RAD_GGA_XC(R,XC,TLDA,WORK2(:,1),WORK1(:,1,1),V(:,1,1),DEXC_GGA,DVXC_GGA)
         ELSE
            IF (PRESENT(RHOC)) THEN
               DO K=1,R%NMAX
            ...
            ENDIF
! add partial core charge if present
            CALL RAD_GGA_XC_SPIN(R,XC,TLDA,WORK2,WORK1(:,1,:),V(:,1,:),DEXC_GGA,DVXC_GGA)
         ENDIF
      ENDIF gga
!========================================================================
! 1._q
...
!>  where
!> ~~~
!> C(LM,ll',mm') = \int Y_LM Y_lm Y_l'm' d Omega
!> ~~~
...
!
!>  Set up the compensation charges on the radial grid,
...
# 4834
      ...
      DO NT=1,NTYP
         ...
         NI=TYPE2ION(NT)
         PP=>SET_PP_POINTER(NI)      
         CALL GET_ECORE_TEST(PP,XC)
      ENDDO
        
      RETURN
      END SUBROUTINE RCPOSTPROC

      SUBROUTINE GET_ECORE_TEST(PP,XC)
      USE prec
      USE constant
      USE pseudo
      USE radial
      ...
      CALL ATOM(RHO,INT(PP%ZVALF_ORIG+PP%ZCORE),PP,XC,EIG,WORK1)
! \int n_c(r) (V_H[n_v+n_Zc]+V_xc[n_v+n_c]) dr
      DBLC=0
      DO I=1,PP%R%NMAX
...
! \int n_c(r) V_H[n_Z] dr
      ...
      ENDDO
! Exc[rhoc]
      CALL RAD_CORE_XC(PP%R,XC,WORK1,DEXC)
# 4937
...
!
!> Calculate the average electrostatic potential at
...
!************************************************************************
!>     Factor a real(q) matrix by gaussian elimination
...
!>
!> @param[out]        ipvt    integer(n)
!>                an integer vector of pivot indices.
!>
!> @param[out]        rcond   real(q)
...
!>                the order of the matrix  a .
!
!> @param[in]          ipvt    integer(n)
!>                the pivot vector from dgeco or dgefa.
!
!> @param[in]          work    real(q)(n)
!>                work vector.  contents destroyed.
!
!> @param[in]          job     integer
...
! ------------------------------------------------------------------
!
!>     Solve the double precision system
!>     a * x = b  or  'C'(a) * x = b
!>     using the factors computed by #dgeco or #dgefa.
!>
!>
...
!     on entry
!
!> @param[in]       a       double precision(lda, n)
!>                the output from dgeco or dgefa.
!
!> @param[in]        lda     integer
!>                the leading dimension of the array  a .
...
!     on return
!
!> @param[out]        b       the solution vector  x.
!
! ------------------------------------------------------------------
