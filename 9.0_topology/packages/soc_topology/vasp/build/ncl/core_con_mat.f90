...
! than (1._q,0._q) as a last resort
...
! set number of n,l channels for core WF (in principle we need (1._q,0._q), but to keep structure we
! calculate all)
...
! should be (0._q,0._q) if first atom is chosen
...
! if Frequency below tolerance set epsilon_imag to (0._q,0._q)
...
! reduction is over all nodes, this should be fine
      CALL M_sum_z(WDES%COMM,EPSDD(1,1,1),CH_NEDOS*3*3)
