...
! If the number of any basis set equals (0._q,0._q), the machine cannot generate the force field.
...
!****************************************************************************************************
        ...
        SUBROUTINE PUT_ABN_DATA (FF,PAR_SUP_HANDLE,TIM,AB,ABN, &
                   CTIFOR_FF,IERR_FF,IERR_COUNTER_FF, &
                   IERR,IFF_FF,IFF_OLD_FF,ISTART_FF, &
                   LABORT,LDOUB_FF,LFMAT_FF,LGENFF_FF,LLIST_FF, &
                   LOVER_FF,LPS_FF,LSAMPLE_FF,LSIG_FF,LSOAP_FF,NSTEP,SIG_FFM,FORCING_UPDATE)
...
! Input variables
          ...
          REAL(q), INTENT(IN)    :: CTIFOR_FF ! Criteria for force
          INTEGER, INTENT(IN)    :: IERR_FF,IERR_COUNTER_FF
...
! Input or output variables.
          ...
          LOGICAL, INTENT(INOUT) :: LLIST_FF ! If LLIST_FF=.TRUE., the machine generates lists
! storing the location of the design-matrix elements.
...
! Calculate Bayesian errors
          ...
          ELSE IF (FF%CALGO .EQ. 1) THEN
             CERR=CTIFOR_FF * FUNIT
          ENDIF

          IF (IERR_FF .EQ. IERR_COUNTER_FF) THEN
             IF (FF%CALGO .EQ. 0) THEN
                ERR=ABN%BEMAX(2,ABN%NCONF)
             ELSE IF (FF%CALGO .EQ. 1) THEN
                ERR=ABN%SFMAX(ABN%NCONF)
             ENDIF
          ENDIF
...
! Local variable
          ...
          IF(ALLOCATED(ABN%SF)) THEN
             DEALLOCATE(ABN%SF)
          ENDIF
          ALLOCATE(ABN%SF(1:ABN%MIONS,1:ABN%MCONF))
          ABN%SF = 0.0_q
          RMEM_LOC=RMEM_LOC+REAL(ABN%MCONF,q)*REAL(ABN%MIONS,q)*8/1.0E+06_q
          IF(ALLOCATED(ABN%SFMAX)) THEN
             DEALLOCATE(ABN%SFMAX)
          ENDIF
          ALLOCATE(ABN%SFMAX(1:ABN%MCONF))
          ABN%SFMAX = 0.0_q
          RMEM_LOC=RMEM_LOC+REAL(ABN%MCONF,q)*8/1.0E+06_q
          IF(ALLOCATED(ABN%SFMEAN)) THEN
             DEALLOCATE(ABN%SFMEAN)
          ENDIF
          ALLOCATE(ABN%SFMEAN(1:ABN%MCONF))
          ABN%SFMEAN = 0.0_q
          IF(ALLOCATED(ABN%SFMIN)) THEN
             DEALLOCATE(ABN%SFMIN)
          ENDIF
          ALLOCATE(ABN%SFMIN(1:ABN%MCONF))
          ABN%SFMIN = 0.0_q
          RMEM_LOC=RMEM_LOC+REAL(ABN%MCONF,q)*8/1.0E+06_q
          IF(ALLOCATED(ABN%SFVAR)) THEN
             DEALLOCATE(ABN%SFVAR)
          ENDIF
          ALLOCATE(ABN%SFVAR(1:ABN%MCONF))
          ABN%SFVAR = 0.0_q
