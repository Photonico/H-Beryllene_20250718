...
!****************************************************************************************************
      ...
      MODULE MPI_DATA
        ...
        SUBROUTINE M_divide_shmem(COMM_WORLD,COMM,N,COMM_SHMEM,COMM_ZERO)
           USE ml_ff_c2f_interface
           USE ML_FF_STRUCT, ONLY: ML_MPI_PAR
           USE ml_ff_tutor, ONLY: ml_tutor
           IMPLICIT NONE
           TYPE (ML_MPI_PAR) :: COMM_WORLD,COMM,COMM_SHMEM
           TYPE (ML_MPI_PAR) :: COMM_ZERO
           TYPE (ML_MPI_PAR) :: COMM_INTRA
           INTEGER N
! local variables
...
! processors in Scalapack grid
          INTEGER, INTENT(IN)  :: NDIM_IN            ! Input dimension of 1 grid
          INTEGER, INTENT(OUT) :: NDIM_OUT           ! Input dimension of 1 grid
...
! Obtain information on (1._q,0._q) dimensional grid
...
! This subroutine sets all the necessary variables for (1._q,0._q) descriptor used by SCALAPACK.
!******************************************************************************************
        SUBROUTINE SCALAPACK_SET_DESCRIPTOR(DESC, M, N, MB, NB, IRSRC, ICSRC, ICTXT, MYROW, MYCOL, NROW, NCOL, LLD, LOC_M, LOC_N)
           ...
           INTEGER, INTENT(OUT) ::  LOC_M        ! local size of M specific to (1._q,0._q) process
           INTEGER, INTENT(OUT) ::  LOC_N        ! local size of N specific to (1._q,0._q) process
! Local variables
           INTEGER              ::  INFO
...
! This subroutine calculates local size in (1._q,0._q) dimension for a block-cyclic distributed array used in SCALAPACK
!******************************************************************************************
...
! This function calculates the local coordinate index in (1._q,0._q) dimension of a global index
...
!******************************************************************************************
        SUBROUTINE SCALAPACK_REDUCE_TO_ONE_CORE(PAR_SUP_HANDLE,DESCRIPTOR,CURRENT_ARRAY,CONTEXT_CURRENT, &
           ...
           INTEGER, INTENT(IN)        :: PROBLEM_SIZE1
           INTEGER, INTENT(IN)        :: PROBLEM_SIZE2
! Local variables
           INTEGER                    :: I
           INTEGER                    :: IDIVISION
           INTEGER                    :: IERR
...
! Set size of the problem here
!PROBLEM_SIZE(1)=DESCRIPTOR%M
!PROBLEM_SIZE(2)=DESCRIPTOR%N

! Set blocksize and descriptor for the single core descriptor
...
! since scalapack arrays use the smaller integer type
           ...
           IDIVISION=1
           HELP_M=PROBLEM_SIZE1
           HELP_N=PROBLEM_SIZE2

! Determine the size of the blocking that needs to be 1._q for PDGEMR2D
           DO 
! We need a type casting here to long integer
              ...
              IF (HELP_PRODUCT_DIMENSION.GT.LIMIT_INTEGER) THEN
                 ...
                 IF (PROBLEM_SIZE1.GE.PROBLEM_SIZE2) THEN
                    HELP_M=PROBLEM_SIZE1/IDIVISION
                 ELSE
                    HELP_N=PROBLEM_SIZE2/IDIVISION
                 ENDIF
              ELSE
                 EXIT
              ENDIF
...
! Do the remaining block size if there are any
           LDO=.FALSE.
           IF (PROBLEM_SIZE1 .GE. PROBLEM_SIZE2) THEN
              IF ((PROBLEM_SIZE1-IDIVISION*HELP_M).GT.0) THEN
                 MSTART=IDIVISION*HELP_M + 1
                 NSTART=1
                 HELP_M=PROBLEM_SIZE1 - IDIVISION*HELP_M
                 LDO=.TRUE.
              ENDIF
           ELSE
              IF ((PROBLEM_SIZE2-IDIVISION*HELP_N).GT.0) THEN
                 MSTART=1
                 NSTART=IDIVISION*HELP_N + 1
                 HELP_N=PROBLEM_SIZE2 - IDIVISION*HELP_N
                 LDO=.TRUE.
              ENDIF
           ENDIF
           IF (LDO) THEN
...
!******************************************************************************************
        ...
        SUBROUTINE SCALAPACK_DISTRIBUTE_TO_ALL_CORES (PAR_SUP_HANDLE,DESCRIPTOR,CURRENT_ARRAY, &
                   CONTEXT_CURRENT,PROBLEM_SIZE1,PROBLEM_SIZE2)
...
! Set size of the problem here
!PROBLEM_SIZE(1)=PAR_SUP_HANDLE%DESC_GENERAL_SINGLE%M
!PROBLEM_SIZE(2)=PAR_SUP_HANDLE%DESC_GENERAL_SINGLE%N
