...
! where Q is in the IRZ
    ...
    IF (WDES%NKPTS /= KPOINTS_FULL_ORIG%NKPTS) THEN
       CALL vtutor%bug("internal error in SETUP_IRZ_MAP: WDES%NKPTS needs to be identical to &
          ...
          str(KPOINTS_FULL_ORIG%NKPTS), "screened_2e.F", 95)
    ENDIF
...
!
       ...
       IF (NQ > SIZE(KPOINT_VISITED)) THEN
          CALL vtutor%bug("internal error in SETUP_IRZ_MAP: KPOINT_VISITED array too small \n this " &
             // "usually means a point does not map into the IRZ " // str(NQ), "screened_2e.F", 150)
       ENDIF
       ...
       IF (S2E%NUMBER_OF_NQ>SIZE(S2E%NQ)) THEN
          CALL vtutor%bug("internal error in SETUP_IRZ_MAP: NQ array too small", "screened_2e.F", 157)
       ENDIF
    ...
    DO K1_IN_IRZ=1, KPOINTS_ORIG%NKPTS
       ...
       ELSE
          ...
          IF (WEIGHT_SUM/=0) THEN
             CALL vtutor%bug("internal error in VASP: SETUP_IRZ_MAP weight at k-point with zero WTKPT", "screened_2e.F", 233)
          ENDIF
       ENDIF
...
!*********************************************************************
  ...
  FUNCTION BRING_TO_IRZ( VKPT1, VKPT2, LTEST)
    ...
    REAL(q) :: VKPT1(3) !< k-point in the frist Brillouin (1._q,0._q)
    REAL(q) :: VKPT2(3) !< symmetrically equivalent point in the irreducible first Brillouin (1._q,0._q)
    ...
    IF (PRESENT(LTEST)) THEN
       IF (NK_FOUND/=NK1_IN_IRZ) THEN
          CALL vtutor%bug("internal error in BRING_TO_IRZ: NK_FOUND is not idential to NK1_FULL_ORIG " &
             // str(NK_FOUND) // " " // str(NK1_IN_IRZ), "screened_2e.F", 322)
       ENDIF
    END IF
...
! maybe it would be easier to invert IROTOP and apply that to k2
    ...
    ENDDO
    IF (NK_FOUND==-1) THEN
       CALL vtutor%bug("internal error in BRING_TO_IRZ: could not find " // str(VKPT2), "screened_2e.F", 354)
    ENDIF
...
! local
    ...
    IF (NQ>SIZE(S2E%K1_IN_IRZ,1)) THEN
       CALL vtutor%bug("internal error in ENTER_IN_SCREENED_2E: NQ too large " // str(NQ), "screened_2e.F", 400)
    ENDIF
    IF (NK1>SIZE(S2E%K1_IN_IRZ,2)) THEN
       CALL vtutor%bug("internal error in ENTER_IN_SCREENED_2E: NK1 too large " // str(NK1), "screened_2e.F", 403)
    ENDIF
    ...
    IF (K1_IN_IRZ>SIZE(SCREENED_TWO_ELECTRON_INTEGRALS,2)) THEN
       CALL vtutor%bug("internal error in ENTER_IN_SCREENED_2E: K1_IN_IRZ too large " // str(K1_IN_IRZ), "screened_2e.F", 410)
    ENDIF
    ...
    IF (S2E%REQUIRED(NQ, NK1)) THEN
       ...
       IF (K2_STORE_INDEX>SIZE(SCREENED_TWO_ELECTRON_INTEGRALS,4)) THEN
          CALL vtutor%bug("internal error in ENTER_IN_SCREENED_2E: K2_IN_IRZ too large " // &
             str(K2_IN_IRZ), "screened_2e.F", 418)
       ENDIF
...
! local
    ...
    IF (NQ>SIZE(S2E%K1_IN_IRZ,1)) THEN
       CALL vtutor%bug("internal error in REQUIRED_IN_SCREENED_2E: NQ too large " // str(NQ), "screened_2e.F", 456)
    ENDIF
    IF (NK1>SIZE(S2E%K1_IN_IRZ,2)) THEN
       CALL vtutor%bug("internal error in REQUIRED_IN_SCREENED_2E: NK1 too large " // str(NK1), "screened_2e.F", 459)
    ENDIF
...
!     sizes exceeding 2^31-1 elements use now the new "M_sum_single_giant" ...
    CALL M_sum_single8(W%WDES%COMM_KINTER, SCREENED_TWO_ELECTRON_INTEGRALS, SIZE(SCREENED_TWO_ELECTRON_INTEGRALS,KIND=qi8)*2_qi8)
...
! local
    ...
    IF (SIZE(OMEGA) /= SIZE(SCREENED_TWO_ELECTRON_INTEGRALS)) THEN
       CALL vtutor%bug("internal error in SPLINE_2E: different boundaries " // str(SIZE(OMEGA)) // &
          " " // str(SIZE(SCREENED_TWO_ELECTRON_INTEGRALS)), "screened_2e.F", 839)
    ENDIF
    ...
    IF (PRESENT(OMEGAWEIGHT)) THEN
       ...
       IF (NOMEGA/=SIZE(OMEGAWEIGHT)) THEN
          CALL vtutor%bug("internal error in SET_OMEGA_GRID: OMEGAWEIGHT and OMEGA have a different " &
             // "size " // str(SIZE(OMEGA)) // " " // str(SIZE(OMEGAWEIGHT)), "screened_2e.F", 932)
       ENDIF
    ELSE
       LGAUSS=.FALSE.
    ENDIF
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in SPLINE_2E: different boundaries " // str(SIZE(OMEGARES)) &
          // " " // str(SIZE(SIGMA)), "screened_2e.F", 1302)
    ENDIF
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in INTEGRATE_W_2E_SIMPLE: different boundaries " // &
          str(SIZE(OMEGARES)) // " " // str(SIZE(SIGMA)), "screened_2e.F", 1415)
    ENDIF
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in INTEGRATE_W_2E_SPECTRAL: different boundaries " // &
          str(SIZE(OMEGARES)) // " " // str(SIZE(SIGMA)), "screened_2e.F", 1696)
    ENDIF
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in SPLINE_2E: different boundaries " // str(SIZE(OMEGARES)) &
          // " " // str(SIZE(SIGMA)), "screened_2e.F", 1962)
    ENDIF
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in INTEGRATE_W_2E_SIMPLE_C: different boundaries " // &
          str(SIZE(OMEGARES)) // " " // str(SIZE(SIGMA)), "screened_2e.F", 2109)
    ENDIF
    SIGMA=0
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in INTEGRATE_W_2E_IMAG: different boundaries " // &
          str(SIZE(OMEGARES)) // " " // str(SIZE(SIGMA)), "screened_2e.F", 2214)
    ENDIF
...
! write(3002,*)
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in INTEGRATE_W_2E_SPECTRAL_IMAG_PADE: different boundaries " &
          // str(SIZE(OMEGARES)) // " " // str(SIZE(SIGMA)), "screened_2e.F", 2348)
    ENDIF
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in INTEGRATE_W_2E_IMAG: different boundaries " // &
          str(SIZE(OMEGARES)) // " " // str(SIZE(SIGMA)), "screened_2e.F", 2432)
    ENDIF
...
!    CALL SHIFT_CELEN( W, ESHIFT)
    ...
    ALLOCATE(OMEGARES(NOMEGA_SIGMA), SIGMA(NOMEGA_SIGMA), SIGMA_IMAG(NOMEGA_SIGMA), SIGMAFROMK(NOMEGA_SIGMA), A(3,NOMEGA_SIGMA))

    DO I=1, NOMEGA_SIGMA
       OMEGARES(I)=OMEGA_SIGMA_MIN+(OMEGA_SIGMA_MAX-OMEGA_SIGMA_MIN)/(NOMEGA_SIGMA-1)*(I-1)
    ENDDO

    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
    IF (IU6>=0) WRITE(IU6,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
...
!          WRITE(200+NB1,'(3F14.7)') (OMEGARES(I),SIGMA(I),I=1,NOMEGA_SIGMA)
!          WRITE(300+NB1,'(3F14.7)') (OMEGARES(I),SIGMA_IMAG(I),I=1,NOMEGA_SIGMA)
...
! add HF contribution
          ...
          IF (IU6>=0) THEN
               ...
               WRITE(IU6,'(3F14.7)') (OMEGARES(I),SIGMA(I),I=1,NOMEGA_SIGMA,10)
               WRITE(IU6,'(1X,I6,3X,F10.4,3X,F10.5,A)') NB1,REAL( W%CELTOT(NB1,NK1,ISP) ,KIND=q) ,W%FERTOT(NB1,NK1,ISP)*W%WDES%RSPIN, &
                    '  selfenergy along imaginary axis'
               WRITE(IU6,'(3F14.7)') (OMEGARES(I),SIGMA_IMAG(I),I=1,NOMEGA_SIGMA,10)
               WRITE(IU6,*)
               A(1,:)=OMEGARES
               A(2,:)=REAL(SIGMA,q)
               A(3,:)=AIMAG(SIGMA)
               CALL XML_VECARRAY("selfenergy along real axis")
               WRITE(TEXT,"(A,I2,A,I5,1X,3F10.4,A,I5)") " band ",NB1, " k-point ", NK1, W%WDES%VKPT(:,NK1), " ISP=", ISP
               CALL XML_VECARRAY(TRIM(TEXT))
...
! local
    ...
    ALLOCATE(OMEGARES(NOMEGA_SIGMA), SIGMA(NOMEGA_SIGMA), SIGMAFROMK(NOMEGA_SIGMA), A(3,NOMEGA_SIGMA))

    DO I=1, NOMEGA_SIGMA
       OMEGARES(I)=OMEGA_SIGMA_MIN+(OMEGA_SIGMA_MAX-OMEGA_SIGMA_MIN)/(NOMEGA_SIGMA-1)*(I-1)
    ENDDO

    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') ' calculating selfenergy between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') ' complex contour integration'
    IF (IU6>=0) WRITE(IU6,'(A,2F7.2)') ' calculating selfenergy between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
...
! local
    ...
    ALLOCATE(OMEGARES(NOMEGA_SIGMA), SIGMA_IMAG(NOMEGA_SIGMA), &
      SIGMAFROMK_IMAG(NOMEGA_SIGMA), A(3,NOMEGA_SIGMA) )

    DO I=1, NOMEGA_SIGMA
       OMEGARES(I)=OMEGA_SIGMA_MIN+(OMEGA_SIGMA_MAX-OMEGA_SIGMA_MIN)/(NOMEGA_SIGMA-1)*(I-1)
    ENDDO

    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY_IMAG between i w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
    IF (IU6>=0) WRITE(IU6,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY_IMAG between i w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
...
!PK Look over all of screened_2e carefully. Likely just k-point skips and COMM_KINTER sums required.
    ...
    ALLOCATE(OMEGARES(NOMEGA_SIGMA), SIGMA_IMAG(NOMEGA_SIGMA), &
      SIGMAFROMK_IMAG(NOMEGA_SIGMA), SIGMA(NOMEGA_SIGMA), A(3,NOMEGA_SIGMA) )
...
!    CALL SHIFT_CELEN( W)
    ...
    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') 'calculating selfenergy CALC_SELFENERGY_IMAG_GAUSS between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') 'analytical continuation from complex frequencies'
    IF (IU6>=0) WRITE(IU6,'(A,2F7.2)') 'calculating selfenergy CALC_SELFENERGY_IMAG_GAUSS between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
...
! select gamma point only for the test calculations
       ...
       DO NB1=1,NBANDSGW
          ...
          SIGMA=0
          WRITE(200+NB1,'(3F14.7)') (OMEGARES(NOMEGA_REAL+I),SIGMA_IMAG(I),I=1,NOMEGA_SIGMA)

          IF (LFOCK_ADD) THEN
             SIGMA=SIGMA+CELTOT_X(NB1, NK1, ISP)
          ENDIF
          IF (IU6>=0) THEN
             WRITE(IU6,'(3F14.7)') (OMEGARES(I),SIGMA(I),I=1,NOMEGA_SIGMA)
             WRITE(IU6,*)
             A(1,:)=OMEGARES
             A(2,:)=REAL(SIGMA,q)
             A(3,:)=AIMAG(SIGMA)
...
! local
    ...
    IF (NOMEGA_SIGMA/=1) THEN
       DO I=1, NOMEGA_SIGMA
          OMEGARES(I)=OMEGA_SIGMA_MIN+(OMEGA_SIGMA_MAX-OMEGA_SIGMA_MIN)/(NOMEGA_SIGMA-1)*(I-1)
       ENDDO
    ENDIF
    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY_LINEAR between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
    IF (IU6>=0) WRITE(IU6,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY_LINEAR between w=',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
...
! local
    ...
    IF (SIZE(OMEGARES) /= SIZE(SIGMA)) THEN
       CALL vtutor%bug("internal error in SPLINE_2E: different boundaries " // str(SIZE(OMEGARES)) &
          // " " // str(SIZE(SIGMA)), "screened_2e.F", 3122)
    ENDIF
...
!>
!> @details @ref openmp :
!> the inner loop over all bands owned by a particular 1-rank (NB2)
...
! select if (1._q,0._q) or less frequencies along real axis are known
                      CALL vtutor%bug("internal error in VASP: for analytic continuation call " &
                         // "QP_SHIFT_PADE", "screened_2e.F", 3287)
...
!local
    ...
    ALLOCATE(OMEGARES(NOMEGA_SIGMA))
    ALLOCATE( A(3,NOMEGA), B(3,NOMEGA_SIGMA) )
    ALLOCATE(SIGMA_IMAG(NOMEGA), SIGMAFROMK_IMAG(NOMEGA) )
    ALLOCATE(SIGMA_SIN(NOMEGA), SIGMAFROMK_SIN(NOMEGA) )
    ALLOCATE(SIGMA_REAL(NOMEGA_SIGMA), SIGMAFROMK_REAL(NOMEGA_SIGMA) )

    DO I=1, NOMEGA_SIGMA
       OMEGARES(I)=OMEGA_SIGMA_MIN+(OMEGA_SIGMA_MAX-OMEGA_SIGMA_MIN)/(NOMEGA_SIGMA-1)*(I-1)
    ENDDO

    IF (IU0>=0) WRITE(IU0,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY_IMAG_PADE between ',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
    IF (IU6>=0) WRITE(IU6,'(A,2F7.2)') ' calculating selfenergy CALC_SELFENERGY_IMAG_PADE between ',OMEGARES(1),OMEGARES(NOMEGA_SIGMA)
...
!calculate Sigma_n'k-q(w) by pade fit of W(iw) to P(w) and then Sigma(w), correct
          ...
          ENDDO   !NB2
          CALL M_sum_z(W%WDES%COMM_INTER, SIGMA_IMAG, SIZE(SIGMA_IMAG))
          CALL M_sum_d(W%WDES%COMM_INTER, SIGMA_SIN,  SIZE(SIGMA_SIN))
          CALL M_sum_z(W%WDES%COMM_INTER, SIGMA_REAL, SIZE(SIGMA_REAL))
...
! epsilon (E_n') is the pole of the (1._q,0._q)-electron Green's function for state n'
! the structure of the the selfenergy is
! int w' W(w') G(w+w') = int w' W(w') |psi_n'><psi_n'| 1/(w+w'-E_n')
...
!here the Pade fit comes in linear version or QP-(0._q,0._q) and write out
          L_FRAC=.TRUE.
...
! local variables
      ...
      IF (.NOT. ASSOCIATED(IMAG_GRIDS%TO_FER_RE) .OR. .NOT. ASSOCIATED(IMAG_GRIDS%TO_FER_IM)) THEN
         CALL vtutor%bug("internal error in VASP: QP_SHIFT_PADE_SIGMA, TO_FER_RE or TO_FER_IM is not &
            ...
            str(ASSOCIATED(IMAG_GRIDS%TO_FER_IM)), "screened_2e.F", 4001)
      ENDIF
      ...
      IF (NOMEGA_IMAG /= SIZE(IMAG_GRIDS%TO_FER_RE,1)) THEN
         CALL vtutor%bug("internal error in VASP: QP_SHIFT_PADE_SIGMA, frequency points to not match " &
            // str(NOMEGA_IMAG) // " " // str(SIZE(IMAG_GRIDS%TO_FER_RE,1)), "screened_2e.F", 4006)
      ENDIF
