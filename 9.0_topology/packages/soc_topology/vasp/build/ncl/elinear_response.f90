...
!*********************************************************************
!
!> Calculates first derivatives of the wavefunctions with respect
!> to an external field using linear response theory
!>
!> implemented by gK\n
!> Allows to calculate the macroscopic static dielectric tensor
...
! if degenerated eigenvalue pairs have not been resolved
    ...
    INTEGER NI
    INTEGER ISP, NK, M
    REAL(q)             :: RMSC,RMSP,WEIGHT,DE2H ! mixer
    INTEGER             :: IERRBR
    REAL(q)   POL(3)
...
!   uncomment these lines if you want to include only Hartree terms
    ...
    IF (LRPA) CALL PUSH_XC_TYPE(XC,XC_DATA_NOXC)

!   (0._q,0._q) charge residuum
    RMST=0
    FBREAK_MEAN=0          ! do not apply FBREAK_MEAN in first iteration
...
! positive displacement
       ...
       CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
            ...
            CHDEN,CHTOTL,CSTRF,CVTOT1,DENCOR,SV1,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
...
! negative displacement
       ...
       CWORK=0
       CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
            INFO,XC,P,T_INFO,E2,LATT_CUR, &
            CHDEN,CHTOTL,CSTRF,CWORK,DENCOR,SV2,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
       ...
       CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, &
            ...
            E2, LASPH=INFO%LASPH, LCOREL= .FALSE.  )
!
...
! Hartree and LDA contribution
       CALL POTHAR(GRIDC,LATT_CUR,HAMILTONIAN%COULOMB_POT,CHTOT1,CWORK,E1%DENC)
       IF (.NOT.XC%LNOXC) THEN
          CALL POTXC2(GRIDC,LATT_CUR,XC,CHTOT,DENCOR,CHTOT1,CWORK,E1%XCENC)
       ELSE
          CWORK=0
          E1%XCENC=0
       ENDIF
...
! deteriorates with smaller steps
    ...
    CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
         INFO,XC,P,T_INFO,E1,LATT_CUR, &
         CHDEN,CHTOT2,CSTRF,CVTOT1,DENCOR,SV1,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
...
!*********************************************************************
!
!> Calculates the cell periodic part of -r | phi(0)_nk>
!>
!> using
!> ~~~no-highlight
...
!>
!> this is -i beta, where beta is defined in Equ. (30) of
...
! right now calling routine rotates degenerate states so small values likely preferred
    REAL(q) :: CSHIFT=0.002_q            ! complex shift should avoid too large components
...
!=======================================================================
       ...
       IF(ABS(DESUM)<INFO%EDIFF.AND.ABS(DE)<INFO%EDIFF) INFO%LABORT=.TRUE.
       IF(N>=4) THEN
          IF (ABS((RMS(N)-RMS(N-1))/RMS(N))<1E-1 .AND. ABS((RMS(N)-RMS(N-2))/RMS(N))<1E-1 .AND. ABS((RMS(N)-RMS(N-3))/RMS(N))<1E-1) INFO%LABORT=.TRUE.
       ENDIF
...
!*********************************************************************
!
!> Calculates the cell periodic part of -r | phi(0)_nk> in the
!> subspace spanned by all calculated orbitals
!>
!> using
!> ~~~no-highlight
!> -( H(0)- S(0) e(0)_nk) r | phi(0)_nk> =
...
!*********************************************************************
  ...
  SUBROUTINE LRF_RPHI0( &
          ...
          T_INFO,INFO,IO,GRID,GRIDC,GRIDUS,C_TO_US,IRDMAX, &
          CDIJ,CQIJ,SV,LMDIM,DEG_CLUSTER,IDIR,W1,LADD,LVEL)
...
!=======================================================================
    LOGICAL             :: LI=.FALSE.
!   LOGICAL             :: LI=.TRUE.
    LOGICAL             :: LHRPSI=.FALSE.

    

    IF (PRESENT(LVEL)) LHRPSI=LVEL    
...
!test resolve deg end
    IF (LHRPSI) THEN
       IF (.NOT.LNABLA) THEN
! sum_j Q_ij < d/dk p_j | phi_nk>
          W1%CPROJ=WXI%CPROJ 
          IF (LI) W1%CPROJ=W1%CPROJ*(0._q,-1.0_q)
          CALL OVERL_ALL(W1%WDES,W1,W1,.TRUE. ,LMDIM,CQIJ)
! sum_j  tau_ij < p_j | phi_nk>
          CALL SETDIJ_R(W1%WDES,GRIDC,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO,INFO%LOVERL,LMDIM,CDIJ1,IRDMAX,IDIR)
          CALL OVERL_ALL(W1%WDES,W0,W1,.FALSE.,LMDIM,CDIJ1)
          IF (LI) W1%CPROJ=W1%CPROJ*(0._q,1._q)
       ENDIF
       CALL HRPSI(W1,W0,WXI,LMDIM,CQIJ,INFO%LOVERL,.FALSE.,IO%IU0)
    ELSE
       CALL EDDIAG_LR(W1,W0,WXI,LMDIM,CQIJ,INFO%LOVERL,.FALSE.,CSHIFT,IO%IU0,DEG_CLUSTER)
    ENDIF
...
!*************************** LR_READER ********************************
!
!> Reads the INCAR file to determine parameters for
...
!
      LEPSILON=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LEPSILON', LEPSILON, IERR, WRITEXMLINCAR)
!
...
!
      ...
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'DEG_THRESHOLD', DEGENERACY_THRESHOLD, IERR, WRITEXMLINCAR)
!
! direction of dipole correction IDIPOL=0 (off) or 1-3 (1.d slab) or 4 (all direction)
!
      LRPA=.FALSE.
...
!
      LNABLA=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LNABLA', LNABLA, IERR, WRITEXMLINCAR)
!
! velocity operator
!
      LVEL=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LVEL', LVEL, IERR, WRITEXMLINCAR)
!
! CSHIFT from INCAR
!
      LCSHIFT=0.1_q
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'CSHIFT', LCSHIFT, IERR, WRITEXMLINCAR)
!
! OMEGAMAX from INCAR
!
      OMEGAMAX_OPTIC=-1
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'OMEGAMAX', OMEGAMAX_OPTIC, IERR, WRITEXMLINCAR)
!
! relaxation time RTIME from INCAR
!
      RTIME=-1.E-1_q
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'RTIME', RTIME, IERR, WRITEXMLINCAR)
!
...
!
      ...
      ENDIF
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'WPLASMAI', WPLASMAI, IERR, WRITEXMLINCAR)
      IF (WPLASMAI>0) THEN
! 1 eV-1 = 0.6582122 fs
...
!
      WPLASMA=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'WPLASMA', WPLASMA, 9, IERR, WRITEXMLINCAR, FOUNDNUMBER=N)
...
!
      DFIELD=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'DFIELD', DFIELD, 3, IERR, WRITEXMLINCAR)
...
!**************************** LR_WRITER *******************************
!
!> Writes the parameters of the LR routine to the OUTCAR file.
!
!**********************************************************************
    ...
    SUBROUTINE LR_WRITER(IU6)
      ...
      IF (IU6>=0) THEN
         WRITE(IU6,100) LEPSILON, LRPA, LNABLA, LVEL, LCSHIFT, &
                        OMEGAMAX_OPTIC, DEGENERACY_THRESHOLD, RTIME, WPLASMAI, DFIELD
...
!**************************** XML_WRITE_LR ****************************
!
!> Writes the parameters of the LR routine to the XML file
!
!**********************************************************************
