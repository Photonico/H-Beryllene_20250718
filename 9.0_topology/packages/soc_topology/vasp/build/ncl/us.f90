...
!
!> @note  A few comments concerning implementation:
...
!***********************************************************************
!
! Interface for SETDIJ_
! @note the dummy argument CVTOT_ differs here from the actual definition
...
!
!> Interface for DEPLE
!> @note the dummy argument CHTOT_ differs here from the actual
!> definition in the subroutine used below
!
!***********************************************************************
  INTERFACE
...
!************************ SUBROUTINE DEPSUM ****************************
!
!> Calculates the total (1._q,0._q) center (on site) "occupancy" matrix
!> of each ll'mm' augmentation channel from the FERMI weights,
!> the weight of each k-point WTKPT and the wavefunction character
!> of all bands (thesis gK Eq. 10.32)
!>
!> The result is stored in CRHODE
!>
!> @details @ref openmp :
!> the nested loop over \"types\" + \"ions-per-type\" is replaced by
!> a loop over \"all ions\" that is dsitributed over all available
!> threads.
!
!***********************************************************************
...
!! USE moffload
      ...
      INTEGER ISP, NT, NK, N, ISPINOR, ISPINOR_, IBASE, IBASE_, LMMAXC, NI, L, LP
      REAL(q) WEIGHT
...
!$ACC PRESENT(WDES,W,W%CPROJ,CRHODE) PRIVATE(NT,LMMAXC,IBASE,IBASE_)
         spinor: DO ISPINOR =0,WDES%NRSPINORS-1
         DO ISPINOR_=0,WDES%NRSPINORS-1

 !$OMP PARALLEL DO DEFAULT(NONE) &
 !$OMP SHARED(WDES,CRHODE,ISP,ISPINOR,ISPINOR_,WEIGHT,W,N,NK) &
 !$OMP PRIVATE(NI,NT,LMMAXC,IBASE,IBASE_,L,LP)
            DO NI=1,WDES%NIONS
               NT=WDES%ITYP(NI)
               LMMAXC=WDES%LMMAX(NT)
               IF (LMMAXC==0) CYCLE
               IBASE =WDES%LMBASE(NI)+ISPINOR *(WDES%NPRO/2)
               IBASE_=WDES%LMBASE(NI)+ISPINOR_*(WDES%NPRO/2)
...
!OCL NOVREC
               DO LP=1,LMMAXC
                  CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
                     WEIGHT*W%CPROJ(L+IBASE,N,NK,ISP)*CONJG(W%CPROJ(LP+IBASE_,N,NK,ISP))
               ENDDO
               ENDDO
            ENDDO
 !$OMP END PARALLEL DO
...
!
!> Calculates the total (1._q,0._q) center (on site) "occupancy" matrix
!> applying symmetry to determine the wavefunctions at each k-point
!>
!> It should be fully compatible with the Fock-like routines but
!> of course is slightly slower than #DEPSUM
!>
!> If this routine is used to construct CRHODE no a posteriori
!> symmetrization of the charge density should be required
!
!***********************************************************************
...
! local
      INTEGER ISP, ISP_IRZ, NT, NK, N, ISPINOR, ISPINOR_, IBASE, IBASE_, LMMAXC, NI, L, LP
      LOGICAL :: LSHIFT
...
!$ACC ENTER DATA CREATE(WDES1,W1) IF(OFFLOAD_ON)
...
!$ACC PARALLEL LOOP COLLAPSE(3) PRESENT(WDES,W1,CRHODE) PRIVATE(NT,LMMAXC,IBASE,IBASE_) 
            spinor: DO ISPINOR =0,WDES%NRSPINORS-1
            DO ISPINOR_=0,WDES%NRSPINORS-1
...
!OCL NOVREC
                  DO LP=1,LMMAXC
                     CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
                          WEIGHT*W1%CPROJ(L+IBASE)*CONJG(W1%CPROJ(LP+IBASE_))
                  ENDDO
                  ENDDO
...
!************************ SUBROUTINE DEPSUM1 ****************************
!
!> Calculates the first order change of the (1._q,0._q) center (on site) "occupancy"
!> matrix of each ll'mm' augmentation channel from the FERMI weights,
!> the weight of each k-point WTKPT and the wavefunction character
!> of all bands
!>
!> The result is stored in CRHODE1
!
!***********************************************************************
...
!! USE moffload
      ...
      INTEGER ISP, NT, NK, N, ISPINOR, ISPINOR_, LMBASE, LMBASE_, LMMAXC, NI, L, LP
...
!$ACC PARALLEL LOOP COLLAPSE(3) PRESENT(WDES,W0,W1,CRHODE) PRIVATE(NT,LMMAXC,LMBASE,LMBASE_,L,LP) 
            spinor: DO ISPINOR =0,WDES%NRSPINORS-1
            DO ISPINOR_=0,WDES%NRSPINORS-1
...
!************************ SUBROUTINE DEPATO ****************************
!
!> Sets the "occupancy" matrix CRHODE to the values of the
!> the pseudopotential generation (QATO)
!>
!> In the spinpolarized collinear case
!> ~~~
!> CRHODE(:,2) = CRHODE(:,1) / ZVAL * ATOMOM
!> ~~~
!> In the non collinear case the magnetization in each direction
!> x,y and z is set such that the moment is parallel to ATOMOM
!
!***********************************************************************
...
!************************* SUBROUTINE FORDEP ***************************
!
!> Calculates the forces, which are related to the the change in the
!> position of the augmentation charges in a fixed local potential
!> using finite central differences
!>
!> The accuracy of the routine is at least 7 significant digits
!> (Eq. 54 of PRB 59, 1758)
!>
!> @note This routine resets the CDIJ array, so
!>       CDIJ must be restored after calling the routine
!>
!> @details @ref openmp :
!> the nested loops over \"types\" + \"ions-per-type\" are replaced by
!> loops over \"all ions\" that is dsitributed over all available
!> threads.
!
!***********************************************************************
...
!************************* SUBROUTINE STRDEP ***************************
!
!> Calculates the contributions to the stress related to the
!> augmentation charges
!>
!> @note Uncomment "TEST" lines if you want to test finite differences
!
!***********************************************************************
...
!
!> Calculates the charge density on the soft and fine grid
!>
!> Results are returned in the convention (total, magnetization (x,y,z))
!> for all considered arrays (CHDEN, CHTOT, and CRHODE)
!>
!> The charge densities are returned on the reciprocal grid
!
!***********************************************************************
...
! on return the following arrays are set
      ...
      COMPLEX(q)  CHDEN(GRID_SOFT%MPLWV,WDES%NCDIJ)         !< pseudo charge density on soft grid
      COMPLEX(q)  CHTOT(GRIDC%MPLWV,WDES%NCDIJ)             !< total charge density on fine grid
      LOGICAL LOVERL
      INTEGER ISP
...
!GPACC !$ACC UPDATE SELF(W) IF(ASSOCIATED(W).AND.OFFLOAD_ON) ASYNC(ACC_ASYNC_Q)
!! 
...
!>
!> @note The cutoff-sphere extends up to PSDMAX*(NPSRNL-1)/NPSRNL
!
!***********************************************************************
...
! work-arrays
      ...
      IF ((LYDIM+1)**2 > LMYDIM) THEN
         CALL vtutor%bug("internal error: LMYDIM is too small " // str(LYDIM) // " " // str(LMYDIM), "us.F", 1322)
      ENDIF
!=======================================================================
...
!-----------------------------------------------------------------------
      ...
      IF (GRID%RL%I2(NCOL) /= N1P+1 .OR. GRID%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("internal ERROR SETYLM_AUG: " // str(N1P+1) // " " // str(N2P+1) // " " // &
           str(NCOL) // " " // str(GRID%RL%I2(NCOL)) // " " // str(N1P) // " " // &
           str(GRID%RL%I3(NCOL)) // " " // str(N2P), "us.F", 1368)
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      IF (INDMAX>IRMAX) THEN
        CALL vtutor%bug("internal ERROR: DEPLE: IRDMAX must be increased to " // str(INT(INDMAX*1.1)), "us.F", 1468)
      ENDIF
...
!************************ SUBROUTINE MAXL ******************************
!
!> Calculates the maximum L quantum number for the augmentation charges
!>
...
!************************ SUBROUTINE MAXL1 *****************************
!
!> Calculates the maximum L quantum number for (1._q,0._q) particular type
!
!***********************************************************************
...
!************************ SUBROUTINE SETDEP ****************************
!
!> Interpolates the augmentation charge on the grid around (1._q,0._q) ion
!> using a cubic spline interpolation
!>
!> The result is returned in DEP
!
!***********************************************************************
...
!************************ SUBROUTINE SETDIJ  ****************************
!
!> Calculates the corrections DION(I,J) corresponding to the integral
!> over the augmentation-holes * total local potential
!>
!> As input it requires the total potential CVTOT (thesis gK Eqs. 6.12 + 6.16)
!> the subroutine also sets up CQIJ(I,J) (i.e. Q(i,j) in thesis gK)
!
!***********************************************************************
    ...
    SUBROUTINE SETDIJ(WDES, GRIDC_,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO, LOVERL, &
      ...
      COMPLEX(q),TARGET :: CVTOT_(GRIDC_%MPLWV,WDES%NCDIJ)
      LOGICAL  LOVERL
      REAL(q)  DISPL(3,T_INFO%NIONS) !< additional displacements of the ions
...
!> @details @ref openmp :
!> the loops over ions are distributed over all available OpenMP threads.
!
!> @details @ref openacc :
...
!> CDIJ and CQIJ are updated on the device (if present).
!
    SUBROUTINE SETDIJ_(WDES, GRIDC_,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO, LOVERL, &
        LMDIM,CDIJ,CQIJ, CVTOT_, LDIAGONAL_TERMS, IRDMAA,IRDMAX, DISPL)
...
!! USE moffload
      ...
      LOGICAL  LOVERL
      REAL(q)  DISPL(3,T_INFO%NIONS) !< additional displacements of the ions
      LOGICAL  LDIAGONAL_TERMS       !< add atomic reference diagonal terms
!  work arrays
      LOGICAL LADDITIONAL
...
!=======================================================================
!$OMP PARALLEL DO DEFAULT(NONE) &
!$OMP SHARED(T_INFO,P,GRIDC,LATT_CUR,LMYDIM,IRDMAX,DISPL,CVTOT,CTMP,RINPL,WDES,ISP,QVEC,CVTOT_) &
!$OMP PRIVATE(NI,NT,PP,LYMAX,YLM,INDMAX,DIST,NLI,XS,YS,ZS,N,POT,VTUTOR, &
!$OMP LDEP_INDEX,LM,L,LMP,LP,DEP,LL,LLP,M,MPLOW,MP,INDYLM,INDPYL,IND,SUM, &
!$OMP DLM,QR,NMAX_FOCKAE,LMAX_FOCKAE,QDEP_FOCK_INDEX,NAE) &
!$OMP REDUCTION(MAX:IRDMAA)
      ion: DO NI=1,T_INFO%NIONS
      NT=T_INFO%ITYP(NI)
...
!     WRITE(*,*) '1',DLM(1)
      ...
      IF (QDEP_FOCK_INDEX /= SIZE(PP%QDEP_FOCK,3)-1) THEN
         CALL vtutor_bug("internal error in SETDIJ_FOCKAE: size of QDEP_FOCK seems to be incorrect " &
            // str(SIZE(PP%AUG_SOFT,2)) // " " // str(QDEP_FOCK_INDEX) // " " // &
            str(SIZE(PP%QDEP_FOCK,3)), "us.F", 1985)
      ENDIF
...
!      CALL DUMP_DLLMM('real part', CTMP(:,:,1,1), PP)
!      CALL DUMP_DLLMM_IM( CTMP(:,:,1,1), PP)
...
!$OMP PRIVATE(NI,NIP,ISP,LOW,LM,NT,PP,ISP_INC,FAKT,LL,LHI,MMAX,L,LP,M)
...
!! 
!$ACC UPDATE DEVICE(CQIJ) IF_PRESENT IF(OFFLOAD_ON)
...
!
!> Calculates the vector potential related contributions to the
!> non-local pseudopotential strength parameters
!>
...
!> if states ij, and A(r) is the vector potential
!>
...
!> Identical to #SETDIJ_AVEC_ but with (1._q,0._q) more argument A_ONE_CENTER
!>
!> @note We can not have an explict interface for this routines, because
...
!>
!> The reverse operation is possible as well, by setting LBACK=.TRUE.
!
!***********************************************************************
...
!> The reverse operation is possible as well, by setting LBACK=.TRUE.
!>
!> @note Same as #US_FLIP but with CRHODE defined as COMPLEX instead
!>       of COMPLEX(q)
!
!***********************************************************************
...
!************************ SUBROUTINE DEPLE  ****************************
!
!> Calculates the augmentation charge-density-distribution in
!> real space and adds the pseudo charge density CHDEN
!>
...
!
!> Same as #DEPLE but the resul is *added* to the charge density CHTOT
!
...
!
!> Calculates the augmentation charge-density distribution in
!> reciprocal space and adds it to the array CHTOT
!>
...
# 3243
         ...
         ENDIF
      ENDIF
!     CALL DUMP_DLLMM('density real part', CTMP(:,:,1,1), PP)
!     CALL DUMP_DLLMM_IM( CTMP(:,:,1,1), PP)
!=======================================================================
...
!************************ SUBROUTINE CURRENT_AUGMENTATION **************
!
!> Calculates the augmentation contribution to the current density JTOT
!>
!> As input it requires CRHODE(LM,LMP,ION,ISP)
!> and a possible displacement vector DISPL
!
