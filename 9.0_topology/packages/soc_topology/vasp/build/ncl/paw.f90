...
! seek all channels with same L quantum number
             ...
             LHI=LHI-1
             IF (LL>PP%LMAX) THEN
                CALL vtutor%bug("internal error in SET_OPT_PROJ: increase PP%LMAX " // str(PP%LMAX), "paw.F", 199)
             ENDIF

             N_CHANNELS=LHI-LOW+1
             IF (N_CHANNELS> NDMAT) THEN
                CALL vtutor%bug("internal error in SET_OPT_PROJ: increase NDMAT " // str(NDMAT), "paw.F", 204)
             ENDIF
...
! and resort that the (1._q,0._q) with largest norm comes first
          EV_WORK=0
          DO I=1,N_CHANNELS
...
!
!  WRT_RHO_PAW
!> write the PAW occupancies to a file specified by IU
!
!*******************************************************************
...
!=======================================================================
      ...
      ion: DO NI=1,T_INFO%NIONS
         ...
         IF (DO_LOCAL(NI)) THEN
            ...
            IF (NELEMENTS > LMMAX*LMMAX) THEN
               CALL vtutor%bug("internal ERROR: WRT_RHO_PAW running out of buffer", "paw.F", 351)
            ENDIF
      ...
      again: DO NI=1,T_INFO%NIONS
         ...
         IF (DO_LOCAL(NI)) THEN
            ...
            IF (NELEMENTS > LMMAX*LMMAX) THEN
               CALL vtutor%bug("internal ERROR: WRT_RHO_PAW running out of buffer", "paw.F", 383)
            ENDIF
...
!
!  RD_RHO_PAW
!> read the PAW occupancies from a file specified by IU
!>
...
!*******************************************************************
    ...
    SUBROUTINE RD_RHO_PAW(P, T_INFO, LOVERL, RHOLM_STORE, COMM, IU, IERR )
      ...
      INTEGER IU               !< io unit
      INTEGER IERR             !< error status on return 0 ok, 1 error occured
      TYPE (type_info)   T_INFO
      TYPE (potcar),TARGET::  P(T_INFO%NTYP)
      LOGICAL  LOVERL          !< overlap matrix used ?
      REAL(q) RHOLM_STORE(:)   !< storage for the channel occupancies
...
!=======================================================================
      ...
      ion: DO NI=1,T_INFO%NIONS
         ...
         IF (NELEMENTS_READ > LMMAX*LMMAX) THEN
            CALL vtutor%bug("internal ERROR: RD_RHO_PAW running out of buffer", "paw.F", 480)
         ENDIF
...
! if so then read the imaginary parts from file
      ...
      again: DO NI=1,T_INFO%NIONS
         ...
         IF (NELEMENTS_READ > LMMAX*LMMAX) THEN
            CALL vtutor%bug("internal ERROR: RD_RHO_PAW running out of buffer", "paw.F", 557)
         ENDIF
...
!*******************************************************************
!
!> calculate the average local potential
!>
...
!*******************************************************************
!
!> calculate the (1._q,0._q)-centre (on site) potentials
...
!> @param   PP%ZCORE      number of core electrons (integer)
!> @param   PP%DEXCCORE   exchange correlation energy of frozen core
!> @param   PP%POTAE      AE (1._q,0._q) electron potential in reference state
!> @param   PP%POTAE_XCUPDATED update to new xc potential
!> @param   PP%POTPS      PS (1._q,0._q) electron potential
!> @param   PP%POTPSC     PS (1._q,0._q) electron potential core only
!> @param   PP%EATOM      atomic energy
!>    PP%EATOM_CORRECTED
!> @param   PP%DION       (1._q,0._q) center contribution
!>
...
!*******************************************************************
    ...
    SUBROUTINE SET_PAW_ATOM_POT( P , XC, T_INFO, LOVERL, LMDIM, EALLAT, IU6 )
      ...
      USE constant
      USE radial
      USE setexm, ONLY : PUSH_XC_TYPE,POP_XC_TYPE,PUSH_XC_TYPE_METAGGA,POP_XC_TYPE_METAGGA
...
!=======================================================================
      ...
      CALL RAD_AUXILIARY_FUNCTIONS_METAGGA(XC,MAXL(T_INFO%NTYP,P))
...
! (vasp.5.1: the latest potentials are constructed such that DEXC is essentially (0._q,0._q))
         IF (LUSE_THOMAS_FERMI) THEN
            XC_DATA_TEMP=XC
            XC_DATA_TEMP%AEXX=AEXX !Necessary because XC_DATA%AEXX is not updated to AEXX everywhere.
            XC_DATA_TEMP%LDASCREEN=0.0_q
            CALL PUSH_XC_TYPE(XC,XC_DATA_TEMP)
         ENDIF

         CALL APPLY_ONE_CENTER_AEXX(XC)
         CALL PUSH_XC_TYPE_METAGGA(XC)
         CALL RAD_CORE_XC( PP%R, XC, PP%RHOAE, DEXC)
         CALL POP_XC_TYPE_METAGGA(XC)
         CALL RESTORE_ONE_CENTER_AEXX(XC)

         IF (LUSE_THOMAS_FERMI) CALL POP_XC_TYPE(XC)

         PP%DEXCCORE =PP%DEXCCORE+DEXC

! switch to original potential type
         IF(.NOT. VERSION46) CALL PUSH_XC_TYPE(XC,XC_DATA_PP)
         CALL RAD_CORE_XC( PP%R, XC, PP%RHOAE, DEXC_OLD)
         IF(.NOT. VERSION46) CALL POP_XC_TYPE(XC)
...
! (1._q,0._q)-centre xc potential
         CALL PUSH_XC_TYPE_METAGGA(XC)
         CALL RAD_POT( PP%R, XC, ISPIN, 1, 1, .FALSE.,   &
...
! redefine core pseudo potential on radial grid
            PP%POTPSC(1:MIN(SIZE(PP%POTPSC),SIZE(POTDIFF)))=&
            PP%POTPSC(1:MIN(SIZE(PP%POTPSC),SIZE(POTDIFF)))-POTDIFF(1:MIN(SIZE(PP%POTPSC),SIZE(POTDIFF)))
...
!  applied only for MIMIC_US
    ...
    SUBROUTINE SET_PAW_ATOM_POT_RHF(P,XC,T_INFO,LOVERL,EALLAT,IO)
...
!
!  SET_PAW_CORE_FOCK
!> calculates the core-valence exchange contribution for Hartree-Fock
!>>
!> ~~~
...
!>
!> @details @ref openmp :
!> the work on the (1._q,0._q)-center terms is distributed over the ranks
!> in COMM_INB, COMM_INTER, and COMM_KINTER. In case we run on more
!> than (1._q,0._q) OpenMP thread COMM_INB\%NCPU=1, but the work is
!> additionally distributed over the threads.
!>
!> @details @ref openacc :
...
!*******************************************************************
    ...
    SUBROUTINE SET_DD_PAW(WDES, P , XC, T_INFO, LOVERL, &
...
!! USE moffload
      ...
      USE fock_glb, ONLY : AEXX
      USE egrad
      USE mbj, ONLY : CMBJ,LscMBJ,GRHO_OVER_RHO_ONE_CENTER,GRHO_OVER_RHO_AUG,GET_CMBJ_RAD
      USE xc_name
      USE hyperfine
! embedding__
      USE mextpot
...
!$OMP ISPIN,E,LASPH,ALPHA,BETA,NULPOINTER,LCOREL,CL_SHIFT,SPI2,GRHO_OVER_RHO_AUG,XC) &
!$OMP FIRSTPRIVATE(IBASE,IDONE,IDONP,CVMBJ,GRDR,PP) &
...
!atom type for mBJ
            CALL GET_CMBJ_RAD(XC,NT,CVMBJ)
         ELSEIF (XC%ID(1)==ID_XC_LMBJ) THEN
!atom index for local mBJ
            IF (XC%LDOMETAGGA) GRHO_OVER_RHO_AUG(NI)=0._q
            CALL GET_CMBJ_RAD(XC,NI,CVMBJ)
         ENDIF
...
! We'll need the pseudo charge density without augmentation in the metagga
            IF (XC%LNO_AUG_XC) RHOPS_STORE(:,:,ISP)=RHO(:,:,ISP)

! for LDA+U or Hartree fock we need the mixed occupancy matrix COCC
            IF (USELDApU() .OR. XC%LDOMETAGGA .OR. &
...
! bring COCC to (up,down)
         ...
         CALL RAD_KINEDEN(PP,XC,PP%WAE,LMAX_TAU,NCDIJ,COCC,TAUAE)
         CALL RAD_KINEDEN(PP,XC,PP%WPS,LMAX_TAU,NCDIJ,COCC,TAUPS)
...
! do LDA+U instead of LSDA+U (set magnetisation density to 0)
            ...
            IF (L_NO_LSDA().AND.XC%LNO_AUG_XC) RHOPS_STORE(:,:,2:WDES%NCDIJ)=0

            IF (.NOT.XC%LNO_AUG_XC) THEN
               CALL RAD_POT( PP%R, XC, 2, LYMAX, PP%LMAX_CALC, LASPH,   &
                  RHOCOL, PP%RHOPS, PP%POTPS, POT, DOUBLEPS, EXCG)

               CALL RAD_POT_METAGGA( PP%R, XC, 2, PP%LMAX_CALC, LMAX_TAU, LASPH, CVMBJ,  &
                  RHOCOL, PP%RHOPS, PP%POTPS, KINDENSCOL, POT, DOUBLEPS, EXCG, GRDR, MUPS, NI, -1._q, PP%TAUPS)
            ELSE
               CALL RAD_POT_HAR_ONLY( PP%R, 2, LYMAX, PP%LMAX_CALC, RHOCOL, POT, DOUBLEPS)

               CALL RAD_POT_XC_ONLY( PP%R, XC, 2, LYMAX, PP%LMAX_CALC, LASPH,   &
                  RHOPS_STORE, PP%RHOPS-DRHOCORE(1:RNMAX), PP%POTPS, POT, DOUBLEPS, EXCG, POTH)

               CALL RAD_POT_METAGGA( PP%R, XC, 2, PP%LMAX_CALC, LMAX_TAU, LASPH, CVMBJ,  &
                  RHOPS_STORE, PP%RHOPS, PP%POTPS, KINDENSCOL, POT, DOUBLEPS, EXCG, GRDR, MUPS, NI, -1._q, PP%TAUPS)
            ENDIF
            ...
            IF (XC%LNO_AUG_XC) CALL RAD_MAG_DIRECTION( RHO, RHOCOL, POTH, LYMAX, PP%R)
...
! do LDA+U instead of LSDA+U (set magnetisation density to 0)
            ...
            CALL RAD_POT_METAGGA( PP%R, XC, 2, PP%LMAX_CALC, LMAX_TAU, LASPH, CVMBJ, &
               RHOCOL, PP%RHOAE, PP%POTAE_XCUPDATED, KINDENSCOL, POTAE, DOUBLEAE, EXCG, GRDR, MUAE, NI, 1._q, PP%TAUAE)
...
! do LDA+U instead of LSDA+U (set magnetisation density to 0)
            RHOCOL=RHO
            IF (L_NO_LSDA().AND.(NCDIJ==2)) RHOCOL(:,:,2)=0
            IF (L_NO_LSDA().AND.(NCDIJ==2).AND.XC%LNO_AUG_XC) RHOPS_STORE(:,:,2)=0
...
! but only in the Hartree term
            ...
            IF (.NOT.XC%LNO_AUG_XC) THEN
               CALL RAD_POT( PP%R, XC, ISPIN, LYMAX, PP%LMAX_CALC, LASPH,   &
                  RHOCOL, PP%RHOPS-DRHOCORE(1:RNMAX), PP%POTPS, POT, DOUBLEPS, EXCG)

               CALL RAD_POT_METAGGA( PP%R, XC, ISPIN, PP%LMAX_CALC, LMAX_TAU, LASPH, CVMBJ, &
                  RHOCOL, PP%RHOPS-DRHOCORE(1:RNMAX), PP%POTPS, TAUPS, POT, DOUBLEPS, EXCG, GRDR, MUPS, NI, -1._q, PP%TAUPS)
            ELSE
               CALL RAD_POT_HAR_ONLY( PP%R, ISPIN, LYMAX, PP%LMAX_CALC, RHOCOL, POT, DOUBLEPS)

               CALL RAD_POT_XC_ONLY( PP%R, XC, ISPIN, LYMAX, PP%LMAX_CALC, LASPH,   &
                  RHOPS_STORE, PP%RHOPS-DRHOCORE(1:RNMAX), PP%POTPS, POT, DOUBLEPS, EXCG, POTH)

               CALL RAD_POT_METAGGA( PP%R, XC, ISPIN, PP%LMAX_CALC, LMAX_TAU, LASPH, CVMBJ, &
                  RHOPS_STORE, PP%RHOPS-DRHOCORE(1:RNMAX), PP%POTPS, TAUPS, POT, DOUBLEPS, EXCG, GRDR, MUPS, NI, -1._q, PP%TAUPS)
            ENDIF
...
! do LDA+U instead of LSDA+U (set magnetisation density to 0)
            ...
            CALL RAD_POT_METAGGA( PP%R, XC, ISPIN, PP%LMAX_CALC, LMAX_TAU, LASPH, CVMBJ, &
               RHOCOL, PP%RHOAE-DRHOCORE(1:RNMAX), PP%POTAE_XCUPDATED, TAUAE, POTAE, DOUBLEAE, EXCG, GRDR, MUAE, NI, 1._q, PP%TAUAE)
...
! subtract it now
         ...
         CALL HYPERFINE_RAD(T_INFO,NI,PP,XC,RHO,RHOAE,POTAE,COCC)
!-----------------------------------------------------------------------
...
! calculate core wavefunctions in PAW sphere for atomic reference potential
           ...
           IF (MAXNL > SIZE(CL_SHIFT,DIM=1)) THEN
              CALL vtutor%bug("CL_MAXNL too small (cl_shift.F) " // str(MAXNL) // " " &
                 // str(SIZE(CL_SHIFT,DIM=1)),"paw.F",1942)
           ENDIF
...
! multiply potentials by simpson weights
         ...
         IF (XC%LNO_AUG_XC) CALL RAD_POT_WEIGHT( PP%R, NCDIJ, LYMAX, POTH)
...
! transform them using Clebsch Gordan coefficients and add to CDIJ
         ...
         CMETAGGA=0
         CALL RAD_PROJ_METAGGA(PP,XC,PP%WAE,LMAX_TAU,MUAE, 1._q,CMETAGGA)
         CALL RAD_PROJ_METAGGA(PP,XC,PP%WPS,LMAX_TAU,MUPS,-1._q,CMETAGGA)
...
# 2088
      ...
      CALL M_sum_d(WDES%COMM, EPAWPSG, 1)
      CALL M_sum_d(WDES%COMM, EPAWAEG, 1)
      CALL M_sum_d(WDES%COMM, EPAWCORE, 1)

      IF (LscMBJ()) THEN
         CALL M_sum_d(WDES%COMM, GRDRSUM, 1)
      ENDIF
...
# 2115
      ...
      DEALLOCATE(POTAE, RHOAE, POT, RHO, DRHOCORE, RHOCOL)
