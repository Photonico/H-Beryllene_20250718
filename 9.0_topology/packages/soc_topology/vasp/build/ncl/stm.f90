...
! no STM output required, do quick return
    ...
    IF (WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL vtutor%error("WRT_STM_FILE: KPAR>1 not implemented, sorry.")
    END IF
...
! and writen them to the file
       ...
       DO N2=1,GRID%NGY
          IF (DO_IT(N1,N2)) THEN
             ...
             CALL FFT1D(C1, NFFT, 1)
             CALL FFT1D(C2, NFFT, 1)
...
! clean up the mess
!-----------------------------------------------------------------------
    DEALLOCATE( DO_IT, CWORK, C1, C2, COARSE_TO_FINE, GZ)
