...
! of G' = S G, where S is the symmetry operation leaving the k point invariant
          ...
          IF (KPOINTS_TRANS_SG%NINDPW(NI,NOP)==0) THEN
             CALL vtutor%bug("internal error in SMALL_SPACE_GROUP: G vector not found " // str(NQ) &
                // " " // str(NOP) // " " // str(NI) // " " // str(NG1I) // " " // str(NG2I) // " " &
                // str(NG3I) // " " // str(NG1) // " " // str(NG2) // " " // str(NG3) // " " // &
                str(MP(NG1,NGX)) // " " // str(MP(NG2,NGY)) // " " // str(MP(NG3,NGZ)), "sym_grad.F", 254)
          ENDIF
             ...
             DO ISPINOR=0,WDES1%NRSPINORS-1
                ...
                DO NT=1, WDES1%NTYP
                   DO NI=NIS, WDES1%NITYP(NT)+NIS-1
                      ...
                      CALL ROTATE_VECTOR( KPOINTS_TRANS_SG%LINV(NOP),W%CPROJ(NPROP,NB,NK,ISP_OLD),W_NEW%CPROJ(NPRO), &
             ...
             DO ISPINOR=0,WDES1%NRSPINORS-1
                ...
                DO NT=1, WDES1%NTYP
                   DO NI=NIS, WDES1%NITYP(NT)+NIS-1
                      NIP=KPOINTS_TRANS_SG%ROTMAP(NI,NOP)
                      NPROP=ROT_HANDLE%NPRO_NI(NIP)+ISPINOR *(WDES1%NPRO/2)
                      CALL ROTATE_VECTOR( KPOINTS_TRANS_SG%LINV(NOP),WACC%CPROJ(NPROP,NB,NK,ISP_OLD),W_NEW%CPROJ(NPRO), &
                           ROT_HANDLE%MMAX,ROT_HANDLE%LDIM,ROT_HANDLE%SL(1,1,0),ROT_HANDLE%NPS(NT),ROT_HANDLE%LPS(1,NT))
                      NPRO= WDES1%LMMAX(NT)+NPRO
                   ENDDO
                   NIS=NIS+WDES1%NITYP(NT)
