...
!*********************************************************************
  ...
  SUBROUTINE LR_SKELETON( &
          ...
          TOTENIN,EFERMI,FORCE, &
          RETURN_FORCE_CONST, FORCE_CONST_OUT, &
          LFAST)
...
!> Hamiltonian
    ...
    REAL(q), ALLOCATABLE, OPTIONAL, INTENT(OUT) :: FORCE_CONST_OUT(:, :) ! Matrix of inter-atomic force constants
...
! variables for the reevaluation of energy
    REAL(q) TOTEN, DESUM      ! energy and change of (1._q,0._q) electron energies
...
!=======================================================================
    NULLIFY(DEG_CLUSTER)
! no rotations within degenerate clusters, fails for response to fields
# 325

! gK: to avoid LRF_COMMUTATOR internal error messages rotate the degenerate states
!    CALL FIND_DEG_CLUSTERS(WDES, W, DEG_CLUSTER, LPARTIAL_IN=.TRUE.)
...
! gK: no rotations within degenerate clusters, fails for response to external fields
...
! symmetrize Hessian matrix
       ...
       IF (RETURN_FORCE_CONST .AND. PRESENT(FORCE_CONST_OUT)) THEN
          ALLOCATE(FORCE_CONST_OUT(3 * NIONS, 3 * NIONS))
          FORCE_CONST_OUT = -SECOND_DERIV
       ENDIF
       IF (IU6<0) DEALLOCATE(SECOND_DERIV)
    ENDIF
...
! store current positions into last position array
    ...
    IF (NSTEP==1) THEN
! initial dipole is assumed to be (0._q,0._q)
       DIPOLE_TIME(:,NSTEP)=0
    ELSE
...
! essentially it forces the conductivity to be (0._q,0._q), as it should be (but why is there any contribution)
! TODO: the conductivity sigma has a constant real valued offset, which should not be there
       SIGMA0=(SIGMA(:,:,1)+SIGMA(:,:,2))/2 ! (1._q,0._q) can alternatively also use SIGMA(:,:,NSW)
       SIGMA0=0
       IF (IO%IU6>=0) THEN
          DO I=2,NSW
             OMEGA=OMEGA_MAX/NSW*(I-1)
...
! local variables
   TYPE (wavespin),POINTER :: W_KBATCH
...
! Wavefunction container for computation of optical matrix elements
   ...
   REAL(q) :: DOS(NEDOS_IN,W%WDES%NCDIJ),DOSI(NEDOS_IN,W%WDES%NCDIJ)
   COMPLEX(qs), ALLOCATABLE   :: CHAM_INTER(:,:,:,:,:), CHAM_TMP(:,:,:,:)
...
# 1640
   ...
   CALL WAVE_INTERPOLATOR_INIT(WAVE_INTERP, KPOINTS, KPOINTS_INTER%VKPT, GRID, &
...
! TODO: Check if this is needed!
   ...
   END IF
   IF (IO%LOPTICS) THEN
      ALLOCATE(CHAM_TMP(W%WDES%NB_TOT,W%WDES%NB_TOT,WAVE_INTERP%KPOINTS%NKPTS,W%WDES%ISPIN))
...
! projections
         IF (JOBPAR/=0 .OR. IO%LORBIT>=10) THEN
            ...
            IF (IO%LORBIT>=10) THEN
                CALL SPHPRO_FAST( GRID,LATT_CUR, P,T_INFO,W_KBATCH, W_KBATCH%WDES, 71,IO%IU6,&
            ...
            ELSE
                CALL SPHPRO( &
                     GRID,LATT_CUR, P,T_INFO,W_KBATCH, W_KBATCH%WDES, 71,IO%IU6,&
            ...
            DO IK=1,WAVE_INTERP%NKPTS_BATCH
                ...
                IF (MOD(IKCOARSE-1,W_KBATCH%WDES%COMM_KINTER%NCPU).NE.W_KBATCH%WDES%COMM_KINTER%NODE_ME-1) CYCLE
...
! linear optics
         IF (LVEL.OR.IO%LOPTICS) THEN
            CALL ALLOCW(W_KBATCH%WDES,WDIFF)
             
! Find degenerate energies
            NULLIFY(DEG_CLUSTER)
            CALL FIND_DEG_CLUSTERS(W_KBATCH%WDES, W_KBATCH, DEG_CLUSTER)
...
! TODO: The derivatives for the original points are always recomputed
               CALL FOCK_K_DER_ANALYT(KPOINTS, GRID, LATT_CUR, LATT_INI, &
                    T_INFO,  NONLR_S, NONL_S, W_KBATCH, WDIFF, LMDIM, P, CQIJ, &
                    SYMM, IDIR, LDONE, IO%IU0, IO%IU6)

               CALL LRF_RPHI0( P,NONLR_S,NONL_S,W_KBATCH,LATT_CUR, &
                    T_INFO,INFO,IO,GRID,GRIDC,GRIDUS,C_TO_US,IRDMAX, &
                    CDIJ,CQIJ,SV,LMDIM,DEG_CLUSTER, IDIR, WDIFF, LDONE)
    
! Pull out the <u_n| r | u_m > for the calculation of the dielectric function
               IF (IO%LOPTICS) THEN
                  CALL INPROD_W(WDIFF,W_KBATCH,CHAM_TMP(:,:,:,:),INFO%LOVERL,IO%IU0)
               ENDIF
...
! Store the <u_n| r | u_m > in the dense grid
            ...
            CALL DEALLOCW(WDIFF)
            CALL FREE_DEG_CLUSTERS(W_KBATCH%WDES, DEG_CLUSTER)
         ENDIF
...
!-----------------------------------------------
   ...
   IF (JOBPAR/=0 .OR. IO%LORBIT>=10) THEN
      CALL M_sum_d( WDES_INTER%COMM, PAR_INTER, SIZE(PAR_INTER))
      CALL M_sum_z( WDES_INTER%COMM, PHAS_INTER, SIZE(PHAS_INTER))
...
# 1891
      ...
      CALL M_sum_single(WDES_INTER%COMM_KINTER, CHAM_INTER, 2*SIZE(CHAM_INTER))

      CALL M_sum_d(WDES_INTER%COMM_KINTER, EDER_INTER, SIZE(EDER_INTER))
