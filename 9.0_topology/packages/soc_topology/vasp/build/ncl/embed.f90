...
# 257
    ...
    TYPE(in_struct)     :: IO
! local
    COMPLEX(q), ALLOCATABLE :: TRAFO_MATRIX(:, :, :, :)
    COMPLEX(q), ALLOCATABLE :: WORK_MATRIX(:, :, :, :)
    TYPE(wavefuna)    :: WA
    TYPE(wavedes1)    :: WDESK
...
! local
    COMPLEX(q), ALLOCATABLE :: TRAFO_MATRIX(:, :, :, :)
    COMPLEX(q), ALLOCATABLE :: WORK_MATRIX(:, :, :, :)
    COMPLEX(q), ALLOCATABLE :: CLUSTER_PROJECTOR(:, :, :, :)
    COMPLEX(q), ALLOCATABLE :: OVERLAPS(:,:)
...
! TRAFO_MATRIX = TRAFO_MATRIX^H * TRAFO_MATRIX
    ...
    CALL ZGEMM('C', 'N', 2*NEMBED_STATES, 2*NEMBED_STATES, NEMBED_STATES, (1._q,0._q), &
               TRAFO_MATRIX, NEMBED_STATES, TRAFO_MATRIX, NEMBED_STATES, (0._q,0._q), &
...
!**********************************************************************
      SUBROUTINE SETUP_ENTGLT_REST_TRAFO(OVERLAPS, TRAFO_ENTGLT_REST)
        COMPLEX(q) :: OVERLAPS(:,:)
        COMPLEX(q) :: TRAFO_ENTGLT_REST(:,:,:,:)
! local
        COMPLEX(q), ALLOCATABLE    :: U(:,:), VD(:,:), SIGMA(:)
        INTEGER              :: INFO, LWORK
        INTEGER              :: I
        COMPLEX(q)                 :: ASKWORK
...
! TRAFO_ENTGLT_REST(i,n) = sum_m^NEMBED_STATES U(i,m) * VD(m,n)
        CALL ZGEMM('N', 'N', NOCC, NEMBED_STATES, NEMBED_STATES, (1._q,0._q), &
                   U, NOCC, VD, NEMBED_STATES, (0._q,0._q), TRAFO_ENTGLT_REST, NOCC)
...
! local
    ...
    COMPLEX(q), ALLOCATABLE   :: H(:,:,:,:) ! the hamiltonian matrix
...
! store total number of atoms (ions)
    ...
    DO ISP = 1, W%WDES%ISPIN
      ...
      ELSE
        ...
        IBMAX = 0
      ENDIF
      CALL M_sum_i(W%WDES%COMM, SB, SIZE(SB(:)))
      CALL M_sum_i(W%WDES%COMM, IBMIN, 1)
      CALL M_sum_i(W%WDES%COMM, IBMAX, 1)
!IBMIN = 1
...
! setup matrix to reorder bands
      ...
      ROTMAT = (0._q,0._q)
      DO I = 1, ROTDIM
        ROTMAT(NSORT(I),I) = (1._q,0._q)
      ENDDO

!! ---------------------------------------------------
!! setup matrix to reorder bands
!! the strategy is to swap all zeros in SB(:,ISP)
!! with a lower lying (1._q,0._q).
!! ---------------------------------------------------
!ROTDIM = IBMAX-IBMIN+1
!ALLOCATE(ROTMAT(ROTDIM,ROTDIM))
...
!    ROTMAT(HIGHEST_ZERO-IBMIN+1,HIGHEST_ZERO-IBMIN+1) = (0._q,0._q)
!    ROTMAT(LOWEST_ONE  -IBMIN+1,LOWEST_ONE  -IBMIN+1) = (0._q,0._q)
!    ROTMAT(HIGHEST_ZERO-IBMIN+1,LOWEST_ONE  -IBMIN+1) = (1._q,0._q)
!    ROTMAT(LOWEST_ONE  -IBMIN+1,HIGHEST_ZERO-IBMIN+1) = (1._q,0._q)
...
! store total number of atoms (ions)
    ...
    DO ISP = 1, W%WDES%ISPIN
      ...
      ELSE
        ...
        IBMAX = 0
      ENDIF
      CALL M_sum_i(W%WDES%COMM, SB, SIZE(SB(:,ISP)))
      CALL M_sum_i(W%WDES%COMM, IBMIN, 1)
      CALL M_sum_i(W%WDES%COMM, IBMAX, 1)
      WRITE(*,*) "MIN, MAX", IBMIN, IBMAX
...
! local
    INTEGER              :: NWF
    INTEGER, ALLOCATABLE :: NSORT(:)
    COMPLEX(q), ALLOCATABLE    :: USORT(:,:) ! unitary matrix to re-sort orbitals
    REAL(q)              :: D1, D2, D3
...
!REAL(q)            :: CENTROIDS(:,:)
! local
    COMPLEX(q),    ALLOCATABLE :: CRHODE(:,:,:,:) ! occupancy of augmentation channels
    ...
    COMPLEX(q),  ALLOCATABLE     :: GRID_WORK(:)
...
! GAMMA ONLY, NO SPIN POLARIZATION
    ...
    ALLOCATE(GRID_WORK(GRIDC%MPLWV))
...
!> energy
!***************************************************************
  SUBROUTINE EMBED_CHARGE(KINEDEN, HAMILTONIAN, W, WDES, GRID, GRIDC, GRID_SOFT, GRIDUS, C_TO_US, &
                          SOFT_TO_C, T_INFO, CSTRF, DENCOR, LATT_CUR, P,    &
                          SYMM, INFO, XC, LMDIM, N_MIX_PAW, IRDMAX, IU0, IU6)
    USE poscar
    USE lattice
    USE pseudo
    USE pawm
...
! GAMMA ONLY, NO SPIN POLARIZATION
    ...
    ALLOCATE(SV(GRID%MPLWV, WDES%NCDIJ))
...
!!!!!!!!!!!!!!!!!!!!!! ALL !!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    CALL POTLOK(KINEDEN, GRID, GRIDC, GRID_SOFT, WDES%COMM_INTER, WDES, INFO, XC, P,  &
                T_INFO, E, LATT_CUR, CHDEN, CHTOT, CSTRF, CVTOT, DENCOR, &
                SV, HAMILTONIAN%MUTOT, HAMILTONIAN%MU, SOFT_TO_C, XCSIF)

    CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, WDES%NCDIJ, LMDIM, &
                    CDIJ(1,1,1,1), RHOLM, CRHODE(1,1,1,1), E,        &
                    LASPH=INFO%LASPH, LCOREL=.FALSE.)
...
!    TYPE (symmetry) :: SYMM
!    COMPLEX(q) :: CDIJ(:, :, :, :)
!    COMPLEX(q)   SV(GRID%MPLWV,W%WDES%NCDIJ) ! local potential
!  ! local
!    COMPLEX(q), ALLOCATABLE :: UK(:, :, :, :)
!    COMPLEX(q), ALLOCATABLE :: UK_TRAFO(:, :, :, :)
