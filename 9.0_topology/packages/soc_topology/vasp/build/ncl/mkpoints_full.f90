...
!!   USE moffload
        ...
        IF (.NOT.LFULL_KPOINTS .OR. .NOT. ASSOCIATED(KPOINTS_FULL)) THEN
           CALL vtutor%bug("internal error in CHECK_FULL_KPOINTS: KPOINTS_FULL not properly initialised", "mkpoints_full.F", 137)
        ENDIF
# 141
...
!     LSKIP_IRREGULAR_POINTS=(KPOINTS%NKPX>0.AND.KPOINTS%NKPY>0.AND.KPOINTS%NKPZ>0 .AND. .NOT.KPOINTS%LKBLOWUP)
      ...
      IF (NIONS/=SIZE(ROTMAP,1)) CALL vtutor%bug("NIONS does not match ROTMAP","mkpoints_full.F",213)
...
! add if it does not exist yet
            IF (.NOT. LEXIST) THEN
               NKTMP=NKTMP+1
               IF (NKTMP>SIZE(K_TMP%VKPT,2)) CALL vtutor%bug("K_TMP is not sufficiently large","mkpoints_full.F",314)
...
! add if it does not exist yet
               IF (.NOT. LEXIST) THEN
                  NKTMP=NKTMP+1
                  IF (NKTMP>SIZE(K_TMP%VKPT,2)) CALL vtutor%bug("K_TMP is not sufficiently large","mkpoints_full.F",386)
                  K_TMP%VKPT(1,NKTMP)=V(1)
                  K_TMP%VKPT(2,NKTMP)=V(2)
                  K_TMP%VKPT(3,NKTMP)=V(3)
                  K_TMP%WTKPT(NKTMP) =KPOINTS%WTKPT(NK)
...
! add if it does not exist yet
            IF (.NOT. LEXIST) THEN
               NKTMP=NKTMP+1
               IF (NKTMP>SIZE(K_TMP%VKPT,2)) CALL vtutor%bug("K_TMP is not sufficiently large","mkpoints_full.F",460)
               K_TMP%VKPT(1,NKTMP)=V(1)
               K_TMP%VKPT(2,NKTMP)=V(2)
               K_TMP%VKPT(3,NKTMP)=V(3)
               K_TMP%WTKPT(NKTMP) =KPOINTS%WTKPT(NK)
...
! add if it does not exist yet
            IF (.NOT. LEXIST) THEN
               NKTMP=NKTMP+1
               IF (NKTMP>SIZE(K_TMP%VKPT,2)) CALL vtutor%bug("K_TMP is not sufficiently large","mkpoints_full.F",521)
               K_TMP%VKPT(1,NKTMP)=V(1)
               K_TMP%VKPT(2,NKTMP)=V(2)
               K_TMP%VKPT(3,NKTMP)=V(3)
               K_TMP%WTKPT(NKTMP) =KPOINTS%WTKPT(NK)
...
!         ENDIF
      ...
      IF (ABS(SUM(KPOINTS_F%WTKPT(:))-1)>1E-6_q) THEN
         CALL vtutor%bug("The k-point weights do not sum to one \n the (routine has &
            ...
            &// str(KPOINTS_F%WTKPT),"mkpoints_full.F",605)
      ENDIF
...
! If there are more than NKPT_REGULAR points with non-(0._q,0._q) weight then
...
!***********************************************************************
  ...
  SUBROUTINE SET_FULL_KPOINTS(NKPTS_FOR_GEN_LAYOUT,VKPT)
    ...
    IF (.NOT. ASSOCIATED(KPOINTS_FULL)) THEN
       CALL vtutor%bug("internal error in SET_FULL_KPOINTS: KPOINTS_FULL not properly initialised", "mkpoints_full.F", 739)
    ENDIF
...
! local
    ...
    IF (GRID%RC%NCOL/=GRID_NEW%RC%NCOL)  THEN
       CALL vtutor%bug("internal error in RE_GEN_LAYOUT_GRID: GRID%RC%NCOL/=GRID_NEW%RC%NCOL " // &
          str(GRID%RC%NCOL) // " " // str(GRID_NEW%RC%NCOL), "mkpoints_full.F", 943)
    ENDIF
    ...
    DO IND=1,GRID%RC%NCOL
       IF (GRID%RC%I2(IND)/=GRID_NEW%RC%I2(IND)) THEN
          CALL vtutor%bug("internal error in RE_GEN_LAYOUT_GRID: GRID I2 changed " // str(IND) // &
             " " // str(GRID%RC%I2(IND)) // " " // str(GRID_NEW%RC%I2(IND)), "mkpoints_full.F", 949)
       ENDIF
       IF (GRID%RC%I3(IND)/=GRID_NEW%RC%I3(IND)) THEN
          CALL vtutor%bug("internal error in RE_GEN_LAYOUT_GRID: GRID I3 changed " // str(IND) // &
             " " // str(GRID%RC%I3(IND)) // " " // str(GRID_NEW%RC%I3(IND)), "mkpoints_full.F", 953)
       ENDIF
    ENDDO
...
! check the number of G-vectors for the first NK_TEST k-points
    DO NK=1,NK_TEST
       IF (WDES%NGVECTOR(NK)/=WDES_NEW%NGVECTOR(NK)) THEN
          CALL vtutor%bug("internal error in CHECK_GEN_LAYOUT: G-vectors changed at ik:" // str(NK) // &
                        " old: " // str(WDES%NGVECTOR(NK)) //&
                        " new: " // str(WDES_NEW%NGVECTOR(NK)), "mkpoints_full.F", 984)
       ENDIF
       DO IND=1,WDES%NGVECTOR(NK)
          IF (WDES_NEW%IGX(IND,NK)/=WDES%IGX(IND,NK) .OR. &
              ...
              CALL vtutor%bug("internal error in CHECK_GEN_LAYOUT: G-vectors changed at ik:" // str(NK) // &
              " ig: " // str(IND) // &
              " new igx: " // str(WDES_NEW%IGX(IND,NK)) // " old igx: " // str(WDES%IGX(IND,NK)) // &
              " new igy: " // str(WDES_NEW%IGY(IND,NK)) // " old igx: " // str(WDES%IGY(IND,NK)) // &
              " new igz: " // str(WDES_NEW%IGZ(IND,NK)) // " old igz: " // str(WDES%IGZ(IND,NK)), "mkpoints_full.F", 994)
           ENDIF
       ENDDO
    ENDDO
...
# 1065
      ...
      IF (WDES%NKDIM<KPOINTS_F%NKPTS) THEN
         CALL vtutor%bug("internal error in SET_INDPW_FOCK: WDES%NKDIM is too small for HF " // &
            str(WDES%NKDIM) // " " // str(KPOINTS_F%NKPTS), "mkpoints_full.F", 1070)
      ENDIF
      IF (SIZE(WDES%NINDPW,2)<KPOINTS_F%NKPTS .OR. SIZE(WDES%IGX,2)<KPOINTS_F%NKPTS ) THEN
         CALL vtutor%bug("internal error in SET_INDPW_FOCK: WDES arrays not properly allocated", "mkpoints_full.F", 1073)
      ENDIF
...
! if this is not the case, we are in trouble
            IF (IGRIDIND_MERGED(MP(NG1,NGX),MP(NG2,NGY),MP(NG3,NGZ))==0) THEN
               CALL vtutor%bug("internal error in SET_INDPW_FULL: G vector not found " // str(NK) &
                  ...
                  " " // str(MP(NG2,NGY)) // " " // str(MP(NG3,NGZ)), "mkpoints_full.F", 1166)
            ENDIF
            ...
            IF (IGRIDIND(MP(NG1,NGX),MP(NG2,NGY),MP(NG3,NGZ))/=0) THEN
               IF ( NGVEC>SIZE(WDES%NINDPW,1)) THEN
                  CALL vtutor%bug("internal error in SET_INDPW_FULL: insufficient memory (see wave.F " &
                     // "safeguard) " // str(NGVEC) // " " // str(SIZE(WDES%NINDPW,1)), "mkpoints_full.F", 1174)
               ENDIF
...
! local
    ...
    IF (NK>KPOINTS_F%NKPTS) THEN
       CALL vtutor%bug("internal error in KPOINT_IN_FULL_GRID: can not find " // str(VKPT), "mkpoints_full.F", 1288)
    ENDIF
    ...
    IF (KPOINT_IN_FULL_GRID/= NK_TEST .AND. NK_TEST /= -1) THEN
       CALL vtutor%bug("internal error in VASP: KPOINT_IN_FULL_GRID is wrong " // str(NK_TEST) // &
          " " // str(KPOINT_IN_FULL_GRID) // " " // str(VKPT(1:3) - KPOINTS_F%VKPT(1:3,NK)), "mkpoints_full.F", 1297)
    ENDIF
             ...
             DO IZ=1,KPOINTS_F%NKPZ
                IF (KPOINTS_F%MAP_INTO_BZ(IX, IY, IZ)==-1) THEN
                   CALL vtutor%bug("internal error in VASP: SET_KPOINT_MAP_INTO_BZ not all k-points " &
                      // "found " // str(IX) // " " // str(IY) // " " // str(IZ), "mkpoints_full.F", 1344)
                ENDIF
             ENDDO
          ENDDO
       ENDDO
...
! we are 1._q
            ...
            ENDDO
            IF (.NOT. LEXIST) THEN
               CALL vtutor%bug("internal error in SMALL_SPACE_GROUP_WEIGHTS: NK2 not found", "mkpoints_full.F", 1531)
            ENDIF
         ENDDO
      ENDDO
!      WRITE(*,'(16F4.1)') WEIGHT_K_POINT_PAIR_SMALL_GROUP(NK1,:)
      IF (SUM(WEIGHT_K_POINT_PAIR_SMALL_GROUP(NK1,:)) /=KPOINTS_F%NKPTS_NON_ZERO) THEN
         CALL vtutor%bug(str(WEIGHT_K_POINT_PAIR_SMALL_GROUP(NK1,:)) // "\n internal error in " &
            // "SMALL_SPACE_GROUP_WEIGHTS: NK2 not found", "mkpoints_full.F", 1538)
      ENDIF
...
!*********************************************************************
  ...
  SUBROUTINE ROTATE_WAVE_CHARACTER_LOW(ROT_HANDLE, P, LATT_CUR, WDES1, W, W_NEW, LINV, ISYMOP, ROTMAP, RSSYMOP)
...
!! USE moffload
    ...
    INTEGER :: ISYMOP(3,3)
    INTEGER :: ROTMAP(:)
    COMPLEX(q),INTENT(IN) :: RSSYMOP(2,2)
! local
    INTEGER :: NPRO, NT, NI, NI_GLBL, NPROP, NIP, ISPINOR
! local for setup
    COMPLEX(q) :: CPROJ(WDES1%NPRO_TOT)

    CALL SET_ROT_HANDLE(P,LATT_CUR,WDES1,ROT_HANDLE,ISYMOP)

!$ACC ENTER DATA CREATE(CPROJ) 
    CALL MRG_PROJ(WDES1,CPROJ(1),W%CPROJ)

!$ACC PARALLEL LOOP COLLAPSE(2) GANG PRESENT(WDES1,LINV,ISYMOP,ROTMAP,ROT_HANDLE,CPROJ,W_NEW) &
!$ACC PRIVATE(NT,NPRO,NI_GLBL,NIP,NPROP) 
...
# 1683
          ...
          CALL ROTATE_VECTOR( LINV,CPROJ(NPROP),W_NEW%CPROJ(NPRO), &
...
!$ACC EXIT DATA DELETE(CPROJ) 
    ...
    CALL ROTATE_WAVE_CHARACTER_SPIN(WDES1, W_NEW%CPROJ, RSSYMOP )


  END SUBROUTINE ROTATE_WAVE_CHARACTER_LOW

!*********************************************************************
!
! high level call to ROTATE_WAVE_CHARACTER_LOW uses the symmetry operations stored
! in KPOINTS_FULL
!
!*********************************************************************
...
!! USE moffload
    ...
    CALL ROTATE_WAVE_CHARACTER_LOW(ROT_HANDLE, P, LATT_CUR, WDES1, W, W_NEW, KPOINTS_FULL%LINV(WDES1%NK),&
                                                                             KPOINTS_FULL%ISYMOP(:,:,WDES1%NK),&
                                                                             KPOINTS_FULL%ROTMAP(:,WDES1%NK),&
                                                                             KPOINTS_FULL%RSSYMOP(:,:,WDES1%NK))
...
!*********************************************************************
  ...
  SUBROUTINE SET_ROT_HANDLE(P,LATT_CUR,WDES1,ROT_HANDLE,ISYMOP)
    USE wave_high
    USE pseudo
    USE lattice
    USE nonl_high
...
! this array is compatible to seriel version
       ...
       IF (NPRO-1 /= WDES1%NPRO_TOT/WDES1%NRSPINORS) THEN
          CALL vtutor%bug("internal error in ROTATE_WAVE_CHARACTER: NPRO is not correct " // &
             str(NPRO) // " " // str(WDES1%NPRO_TOT), "mkpoints_full.F", 1791)
       ENDIF
...
!$ACC ENTER DATA COPYIN(ROT_HANDLE) 
!$ACC ENTER DATA COPYIN(ROT_HANDLE%NPRO_NI,ROT_HANDLE%NPS,ROT_HANDLE%LPS) 
!$ACC ENTER DATA CREATE(ROT_HANDLE%SL) 
    ENDIF

    IF (ROT_HANDLE%NK/= WDES1%NK) THEN
       ROT_HANDLE%NK = WDES1%NK
       CALL SETUP_SYM_LL(ROT_HANDLE%MMAX,ROT_HANDLE%LDIM,ISYMOP(:,:), &
            ROT_HANDLE%SL,LATT_CUR%A,LATT_CUR%B)
!$ACC UPDATE DEVICE(ROT_HANDLE%NK,ROT_HANDLE%SL) IF_PRESENT 
    ENDIF
...
!$ACC EXIT DATA DELETE(ROT_HANDLE%SL,ROT_HANDLE%NPRO_NI,ROT_HANDLE%NPS,ROT_HANDLE%LPS,ROT_HANDLE) FINALIZE 
...
! per orbital each 1-rank must be able to store for orbitals at
...
! orbital each 1-rank must be able to store for orbitals at
...
! i.e. the plane wave coefficients for the rotated wave function
    IF (LSHIFT .AND. LINV) THEN
!$ACC PARALLEL LOOP PRESENT(C_ROT,C,CPHASE) 
       DO M=1,NPL_ORIG
          C_ROT(M)=CONJG(C(M))*CPHASE(M)
       ENDDO
    ELSE IF (LSHIFT) THEN
!$ACC PARALLEL LOOP PRESENT(C_ROT,C,CPHASE) 
       ...
       ENDDO
    ELSE IF (LINV) THEN
!$ACC PARALLEL LOOP PRESENT(C_ROT,C) 
       DO M=1,NPL_ORIG
          C_ROT(M)=CONJG(C(M))
       ENDDO
    ELSE
...
!$ACC PARALLEL LOOP PRESENT(CR) 
    DO M=1,GRID%MPLWV
       CR(M)=(0.0_q,0.0_q)
    ENDDO

! now merge from all nodes using global sum
!$ACC ENTER DATA CREATE(C_TMP) 
!$ACC KERNELS PRESENT(C_TMP) 
    C_TMP=0
!$ACC END KERNELS
!$ACC PARALLEL LOOP PRESENT(C_TMP,C_ROT) 
    DO M=1,NPL_ORIG
...
!DIR$ IVDEP
!OCL NOVREC
!$ACC PARALLEL LOOP PRESENT(CR,NINDPW,C_TMP,NG_INDEX) 
    DO M=1,NPL
       CR(NINDPW(M))=C_TMP(NG_INDEX(M))
    ENDDO
!$ACC EXIT DATA DELETE(C_TMP) 
! in place fft from reciprocal to real space
    CALL FFT3D(CR,GRID,1)
    RETURN
...
!***********************************************************************
!
! rotate a vector (G+q) from (1._q,0._q) q-point to another
!
!***********************************************************************
