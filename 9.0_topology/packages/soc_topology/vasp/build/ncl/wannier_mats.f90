...
!>
!> @var NUM_PRIM_ORBITALS  Number of orbitals in (1._q,0._q) primitive cell
...
!_______________________________________________________________________________
   TYPE WANNIER_DESCRIPTOR_T
      ...
      PROCEDURE,  PUBLIC   :: INIT => WANNIER_DESCRIPTOR_INIT
...
!>
!> @var MAT_SIZE     Size of the matrix along (1._q,0._q) of its dimensions. This is
...
!_______________________________________________________________________________
   TYPE WANNIER_MAT_SYMM_T
      ...
      COMPLEX(q), ALLOCATABLE,                     PUBLIC   :: ELEMENTS(:, :, :)
...
!>
!> The perturbation is confined to (1._q,0._q) primitive cell with the number of modes
   ...
   SUBROUTINE CALC_WANNIER_CENTERS( &
      ...
      DO I = 1, 3
         ...
         CALL CALC_WANNIER_SHIFT_MATRIX( &
            ...
            LMDIM, LOVERL, IRDMAX, G_INDICES, SPIN, SHIFT_MAT)
...
! Yes, index order is swapped; avoids unnecessary allocations.
   ...
   SUBROUTINE WANNIER_DESCRIPTOR_INIT(THIS, &
      W_WANNIER, NUM_PRIM_ORBITALS, PRIM_CELL, GRID, GRIDC, GRIDUS, &
      C_TO_US, T_INFO, P, LMDIM, LOVERL, IRDMAX, SPIN, FORCE_ATOM_CENTER)
      ...
      THIS%NUM_PRIM_ORBITALS = NUM_PRIM_ORBITALS
      ...
      CALL CALC_WANNIER_CENTERS( &
         ...
         T_INFO, P, LMDIM, LOVERL, IRDMAX, SPIN, THIS%CENTERS)

      IF (MY_FORCE) THEN
         CALL THIS%ENFORCE_ATOM_CENTERED()
      ELSE
   ...
   SUBROUTINE WANNIER_DESCRIPTOR_WRITE_HDF5(THIS, LOC_ID, GROUP_PATH)
      ...
      USE msymmetry,    ONLY: WRITE_MIN_IMAGES_HDF5
      IMPLICIT NONE
      ...
      INTEGER :: DIMS(3)
      INTEGER :: I1, I2, I3
      TYPE(MIN_IMAGE_T), ALLOCATABLE :: BUFFER(:, :, :)

      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/prim_orbital_count", THIS%NUM_PRIM_ORBITALS),"wannier_mats.F",404)
      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/prim_centers", MATMUL(THIS%PRIM_CELL%SUPER_TO_PRIM, THIS%CENTERS)),"wannier_mats.F",405)

      DIMS = SHAPE(THIS%MIN_IMAGES)
      ALLOCATE(BUFFER(DIMS(3), DIMS(1), DIMS(2)))
      
      DO I3 = 1, SIZE(THIS%MIN_IMAGES, 3)
         DO I2 = 1, SIZE(THIS%MIN_IMAGES, 2)
            DO I1 = 1, SIZE(THIS%MIN_IMAGES, 1)
               BUFFER(I3, I1, I2) = THIS%MIN_IMAGES(I1, I2, I3)
            ENDDO
         ENDDO
      ENDDO
      CALL WRITE_MIN_IMAGES_HDF5(BUFFER, THIS%PRIM_CELL%PRIM_LATT, LOC_ID, GROUP_PATH // "/min_images")

      DEALLOCATE(BUFFER)
      DIMS = SHAPE(THIS%MIN_IMAGES_TO_ATOM)
      ALLOCATE(BUFFER(DIMS(2), DIMS(1), DIMS(3)))
      
      DO I2 = 1, SIZE(THIS%MIN_IMAGES_TO_ATOM, 2)
         DO I1 = 1, SIZE(THIS%MIN_IMAGES_TO_ATOM, 1)
            BUFFER(I2, I1, :) = THIS%MIN_IMAGES_TO_ATOM(I1, I2, :)
         ENDDO
      ENDDO
      CALL WRITE_MIN_IMAGES_HDF5(BUFFER, THIS%PRIM_CELL%PRIM_LATT, LOC_ID, GROUP_PATH // "/min_images_to_atom")
   END SUBROUTINE
   ...
   SUBROUTINE WANNIER_DESCRIPTOR_READ_HDF5(THIS, PRIM_CELL, LOC_ID, GROUP_PATH)
      ...
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/prim_orbital_count", THIS%NUM_PRIM_ORBITALS),"wannier_mats.F",445)
      IF (ALLOCATED(THIS%CENTERS)) DEALLOCATE(THIS%CENTERS)
      ALLOCATE(THIS%CENTERS(3, THIS%NUM_PRIM_ORBITALS))
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/prim_centers", THIS%CENTERS),"wannier_mats.F",448)
      THIS%CENTERS = MATMUL(PRIM_CELL%PRIM_TO_SUPER, THIS%CENTERS)
...
!>                         must equal number of Wannier orbitals in (1._q,0._q)
!>                         primitive cell
!_______________________________________________________________________________
   TYPE(WANNIER_MAT_SYMM_T) FUNCTION WANNIER_MAT_SYMM_PACK(DESCRIPTOR, MATRIX_STRIP) RESULT(WANNIER_MAT)
      ...
      COMPLEX(q),                         INTENT(IN)  :: MATRIX_STRIP(:, :)
      ...
      IF (MAT_SHAPE(1) /= WANNIER_MAT%MAT_SIZE) CALL vtutor%bug( &
         ...
         "found in descriptor (" // str(WANNIER_MAT%MAT_SIZE) // ").", "wannier_mats.F", 510)
      ...
      IF (MAT_SHAPE(2) /= DESCRIPTOR%NUM_PRIM_ORBITALS) CALL vtutor%bug( &
         ...
         "found in descriptor (" // str(DESCRIPTOR%NUM_PRIM_ORBITALS) // ").", "wannier_mats.F", 514)
...
!_______________________________________________________________________________
   SUBROUTINE WANNIER_MAT_SYMM_UNPACK(THIS, MATRIX_OUT)
      ...
      COMPLEX(q),                      INTENT(OUT) :: MATRIX_OUT(:, :)
...
! Return MAT_SIZE x N_ORBS strip of matrix
         ...
         ELSE
            ...
            " /= " // str(THIS%MAT_SIZE) // " and /= " // str(N_ORBS), "wannier_mats.F", 573)
         ENDIF
      END ASSOCIATE
   END SUBROUTINE
...
! Perform fourier sum to primitive cell.
      ...
      EIGEN_VECS = (EIGEN_VECS + CONJG(TRANSPOSE(EIGEN_VECS))) * 0.5_q
...
! Perform the diagonalization.
   ...
   SUBROUTINE WANNIER_MAT_SYMM_WRITE_HDF5(THIS, LOC_ID, GROUP_PATH)
      ...
      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/matrix_size", THIS%MAT_SIZE),"wannier_mats.F",680)
      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/elements", THIS%ELEMENTS),"wannier_mats.F",681)

# 685

      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/is_real", .FALSE.),"wannier_mats.F",686)
   ...
   SUBROUTINE WANNIER_MAT_SYMM_READ_HDF5(THIS, DESCRIPTOR, LOC_ID, GROUP_PATH)
      ...
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/is_real", IS_REAL),"wannier_mats.F",709)
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/matrix_size", THIS%MAT_SIZE),"wannier_mats.F",710)
...
# 717
      ...
      IF (IS_REAL) THEN
         BLOCK
            ...
            ALLOCATE(BUFFER(N_ORBS, N_ORBS, N_CELLS))
            call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/elements", BUFFER),"wannier_mats.F",722)
            THIS%ELEMENTS = REAL(BUFFER, q)
         END BLOCK
      ELSE
         call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/elements", THIS%ELEMENTS),"wannier_mats.F",726)
      ENDIF
...
!_______________________________________________________________________________
   TYPE(WANNIER_MAT_MODES_T) FUNCTION WANNIER_MAT_MODES_PACK(DESCRIPTOR, MATRIX_IN) RESULT(WANNIER_MAT)
      ...
      IF (MAT_SIZE /= SIZE(MATRIX_IN, 2)) CALL vtutor%bug( &
         "Input matrix not square! " // str(SHAPE(MATRIX_IN)), "wannier_mats.F", 790)
...
!
!> @brief Allows to set a Wannier-space matrix explicitly for (1._q,0._q) individual
!> mode.
!>
...
! Loop over all Cartesian directions or just the requested (1._q,0._q).
         DO CART_IDX = 1, 3
...
!> vectors in the Brillouin (1._q,0._q).
!>
!> @param[in] THIS            Instance reference
...
!_______________________________________________________________________________
   ...
   SUBROUTINE WANNIER_MAT_MODES_WRITE_HDF5(THIS, LOC_ID, GROUP_PATH)
      ...
      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/mode_count", THIS%NUM_MODES),"wannier_mats.F",1152)
      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/matrix_size", THIS%MAT_SIZE),"wannier_mats.F",1153)
      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/elements", THIS%ELEMENTS),"wannier_mats.F",1154)

# 1158

      call vh5_error(vh5_write(LOC_ID, GROUP_PATH // "/is_real", .FALSE.),"wannier_mats.F",1159)
   ...
   SUBROUTINE WANNIER_MAT_MODES_READ_HDF5(THIS, DESCRIPTOR, LOC_ID, GROUP_PATH)
      ...
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/is_real", IS_REAL),"wannier_mats.F",1182)
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/mode_count", THIS%NUM_MODES),"wannier_mats.F",1183)
      call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/matrix_size", THIS%MAT_SIZE),"wannier_mats.F",1184)
...
# 1191
      ...
      IF (IS_REAL) THEN
         BLOCK
            REAL(q), ALLOCATABLE :: BUFFER(:, :, :, :, :)
            ALLOCATE(BUFFER(N_ORBS, N_ORBS, N_CELLS, N_CELLS, THIS%NUM_MODES))
            call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/elements", BUFFER),"wannier_mats.F",1196)
            THIS%ELEMENTS = REAL(BUFFER, q)
         END BLOCK
      ELSE
         call vh5_error(vh5_read(LOC_ID, GROUP_PATH // "/elements", THIS%ELEMENTS),"wannier_mats.F",1200)
      ENDIF
