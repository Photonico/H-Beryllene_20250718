...
SUBROUTINE QP_PADE_FIT_FROM_TAU(W,WMEAN,IMAG_GRIDS,&
    ...
    COMPLEX(q)                   :: SIGMAO(:,:,:,:)      !< diagonal of self-energy in tau domain
    COMPLEX(q)                   :: SIGMAU(:,:,:,:)      !< diagonal of self-energy in tau domain
    INTEGER                :: NKPTS_IRZ            !< number of KPOINTS in the IBZ
    INTEGER                :: NBANDSGW             !< number of bands to be written to file
    TYPE(in_struct)        :: IO
...
! store self-energy to auxillary arrays, this includes (0._q,0._q) point
             DO I=1,NOMEGA
...
! negative imaginary axis to negative real axis to avoid the branch cut
    ...
    IF (LPADE_FIT) THEN
       CALL M_sum_z(W%WDES%COMM,SVD_COEFF_DN,SIZE(SVD_COEFF_DN))
    ENDIF 
    CALL M_sum_z(W%WDES%COMM,SVD_COEFF_UP,SIZE(SVD_COEFF_UP))
    CALL M_sum_z(W%WDES%COMM,SIGMA_SVD,SIZE(SIGMA_SVD))
    CALL M_sum_z(W%WDES%COMM,ZEROS_SVD,SIZE(ZEROS_SVD))
    CALL M_sum_d(W%WDES%COMM,QP_SVD,SIZE(QP_SVD))
...
! if still problematic, we report error and use original NOMEGA (1._q,0._q)
...
! negative imaginary axis to negative real axis to avoid the branch cut
    ...
    IF (LPADE_FIT) THEN
       CALL M_sum_z(W%WDES%COMM,FRAC_COEFF_DN,SIZE(FRAC_COEFF_DN))
    ENDIF
    CALL M_sum_z(W%WDES%COMM,FRAC_COEFF_UP,SIZE(FRAC_COEFF_UP))
    CALL M_sum_z(W%WDES%COMM,SIGMA_FRAC,SIZE(SIGMA_FRAC))
    CALL M_sum_z(W%WDES%COMM,ZEROS_FRAC,SIZE(ZEROS_FRAC))
    CALL M_sum_d(W%WDES%COMM,QP_FRAC,SIZE(QP_FRAC))
    CALL M_sum_i(W%WDES%COMM,I_FRAC,SIZE(I_FRAC))


!=====================================================================!
! third, we compare SVD and fractional version to select the 'best' (1._q,0._q)
!=====================================================================!
    DO ISP=1, W%WDES%ISPIN  
      DO NK=1,NKPTS_IRZ
         DO NB=1,NBANDSGW
...
! for Zeros
    ...
    CALL M_sum_i(W%WDES%COMM,TAG,SIZE(TAG))
...
! otherwise choose the (1._q,0._q) that is closer to 0.8_q
...
! if still problematic, we report error and use original (1._q,0._q)
...
SUBROUTINE WRITE_QP_SHIFT(W,NKPTS_IRZ,NBANDSGW,TAG,IU6)
    ...
    DO ISP=1, W%WDES%ISPIN
       ...
       DO NK=1,NKPTS_IRZ
          IF (W%WDES%WTKPT(NK)==0) CYCLE ! (0._q,0._q) k-point weighted points are useless right now in the GW code
          IF (IU6>=0) THEN
...
! quick check
    IF (MOD(NOMEGA_SVD,2) /=0 ) THEN
      CALL vtutor%bug("internal error in PADE_SVD_COEFF"//str( NOMEGA_SVD), "pade_fit.F", 2292)
    ENDIF
...
! quick check
    IF (MOD(NOMEGA_SVD,2) /=0 ) THEN
      CALL vtutor%bug("internal error in PADE_SVD_COEFF"//str( NOMEGA_SVD), "pade_fit.F", 2355)
    ENDIF
...
!A(I,J+1)=-A(I,J)*CMPLX(REAL(IH(NOMEGA-I+1),q),AIMAG( IH(NOMEGA-I+1) ),KIND=qd)
         A(I,J+1)=-A(I,J)*CMPLX(R1,R2,KIND=qd)
       ENDDO
       WEIGHT=0
       DO J=1,NOMEGA_SVD
          WEIGHT=WEIGHT+ABS(A(I,J))
...
!B(I)=IH(NOMEGA-I+1)*(IZ(NOMEGA-I+1)**N)/WEIGHT
       ...
       R1 = REAL( CD , q)
       R2 = AIMAG( CD )
       B(I)= CMPLX(R1,R2,KIND=qd)/WEIGHT
    ENDDO
...
! quick return
    IF (M<N) THEN
      CALL vtutor%bug('internal error in VASP: SOLVE_LSQ_QR: M <N in SOLVE_LSQ_QR '//str(M)//" "//str(N), "pade_fit.F", 2482)
    ENDIF
...
! obtain optimal working array sizes
    ...
    LWORK=INT(WORK(1))
    IF ( LWORK <= 0 )  THEN
       CALL vtutor%bug('internal error in VASP: SOLVE_LSQ_QR: illegal size from ZGELS '//str(LWORK), "pade_fit.F", 2501)
    ENDIF
...
! actural LSQ QR sovle AX=B comes now
    CALL ZGELS('N',M,N,1,A2,LDA,B2,LDB,WORK,LWORK,INFO)
    IF ( INFO/=0) THEN
       CALL vtutor%bug('internal error in VASP: SOLVE_LSQ_QR: ZGELS reports'//str(INFO), "pade_fit.F", 2510)
    ENDIF
...
! quick check
    IF (MOD(NOMEGA,2) /=0 ) THEN
      CALL vtutor%bug( "internal error in PADE_LINEAR_COEFF "//str(NOMEGA), "pade_fit.F", 2543)
    ENDIF
...
SUBROUTINE ACONT_PADE_SVD(NOMEGA,IZ,IH,M,RZ,RH,IU0,NOMEGA_SVD,WMEAN_CELTOT)
    ...
    INTEGER,   INTENT(IN)   :: M             !< number of real data sets (rz(i),rh(i),i=1,...M))
    COMPLEX(q),INTENT(IN)   :: RZ(M)         !< real frequencies for which analytic continuation is 1._q
    COMPLEX(q),INTENT(OUT)  :: RH(M)         !< function evaluated at real frequencies rz(i)
    INTEGER,   INTENT(IN)   :: IU0
    INTEGER,   INTENT(IN)   :: NOMEGA_SVD
    COMPLEX(q),OPTIONAL     :: WMEAN_CELTOT  !< mean-field HF eigenvalue
...
SUBROUTINE QQRDC(A, M, N, QQ, R, P, PIVOTING)
    ...
    LOGICAL              :: PIVOTING
! local
    COMPLEX(qd), ALLOCATABLE :: H(:,:)
    COMPLEX(qd), ALLOCATABLE :: V(:,:)
    COMPLEX(qd), ALLOCATABLE :: Y(:)
    REAL(qd), ALLOCATABLE    :: NORMS(:)
    COMPLEX(qd)              :: ALPHA
    INTEGER           :: I, J, K
    INTEGER           :: PIVCOL
    REAL(qd)           :: TMP, R1, R2
...
!ALPHA = SQRT(SUM(Y*Y)) * (-REAL(1,KIND=qd)) * SIGN_QD( REAL( Y(I), qd ) )
      TMP = 0 
      DO K = 1, SIZE( Y ) 
         TMP = TMP + ABS( REAL( Y(K)*Y(K), qd ) )
      ENDDO
      R1 = -SIGN_QD( REAL( Y(I), qd ) )
      R2 = 0
      ALPHA = CMPLX(SQRT( TMP ),REAL(0,KIND=qd),KIND=qd) * CMPLX(R1,R2,KIND=qd)
...
! compute the Householder matrix
      ...
      ENDDO
!H = H - REAL(2,KIND=qd)*MATMUL_QDC(V, TRANSPOSE_QDC(V)) / SUM(V(:,I) * V(:,I))
      H = H - REAL(2,KIND=qd)*MATMUL_QDC(V, TRANSPOSE_QDC(V)) / TMPC
...
SUBROUTINE QQRSOLVE(A, M, N, B, X)
    ...
    INTEGER           :: M, N
! local
    COMPLEX(qd), ALLOCATABLE :: QQ(:,:), R(:,:)
    COMPLEX(qd), ALLOCATABLE :: QB(:)
    INTEGER, ALLOCATABLE :: P(:)
    INTEGER :: I
...
!*****************************
   FUNCTION MATMUL_QDC( A, B )
       COMPLEX(qd) :: A(:,:), B(:,:)
       COMPLEX(qd) :: MATMUL_QDC(SIZE(A,1),SIZE(B,2))
! logical
       INTEGER    :: I, J, K
       COMPLEX(qd) :: R(SIZE(A,1),SIZE(B,2))
...
!*****************************
   FUNCTION MATMULX_QDC( A, B )
       COMPLEX(qd) :: A(:,:), B(:)
       COMPLEX(qd) :: MATMULX_QDC(SIZE(A,1))
! logical
       INTEGER    :: I, J, K
       COMPLEX(qd) :: R(SIZE(A,1))
...
!*****************************
   FUNCTION TRANSPOSE_QDC( A )
       COMPLEX(qd) :: A(:,:)
       COMPLEX(qd) :: TRANSPOSE_QDC(SIZE(A,2),SIZE(A,1))
! logical
       INTEGER    :: I, J
