...
! local variables
      ...
      LDO_HYPERFINE=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LHYPERFINE', LDO_HYPERFINE, IERR, WRITEXMLINCAR)

      ALLOCATE(NUCLEAR_GYROMAGN_RATIO(NTYP))
      NUCLEAR_GYROMAGN_RATIO=1._q
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'NGYROMAG', NUCLEAR_GYROMAGN_RATIO, NTYP, IERR, WRITEXMLINCAR)
...
!
!***********************************************************************
      SUBROUTINE HYPERFINE_RAD(T_INFO,NI,PP,XC,RHOPS,RHOAE,POTAE,COCC)
      USE poscar
      USE pseudo
      USE constant
      USE radial
...
! that was used for the generation of the PAW data set
     ...
     &   WUP,N,LC,EUP,(POTAE(1:PP%R%NMAX,1,1)+POTAE(1:PP%R%NMAX,1,2))/2._q) 
      ...
      DO I=1,PP%R%NMAX
         RHO(I,1,1)=RHOAE(I,1,1)+RHOC(I)
      ENDDO
!     RHO(:,1,1)=RHOAE(:,1,1)+PP%RHOAE(:)
      DO I=1,PP%R%NMAX
         RHO(I,1,2)=RHOAE(I,1,2)
      ENDDO
...
! check for convergence
         ...
         DO I=1,PP%R%NMAX
            RHO(I,1,1)=RHO(I,1,1)+RHOAE(I,1,1)
         ENDDO
         DO I=1,PP%R%NMAX
            RHO(I,1,2)=RHO(I,1,2)+RHOAE(I,1,2)
         ENDDO
...
# 754
      ...
      IF (INFO/=0) THEN
         CALL vtutor%error("HYPERFINE_RAD: ERROR: DGETRF failed: " // str(INFO))
      ENDIF

      CALL DGETRS('T',4,1,A,4,IPVT,B,4,INFO)
      IF (INFO/=0) THEN
         CALL vtutor%error("HYPERFINE_RAD: ERROR: DGETRS failed: " // str(INFO))
      ENDIF
...
!     ENDDO
      ...
      DO I=0,3
         ...
         ENDDO
         IF (J>JMAX) THEN
            CALL vtutor%error("HYPERFINE_RAD: ERROR: taylor series did not converge: " // str(I) // &
               " " // str(JMAX) // " " // str(J))
         ENDIF
      ENDDO
