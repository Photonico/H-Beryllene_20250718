...
!*********************************************************************
  ...
  SUBROUTINE PUSH_KPOINTS_ORIG
    ...
    IF (ISTACK_KPOINTS >= NSTACK_KPOINTS) THEN
       CALL vtutor%bug('internal error in VASP: PUSH_KPOINTS_ORIG exhausted the stack', "mkpoints_change.F", 52)
    ENDIF
...
! that is why KPOINTS_ORIG and KPOINTS_FULL_ORIG need to be deallocated below
  ...
  SUBROUTINE POP_KPOINTS_ORIG
    ...
    IF (ISTACK_KPOINTS == 0) THEN
       CALL vtutor%bug('internal error in VASP: POP_KPOINTS_ORIG stack is empty, pop not possible', "mkpoints_change.F", 69)
    ENDIF
...
! local
    ...
    IF(.NOT. ASSOCIATED(KPOINTS_FULL_ORIG)) THEN
       CALL vtutor%bug("RE_READ_KPOINTS: KPOINTS_FULL_ORIG not associated \n call " &
          // "routine USE_FULL_KPOINTS first", "mkpoints_change.F", 130)
    ENDIF
!=======================================================================
...
! workaround for IFX 2024
!KPOINTS_RD=KPOINTS_ORIG
    CALL KPOINTS_ASSIGN(KPOINTS_ORIG,KPOINTS_RD)
...
!=======================================================================
    DO NK=1,KPOINTS_ORIG%NKPTS
       ...
       IF (NKP>NKPTS_RD) THEN
          MSG = 'RE_READ_KPOINTS: the new IBZ does not contain all k-points of the original IBZ.\n'
          IF (.NOT.KPOINTS%LGAMMA) &
             MSG = MSG // '\n... try a Gamma-centered k-lattice'
          IF (KPOINTS%LKBLOWUP) &
             MSG = MSG // '\n... try setting KBLOWUP=.FALSE.'
          IF (KPOINTS%MODE == GeneratingLattice) &
             MSG = MSG // '\n... try a different generating k-lattice'
          IF (ISYM>0) &
...
! search k-points that have not yet been stored in the KPOINTS structure
       ...
       ENDDO
       IF (NKP>NKPTS_RD) THEN
          CALL vtutor%bug("RE_READ_KPOINTS: KPOINTS list is empty", "mkpoints_change.F", 233)
       ENDIF
    ...
    DO N=1,KPOINTS_RD%NTET
       DO I=1,4
          IF (KPOINTS_RD%IDTET(I,N).NE.0) THEN
             CALL vtutor%bug("RE_READ_KPOINTS: tetrahedron list screwed up! " // &
                str(I) // " " // str(N) // " " // str(KPOINTS_RD%IDTET(I,N)), "mkpoints_change.F", 265)
          ENDIF
       ENDDO
    ENDDO
...
! at this point we can recalculate the full set of kpoints
    ...
    IF (KPOINTS_FULL%NKPTS_NON_ZERO  /= KPOINTS_FULL_ORIG%NKPTS_NON_ZERO) THEN
       IF (IU0>=0) THEN
          WRITE(IU0,*) 'RE_READ_KPOINTS: the total number of non zero k-points in the full Brillouine zone has changed',KPOINTS_FULL%NKPTS_NON_ZERO, KPOINTS_FULL_ORIG%NKPTS_NON_ZERO
...
!*********************************************************************
!
!> @brief RE_GEN_LAYOUT regenerates the data layout and checks whether
...
!   gamma only: nothing usefull can be 1._q here
...
!***********************************************************************
    ...
    SUBROUTINE GENERATE_KPOINTS_TRANS(GRID, NKPTS_ORIG, WDES, KPOINTS_F, KPOINTS_TRANS)
      ...
      TYPE (skpoints_full) :: KPOINTS_F     ! k-points in the full BZ
...
! allocate arrays and set some values to (0._q,0._q)
      IF (WDES%NKPTS < NKPTS_ORIG) THEN
         CALL vtutor%bug("GENERATE_KPOINTS_TRANS: not sufficient k-points " // &
            str(WDES%NKPTS) // " " // str(NKPTS_ORIG), "mkpoints_change.F", 737)
      ENDIF
...
! number of G vectors for original k in IRBZ must be identical
         ...
         IF (NGVECI /= WDES%NGVECTOR(NK)) THEN
            CALL vtutor%bug("internal error in GENERATE_KPOINTS_TRANS: number of G-vector changed " &
               // "in star " // str(NGVECI) // " " // str(WDES%NGVECTOR(NK)), "mkpoints_change.F", 797)
         ENDIF
...
! KPOINTS_TRANS%NINDPW(NI,NK) stores the index of G' = S G
            ...
            IF (KPOINTS_TRANS%NINDPW(NI,NK-NKPTS_START)==0) THEN
               CALL vtutor%bug("internal error in GENERATE_KPOINTS_TRANS: G vector not found " // &
                  ...
                  // " " // str(NG2I) // " " // str(NG3I) // " " // str(NI), "mkpoints_change.F", 826)
            ENDIF
...
! KPOINTS_TRANS%NINDPW(NI,NK) stores the index of G' = S G
            ...
            IF (KPOINTS_TRANS%NINDPW(NI,NK)==0) THEN
               CALL vtutor%bug("internal error in KPOINTS_TRANS_Q: G vector not found " // str(NK) &
                  ...
                  str(NROTK), "mkpoints_change.F", 1056)
            ENDIF
...
!***********************************************************************
  ...
  SUBROUTINE GENERATE_ROT_HANDLE(ROT_HANDLE, KPOINTS_TRANS, P, LATT_CUR, NK, WDES1 )
    ...
    CALL GENERATE_ROT_HANDLE_LOW(ROT_HANDLE, KPOINTS_TRANS%ISYMOP(:,:,NK), P, LATT_CUR, NK, WDES1 )

  END SUBROUTINE GENERATE_ROT_HANDLE

  SUBROUTINE GENERATE_ROT_HANDLE_LOW(ROT_HANDLE, ISYMOP, P, LATT_CUR, NK, WDES1 )
    USE wave_high
    USE pseudo
    USE lattice
    USE nonl_high
...
! and copy some stuff from the potcar structure into ROT_HANDLE
  ...
  END SUBROUTINE GENERATE_ROT_HANDLE_LOW
