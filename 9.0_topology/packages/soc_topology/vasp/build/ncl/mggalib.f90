...
! test
      ...
      PARAMETER (zinfinity=2._q)
! test
!$ACC ROUTINE(COE_RPKZB_TPSS) SEQ
!$ACC ROUTINE(CORGGA_TPSS) SEQ
...
!$ACC ROUTINE(VS98C) SEQ
!$ACC ROUTINE(M06CSS) SEQ
...
!      PARAMETER (PI =3.141592653589793238d0)
      ...
      PARAMETER (CREGU = (/ 1.0_q, -0.64_q, -0.4352_q,  -1.535685604549_q,&
        ...
        &             -0.051848879792_q /) )
!$ACC ROUTINE(CORGGA_0) SEQ
!$ACC ROUTINE(CORGGA_1) SEQ
...
!$ACC ROUTINE(exchange_enhancement) SEQ
...
!$ACC ROUTINE(get_dx_terms) SEQ
!$ACC ROUTINE(get_fourth_order_damp) SEQ
...
!$ACC ROUTINE SEQ
          ...
          real(kdp), parameter :: PI = 3.1415926535897932384626433832795_kdp
          real(kdp), parameter :: PI2 = PI*PI
!$ACC ROUTINE(scan_ec0) SEQ
!$ACC ROUTINE(scan_ec1) SEQ
...
!$ACC ROUTINE SEQ
          ...
          real(kdp) :: AFIX_T
!$ACC ROUTINE(lda_0) SEQ
!$ACC ROUTINE(lsda_1) SEQ
!$ACC ROUTINE(get_y) SEQ
!$ACC ROUTINE(get_del_y) SEQ
...
!$ACC ROUTINE(tau_rpp_unp) SEQ
!$ACC ROUTINE(pc_tau_unp) SEQ
!$ACC ROUTINE(VSCAN2x) SEQ
!$ACC ROUTINE(VSCAN2c) SEQ
...
! --------------------------------------------
      ...
      REAL(q), PARAMETER :: THREE=3.0_q
      REAL(q), PARAMETER :: FOUR=4.0_q
!$ACC ROUTINE(calc_gx) SEQ
!$ACC ROUTINE(calc_dgx) SEQ
!$ACC ROUTINE(ChebEv) SEQ
!$ACC ROUTINE(ChebDerEv) SEQ
...
!$ACC ROUTINE(calc_gx) SEQ
!$ACC ROUTINE(calc_gnum) SEQ
!$ACC ROUTINE(calc_hx1GE4) SEQ
!$ACC ROUTINE(calc_fxGE4) SEQ
!$ACC ROUTINE(calc_dgx) SEQ
!$ACC ROUTINE(calc_dgnum) SEQ
!$ACC ROUTINE(calc_dhx1GE4) SEQ
!$ACC ROUTINE(calc_dfxGE4) SEQ
