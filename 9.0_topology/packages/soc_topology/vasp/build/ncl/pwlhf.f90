...
! use temporarily another WDES
    ...
    IF (WHF%WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL vtutor%error("PW_SLATER_POTENTIAL: KPAR>1 not implemented, sorry.")
    END IF
# 140
...
!
       IF (W%FERTOT((NPOS-1)*NCPU+N,NK,ISP)/=W%FERWE(N_+NPOS-1,NK,ISP)) THEN
          CALL vtutor%bug("internal error in PW_SLATER_POTENTIAL: index into FERTOT incorrect", "pwlhf.F", 405)
       ENDIF
...
! charge and potential
    ...
    IF (W%WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL vtutor%error("PW_LHF_CORR_POTENTIAL: KPAR>1 not implemented, sorry.")
    END IF
# 564
...
! speed up when band is empty
          ...
          IF (W%FERTOT((MQ-1)*W%WDES%NB_PAR+W%WDES%NB_LOW,NK,ISP)/=W%FERWE(MQ,NK,ISP)) THEN
             CALL vtutor%bug("internal error in PW_LHF_CORR_POTENTIAL: index into full array " &
                // "incorrect " // str(MQ-1) // " " // str(MQ), "pwlhf.F", 649)
          ENDIF
...
!-----------------------------------------------------------------------
    ...
    IF (WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL vtutor%error("LHF: KPAR>1 not implemented, sorry.")
    END IF
# 931
...
! merge charge from all bands
    ...
    IF (ABS(REAL(CDCHF_HARTREE_FOCK,q)-REAL(CDCHF,q))>1E-2) THEN
       CALL vtutor%bug("internal error in LHF: Hartree-Fock energy and Slater energy differ " // &
          str(REAL(CDCHF_HARTREE_FOCK,q)) // " " // str(REAL(CDCHF,q)), "pwlhf.F", 1064)
    ENDIF
...
# 1081
    ...
    IF (ABS(REAL(CDCHF_HARTREE_FOCK,q)-REAL(CDCHF,q))>2E-2) THEN
       CALL vtutor%bug("internal error in LHF: Hartree-Fock energy and Slater energy differ after &
          &division by potential " // str(REAL(CDCHF_HARTREE_FOCK,q)) // " " // str(REAL(CDCHF,q)), &
          "pwlhf.F", 1103)
    ENDIF
...
! Take QSPIRAL from direct to cartesian coordinates
         ...
         CALL vtutor%bug("internal error in SETDIJ_FOCK: LSPIRAL is not supported", "pwlhf.F", 1707)
      ENDIF
...
!  IF (L<=1) WRITE(0,'("DLM",I2,10F7.4)') L,(1E6*DLM(L**2+M),M=1,(L*2)+1)
      ...
      IF (QDEP_FOCK_INDEX /= SIZE(PP%QDEP_FOCK,3)-1) THEN
         CALL vtutor%bug("internal error in SETDIJ_FOCKAE: size of QDEP_FOCK seems to be incorrect", "pwlhf.F", 1795)
      ENDIF
      ENDIF
      ENDIF
!=======================================================================
