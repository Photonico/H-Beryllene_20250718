...
! check for necessary potential typ information
          IF ((uppercase(POTENTIALTYP(1:3)) /= 'LDA') .AND. &
              (uppercase(POTENTIALTYP(1:4)) /= 'PW91') .AND. &
              (uppercase(POTENTIALTYP(1:3)) /= 'PBE')) THEN
            CALL vtutor%error("POTENTIALTYP must be present in INCAR and have value LDA, PW91 or PBE")
          ENDIF
...
!
! generate POTCAR in case it does not exist (1._q for each image)
!
      IF (LWRITEH5) THEN
...
!wait until the POTCAR file is present
        DO TRY = 1, MAXTRY
         ...
         CALL SLEEP(1)
        ENDDO
        CALL M_and(COMM_WORLD,LPOTCAR_WRITTEN,1)
  ...
  610 READ(10,'(1X,A1)',ERR=600,END=600) CSEL
     ...
     &            P(NTYP)%PSRMAX
      ...
      IF (CHANNELS+IC>LDIM) THEN
         CALL vtutor%bug("RD_PSEUDO: internal ERROR: increase LDIM", "pseudo.F", 417)
      ENDIF
...
!-----------------------------------------------------------------------
  ...
  661   CONTINUE
        ...
        IF (L2> LDIM2) THEN
           CALL vtutor%bug("RD_PSEUDO: internal ERROR increase LDIM2", "pseudo.F", 515)
        ENDIF
...
!
      ...
      IF (TAG(1:L)=='LEXCH')  THEN
        XC_DATA_PP%NAME(1)=VALUE(1:2)
        XC_DATA_PP%NXC=1
        XC_DATA_PP%LSFBXC=.FALSE.
        XC_DATA_PP%LNOXC=.FALSE.
        XC_DATA_PP%LDOLDA=.FALSE.
        XC_DATA_PP%LDOGGA=.FALSE.
        XC_DATA_PP%LDOMETAGGA=.FALSE.
        XC_DATA_PP%LUSE_LIBXC=.FALSE.
        XC_DATA_PP%LTAU=.FALSE.
        XC_DATA_PP%LLAP=.FALSE.
        XC_DATA_PP%LMU=.FALSE.
        XC_DATA_PP%LDLAP=.FALSE.
        XC_DATA_PP%ALDAX=1.0_q
        XC_DATA_PP%ALDAC=1.0_q
        XC_DATA_PP%AGGAX=1.0_q
        XC_DATA_PP%AGGAC=1.0_q
        XC_DATA_PP%AMGGAX=1.0_q
        XC_DATA_PP%AMGGAC=1.0_q
        XC_DATA_PP%AEXX=0.0_q
        XC_DATA_PP%LDASCREEN=0.0_q
        XC_DATA_PP%LDASCREENC=0.0_q
        XC_DATA_PP%LUSE_LONGRANGE_HF=.FALSE.
        XC_DATA_PP%LRANGE_SEPARATED_CORR=.FALSE.
        XC_DATA_PP%LUSE_THOMAS_FERMI=.FALSE.
        XC_DATA_PP%LUSE_MODEL_HF=.FALSE.
        XC_DATA_PP%LUSE_VDW=.FALSE.
        XC_DATA_PP%LSPIN_VDW=.FALSE.
!        XC_DATA_PP%LFCI is not determined here but in fock.F
        XC_DATA_PP%IVDW_NL=-1
        CALL SET_XC_DATA(XC_DATA_PP)
        XC_DATA_PP%NPARAM(1)=0
        XC_DATA_PP%PARAM(1,:)=0._q
        XC_DATA_PP%COEFF(1)=1._q
        XC_DATA_PP%PARAM1=0.1234000000_q
        XC_DATA_PP%PARAM2=1.00000000_q
        XC_DATA_PP%Zab_VDW=-0.8491_q
        XC_DATA_PP%GAMMA_VDW=4._q*3.1415926535_q/9._q
        XC_DATA_PP%ALPHA_VDW=0._q
        XC_DATA_PP%BPARAM=6.3_q
        XC_DATA_PP%CPARAM=0.0093_q
        XC_DATA_PP%LNO_AUG_XC=.FALSE.
        P%ID_XC=XC_DATA_PP%ID(1)

        IF (POTCAR_FOUND) THEN
          IF (XC_DATA_PP%ID(1)==ID_XC_PZ) THEN
             POTENTIALTYP(1:3) = 'LDA'
          ELSE IF (XC_DATA_PP%ID(1)==ID_XC_PW91) THEN
             POTENTIALTYP(1:4) = 'PW91'
          ELSE IF (XC_DATA_PP%ID(1)==ID_XC_PBE) THEN
             POTENTIALTYP(1:3) = 'PBE'
          ENDIF
        ENDIF
...
! local
      ...
      DO I=ILOW+2,IREAD
         ...
         IF (LL>LMAX) THEN
            CALL vtutor%bug("internal error in RDPARS_ENERGY: L is too large " // str(LL), "pseudo.F", 1119)
         ENDIF
         LN(LL)=LN(LL)+1
         IF (LN(LL)>NMAX) THEN
            CALL vtutor%bug("internal error in RDPARS_ENERGY: too many entries for L= " // str(LL), "pseudo.F", 1123)
         ENDIF
         ENERGY(LN(LL),LL)=E
      ENDDO
      ...
      DO I=1,P%LMAX
         LL=P%LPS(I)
         IF (LN(LL)==0) THEN
            CALL vtutor%bug("internal error in RDPARS_ENERGY: not sufficient entries for L= " // str(LL), "pseudo.F", 1131)
         ENDIF
...
!**********************************************************************
      ...
      SUBROUTINE POST_PSEUDO(NTYPD,NTYP_PP,NTYP,NIONS,NITYP,VCA,P,XC,INFO, &
...
!! USE moffload_struct_def
      ...
      USE xc_name
      USE string, ONLY: str
      USE tutor, ONLY: vtutor, isError, isAlert, isAdvice, newArgument, DifferentLDAXCTypes, &
...
! arryas for tutor call
      INTEGER IU6,IU0,IDIOT,CDUM,LDUM
      REAL(q)    RTUT(3)
! temporary variables
      LOGICAL LREALT
      INTEGER NT,LEXCH
...
!-----------------------------------------------------------------------
      IF (P(1)%ID_XC==ID_XC_PZ) THEN
         CTUT(1)='CA'
      ELSE IF (P(1)%ID_XC==ID_XC_PBE) THEN
         CTUT(1)='PE'
      ENDIF
      DO NT=2,NTYP
         IF (P(1)%ID_XC /= P(NT)%ID_XC) THEN
            IF (P(NT)%ID_XC==ID_XC_PZ) THEN
               CTUT(2)='CA'
            ELSE IF (P(NT)%ID_XC==ID_XC_PBE) THEN
               CTUT(2)='PE'
            ENDIF
            CTUT(3)=str(NT)
            CALL vtutor%write(isError, DifferentLDAXCTypes, newArgument(cval = CTUT))
         ENDIF
      ENDDO
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
     ...
     &      > 0.1_q) THEN
         ...
         CALL vtutor%write(isAdvice, WrongOptimizationRealSpace, newArgument(rval = RTUT))
        ENDIF
      ENDIF
      ENDDO
...
!-----------------------------------------------------------------------
      IF ((XC%ID(1)/=XC_DATA_PP%ID(1)) .OR. &
        & (XC%NXC/=XC_DATA_PP%NXC) .OR. &
        & (XC%COEFF(1)/=XC_DATA_PP%COEFF(1))) THEN
         IF (XC%LDOGGA) THEN
!  Add GGA corrections to EATOM.
!  For some of the LDA potential (mostly USPP_LDA and a few USPP_GGA) this is possible.
!  Note that the GGAs PB, PW and LM, which are not in VASP anymore, would correspond
!  to LXC=4, 5 and 6, respectively.
            IF ((XC%ID(1)==ID_XC_PW91).AND.(XC%NXC==1).AND.(XC%COEFF(1)==1)) THEN
               LEXCH=7
               DO NT=1,NTYP
