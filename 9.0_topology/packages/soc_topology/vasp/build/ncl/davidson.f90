...
!< required for HF
    ...
    COMPLEX(q), ALLOCATABLE :: CORTHO(:,:) !< stores \f$ <\psi_i| S |\psi_k> \f$
...
!=======================================================================
    ...
    IF (NSIM_LOCAL*NCPU /= NSIM) CALL vtutor%bug("EDDAV: NSIM is not correct: " // str(NSIM),"davidson.F",172)
...
!$ACC ENTER DATA CREATE(WOPT,WHAM,WS) 
...
!$ACC ENTER DATA CREATE(CHAM,COVL,COVL_,CEIG,PRECON,R,CORTHO) 

    ALLOCATE(FNORM(NSIM_LOCAL),FPRE(NSIM_LOCAL))
!$ACC ENTER DATA CREATE(FNORM,FPRE) 

    LHFCALC_DAV=.FALSE.
    IF (PRESENT(LHF)) THEN
       IF (LHF .AND. LHFCALC .AND. AEXX/=0) THEN
          LHFCALC_DAV=.TRUE.
!$ACC ENTER DATA CREATE(WHF) 
          CALL NEWWAVA(WHF, WDES1, NSIM_LOCAL)

          IF (LFOCK_ACE()) THEN
!$ACC ENTER DATA CREATE(W1_ORIG(:)) 
             DO I=1,NSIM_LOCAL
...
!$ACC ENTER DATA CREATE(W_ORIG) 
!$ACC ENTER DATA CREATE(W_ORIG%CPTWFP,W_ORIG%CPROJ,W_ORIG%CELTOT,W_ORIG%FERTOT) 
!$ACC KERNELS PRESENT(W_ORIG,W) 
...
! allocate array for the subspace diagonalisation
    IF (LSUBROT) THEN
       ...
       ENDIF
!$ACC ENTER DATA CREATE(CHAM_ALL) 
    ENDIF
...
!=======================================================================
          IF (LHFCALC_DAV.AND.LFOCK_ACE()) THEN
             CALL FOCK_ACE_ACC_APPLY(W1(1:NSIM_LOCAL_),ISP,WHF%CPTWFP(:,1:NSIM_LOCAL_),CDCHF)
             IF (IT(1)==1) THEN
...
!$ACC UPDATE SELF(FNORM(NP)) 

             CALL W1_COPY( ELEMENT( WHAM, NPP), W1(NP))

! rearrange WHAM%CPTWFP
             IF (WDES%DO_REDIS) CALL REDIS_PW(WDES1, 1, WHAM%CPTWFP(1,NPP))
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(COVL) 
          DO M=1,NSIM_*(ITER-1)
             ...
             ENDDO
          ENDDO
!$ACC ENTER DATA COPYIN(EVALUE_INI_GLBL) 
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CHAM,COVL,EVALUE_INI_GLBL) 
! correct CHAM by subtraction of epsilon COVL
          DO M=1,NSIM_*ITER
             ...
             ENDDO
          ENDDO
!$ACC EXIT DATA DELETE(EVALUE_INI_GLBL) 
...
!$ACC UPDATE SELF(CHAM,COVL) IF(OFFLOAD_ON)
...
!$ACC KERNELS PRESENT(COVL_,COVL) 
...
# 661
             ...
             IF (IFAIL==0) THEN
!$ACC UPDATE SELF(COVL_) IF(OFFLOAD_ON) WAIT(ACC_ASYNC_Q)
# 669
...
! to have some headroom I set it to 1E-13 (1E-14 worked in my tests however)
             ...
             IF (IFAIL==0) THEN
!$ACC KERNELS PRESENT(CEIG,CHAM,COVL,COVL_) 
...
!$ACC UPDATE SELF(R) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)

          IF (.FALSE.) THEN
             IF (NODE_ME==IONODE) THEN
!$ACC WAIT IF(OFFLOAD_ON)
!$ACC UPDATE SELF(CEIG) IF(OFFLOAD_ON)
...
! if the change in the eigenenergy is very small
             ...
             ENDDO
          ENDDO
          CALL M_bcast_d(WDES%COMM_KIN, DEMAX, 1)  ! broadcast from root node
!-----------------------------------------------------------------------
...
! store in the corresponding (H - epsilon S)  in WOPT%CW_RED
                ...
                IF (WDES1%NPL_RED/=0) &
                     CALL ZGEMM('N', 'N',  WDES1%NPL_RED , NSIM_, NSIM_*ITER, (1._q,0._q), &
                     &               WHAM%CW_RED,  WDES%NRPLWV_RED, CEIG, NSUBD,  &
                     &               (0._q,0._q), WOPT%CW_RED,  WDES%NRPLWV_RED)
...
!$ACC KERNELS PRESENT(CHAM_ALL,R) 
...
!$ACC END KERNELS
          ...
          IF (.FALSE.) THEN
             IF (NODE_ME==IONODE) THEN
!$ACC WAIT IF(OFFLOAD_ON)
!$ACC UPDATE SELF(CORTHO) IF(OFFLOAD_ON)
...
! add lower triangle
!$ACC PARALLEL LOOP PRESENT(CHAM_ALL) GANG 
             DO N=1,NB_TOT
!$ACC LOOP VECTOR
                DO NP=N+1,NB_TOT
...
!$ACC KERNELS PRESENT(CHAM_ALL,COVL_ALL) 
          CHAM_ALL=COVL_ALL
!$ACC END KERNELS
!$ACC END DATA
          DEALLOCATE(COVL_ALL)
...
!$ACC EXIT DATA DELETE(FNORM,FPRE) 
    DEALLOCATE(FNORM,FPRE)

!$ACC EXIT DATA DELETE(SV,CDIJ,CQIJ,PRECON,R,CHAM,COVL,COVL_,CEIG,CORTHO) 
    DEALLOCATE(PRECON, CHAM, COVL, CEIG, COVL_, CORTHO, R, RWORK, CWRK)
!$ACC EXIT DATA DELETE(CHAM_ALL) ASYNC(ACC_ASYNC_Q) IF(OFFLOAD_ON.AND.LSUBROT)
...
!$ACC EXIT DATA DELETE(WOPT,WHAM,WS) 

    IF (LHFCALC_DAV) THEN
       CALL DELWAVA(WHF)
!$ACC EXIT DATA DELETE(WHF) 
       IF (LFOCK_ACE()) THEN
          DO I=1,NSIM_LOCAL
             CALL DELWAV(W1_ORIG(I), .FALSE.)
          ENDDO
!$ACC EXIT DATA DELETE(W1_ORIG(:)) 
       ELSE
!$ACC EXIT DATA DELETE(W_ORIG%CPTWFP,W_ORIG%CPROJ,W_ORIG%CELTOT,W_ORIG%FERTOT) 
!$ACC EXIT DATA DELETE(W_ORIG) 
