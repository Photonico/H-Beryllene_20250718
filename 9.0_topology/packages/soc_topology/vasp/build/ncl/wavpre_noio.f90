...
!>
!> In addition, the charge density is seprately predicted.
...
!>-@param POSION  must contain the new positions
!>-@param CSTRF   must contain the *old* structure-factor
!>         on return it holds the new structure-factor if IPRE<0
!>-@param CPTWFP  must contain the old wavefunctions
!>-@param IU      File Unit to use
!>-@param ICMPLX  size of COMPLEX(q) item
!>-@param IPRE    prediction of wavefunctions and eigenvalues performed
!>        -   0  nothing 1._q
...
!>-@param   W_1 ... previous-wavefunctions
!>-@param   W_2 ... previous-change in wf.
!>-@param   CHTOT_1 ... last charge
!>-@param   CHTOT_2 ... last change in charge
!>-@param   RHOLM_1 ... last paw onsite charge terms
!>-@param   RHOLM_2 ... last change in there terms
!>-@param   POS_1, POS_2, POS_3 ... last position and last changes in positions
!>-@param   ICALL_STORE ... Nr. of calls for each band
!>
...
!*********************************************************************
      ...
      SUBROUTINE WAVPRE_NOIO(GRIDC,P,XC,PRED,T_INFO,W,WDES,LATT_CUR,LOPEN, &
...
!! USE moffload
      ...
      COMPLEX(q) CQIJ(LMDIM,LMDIM,T_INFO%NIOND,WDES%NCDIJ)
      COMPLEX(q) CHTOT(GRIDC%MPLWV,WDES%NCDIJ)
      REAL(q)    RHOLM(NPAW,WDES%NCDIJ)
      COMPLEX(q) CSTRF(GRIDC%MPLWV,T_INFO%NTYP)

! these variables have to be saved
...
# 116
      ...
      REAL(q) RWORK(7*WDES%NB_TOT)
      INTEGER IWORK(5*WDES%NB_TOT), INFO(WDES%NB_TOT)
      PARAMETER (LWORK=32)
      COMPLEX(q), ALLOCATABLE :: CTMP(:)


! work arrays
      COMPLEX(q), ALLOCATABLE, TARGET :: CPROW(:,:),CPRO0(:),CPRO1(:)
      COMPLEX(q), ALLOCATABLE         :: CA(:,:),CAP(:,:),CU(:,:)
      COMPLEX(q), ALLOCATABLE   :: CD0(:),CD1(:)
...
# 135
      ...
      REAL(q), ALLOCATABLE :: WFMAG(:), WFMAGL(:)
      
! small real to check if charge has to be updated
      REAL(q) :: SMALL
...
!***********************************************************************
      ...
      IF (PRED%INIPRE==2 .OR.PRED%INIPRE>5 .OR.PRED%INIPRE<0) THEN
        CALL vtutor%bug("internal ERROR: WAVPRE_NOIO: INIPRE= " // str(PRED%INIPRE), "wavpre_noio.F", 199)
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      ALLOCATE(CD0(WDES%NRPLWV),CD1(WDES%NRPLWV), &
               CPRO0(WDES%NPROD),CPRO1(WDES%NPROD), &
...
!-----------------------------------------------------------------------
! after 3 calls fit new ionic positions to a linearcombination
...
!-----------------------------------------------------------------------
        IF ( ABS( 4*A2*B2-AB*AB ) .GT. SMALL*SMALL ) THEN
...
!$ACC KERNELS PRESENT(CTMP) 
      CTMP=0._q
!$ACC END KERNELS
      IF (NCOL(WDES%COMM_KIN%NODE_ME)>0) THEN
      ...
      CALL M_allgatherv_z(WDES%COMM_KIN, CTMP, CAP, NCTOT, OFFDATA)
...
!$ACC END KERNELS
 ...
 100  CONTINUE
      ...
      IF (IFAIL/=0) THEN
        CALL vtutor%error("ERROR WAVPRE_NOIO: Call to routine ZHEEV failed! " // str(IFAIL))
      ENDIF

! Store the matrix in A(N,K)=CONJG(EIGENV(K,N))
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CA,CAP) 
      DO N1=1,NB_TOT
      DO N2=1,NB_TOT
...
!$ACC ENTER DATA CREATE(CTMP) 
      ...
      IF (NCOL(WDES%COMM_KIN%NODE_ME)>0) THEN
         CALL ZGEMM( 'N','C', NB_TOT, NCOL(WDES%COMM_KIN%NODE_ME), NB_TOT, (1._q,0._q), CA, NB_TOT, &
                      CU(1+OFFSET(WDES%COMM_KIN%NODE_ME),1), NB_TOT, (0._q,0._q), CTMP, NB_TOT )
      ENDIF
...
!-----------------------------------------------------------------------
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CAP,R) PRIVATE(ESQR) 
      DO N1=1,NB_TOT
   ESQR=1/SQRT(R(N1))
      DO N2=1,NB_TOT
!!   ESQR=1/SQRT(R(N1))
...
!$ACC END KERNELS
      ...
      CALL M_allgatherv_z(WDES%COMM_KIN, CTMP, CU, NCTOT, OFFDATA)
...
!=======================================================================
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CAP,CU) 
      DO N2=1,NB_TOT
      DO N1=1,NB_TOT
...
!-----------------------------------------------------------------------
      ALLOCATE(WFMAG(WDES%NB_TOT),WFMAGL(WDES%NB_TOT))

      DO N=1,WDES%NB_TOT
         WFMAG (N)= ZDOTC(NPL,CW_RED(1,N),1,W_1%CPTWFP(1,N,NK,ISP),1)
         WFMAGL(N)= ZDOTC(NPL,CW_RED(1,N),1,CW_RED(1,N),1)
         IF (LOVERL) THEN
            WFMAG (N)=WFMAG (N)+ZDOTC(NPRO_O,CPROW_RED(1,N),1,W_1%CPROJ(1,N,NK,ISP),1)
            WFMAGL(N)=WFMAGL(N)+ZDOTC(NPRO_O,CPROJ_RED(1,N),1,CPROW_RED(1,N),1)
         ENDIF
      ENDDO
# 723

      CALL M_sum_d(WDES%COMM_KIN, WFMAG, WDES%NB_TOT)
      CALL M_sum_d(WDES%COMM_KIN, WFMAGL, WDES%NB_TOT)

!$ACC PARALLEL LOOP GANG PRESENT(WDES,W_1,W_2,ICALL_STORE,WFMAG,WFMAGL,CW_RED,CPROJ_RED,W) &
!$ACC PRIVATE(ICALL,CELEL,CDCEL1,CDCEL0,CD0,CPRO0,CD1,CPRO1) 
...
!-----------------------------------------------------------------------
      ...
      ! N,ICALL,WFMAG(N),WFMAGL(N)

      IF (ABS(WFMAGL(N)-1._q) > 1E-3 .AND. ICALL >=1 ) THEN
!$ACC ATOMIC UPDATE
        IWARN=IWARN+1
      ENDIF
      WFMAG(N)=WFMAG(N)/WFMAGL(N)

      CDCEL0       =0
      CD0(1:NPL)   =0
      CPRO0(1:NPRO)=0

      IF (ICALL/=0 .AND.ABS(WFMAG(N))>0.90_q) THEN
        ICALL=ICALL+1
!$ACC LOOP VECTOR
        DO M=1,NPL
          CD0(M)  =CW_RED(M,N)-W_1%CPTWFP(M,N,NK,ISP)
        ENDDO
!$ACC LOOP VECTOR
        DO M=1,NPRO
          CPRO0(M)=CPROJ_RED(M,N)-W_1%CPROJ(M,N,NK,ISP)
        ENDDO
      ...
      ELSE
        ICALL=1
   IF (IU0>=0) WRITE(IU0,'(A,I4,F8.4)')'Information: wavefunction orthogonal band ',N,WFMAG(N)
      ENDIF
...
!$ACC LOOP VECTOR
      DO M=1,NPL
         W_1%CPTWFP(M,N,NK,ISP)     = CW_RED(M,N)
         W_2%CPTWFP(M,N,NK,ISP)     = CD0(M)
      ENDDO
!$ACC LOOP VECTOR
      DO M=1,NPRO
         W_1%CPROJ(M,N,NK,ISP) = CPROJ_RED(M,N)
         W_2%CPROJ(M,N,NK,ISP) = CPRO0(M)
      ENDDO
# 794
...
!-----------------------------------------------------------------------
      ENDDO band
!$ACC EXIT DATA DELETE(WFMAG,WFMAGL) 
      DEALLOCATE(WFMAG,WFMAGL)
!-----------------------------------------------------------------------
      IF (DO_REDIS) THEN
        CALL REDIS_PROJ(WDES1, NBANDS, W%CPROJ(1,1,NK,ISP))
        CALL REDIS_PW  (WDES1, NBANDS, W%CPTWFP   (1,1,NK,ISP))
...
!***********************************************************************
      IF (PRED%INIPRE==4) THEN
         ICALLS=MIN(1,ICALLS)
      ENDIF
!$ACC EXIT DATA DELETE(CD0,CPRO0,CPROW,CA,CAP,CU,R) 
      DEALLOCATE(CD0,CD1,CPRO0,CPRO1,CPROW,CA,CAP,CU)
      ALLOCATE(CD0(GRIDC%MPLWV))
!$ACC ENTER DATA CREATE(CD0) COPYIN(CHTOT) 

!---- subtract atomic charge-density for old positions
      IF ( new_version ) THEN
         CALL RHOATO_WORK(.FALSE.,.FALSE.,GRIDC,T_INFO,LATT_CUR%B,P,CSTRF,CD0)
!$ACC PARALLEL LOOP PRESENT(GRIDC,CHTOT,CD0) 
         DO N=1,GRIDC%RC%NP
      ...
      IF (symplectic) THEN
         DO ISP=1,WDES%NCDIJ
!$ACC PARALLEL LOOP PRESENT(GRIDC,CHTOT_2,CHTOT_1,CHTOT) PRIVATE(CH1,CH0) 
            DO N=1,GRIDC%RC%NP
! tilde rho(n-2)
               CH1=CHTOT_2(N,ISP)
! tilde rho(n-1)
...
! shift slot by (1._q,0._q) and store predicted charge
            ...
            IF (XC%LTAU) THEN
!$ACC PARALLEL LOOP PRESENT(GRIDC,TAU_2,TAU_1,KINEDEN) PRIVATE(CH1,CH0) 
               DO N=1,GRIDC%RC%NP
! tilde rho(n-2)
                  CH1=TAU_2(N,ISP)
! tilde rho(n-1)
...
!$ACC EXIT DATA DELETE(CD0) COPYOUT(CHTOT) 
      DEALLOCATE(CD0)
