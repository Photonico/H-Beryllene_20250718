...
!$OMP THREADPRIVATE(CORE_CONF_NUM_STATES,CORE_CONF_N_OF_STATES,CORE_CONF_L_OF_STATES,CORE_CONF_OCC_OF_STATES)
    CONTAINS
...
! core level shifts
      ICORELEVEL=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'ICORELEVEL', ICORELEVEL, IERR, WRITEXMLINCAR)
! index of ion
!     NT_CL=1
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'CLNT', NT_CL, IERR, WRITEXMLINCAR)
      NT_CL=MIN(MAX(NT_CL,1),NTYP)

! main quantum number (n)
      N_CL=1
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'CLN', N_CL, IERR, WRITEXMLINCAR)
      N_CL=MIN(MAX(N_CL,0),7)

! l quantum number
      L_CL=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'CLL', L_CL, IERR, WRITEXMLINCAR)
      L_CL=MAX(MIN(L_CL,N_CL-1),0)

      Z_CL=0.0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'CLZ', Z_CL, IERR, WRITEXMLINCAR)
...
! state (V_atom)
      ...
      REAL(q) ZVAL           ! number of valence electrons
      REAL(q) W(:,:)         ! on return: AE orbital charge densities |phi(r,(n,l))|^2  (= A_*A_ + B_*B_)
...
! large and small components of the AE orbitals
...
! initialized by a previous call of CL_INIT_CORE_CONF
      IF (CORE_CONF_NUM_STATES==-1) THEN
         CALL vtutor%error("SET_CORE_WF: ERROR: core configuration was not initialized")
      ENDIF
...
!      WRITE(94,"(2F14.7)") (R%R(N1), APOT(N1)*R%R(N1)/RYTOEV/AUTOA,N1=1,R%NMAX)
      ...
      IF (ZCORE_DONE /= ZCORE) THEN
         CALL vtutor%bug("internal error in SET_CORE_WF: core electrons incorrect " // str(ZCORE) &
            // " " // str(ZCORE_DONE), "cl_shift.F", 431)
      ENDIF
...
!***********************************************************************
      ...
      SUBROUTINE CORE_WAVE_FKT(RHO, EIG, N, L, APOT, R ,Z, A_, B_, KAPPA, DPOT, TOL, KI_OUT, KJ_OUT)
      ...
      REAL(q), OPTIONAL :: TOL             ! Tolerance for outwards integration
      INTEGER, OPTIONAL :: KI_OUT
      INTEGER, OPTIONAL :: KJ_OUT
...
! local variables
      ...
      MY_TOL = 1E-6; IF (PRESENT(TOL)) MY_TOL = TOL
...
! Check consistency of input
      ...
      IF (PRESENT(KAPPA)) THEN
         IF ((KAPPA/=L).AND.(KAPPA/=-L-1)) THEN
            CALL vtutor%error("CORE_WAVE_FKT: relativistic quantum number does not match angular moment")
         ENDIF
         IF (KAPPA==L) J=L-0.5
 ...
 tries:  DO NTIMES=1,NTRIES
         ...
         IF (PRESENT(KAPPA).AND.PRESENT(DPOT)) THEN
            CALL OUTINT(E,L,R,APOT(1:R%NMAX),KI,NODES,A,B,NERR,KAPPA,DPOT(1:R%NMAX),TOL=MY_TOL)
         ELSEIF (PRESENT(KAPPA)) THEN
            CALL OUTINT(E,L,R,APOT(1:R%NMAX),KI,NODES,A,B,NERR,KAPPA,TOL=MY_TOL)
         ELSE
            CALL OUTINT(E,L,R,APOT(1:R%NMAX),KI,NODES,A,B,NERR,TOL=MY_TOL)
         ENDIF      
...
! too few nodes
            ...
            IF ((EMAX-EMIN)<(E*1E-10)) THEN
               CALL vtutor%error("TOO FEW NODES: " // str(NODES) // " " // str(N) // " " // str(L) // &
                  " " // str(J) // " " // str(EMIN) // " " // str(E) // " " // str(EMAX))
            ENDIF      
...
!              WRITE(*,'("n=",I4," l=",I4," j=",F5.1," energy interval:",4F16.8)') N,L,J,EMIN,E,EMAX,DE
               IF (PRESENT(KI_OUT)) KI_OUT = KI
               IF (PRESENT(KJ_OUT)) KJ_OUT = KJ
               RETURN
            ENDIF
         ENDIF
...
!************************************************************************
!
!>@brief  Performs outward integration. The Runge-Kutta procedure
!>        is used to provide the starting values for the MILNE
!>        integration.
!>        The 5th order predictor-corrector formula for the MILNE
!>        procedure can be found in Abramowitz and Stegun
!
!************************************************************************

      SUBROUTINE OUTINT(E,L,R,APOT,KI,NODES,A,B,NERR,KAPPA,DPOT,RC,IH,TOL)
      ...
      REAL(q), OPTIONAL :: TOL                    ! Tolerance for Milne procedure

      INTEGER :: KI,IK,K,KP1
      INTEGER :: KM1,KM2,KM3,KM4,KM5,KIT

      REAL(q),PARAMETER :: DEFAULT_TOL = 1e-6     ! Default tolerance for Milne procedure
      REAL(q) :: DX2
      REAL(q) :: RA
...
! convert to atomic units
      ...
      XK=L*(L+1)
      XMFT=2._q/45._q*R%H
      TEST=1._q/DEFAULT_TOL; IF (PRESENT(TOL)) TEST = 1._q/TOL
...
!************************************************************************
      ...
      SUBROUTINE INWINT(E,L,R,APOT,KI,KJ,NODES,A,B,KAPPA,DPOT,EPS)
      ...
      REAL(q) :: APOT(R%NMAX)            ! V(r)
      REAL(q), OPTIONAL :: DPOT(R%NMAX)  ! dV(r)/dr
      REAL(q), OPTIONAL :: EPS           ! VALUE USED TO DETERMINE THE PRACTICAL INFINITY
      ...
      MY_EPS=75. ; IF (PRESENT(EPS)) MY_EPS = EPS
      H3=-R%H/3.
      DR=1./R%D
...
! convert to atomic units
      ...
      DO KJ=KI+11,R%NMAX
        IF(RA*(VR(KJ)-EAU*RA) > MY_EPS) EXIT
        RA=RA*R%D
      ENDDO
...
!TODO: understand why PGI compiler sets DELQL = 0 for ICORELEVEL = 2
      DELQL= P%PSGMAX/NQL
      DO I=2,NQL
