...
!  maximum number of cache lines
  ...
  ENUM, BIND(C)
     ENUMERATOR :: ID_INDEPENDENT_PARTICLE, ID_RESONANT, ID_POLARIZABILITY, &
        ID_RPA, ID_DFT, ID_CHARGE_ELECTRON, ID_CHARGE_CHARGE, ID_MICROSCOPIC, &
        ID_MICROSCOPIC_DFT, ID_SCREENED_COULOMB, ID_XI_LOCAL_FIELD, ID_XI_GW, &
        ID_GG, ID_GG_ANTIRESONANT, ID_FXC, ID_FXC_ANTIRESONANT
  END ENUM
...
!
!> @details @ref openmp :
!> Under OpenMP the loop over NB2 (unoccupied orbitals) in the
!> determination of the overlap charge densities GCHG is distributed
!> over the available OpenMP threads (OMP PARALLEL DO).
!>
!> In addition, chi_base::add_responsefunction_cache is called from
!> within a parallel region (for -Dnew_version).
!
!**********************************************************************
...
!weights need to be allocated for Friedrich's version of CRPA
    IF ( LWEIGHTED .AND. ( .NOT.ALLOCATED(CRPA_WEIGHTS) ) ) THEN
       CALL vtutor%bug("internal error in VASP: ADD_XI weighting without weights does not work", "chi_base.F", 239)
    ENDIF 
    IF ( LDISENTANGLED .AND. ( .NOT.ALLOCATED(UCRPA) ) ) THEN
       CALL vtutor%bug("internal error in VASP: ADD_XI disentanglement without UCRPA will not work", "chi_base.F", 242)
    ENDIF 
! set fermi energy
    EFERMI = W%EFERMI(ISP)
...
! determined the index of k2=k1-q
    ...
    IF (W%WDES%WTKPT(NK1)==0 .OR. W%WDES%WTKPT(NK2)==0) THEN
       IF (W%WDES%WTKPT(NK1)/=0 .OR. W%WDES%WTKPT(NK2)/=0) THEN
          CALL vtutor%bug("internal error in CALCULATE_XI: one k-point weight is zero", "chi_base.F", 259)
       ENDIF
    ...
    IF (WDESQ%LGAMMA .NEQV. CHI%LREAL) THEN
       CALL vtutor%bug("chi_base.F: internal error WDESQ%LGAMMA .NEQV. CHI%LREAL", "chi_base.F", 266)
    ENDIF
...
!==========================================================================
    ...
    NP=WDESQ%NGVECTOR
! at T>0 those states are states contributing to positive imaginary time
    mband: DO N2=FIRST_POS_TIME_LOCAL_ORBITAL( WHF, NK2, ISP, EFERMI, CHI%BETA), LAST_EMPTY_XI_LOCAL( WHF, NK2, ISP), NSTRIP2
...
!$OMP PARALLEL PRIVATE(W2)
!$     CALL NEWWAV(W2,WDESK2,.TRUE.)
!$OMP DO PRIVATE(N2_IND,NB2_INTO_TOT,NB1_INTO_TOT,N,CWORK,GWORK,CRHOLM)
...
! skip dummy states introduced due to parallelization
          IF (WHF%AUX(NB2, NK2, ISP)==0 ) CYCLE
! skip non-positive time states
          IF ( LFINITE_TEMPERATURE ) THEN
            IF( .NOT. POSITIVE_TIME_ORBITAL( WHF%FERWE( NB2, NK2, ISP),&
                REAL(WHF%CELEN(NB2,NK2,ISP)-EFERMI,q)*CHI%BETA )) CYCLE
! skip occupied states at T=0
          ELSE 
            IF( FILLED_XI_ORBITAL( WHF%FERWE( NB2, NK2, ISP) ) ) CYCLE 
          ENDIF
...
! should contain only negative time contributions
          nband: DO N=1,NGLB
...
! skip non-negative time states at T>0
             IF ( LFINITE_TEMPERATURE ) THEN
                IF( .NOT. NEGATIVE_TIME_ORBITAL( WHF%FERTOT( NB1_INTO_TOT, NK1, ISP),&
                    REAL(WHF%CELTOT(NB1_INTO_TOT,NK1,ISP)-EFERMI,q)*CHI%BETA )) CYCLE
...
! skip dummy states introduced due to parallelization
                IF (WHF%AUX(NB2, NK2, ISP)==0 ) CYCLE
! skip non-positive time states for T>0
                IF ( LFINITE_TEMPERATURE ) THEN
                  IF( .NOT. POSITIVE_TIME_ORBITAL( WHF%FERWE( NB2, NK2, ISP),&
                      REAL(WHF%CELEN(NB2,NK2,ISP)-EFERMI,q)*CHI%BETA )) CYCLE
! skip occupied states at T=0
...
! wrong order skip
                WEIGHT=0
! transition energy
                DCELTOT = REAL(WHF%CELTOT(NB2_INTO_TOT, NK2, ISP)-WHF%CELTOT(NB1_INTO_TOT,NK1,ISP),q)
! finite temperature MBPT takes all transitions into account,
! also the negative ones.
                IF( LFINITE_TEMPERATURE ) THEN
!TEST
                   IF ( LLTDMP2 ) THEN
                      CALL SET_GG_TAU_WEIGHT( WEIGHT, &
                          REAL(WHF%CELEN(NB2, NK2, ISP)-EFERMI,q),&
                          REAL(WHF%FERWE(NB2, NK2, ISP),q),&
                          REAL(WHF%CELTOT(NB1_INTO_TOT, NK1, ISP)-EFERMI,q),&
                          REAL(WHF%FERTOT(NB1_INTO_TOT, NK1, ISP),q),&
                          CHI%BETA, CHI%COMEGA(NOMEGA) )
! evaluation on complex frequency axes
                   ELSE
                      CALL SET_GG_OMEGA_WEIGHT( WEIGHT, &
                          ...
                          REAL(WHF%CELTOT(NB1_INTO_TOT, NK1, ISP)-EFERMI,q),&
                          REAL(WHF%FERTOT(NB1_INTO_TOT, NK1, ISP),q),&
                          CHI%BETA, CHI%COMEGA(NOMEGA),CHI%OMEGA0 )
                   ENDIF
                ELSE
! vanishing transition energies have a contribution for omega->0
                  IF ( DCELTOT >= DEGENERACY_THRESHOLD ) THEN
                     IF ( LLTDMP2 ) THEN
                        CALL SET_XI_TAU_WEIGHT( WEIGHT, DCELTOT, CHI%COMEGA(NOMEGA) )
! evaluation on complex frequency axes
                     ELSE
                        CALL SET_XI_WEIGHT( WEIGHT, REAL(WHF%CELTOT(NB1_INTO_TOT,NK1,ISP)-WHF%CELEN(NB2, NK2, ISP),q),&
                           CHI%SHIFT, CHI%COMEGA(NOMEGA), ISMEAR, SIGMA)
                     ENDIF
                  ENDIF
                ENDIF
!mK: The correct weight should be f_n*(1-f_m) where n and m runs over ALL states
!    For systems with a band gap this is the same as f_i-f_a where f_i is occupied and f_a unoccupied
!    For metals, however, this factor would kill the term omega->0, which is propotional to beta
!    and diverges for T=0 in the MP2 approximation (as it should)
!    This term is only present for terms with (0._q,0._q) transition energy
                IF ( LFINITE_TEMPERATURE ) THEN
...
!WEIGHT=WHF%WDES%RSPIN*WHF%WDES%WTKPT(NK1)*WHF%FERTOT(NB1_INTO_TOT,NK1,ISP)*(1-WHF%FERWE(NB2, NK2, ISP))*WEIGHT
                ENDIF 
! the remaining multiplication factor is the same in both cases
...
!removes disentangled states completely
                IF ( ALLOCATED( LCRPA_STATE)) THEN
                  IF ( LCRPA_STATE(ISP,NK1,NB1_INTO_TOT) .AND.  LCRPA_STATE(ISP,NK2,NB2_INTO_TOT) ) &
                     WEIGHT = 0 
                ENDIF 
...
!compute correlated head and wings if necessary
                IF (ALLOCATED(UCRPA).AND.((LCHIC.AND.LSUBTRACT_CHIC).OR.ALLOCATED(LCRPA_STATE)) )THEN !rotate correlated long-wave limit
...
!-----------------------------------------------------------------------------
!
! helper, set Matsubara frequency weight for polarizability
!
!-----------------------------------------------------------------------------

  SUBROUTINE SET_XI_WEIGHT_BETA(WEIGHT, DECEL, COMEGA, BETA, OMEGA0 )
    USE constant
    COMPLEX(q) :: WEIGHT
    REAL(q)    :: DECEL, SHIFT, BETA 
    COMPLEX(q) :: COMEGA
    INTEGER    :: ISMEAR
    REAL(q)    :: SIGMA
    REAL(q)    :: OMEGA0

! note that DCEL has distinct sign passed than in SET_XI_WEIGHT
! is typically positive
!
! if COMEGA = I * w_n , where w_n = 2*pi *n / beta, then the correct weight
! for T>0 is given by
!
! weight = 2 * D/(D^2 + w_n^2) * ( 1-Exp(-D*beta) ) +
!          Exp(-D*beta) w_n Sin( 2*pi*n ) / D^2 + w_n^2)
!
! note that the second term is non-(0._q,0._q) only if D=0, becaus then:
!
! weight - > delta(n)*beta
!
! so for metals the weight is delta-function like and propotional to beta
! it diverges for T->0, as it should!
!
! for omega->0 the weight has and additional factor if DECEL = 0
! the finite temperature frequency grid resembles the Matsubara points
! close to 0 and is stored in such a way that the lowest point
! is very close to (0._q,0._q),
! in parallel execution COMEGA can be different on distinct 1 ranks
! but OMEGA0 is the first point (almost 0) on all ranks
    IF ( ABS(DECEL) <= XI_EMPTY_THRESHHOLD .AND. ABS( ABS( COMEGA )- OMEGA0 ) < XI_EMPTY_THRESHHOLD) THEN
! (0._q,0._q) frequency contribution is non-(0._q,0._q) only for (0._q,0._q) transition energy :)
       WEIGHT =-BETA
    ELSE
       WEIGHT=-2*DECEL/(ABS(COMEGA)**2+DECEL**2)*(1-EXP(-DECEL*BETA) )
    ENDIF
  END SUBROUTINE SET_XI_WEIGHT_BETA

!-----------------------------------------------------------------------------
!
! used for dMP2, that is chi in imaginary time  domain
! (at least if broadening is selected in the INCAR file)
!
!-----------------------------------------------------------------------------
  SUBROUTINE SET_XI_TAU_WEIGHT(WEIGHT, DELTOT, TAU )
    COMPLEX(q) :: WEIGHT
    REAL(q)    :: DELTOT
    COMPLEX(q) :: TAU

!    IF ( ABS(DELTOT) > XI_EMPTY_THRESHHOLD ) THEN
       WEIGHT=-EXP( -DELTOT*ABS(TAU) )
! transition energies smaller than XI_EMPTY_THRESHHOLD are considered to be (0._q,0._q)
! this is a precaution to be consistent when evaluating chi on the imaginary axis
! where terms with (0._q,0._q) transition energy have a contribution at w->0
!    ELSE
!       WEIGHT=-1
!    ENDIF
  END SUBROUTINE SET_XI_TAU_WEIGHT
...
!
!-----------------------------------------------------------------------------
  SUBROUTINE SET_GG_TAU_WEIGHT(WEIGHT, EU, FU, EO, FO, BETA, TAU )
    USE subrot_cluster, ONLY : DEGENERACY_THRESHOLD
    COMPLEX(q) :: WEIGHT
    REAL(q)    :: EU,FU, EO,FO, BETA
    COMPLEX(q) :: TAU
    

! nummerically save way to evaluate the time weight of two greens functions:
! GUNO ... Green's function for positive times, mostly unoccpuied states
! GOCC ... Green's function for negative times, mostly occupied states
    IF ( LDMP1 ) THEN
        WEIGHT = -GOCC_WEIGHT_TIME( EU, FU, BETA, 0._q)*&
           GOCC_WEIGHT_TIME( EO, FO, BETA,0._q)
    ELSE
        WEIGHT = -GUNO_WEIGHT_TIME( EU, FU, BETA, AIMAG(TAU) )*&
           GOCC_WEIGHT_TIME( EO, FO, BETA,-AIMAG(TAU))
    ENDIF
    
  END SUBROUTINE SET_GG_TAU_WEIGHT
...
! unoccupied or at fermi level
  ...
  SUBROUTINE SET_GG_OMEGA_WEIGHT(WEIGHT, EU,FU, EO,FO, BETA, OMEGA, OMEGA0 )
    COMPLEX(q) :: WEIGHT
...
!
! the first and second term are (0._q,0._q) if w /= 0 and D_uo = 0 ,
! but non-(0._q,0._q) if w = 0 and D_uo =0
...
! for (0._q,0._q) transition, there is a contribution only for w->0 from both terms
! which for finite temperature is the first frequency point OMEGA0
       IF ( ABS(AIMAG(OMEGA))-OMEGA0 < 1.E-6_q .OR. AIMAG(OMEGA)==0) THEN
...
! determine time weight and integrate to obtain omega = 0 value
       ...
       ENDIF
    ELSE 
! for non-(0._q,0._q) transitions, (1._q,0._q) has following term
! D_mn/(D_mn^2+w^2)(f_n-f_m)
       WEIGHT = (D/(D**2+AIMAG(OMEGA)**2))*(FO-FU) 
    ENDIF
! factor 2 missing for non self-exitations
  ...
  END SUBROUTINE SET_GG_OMEGA_WEIGHT
...
!
!> @details @ref openmp :
!> Under OpenMP the loop over NB2 (unoccupied orbitals) is distributed
!> over the available OpenMP threads.
!> In doing so, chi_base::add_responsefunction_cache is called from
!> within a parallel region
!>
!> Accesses of CHI\%HEAD (responsefunction::head) by the different
!> threads is regulated by means of the OpenMP lock CHI\%OMP_LCK2
!> (responsefunction::omp_lck2) to prevent data races.
!
!**********************************************************************
...
! determined the index of k2= k1+q
    ...
    IF (W%WDES%WTKPT(NK1)==0 .OR. W%WDES%WTKPT(NK2)==0) THEN
       IF (W%WDES%WTKPT(NK1)/=0 .OR. W%WDES%WTKPT(NK2)/=0) THEN
          CALL vtutor%bug("internal error in ADD_XI_SPECTRAL: one k-point weight is zero", "chi_base.F", 1110)
       ENDIF
    ...
    IF (WDESQ%LGAMMA .NEQV. CHI%LREAL) THEN
       CALL vtutor%bug("chi_base.F: internal error WDESQ%LGAMMA .NEQV. CHI%LREAL", "chi_base.F", 1117)
    ENDIF
...
!$OMP PARALLEL PRIVATE(W2)
    CALL NEWWAV(W2,WDESK2,.TRUE.)
!$OMP DO PRIVATE(NB2_INTO_TOT,N,NB1_INTO_TOT,WEIGHT,MOMEGA1,MOMEGA2,NOMEGA1,NOMEGA2,WEIGHT1,WEIGHT2,GCHG,CRHO,CRHOLM,GWORK,CDER_BETWEEN_STATE,I,J) &
!$OMP REDUCTION(+:NFLOAT,NOPER)
...
! CDER_BETWEEN_STATE =  <u_k1,n1|- i d/dq_j | u_k1+q,n2>
          IF (ABS(SUM(CHI%VKPT*CHI%VKPT))<G2ZERO) THEN
             ...
             IF (NOMEGA1>0) THEN
!!$OMP CRITICAL
!$              CALL OMP_SET_LOCK(CHI%OMP_LCK2(NOMEGA1))
                DO I=1,3
                   DO J=1,3
...
! complex array with NP entries
                   ...
                   ENDDO
                ENDIF
!$              CALL OMP_UNSET_LOCK(CHI%OMP_LCK2(NOMEGA1))
!!$OMP END CRITICAL
             ENDIF
             IF (NOMEGA2>0) THEN
!!$OMP CRITICAL
!$              CALL OMP_SET_LOCK(CHI%OMP_LCK2(NOMEGA2))
                DO I=1,3
                   DO J=1,3
                ...
                ELSE
                   ...
                   ENDDO
                ENDIF
!$              CALL OMP_UNSET_LOCK(CHI%OMP_LCK2(NOMEGA2))
!!$OMP END CRITICAL
             ENDIF
          ENDIF
       ENDDO
...
! determined the index of k2= k1-q
    ...
    IF (W%WDES%WTKPT(NK1)==0 .OR. W%WDES%WTKPT(NK2)==0) THEN
       IF (W%WDES%WTKPT(NK1)/=0 .OR. W%WDES%WTKPT(NK2)/=0) THEN
          CALL vtutor%bug("internal error in SCREENED_TWO_ELECTRON_INTEGRAL: one k-point weight is zero", "chi_base.F", 1436)
       ENDIF
       RETURN
    ENDIF
    CALL SETWDES(WHF%WDES,WDESK2,NK2)
    ...
    IF (WDESQ%LGAMMA .NEQV. CHI%LREAL) THEN
       CALL vtutor%bug("chi_base.F: internal error WDESQ%LGAMMA .NEQV. CHI%LREAL", "chi_base.F", 1443)
    ENDIF
...
!
!> @details @ref openmp :
!> Under OpenMP the loop over NB2 (all bands owned by a particular
!> 1-rank) is distributed over the available OpenMP threads.
!> In doing so, chi_base::add_responsefunction_int is called from
!> within a parallel region
!>
!***********************************************************************
...
! use temporarily another WDES
    ...
    IF (ASSOCIATED(WCACHE) .AND. ISIGN==0 .AND. CHI%NOMEGA>1) THEN
       CALL vtutor%bug("SCREENED_TWO_ELECTRON_CACHED: internal error more than two frequencies and " &
          // "WCACHE associated", "chi_base.F", 1707)
    ENDIF
    ...
    IF (WDESQ%LGAMMA .NEQV. CHI%LREAL) THEN
       CALL vtutor%bug("SCREENED_TWO_ELECTRON_CACHED: internal error WDESQ%LGAMMA .NEQV. CHI%LREAL", "chi_base.F", 1716)
    ENDIF
...
! determined the index of k2= k1-q
    ...
    IF (W%WDES%WTKPT(NK1)==0 .OR. W%WDES%WTKPT(NK2)==0) THEN
       IF (W%WDES%WTKPT(NK1)/=0 .OR. W%WDES%WTKPT(NK2)/=0) THEN
          CALL vtutor%bug("internal error in SCREENED_TWO_ELECTRON_INTEGRAL_CACHED: one k-point " &
             // "weight is zero", "chi_base.F", 1724)
       ENDIF
...
!$OMP PARALLEL PRIVATE(W2)
    CALL NEWWAV(W2, WDESK2, .TRUE.)
!$OMP DO PRIVATE(N,NB1_INTO_TOT,EDIFF,MOMEGA1,MOMEGA2,NOMEGA,NOMEGA1,NOMEGA2,W_INTER,LCYCLE,IERR,GCHG,CRHO,CRHOLM,GWORK) &
!$OMP REDUCTION(+:NFLOAT,NOPER)
...
!                IF ((EDIFF>=-1E-10_q .AND. ISIGN>0).OR. (EDIFF<-1E-10_q  .AND. ISIGN<0)) THEN
                IF ((EDIFF>=0 .AND. ISIGN>0).OR. (EDIFF<0 .AND. ISIGN<0)) THEN
                   ...
                   IF (IERR==1) THEN
                      CALL vtutor%error("SCREENED_TWO_ELECTRON_CACHED involved bands " // &
                         str(REAL(WHF%CELTOT(NB1_INTO_TOT,NK1,ISP),q)) // " " // &
                         str(REAL(WHF%CELEN(NB2,NK2,ISP),q)) // " " // str(NB1_INTO_TOT) // " " // &
                         str(NK1) // " " // str(NB2) // " " // str(NK2) // " " // str(ISP))
                   ENDIF
                ENDIF
             ENDIF
...
!                IF ((EDIFF<-1E-10_q .AND. ISIGN>0) .OR. (EDIFF>=-1E-10_q .AND. ISIGN<0)) THEN
                IF ((EDIFF<0 .AND. ISIGN>0) .OR. (EDIFF>=0 .AND. ISIGN<0)) THEN
                   IF (.NOT. LCYCLE) THEN
                      CALL vtutor%bug("internal error in SCREENED_TWO_ELECTRON_CACHED: double use " &
                         // "of entries", "chi_base.F", 1852)
                   ENDIF
                   LCYCLE=.FALSE.
          ...
          IF (ISIGN==0) THEN
             DO NOMEGA=1,CHI%NOMEGA
                ...
                CALL ADD_RESPONSEFUNCTION_INT(WDESQ, CHI, GWORK(1:NP), NOMEGA, NP, & 
                     ...
                     NQ, NK1, NB1_INTO_TOT, NB2, ISP, NOMEGA, NK2 , W_INTER(MIN(2,NOMEGA),:))
                IF (CHI%LREAL) THEN
                   NFLOAT=NFLOAT+16._q*NP*NP
                   NOPER=NOPER+1
                ELSE
...
! by selecting ALGO =EXX (setting LEXX to .TRUE.)
! note that for coding reasons NOMEGA must be 2 in this case
!
!
! on entry: CVTOT contains the local potential: V_ext + V_h + V_xc^old
!           CHTOT is the total charge on fine grid
!           also XC-Stack variables are set to compute exchange only
!
!***********************************************************************

  SUBROUTINE OEP_GW(HAMILTONIAN, KINEDEN, NBANDSGW_, WDES, LATT_CUR, NONLR_S, NONL_S, W, WACC1, WACC2, LEXX, &
      LMDIM, CDIJ, CQIJ, SV, T_INFO, P, STEP, LGW0, IU0, IU6, &
      ISMEAR, SIGMA, SYMM, LCOHSEX, &
      INFO, XC, WDESQ, CHI0, GRID, GRIDC, GRID_SOFT, GRIDUS, C_TO_US, SOFT_TO_C, &
      CHDEN, CHTOT, DENCOR, CVTOT, CSTRF, IRDMAX, CRHODE, N_MIX_PAW, AMIX, RHOLM )
...
!  SV_HARTREE_ION and CDIJ_HARTREE_IONS
!=======================================================================
!  calculate local potential
    CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT, WDES%COMM_INTER, WDES,  &
         INFO,XC,P,T_INFO,E,LATT_CUR, &
         CHDEN,CHTOT,CSTRF,CVTOT1,DENCOR,SV_HARTREE_ION,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
! SV_HARTREE_ION = V_ext + V_h   (on soft grid)
! CVTOT1         = V_ext + V_h   (on hard grid)
    
! add related (1._q,0._q)-center augmentation terms  CDIJ_HARTREE_ION
...
! (this will be used to calculate the full Hamiltonian including the selfenergy)
    LHFCALC_FORCE=.TRUE.
! add PAW contributions to CDIJ_HARTREE_ION
...
!  CVTOT1, CDIJ1 and SV1
!=======================================================================
! determine local potential (excludes OEP potential)
    CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
         INFO,XC,P,T_INFO,E,LATT_CUR, &
         CHDEN,CHTOT,CSTRF,CVTOT1,DENCOR,SV1,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
! CVTOT1 = V_ext + V_h   (on hard grid)
! SV1    = V_ext + V_h   (on soft grid)
! sets plane-wave part ~V_eff in Eq. (47) of Kresse&Joubert
    
! determine CDIJ1 from CVTOT1
    CDIJ1=0
! SETDIJ sets ^D_ij in Eq. (47) of Kresse & Joubert
! i.e. plane-wave contribution
    CALL SETDIJ(WDES,GRIDC,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO,INFO%LOVERL, &
         LMDIM,CDIJ1,CQIJ,CVTOT1,IRDMAA,IRDMAX)
    
! enforce calculation of HF (1._q,0._q) center contributions -> CDIJ1
    LHFCALC_FORCE=.TRUE.
! this sets D^1_ij - ~D^-1_ij in Eq. (47) of Kresse & Joubert
! i.e. augmentation correction
    CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, &
...
! and ionic and Hartree contribution to local potential stored in CVTOT1, CDIJ1, and SV1
       ...
       CVTOT1=CVTOT1-CVTOT
       SV1   =SV1-SV
! CVTOT1 = [ V_ext + V_h ] - [ V_ext + V_h + V_xc(KS) ]^old
!        = V_x(KS) - V_xc(KS)^old  ! gK: this seems to be wrong
!
! SV_HARTREE_ION = V_ext + V_h  (on soft grid)
    ELSE
...
! (1._q,0._q) could also use the full original Hamiltonian
    ENDIF
!
! SV1 and SV_HARTREE_ION is used below to set up  CHAM  and CTMP
!
! CHAM is used to set up OEP charge, CTMP for qp-energies
!
!=======================================================================
...
sp1: DO ISP=1,WDES%ISPIN
    ...
    IF (ABS(CHI0%COMEGA(1)-0)/=0) THEN
       CALL vtutor%bug("internal error in OEP_GW: CHI0 stores the wrong frequency " // str(CHI0%COMEGA), "chi_base.F", 2309)
    ENDIF
...
! WHAM <- V_x
! WHAM_HARTREE_ION <- V_x
!#define onlyHF

             CALL HAMILT_LOCAL(W1, SV1, ISP, WHAM%CPTWFP(:,NP),.TRUE.,LKINETIC)
! WHAM <- V_x + [V_h + V_ext] - [V_h + V_ext + V_xc(KS)]^old
! alternatively for LKINETIC = .TRUE. WHAM stores
! WHAM <- T+V_x + [V_h + V_ext]
             CALL HAMILT_LOCAL(W1, SV_HARTREE_ION, ISP, WHAM_HARTREE_ION%CPTWFP(:,NP),.TRUE.)

! WHAM_HARTREE_ION <- T + V_x + [V_ext + V_h ]
...
! CTMP(2,1) = <u_2 | V_x + [V_loc]^new + sigma | u_1 >
          CALL ORTH2( &
...
!sitting on a node with kpt NK
       ...
       DO  N=1,NBANDSGW
          ...
          ENDDO
!DO  NP=1,NBANDSGW
!WRITE(90+WDES%COMM%NODE_ME,'(2I4,2F20.10)')N,NP, CHAM(N,NP)
!ENDDO
       ENDDO

! mK:
! The OEP potential V_OEP is derived from following condition
!
!  Tr{ G_0[ Sigma_xc(w) - V_OEP ] G_0 } = 0
!
! which can be recast into
!
!  [H_hf - H_0] (f_i-f_j)/(e_i-e_j) + Gamma_c  =  (V_OEP - V_xc ) * X_KS
!
! by adding and removing the KS-Hamiltonian H_0 = T + V_ext + V_h + V_xc.
! Here e_i, f_i are the eigenvalues and occupancies of H_0.
!
...
! align top of valence band
    ...
    IF (IU0 >0) THEN
       WRITE(IU0,"(' OEP shift',F14.5)") ESHIFT
    ENDIF
    IF (IU6>=0) THEN
       WRITE(IU6,"(' OEP shift',F14.5)") ESHIFT
    ENDIF
!ESHIFT=0
!WRITE(*,*)"WARNING ESHIFT set to ",ESHIFT
!=======================================================================
...
!=======================================================================
!WRITE(100+WDES%COMM%NODE_ME, '(F20.10)')GCHG
      CALL SYMMETRIZE_CHARGE( GCHG, SV, WDES, WDESQ, GRID, GRID_SOFT )
!WRITE(200+WDES%COMM%NODE_ME, '(F20.10)')GCHG
!=======================================================================
...
!=======================================================================
    CALL SYMMETRIZE_CHARGE( GCHG, SV1, WDES, WDESQ, GRID, GRID_SOFT, SV )

!WRITE(400+WDES%COMM%NODE_ME, '(F20.10)')GCHG
!WRITE(500+WDES%COMM%NODE_ME, '(F20.10)')SV
!
! now SV stores following potential:
! SV = chi^-1 * [ G_KS( V_x + V_loc^new - V_loc^old + Sigma_c ) G_KS ]
! CVTOT = V_loc^old
!
! V_loc^new = V_loc^old + DV
!
! where DV = V_loc^new - V_loc^old

!=======================================================================
! now update CVTOT and CDIJ
!=======================================================================
!    IF (IU6>=0) THEN
...
! GRID_SOFT -> GRIDC , SOFT_TO_C handle contains info about mapping
...
10  FORMAT((3X,I4,3X,7(F10.4,3X)))
   ...
   CONTAINS
!
! helper routine that symmetrizes charge density given in reciprocal space
! optionally, real-space representation is stored to SVSTORE if argument is
! passed
!
...
! change storage convention to (total, magnetization)
         ...
         IF (SYMM%ISYM==2) THEN
            IF (WDES%LNONCOLLINEAR) THEN
               CALL vtutor%bug("internal error in OEP_POTENTIAL: presently non collinear calculations are not " &
                  // "supported", "chi_base.F", 2759)
            ELSE
               DO ISP=1,WDES%ISPIN
...
! helper, dumps charge density given on COMPLEX(q)
...
!=======================================================================
    CALL ZHEEV &
         ...
         &           W,  IFAIL)
    IF (IFAIL/=0) THEN
       CALL vtutor%bug("internal ERROR in CHI_INVERT: Call to routine ZHEEV failed! Error code was " &
          // str(IFAIL), "chi_base.F", 2871)
    ENDIF
...
!NB1_INTO_TOT is occupied
!NB2_INTO_TOT is unoccupied
...
!*********************************************************************
  ...
  SUBROUTINE ALLOCATE_RESPONSEFUN( CHI, NALLOC, LGAMMA, LREALSTORE, NOMEGA_CHI)
    USE ini
    USE tutor, ONLY: argument, VTUTOR, ISERROR, ALLOC_RESPONSE
    TYPE (responsefunction) :: CHI
...
! allocate response function (body)
    ALLOCATE(CHI%RESPONSEFUN(N1, N2, MAX(NOMEGA_CHI,1)), STAT = ISTATUS )
    IF (ISTATUS/=0) THEN
       ALLOCATE(ARG%IVAL(1))
       ARG%IVAL(1)=16._q*N1*N2*NOMEGA_CHI/1024/1024
       CALL VTUTOR%WRITE(ISERROR, ALLOC_RESPONSE, ARG)
    ENDIF
    CALL REGISTER_ALLOCATE(16._q*SIZE(CHI%RESPONSEFUN,KIND=8), "response")
   
    ALLOCATE( & 
         CHI%WING(MAX(N1,1), 1:3, MAX(NOMEGA_CHI,1)), &
         CHI%CWING(MAX(N1,1), 1:3, MAX(NOMEGA_CHI,1)), &
...
! complex entries in the first direction
  ...
  SUBROUTINE ALLOCATE_RESPONSEFUN_SHMEM( CHI, NALLOC, LGAMMA, LREALSTORE, NOMEGA_CHI, COMM_SHMEM, IU0, IU6, LSEM )
    ...
    USE tutor, ONLY: argument, vtutor, isError, SHMEMError, ALLOC_RESPONSE
    TYPE (responsefunction) :: CHI
    LOGICAL :: LGAMMA       ! special mode using sin and cosine transforms
    LOGICAL :: LREALSTORE   ! special mode using real valued responsefunctions
    INTEGER, INTENT(IN)  :: NALLOC, NOMEGA_CHI
...
!*********************************************************************
! sets local number of frequency points
!*********************************************************************
    SUBROUTINE SET_NOMEGA_CHI( WGW, NCPU, NOMEGA_CHI, LNOALERT ) 
    USE tutor, ONLY: ISALERT, LSPECTRALINEFFICIENT
    USE ini, ONLY: QUERRY_ALLOCATE
    TYPE( wavedes )        :: WGW 
    INTEGER, INTENT(INOUT) :: NCPU
    INTEGER, INTENT(INOUT) :: NOMEGA_CHI
    LOGICAL, OPTIONAL      :: LNOALERT
    INTEGER :: MAXMEM_
...
# 3314
    ...
    IF (LSPECTRAL .AND. NOMEGA_CHI<(NOMEGA+NCPU-1)/NCPU ) THEN
        IF ( .NOT. PRESENT( LNOALERT ) ) THEN
           CALL VTUTOR%WRITE(ISALERT, LSPECTRALINEFFICIENT)
        ENDIF 
...
! but this would be very inefficient (untested as well)
    ...
    ENDIF
    END SUBROUTINE SET_NOMEGA_CHI

!*********************************************************************
! sets local number of frequency points for cached response function
!*********************************************************************
   SUBROUTINE DETERMINE_NOMEGA_WPOT( WGW, NCPU, NOMEGA_CHI, NOMEGA_WPOT ) 
      USE tutor, ONLY: ISALERT, LSPECTRALINEFFICIENT
      USE ini, ONLY: QUERRY_ALLOCATE
      TYPE( wavedes )        :: WGW 
      INTEGER, INTENT(IN)    :: NCPU
      INTEGER, INTENT(INOUT) :: NOMEGA_CHI
      INTEGER, INTENT(INOUT) :: NOMEGA_WPOT
      INTEGER :: MAXMEM_
...
! NOMEGA_CHI on each core x  number of cores, but certainly not more than NOMEGA
      ...
      ENDIF
   END SUBROUTINE DETERMINE_NOMEGA_WPOT
...
!
  ...
  SUBROUTINE ALLOCATE_RESPONSEFUN_DISTRI( CHI, NALLOC_ROW, NALLOC_COL, LGAMMA, LREALSTORE, NOMEGA_CHI)
    USE ini
    USE tutor, ONLY: argument, VTUTOR, ISERROR, ALLOC_RESPONSE_DISTR
    TYPE (responsefunction) :: CHI
    LOGICAL :: LGAMMA       ! special mode using sin and cosine transforms
    LOGICAL :: LREALSTORE   ! special mode using real valued responsefunctions
...
! this is usually used for complex frequencies (or at w=0)
    ...
    ALLOCATE(CHI%RESPONSEFUN(N1, N2, MAX(NOMEGA_CHI,1)), STAT = ISTATUS )
! CHI%RESPONSEFUN=(0.0_q, 0.0_q) ! Breaks SiC_OEPR (gnu), may pass uninitialized parts to FFTs in CALCULATE_SIGMA_SUPER.
    IF (ISTATUS/=0) THEN
       ALLOCATE(ARG%IVAL(1))
       ARG%IVAL(1)=16._q*N1*N2*NOMEGA_CHI/1024/1024
       CALL VTUTOR%WRITE(ISERROR, ALLOC_RESPONSE_DISTR, ARG)
    ENDIF
    CALL REGISTER_ALLOCATE(16._q*SIZE(CHI%RESPONSEFUN,KIND=8), "response")
...
!> @details @ref openmp :
!> Allocate
!>    responsefunction::ncache, responsefunction::storage,
!>    responsefunction::weight, responsefunction::stpos, and
!>    responsefunction::w_inter,
!> with an additional dimension omp_get_max_threads (slowest index).
!> This allows each thread to work with its own cache structure lateron.
!>
!> In addition the arrays
!>    responsefunction::omp_lck1 and responsefunction::omp_lck2
!> are allocated and initialized.
!> These provide OpenMP locks on
!>     responsefunction::responsefun and responsefunction::head
!> respectively.
!> Aforementioned locks are used to prevent different threads from
!> writing to the same entries of CHI\%RESPONSEFUN and CHI\%HEAD
!> simultaneously (data race).
!
...
! leave as is
    ...
    ALLOCATE( CHI%STORAGE( NALLOC, MAXCACHE, NOMEGA_CHI ), STAT= ISTATUS)
    IF (ISTATUS/=0) THEN
       ALLOCATE(ARG%IVAL(1))
       ARG%IVAL(1)=16._q*NALLOC*MAXCACHE*NOMEGA_CHI
!$     ARG%IVAL(1)=ARG%IVAL(1) * OMP_GET_MAX_THREADS()
       ARG%IVAL(1)=ARG%IVAL(1)/1024/1024
       CALL VTUTOR%WRITE(ISERROR, ALLOC_RESPONSE, ARG)
    ENDIF
    CALL REGISTER_ALLOCATE(16._q*SIZE(CHI%STORAGE,KIND=8), "response")
...
!$  ALLOCATE(CHI%OMP_LCK1(NOMEGA_CHI),CHI%OMP_LCK2(NOMEGA_CHI))
!$  DO N=1,NOMEGA_CHI
!$     CALL OMP_INIT_LOCK(CHI%OMP_LCK1(N))
!$     CALL OMP_INIT_LOCK(CHI%OMP_LCK2(N))
!$  ENDDO
...
# 3622
    ...
    CALL DEREGISTER_ALLOCATE(16._q*SIZE(CHI%RESPONSEFUN,KIND=8), "response")
...
# 3627
    ...
    IF (ASSOCIATED(CHI%STORAGE)) THEN
       CALL DEREGISTER_ALLOCATE(16._q*SIZE(CHI%STORAGE,KIND=8), "response")
       DEALLOCATE(CHI%STORAGE, CHI%WEIGHT, CHI%STPOS, CHI%W_INTER, CHI%NCACHE)
       NULLIFY(CHI%STORAGE, CHI%WEIGHT, CHI%STPOS, CHI%W_INTER)
!$     DO N=1,CHI%NOMEGA
!$        CALL OMP_DESTROY_LOCK(CHI%OMP_LCK1(N))
!$        CALL OMP_DESTROY_LOCK(CHI%OMP_LCK2(N))
!$     ENDDO
!$     DEALLOCATE(CHI%OMP_LCK1); NULLIFY(CHI%OMP_LCK1)
!$     DEALLOCATE(CHI%OMP_LCK2); NULLIFY(CHI%OMP_LCK2)
    END IF
...
!
!> @details @ref openmp :
!> Under OpenMP this subroutine is called from within a parallel region.
!> Each thread will work with its own cache structure. To this end
!>    CHI\%NCACHE  (responsefunction::ncache),
!>    CHI\%STORAGE (responsefunction::storage),
!>    CHI\%WEIGHT  (responsefunction::weight)
!> acquired an additional index that is set to thread-id.
!
!***********************************************************************
...
!$  INTEGER 1
!$  1=OMP_GET_THREAD_NUM()+1

    

    IF (.NOT. ASSOCIATED(CHI%NCACHE) ) THEN
       CALL vtutor%bug("internal error in ADD_RESPONSEFUNCTION_CACHE: cache structure not allocated " &
          // "\n search in chi.F and comment in ALLOCATE_RESPONSEFUN_CACHE", "chi_base.F", 3701)
    ENDIF


    IF (CHI%NCACHE(NOMEGA )/=0) THEN
       IF (NP/=CHI%NP) THEN
          CALL vtutor%bug("internal error in ADD_RESPONSEFUNCTION_CACHE: NP changed " // str(NP) // &
             " " // str(CHI%NP), "chi_base.F", 3708)
       ENDIF
    ENDIF
    CHI%NP=NP
...
!>    CHI\%STORAGE (responsefunction::storage),
!>    CHI\%WEIGHT  (responsefunction::weight)
!> acquired an additional index that is set to the thread-id.
!>
!> The OpenMP lock CHI\%OMP_LCK1 (responsefunction::omp_lck1) is used
!> to prevent different threads from accessing the same entries of
!> CHI\%RESPONSEFUN (responsefunction::responsefun) simultaneously
!> (data race).
!
!*************************************************************************
...
!!$  IF (OMP_GET_NUM_THREADS()/=SIZE(CHI%NCACHE,2)) THEN
!!$     WRITE(71+OMP_GET_THREAD_NUM(),*) 'ERROR:',OMP_GET_THREAD_NUM(),OMP_GET_NUM_THREADS()
!!$  ENDIF
...
!$        CALL OMP_SET_LOCK(CHI%OMP_LCK1(NOMEGA))
          IF (CHI%LREALSTORE) THEN
          ...
          ELSE
             CALL ZGEMM('N','C', CHI%NP, CHI%NP, CHI%NCACHE(NOMEGA ), (1.0_q, 0.0_q), &
                  ...
                  (1.0_q,0.0_q), CHI%RESPONSEFUN(1,1,NOMEGA), SIZE(CHI%RESPONSEFUN,1))
          ENDIF
!$        CALL OMP_UNSET_LOCK(CHI%OMP_LCK1(NOMEGA))
# 3814
...
!> Each thread will work with its own cache structure. To this end
!>    CHI\%NCACHE  (responsefunction::ncache),
!>    CHI\%STPOS   (responsefunction::stpos),
!>    CHI\%W_INTER (responsefunction::w_inter)
!> acquired an additional index that is set to the thread-id.
!
!***********************************************************************
...
!$  1=OMP_GET_THREAD_NUM()+1
    ...
    IF (.NOT.ASSOCIATED(CHI%NCACHE)) THEN
       CALL vtutor%bug("internal error in ADD_RESPONSEFUNCTION_INT: cache not allocated \n search " &
          // "in chi.F and comment in ALLOCATE_RESPONSEFUN_CACHE", "chi_base.F", 3877)
    ENDIF

    IF (CHI%NCACHE(NOMEGA )/=0) THEN
       IF (NP/=CHI%NP) THEN
          CALL vtutor%bug("internal error in ADD_RESPONSEFUNCTION_CACHE_INT: NP changed " // str(NP) &
             // " " // str(CHI%NP), "chi_base.F", 3883)
       ENDIF
    ENDIF
    CHI%NP=NP
...
!       WRITE(*,'(3F14.7)') WPLASMON
       ...
       IF (WPOT%NOMEGA_LOW<=2 .AND. 2<=WPOT%NOMEGA_HIGH) THEN
          ...
          IF (ABS(WPOT%COMEGA(NOMEGA)-COMEGA(2))>1E-6) THEN
             CALL vtutor%bug("internal error in ADD_DRUDE_TERM: incorrect slot", "chi_base.F", 5097)
          ENDIF
...
# 5210
    ...
    IF (CHI%LREALSTORE .OR. WPOT%LREALSTORE) THEN
       CALL vtutor%bug("internal error in DO_CHI_KRAM_KRON_TABLE: LREALSTORE is not supported yet", "chi_base.F", 5216)
    ENDIF
...
!*********************************************************************
  ...
  SUBROUTINE WING_FROM_BODY( CHI, IDIR, NOMEGA)
    ...
    IF (.NOT.CHI%LGAMMA) THEN
       CALL vtutor%bug("internal error in WING_FROM_BODY: LGAMMA is not set", "chi_base.F", 5387)
    ENDIF
  ...
  SUBROUTINE BODY_FROM_WING( CHI, IDIR, NOMEGA)
    ...
    IF (.NOT.CHI%LGAMMA) THEN
       CALL vtutor%bug("internal error in BODY_FROM_WING: LGAMMA is not set", "chi_base.F", 5420)
    ENDIF
...
!*********************************************************************
  ...
  SUBROUTINE SET_MAT_FROM_RESPONSE( MAT, CHI, NOMEGA, IDIR)
    ...
    IF (CHI%LGAMMA) THEN
       IF (.NOT.PRESENT(IDIR)) THEN
          CALL vtutor%bug("internal error in SET_MAT_RESPONSE_FROM: IDIR not specified", "chi_base.F", 5485)
       ENDIF
    ELSE
       IF (PRESENT(IDIR)) THEN
          CALL vtutor%bug("internal error in SET_MAT_RESPONSE_FROM: IDIR must not be specified", "chi_base.F", 5489)
       ENDIF
    ENDIF
  ...
  SUBROUTINE SET_RESPONSE_FROM_MAT( MAT, CHI, NOMEGA, IDIR)
    ...
    IF (CHI%LGAMMA) THEN
       IF (.NOT.PRESENT(IDIR)) THEN
          CALL vtutor%bug("internal error in SET_RESPONSE_FROM_MAT: IDIR not specified", "chi_base.F", 5524)
       ENDIF
    ELSE
       IF (PRESENT(IDIR)) THEN
          CALL vtutor%bug("internal error in SET_RESPONSE_FROM_MAT: IDIR must not be specified", "chi_base.F", 5528)
       ENDIF
    ENDIF
  ...
  SUBROUTINE SET_WING_FROM_MAT( MAT, CHI, NOMEGA, IDIR)
    ...
    IF (CHI%LGAMMA) THEN
       IF (.NOT.PRESENT(IDIR)) THEN
          CALL vtutor%bug("internal error in SET_RESPONSE_FROM_MAT: IDIR not specified", "chi_base.F", 5565)
       ENDIF
    ELSE
       IF (PRESENT(IDIR)) THEN
          CALL vtutor%bug("internal error in SET_RESPONSE_FROM_MAT: IDIR must not be specified", "chi_base.F", 5569)
       ENDIF
    ENDIF
...
!*********************************************************************
  ...
  SUBROUTINE DUMP_RESPONSE( CHI, WDESQ, NOMEGA, RESPONSE_ID, IU, NWRITE, LSUMK, LINVERS, FAKT, ADD)

    USE vhdf5

    TYPE (responsefunction), INTENT(IN) :: CHI
    INTEGER, INTENT(IN) :: RESPONSE_ID, IU
...
! local
    ...
    CHARACTER(LEN=:), ALLOCATABLE :: STRING, PREFIX
    ...
    COMPLEX(q) :: WING(10,3,NOMEGA), CWING(10,3,NOMEGA), CEPS(NOMEGA,3,3)
    ...
    CALL RESPONSE_STRING(RESPONSE_ID, STRING, PREFIX)
...
! test
       ...
       DO N = 1, NOMEGA
          CEPS(N,:,:) = CMPLX(EPS_REAL(:,:,N), EPS_IMAG(:,:,N), KIND=q)
       END DO
       IF (ALLOCATED(PREFIX)) THEN
          CALL VH5_WRITE_DIELECTRIC_ENERGIES(IH5OUTFILEID, DOMEGA, PREFIX)
          CALL VH5_WRITE_DIELECTRIC_DYNAMIC(IH5OUTFILEID, CEPS, PREFIX)
       END IF
...
1100 FORMAT(/" ",A/, &
  ...
  SUBROUTINE RESPONSE_STRING(RESPONSE_ID, LONG_FORM, SHORT_FORM)
     INTEGER, INTENT(IN) :: RESPONSE_ID
     CHARACTER(LEN=:), ALLOCATABLE, INTENT(OUT) :: LONG_FORM, SHORT_FORM
     SELECT CASE (RESPONSE_ID)
     CASE (ID_INDEPENDENT_PARTICLE)
        LONG_FORM = "HEAD OF MICROSCOPIC DIELECTRIC TENSOR (INDEPENDENT PARTICLE)"
        SHORT_FORM = "ipa"
     CASE (ID_RESONANT)
        LONG_FORM = "HEAD OF MICROSCOPIC DIELECTRIC TENSOR (RESONANT)"
        SHORT_FORM = "resonant"
     CASE (ID_POLARIZABILITY)
        LONG_FORM = "1 + v P,  with REDUCIBLE POLARIZABILTY P=P_0 (1 -(v+f) P_0)^-1"
     CASE (ID_RPA)
        LONG_FORM = "INVERSE MACROSCOPIC DIELECTRIC TENSOR (including local field effects in RPA (Hartree))"
        SHORT_FORM = "rpa"
     CASE (ID_DFT)
        LONG_FORM = "INVERSE MACROSCOPIC DIELECTRIC TENSOR (including local field effects in DFT)"
        SHORT_FORM = "dft"
     CASE (ID_CHARGE_ELECTRON)
        LONG_FORM = "INVERSE MACROSCOPIC DIELECTRIC TENSOR (test charge-test electron, local field effects in DFT)"
        SHORT_FORM = "tcte"
     CASE (ID_CHARGE_CHARGE)
        LONG_FORM = "INVERSE MACROSCOPIC DIELECTRIC TENSOR (test charge-test charge, local field effects in DFT)"
        SHORT_FORM = "tctc"
     CASE (ID_MICROSCOPIC)
        LONG_FORM = "HEAD OF MICROSCOPIC DIELECTRIC TENSOR"
     CASE (ID_MICROSCOPIC_DFT)
        LONG_FORM = "HEAD OF MICROSCOPIC DIELECTRIC TENSOR (DFT)"
     CASE (ID_SCREENED_COULOMB)
        LONG_FORM = "screened Coulomb potential"
     CASE (ID_XI_LOCAL_FIELD)
        LONG_FORM = " Xi from CALCULATE_LOCAL_FIELD_FOCK"
     CASE (ID_XI_GW)
        LONG_FORM = " Xi from GW routine"
     CASE (ID_GG)
        LONG_FORM = " GG W GG from CALCULATE_LOCAL_FIELD_FOCK"
     CASE (ID_GG_ANTIRESONANT)
        LONG_FORM = " GG W GG antiresonant from CALCULATE_LOCAL_FIELD_FOCK"
     CASE (ID_FXC)
        LONG_FORM = " fxc from DETERMINE_FXC_FROM_TBSE"
     CASE (ID_FXC_ANTIRESONANT)
        LONG_FORM = " fxc antiresonant from DETERMINE_FXC_FROM_TBSE"
     END SELECT
  END SUBROUTINE RESPONSE_STRING
...
! local
    ...
    IF (CHI%LREALSTORE .OR. WPOT%LREALSTORE) THEN
       CALL vtutor%bug("internal error in DO_POT_HILBERT_TABLE: LREALSTORE is not supported yet", "chi_base.F", 6570)
    ENDIF
...
!***********************************************************************
! DESCRIPTION:
!>generalization of EMPTY_XI_ORBITAL for finite temperature
!>returns .TRUE. if state is included in positive time green's function
!>
!>positive time green's function is defined as
!>  (1-f)e^(-en t)    , en > 0
!>= f e^(en(beta-t))  , en < 0
!>
!>because  0 < t < beta/2 (1._q,0._q) has
!
!>  (1-f)                      ,  t = 0     , en > 0
!>  (1-f)e^( -en beta/2 )      ,  t = beta/2, en > 0
!>  f e^( en beta )            ,  t = 0     , en < 0
!>  f e^( en beta/2 )          ,  t = beta/2, en < 0
!
!>if x_max is the positive numbe for with nummerically e( - x_max ) = 0
!>then following states should be included in positive time green's
!>function
!>
!> -x_max < en beta < 2 x_max
!
!***********************************************************************

  FUNCTION POSITIVE_TIME_ORBITAL(F, EBETA)
    USE prec
    IMPLICIT NONE
    LOGICAL POSITIVE_TIME_ORBITAL
    REAL(q) :: F, EBETA 

    

    IF ( LFINITE_TEMPERATURE ) THEN
!IF ( -GPOS_THRESHHOLD < EBETA .AND. EBETA < 2*GPOS_THRESHHOLD ) THEN
       IF ( -GPOS_THRESHHOLD < EBETA ) THEN
          POSITIVE_TIME_ORBITAL = .TRUE. 
       ELSE
          POSITIVE_TIME_ORBITAL = .FALSE. 
       ENDIF
POSITIVE_TIME_ORBITAL = .TRUE. 
! To keep backwards compatibility use EMPTY_XI_ORBITAL for T=0
    ELSE
       POSITIVE_TIME_ORBITAL = EMPTY_XI_ORBITAL( F ) 
    ENDIF


    
  END FUNCTION POSITIVE_TIME_ORBITAL
...
!***********************************************************************
! DESCRIPTION:
!>generalization for finite temperature
!>returns .TRUE. if state is included in negative time green's function
!
!>negative time green's function is defined as
!>  f e^( -en t )             , en < 0
!>= (1-f) e^( -en (beta+t) )  , en > 0
!>
!>because  0 > t > -beta/2 (1._q,0._q) has
!
!>  f                          ,  t = 0     , en < 0
!>  f e^( en beta/2 )          ,  t =-beta/2, en < 0
!>  (1-f) e^( -en beta )       ,  t = 0     , en > 0
!>  (1-f) e^( -en beta/2 )     ,  t = beta/2, en > 0
!
!>if x_max is the positive number for with nummerically e( - x_max ) = 0
!>then following states should be included in positive time green's function
!>
!> -2*x_max < en beta < x_max  for en > 0
!>            en beta < x_max  for en < 0
!>
!>
!***********************************************************************

  FUNCTION NEGATIVE_TIME_ORBITAL(F, EBETA)
    USE prec
    IMPLICIT NONE
    LOGICAL NEGATIVE_TIME_ORBITAL
    REAL(q) :: F, EBETA 

    
    
    IF ( LFINITE_TEMPERATURE ) THEN
!IF ( -2*GPOS_THRESHHOLD < EBETA .AND. EBETA < GPOS_THRESHHOLD ) THEN
!IF ( EBETA < GPOS_THRESHHOLD ) THEN
!   NEGATIVE_TIME_ORBITAL = .TRUE.
!ELSE
!   NEGATIVE_TIME_ORBITAL = .FALSE.
!ENDIF
       NEGATIVE_TIME_ORBITAL = .TRUE. 
! To keep backwards compatibility use FILLED_XI_ORBITAL for T=0
    ELSE
       NEGATIVE_TIME_ORBITAL = FILLED_XI_ORBITAL( F ) 
    ENDIF

    
  END FUNCTION NEGATIVE_TIME_ORBITAL
...
!***********************************************************************
  ...
  FUNCTION LAST_FILLED_XI( W, K1, ISP, BETA)
    USE prec
    USE wave
    IMPLICIT NONE
    TYPE (wavespin) W
    ...
    INTEGER K1, ISP
    REAL(q), OPTIONAL :: BETA
! local
    INTEGER NB

    

    IF ( PRESENT( BETA ) .AND. LFINITE_TEMPERATURE ) THEN
       DO NB=W%WDES%NB_TOT, 1, -1
! this gives the last positive time state ( or last occupied for T=0)
          IF( NEGATIVE_TIME_ORBITAL( W%FERTOT( NB, K1, ISP),&
            REAL(W%CELTOT(NB,K1,ISP)-W%EFERMI(ISP),q)*BETA )) EXIT
       ENDDO
    ELSE
       DO NB=W%WDES%NB_TOT, 1, -1
! if partially occupied band is required
...
! this version is without the modulo
  FUNCTION LAST_FILLED_XI_NOMOD( W, K1, ISP, BETA)
    USE prec
    USE wave
    IMPLICIT NONE
    TYPE (wavespin) W
...
!***********************************************************************
! finite temperture generalization
!***********************************************************************

  FUNCTION FIRST_POS_TIME_LOCAL_ORBITAL( W, K1, ISP, EF, BETA )
    USE prec
    USE wave
    IMPLICIT NONE
    INTEGER         :: FIRST_POS_TIME_LOCAL_ORBITAL
    TYPE (wavespin) :: W
    REAL(q)         :: EF
    REAL(q)         :: BETA
    INTEGER         :: K1, ISP
! local
    ...
    IF ( LFINITE_TEMPERATURE ) THEN
       DO NB=1,W%WDES%NBANDS
          IF ( POSITIVE_TIME_ORBITAL( W%FERWE( NB, K1, ISP) , &
             REAL(W%CELEN(NB,K1,ISP)-EF,q)*BETA )  ) EXIT
       ENDDO
       FIRST_POS_TIME_LOCAL_ORBITAL = NB

! revert to T=0 selection of first empty local orbital
    ELSE
       FIRST_POS_TIME_LOCAL_ORBITAL=FIRST_EMPTY_XI_LOCAL( W, K1, ISP )
    ENDIF
    
     
    
  END FUNCTION FIRST_POS_TIME_LOCAL_ORBITAL
...
!***********************************************************************
! finite temperature generalization of LAST_EMPTY_XI_LOCAL
!***********************************************************************

  FUNCTION LAST_POS_TIME_LOCAL_ORBITAL( W, K1, ISP, EF, BETA)
    USE prec
    USE wave
    IMPLICIT NONE
    INTEGER         :: LAST_POS_TIME_LOCAL_ORBITAL
    TYPE (wavespin) :: W
    REAL(q)         :: EF
    REAL(q)         :: BETA
    INTEGER         :: K1, ISP
! local
    ...
    IF ( LFINITE_TEMPERATURE ) THEN
! since I dont know how to deals with this, we use the T=0 call
! and allow all unoccpied states
       LAST_POS_TIME_LOCAL_ORBITAL = LAST_EMPTY_XI_LOCAL( W, K1, ISP ) 
...
! revert to T=0 selection of first empty local orbital
  ...
  END FUNCTION LAST_POS_TIME_LOCAL_ORBITAL
...
!***********************************************************************
! prints required storage for job and a warning if MAXMEM is too low
!***********************************************************************
  SUBROUTINE DUMP_REQUIRED_STORAGE( WDES, WGW, S2E, IO ) 
    USE ini, ONLY: QUERRY_ALLOCATE
    TYPE (wavedes) :: WDES
    TYPE (wavedes) :: WGW
    TYPE (screened_2e_handle) S2E
    TYPE (in_struct)   IO
! local
    TYPE( mem_gw_handle ) :: MEM

! obtain size of already allocated and registered memory
    MEM%M_REST = QUERRY_ALLOCATE()/1024._q

! contribution from screened2e integrals
    CALL MEM_CHI_QUARTIC( MEM%M_CHIOMEGA, WDES, WGW, S2E ) 

! contribution from screened2e integrals
    CALL MEM_SCREENED_TWO_E( MEM%M_SCREENED2E, WDES, S2E ) 

! contribution from additionally requiered wavefunctions
    CALL MEM_WAVEFUNCTIONS( MEM%M_WAVEFUN, WDES, S2E ) 

! dump info to screen
    CALL DUMP_INFO( MEM, IO%IU0, IO%NWRITE, .FALSE.  ) 

! dump info to OUTCAR
    CALL DUMP_INFO( MEM, IO%IU6, IO%NWRITE, .TRUE.  ) 

    CONTAINS 

!*******************************************************************
! determines memory requirement from screened two electron array
! in quartic GW code ( in MB )
!*******************************************************************
    SUBROUTINE MEM_CHI_QUARTIC( M_CHIOMEGA, WDES, WGW, S2E)
...
!$     INTEGER, EXTERNAL :: OMP_GET_MAX_THREADS
       ...
       IF ( ICHIREAL==0 ) THEN
! set in ALLOCATE_RESPONSEFUNCTION
          LREALSTORE =  LACFDT .AND. WGW%LGAMMA
          N1 = WGW%NGDIM
          N2 = WGW%NGDIM 
          IF ( WGW%LGAMMA .ANd. LREALSTORE ) THEN
             N1 = N1 * 2 
             N2 = N2 * 2 
          ELSE IF ( WGW%LGAMMA ) THEN
             N2 = N2 * 2 
          ENDIF 
          CALL SET_NOMEGA_CHI( WGW, NCPU, NOMEGA_CHI, LNOALERT = .TRUE. ) 

! GW calculations for more than 1 k-point using spectral decompositions
          IF ( LGW0 .AND. LSPECTRAL .AND. S2E%NUMBER_OF_NQ > 1 ) THEN
             M_CHI(1) = M_CHI(1) + N1* N2 * NOMEGA_CHI 
          ENDIF

          IF (LSPECTRAL .AND. LADDER .AND. NBANDSO>0) THEN
             M_CHI(2) = M_CHI(2) + N1* N2 * NOMEGA_CHI 
          ENDIF 

! number of frequency points of cached response function
          CALL DETERMINE_NOMEGA_WPOT( WGW, NCPU, NOMEGA_CHI, NOMEGA_WPOT ) 
          M_CHI(3) = M_CHI(3) + N1* N2 * NOMEGA_WPOT
# 7208

         
          IF (LSPECTRAL .OR. LscQPGW) THEN
             M_CHI(4) = N1*MAXCACHE*NOMEGA_WPOT
!$           M_CHI(4) = M_CHI(4)*OMP_GET_MAX_THREADS()
          ENDIF

! need cache structure to evaluate selfenergy between states
          IF ( LGW ) THEN
             M_CHI(5) = N1*MAXCACHE*NOMEGA_WPOT
!$           M_CHI(5) = M_CHI(5)*OMP_GET_MAX_THREADS()
          ENDIF
...
! obtain maximum value
       M_CHIOMEGA = SUM( M_CHI(:) ) 
       M_CHIOMEGA = M_CHIOMEGA * 8._q * 2 / 1024/ 1024 
 
    END SUBROUTINE MEM_CHI_QUARTIC

!*******************************************************************
! determines memory requirement from screened two electron array
! in quartic GW code ( in MB )
!*******************************************************************
    SUBROUTINE MEM_SCREENED_TWO_E( M_SCREENED2E, WDES, S2E)
       USE kpoints_change, ONLY : KPOINTS_ORIG
       REAL(q),INTENT(INOUT) :: M_SCREENED2E
       TYPE (wavedes) :: WDES
       TYPE (screened_2e_handle) S2E

       M_SCREENED2E = 0 

       IF (LGW .AND. ICHIREAL == 0 ) THEN
          IF (LSPECTRAL) THEN
             IF (LSELFENERGY ) THEN
                M_SCREENED2E = NBANDSGW*KPOINTS_ORIG%NKPTS*WDES%NBANDS &
                             * MAXVAL(S2E%K2_STORE_INDEX)*WDES%ISPIN*NOMEGA*2
             ELSE
                M_SCREENED2E  = NBANDSGW*KPOINTS_ORIG%NKPTS*WDES%NBANDS &
                             * MAXVAL(S2E%K2_STORE_INDEX)*WDES%ISPIN*2
             ENDIF
          ELSE
             M_SCREENED2E  = NBANDSGW*KPOINTS_ORIG%NKPTS*WDES%NBANDS &
                           * MAXVAL(S2E%K2_STORE_INDEX)*WDES%ISPIN*NOMEGA
          ENDIF
       ENDIF

       M_SCREENED2E = 16._q*M_SCREENED2E/1024/1024

    END SUBROUTINE MEM_SCREENED_TWO_E

!*******************************************************************
! determines memory requirement for additional wavefunctions
! in quartic GW code ( in MB )
!*******************************************************************
    SUBROUTINE MEM_WAVEFUNCTIONS( M_WAVEFUN, WDES, S2E)
       REAL(q),INTENT(INOUT) :: M_WAVEFUN
       TYPE (wavedes) :: WDES
       TYPE (screened_2e_handle) S2E
       REAL(q)        :: MWAVE
       INTEGER        :: I
...
! storage for (1._q,0._q) wavefunction
       MWAVE = 2*WDES%NRPLWV*WDES%NBANDS*WDES%NKPTS*WDES%ISPIN
       IF (WDES%LGAMMA) THEN
          MWAVE = MWAVE + &
                      WDES%NPROD*WDES%NBANDS*WDES%NKPTS*WDES%ISPIN
       ELSE
          MWAVE = MWAVE + &
                      2*WDES%NPROD*WDES%NBANDS*WDES%NKPTS*WDES%ISPIN
       ENDIF

       LWFROMFILE = .FALSE.
       INQUIRE( FILE='WAVECAR.chi', EXIST = LWFROMFILE ) 

! count how many additional wavefunction are requiered
       I=0
       IF (LscQPGW .AND. ICHIREAL == 0 ) I = I + 2 

       IF (LscQPGW .AND. LGW0 .AND. .NOT. LWFROMFILE) I = I + 1 

       M_WAVEFUN = 8._q*I*MWAVE/1024/1024

    END SUBROUTINE MEM_WAVEFUNCTIONS

!*******************************************************************
! prints info to screened or file
! for LPRINTWARNING = T warning is printed in case MAXMEM is smaller
! than estimated memory requirement for job
!*******************************************************************
     SUBROUTINE DUMP_INFO( MEM, IU0, NWRITE, LPRINTWARNING ) 
        USE tutor, ONLY: argument, VTUTOR, ISALERT, MAXMEM_LOW
        USE main_mpi, only: COMM_INTRA_NODE_WORLD
        TYPE( mem_gw_handle ) :: MEM
        INTEGER,INTENT(IN)    :: IU0
        INTEGER,INTENT(IN)    :: NWRITE
        LOGICAL               :: LPRINTWARNING
! local
        REAL(q)               :: M_MASTER   
        TYPE( argument )      :: ARG 
        INTEGER               :: NCPU_NODE = 1 
...
! add auxillary wavfunction storage
        MEM%M_REST = MEM%M_REST+MEM%M_WAVEFUN

! accumulate all pieces required to store for job
        M_MASTER = M_MASTER + MEM%M_REST + MEM%M_CHIOMEGA + MEM%M_SCREENED2E 

! print warning if more memory is required than presently available
! MAXMEM is either set by user or automatically detected with
! AUTOSET_AVAILABLE_MEMORY in ini.F
        IF ( LPRINTWARNING .AND. M_MASTER > MAXMEM ) THEN
           ALLOCATE( ARG%IVAL(2) ) 
           ARG%IVAL(1) = MAXMEM
           ARG%IVAL(2) = INT(M_MASTER)
           CALL VTUTOR%WRITE( ISALERT, MAXMEM_LOW, ARG ) 
           DEALLOCATE( ARG%IVAL )
        ENDIF 
     
        IF ( IU0>=0 ) WRITE(IU0, 990 ) M_MASTER, M_MASTER*NCPU_NODE

990   FORMAT( /," min. memory requirement per mpi rank ", &
...
! set
  ...
  END SUBROUTINE DUMP_REQUIRED_STORAGE
