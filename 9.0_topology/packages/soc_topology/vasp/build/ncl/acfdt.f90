...
!> number of cutoffs for regression
  INTEGER, SAVE, PRIVATE :: NE=8
...
! use single shot method with SCK for RPA
    ...
    IF (CHI%LREAL .AND. .NOT. CHI%LREALSTORE) THEN
       CALL vtutor%bug("internal error in XI_ACFDT_ALL_RPA: for the Gamma point version " &
          // "CHI%LREALSTORE must be set", "acfdt.F", 173)
    ENDIF
...
! fully for x, y and z direction in components 2, 3 and 4
                ...
                IF (.NOT. LADDER) THEN
                   CALL XI_LOCAL_FIELD_ACFDT( CHI_WORK, CHI, LATT_CUR, & 
                        COR, I, NOMEGA , WGWQ , NP_NEW, .FALSE.)
                   IF ( LESF_SPLINES ) THEN
! stores diagonal of electronic structure factor
                      CALL EXTRACT_ELSTUFAC( CHI_WORK, NP_NEW, CHI%LREAL, WGWQ, LATT_CUR, &
                         COR%ESF(I), NQ, LAMBDA, OWEIGHT/IDIR_MAX/2, SUM, SUMMP2, .FALSE., IU0 ) 
                   ELSE
! perform trace
...
! stores diagonal of electronic structure factor
                   CALL EXTRACT_ELSTUFAC( CHI_WORK, NP_NEW, CHI%LREAL, WGWQ, LATT_CUR,&
                      COR%ESF(I), NQ, LAMBDA, OWEIGHT/2, SUM, SUMMP2, .FALSE., IU0 ) 
                ELSE
...
! have to take care of that now
    ...
    IF( LESF_SPLINES ) THEN
! twisted, some ranks might not have set NG correctly
       DO I=1,NE
          CALL M_max_i(WGWQ%COMM, COR%ESF(I)%NG(NQ), 1)
       ENDDO
       IF ( NQ == COR%ESF(1)%NUMBER_OF_NQ ) THEN
!summation withoug symmetry
       CALL EVALUATE_TR( COR , WGWQ) 
!WRITE(100+NQ,'(3F12.6)') COR%ESF(1)%STUFAK
!CALL M_stop('VASP aborting ...'); stop
    ...
    SUBROUTINE EVALUATE_TR( COR , WGWQ) 
...
! require a collective sum for the structure factor
          CALL M_sum_d(WGWQ%COMM_INTER, COR%ESF(I)%S(1,1), SIZE( COR%ESF(I)%S) )
          IF ( L_SUM_KINTER) THEN
             CALL M_sum_d(WGWQ%COMM_KINTER, COR%ESF(I)%S(1,1), SIZE( COR%ESF(I)%S) )
          ENDIF
...
!=======================================================================
    IF (LREAL) THEN
       ...
       CALL vtutor%bug("internal error in ROTLN_TRACE: LREAL is set, but complex code", "acfdt.F", 488)
...
!
!> this subroutine corrects the truncated coulomb potential with
!> multipoles
!
!***********************************************************************
  SUBROUTINE  APPLY_MULTIPOLE_CORRECTIONS( CHI_WORK, NP, NPLWV  )
    COMPLEX(q)   , INTENT(INOUT) :: CHI_WORK(:,:)
...
! acfdt does not need this extra factor so we have to take it out again:
        CHI_WORK=CHI_WORK*NPLWV
     ENDIF

  END SUBROUTINE  APPLY_MULTIPOLE_CORRECTIONS
...
!***********************************************************************
  ...
  SUBROUTINE XI_LOCAL_FIELD_ACFDT( CHI_WORK, CHI, LATT_CUR, & 
       COR, I , NOMEGA, WGWQ , MAXINDEX, LSR)
    USE constant
    USE fock
    ...
    LOGICAL :: LSR !< short range coulomb kernel
! local
    ...
    CALL SETUP_COULOMB_DIAGONAL( LATT_CUR, ENCUT, ENCUTSOFT, WGWQ, MAXINDEX, LSR, OLD_INDEX, DATAKE )
! setup index size
    NP=WGWQ%NGVECTOR
    IF (WGWQ%LGAMMA) NP=NP*2
   
! store truncated coulomb diagonal
    IF ( LESF_SPLINES ) THEN
       COR%ESF(I)%V = 0 
...
!=======================================================================
    ...
    DO NI=1,NP
! get v^ 1/2
       CHI_WORK(1:NP,NI)=CHI_WORK(1:NP,NI)*DATAKE(NI)
       CHI_WORK(NI,1:NP)=CHI_WORK(NI,1:NP)*DATAKE(NI)
    ENDDO

! potentially apply multipole corrections
    CALL APPLY_MULTIPOLE_CORRECTIONS( CHI_WORK, NP, WGWQ%GRID%NPLWV  )
...
!> local
  ...
  REAL(q) :: LAMBDA
    ...
    IF ( LESF_SPLINES ) THEN
       IF (INOUT>=0) WRITE(INOUT,'("      cutoff energy     smooth cutoff   RPA   correlation   Hartree contr. to MP2  RPA spline-interp.")')
