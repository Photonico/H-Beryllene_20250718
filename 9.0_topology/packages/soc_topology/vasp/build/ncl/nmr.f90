...
!> magnitude of magnetic moment from current density
  ...
  REAL(q), SAVE :: MAGDIPOLOUT(3) = 0.0_q
...
!> reconstruct and use the B-component in the AE (1._q,0._q)-center terms
...
! calculate also the vGv approximation to the susceptibility.
! linear response only.

  LOGICAL, SAVE :: LVGVCALC=.TRUE.

! apply the vGv susceptibility instead of the pGv susceptibility
! for the G==0 component of the shielding/shift.
! default: pGv for backward compatibility.
! linear response only.

  LOGICAL, SAVE :: LVGVAPPL=.FALSE.
...
!**********************************************************************
!
!> Reads all variables related to the orbital magnetization
!> from the INCAR file
!
!**********************************************************************
  ...
  SUBROUTINE ORBITALMAG_READER(IU0, IU5, IU6, NIONS)
      ...
      USE base
      USE reader_tags
      USE tutor, ONLY: vtutor, isWarning, LBONEandLZORA, noVGVapplied
...
! read in at which atom we place a magnetic moment
      MAGATOM=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'MAGATOM', MAGATOM, IERR, WRITEXMLINCAR)
      MAGATOM=MAX(0,MIN(NIONS,MAGATOM))

! read in at which atom we place a magnetic moment
      JATOM=MAGATOM
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'JATOM', JATOM, IERR, WRITEXMLINCAR)
      JATOM=MAX(0,MIN(NIONS,JATOM))
 
! magnitude of magnetic moment
      MAGDIPOL=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'MAGDIPOL', MAGDIPOL, 3, IERR, WRITEXMLINCAR)

      IF (MAGATOM>0) ORBITALMAG=.TRUE.
! magnitude of magnetic moment
      AVECCONST=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'AVECCONST', AVECCONST, 3, IERR, WRITEXMLINCAR)

! read in whether ORBITALMAG is set or not
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'ORBITALMAG', ORBITALMAG, IERR, WRITEXMLINCAR)

! read in whether LCHIMAG is set or not
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LCHIMAG', LCHIMAG, IERR, WRITEXMLINCAR)

! magnitude of dq
      DQ=1.E-3_q
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'DQ', DQ, IERR, WRITEXMLINCAR)

! stencil used to compute chi_bare
      ICHIBARE=1
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'ICHIBARE', ICHIBARE, IERR, WRITEXMLINCAR)
      IF (ICHIBARE<1.OR.ICHIBARE>3) ICHIBARE=1

! read in whether LWRTCUR is set or not
      LWRTCUR=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LWRTCUR', LWRTCUR, IERR, WRITEXMLINCAR)

! read in whether LNMR_SYM_RED is set or not
      LNMR_SYM_RED=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LNMR_SYM_RED', LNMR_SYM_RED, IERR, WRITEXMLINCAR)

! read in whether LZORA is set or not
      LZORA=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LZORA', LZORA, IERR, WRITEXMLINCAR)

! read in whether LBONE is set or not
      LBONE=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LBONE', LBONE, IERR, WRITEXMLINCAR)

! let LBONE take precedence over LZORA
      IF (LBONE.AND.LZORA) THEN
         LZORA=.FALSE.
! and warn the user ...
         CALL vtutor%write(isWarning, LBONEandLZORA)
      ENDIF

! read in whether LMAGBLOCH is set or not
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LMAGBLOCH', LMAGBLOCH, IERR, WRITEXMLINCAR)

! read in whether LGAUGE is set or not
      LGAUGE=.TRUE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LGAUGE', LGAUGE, IERR, WRITEXMLINCAR)

! read in whether LBFCONST is set or not
      LBFCONST=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LBFCONST', LBFCONST, IERR, WRITEXMLINCAR)

! magnitude of magnetic moment
      BCONST=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'BCONST', BCONST, 3, IERR, WRITEXMLINCAR)

! nuclear independent calculation
        NUCIND=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'NUCIND', NUCIND, IERR, WRITEXMLINCAR)

! read in whether LNICSALL is set or not
      LNICSALL=.TRUE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LNICSALL', LNICSALL, IERR, WRITEXMLINCAR)

!read coordinates of NICS calculation (converse approach)
 
      MAGPOS=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'MAGPOS', MAGPOS, 3, IERR, WRITEXMLINCAR)

! read in whether LLRAUG is set or not
      LLRAUG=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LLRAUG', LLRAUG, IERR, WRITEXMLINCAR)

! read in whether LVGVCALC is set or not
      LVGVCALC=.TRUE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LVGVCALC', LVGVCALC, IERR, WRITEXMLINCAR)

! read in whether LVGVAPPL is set or not
      LVGVAPPL=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LVGVAPPL', LVGVAPPL, IERR, WRITEXMLINCAR)

! let LVGVAPPL force LVGVCALC
      IF ((.NOT.LVGVCALC).AND.(LVGVAPPL)) THEN
         LVGVCALC=.TRUE.
! and warn the user ...
         CALL vtutor%write(isWarning, NoVGVapplied) ! read as: NoVGV and VGVapplied
      ENDIF
...
!**********************************************************************
!
!> Writes the Fock and exchange correlation related
...
!**********************************************************************
    ...
    SUBROUTINE WRITE_ORBITALMAG(IU6)
      ...
      IF (IU6>=0 .AND. .NOT. ORBITALMAG) THEN
         WRITE(IU6,100) ORBITALMAG, LCHIMAG, DQ, LLRAUG, LBONE, LVGVCALC, LVGVAPPL
      ENDIF
  ...
  100 FORMAT(' Orbital magnetization related:'  / &
             ...
             '   LBONE     =',L6,   '  B-component reconstruction in AE one-centre'/ &
             '   LVGVCALC  =',L6,   '  calculate vGv susceptibility'/ &
             '   LVGVAPPL  =',L6,   '  apply vGv susceptibility instead of pGv for G=0'/ &
             )
...
!
!> Allocates and deallocates orbital magnetization dependent quantities
!
!***********************************************************************************
...
!> ~~~
!> comment second terms stem from
!> ~~~
!> <phi^AE_j |nabla r Y_lm(r) r^L |phi^AE_i> =
...
!
      ...
      &                  )
                     ...
                     IF (LM2 <=0 .OR. LM2 > SIZE(PP%JPAW,3)) THEN
                        CALL vtutor%bug("internal error in CALC_JPAW: index 3 into JPAR exceeds bounds", "nmr.F", 684)
                     ENDIF
                     IF (LM0+M0-1 <=0 .OR. LM0+M0-1 > SIZE(PP%JPAW,1)) THEN
                        CALL vtutor%bug("internal error in CALC_JPAW: index 1 into JPAR exceeds bounds", "nmr.F", 687)
                     ENDIF
                     IF (LM1+M1-1 <=0 .OR. LM1+M1-1 > SIZE(PP%JPAW,2)) THEN
                        CALL vtutor%bug("internal error in CALC_JPAW: index 2 into JPAR exceeds bounds", "nmr.F", 690)
                     ENDIF

                     IF (LM3+M3-1 <=0 .OR. LM3+M3-1 > SIZE(YLM_X_YLM,1)) THEN
                        CALL vtutor%bug("internal error in CALC_JPAW: index 1 into YLM_X_YLM exceeds " &
                           // "bounds", "nmr.F", 695)
                     ENDIF
                     IF (L1*L1+M1 <=0 .OR. L1*L1+M1 > SIZE(YLM_X_YLM,2)) THEN
                        CALL vtutor%bug("internal error in CALC_JPAW: index 2 into YLM_X_YLM exceeds " &
                           // "bounds", "nmr.F", 699)
                     ENDIF
                     IF (LM3+M3-1 <=0 .OR. LM3+M3-1 > SIZE(YLM_NABLA_YLM,1)) THEN
                        CALL vtutor%bug("internal error in CALC_JPAW: index 1 into YLM_NABLA_YLM " &
                           // "exceeds bounds", "nmr.F", 703)
                     ENDIF
                     IF (L1*L1+M1 <=0 .OR. L1*L1+M1 > SIZE(YLM_NABLA_YLM,2)) THEN
                        CALL vtutor%bug("internal error in CALC_JPAW: index 2 into YLM_NABLA_YLM " &
                           // "exceeds bounds", "nmr.F", 707)
                     ENDIF
...
!
!> Calculates the vector potential in reciprocal space on both the
...
!!    
      ...
      IF (.NOT. ASSOCIATED(AVEC) .OR. .NOT. ASSOCIATED(AVTOT)) THEN
         CALL vtutor%bug("internal error in VECTORPOT: AVEC or AVTOT not associated", "nmr.F", 788)
      ENDIF
...
!! 
!$ACC UPDATE DEVICE(AVEC) IF_PRESENT IF(OFFLOAD_ON)
...
! 4 grid points around the step function are usually a good choice
      ...
      IF (.NOT.ORBITALMAG) RETURN
      IF (.NOT. ASSOCIATED(AVEC) .OR. .NOT. ASSOCIATED(AVTOT)) THEN
         CALL vtutor%bug("internal error in VECTORPOT: AVEC or AVTOT not associated", "nmr.F", 1039)
      ENDIF
...
!
!> @details @ref openacc :
!> On entering this subroutine OpenACC execution is switched off.
!> On exit the OpenACC execution mode reverts to its original status,
!> and when moffload_struct_def::offload_on = .TRUE. the array
!> CDIJ is updated on the device (if present).
!
!***********************************************************************************
...
!! 
!$ACC UPDATE DEVICE(CDIJ) IF_PRESENT IF(OFFLOAD_ON)
         RETURN
      ENDIF
...
! it is not so bad to have the radial integration here
      ...
      &                     ) * FACT / PP%R%R(K)**2
                        ...
                        IF (LM0+M0-1 <=0 .OR. LM0+M0-1 > SIZE(AGRAD,1)) THEN
                           CALL vtutor%bug("internal error in CALC_D_MAGATOM: index 1 into AGRAD " &
                              // "exceeds bounds", "nmr.F", 1865)
                        ENDIF
                        IF (LM1+M1-1 <=0 .OR. LM1+M1-1 > SIZE(AGRAD,2)) THEN
                           CALL vtutor%bug("internal error in CALC_D_MAGATOM: index 2 into AGRAD " &
                              // "exceeds bounds", "nmr.F", 1869)
                        ENDIF
...
!
!> One centre term for correction of (1._q,0._q)-center augmentation via moment
!> ~~~
         ...
         DO M0=1,2*L0+1
            ...
            DO M2=1,2*L2+1
               ...
               IF (LM2_IND <=0 .OR. LM2_IND > SIZE(JPAWRAD,2)) THEN
                  CALL vtutor%bug("internal error in SET_DD_BFCONST: index 2 into JPAWRAD exceeds " &
                     // "bounds", "nmr.F", 2184)
               ENDIF
            ...
            DO LM2_IND=1,LM2MAX
               DO I=1,3
                  ...
                  IF (LM2_IND <1 .OR. LM2_IND > SIZE(JPAWRAD,2)) THEN
                     CALL vtutor%bug("internal error SET_DD_BFCONST " // str(LM2) // " " // &
                        str(SIZE(JPAWRAD,2)), "nmr.F", 2218)
                  ENDIF
...
!****************************** CALC_MU_MAGATOM ************************************
!
!> Calculates (1._q,0._q)-centre magnetic moment on atom where external moment is placed.
!>
            ...
            DO M3=1,2*L3+1
               ...
               DO IC1=ISTART1,IEND1-1
                  ...
                  IF (LM2 <=0 .OR. LM2 > SIZE(JPAWRAD,2)) THEN
                     CALL vtutor%bug("internal error in CALC_MU_MAGATOM: index 2 into JPAWRAD " &
                        // "exceeds bounds", "nmr.F", 2540)
                  ENDIF
                  LM2MAX=MAX(LM2, LM2MAX)
                  L2MAX=MAX(L2, L2MAX)
...
!****************************** CALC_B_MAGATOM *************************************
!
!> Calculates (1._q,0._q)-centre contribution to magnetic field at atomic nuclei.
!>
         ...
         DO M1=1,2*L1+1
            ...
            DO M3=1,2*L3+1
               ...
               DO IC1=ISTART1,IEND1-1
                  ...
                  L2  =JL(IC1)
                  IF (LM2 <=0 .OR. LM2 > SIZE(JPAWRAD,2)) THEN
                     CALL vtutor%bug("internal error in CALC_B_MAGATOM: index 2 into JPAWRAD exceeds " &
                        // "bounds", "nmr.F", 3362)
                  ENDIF
                  LM2MAX=MAX(LM2, LM2MAX)
                  L2MAX=MAX(L2, L2MAX)
...
!
!> Evaluates total moment using a summation over states
...
!GIL 24 01 11      COMPLEX(q) :: MU_PARA_MIXED(3), MU_DIA_MIXED(3)
      ...
      IF (W%WDES%COMM_KINTER%NCPU.NE.1) THEN
         CALL vtutor%error("BLOCH_CURRENT: KPAR>1 not implemented, sorry.")
      END IF
...
!GIL 24 01 11                     CALL M_stop('VASP aborting ...'); stop
!GIL 24 01 11                  ENDIF
...
!
!> Calculates  the first order change of the
...
!
!> Calculates the augmentation contribution to
...
!*********************************** CALC_VIJ **************************************
!
!> Calculates (1._q,0._q)-centre integrals of mixed contributions the magnetic moment on
...
! perferable maybe to use y d/dr [r*psi(r)] = r * [d/dr psi(r) + psi(r)/r]
         ...
         DO M0=1,2*L0+1
            ...
            IF ((L1*L1+M1) <0 .OR. (L1*L1+M1) > SIZE(YLM_MxR_YLM,1)) THEN
               CALL vtutor%bug("internal error CALC_VIJ_ONECTR " // str(LM1+M1-1) // " " // &
                  str(SIZE(YLM_MxR_YLM,1)), "nmr.F", 4426)
            ENDIF
            IF ((L0*L0+M0) <0 .OR. (L0*L0+M0) > SIZE(YLM_MxR_YLM,2)) THEN
               CALL vtutor%bug("internal error CALC_VIJ_ONECTR " // str(LM0+M0-1) // " " // &
                  str(SIZE(YLM_MxR_YLM,2)), "nmr.F", 4430)
            ENDIF
            IF ((LM1+M1-1) <0 .OR. (LM1+M1-1) > SIZE(VIJ_DIA,1)) THEN
               CALL vtutor%bug("internal error CALC_VIJ_ONECTR " // str(LM1+M1-1) // " " // &
                  str(SIZE(VIJ_DIA,1)), "nmr.F", 4434)
            ENDIF
            IF ((LM0+M0-1) <0 .OR. (LM0+M0-1) > SIZE(VIJ_DIA,2)) THEN
               CALL vtutor%bug("internal error CALC_VIJ_ONECTR " // str(LM0+M0-1) // " " // &
                  str(SIZE(VIJ_DIA,2)), "nmr.F", 4438)
            ENDIF
         ...
         DO LRHO=0,2*LMAX
            ...
            DO MRHO=1,2*LRHO+1
               IF (LMRHO+MRHO-1>SIZE(YLM_MxR_YLM,2)) THEN
                  CALL vtutor%bug("internal error in CALC_VIJ_ONECTR: " // str(LMRHO+MRHO-1) // " " &
                     // str(LMRHO) // " " // str(MRHO) // " " // str(SIZE(YLM_MxR_YLM)), "nmr.F", 4509)
               ENDIF
...
!  local
      ...
      IF (LGAUGE) THEN
         IF (NONLR_S%LREAL) THEN
            ...
            ENDIF
         ELSE
            CALL vtutor%error("SETDIJ_AVEC: ERROR: when LGAUGE=.TRUE. you must set LREAL=.TRUE. as &
               &well, stopping ...")
         ENDIF
      ENDIF
# 4590
...
!
!> Converts shift/shielding tensor to csa parameters
!
...
!        WRITE (0,*) 'shielding'
      ELSE 
         CALL vtutor%error("ERROR IN DAIGSHIFT INPUT, STOP")
      ENDIF
...
!         AA(J,I)=0.5_q*(A(J,I)-A(I,J))
      ...
      IF (IFAIL.NE.0) THEN
         CALL vtutor%bug("INTERNAL ERROR DIAGSHIFT, DSYEV, STOP", "nmr.F", 4672)
      ENDIF
      ...
      ELSEIF ((EIG(3).GE.EIG(1)).AND.(EIG(3).GE.EIG(1))) THEN
        ...
        ENDIF
      ELSE
        CALL vtutor%bug("INTERNAL ERROR DIAGSHIFT, STOP", "nmr.F", 4703)
      ENDIF
      ...
      ELSEIF ((MODUS.EQ.'SHIEL').OR.(MODUS.EQ.'shiel')) THEN
        ...
        ENDIF
      ELSE
        CALL vtutor%bug("INTERNAL ERROR DIAGSHIFT, STOP", "nmr.F", 4726)
      ENDIF
