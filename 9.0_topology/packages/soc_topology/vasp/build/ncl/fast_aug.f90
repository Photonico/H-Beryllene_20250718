...
!> augmentation of pseudo charge densities on a plane wave grid
!> (the coarse FFT grid GRID)
!>
!> The augmentation is 1._q in real space and the routines in this
!> module strongly rely on nonlr.F and mimic the functionality of us.F
!
! written by gK
!
!***********************************************************************
...
!
!> Sets up the arrays required to perform a fast augmentation
!> using the non-local pseudopotential routines (in real space)
!>
!> The final FAST_AUG structure is a non local pseudopotential
!> descriptor that can be handled by all routines in nonlr.F
!>
...
!
    IF ( WDES%LSPIRAL) THEN
       CALL vtutor%error("SETUP_FASTAUG: spin spirals are presently not supported")
    ENDIF
...
! (testing only)
          ...
          ELSE
! fallback: PRECFOCK = Medium / Single
...
!  after this has been carefully evaluated
       IF (LMAX_FOCKAE_TYPE_STORE(NTP) /= LMAX_FOCKAE_TYPE) THEN
          CALL vtutor%bug("internal error in SETUP_FASTAUG: LMAX_FOCKAE_TYPE incorrect " // str(NT) &
             // " " // str(NTP) // " " // str(LMAX_FOCKAE_TYPE_STORE(NTP)) // " " // str(LMAX_FOCKAE_TYPE), "fast_aug.F", 322)
       ENDIF
...
!
!> Copies the structure FASTAUG and reinitialize those elements
!> that might have changed when a super cell is used
!
!***********************************************************************
...
!   FAST_AUG_NEW=FAST_AUG
    CALL NONLR_ASSIGN(FAST_AUG_NEW,FAST_AUG)
    NULLIFY(FAST_AUG_NEW%NLIMAX,FAST_AUG_NEW%NLI,FAST_AUG_NEW%CRREXP,FAST_AUG_NEW%LMBASE,FAST_AUG_NEW%NLIBASE)
...
! determine global storage index for this ion
       ...
       NTP=T_INFO%ITYP(NI)
       IF (NTP /= WDES%NT_GLOBAL(NT)) THEN
          CALL vtutor%bug("internal error in FASTAUG_SUPER: WDES%NT_GLOBAL inconsistent with NTP "&
              // str(WDES%NT_GLOBAL(NT)) // " " // str(NIS) // " " // str(NI) // " " // str(NTP), "fast_aug.F", 465)
       ENDIF
       IF (LMAX_FOCKAE>=0 ) THEN
          LMAX_FOCKAE_TYPE=MIN(FAST_AUG_NEW%LMAX(NTP), LMAX_FOCKAE)
       ELSE
...
!******************** SUBROUTINE DEALLOC_FASTAUG ***********************
!
!> Compatible with #SETUP_FASTAUG, where FAST_AUG\%BETA%LPS is allocated
!> (so deallocation of FAST_AUG\%BETA\%LPS here is safe)
!>
!> @note Should *not* be called to deallocate a structure that was
!        set up by #FASTAUG_SUPER
!
!***********************************************************************
...
!   FAST_AUG2=FAST_AUG1
    CALL NONLR_ASSIGN(FAST_AUG2,FAST_AUG1)
    NULLIFY(FAST_AUG2%NLIMAX,FAST_AUG2%NLI,FAST_AUG2%RPROJ,FAST_AUG2%CRREXP,FAST_AUG2%LMBASE,FAST_AUG2%NLIBASE)
...
!
!> Sets up the wave_struct_def::wavedes1 descriptor AUG_DES
!>
!> In adddition to a non local pseudopotential descriptor the non local
!> pseudopotential routines require also a wave_struct_def::wavedes1
!> descriptor for the layout of the site occupancy matrix
!> (usually the wavefunction character)
!>
!> This descriptor AUG_DES describes how the moments are stored and
!> arranged
!>
!> It must be compatible to FAST_AUG and is for most of the entries
!> only a copy of FAST_AUG
!>
!> @details @ref openmp :
!> In addition this subroutine sets AUG_DES\%LMBASE (wavedes1::lmbase)
!> This arrays allows us to contract nested loops over
!> \"types\" + \"ions-of-typ\" to single loops over \"all ions\".
!> In the OpenMP version this is used in many routines
!> (see for instance augfast::calc_dllmm_trans).
!
!***********************************************************************
...
! needs to be reset
    ...
    IF (SIZE(AUG_DES%LMMAX) < AUG_DES%NTYP) THEN
       CALL vtutor%bug("internal ERROR 1 in SETUP_AUG_DES: size insufficient " // &
          str(SIZE(AUG_DES%LMMAX)) // " " // str(AUG_DES%NTYP), "fast_aug.F", 600)
    ENDIF
...
100    NIS = NIS+FAST_AUG%NITYP(NT)
    ...
    IF (IONS_LOCAL /= AUG_DES%NIONS) THEN
       CALL vtutor%bug("internal ERROR 2 in SETUP_AUG_DES: local ions do not match " // &
          str(IONS_LOCAL) // " " // str(AUG_DES%NIONS), "fast_aug.F", 640)
    ENDIF
    IF (NT_LOCAL /= AUG_DES%NTYP) THEN
       CALL vtutor%bug("internal ERROR 3 in SETUP_AUG_DES: local types do not match " // &
          str(NT_LOCAL) // " " // str(AUG_DES%NTYP), "fast_aug.F", 644)
    ENDIF
...
! total number of entries equals FAST_AUG
    ...
    IF (NPRO/= AUG_DES%NPRO_TOT) THEN
       MSG = 'internal ERROR 1 in SETUP_AUG_DES: NPRO does not match ' // str(NPRO) // ' ' // str(AUG_DES%NPRO_TOT)

       MSG = MSG // '\nNTYP' // str(COMM%NODE_ME) // ' ' // str(AUG_DES%NTYP) // ' ' // str(AUG_DES%NPRO) // &
          '\nNITYP' // str(COMM%NODE_ME) // ' ' // str(AUG_DES%NITYP) // '\nLMMAX' // str(COMM%NODE_ME) // &
          ' ' // str(AUG_DES%LMMAX)

       CALL vtutor%bug(MSG, "fast_aug.F", 662)
    ENDIF
...
! LMBASE(NI)+1.
! The array AUG_DES%LMBASE is allocated with dimension AUG_DES%NIONS+1
! so that we may set its first element even if AUG_DES%NIONS=0.
    NULLIFY(AUG_DES%LMBASE); ALLOCATE(AUG_DES%LMBASE(AUG_DES%NIONS+1))
    AUG_DES%LMBASE(1)=0
    DO NI=1,AUG_DES%NIONS
       NT=AUG_DES%ITYP(NI)
       AUG_DES%LMBASE(NI+1)=AUG_DES%LMBASE(NI)+AUG_DES%LMMAX(NT)
    ENDDO
  ...
  SUBROUTINE DEALLOC_AUG_DES(AUG_DES)
    ...
    DEALLOCATE(AUG_DES%LMMAX,AUG_DES%LMBASE)
    NULLIFY(AUG_DES%LMMAX,AUG_DES%LMBASE)

    DEALLOCATE(AUG_DES%NPRO_POS)
    NULLIFY(AUG_DES%NPRO_POS)
...
!*******************************************************************
!
!> Sets up an array of orthogonal functions that can be used to
!> restore the AE charge density on the plane wave grid
!> accurately
!>
!> VASP usually restores all moments (monopole, dipole, quadrupole,
!> etc. on the plane wave grid).
!>
!> Sometimes this is not sufficient (GW, RPA correlation) and (1._q,0._q)
!> would like to restore the AE charge density accurately
...
! QQ=0.0_q; A=0.0_q ! Breaks bulk_CoO_wurzite_SOC_G0W0_sym (gnu), entire arrays used uninitialized in AUG_SETQ_NTH.
...
!
    ...
    DO L=0,LMAX_BESSEL
       IF (QMAX>0) THEN
          CALL vtutor%bug("internal error SET_BESSEL_AUG: combination of LMAX_BESSEL and QMAX is not " // &
             "allowed " // str(LMAX_BESSEL) // " " // str(QMAX), "fast_aug.F", 829)
       ENDIF
...
!*******************************************************************
!
!> Finds a set of (1._q,0._q) or two {q_i} and coefficients {A_i} such that
!> ~~~
...
! gaussian integration
    ...
    IF (NBESSEL>2) THEN
       CALL vtutor%bug("internal error in AUG_SETQ_NTH: presently only two functions are supported", "fast_aug.F", 937)
    ENDIF
!-----------------------------------------------------------------------
...
!*******************************************************************
!
!> Calculates the difference between the AE and PS charge and
!> develops this difference into a set of Besselfunctions
!>
...
! screwed if we do not have (0._q,0._q) charge
            IF ( ABS(RES) > 1E-4 ) THEN
               CALL vtutor%bug("internal error RAD_QPAW_FOCK: QPAW is incorrect " // str(CH1) // " " &
                  // str(CH2) // " QPAW= " // str(RES) // " int= " // str(QPAW(CH1,CH2,LMAIN)), "fast_aug.F", 1121)
            ENDIF
...
!
!> Develops a function into a set of spherical besselfunction
!
...
!*******************************************************************
!
!> Calculates the transformation matrix that is required to go
!> from the occupancies RHO(lm, l'm') to
...
!> and C(LM,ll',mm') are the Clebsch Gordan coefficients
!>
...
!*******************************************************************
!
!> Sets up the (1._q,0._q)-center handle that allows to retrieve selected
...
! due to sum rules only LMIN to LMAX in steps of 2 are allowed
         ...
         DO MMAIN=1,LMAIN*2+1
            ...
            IF (ENTRIES_FOR_TYPE > SIZE(H%NR_OF_CG,1)) THEN
               CALL vtutor%bug("internal error in SET_UP_ONE_CENTER_H: SIZE(H%NR_OF_CG) not sufficient", "fast_aug.F", 1536)
            ENDIF
               ...
               DO  IC=ISTART,IEND-1
                  IF (JS(IC) == LMMAIN) THEN
                     ...
                     IF (INDEX_INTO_CG > NALLOC) THEN
                        CALL vtutor%bug("internal error in SET_UP_ONE_CENTER_H: dimension of CG and " &
                           // "INDEX not sufficient", "fast_aug.F", 1562)
                     ENDIF
...
! after CG transformation
                     IF (CH1/=CH2) THEN
                        ...
                        INDEX_INTO_CG=INDEX_INTO_CG+1
                        IF (INDEX_INTO_CG > NALLOC) THEN
                           CALL vtutor%bug("internal error in SET_UP_ONE_CENTER_H: dimension of CG " &
                              // "and INDEX not sufficient", "fast_aug.F", 1576)
                        ENDIF
! store CG coefficient
                        CG(INDEX_INTO_CG)=YLM3(IC)
! convert index LM+M-1,LMP+MP-1 into (1._q,0._q) dim index
...
!*******************************************************************
!
!> Deallocate (1._q,0._q)-center handle
!
!*******************************************************************
...
!***********************************************************************
!
!> Applies the phase factor e^(i q R_i) to a linear CG-transformed
!> occupancy matrix.
!>
!> This is required since the four orbital integrals
!> and the occupancies matrices might be set off by a reciprocal
!> lattice vector e^(i G r) and hence a shift of
!> e^(i G R_ion) is missing in CRHO
!
...
!*******************************************************************
!
!> Applies a (1._q,0._q)-center potential to a set of (1._q,0._q)-center occupancies
!
!*******************************************************************
...
!
!> Calculates the net LM moment of the soft augmentation charges
!> (compensation charges in PAW language) RHO(LM) at all sites
!>
!> This routine is similar to the routine paw::CALC_RHOLM (in paw_base.F)
!> but tuned for efficiency by using the stored transformation matrix
!> ~~~
!> RHO(LM)= sum TRANS_MATRIX(lm,l'm',LM,type) RHO(lm,l'm')
!> ~~~
!
...
!
!> Is a combination of #DEPSUM_TWO_BANDS and #CALC_RHOLM_TRANS
!>
!> It returns the charge from the up and down channel seperately
!
!  For efficiency reasons (1._q,0._q) should use this subroutine
!
!***********************************************************************
...
!
!> Calculates the total charge from two bands
!>
!> It differs from the #DEPSUM_TWO_BANDS_RHOLM in the way spinors are
!> handled: this routine returns only the spin-trace (total charge)
!>
!> @details @ref openmp :
!> Under OpenMP the nested loop over \"types\" + \"ions-of-typ\"
!> is contracted to a single loop over \"all ions\"
!> (using AUG_DES\%LMBASE), and is distributed over the available
...
!! USE moffload_struct_def
      ...
      TYPE (wavedes1) WDES, AUG_DES
      COMPLEX(q) :: CPROJ1(:), CPROJ2(:), CRHOLM(:)
      REAL(q), CONTIGUOUS :: TRANS_MATRIX(:, :, :, :)
      REAL(q) :: FACT
      LOGICAL LOVERL
...
# 1978
      ...
      CALL DEPSUM_TWO_BANDS_RHOLM_TRACE_(CPROJ1, CPROJ2, WDES, AUG_DES, TRANS_MATRIX, CRHOLM, FACT, LOVERL)
    ...
    SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_( CPROJ1, CPROJ2, WDES, AUG_DES, &
         TRANS_MATRIX, CRHOLM, FACT, LOVERL)
      USE prec
      USE wave
      IMPLICIT NONE
...
# 2059
      ...
      RETURN
    END SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_

# 2123


    SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_MU( &
       W1, CPROJ, WDES, AUG_DES, TRANS_MATRIX, CRHOLM, FACT, LOVERL, NSIM)
...
# 2160
      ...
      CALL DEPSUM_TWO_BANDS_RHOLM_TRACE_MU_(W1, CPROJ, WDES, AUG_DES, TRANS_MATRIX, CRHOLM, FACT, LOVERL, NSIM)

      

      RETURN
    END SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_MU


    SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_MU_( &
       W1, CPROJ, WDES, AUG_DES, TRANS_MATRIX, CRHOLM, FACT, LOVERL, NSIM)
      USE prec
      USE wave
      IMPLICIT NONE
...
! local
      ...
      DO N=1,NSIM
         CALL DEPSUM_TWO_BANDS_RHOLM_TRACE(W1(N)%CPROJ(:), CPROJ(:), WDES, AUG_DES, &
            TRANS_MATRIX, CRHOLM(:,N), FACT, LOVERL)
      ENDDO

      

      RETURN
    END SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_MU_

# 2267


    SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_MU2( &
       CPROJ1, CPROJ2, WDES, AUG_DES, TRANS_MATRIX, CRHOLM, FACT, LOVERL, NSIM)
...
# 2306
      ...
      DO N=1,NSIM
         CALL DEPSUM_TWO_BANDS_RHOLM_TRACE_(CPROJ1(:,N), CPROJ2(:,N), WDES, AUG_DES, TRANS_MATRIX, CRHOLM(:,N), FACT, LOVERL)
      ENDDO

      

      RETURN
    END SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_TRACE_MU2

# 2379


    SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_NO_CONJG_MU( &
       W1, CPROJ, WDES, AUG_DES, TRANS_MATRIX, CRHOLM, FACT, LOVERL, NSIM)
...
! local
      INTEGER :: N,ISPINOR,NI,NT,LMMAXC,LMBASE,NAUG,L,LP,LM
      COMPLEX(q) :: CTMP
...
!$ACC KERNELS PRESENT(CRHOLM) 
      CRHOLM=0
!$ACC END KERNELS

      spinor: DO ISPINOR =0,WDES%NRSPINORS-1
!$ACC PARALLEL LOOP COLLAPSE(2) GANG PRIVATE(LMBASE,NT,LMMAXC,NAUG) &
!$ACC PRESENT(WDES,W1,CPROJ,TRANS_MATRIX,CRHOLM,AUG_DES) 
         strip: DO N=1,NSIM
            ion: DO NI=1,WDES%NIONS
               ...
               NAUG  =AUG_DES%LMBASE(NI)
!$ACC LOOP VECTOR PRIVATE(CTMP)
               DO LM=1,AUG_DES%LMMAX(NT)
                  CTMP=0
!!!$ACC LOOP VECTOR COLLAPSE(2) REDUCTION(+:CTMP)
                  DO L=1,LMMAXC
                     DO LP=1,LMMAXC
                        CTMP=CTMP+W1(N)%CPROJ(LMBASE+L)*CPROJ(LMBASE+LP)*TRANS_MATRIX(LP,L,LM,NT)
                     ENDDO
                  ENDDO

!!!$ACC ATOMIC UPDATE
                  CRHOLM(NAUG+LM,N)=CRHOLM(NAUG+LM,N)+CTMP*FACT
# 2444
      ...
      RETURN
    END SUBROUTINE DEPSUM_TWO_BANDS_RHOLM_NO_CONJG_MU
...
! local
      ...
      typ:  DO NT=1,WDES%NTYP
         ...
         IF (LMMAX > AUG_DES%LMMAX(NT)) THEN
            CALL vtutor%bug("internal error in DEPSUM_TWO_BANDS_RHOLM_TRACE_NOAE: LMMAX too large " &
               // str(LMMAX) // " " // str(AUG_DES%LMMAX(NT)), "fast_aug.F", 2492)
         ENDIF
...
!
!> Calculates the occupancy matrix for (1._q,0._q) ion
!>
!> The input array is a vector
!> ~~~
...
# 2551
      ...
      CALL DEPSUM_VECTOR_( GO_R_PROJ, GU_R_PROJ, GWORK, NP, TRANS_MATRIX, LMMAXC, LMBASE, NLM_LMMAX )
    ...
    SUBROUTINE DEPSUM_VECTOR_( GO_R_PROJ, GU_R_PROJ, GWORK, NP, TRANS_MATRIX, LMMAXC, LMBASE, NLM_LMMAX )
      USE prec
      USE dfast
...
!$OMP PARALLEL DO SHARED(NBLK,NP,LMMAXC,GO_R_PROJ,GU_R_PROJ,NLM_LMMAX,GWORK) &
!$OMP PRIVATE(GO_GU_PROJ,L,LP,LM,NR,NRP)
      DO NR=1, NP, NBLK
...
!$OMP END PARALLEL DO
    ...
    END SUBROUTINE DEPSUM_VECTOR_

# 2648



!************************ SUBROUTINE DEPSUM_VECTOR_REAL **************
!
!> Calculates the occupancy matrix for (1._q,0._q) ion
!>
!> The only difference w.r.t. #DEPSUM_VECTOR is the fact that the
!> input arrays
!> ~~~
!>   G(r', N alpha) G*+(r', N alpha') T_alpha alpha'^NLM
!> ~~~
!> are defined REAL instead of COMPLEX(q)
!>
!> Performance is much improved by blocking over the first index
!
!***********************************************************************

    SUBROUTINE DEPSUM_VECTOR_REAL( GO_R_PROJ, GU_R_PROJ, GWORK, NP, TRANS_MATRIX, LMMAXC, LMBASE, NLM_LMMAX )
...
# 2690
      ...
      CALL DEPSUM_VECTOR_REAL_( GO_R_PROJ, GU_R_PROJ, GWORK, NP, TRANS_MATRIX, LMMAXC, LMBASE, NLM_LMMAX )

      

      RETURN
    END SUBROUTINE DEPSUM_VECTOR_REAL


    SUBROUTINE DEPSUM_VECTOR_REAL_( GO_R_PROJ, GU_R_PROJ, GWORK, NP, TRANS_MATRIX, LMMAXC, LMBASE, NLM_LMMAX )
      USE prec
      USE dfast
...
! loop over PAW alpha and alpha'
         DO L=1,LMMAXC
            DO LP=1,LMMAXC
               GO_GU_PROJ(1:NRP-NR+1)=REAL(GO_R_PROJ(NR:NRP, L+LMBASE)*CONJG(GU_R_PROJ(NR:NRP, LP+LMBASE)),q)
! loop over charge index NLM
               DO LM=1,NLM_LMMAX
! index build from NI and LM
                  GWORK(NR:NRP,LM)=GWORK(NR:NRP,LM)+ &
...
!$OMP END PARALLEL DO
    ...
    END SUBROUTINE DEPSUM_VECTOR_REAL_
...
!> Calculates the total charge from two bands
!>
!> It differs from #DEPSUM_TWO_BANDS_RHOLM by the fact that the
...
!> Calculates the total charge from two bands
!>
!> It differs from #DEPSUM_TWO_BANDS_RHOLM in the way spinors are handled:
!> this routine is a full spinor version i.e. four components
...
!
!> Is very similar to #DEPSUM_TWO_BANDS_RHOLM_FULL but does not include
!> the additional (1._q,0._q)-center augmentation charges
!>
!> It is required by the local Hartree Fock routines (pwlhf.F)
!
...
! local
      ...
      typ:  DO NT=1,WDES%NTYP
         ...
         LMMAX=(LMAX(NT)+1)*(LMAX(NT)+1)
         IF (LMMAX > AUG_DES%LMMAX(NT)) THEN
            CALL vtutor%bug("internal error in DEPSUM_TWO_BANDS_RHOLM_FULL_NOAE: LMMAX too large "&
                // str(LMMAX) // " " // str(AUG_DES%LMMAX(NT)), "fast_aug.F", 2968)
         ENDIF
...
!
!> Calculates the occupancy matrix for two wavefunctions
!> ~~~
...
!> ~~~
!> As input it requires the projected character of the two wave functions
!>
!> @note Alpha and beta are interchanged as everywhere in VASP
!
!***********************************************************************
...
!
!> Transforms D(LM) to D(lm,l'm'), where D(LM) is defined as
!> ~~~
!> D(LM) = \int V Y(L,M) Q(L)(r) d^3 r
!> ~~~
!> and \int Q(L)(r) r^2 dr is integrating to 1.
!>
!> The result is *added* to CDIJ
!>
!> This routine is similar to the routine paw::CALC_DLLMM (paw_base.F)
!> but tuned for efficiency by using the stored transformation matrix
!> ~~~
...
# 3216
    ...
    SUBROUTINE CALC_DLLMM_TRANS_AND_OVERL_FOCK_MU(WDES, AUG_DES, TRANS_MATRIX, CDLM, LMDIM, CPROJ , WXI, LADD, NSIM)
...
! local variables
      INTEGER :: ISPINOR,N,NI,NT,LMMAXC,NPRO,NAUG,L,LP,LM
      COMPLEX(q) :: CTMP,CDIJ

      

      IF (.NOT.LADD) THEN
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(WXI) 
...
!$ACC PARALLEL LOOP COLLAPSE(2) GANG PRESENT(AUG_DES,TRANS_MATRIX,CDLM,WDES,CPROJ,WXI) &
!$ACC PRIVATE(NT,LMMAXC,NPRO,NAUG,L,LP,CTMP) 
 !$OMP PARALLEL DO COLLAPSE(2) PRIVATE(NT,LMMAXC,NPRO,NAUG,L,CTMP,LP,CDIJ,LM)
         DO N=1,NSIM
            DO NI=1,WDES%NIONS
               NT=WDES%ITYP(NI)
               ...
               NPRO=WDES%LMBASE(NI)+ISPINOR *WDES%NPRO/2
               NAUG=AUG_DES%LMBASE(NI)+ISPINOR*AUG_DES%NPRO

!$ACC LOOP WORKER PRIVATE(CTMP)
               DO L=1,LMMAXC
                  CTMP=0
!$ACC LOOP VECTOR REDUCTION(+:CTMP) PRIVATE(CDIJ)
                  DO LP=1,LMMAXC
                     CDIJ=0
!$ACC LOOP REDUCTION(+:CDIJ)
                     DO LM=1,AUG_DES%LMMAX(NT)
                        CDIJ=CDIJ+TRANS_MATRIX(LP,L,LM,NT)*CDLM(LM+NAUG,N)
                     ENDDO
                     CTMP=CTMP+CDIJ*CPROJ(LP+NPRO)
                  ENDDO
                  WXI(N)%CPROJ(L+NPRO)=WXI(N)%CPROJ(L+NPRO)+CTMP
               ENDDO
 ...
 !$OMP END PARALLEL DO
      ...
      RETURN
    END SUBROUTINE CALC_DLLMM_TRANS_AND_OVERL_FOCK_MU

!
!> Very similar to #CALC_DLLMM_TRANS but does not include the additional
!> (1._q,0._q)-center augmentation charges
!>
!> It is required in the the local Hartree Fock routines (pwlhf.F)
!
...
!$OMP PARALLEL DO PRIVATE(NR,NRP,L,LP,PROJ,LM)
      DO NR=1, NP, NBLK
         NRP=MIN(NR+NBLK-1, NP)
! loop over PAW alpha and alpha'
         DO L=1,LMMAXC
...
!
!> Same as #CALC_DLLMM_VECTOR but with GWORK defined to be REAL
!>
!> @note Although the end results GO_R_PROJ is also strictly real valued that array is declared COMPLEX
!
    SUBROUTINE CALC_DLLMM_VECTOR_REAL( GO_R_PROJ, GI_R_PROJ, GWORK, NP, TRANS_MATRIX, LMMAXC, LMBASE, NLM_LMMAX )
      USE prec
      USE dfast
...
!$OMP END PARALLEL DO
    ...
    END SUBROUTINE CALC_DLLMM_VECTOR_REAL
...
!***********************************************************************
!
!>  Apples the phase factor e^(i q R_i) to a linear CG transformed
!>  occupancy matrix.
!>
!>  This is a copy of #APPLY_PHASE_ONE_CENTER but without an
...
!> Calculates RHOLM
!>
!> This version maps onto DGEMV
!>
!> @note Can be called F77 style (has no explicit interface)
!
...
!$OMP PRIVATE(L,LP) &
!$OMP SHARED(LMMAXC,CPROJ1,CPROJ2,FACT,CTMP)
    DO L=1,LMMAXC
!DIR$ IVDEP
!OCL NOVREC
       DO LP=1,LMMAXC
...
!$OMP SCHEDULE(STATIC) DEFAULT(NONE) &
!$OMP PRIVATE(L,LP,CM) &
!$OMP SHARED(LMMAXC,CPROJ1,CPROJ2,FACT,NI,NIONS,CTMP)
       DO L=1,LMMAXC
!DIR$ IVDEP
!OCL NOVREC
          DO LP=1,LMMAXC
...
!> and contract the (1._q,0._q)-center occupancy matrix
!
!**********************************************************************
...
!$  INTEGER, EXTERNAL :: OMP_GET_NUM_THREADS
!$  INTEGER INDEX_INTO_CG_START
!$  INDEX_INTO_CG_START=INDEX_INTO_CG

!$OMP PARALLEL DO IF (LMMAXC>=OMP_GET_NUM_THREADS()) &
!$OMP SCHEDULE(STATIC) DEFAULT(NONE) &
!$OMP PRIVATE(L,LP,CTMP_,LM) &
!$OMP SHARED(LMMAXC,FACT,CPROJ1,CPROJ2,CTMP,TRANS_MATRIX,LMMAX) &
!$OMP REDUCTION(+:CRHOLM)
    DO L=1,LMMAXC
!DIR$ IVDEP
!OCL NOVREC
       DO LP=1,LMMAXC
...
!$OMP PARALLEL DO IF (ENTRIES_FOR_TYPE>=OMP_GET_NUM_THREADS()) &
!$OMP SCHEDULE(STATIC) DEFAULT(NONE) &
!$OMP PRIVATE(I,J,INDEX_INTO_CG) &
!$OMP SHARED(ENTRIES_FOR_TYPE,INDEX_INTO_CG_START,NR_OF_CG,CRHO_ONE_CENTER,CG,CTMP,INDEX)
    DO I=1,ENTRIES_FOR_TYPE
!$     INDEX_INTO_CG=INDEX_INTO_CG_START
!$     IF (I>1) INDEX_INTO_CG=INDEX_INTO_CG_START+SUM(NR_OF_CG(1:I-1))
       DO J=1,NR_OF_CG(I)
    ...
    ENDDO
!$OMP END PARALLEL DO
!$  INDEX_INTO_CG=INDEX_INTO_CG_START+SUM(NR_OF_CG(1:ENTRIES_FOR_TYPE))
