...
!
!> Performs electronic minimization using the KLI/ LHF or EXX method
!
!**********************************************************************
...
! local l-projected wavefunction characters (not really used here)
  ...
  REAL(q) BTRIAL,RDUM,RMS,RMS_,ORT,TOTEN2,RMS2,RMST, &
       WEIGHT,BETATO,DESUM2,RMSC,RMSP,X,DE2H
...
!PK Look for inspiration in electron_all
      CALL vtutor%error("ELMIN_OEP: KPAR>1 not implemented, sorry.")
  END IF
...
!=======================================================================
        ...
        FBREAK_MEAN=0.0         ! do not apply FBREAK_MEAN in first iteration
...
! consider LRESET more carefully
           CALL LINEAR_RESPONSE_DIIS(GRID,INFO,LATT_CUR,NONLR_S,NONL_S,W1,WXI,W,WDES, &
                ...
                LRESET=NLINEAR==1, IERROR=IERROR, FBREAK_MEAN_IN=FBREAK_MEAN)
           ...
           ELSE IF (IERROR>0) THEN
              CALL vtutor%bug("internal ERROR: LINEAR_RESPONSE_DIIS matrix is zero, required " &
                 // "accuracy not reached", "electron_OEP.F", 636)
           ENDIF
...
! check for "drift" in potential
        IF (ABS(RHO0(GRIDC, CVTOT1(1,1)))>=1E-2) THEN
           CALL vtutor%bug("internal error in electron_OEP: potential shift observed", "electron_OEP.F", 746)
        ENDIF
...
!  simple mixing ... :
              ...
              CALL MIX_SIMPLE(GRIDC,MIX,WDES%NCDIJ, CVTOT,CVTOTL, &
                   N_MIX_PAW, DLM_EXX, DLM_EXX_LAST, LATT_CUR%B, LATT_CUR%OMEGA, DE2H, RMST)
...
!************************ SUBROUTINE XINV   ****************************
!
!> Calculates the inverse of the Lindhard
...
!>   `G= (H(0)-e(0) S(0))^-1` and therefore almost impossible to converge
