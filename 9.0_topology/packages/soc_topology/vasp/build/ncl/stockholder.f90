...
! read relevant stuff from INCAR
      ...
      LCHGFIT=.FALSE.     
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LCHGFIT', LCHGFIT, IERR, WRITEXMLINCAR)

      LGAUSRC=.FALSE.     
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LGAUSRC', LGAUSRC, IERR, WRITEXMLINCAR)

      ENCHG=-1
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'ENCHG', ENCHG, IERR, WRITEXMLINCAR)

! the number of Gaussians that define the "charge-transfer"
! charges for non-scf calculations (default is 0)
      NGAUS=0
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'NGAUS', NGAUS, IERR, WRITEXMLINCAR)

      IF (NGAUS>0) THEN
      ALLOCATE(ZCT(NTYPP*NGAUS),RGAUS(NTYPP*NGAUS))
! "charge transfer" charges for non-scf calculations (default is 0)
      ZCT=0
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'ZCT', ZCT, NGAUS*NTYPP, IERR, WRITEXMLINCAR)

! widths of Gaussian CT charge distributions
      RGAUS=0
        CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'RGAUS', RGAUS, NGAUS*NTYPP, IERR, WRITEXMLINCAR)
      ENDIF

      TAU0=-1    
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'TAU0', TAU0, IERR, WRITEXMLINCAR)
...
! read relevant stuff from INCAR
      CALL OPEN_INCAR_IF_FOUND(IO%IU5, LOPEN)
      LSTOCKHOLDER=.FALSE.     
      CALL PROCESS_INCAR(LOPEN, IO%IU0, IO%IU5, 'LSTOCKHOLDER', LSTOCKHOLDER, IERR, WRITEXMLINCAR)
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 1989)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 2127)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 2321)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 2452)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 2622)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 2767)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 2936)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 3058)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-Pos_Gaus(3,NG))
...
!$ACC ENTER DATA CREATE(WORK,HARFOR_LOCAL) 

      NQ=NGAUS
!$ACC KERNELS PRESENT(HARFOR_LOCAL) 
      HARFOR_LOCAL=0
!$ACC END KERNELS
!=======================================================================
! loop over all types of atoms
...
!$ACC PARALLEL LOOP PRIVATE(N1,NC,N2,N3,GX,GY,GZ,G2) &
!$ACC& PRESENT(GRIDC,LATT_CUR,WORK) 
       DO N=1,GRIDC%RC%NP
        N1= MOD((N-1),GRIDC%RC%NROW) +1
        NC= (N-1)/GRIDC%RC%NROW+1
        N2= GRIDC%RC%I2(NC)
...
!$ACC& PRESENT(GRIDC,T_INFO%POSION,T_INFO%VCA,WORK,HARFOR_LOCAL,CHGGA) 
...
!$ACC ATOMIC UPDATE
       ...
       NIS=NIS+NIADD
      ENDDO typ
!$ACC EXIT DATA COPYOUT(HARFOR_LOCAL) DELETE(WORK) WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!=======================================================================
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 3482)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 3621)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 3831)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 4039)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 4227)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
...
!-----------------------------------------------------------------------
      ...
      IF (GRIDC%RL%I2(NCOL) /= N1P+1 .OR. GRIDC%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("RHOADD: internal ERROR: " // str(GRIDC%RL%I2(NCOL)) // " " // str(N1P+1) &
           // " " // str(GRIDC%RL%I3(NCOL)) // " " // str(N2P+1), "stockholder.F", 4466)
      ENDIF
!OCL SCALAR
      DO N3=N3LOW,N3HI
      X3=(N3*F3-T_INFO%POSION(3,NI))
