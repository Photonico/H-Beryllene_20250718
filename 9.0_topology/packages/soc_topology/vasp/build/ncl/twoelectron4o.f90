...
! furthermore, if LMP2_EMTPY=.TRUE., the Fock energy can not be calculated
    ...
    IF (W%WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL vtutor%error("TWOELECTRON4O_MAIN: KPAR>1 not implemented, sorry.")
    END IF
# 112
          ...
          DO NN1=1,NSTRIP1_ACT
             ...
             DO K2=KSTART,WDES%NKPTS,KSTRIDE
                ...
                ELSE IF (K3==K1) THEN
                   ...
                   ENDDO
                ELSE
                   CALL vtutor%bug("internal error in TWOELECTRON4O_MAIN: the K3_ index array is " &
                      // "corrupted " // str(K1) // " " // str(K3), "twoelectron4o.F", 299)
                ENDIF
             ENDDO
          ENDDO
...
!**********************************************************************
!
!> Determine the kinetic energy of each wavefunction (EKIN) and
...
! local
    ...
    IF (W%WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL vtutor%error("SET_EKIN: KPAR>1 not implemented, sorry.")
    END IF
...
!PK Consider also disabling the KIMAGES code as it is now redundant
       CALL vtutor%error("TWOELECTRON4O_ACC: KPAR>1 not implemented, sorry.")
    END IF
...
! local
    ...
    strip: DO N4=N4LOW,N4UP,NSTRIP_STANDARD_GLOBAL
       ...
       NFLOAT=NFLOAT+8._q*WDES%NB_TOT*NSTRIP_ACT*WA1%WDES1%NRPLWV_RED
...
# 1132
    ...
    IF (ABS(VKPT(1))>TINY .OR. ABS(VKPT(2))>TINY .OR. ABS(VKPT(3))>TINY) THEN
       IF (ABS(MOD(VKPT(1)+100.5_q,1._q)-0.5_q)>TINY .OR. &
           ...
           CALL vtutor%bug("internal error: non integer shift in SETPHASE in twoelectron4o.F " // &
              str(VKPT(:)), "twoelectron4o.F", 1139)
       ENDIF
       LPHASE=.TRUE.
...
# 1202
    ...
    IF (ABS(VKPT(1))>TINY .OR. ABS(VKPT(2))>TINY .OR. ABS(VKPT(3))>TINY) THEN
       ...
       IF (GRID%RL%NFAST==3) THEN
          CD=EXP(CMPLX(0,F3,q))
!$ACC PARALLEL LOOP PRIVATE(C,N,IND) PRESENT(GRID,CPHASE) 
          DO NC=1,GRID%RL%NCOL
             C=EXP(CMPLX(0,F1*(GRID%RL%I2(NC)-1)+F2*(GRID%RL%I3(NC)-1),q))
             IND=(NC-1)*GRID%RL%NROW
             DO N=1,GRID%RL%NROW
                IND=IND+1
                CPHASE(IND)=C
                C=C*CD
       ...
       ELSE
          CD=EXP(CMPLX(0,F1,q))
!$ACC PARALLEL LOOP PRIVATE(C,IND,N) PRESENT(GRID,CPHASE) 
          DO NC=1,GRID%RL%NCOL
             C=EXP(CMPLX(0,F2*(GRID%RL%I2(NC)-1)+F3*(GRID%RL%I3(NC)-1),q))
             IND=(NC-1)*GRID%RL%NROW
             DO N=1,GRID%RL%NROW
...
!
  ...
  SUBROUTINE APPLY_PHASE(GRID, SV, CR , CRP)
    ...
    TYPE (grid_3d)     GRID
    COMPLEX(q) ::  SV(GRID%RL%NP)  ! complex phase factor
    COMPLEX(q) ::  CR(GRID%RL%NP)
    COMPLEX(q) ::  CRP(GRID%RL%NP)
    INTEGER NRSPINORS
! local variables
...
!DIR$ IVDEP
!OCL NOVREL
!$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) PRIVATE(M) SHARED(GRID,CRP,SV,CR)
    DO M=1,GRID%RL%NP
       CRP(M)= SV(M) *CR(M)
    ENDDO
!$OMP END PARALLEL DO
...
!
  ...
  SUBROUTINE APPLY_PHASE_MU(GRID, SV, CR , CRP, NSIM)
...
!! USE moffload_struct_def
    ...
    TYPE (grid_3d)     GRID
    COMPLEX(q) ::  SV(GRID%RL%NP)  ! complex phase factor
    COMPLEX(q) ::  CR(GRID%RL%NP, NSIM)
    COMPLEX(q) ::  CRP(GRID%RL%NP, NSIM)
    INTEGER NRSPINORS
! local variables
    INTEGER ISPINOR, M, MM, N, NSIM

# 1302


    
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(GRID,CRP,SV,CR) 
    DO N=1,NSIM
      DO M=1,GRID%RL%NP
         CRP(M,N)= SV(M) *CR(M,N)
      ENDDO
...
!DIR$ IVDEP
!OCL NOVREL
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) PRIVATE(M) SHARED(GRID,SV,CR)
!$ACC PARALLEL LOOP PRESENT(GRID,CR,SV) 
    DO M=1,GRID%RL%NP
       CR(M)= SV(M) *CR(M)
    ENDDO
!$OMP END PARALLEL DO
  ...
  SUBROUTINE APPLY_PHASE_INPLACE_MU(GRID, SV, CR, NSIM )
...
!DIR$ IVDEP
!OCL NOVREL
 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) PRIVATE(N,M) SHARED(GRID,SV,CR,NSIM)
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(GRID,CR,SV) 
    DO N=1,NSIM
       DO M=1,GRID%RL%NP
          CR(M,N)= SV(M) *CR(M,N)
       ENDDO
 ...
 !$OMP END PARALLEL DO
  ...
  END SUBROUTINE APPLY_PHASE_INPLACE_MU
