...
!***********************************************************************
  ...
  SUBROUTINE CALCULATE_XI_REAL_GWRK( & 
    ...
    USE tutor, ONLY: vtutor, isError, isAlert, ACFDTRnotSupported, FermiFiniteTemp, &
        ...
        chiGG_NOmega, OmegaGridOmegaTL, newArgument, & 
! OEP related
        LPOTOK  
! required for OEP
...
!!!need to figure out which (1._q,0._q) we need in the WORK state
    ...
    COMPLEX(q), POINTER       :: SIGMAO_DIAG(:)      ! diagonal of <i| Sigma_o(tau) | i> of self-energy
    COMPLEX(q), POINTER       :: SIGMAU_DIAG(:)      ! diagonal of <i| Sigma_u(tau) | i> of self-energy
    COMPLEX(q), POINTER, CONTIGUOUS :: SIGMAO_MAT(:,:,:,:) ! matrix <i| Sigma_o(tau) | a> stored distributed
    COMPLEX(q), POINTER, CONTIGUOUS :: SIGMAU_MAT(:,:,:,:) ! matrix <i| Sigma_u(tau) | a> stored distributed
    COMPLEX(q), POINTER, CONTIGUOUS :: CHAM_MAT(:,:,:,:)   ! Hamilton matrix T + V_local + V_x or density matrix
    COMPLEX(q), POINTER, CONTIGUOUS :: GAMMA(:,:,:,:)      ! density matrix for (1._q,0._q) k-point
    COMPLEX(q), POINTER, CONTIGUOUS :: CORR_MAT(:,:,:,:)   ! correlation contribution to density matrix
    COMPLEX(q), POINTER, CONTIGUOUS :: SIGMA_MAT(:,:,:,:)=>NULL()  ! int G_0(w) (\Sigma(w)+H_HF) G_0(w) w dw
    COMPLEX(q), POINTER, CONTIGUOUS :: GO_MAT(:,:,:,:)     ! matrix <i| G_o(tau) | a> stored distributed
...
! check imaginary grid quality
    IF( LUTTINGER_WARD .AND. ( LG0W0 .OR. LGW0 ) ) CALL CHECK_IMAG_GRID_QUALITY( W, IMAG_GRIDS, INFO, IO )
...
! set GAMMA to Hartree-Fock density matrix, and add mean field Green's function to GO_MAT
          IF (NELM_HF==NELMHF) THEN
! dont add mean field part, because so subtraction is 1._q
...
!
! (0._q,0._q)-temperature case computes only Galitskii-Migdal energy
!
