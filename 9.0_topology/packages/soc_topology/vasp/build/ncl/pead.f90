...
!***********************************************************************
!
!> Contains the routines that calculate the derivative
!> \f$ d\psi_k / dk \f$, in accordance with the finite difference approximation
...
!***********************************************************************
      ...
      PUBLIC :: LUSEPEAD,PEAD_REQUEST,LPEAD_SYM_RED, &
                LPEAD_NO_SCF,LPEAD_EFIELD_SWITCHED_ON,LPEAD_ABORT, &
                LPEAD_CALC_EPS,LPEAD_NONZERO_EFIELD, &
                PEAD_DPSI_DK_ALL,PEAD_DPSI_DK_IDIR, &
                PEAD_ACC_CALC_ALL,PEAD_EDOTP,PEAD_EIGENVALUES, &
                PEAD_ACC_ADD,PEAD_ACC_ADD_PW,PEAD_ACC_ADD_CPROJ, &
                PEAD_CALC_OVERLAP,PEAD_WA_ROTATE, &
                LPEAD_RELAX_STRUCTURE,PEAD_ADD_FZ_EFIELD, &
                PEAD_SETUP,PEAD_RESETUP_WDES,WDES_FULL_PEAD, &
                PEAD_ELMIN,PEAD_READER,PEAD_WRITER, &
                PEAD_REINIT_EDWAV,LPEAD_RETURN_Q_CPROJ, &
                LBORN,BORN_CHARGES_PEAD,W_STORE,FAC, &
                PEAD_DPSI_DK_1K
...
!
!> determines whether Q_ij d <p_j| psi> dk  is returned or <p_j| psi> dk
...
!> reduce the set of symmetry operations to those that
!> conserve the finite difference stencil used in the PEAD
      LOGICAL, PRIVATE, SAVE :: LREDUCE_SYM=.FALSE.

!> cutoff criterion for electronic convergence in PEAD_ELMIN
      REAL(q), PRIVATE, SAVE :: EDIFF_PEAD
!
! Local variable set by PEAD_REQUEST
!
!> A call to PEAD_REQUEST *before* the call to PEAD_READER
...
!> stores the initial total ionic dipole moment
!> (when PEAD_POLARIZATION_CALC is called with LINITIALIZE=.TRUE.)
      REAL(q), PRIVATE, SAVE :: IONIC_DIPOLE_INIT(3)=0
...
!
! variable used by PEAD_(RE)STORE_WAVE_ORIG
!
!> stores a copy of the wave functions
      TYPE(wavespin), SAVE :: W_STORE

!
! Local variable used needed for structural relaxation
!
!> for LRELAX_STRUCTURE=.TRUE. the structure will
!> be relaxed in the presence of EFIELD_PEAD
      LOGICAL, PRIVATE, SAVE :: LRELAX_STRUCTURE=.FALSE.

!> stores the force EFIELD_PEAD exerts on the
!> ionic core
      REAL(q), PRIVATE, ALLOCATABLE, SAVE :: FZ_EFIELD_PEAD(:,:)
...
!***********************************************************************
      ...
      USE main_mpi
      USE reader_tags
      USE tutor, ONLY: isError, PEAD_NCOREnot1
...
! local variables
      ...
      LPEAD=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LPEAD', LPEAD, IERR, WRITEXMLINCAR)

      LCALCPOL=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LCALCPOL', LCALCPOL, IERR, WRITEXMLINCAR)

      EFIELD_PEAD=0
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'EFIELD_PEAD', EFIELD_PEAD, 3, IERR, WRITEXMLINCAR)

      LCALCEPS=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LCALCEPS', LCALCEPS, IERR, WRITEXMLINCAR)

      LRELAX_STRUCTURE=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LPEAD_RELAX', LRELAX_STRUCTURE, IERR, WRITEXMLINCAR)
...
! sanity check, the PEAD routines need NCORE=1
         IF (NCORE/=1) THEN
            CALL vtutor%write(isError, PEAD_NCOREnot1)
         ENDIF


         PEAD_ORDER=4
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'IPEAD', PEAD_ORDER, IERR, WRITEXMLINCAR)

         LSKIP_EDOTP=.FALSE.
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'SKIP_EDOTP', LSKIP_EDOTP, IERR, WRITEXMLINCAR)

         LRPA=.FALSE.
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LRPA', LRPA, IERR, WRITEXMLINCAR)
         LSKIP_NSCF=.NOT.LRPA

         LSKIP_SCF=.FALSE.
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'SKIP_SCF', LSKIP_SCF, IERR, WRITEXMLINCAR)

         POSCEN=-100
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'DIPOL', POSCEN, 3, IERR, WRITEXMLINCAR)

         LREDUCE_SYM=.FALSE.
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LPEAD_SYM_RED', LREDUCE_SYM, IERR, WRITEXMLINCAR)

         EDIFF_PEAD=-1
         CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'EDIFF_PEAD', EDIFF_PEAD, IERR, WRITEXMLINCAR)
...
!******************** SUBROUTINE PEAD_WRITER ***************************
!
!> Write PEAD related settings to OUTCAR
!
!***********************************************************************
...
!***********************************************************************
     ...
     &   LATT_CUR,LATT_INI,T_INFO,P,LMDIM,LOVERL,IRDMAX,IO)
      ...
      USE pseudo
      USE kpoints_change
      USE tutor, ONLY: vtutor, isError, KPOINTS_PEAD
      IMPLICIT NONE
...
! currently the PEAD routines only work for Monkhorst-Pack grids
      IF (KPOINTS%MODE<=ModeUnknown.OR.KPOINTS%MODE>=GeneratingLattice) &
         CALL vtutor%error('The PEAD routines require you to use a Monkhorst-Pack type of k-points grid.')

! and only for grids that conserve the symmetry of the reciprocal lattice
      IF (KPOINTS_FULL%NKPTS/=KPOINTS_FULL%NKPX*KPOINTS_FULL%NKPY*KPOINTS_FULL%NKPZ) &
         CALL vtutor%write(isError, KPOINTS_PEAD)
...
! set d_IJ (d_IJ = \int r*Q_ij(r) dr)
      ...
      IF (LPEAD_RELAX_STRUCTURE()) THEN
         ALLOCATE(FZ_EFIELD_PEAD(3,T_INFO%NIONS)); FZ_EFIELD_PEAD=0
      ENDIF
...
!
!>  Adapt the wavefunctions to new (lower) symmetry
!
!***********************************************************************
...
!     IF (.NOT.LPEAD_NONZERO_EFIELD()) RETURN
      ...
      IF (SYMM%ISYM>0) THEN
         ...
         CALL INISYM(LATT_CUR%A,DYN%POSION,VEL,DYN%EFOR,T_INFO%LSFOR, &
             T_INFO%LSDYN,LPEAD_SYM_RED(),T_INFO%NTYP,T_INFO%NITYP,T_INFO%NIOND, &
...
!******************** SUBROUTINE PEAD_STORE_WAVE_ORIG ******************
!
!***********************************************************************
...
!******************** SUBROUTINE PEAD_RESTORE_WAVE_ORIG ****************
!
!***********************************************************************
...
! First order differentiation
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ)
      CASE(2)
! Second order differentiation
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=1,PREFAC=1.333333333_q)
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=2,PREFAC=-.166666666_q)
      CASE(3)
! Third order differentiation
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=1,PREFAC=1.5_q)
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=2,PREFAC=-.3_q)
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=3,PREFAC=0.033333333_q)
         CASE(4)
! Fourth order differentiation
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=1,PREFAC=1.6_q)
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=2,PREFAC=-.4_q)
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=3,PREFAC=0.076190476_q)
         CALL DPSI_DK_BERRY( &
        &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI_1K=RPHI,RPHI_1K_CPROJ=RPHI_CPROJ,STRIDE=4,PREFAC=-.007142857_q)
      END SELECT

      IF (LPEAD_RETURN_Q_CPROJ) THEN
         CALL DPSI_DK_EV(W,K,ISP,NOCC(ISP),RPHI_1K_CPROJ=RPHI_CPROJ)
      ENDIF
...
! First order differentiation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ)
         CASE(2)
! Second order differentiation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=1,PREFAC=1.333333333_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=2,PREFAC=-.166666666_q)
         CASE(3)
! Third order differentiation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=1,PREFAC=1.5_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=2,PREFAC=-.3_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=3,PREFAC=0.033333333_q)
         CASE(4)
! Fourth order differentiation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=1,PREFAC=1.6_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=2,PREFAC=-.4_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=3,PREFAC=0.076190476_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),RPHI,RPHI_CPROJ,STRIDE=4,PREFAC=-.007142857_q)
         END SELECT

         IF (LPEAD_RETURN_Q_CPROJ) THEN
            CALL DPSI_DK_EV(W,K,ISP,NOCC(ISP),RPHI_CPROJ)
         ENDIF
...
!        RPHI_CPROJ(:,:,IK,ISP,1:3)=WEIGHT*RPHI(:,:,IK,ISP,1:3)
      ...
      CALL M_sum_single8(W%WDES%COMM_KINTER,RPHI(1,1,1,1,1),2_qi8*SIZE(RPHI,kind=qi8))
# 806

      CALL M_sum_single8(W%WDES%COMM_KINTER,RPHI_CPROJ(1,1,1,1,1),2_qi8*SIZE(RPHI_CPROJ,kind=qi8))
...
! First order differentiation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK)
         CASE(2)
! Second order differentation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=1,PREFAC=1.333333333_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=2,PREFAC=-.166666666_q)
         CASE(3)
! Third order differentation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=1,PREFAC=1.5_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=2,PREFAC=-.3_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=3,PREFAC=0.033333333_q)
         CASE(4)
! Fourth order differentiation
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=1,PREFAC=1.6_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=2,PREFAC=-.4_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=3,PREFAC=0.076190476_q)
            CALL DPSI_DK_BERRY( &
           &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK,STRIDE=4,PREFAC=-.007142857_q)
         END SELECT

         IF (LPEAD_RETURN_Q_CPROJ) THEN
            CALL DPSI_DK_EV(W,K,ISP,NOCC(ISP),DIR=IDIR,DWDK_DIR=DWDK)
         ENDIF
...
!!      CALL M_sum_single(W%WDES%COMM_KINTER,DWDK%CPTWFP(1,1,1,1),2*SIZE(DWDK%CPTWFP))
!!#ifdef gammareal
!!      CALL M_sum_single(W%WDES%COMM_KINTER,DWDK%CPROJ(1,1,1,1),SIZE(DWDK%CPROJ))
!!#else
!!      CALL M_sum_single(W%WDES%COMM_KINTER,DWDJ%CPROJ(1,1,1,1),2*SIZE(DWDK%CPROJ))
!!#endif
...
!***********************************************************************
     ...
     &   W,P,CQIJ,LATT_CUR,T_INFO,LINITIALIZE,LSKIP_IF_POSSIBLE)
      ...
      USE full_kpoints
      USE mdipol
      USE tutor, ONLY: isError, PEADnowIsMetal
      IMPLICIT NONE
      TYPE(wavespin) W
      TYPE(type_info) T_INFO
      TYPE(potcar) P(T_INFO%NTYP)
...
! early exit if possible
      ...
      IF (isMetal(W%FERTOT)) THEN
         CALL vtutor%write(isError, PEADnowIsMetal)
      ENDIF
      ...
      CALL POINT_CHARGE_DIPOL(T_INFO,LATT_CUR,P%ZVALF,POSCEN,DIPOLE_DIRECT, &
                              RDUM,.FALSE.)
      ...
      IF (LINIT) IONIC_DIPOLE_INIT=IONIC_DIPOLE

! write to stdout
      IF (LDOIO) THEN
         RSUM(:)=RSTORE(:,1)+IONIC_DIPOLE
     ...
     &    (.NOT.LINIT).AND.ALLOCATED(R_EV_INIT)) THEN
         ...
         DRSUM=DRSUM+(IONIC_DIPOLE-IONIC_DIPOLE_INIT)
...
!
!> Write macroscopic dipole moment to the OUTCAR and vaspxml files
!
!***********************************************************************
...
! write total macroscopic dipole moment
 ...
 100  FORMAT('    Spin resolved dipole moment: p[sp1]=(',3F12.5,' ) |e| Angst')
 110  FORMAT('                                 p[sp2]=(',3F12.5,' ) |e| Angst')
 120  FORMAT(' Total electronic dipole moment: p[elc]=(',3F12.5,' ) |e| Angst')


      CALL VH5_WRITE_DIPOLE_MOMENT(IH5OUTFILEID, "ion", IONIC_DIPOLE)
      CALL VH5_WRITE_DIPOLE_MOMENT(IH5OUTFILEID, "electron", RSUM)
      IF (SIZE(RSTORE,2) == 2) &
         CALL VH5_WRITE_SPIN_RESOLVED_DIPOLE_MOMENT(IH5OUTFILEID, RSTORE)
...
! write total change macroscopic dipole moment
      WRITE(IO%IU6,220) DRSUM
      WRITE(IO%IU6,*)

 200  FORMAT('                                dp[sp1]=(',3F12.5,' ) |e| Angst')
 210  FORMAT('                                dp[sp2]=(',3F12.5,' ) |e| Angst')
 220  FORMAT(' Total change in dipole moment: dp[tot]=(',3F12.5,' ) |e| Angst')
...
! do not recalculate RPHI if LPEAD_NOSCF=.TRUE.
      ...
      IF (.NOT.ALLOCATED(RPHI_STORE_LOCAL_ALL)) THEN
         ALLOCATE(RPHI_STORE_LOCAL_ALL(W%WDES%NRPLWV,W%WDES%NBANDS,W%WDES%NKPTS,W%WDES%ISPIN,3))
!$ACC ENTER DATA CREATE(RPHI_STORE_LOCAL_ALL) 
      ENDIF

      IF (.NOT.ALLOCATED(RPHI_STORE_LOCAL_ALL_CPROJ)) THEN
         ALLOCATE(RPHI_STORE_LOCAL_ALL_CPROJ(W%WDES%NPROD,W%WDES%NBANDS,W%WDES%NKPTS,W%WDES%ISPIN,3))
!$ACC ENTER DATA CREATE(RPHI_STORE_LOCAL_ALL_CPROJ) 
      ENDIF
...
!! 
!$ACC UPDATE DEVICE(RPHI_STORE_LOCAL_ALL,RPHI_STORE_LOCAL_ALL_CPROJ) 
      RETURN
      END SUBROUTINE PEAD_ACC_CALC_ALL


!******************** SUBROUTINE PEAD_ACC_ADD **************************
!
!***********************************************************************
...
!$ACC ENTER DATA COPYIN(EFIELD_PEAD) 
      DO IDIR=1,3
!$ACC KERNELS PRESENT(CF,CPROF,EFIELD_PEAD,RPHI_STORE_LOCAL_ALL,RPHI_STORE_LOCAL_ALL_CPROJ) 
...
!$ACC END KERNELS
      ENDDO
!$ACC EXIT DATA DELETE(EFIELD_PEAD) 

      RETURN
      END SUBROUTINE PEAD_ACC_ADD


!******************** SUBROUTINE PEAD_ACC_ADD_PW ***********************
!
!***********************************************************************
...
!! USE moffload_struct_def
      ...
      USE prec
      IMPLICIT NONE
      IF (ALLOCATED(RPHI_STORE_LOCAL_ALL)) THEN
!$ACC EXIT DATA DELETE(RPHI_STORE_LOCAL_ALL) ASYNC(ACC_ASYNC_Q) &
!$ACC IF(OFFLOAD_ON.AND.ACC_IS_PRESENT(RPHI_STORE_LOCAL_ALL,1))
         DEALLOCATE(RPHI_STORE_LOCAL_ALL)
      ENDIF
      IF (ALLOCATED(RPHI_STORE_LOCAL_ALL_CPROJ)) THEN
!$ACC EXIT DATA DELETE(RPHI_STORE_LOCAL_ALL_CPROJ) ASYNC(ACC_ASYNC_Q) &
!$ACC IF(OFFLOAD_ON.AND.ACC_IS_PRESENT(RPHI_STORE_LOCAL_ALL_CPROJ,1))
         DEALLOCATE(RPHI_STORE_LOCAL_ALL_CPROJ)
      ENDIF
      RETURN
      END SUBROUTINE PEAD_ACC_DEALLOCATE


!******************** SUBROUTINE PEAD_EIGENVALUES **********************
!
!***********************************************************************
...
! interaction energy
      ...
      IF (LPEAD_RELAX_STRUCTURE()) RSUM=RSUM+IONIC_DIPOLE
...
!******************** SUBROUTINE PEAD_CHECK_FIELD_CONDITION ************
!
!***********************************************************************
      ...
      USE wave_high
      USE full_kpoints
      USE tutor, ONLY: isAlert, newArgument, PEADlargeField, PEADnoVirtuals
      IMPLICIT NONE
      TYPE(wavespin) W
      TYPE(latt) LATT_CUR
      TYPE(in_struct) IO
...
! to be too large for comfort
            CALL vtutor%write(isAlert, PEADlargeField, newArgument(rval = RTUT))
         ENDIF
      ELSE
! write a warning that VASP cannot estimate whether
! the critical field strength is being exceeded
         CALL vtutor%write(isAlert, PEADnoVirtuals)
      ENDIF
...
!
!> Called from main.F and takes care of (1._q,0._q) or all
!> of the following:
...
!***********************************************************************
     ...
     &   T_INFO,DYN,INFO,XC,IO,MIX,KPOINTS,SYMM,GRID,GRID_SOFT, &
     ...
     &   TOTEN,EFERMI,LDIMP,LMDIMP )
      ...
      USE tutor, ONLY: isAlert, PEADPolNoMetal, PEADEpsNoMetal
      IMPLICIT NONE
...
! local variables
      ...
      REAL(q) TMP(3,3),BORN_NSCF(3,3,T_INFO%NIOND),BORN_SCF(3,3,T_INFO%NIOND)
      ...
      INTEGER :: IRDMAA
      INTEGER :: IALGO_SAVE
      LOGICAL :: LDIAG_SAVE,LSUBROT_SAVE,LREDUCE_SYM_SAVE
      REAL(q) :: EDIFF_SAVE
!=======================================================================
...
!=======================================================================
      IF (LPEAD_CALC_POL()) THEN
         IF (isMetal(W%FERTOT)) THEN
            CALL vtutor%write(isAlert, PEADPolNoMetal)
            RETURN
         ENDIF
         CALL PEAD_POLARIZATION_CALC(W,P,CQIJ,LATT_CUR,T_INFO)
...
!=======================================================================
      IF (LPEAD_CALC_EPS()) THEN
         IF (isMetal(W%FERTOT)) THEN
            CALL vtutor%write(isAlert, PEADEpsNoMetal)
            RETURN
         ENDIF
...
! and switch off the scf subspace preconditioning
         ...
         IF (EDIFF_PEAD>0) INFO%EDIFF=EDIFF_PEAD

         LREDUCE_SYM_SAVE=LREDUCE_SYM
...
! electric field and the finite difference stencils
! and reallocate the wave functions
            LREDUCE_SYM=.TRUE.
...
!-----------------------------------------------------------------------
! deallocate RPHI_STORE_LOCAL_ALL and _CPROJ
!!       
...
! symmetrize DRNSCF, DRSCF, BORN_NSCF, and BORN_SCF
         IF (SYMM%ISYM>0) THEN
            IF (LNSCF_RESPONSE()) THEN
               CALL TSYM(DRNSCF,ISYMOP,NROTK,LATT_CUR%A)
! remove drift from NSCF Born effective charge
               TMP=0
               DO I=1,T_INFO%NIONS
                  TMP(:,:)=TMP(:,:)+BORN_NSCF(:,:,I)/T_INFO%NIONS
               ENDDO
               DO I=1,T_INFO%NIONS
                  BORN_NSCF(:,:,I)=BORN_NSCF(:,:,I)-TMP(:,:)
               ENDDO
! and symmetrize the NSCF Born effective charge tensors
...
! remove drift from SCF Born effective charge
               TMP=0
               DO I=1,T_INFO%NIONS
                  TMP(:,:)=TMP(:,:)+BORN_SCF(:,:,I)/T_INFO%NIONS
               ENDDO
               DO I=1,T_INFO%NIONS
                  BORN_SCF(:,:,I)=BORN_SCF(:,:,I)-TMP(:,:)
               ENDDO
! and symmetrize the SCF Born effective charge tensors
...
! This is to keep consistency with replacement in linear response driver
            IF (LNSCF_RESPONSE()) CALL XML_TENSOR("dielectric_nscf",EPSILON_NSCF)
            IF (LSCF_RESPONSE())  CALL XML_TENSOR("dielectric_scf" ,EPSILON_SCF )

            IF (LNSCF_RESPONSE()) &
               CALL VH5_WRITE_DIELECTRIC_STATIC(IH5OUTFILEID,"electron",EPSILON_NSCF,"nscf")
            IF (LSCF_RESPONSE()) &
               CALL VH5_WRITE_DIELECTRIC_STATIC(IH5OUTFILEID,"electron",EPSILON_SCF,"scf")
...
! write PIEZO electric tensor to OUTCAR and vasprun.xml
         IF (LDOIO) THEN
            ...
            IF (LNSCF_RESPONSE()) THEN
               ...
               CALL VH5_WRITE_PIEZOELECTRIC_TENSOR(IH5OUTFILEID, "electron", PIEZO_NSCF * FACT)
            ...
            IF (LSCF_RESPONSE()) THEN
               ...
               CALL VH5_WRITE_PIEZOELECTRIC_TENSOR(IH5OUTFILEID, "electron", PIEZO_SCF * FACT)
...
! OUTCAR and vasprun.xml
         IF (LDOIO) THEN
            ...
            IF (LSCF_RESPONSE()) THEN
               ...
               CALL VH5_WRITE_BORN_CHARGES(IH5OUTFILEID,BORN_SCF)
...
! we are 1._q and switch off the field just to be sure
         CALL PEAD_SWITCH_EFIELD_OFF()
...
! check the Zener field condition
         ...
         startup: IF (NSTEP==1) THEN

! reduce the symmetry in accordance with the
! electric field and the finite difference stencils
! and reallocate the wave functions
         LREDUCE_SYM_SAVE=LREDUCE_SYM ; LREDUCE_SYM=.TRUE.
         CALL PEAD_RESYMMETRIZE(KPOINTS,WDES,NONLR_S,NONL_S,W,W_F,W_G,CHAM,CHF, &
        &    GRID,P,T_INFO,INFO,LATT_CUR,LATT_INI,DYN,SYMM,IO)
...
!        CALL PEAD_POLARIZATION_WRITE(IO,LATT_CUR,LWARNING=.TRUE.)
         ...
         IF (LPEAD_RELAX_STRUCTURE()) THEN
            ...
            DO IDIR=1,3
              ...
              &   W,KPOINTS,P,CQIJ,CRHODE,NONLR_S,NONL_S,LATT_CUR,T_INFO,DYN,SYMM,IDIR, &
              &   EWFOR,EWSIF)
               FZ_EFIELD_PEAD=FZ_EFIELD_PEAD-EWFOR*EFIELD_PEAD(IDIR)
            ENDDO
         ENDIF

         cleanup: IF (.NOT.LPEAD_RELAX_STRUCTURE()) THEN
...
!******************** SUBROUTINE PEAD_ADD_FZ_EFIELD ********************
!
!***********************************************************************
      SUBROUTINE PEAD_ADD_FZ_EFIELD(T_INFO,FORCE)
      USE poscar
      TYPE (type_info) :: T_INFO
      REAL(q) :: FORCE(3,T_INFO%NIOND)
! local variables
      INTEGER :: I

      DO I=1,T_INFO%NIONS
         FORCE(:,I)=FORCE(:,I)+FZ_EFIELD_PEAD(:,I)
      ENDDO

      RETURN
      END SUBROUTINE PEAD_ADD_FZ_EFIELD
...
!***********************************************************************
!
!> LPEAD_SYM_RED exposes the logical variable LREDUCE_SYM (read from
!> INCAR). When LPEAD_SYM_RED=.TRUE. the subroutine INISYM will only
!> keep symmetry operations that conserves the finite difference stencil
!> used in the PEAD routines.
!
!***********************************************************************
      FUNCTION LPEAD_SYM_RED()
      IMPLICIT NONE
      LOGICAL LPEAD_SYM_RED
      LPEAD_SYM_RED=LREDUCE_SYM
      END FUNCTION LPEAD_SYM_RED
...
!***********************************************************************
!
! LPEAD_RELAX_STRUCTURE
!
!***********************************************************************
      FUNCTION LPEAD_RELAX_STRUCTURE()
      IMPLICIT NONE
      LOGICAL LPEAD_RELAX_STRUCTURE
      LPEAD_RELAX_STRUCTURE=LRELAX_STRUCTURE
      END FUNCTION LPEAD_RELAX_STRUCTURE
...
! copy all data from WDES to WDES_TMP
      ...
      WDES_FULL_PEAD%WTKPT=>KPOINTS_FULL%WTKPT
...
! plane wave contributions
      ...
      CALL ROTATE_WAVE_SPIN_RECIP( &
         W%WDES, KPOINTS_TRANS_FULL_PEAD%RSSYMOP(:,:,NK), NK, KPOINTS_TRANS_FULL_PEAD%SPINFLIP(NK), W1%CPTWFP)
...
! (1._q,0._q)-center contributions
      ...
      DO ISPINOR=0,W%WDES%NRSPINORS-1
         ...
         DO NT=1,W%WDES%NTYP
            DO NI=NIS,W%WDES%NITYP(NT)+NIS-1
              ...
              &   ROT_HANDLE%NPS(NT),ROT_HANDLE%LPS(1,NT))
      ...
      CALL ROTATE_WAVE_CHARACTER_SPIN(W1%WDES1, W1%CPROJ, KPOINTS_TRANS_FULL_PEAD%RSSYMOP(:,:,NK))
...
! plane wave contributions
      DO N=1,W%WDES%NBANDS
         ...
         CALL ROTATE_WAVE_SPIN_RECIP( &
            W%WDES, KPOINTS_TRANS_FULL_PEAD%RSSYMOP(:,:,NK), NK, KPOINTS_TRANS_FULL_PEAD%SPINFLIP(NK), WA%CPTWFP(:,N))
...
! (1._q,0._q)-center contributions
      ...
      DO N=1,W%WDES%NBANDS
         ...
         CALL ROTATE_WAVE_CHARACTER_SPIN(WA%WDES1, WA%CPROJ(:,N), KPOINTS_TRANS_FULL_PEAD%RSSYMOP(:,:,NK))
...
!***********************************************************************
      SUBROUTINE CALC_EWFOR_AND_EWSIF( &
     &   W,KPOINTS,P,CQIJ,CRHODE,NONLR_S,NONL_S,LATT_CUR,T_INFO,DYN,SYMM,IDIR,EWFOR,EWSIF)
      USE prec
      USE base
      USE mpimy
      USE poscar
...
!>                 |u_{m,k-p*dk_{\beta}}> S^{-1}_{mn}(k,k-p*dk_{\beta}) ]
!> \f]
!> \f$ \alpha = 1,2,3 \f$ labels the cartesion directions x, y, and z.
!>
!> \f$ dk_{\beta} \f$ is the distance between neighbouring k-points along
!> direction \f$ \beta \f$ in reciprocal space (in direct coordinates),
!> \f$ dk_{1,2,3} = \f$ ``2*pi/NKPX`` , ``2*pi/NKPY`` , ``2*pi/NKPZ``
!>
!> ``c = PREFAC``, ``p = STRIDE``
!>
...
!> \f$ d\psi_k/dk_{\alpha}\f$
!
!***********************************************************************
      SUBROUTINE DPSI_DK_BERRY( W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NBMAX, &
...
! NK must be a kpoint in the IBZ
      IF (NK/=KPOINTS_FULL%NEQUIV(NK)) THEN
         CALL vtutor%error("DPSI_DK_BERRY: ERROR, k-point not in IBZ: " // str(K(1:3)))
      ENDIF
...
! allocate workspace
      ALLOCATE(IPVT(NBMAX),CWORK(NBMAX))
...
! add + < k | k+b_i >  or  - < k | k-b_i >
         ...
         CALL CALC_OVERLAP(W,K1,K2,ISP,P,CQIJ,LATT_CUR,T_INFO,S,NBMAX=NBMAX)
!!         CALL CALC_OVERLAP(W,K1,K2,ISP,P,CQIJ,LATT_CUR,T_INFO,S,NBMAX=NBMAX,LUSEQIJB=.TRUE.)
!        IF (LDOIO) CALL DUMP_HAM_PEAD("     S_k1_k2",W%WDES,S)
! Find LU decomposition and reciprocal condition of S
         CALL ZGECO(S,W%WDES%NB_TOT,NBMAX,IPVT,RCOND,CWORK)
         IF (RCOND.GT.0._q) THEN
! Calculate inv(S)
            CALL ZGEDI(S,W%WDES%NB_TOT,NBMAX,IPVT,CDET,CWORK,1)
         ELSE
! S is singular to working precision
            CALL vtutor%error("DPSI_DK_BERRY: ERROR, S(k1,k2) is singular: k1= " // str(K1) // " k2= &
               &" // str(K2))
         ENDIF
! only S(1:NBMAX,1:NBMAX) is to be used, set the rest to (0._q,0._q)
         IF (NBMAX<W%WDES%NB_TOT) THEN
            S(:,NBMAX+1:)=0
            S(NBMAX+1:,:)=0
         ENDIF
...
! search for kpoint k2 in BZ
         ...
         IF (NBMAX<W%WDES%NB_TOT) THEN
            NBANDS=CEILING(REAL(NBMAX)/REAL(WP%WDES%NB_PAR))
         ELSE
...
! add DWDK to DWDK_DIR
      IF (PRESENT(DIR)) THEN
         IF (PRESENT(DWDK_DIR)) THEN
            DWDK_DIR%CPTWFP(:,:,NK,ISP)   = DWDK_DIR%CPTWFP(:,:,NK,ISP)+ &
               WEIGHT*LATT_CUR%A(DIR,I)*DWDK%CPTWFP(:,1:SIZE(DWDK_DIR%CPTWFP,2))
            DWDK_DIR%CPROJ(:,:,NK,ISP)= DWDK_DIR%CPROJ(:,:,NK,ISP)+ &
               WEIGHT*LATT_CUR%A(DIR,I)*DWDK%CPROJ(:,1:SIZE(DWDK_DIR%CPROJ,2))
         ENDIF
      ENDIF
...
!> This subroutine implements the following expression
!> \f[
!> D_{kn,\alpha,i} = \sum_j (d_ij)_{\alpha} < p_j | \tilde{\psi}_{kn} >
!> \f]
!> \f[
!> d_{ij} = -\int Q_{ij}(r) r dr
!> \f]
!> (note the sign!)
!> \f$ \alpha = 1,2,3 \f$ labels the cartesion directions x, y, and z
!>
...
!
!***********************************************************************
      SUBROUTINE DPSI_DK_EV(W,K,ISP,NBMAX,RPHI_CPROJ,DIR,DWDK_DIR,RPHI_1K_CPROJ)
      USE prec
      USE poscar
      USE pseudo
      USE lattice
...
! NK must be a kpoint in the IBZ
      IF (NK/=KPOINTS_FULL%NEQUIV(NK)) THEN
         CALL vtutor%error("DPSI_DK_EV: ERROR, k-point not in IBZ: " // str(K(1:3)))
      ENDIF
...
! unoccupied bands do not contribute
         DO N=1,WDESK%NBANDS
            IF ( (WDESK%NB_LOW+WDESK%NB_PAR*(N-1))>NBMAX ) DWDK%CPROJ(:,N)=0
         ENDDO
...
! add DWDK%CPROJ to DWDK_DIR
         IF (PRESENT(DIR)) THEN
            IF (PRESENT(DWDK_DIR).AND.(I==DIR)) THEN
               DWDK_DIR%CPROJ(:,:,NK,ISP)=DWDK_DIR%CPROJ(:,:,NK,ISP)+ &
                  WEIGHT*DWDK%CPROJ(:,1:SIZE(DWDK_DIR%CPROJ,2))
            ENDIF
         ENDIF
...
!
!> Orthogonalize the gradient \f$ d\psi/dk \f$ with respect
!> to the occupied states.
!
!***********************************************************************
      SUBROUTINE DPSI_DK_ORTHO( &
     &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,NBMAX,RPHI,RPHI_CPROJ,DIR,DWDK_DIR)
      USE prec
      USE poscar
      USE pseudo
      USE lattice
...
! NK must be a kpoint in the IBZ
      IF (NK/=KPOINTS_FULL%NEQUIV(NK)) THEN
         CALL vtutor%error("DPSI_DK_ORTHO: ERROR, k-point not in IBZ: " // str(K(1:3)))
      ENDIF
...
! add DWDK to DWDK_DIR
         IF (PRESENT(DIR)) THEN
            IF (PRESENT(DWDK_DIR).AND.(I==DIR)) THEN
               DWDK_DIR%CPTWFP(:,:,NK,ISP)=DWDK_DIR%CPTWFP(:,:,NK,ISP)+ &
                  WEIGHT*DWDK%CPTWFP(:,1:SIZE(DWDK_DIR%CPTWFP,2))
               DWDK_DIR%CPROJ(:,:,NK,ISP)=DWDK_DIR%CPROJ(:,:,NK,ISP)+ &
                  WEIGHT*DWDK%CPROJ(:,1:SIZE(DWDK_DIR%CPROJ,2))
            ENDIF
         ENDIF
...
! on the strings of k-points
                  DS=DETS(W,K1,K2,ISP,P,CQIJ,LATT_CUR,T_INFO,NOCC(ISP))
...
!           R_BP(:,ISP)=R_BP(:,ISP)+IMLNDS*LATT_CUR%A(:,IDIR)*W%WDES%RSPIN/(KPOINTS_FULL%NKPTS*DK(IDIR))/TPI
         ENDDO
      ENDDO
      ...
      IF (.NOT.LINIT.AND.ALLOCATED(SUM_PHASE_ON_STRING_INIT)) THEN
         ...
         DO ISP=1,W%WDES%ISPIN
            DO IDIR=1,3
               ...
               END SELECT
               DR_BP(:,ISP)=DR_BP(:,ISP)+IMLNDS*LATT_CUR%A(:,IDIR)*W%WDES%RSPIN/(KPOINTS_FULL%NKPTS*DK(IDIR))/TPI
            ENDDO
         ENDDO
...
! Calculate determinant of S
      ...
      ELSE
! S is singular to working precision
         CALL vtutor%error("IMLNDET: ERROR, S(k1,k2) is singular: k1= " // str(K1) // " k2= " // str(K2))
      ENDIF
...
!
!***********************************************************************
      FUNCTION DETS(W,K1,K2,ISP,P,CQIJ,LATT_CUR,T_INFO,NBMAX)
      USE prec
      USE pseudo
      USE poscar
      USE lattice
...
! Find LU decomposition and reciprocal condition of S
      ...
      IF (RCOND.GT.0._q) THEN
! Calculate determinant of S
         CALL ZGEDI(S,W%WDES%NB_TOT,NBMAX,IPVT,CDET,CWORK,10)
         DETS=CDET(1)*10._q**CDET(2)
      ELSE
! S is singular to working precision
         CALL vtutor%error("DETS: ERROR, S(k1,k2) is singular: k1= " // str(K1) // " k2= " // str(K2))
      ENDIF
...
!************************ SUBROUTINE SET_EV ****************************
!
!> Calculate the "expectation-value" (EV) term,
!> ~~~
...
!***********************************************************************
      ...
      TYPE(wavefun1) W1
      INTEGER ISP, LMDIM
      INTEGER IK, IB, ISPINOR, ISPINOR_, IND
      INTEGER NPRO, NPRO_, NI, NIS, NT
      INTEGER L, LP, LMMAXC
      REAL(q) RSUM(3),RSUM_(3),WEIGHT
      LOGICAL LOVERL
...
!***********************************************************************
     ...
     &   NBMAX,LPLANEWAVE,LONECENTER,LUSEQIJB &
     &)
      USE prec
      USE poscar
      USE pseudo
...
! local variables
      ...
      CALL CALC_OVERLAP_(W,K1,K2,ISP,P,CQIJ,LATT_CUR,T_INFO,S_,NB,LPWV,LONE,LQIJB)
      S=S_
# 4142
...
!        IF (ABS(MOD(DK1(I)+10._q,1._q))>TINY) LERR=.TRUE.
      ...
      ENDDO
      IF (LERR) THEN
         CALL vtutor%error("SET_PHASE_SHIFT: ERROR, DK1 is not a reciprocal lattice vector \n " // &
            str(K1(:)) // " " // str(KPOINTS_FULL%VKPT(:,NK1)))
      ENDIF
      LERR=.FALSE.
      DO I=1,3
...
!        IF (ABS(MOD(DK2(I)+10._q,1._q))>TINY) LERR=.TRUE.
      ...
      ENDDO
      IF (LERR) THEN
         CALL vtutor%error("SET_PHASE_SHIFT: ERROR, DK2 is not a reciprocal lattice vector \n " // &
            str(K2(:)) // " " // str(KPOINTS_FULL%VKPT(:,NK2)))
      ENDIF
...
! consistency tests
     ...
     &    (SIZE(CQIJ,3)/=WDES1%NIONS)) THEN
         CALL vtutor%error("OVERL_AND_APPLY_PHASE_SHIFT: ERROR, CQIJ inconsistently dimensioned")
      ENDIF
...
! store into CPROJ_RESUL or back into W1%CPROJ
      IF (PRESENT(CPROJ_RESUL)) THEN
         IF (SIZE(CPROJ_RESUL)<SIZE(CRESUL)) THEN
            CALL vtutor%error("OVERL_AND_APPLY_PHASE_SHIFT: ERROR, CPROJ_RESUL too small")
         ENDIF
         DO N=1,WDES1%NPRO
...
! consistency tests
      IF ((SIZE(CQIJ,1)/=SIZE(CQIJ,2)).OR.(SIZE(CQIJ,1)<MAXVAL(WDES1%LMMAX)).OR. &
     &    (SIZE(CQIJ,3)/=WDES1%NIONS)) THEN
         CALL vtutor%error("OVERL_AND_APPLY_PHASE_SHIFT_C: ERROR, CQIJ inconsistently dimensioned")
      ENDIF
...
! store into CPROJ_RESUL or back into W1%CPROJ
      IF (PRESENT(CPROJ_RESUL)) THEN
         IF (SIZE(CPROJ_RESUL)<SIZE(CRESUL)) THEN
            CALL vtutor%error("OVERL_AND_APPLY_PHASE_SHIFT_C: ERROR, CPROJ_RESUL too small")
         ENDIF
         DO N=1,WDES1%NPRO
            CPROJ_RESUL(N)=CRESUL(N)
         ENDDO
...
! store into CPROJ_RESUL or back into W1%CPROJ
      IF (PRESENT(CPROJ_RESUL)) THEN
         IF (SIZE(CPROJ_RESUL)<SIZE(CRESUL)) THEN
            CALL vtutor%error("APPLY_PHASE_SHIFT_CPROJ: ERROR, CPROJ_RESUL too small")
         ENDIF
         DO N=1,WDES1%NPRO
            CPROJ_RESUL(N)=CRESUL(N)
         ENDDO
...
! have we been able to set the correct component?
      IF (.NOT.LFOUND) THEN
         CALL vtutor%error("SET_PHASE_SHIFT_GRID: ERROR, unable to set pw-component: " // str(G))
      ENDIF
...
! consistency check
      IF (SIZE(SV)/=W1%WDES1%GRID%RL%NP) THEN
         CALL vtutor%error("APPLY_PHASE_SHIFT_GRID: ERROR, grid inconsistency")
      ENDIF
...
!>
!> @note the only difference between this subroutine and the SETYLM_AUG
...
!***********************************************************************
     ...
     &  LMYDIM,LYDIM,YLM,IRMAX,INDMAX,DISX,DISY,DISZ,XS,YS,ZS,DIST)
      ...
      INTEGER N1LOW, N2LOW, N3LOW, N1HI, N2HI, N3HI, N1, N2, N3, N1P, N2P
      INTEGER LMYDIM, LYDIM, IND, NPSRNL, NCOL, NADDR, INDMAX, IRMAX
      REAL(q) F1, F2, F3, D1, D2, D3, D, X1, X2, X3, X, Y, Z, XX, YY, ZZ
      REAL(q) ARGSC, PSDMAX, ARG, DISX, DISY, DISZ
      REAL(q) POSION(3)
      REAL(q) DIST(IRMAX)
      ...
      IF ((LYDIM+1)**2 > LMYDIM) THEN
         CALL vtutor%bug("internal error: LMYDIM is too small " // str(LYDIM) // " " // str(LMYDIM), "pead.F", 5011)
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      IF (GRID%RL%I2(NCOL) /= N1P+1 .OR. GRID%RL%I3(NCOL) /= N2P+1) THEN
        CALL vtutor%bug("internal ERROR SETYLM: " // str(N1P+1) // " " // str(N2P+1) // " " // &
           str(NCOL) // " " // str(GRID%RL%I2(NCOL)) // " " // str(N1P) // " " // &
           str(GRID%RL%I3(NCOL)) // " " // str(N2P), "pead.F", 5058)
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      IF (INDMAX>IRMAX) THEN
        CALL vtutor%bug("internal ERROR: DEPLE: IRDMAX must be increased to " // str(INT(INDMAX*1.1)), "pead.F", 5139)
      ENDIF
...
!************************ SUBROUTINE SETDIJ_R **************************
!
!> Calculate minus the dipole moment of the augmentation
!> charges
!> ~~~
...
! set new LOW value and LM value and go on
      ...
      CALL M_sum_d(WDES%COMM_INB, SUM, 1)
...
!> @note Strictly speaking we should account for e^(-i b \cdot R_ion)
