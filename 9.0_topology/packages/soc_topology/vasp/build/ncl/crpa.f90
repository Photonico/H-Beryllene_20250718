...
!disentanglement approach is 1._q
      LPROJECTED=.FALSE.
...
! if at least (1._q,0._q) entry is found, select LOCPROJ basis
            NUM_WAN = LPRJ_GET_NUM_WAN()
            IF ( NUM_WAN > 0 ) THEN
               LUSELOCPROJ = .TRUE. 
               IF ( IU0 >= 0 ) &
...
!> assume these are localized Wannier functions
            ...
            ELSE IF ( (LEN_TRIM(WANNIER90_WIN) > 0) .OR. LWIN_FOUND ) THEN
# 420
...
!set targetbands from NTARGET_STATES if not full RPA is 1._q
         IF ( .NOT. LNLRPA ) THEN
...
!otherwise disentanglement or projector method is 1._q.
...
! this signals that there is at least (1._q,0._q) CRPA band
...
! local variables
      ...
      COMPLEX(q), ALLOCATABLE :: UNCORR(:,:)
      COMPLEX(q), ALLOCATABLE :: UPLOT(:,:)
...
!construct overlap
            ...
            DO I=1,WDES%NB_TOT
               DO J=1,WDES%NB_TOT
                  DO K = 1, WDES%NB_TOT
                     U(I,J) = U(I,J) + UCRPA(I,K,NK,ISP)*CONJG(UCRPA(J,K,NK,ISP))
                  ENDDO 
               ENDDO
            ENDDO
...
!block not found if NROWMAX1 - NROWMIN1 + 1 /= CRPA_NUM_WAN
      IF (  NROWMAX1-NROWMIN1+1< CRPA_NUM_WAN ) THEN
         CALL vtutor%bug("internal error in VASP from DISENTANGLE: sub block not found", "crpa.F", 843)
      ENDIF 
...
!UNCORR is the projector onto non-correlated system
            ...
            DO I = 1, WDES%NB_TOT
               DO K = 1, WDES%NB_TOT
                  DO J = 1, CRPA_NUM_WAN
                     UNCORR(K,I)=UNCORR(K,I)-CONJG(U(I,J))*U(K,J)
                  ENDDO         
               ENDDO              
            ENDDO          
            DO I=1,WDES%NB_TOT
...
!correlated ones are in NEWEIGC
            NEWEIG(:,NK,ISP)=(0._q,0._q)
            NEWEIGC(:,NK,ISP)=(0._q,0._q)
            DO I = 1,WDES%NB_TOT
               DO J=1,WDES%NB_TOT
...
! local variables
      ...
      COMPLEX(q), ALLOCATABLE :: NEWEIG(:,:,:)
      REAL(q), ALLOCATABLE ::  COREIG(:)  !correlated eigenvalues
      COMPLEX(q), ALLOCATABLE :: WINDOWFUN(:,:,:)
      COMPLEX(q), ALLOCATABLE :: U(:,:)
      COMPLEX(q), ALLOCATABLE :: UPLOT(:,:)
      INTEGER :: NMIN, NMAX
      INTEGER :: NMIN1, NMAX1
...
!the roation of the CDER needs to be 1._q on the full mesh unfortunately
...
!rotate correlated function
            ...
            IF (WDES%DO_REDIS) THEN
               ...
               CALL REDIS_PW  (WDES1, WDES%NBANDS, WC%CPTWFP   (1,1,NK,ISP))
            ENDIF
! build linear combinations of the wave functions in
! accordance with the effective rotation matrix U
            CALL LINCOM('F',WA%CW_RED,WA%CPROJ_RED,UCRPA(1,1,NK,ISP), &
...
!dump some information
      DO ISP = 1,WDES%ISPIN
         ...
         DO NK = 1,WDES%NKPTS
!per default print only irreducible wedge of Brillouin (1._q,0._q)
             IF ( IO%NWRITE < 3 .AND. NK > NQ_IBZ ) CYCLE
             IF( IO%IU6>=0 ) THEN
                 WRITE(IO%IU6,2202)NK,WDES%VKPT(:,NK), &
...
!**********************************************************************
   ...
   SUBROUTINE  DETERMINE_CORRELATED_PROJECTOR(NB_TOT,U,P,NK,ISP,IU0,W)
      ...
      COMPLEX(q)          P(:,:) 
      INTEGER       IU0,NK,ISP
      COMPLEX(q),OPTIONAL :: W(:,:,:) !window function
!local
      INTEGER I,J,K
...
! build correlated hamiltonian
      ...
      DO I = 1, NB_TOT
         DO J = 1, NB_TOT
             DO K = 1, NB_TOT
                H(I,J) = H(I,J) + U(K, J)*REAL(W%CELTOT(K,NK,ISP),q)*CONJG(U(K,I))
             ENDDO
         ENDDO
      ENDDO
...
! check result
! find number of non-(0._q,0._q) eigenvalues (should be NCORR eigenvalues )
      NC = 0
      SIGMA=0
      SIGMA=EIG
      EIG=0  
...
!resulting matrix has a block of mostly (0._q,0._q) entries between NCOLSTART and NCOLSTOP
!these band indices are determined by the energy window in the Wannier projection
!there are only NCOLSTOP-NCOLSTART+1-DMFT_num_states non-(0._q,0._q) states in this block
!these columns have to be identified and shifted to the right in this block
      H=(0._q,0._q)
      H=U
      ALLOCATE(LTAG(NB_TOT))
      LTAG(:)=.FALSE. 
!scan relevant block for non-(0._q,0._q) elements
!WRITE(*,*)NK,NB_TOT
...
!RTMP = RTMP + REAL(H(I,J),q)
         ENDDO 
!tag column for non-(0._q,0._q) entries
...
! build correlated hamiltonian
      ...
      DO I = 1, NB_TOT
         DO J = 1, NB_TOT
             DO K = 1, NB_TOT
                H(I,J) = H(I,J) + U(K, J)*W%CELTOT(K,NK,ISP)*CONJG(U(K,I))
             ENDDO
         ENDDO
      ENDDO
...
# 2185
      ...
      IF (IFAIL/=0) THEN
         CALL vtutor%bug("internal error in VASP from DIAGH: Call to routine ZHEEV failed! Error " &
            // "code was " // str(IFAIL), "crpa.F", 2191)
...
# 2445
          ...
          CALL DIS_PW_BAND(WDES1, J, CW1, W%CPTWFP(1,1,IKPT_SELECT(K),ISP))
...
!local
    INTEGER           :: N1, N2, NPL2
    INTEGER           :: I 
    COMPLEX(q)              :: AUX(NB_TOT,NB_TOT)
    COMPLEX(q)              :: BUX(NB_TOT,NB_TOT)
    COMPLEX(q)              :: CUX(NB_TOT,NB_TOT)
...
!**********************************************************************
   ...
   SUBROUTINE JOINT_DIAGONALIZER(D,MAT,THRESH,M,NKPTS,ISPIN,IU0,IU6,MAXITER,LSINGLE)
      ...
      USE tutor, ONLY: vtutor
      IMPLICIT NONE
      COMPLEX(q) :: D(:,:,:)     !matrix which diagonalizes all input matrices
      COMPLEX(q) :: MAT(:,:,:,:) !set of matrices
      REAL(q)    :: THRESH       !convergence criterium
      INTEGER    :: M            !dimension of (1._q,0._q) matrix
...
!dump convergence info
!IF ( IU0 >= 0 ) THEN
!   WRITE(IU0,'(A,I4,A,E13.6)')'K-Point ',NKPTS,' 1._q E=',ABS(S)
!ENDIF
         ELSE
            DO I=1, M
               DO J=1, M
...
! Check for convergence.
      IF( INFO /= 0 ) THEN
         CALL vtutor%bug("internal error in VASP from RSYS_EIGENSYSTEM: DSYEV returns " // str(INFO), "crpa.F", 3181)
      ENDIF
...
! set up permutation matrix
      ...
      DO ISP = IHOMO+1, WDES%NB_TOT 
         S(ISP,ISP)=1
      ENDDO
! meddling is 1._q only in the occupied-occupied block
...
! sort after projection that is UEFF = S.U
            CALL ZGEMM( 'N', 'N', WDES%NB_TOT, WDES%NB_TOT, WDES%NB_TOT, &
