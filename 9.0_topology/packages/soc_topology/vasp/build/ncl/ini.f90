...
! this allows to set a maxmimum of registered allocates
      ...
      CHARACTER (LEN=40), PRIVATE :: alloc_tag(maxalloc)
      ...
      CHARACTER (LEN=40), PRIVATE :: alloc_tag_hi(maxalloc)
...
!***********************************************************************
      ...
      SUBROUTINE STOP_TIMING(TAG,IU,NAME,XMLTAG,WRITE_FILES)
      USE vaspxml
      USE c2f_interface, ONLY : VTIME
      IMPLICIT NONE
      INTEGER ENTRY
      ...
      WRITE_FILES_ = .TRUE.
      IF (PRESENT(WRITE_FILES)) WRITE_FILES_ = WRITE_FILES
    
      CALL VTIME(TV,TC)
      IF (WRITE_FILES_) THEN
...
!>If this file is not present, nothing is 1._q
!
!***********************************************************************
...
! communicate inside node
      CALL M_sum_i( COMM_INTRA_NODE_WORLD, MEMSIZE, 1 ) 

! communicate only minimum storage available on all nodes
      MEMSIZE = -MEMSIZE 
      CALL M_max_i( COMM_WORLD, MEMSIZE, 1 ) 
      MEMSIZE = -MEMSIZE 
...
! local
         ...
         CALL M_max_d( COMM_WORLD, MYRSIZM, 1 ) 
