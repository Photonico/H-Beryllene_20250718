...
!*********************************************************************
!
!> Displaces an atom in the cartesian direction IDIR by a step STEP
!
!*********************************************************************
...
!
!> Calculates the result of the overlap-operator acting onto the
...
!
!> Evaluates calculates the inproduct between two sets
!> of wavefunctions
!> ) LOVERL = .FALSE.
!> ~~~
...
! local variables
      ...
      IF (WDES%COMM_KINTER%NCPU.NE.1) THEN
         CALL vtutor%error("SUM_INPROD_C: KPAR>1 not tested (but implemented), sorry.")
      END IF
...
!
!> Calculates the total (1._q,0._q) centre charge density
!
!***********************************************************************
...
!************************ SUBROUTINE SETDIJ_R **************************
!
!> Calculates minus the dipole moment of the augmentation
!> charges
!> ~~~
...
!=======================================================================
!>  Calculates first derivative of potential
...
!=======================================================================
    ...
    SUBROUTINE POT_DER(KINEDEN, HAMILTONIAN, WDES, GRID, GRIDC, GRIDUS, GRID_SOFT, C_TO_US, SOFT_TO_C, &
         INFO, XC, T_INFO, P,  LATT_CUR, E1, E2, CSTRF, CSTRF1, CSTRF2, &
         CHDEN, CHTOT, CHTOT1, CHTOT_, DENCOR, CVTOT1, CVTOT2, SV1, SV2, & 
...
! positive displacement
    ...
    CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
         ...
         CHDEN,CHTOT_,CSTRF1,CVTOT1,DENCOR,SV1,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
...
! negative displacement
    ...
    CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
         ...
         CHDEN,CHTOT_,CSTRF2,CVTOT2,DENCOR,SV2,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
...
! once and forever, and evaluating CDIJ1 from the derivative of the potential
  ...
  END SUBROUTINE POT_DER
    ...
    SUBROUTINE POT_DER4( KINEDEN, HAMILTONIAN, WDES, GRID, GRIDC, GRIDUS, GRID_SOFT, C_TO_US, SOFT_TO_C, &
...
! positive displacement (doubled)
    ...
    CVTOT2=0
    CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
         INFO,XC,P,T_INFO,E3,LATT_CUR, &
         CHDEN,CHTOT_,CSTRF3,CVTOT2,DENCOR,SV2,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
...
! SETDIJ routine can handle finite displacements directly
    ...
    CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, &
         WDES%NCDIJ, LMDIM, CDIJ2(1,1,1,1), RHOLM_ , CRHODE_(1,1,1,1), &
         E3, LASPH=INFO%LASPH, LCOREL= .FALSE.  )
...
! negative displacement (doubled)
    ...
    CVTOT2=0
    CALL POTLOK(KINEDEN,GRID,GRIDC,GRID_SOFT,WDES%COMM_INTER,WDES, &
         INFO,XC,P,T_INFO,E4,LATT_CUR, &
         CHDEN,CHTOT_,CSTRF4,CVTOT2,DENCOR,SV2,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT_TO_C,XCSIF)
...
! SETDIJ routine can handle finite displacements directly
    ...
    CALL SET_DD_PAW(WDES, P, XC, T_INFO, INFO%LOVERL, &
         WDES%NCDIJ, LMDIM, CDIJ2(1,1,1,1), RHOLM_ , CRHODE_(1,1,1,1), &
         E4, LASPH=INFO%LASPH, LCOREL= .FALSE.  )
             ...
             DO ISPINOR=0,WDES%NRSPINORS-1
                ...
                DO NT=1, WDES%NTYP
                   DO NI=NIS, WDES%NITYP(NT)+NIS-1
                      ...
                      NPROP=NPRO_NI(NIP)+ISPINOR*(WDES%NPRO/2)
...
! rotate spinors
          ...
          DO N=1,WDES%NBANDS
             CALL ROTATE_WAVE_SPIN_RECIP(WDES, KPOINTS_TRANS%RSSYMOP(:,:,NK), &
                  NK, KPOINTS_TRANS%SPINFLIP(NK), W%CPTWFP(:,N,NK,ISP))
          ENDDO

          CALL SETWDES(WDES,WDES1,NK)

          DO N=1,WDES%NBANDS
            CALL ROTATE_WAVE_CHARACTER_SPIN(WDES1, W%CPROJ(:,N,NK,ISP), KPOINTS_TRANS%RSSYMOP(:,:,NK))
          ENDDO
...
!***********************************************************************
    ...
    SUBROUTINE POTXC2(GRID,LATT_CUR, XC, CHTOT0, DENCOR, CHTOT1, CWORK, E)

      USE prec
      USE base, only : CAST_RGRID_TO_COMPLEX
      USE mpimy
      USE mgrid
      USE pseudo
      USE lattice
      ...
      COMPLEX(q), TARGET :: CVTOT(GRID%MPLWV)
      COMPLEX(q), TARGET :: CWORK(GRID%MPLWV),CHTOT0_(GRID%MPLWV)
      COMPLEX(q) DENCOR(GRID%RL%NP)
      REAL(q) SUM, E, XCENC, EXC, XCSIF(3,3)
      COMPLEX(q) EZERO
      COMPLEX(q),pointer :: CCHTOT0_(:), CCWORK(:), CCVTOT(:)
      ...
      CALL CAST_RGRID_TO_COMPLEX(CHTOT0_,CCHTOT0_)
      CALL CAST_RGRID_TO_COMPLEX(CWORK,CCWORK)
      CALL CAST_RGRID_TO_COMPLEX(CVTOT,CCVTOT)

      CALL FEXCP(GRID,XC,LATT_CUR%OMEGA, &
               CCHTOT0_,DENCOR,CCWORK,CCVTOT,EZERO,EXC,XCENC,XCSIF,.FALSE.)
