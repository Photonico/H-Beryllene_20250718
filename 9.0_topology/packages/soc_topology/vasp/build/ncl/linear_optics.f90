...
MODULE mlr_optic
  ...
  USE lr_helper
  USE dfast
  USE chi_glb, ONLY: LFINITE_TEMPERATURE
  IMPLICIT NONE
...
!  (1._q,0._q) minus comes from i, the second (1._q,0._q) from partial differentiation
...
!$ACC DECLARE CREATE(CDER_BETWEEN_STATES)
!
...
!
!
  COMPLEX(q), ALLOCATABLE,   SAVE :: CDER_BETWEEN_STATES_Q(:,:,:,:)
!$ACC DECLARE CREATE(CDER_BETWEEN_STATES_Q)
...
!
  ...
  REAL(q), SAVE :: WPLASMON(3,3) = 0.0_q
...
!*********************************************************************
!
!> Calculates the macroscopic frequency dependent dielectric function
...
!*********************************************************************
  ...
  SUBROUTINE LR_OPTIC( &
    ...
    USE tutor, ONLY: vtutor, isAlert, TetNoMetal, FERWEOptics, MetaGGAResponse
    IMPLICIT NONE
!=======================================================================
!  structures
!=======================================================================
...
!=======================================================================
    ...
    IF ((KPOINTS%ISMEAR<=-4).AND.(isMetal(W%FERTOT))) THEN
       CALL vtutor%write(isAlert, TetNoMetal)
       
       RETURN
    ENDIF

    IF ((KPOINTS%ISMEAR==-2)) THEN
       CALL vtutor%write(isAlert, FERWEOptics)
...
! in most cases it is safer to recalculate the potential
    ...
    ENDIF
    IF (.NOT. LUSEPEAD() .AND. XC%LDOMETAGGA) THEN
       CALL vtutor%write(isAlert, MetaGGAResponse)
    ENDIF
...
! required optical transition elements might differ for MBPT with T>0
    IF ( LFINITE_TEMPERATURE ) THEN
       NBANDS_CDER=MIN(LAST_NEGATIVE_BAND_OPTICS(W,1/KPOINTS%SIGMA), WDES%NB_TOT)
    ELSE
       NBANDS_CDER=MIN( ((LAST_FILLED_OPTICS_NO_MOD(W)*2+WDES%NB_PAR-1)/W%WDES%NB_PAR)*W%WDES%NB_PAR, WDES%NB_TOT)
    ENDIF
    IF (LVEL) THEN
    ...
    CALL LRF_EPSILON_MELS( P, WDES, W, LATT_CUR, LATT_INI, T_INFO, INFO, IO, KPOINTS, SYMM, &
                           NONL_S, NONLR_S, IRDMAX, SV, DEG_CLUSTER, LUSEPEAD(), INFO%LOVERL, NBANDS_CDER, &
                           LMDIM, CDIJ, CQIJ, GRID, GRIDC, GRIDUS, C_TO_US, ENERGY_DER, CDER_BETWEEN_STATES )
    CALL LRF_EPSILON_COMPUTE( WDES, W, LATT_CUR, LATT_INI, INFO, IO, KPOINTS, SYMM, &
                              EFERMI, DELTAE, EMAX, EMAX_COND, NEDOS, &
                              ENERGY_DER, CDER_BETWEEN_STATES, &
...
!   CALL WRT_CDER_BETWEEN_STATES_FORMATTED(WDES,W,IO%IU0,IU=55)
    ...
    CALL XML_EPSILON_W_COMMENT(DELTAE, REAL(EPSDD,q), AIMAG(EPSDD), NEDOS, "density-density")
    CALL XML_EPSILON_W_COMMENT(DELTAE, REAL(EPSJJ,q), AIMAG(EPSJJ), NEDOS, "current-current")


    CALL VH5_WRITE_DIELECTRIC_ENERGIES(IH5OUTFILEID, DELTAE, NEDOS)
    CALL VH5_WRITE_DIELECTRIC_DYNAMIC(IH5OUTFILEID, EPSDD, "density_density")
    CALL VH5_WRITE_DIELECTRIC_DYNAMIC(IH5OUTFILEID, EPSJJ, "current_current")
...
!************************ FUNCTION LR_OPTIC_DONE ***********************
!
!>  Queries function whether CDER_BETWEEN_STATES is already set
!
!***********************************************************************
  LOGICAL FUNCTION LR_OPTIC_DONE()
    LR_OPTIC_DONE= ALLOCATED(CDER_BETWEEN_STATES)
  END FUNCTION

!************************ SUBROUTINE LR_OPTIC_OUTPUT *******************
!
!> Writes the results of the optics calculation in the outcar file
!
!***********************************************************************

  SUBROUTINE LR_OPTIC_OUTPUT(EPSDD,EPSJJ,WPLASMA_INTER,CON,BIMAG,NEDOS,DELTAE,KPOINTS,LATT_CUR,INFO,IO)
    USE constant
    ...
    COMPLEX(q), INTENT(IN) :: EPSDD(:,:,:) !< density-density response function
    COMPLEX(q), INTENT(IN) :: EPSJJ(:,:,:) !< velocity-velocity response function
    COMPLEX(q), INTENT(IN) :: WPLASMA_INTER(3,3)
    REAL(q), INTENT(IN) :: CON(3,3), BIMAG(3,3)
    INTEGER :: NEDOS
    REAL(q) :: DELTAE
! local variables
    INTEGER :: I, ISTEP
...
!  in CGS the conversion factor is h^2/m_e  (4 pi e^2)
  ...
  END SUBROUTINE LR_OPTIC_OUTPUT

!************************ SUBROUTINE FULL_KPOINT_AND_ENERGY_DER ********
!
!> Calculates the velocity operator
...
! temporary array to store k-points in extended BZ, eigenvalues and second energy derivative
    ...
    IF (KPOINTS_FULL%NKPTS/=KPOINTS_FULL%NKPX*KPOINTS_FULL%NKPY*KPOINTS_FULL%NKPZ) THEN
       CALL vtutor%write(isAlert, KPOINTS_HF)
       RETURN
    ENDIF
...
!=============================LRF_EPSILON_MELS===============================
!>
!> Computes optical matrix elements.
!>
!> This routine was formelly part of LRF_EPSILON which was split into
!> #LRF_EPSILON_MELS for computing the matrix elements and #LRF_EPSILON_COMPUTE summing over states
...
!>
!============================================================================
  SUBROUTINE LRF_EPSILON_MELS(P, WDES, W0, LATT_CUR, LATT_INI, T_INFO, INFO, IO, KPOINTS, SYMM, &
                              NONL_S, NONLR_S, IRDMAX, SV, DEG_CLUSTER, LPEAD, LOVERL, NBANDS_CDER, &
    ...
    USE msymmetry
    USE pead, ONLY : PEAD_DPSI_DK_IDIR
!USE kpoints_change
!USE fock
    IMPLICIT NONE
       ...
       DO ISP=1,WDES%ISPIN
          DO NK=1,WDES%NKPTS
             ...
             DO N1=1,WDES1%NB_TOT
                ENERGY_DER(N1,NK,ISP,IDIR)=REAL(W1%CELTOT(N1,NK,ISP),q)
             ENDDO
          ENDDO
       ENDDO
    ENDDO
...
!=============================LRF_EPSILON_COMPUTE============================
!>
!> Computes imaginary part of the macroscopic dielectric function
!>
!>  epsilon_q->0(G=0,G'=0,w)
!>
!> in the random phase approximation (independent particle approximation)
...
!>
!> This routine was formelly part of LRF_EPSILON which was split into
!> #LRF_EPSILON_MELS for computing the matrix elements and
!> #LRF_EPSILON_COMPUTE summing over states so that it can be re-used
!>
!============================================================================
  SUBROUTINE LRF_EPSILON_COMPUTE(WDES, W0, LATT_CUR, LATT_INI, INFO, IO, KPOINTS, SYMM, &
                                 EFERMI, DELTAE, EMAX, EMAX_COND, NEDOS, &
                                 ENERGY_DER, CHAM, &
                                 WPLASMON, CON, BIMAG, WPLASMA_INTER, EPSDD, EPSJJ, COND_ENERGY )
    USE base
    ...
    USE msymmetry
    USE mkpoints,  ONLY: kpoints_struct
    USE mlrf_main, ONLY: RTIME, WPLASMAI, LCSHIFT
    USE tutor,     ONLY: vtutor, isAlert, SmallCSHIFT
    IMPLICIT NONE
...
! input from previous routine
    REAL(q) :: EFERMI           !< fermi-energy
    REAL(q) :: EMAX, EMAX_COND  !< maximum energy
    INTEGER :: NEDOS
    REAL(q) :: DELTAE
    COMPLEX(qs)   :: CHAM(:,:,:,:,:)
    REAL(q) :: ENERGY_DER(:,:,:,:)
...
! Compute energy resolved conductivity
    ...
    IF (KPOINTS%ISMEAR >= -1) THEN
      DO IDIR=1,3
        DO JDIR=1,3
          ...
          EFERMI, NEDOS, EPSDD(:,IDIR,JDIR), EPSJJ(:,IDIR,JDIR), DELTAE, KPOINTS%ISMEAR, KPOINTS%SIGMA, &
          LATT_CUR%OMEGA, BIMAG(IDIR, JDIR), LCSHIFT)
          CALL CONDUCTIVITY( WDES, W0, ENERGY_DER(:,:,:,IDIR), ENERGY_DER(:,:,:,JDIR), EFERMI,&
          KPOINTS%ISMEAR, KPOINTS%SIGMA, LATT_CUR%OMEGA, WPLASMON(IDIR, JDIR), CON(IDIR, JDIR), RTIME)
        ENDDO
      ENDDO
      CALL M_sum_z(WDES%COMM, EPSDD, SIZE(EPSDD))
      CALL M_sum_z(WDES%COMM, EPSJJ, SIZE(EPSJJ))
      CALL M_sum_d(WDES%COMM, BIMAG, SIZE(BIMAG))
    ...
    ELSEIF (KPOINTS%ISMEAR <= -4) THEN
      DO IDIR=1,3
        DO JDIR=1,3
          ...
          NEDOS, EPSDD(:,IDIR,JDIR), EPSJJ(:,IDIR,JDIR), DELTAE, LATT_CUR%OMEGA, BIMAG(IDIR, JDIR), IO, INFO, KPOINTS)
        ENDDO
      ENDDO
! CON is presently not calculated by EPSILON_IMAG_TET
      CON=SUM(COND_ENERGY(NEDOS/2,:,:,:),3)
    ...
    IF (LWARN) THEN
       CALL vtutor%write(isAlert, SmallCSHIFT)
    ENDIF
...
! TODO: (1._q,0._q) should diagonalize WPLASMON and add contributions along principle axis
    IF (WPLASMAI>0) THEN
       DO IDIR=1,3
          DO I=1,NEDOS
! avoid (0._q,0._q) frequency
             W=DELTAE*(I-1)
             EPSDD(I,IDIR,IDIR)=EPSDD(I,IDIR,IDIR)+ &
                  WPLASMON(IDIR,IDIR)/(W**2+WPLASMAI**2)* &
                  CMPLX(-1.0_q,WPLASMAI/MAX(W,DELTAE),q)
...
!***********************************************************************
!
!> Yields .TRUE. if a state is empty
!
!***********************************************************************
  FUNCTION EMPTY(F)
    USE prec
...
!***********************************************************************
!
!> Yields .TRUE. if a state is filled
!
!***********************************************************************
  FUNCTION FILLED(F)
    USE prec
...
!
!> Determines the energy range of the unoccupied states with respect
...
! 40 eV seems to be a good safe guard
  ...
  FUNCTION MAX_ENERGY_OCC_UNOCCUPIED(WDES, W, NB)
    ...
    DO ISP=1,WDES%ISPIN
       DO NK=1,WDES%NKPTS
          DO N=1,MIN(NB,WDES%NB_TOTK(NK,ISP))
             EMIN=MIN(EMIN,REAL(W%CELTOT(N,NK,ISP),q))
             EMAX=MAX(EMAX,REAL(W%CELTOT(N,NK,ISP),q))
          ENDDO
       ENDDO
...
!************************ SUBROUTINE INPROD_W **************************
!
!> Calculates the inproduct between two sets of
...
!-----------------------------------------------------------------------
      ...
      IF (SIZE(CHAM,1) /= NB_TOT .OR. SIZE(CHAM,2) /= NB_TOT_W1) THEN
         CALL vtutor%bug("internal error in INPROD_W: shape not conform " // str(SIZE(CHAM,1)) // &
            " " // str(NB_TOT) // " " // str(SIZE(CHAM,2)) // " " // str(NB_TOT_W1), "linear_optics.F", 1148)
      ENDIF
...
!***********************************************************************
!
!> Performs a unitary transformation between the optical matrix elements
...
!
!> Calculates the imaginary part of the dielectric function
...
!***********************************************************************
    ...
    SUBROUTINE EPSILON_IMAG( WDES, W0, CHAM1, CHAM2, EFERMI, &
      NEDOS, EPSDD, EPSJJ, DELTAE, ISMEAR, SIGMA, OMEGA, BIMAG, CSHIFT)
      USE wave
      USE constant
      IMPLICIT NONE
      TYPE (wavedes)     WDES
...
! the polarizability is then given by    4 pi^2 e^2/omega  <u_m|  -i d/dk_j u_n>^* <u_m|  -i d/dk_j u_n>
! (1._q,0._q) can also calculate the polarizability from the related velocity operator
...
! TODO gK: 28.12.16 conjugated to have same sign as Kunes, Marini on off diagonal terms e.g. xy
            A =CHAM1(N2,N1,NK,ISP)*CONJG(CHAM2(N2,N1,NK,ISP))
! conversion factor 4 pi^2 e^2/ volume to obtain polarization
            ...
            CALL SLOT( REAL(DECEL,q), ISMEAR, SIGMA, NEDOS, DELTAE,  WEIGHT*A*CONST*DECEL*DECEL, EPSJJ)
...
! (factor 2/PI compare calculation of WPLASMA_INTER in EPSILON_REAL)
    ...
    FUNCTION F(E,SIG)
      REAL(q) F,E,SIG
! IF (E / SIG > 709.78_q) THEN; F = 0.0_q; RETURN; END IF ! Breaks FeAl_ACFDTR_T (gnu), overflow in exp possible.
      F=  1/(1 + EXP(E /SIG))
    END FUNCTION F

    FUNCTION G(E,SIG)
      REAL(q) G,E,SIG
! IF (E / SIG < -709.78_q) THEN; G = 0.0_q; RETURN; END IF ! Breaks FeAl_ACFDTR_T (gnu), overflow in exp possible.
...
!
!> Calculates the energy resolved conductivity tensor
!
!***********************************************************************

    SUBROUTINE CONDUCTIVITY_ENERGY_RESOLVED( WDES, W0, ENERGY_DER,  EFERMI, &
         NEDOS, EMAX_COND, COND_ENERGY, NELECT, NUP_DOWN, KPOINTS, SYMM, LATT_CUR, TAU)
      USE base, ONLY: symmetry
      USE msymmetry, ONLY: latt
      USE wave
      USE constant
      USE mkpoints
      IMPLICIT NONE
...
! symmetry related quantities (common block)
      ...
      DOS = 0.0_q; DOSI = 0.0_q; SIGMA = 0.0_q
...
! TODO adapt to spin orbit coupled case
            DO IDIR=1,3
               DO JDIR=1,3
                  SIGMA(N1,NK,IDIR,JDIR,ISP)=ENERGY_DER(N1,NK,ISP,IDIR)*ENERGY_DER(N1,NK,ISP,JDIR)*(4*PI*FELECT/LATT_CUR%OMEGA)*ABS(TAU)*CONTCON
               ENDDO
            ENDDO
         ENDDO
      ENDDO
...
!
!> Writes CDER_BETWEEN_STATES to a formatted file
!
!***********************************************************************
...
! somewhat dirty, if CDER_BETWEEN_STATES_Q is allocated call
      ...
      IF (N2 > NBANDS_CDER) THEN
         CALL vtutor%bug("internal error in CDER_BETWEEN_STATES_ROTATED: optical matrix elements not &
            &calculated " // str(NK) // " " // str(N1) // " " // str(N2) // " " // str(NBANDS_CDER), &
            "linear_optics.F", 1737)
      ENDIF
...
!      WRITE(*,'(3F14.7)') CDER_BETWEEN_STATE(:)
    ...
    SUBROUTINE CDER_BETWEEN_STATES_ROTATED_MU(CDER_BETWEEN_STATE, LATT_CUR, NK, ISP, N1_IN, N2_IN, NSIM)
...
!! USE moffload
      ...
      USE kpoints_change
      IMPLICIT NONE
      INTEGER NK, ISP, N1_IN, N2_IN, N, NSIM
      INTEGER IDIR
      COMPLEX(q) CDER_BETWEEN_STATE(3,NSIM)
      TYPE (latt)        LATT_CUR
! local
      INTEGER NK_OLD, ISP_OLD, L, K, J, I, IDIR_OLD, N1, N2
      REAL(q) :: S(3,3)
      LOGICAL :: LINV, LINV_NK

!$ACC KERNELS PRESENT(CDER_BETWEEN_STATE) 
      CDER_BETWEEN_STATE=0
!$ACC END KERNELS
      ...
      IF (KPOINTS_FULL_ORIG%SPINFLIP(NK)==1) THEN
         ISP_OLD=3-ISP
      ENDIF
      LINV_NK =  KPOINTS_FULL_ORIG%LINV(NK)
...
!$ACC PARALLEL LOOP PRESENT(LATT_CUR,S,KPOINTS_FULL_ORIG%ISYMOP)  
      DO L=1,3
         DO K=1,3
            DO J=1,3
               DO I=1,3
...
!$ACC PARALLEL LOOP PRESENT(S,CDER_BETWEEN_STATE,CDER_BETWEEN_STATES) 
      DO N=1,NSIM
         IF (N1_IN<=N2_IN+N-1) THEN
            N1=N2_IN+N-1
            N2=N1_IN
            LINV=.NOT. LINV_NK
         ELSE
         ...
         IF (N2 > NBANDS_CDER .OR. N1 > SIZE(CDER_BETWEEN_STATES,1) ) CDER_BETWEEN_STATE(:,N)=0

! a vector in direction IDIR_OLD will rotate into a vector
!  S(:,IDIR_OLD)
         DO IDIR_OLD=1,3
            CDER_BETWEEN_STATE(:,N)=CDER_BETWEEN_STATE(:,N)+S(:,IDIR_OLD)*CDER_BETWEEN_STATES(N1,N2,NK_OLD,ISP_OLD,IDIR_OLD)
         ENDDO

         IF (LINV) CDER_BETWEEN_STATE(:,N)=CONJG(CDER_BETWEEN_STATE(:,N))
      ENDDO
!$ACC EXIT DATA DELETE(S)  

         
      
    END SUBROUTINE CDER_BETWEEN_STATES_ROTATED_MU

!***********************************************************************
!  batched verison of CDER_BETWEEN_STATES_ROTATED_Q
...
!***********************************************************************
    ...
    SUBROUTINE CDER_BETWEEN_STATES_ROTATED_Q_MU(CDER_BETWEEN_STATE, LATT_CUR, NK, ISP, N1_IN, N2_IN, NSIM)
      USE lattice
      USE full_kpoints
      USE kpoints_change
      IMPLICIT NONE
      ...
      TYPE (latt)        LATT_CUR
! local
      INTEGER NK_OLD, ISP_OLD, L, K, J, I, N1, N2, N, NSIM
      LOGICAL :: LINV
...
!$ACC PARALLEL LOOP PRESENT(CDER_BETWEEN_STATE,CDER_BETWEEN_STATES_Q)
      DO N=1,NSIM
         LINV=.FALSE.
         IF (N1_IN<=N2_IN+N-1) THEN
            N1=N2_IN+N-1
         ...
         CDER_BETWEEN_STATE(:,N)=CDER_BETWEEN_STATES_Q(N1, N2, NK, ISP)
         IF (LINV) CDER_BETWEEN_STATE(:,N)=CONJG(CDER_BETWEEN_STATE(:,N))
      ENDDO

      
    END SUBROUTINE CDER_BETWEEN_STATES_ROTATED_Q_MU

!***********************************************************************
!> Allocates CDER_BETWEEN_STATES_Q
!***********************************************************************
...
!***********************************************************************
!> Deallocates CDER_BETWEEN_STATES_Q
!***********************************************************************
...
!***********************************************************************
!
!>  Stores the transition matrix elements
...
! local
      ...
      COMPLEX(qs), ALLOCATABLE ::  C(:,:),C0(:,:),CTRANS_single(:,:)
      COMPLEX(qs), ALLOCATABLE, SAVE :: CTMP(:,:,:)
      INTEGER NB_TOT,NB
      ...
      IF ( SIZE(CDER_BETWEEN_STATES,1) /= SIZE(CTRANS,1) ) THEN
         CALL vtutor%error("ERROR in CDER_CRPA: # of bands dont match " // &
            str(SIZE(CDER_BETWEEN_STATES,1)) // " " // str(SIZE(CTRANS,1)))
      ENDIF
      IF ( SIZE(CDER_BETWEEN_STATES,2) /= NBANDS_CDER) THEN
         CALL vtutor%error("ERROR in CDER_CRPA: # of NBANDS_CDER does not match " // &
            str(SIZE(CDER_BETWEEN_STATES,2)) // " " // str(NBANDS_CDER))
      ENDIF
      IF ( SIZE(CDER_BETWEEN_STATES,4) /= SIZE(CTRANS,4) ) THEN
         CALL vtutor%error("ERROR in CDER_CRPA: # of spins dont match " // &
            str(SIZE(CDER_BETWEEN_STATES,4)) // " " // str(SIZE(CTRANS,4)))
      ENDIF
...
!for (1._q,0._q) matrix element I, J, this is 1._q by
     DO I = 1, NB_TOT
        DO J = 1, NB_TOT
...
!note that only the "wings and head of width NBANDS_CDER" are non-(0._q,0._q)
           IF ( N2 > NBANDS_CDER ) THEN
           ...
           ELSE
              DO IDIR = 1, 3
                 DO IDIR_OLD=1,3
                    CTMP(I, J, IDIR)=CTMP(I, J, IDIR)+S(IDIR,IDIR_OLD)*&
                                   CONJG(CDER_BETWEEN_STATES(N1,N2,NK_OLD,ISP_OLD,IDIR_OLD))
                 ENDDO
              ENDDO
           ENDIF

           IF (LINV) THEN
              CTMP(I, J, : )=CONJG(CTMP( I, J, : ))
           ENDIF
        ENDDO
     ENDDO
...
!it turs out that the complex conjugate is the correct element
      CDER_BETWEEN_STATE(:) = CONJG(CTMP(N1_IN,N2_IN,:))
      RETURN
...
!***********************************************************************
!>
!> Adds a value VALUE at the energy EPS to a density of states array DOS
!>
!***********************************************************************
...
!***********************************************************************
    ...
    SUBROUTINE EPSILON_IMAG_TET( WDES, W0, CHAM1, CHAM2, EMAX, NEDOS, DOS, DOSJ, DELTAE, &
                   OMEGA, BIMAG, IO, INFO, KPOINTS)
      USE wave
      USE constant
      USE base
      USE kpoints_change
      ...
      INTEGER NEDOS
!local
      INTEGER ISP, NK, N1, N2, NC, NTRANS, NKPTS, NBCON, NBVAL, NBFULL, NBTOT, J
      REAL(q) EMAX, CONST, DECEL
      TYPE (tetrahedron_calculator) CALCULATOR
      REAL(q), ALLOCATABLE :: AMP(:,:,:,:),ETRANS(:,:,:),DOST(:,:,:),ENERGIES(:)


      DOS    = 0._q
      DOSJ   = 0._q
      CONST  = EDEPS * PI / OMEGA
      ALLOCATE(ENERGIES(SIZE(DOS)))
      ENERGIES = [((J-1) * EMAX / (SIZE(DOS) - 1), J = 1, SIZE(DOS))]
...
! test_
         ...
         ALLOCATE( AMP(NTRANS,NKPTS,2,1), ETRANS(NTRANS,NKPTS,1) )
         ALLOCATE( DOST(NEDOS,2,1) )
         AMP    = 0._q
         ETRANS = 0._q
         ...
         DO NK = 1, NKPTS
            DO N1 = 1, NBVAL
               DO N2 = 1, NBCON
                  ...
                  NC = NBFULL + N2
                  IF (NC>N1) THEN
                     DECEL = W0%CELTOT(NC,NK,ISP) - W0%CELTOT(N1,NK,ISP)
                     AMP(J,NK,1,1) = CONJG(CHAM1(NC,N1,NK,ISP)) * CHAM2(NC,N1,NK,ISP) &
                        * CONST * (W0%FERTOT(N1,NK,ISP)-W0%FERTOT(NC,NK,ISP))
                     AMP(J,NK,2,1) = AMP(J,NK,1,1) * DECEL**2
                     ETRANS(J,NK,1) = DECEL
                  END IF
               ENDDO
            ENDDO
         ...
         CALCULATOR = NEW_CALCULATOR(ETRANS, WDES%RSPIN, KPOINTS%VOLWGT * KPOINTS%IDTET(0,:), &
            KPOINTS%IDTET(1:4,:), WDES%COMM, USE_DELTAS=(KPOINTS%ISMEAR==-15.OR.KPOINTS%ISMEAR==-14))
         CALL CALCULATOR%DOS_WITH_WEIGHT(ENERGIES, AMP, DOST)
         DOS(:) = DOS(:) + DOST(:,1,1)
         DOSJ(:) = DOSJ(:) + DOST(:,2,1)

         DEALLOCATE(AMP, ETRANS, DOST)
      ENDDO
...
!
!> Calculates the real part of the dielectric function
...
# 2556
      ...
      CALL M_sum_z(COMM, EPS_REAL, SIZE(EPS_REAL))
...
! valued
      ...
      IF (ABS(CSHIFT) < DELTAE .AND. LCOPY) THEN
         EPS_REAL=CMPLX(REAL(EPS_REAL,q),REAL(EPSDD,q),q)
      ENDIF
...
!
!> Calculates the commutator of the [H_x,r]  phi_i
!>
...
!***********************************************************************
    ...
    SUBROUTINE FOCK_K_DER_ANALYT(KPOINTS, GRID, LATT_CUR, LATT_INI, T_INFO,  &
      ...
      USE constant
      USE choleski
      USE tutor, ONLY: vtutor, isError, GW_HFoptics
      IMPLICIT NONE
...
! passed variables
  ...
  END INTERFACE
      ...
      IF (NONLR_S%LREAL) THEN
         CALL vtutor%write(isError, GW_HFoptics)
      ENDIF
...
!> instead #FOCK_K_DER_ANALYT relying on an analytical derivative
!> should be used.
!> @warning The finite difference version fails for the Gamma-point only
!
!***********************************************************************
...
!    local
      ...
      IF (W%WDES%COMM_KINTER%NCPU.NE.1) THEN
         CALL vtutor%error("FOCK_K_DER: KPAR>1 not tested (but implemented), sorry.")
      END IF
...
! round to next larger value modulo W%WDES%NB_PAR
  ...
  FUNCTION LAST_FILLED_OPTICS_THRESH( W, EPS)
    USE prec
    USE wave
    IMPLICIT NONE
    TYPE (wavespin) :: W
    REAL(q)         :: EPS
!local
    INTEGER LAST_FILLED_OPTICS_THRESH
    INTEGER K1, ISP
    INTEGER NB, NB_MAX
    ...
    DO ISP=1,W%WDES%ISPIN
       DO K1=1,W%WDES%NKPTS
          DO NB=W%WDES%NB_TOT, 1, -1
             IF (W%FERTOT( NB, K1, ISP)>EPS ) EXIT
          ENDDO
          NB_MAX=MAX(NB,NB_MAX)
       ENDDO
    ENDDO
...
! round to next larger value modulo W%WDES%NB_PAR
    LAST_FILLED_OPTICS_THRESH=((NB_MAX+W%WDES%NB_PAR-1)/W%WDES%NB_PAR)*W%WDES%NB_PAR
  END FUNCTION LAST_FILLED_OPTICS_THRESH

!***********************************************************************
!>  Returns the last filled band
!>  modulo the number of bands 1._q in parallel for T>0
!***********************************************************************

  FUNCTION LAST_NEGATIVE_BAND_OPTICS( W, BETA )
    USE prec
    USE wave
    USE chi_glb, ONLY : GPOS_THRESHHOLD
    IMPLICIT NONE
    TYPE (wavespin) W
    INTEGER LAST_NEGATIVE_BAND_OPTICS
    INTEGER K1, ISP
    REAL(q) :: BETA
! local
    INTEGER NB, NB_MAX
    ...
    DO ISP=1,W%WDES%ISPIN
       DO K1=1,W%WDES%NKPTS
          DO NB=W%WDES%NB_TOT, 1, -1
             IF ( LFINITE_TEMPERATURE ) THEN
                EXIT
!IF ( REAL(W%CELTOT( NB, K1, ISP)-W%EFERMI(ISP),q)*BETA < GPOS_THRESHHOLD ) EXIT
             ELSE
                IF (W%FERTOT( NB, K1, ISP)>0.05 ) EXIT
             ENDIF
          ENDDO
...
! round to next larger value modulo W%WDES%NB_PAR
    LAST_NEGATIVE_BAND_OPTICS=((NB_MAX+W%WDES%NB_PAR-1)/W%WDES%NB_PAR)*W%WDES%NB_PAR
    

  END FUNCTION LAST_NEGATIVE_BAND_OPTICS
...
!***********************************************************************
!>  Returns the first (partially) empty band
!***********************************************************************
...
!
!> Writes CDER_BETWEEN_STATES  to a file.
!> It is required for GW calculations.
!
!***********************************************************************
...
!************************ SUBROUTINE WRT_CDER_BETWEEN_STATES ***********
!
!> Reads CDER_BETWEEN_STATES from a file
!>
...
!***********************************************************************
    ...
    SUBROUTINE READ_CDER_BETWEEN_STATES(WDES, IU0, IU, EXT)
      ...
      USE mlrf_main
      USE chi_glb
      USE tutor, ONLY: vtutor, isAlert, argument, GWoptics, BSEoptics
...
! local
      ...
      COMPLEX(qs),ALLOCATABLE :: CDER_TMP(:,:,:,:,:)
...
!allow for downsampling from denser k-grids
      ...
      IF ( ALLOCATED(IKPT_SELECT))THEN
         ...
         IF (IU0>=0) THEN
            ...
            CLOSE(IU)
         ENDIF
         CALL M_bcast_i(WDES%COMM,NKPTS,1)
         IF ( SIZE(IKPT_SELECT)/=NKPTS) THEN
!            IF(IU0>=0) WRITE(IU0,*)'ERROR in READ_CDER_BETWEEN_STATES:'//&
!               ' number of kpoints not compatible', SIZE(IKPT_SELECT),NKPTS
            LREAD=.FALSE.
         ENDIF
      ...
      IF (IU0>=0) THEN
         ...
         IF (IERR==0) THEN
           ...
           IF (NB_TOT < WDES%NB_TOT .AND. .NOT. LBSE) THEN
              WRITE(IU0,'(A)')' ----------------------------------------------------------------------------- '
              WRITE(IU0,'(A,I8,I8)') ' WAVEDER not read: bands not compatible',NB_TOT, WDES%NB_TOT
              IERR=1
           ENDIF

           IF (NB_TOT /= WDES%NB_TOT .AND. LBSE) THEN
              NBANDS = MIN(NB_TOT,WDES%NB_TOT)
              CALL vtutor%write(isAlert, BSEoptics, argument(ival=[NB_TOT, WDES%NB_TOT, NBANDS]))
              IERR=0
           ENDIF
           ...
           IF ((NKPTS /= WDES%NKPTS .AND. NKPTS /= NKPTS_NON_ZERO) .AND. .NOT. ALLOCATED(IKPT_SELECT)) THEN
              WRITE(IU0,'(A)')' ----------------------------------------------------------------------------- '
         ...
         IF (IERR==0) THEN
            ALLOCATE(CDER_BETWEEN_STATES(NBANDS, NBANDS_CDER, WDES%NKPTS, WDES%ISPIN, 3))
            CALL REGISTER_ALLOCATE(16._q*SIZE(CDER_BETWEEN_STATES,KIND=qi8), "cder")
            CDER_BETWEEN_STATES=0
!store CDER appropriately
            IF ( ALLOCATED(IKPT_SELECT) .AND. LREAD )THEN
               ALLOCATE(CDER_TMP(NB_TOT, NBANDS_CDER, NKPTS, WDES%ISPIN, 3))
               CDER_TMP=0
               READ(IU,IOSTAT=IERR) CDER_TMP(1:NB_TOT,1:NBANDS_CDER,1:NKPTS,1:WDES%ISPIN,1:3)
               NKPTS_COUNTER = 0
               DO I=1,NKPTS
                  IF ( IKPT_SELECT(I)>0)THEN
                     NKPTS_COUNTER = NKPTS_COUNTER + 1
! if NB_TOT>WDES%NB_TOT we may copy the corresponding subset 1:WDES%NB_TOT only
                     CDER_BETWEEN_STATES(1:NBANDS,:,IKPT_SELECT(I),:,:)=CDER_TMP(1:NBANDS,:,I,:,:)
                  ENDIF
               ENDDO
               DEALLOCATE(CDER_TMP)
            ...
            ELSE
               ALLOCATE(CDER_TMP(NB_TOT, NBANDS_CDER, NKPTS, WDES%ISPIN, 3))
               CDER_TMP=0
! first read WAVEDER into array CDER_TMP
               READ(IU,IOSTAT=IERR) CDER_TMP(1:NB_TOT,1:NBANDS_CDER,1:NKPTS,1:WDES%ISPIN,1:3)
! ... and then copy again subset 1:WDES%NB_TOT only into CDER_BETWEEN_STATES
               CDER_BETWEEN_STATES(1:NBANDS,:,1:NKPTS,:,:)=CDER_TMP(1:NBANDS,:,1:NKPTS,:,:)
               DEALLOCATE(CDER_TMP)
            ENDIF
         ENDIF
         CLOSE(IU)

         CALL M_bcast_i(WDES%COMM, NBANDS, 1)
