...
!!   4) compute matrix elements for passed screened coulomb kernel
!!      supports data distribution of screened kernels as used in  the
!!      GW spacetime routines (column wise distribuion within
!!      NOMEGAPAR frequency groups)
!
!*********************************************************************

   SUBROUTINE COULOMB_MATRIX_ELEMENTS( &
      WDES,W,KPOINTS,GRID,T_INFO,INFO,P,NONL_S,NONLR_S,SYMM,LATT_CUR,CQIJ,LMDIM,&
      WPOT_DIST, IMAG_GRIDS, S2E, FSG0, IO)
     USE base
     USE paw, ONLY: MIMIC_US
     USE lattice
     USE pseudo
     USE poscar
     USE mgrid
...
! local variables
     ...
     TYPE(wpothandle), POINTER, SAVE :: CRPA_WPOTH => NULL() ! CRPA_WPOTH stores the screened Coulomb potential at each q-point
     COMPLEX(q), ALLOCATABLE :: WIJKL(:,:,:,:,:,:,:)
     COMPLEX(q), ALLOCATABLE :: VIJKL(:,:,:,:,:,:)
     COMPLEX(q), ALLOCATABLE :: CIJKL(:,:,:,:,:,:)   !correction
     COMPLEX(q), ALLOCATABLE :: PIJKL(:,:,:,:)
     COMPLEX(q), ALLOCATABLE :: UIJKL(:,:,:,:,:,:)
     ...
     INTEGER NKPTS_IRZ
     INTEGER IOMEGA  
     INTEGER LMAX_FOCKAE_STORE
     TYPE(wavedes) WDES_LOC
     TYPE(wavespin) W_LOC

     
     IF ( CRPA_NUM_WAN  < 1 ) THEN
        IF ( IO%IU0 >=0 ) WRITE(IO%IU0,*) &
          'CRPA_NUM_WAN not set in COULOMB_MATRIX_ELEMENTS, skipping calculation'
        
        RETURN
     ENDIF

! allocate storage for localized wavefunction
     CALL ALLOCW( W%WDES, W_LOC)
     W_LOC%WDES  =W%WDES
! copy wavefunction to W_LOC, W_LOC is going to be the projected wave function
     W_LOC%CPTWFP=W%CPTWFP
     W_LOC%CPROJ =W%CPROJ 
     W_LOC%FERWE =W%FERWE
     W_LOC%AUX   =W%AUX
     W_LOC%CELEN =W%CELEN
     W_LOC%FERTOT=W%FERTOT
     W_LOC%AUXTOT=W%AUXTOT
     W_LOC%CELTOT=W%CELTOT
     W_LOC%OVER_BAND=W%OVER_BAND
! copy descriptor as well
     WDES_LOC    =W_LOC%WDES

! get projection matrices.
! if they have not been determined yet (for full RPA for instance)
! CRPA_SETUP has not been called and CRPA_NUM_WAN == -1
     IF ( .NOT. LUFROMFILE .AND. CRPA_NUM_WAN < 1 ) THEN
...
# 160
           ...
           CALL MLWF_MAIN(WDES_LOC,W_LOC,P,KPOINTS,CQIJ,T_INFO,LATT_CUR,INFO,IO,MLWF_GLOBAL)
           CRPA_NUM_WAN = MLWF_GLOBAL%NUM_WANN
        ELSEIF ( LUSELOCPROJ ) THEN
! pull breaks here, if VASP was not compiled with wannier90 support
           CALL vtutor%error("LOCPROJ, currently not supported, sorry")
        ENDIF 
     ENDIF 

! project orbitals to localized basis set
     CALL PROJECT_ORBITALS_NOSYMM(WDES_LOC,W_LOC,IO%IU0)
    
!set Target states, i.e. range of bands
!for which matrix elements will be caluclated
     CALL SET_TARGET_STATES( NKPTS_IRZ, IO )
     IF ( VCUTOFF < 0 ) VCUTOFF = MAX(ENCUTGW, 1.1_q*W%WDES%ENMAX)

! allocate W_ijkl
     ALLOCATE(WIJKL(NWTOT,NWTOT,NWTOT,NWTOT,WDES_LOC%ISPIN,WDES_LOC%ISPIN,IMAG_GRIDS%B%NPOINTS))
     ALLOCATE(VIJKL(NWTOT,NWTOT,NWTOT,NWTOT,WDES_LOC%ISPIN,WDES_LOC%ISPIN))
     IF ( LCRPA .OR. L2E4W ) THEN
        ALLOCATE(CIJKL(NWTOT,NWTOT,NWTOT,NWTOT,WDES_LOC%ISPIN,WDES_LOC%ISPIN))
     ENDIF
     WIJKL=0

!in the first run we compute V exactly
!using Gaunt coeffiecients and Slater integrals
     LMAX_FOCKAE_STORE=LMAX_FOCKAE
     IF (MIMIC_US) THEN
       LMAXMP2=-1
     ELSE
       LMAXMP2=4
...
!=======================================================================
    IF (L2E4W_ALL) THEN
       ...
       IF (I/=KPOINTS_FULL%NKPTS) THEN
          CALL vtutor%bug("internal error in COULOMB_MATRIX_ELEMENTS: something is at odds with the " &
             // "KPOINTS " // str(I) // " " // str(KPOINTS_FULL%NKPTS), "dmft.F", 214)
       ENDIF
    ENDIF
...
! bare coulomb interaction is not distributed
        CALL CALCULATE_WIJKL_TWOCENTRE( &
           WDES_LOC,W_LOC,KPOINTS,LATT_CUR,T_INFO,P,WGW,NWLOW,NWHIGH,COMEGA,LMAXMP2,IO,CIJKL)
     ENDIF 

!reduce cutoff and compute V using LMAXFOCKE = 4 and LMAXMP2 = -1
     LMAXMP2=-1
     LMAX_FOCKAE=LMAX_FOCKAE_STORE

     DEALLOCATE( WGW, GRIDWGW)
     CALL SET_CUTOFF( ENCUTGW )
...
! bare coulomb interaction is not distributed
        CALL CALCULATE_WIJKL_TWOCENTRE( &
           WDES_LOC,W_LOC,KPOINTS,LATT_CUR,T_INFO,P,WGW,NWLOW,NWHIGH,COMEGA,LMAXMP2,IO,VIJKL)

!write matrix elements of bare interaction to VIJKL using MP2
        CALL WRITE_DMFT_MATRIX_ELEMENTS(IO,WDES_LOC%ISPIN,.FALSE.,CIJKL,VIJKL,COMEGA=COMEGA)
! dump density matrix in localized basis
        CALL DUMP_ONE_ELECTRON_FCIINPUT( WDES, W, WDES_LOC, CIJKL, IO ) 
...
!print diragram
     ...
     NULLIFY(CRPA_WPOTH)
     CALL INIT_WPOT_HANDLE_DISTRIBUTED( CRPA_WPOTH, IMAG_GRIDS, S2E%NUMBER_OF_NQ, FSG0, &
        WGW, KPOINTS_FULL%NKPTS, IO%IU6, IO%IU0, 1, 1, 1 )
     WIJKL=0
! loop over frequencies for screened coulomb kernel
     CALL WIJKL_TWOCENTRE_DISTRIBUTED( WDES_LOC,W_LOC,KPOINTS,LATT_CUR,T_INFO,P,&
        WGW,NWLOW,NWHIGH,LMAXMP2,CRPA_WPOTH,WPOT_DIST,IO,WIJKL)
!CALL CALCULATE_WIJKL_TWOCENTRE( &
!   WDES_LOC,W_LOC,KPOINTS,LATT_CUR,T_INFO,P,WGW,NWLOW,NWHIGH,COMEGA,-1,IO,WIJKL(:,:,:,:,:,:,1))
!add the correction to VIJKL and UIJKL
     DO N1=1,IMAG_GRIDS%NOMEGA
        WIJKL(:,:,:,:,:,:,N1) = WIJKL(:,:,:,:,:,:,N1) + CIJKL(:,:,:,:,:,:)
!write matrix elements
        COMEGA = IMAG_GRIDS%BOS_RE(N1)*(0._q,1._q)
        CALL WRITE_DMFT_MATRIX_ELEMENTS(IO,WDES_LOC%ISPIN,.TRUE.,WIJKL(:,:,:,:,:,:,N1),&
        COMEGA=COMEGA)
     ENDDO

! get rid of distribued wpot handle
     CALL DESTROY_DISTRIBUTED_WPOTH( CRPA_WPOTH )
     DEALLOCATE( WGW, GRIDWGW )
     DEALLOCATE( VIJKL , CIJKL,  WIJKL ) 
...
!*****************************************************************
! sets cutoff for computation of screened Coulomb interaction
!*****************************************************************
     SUBROUTINE SET_CUTOFF( CUTOFF )
        REAL(q)  CUTOFF 
...
!  init FFT (required if real to complex FFT is used)
        ...
        IF ( ABS( CUTOFF - ENCUTGW ) < 1E-6_q ) THEN
! soft cutoff on kinetic energy
   ...
   END SUBROUTINE COULOMB_MATRIX_ELEMENTS
...
!a projection is necessary only if LOCPROJs or MLWFs are used
     CALL PROJECT_ORBITALS_NOSYMM(WDES,W,IO%IU0)
...
! change the frequency grid point if necessary
     IF (OMEGAMAX>0.AND.NOMEGA==1) THEN
        IF ( NOMEGAR == 0 ) THEN
           COMEGA=OMEGAMAX*(0._q,1._q)
        ELSE
           COMEGA=OMEGAMAX*(1._q,0._q)
        ENDIF
     ELSE
        COMEGA=(0.0_q,0.0_q)
     ENDIF
...
!=======================================================================
    IF (L2E4W_ALL) THEN
       ...
       ENDDO
       IF (I/=KPOINTS_FULL%NKPTS) THEN
          CALL vtutor%bug("internal error in DMFT_MATRIX_ELEMENTS: something is at odds with the " &
             // "KPOINTS " // str(I) // " " // str(KPOINTS_FULL%NKPTS), "dmft.F", 514)
       ENDIF
    ENDIF
...
!write matrix elements of bare interaction to VIJKL using MP2
        CALL WRITE_DMFT_MATRIX_ELEMENTS(IO,WDES%ISPIN,.FALSE.,CIJKL,WIJKL,COMEGA=COMEGA)
! dump density matrix in localized basis
        CALL DUMP_ONE_ELECTRON_FCIINPUT( WDES, W, WDES, CIJKL, IO ) 
...
!write matrix elements
        CALL WRITE_DMFT_MATRIX_ELEMENTS(IO,WDES%ISPIN,.TRUE.,WIJKL,UIJKL,PIJKL,COMEGA=COMEGA)
...
!write matrix elements
        CALL WRITE_DMFT_MATRIX_ELEMENTS(IO,WDES%ISPIN,.TRUE.,WIJKL,COMEGA=COMEGA)
     ENDIF 
...
!*********************************************************************
 ...
 &  WDES,W,KPOINTS,LATT_CUR,T_INFO,P,WGW,NB_START,NB_STOP,LMAX,WIJKL,IO)
    ...
    INTEGER LMAX                 ! maximum L index for (1._q,0._q) center terms
    COMPLEX(q), OPTIONAL :: WIJKL(:,:,:,:)

! local variables
    COMPLEX(q), ALLOCATABLE :: TWOe4o(:,:,:,:),TWOe4o_ADD(:,:,:,:)
    TYPE (wavespin) WHF
...
!==========================================================================
    ...
    CALL M_sum_z(WGW%COMM_INTER, TWOe4o, NGLB*NGLB*NGLB*NGLB)
...
! local
    ...
    COMPLEX(q), ALLOCATABLE :: TWOe4o_BCAST(:,:,:,:)
    ...
    DO N=1,WGW%COMM_INTER%NCPU
       IF (N==WGW%COMM_INTER%NODE_ME) THEN
          ...
          K_BCAST(4)=K4
       ENDIF
       CALL M_bcast_i_from(WGW%COMM_INTER, K_BCAST, 4, N )
# 1080

       CALL M_bcast_z_from(WGW%COMM_INTER, TWOe4o_BCAST, SIZE(TWOe4o_BCAST), N )
...
! local
    ...
    COMPLEX(q), ALLOCATABLE :: E2W4_ALL(:,:,:,:,:,:,:)
...
!===========================================================================
! write out entire matrix in (1._q,0._q) file
...
# 1214
       ...
       END DO
       CALL M_sum_z(WGW%COMM_INTER, E2W4_ALL, SIZE(E2W4_ALL))
...
! the same should hold for k3-k2 (=q)
    ...
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("TWOELECTRON4O_WIJKL: internal error: q-point changed " // str(NQ) // " " // &
          str(NQ_), "dmft.F", 1334)
    ENDIF
...
!       but GCHG14 is conjugated in the ZGEMM routine.
!==========================================================================
    CHAM=0

    CALL ZGEMM('C','N',  NGLB*NGLB, NGLB*NGLB,  NP, -(1._q,0._q), &
         GCHG14(1,1),  SIZE(GCHG14,1), GCHG23(1,1),  SIZE(GCHG23,1), &
         (0._q,0._q), CHAM(1,1,1,1), SIZE(CHAM,1)*SIZE(CHAM,2))
    NFLOAT=NFLOAT+8._q*NGLB*NGLB*NGLB*NGLB*NP
    ...
    IF (ASSOCIATED(H).AND..NOT.ASSOCIATED(WPOTH)) THEN
       ...
       CALL ZGEMM('C','N',  NGLB*NGLB, NGLB*NGLB, H%TOTAL_ENTRIES, -(1._q,0._q), &
            CRHO14(1,1), SIZE(CRHO14,1), CRHO23(1,1), SIZE(CRHO23,1), &
            (1._q,0._q) , CHAM(1,1,1,1), SIZE(CHAM,1)*SIZE(CHAM,2))
       NFLOAT=NFLOAT+8._q*NGLB*NGLB*NGLB*NGLB*H%TOTAL_ENTRIES
    ENDIF
!==========================================================================
...
!*********************************************************************
  ...
  SUBROUTINE CALCULATE_WIJKL_TWOCENTRE( &
    ...
    INTEGER LMAX                 ! maximum L index for (1._q,0._q) center terms
    COMPLEX(q), OPTIONAL :: WIJKL(:,:,:,:,:,:)
    COMPLEX(q), OPTIONAL :: PIJKL(:,:,:,:)

! local variables
    COMPLEX(q), ALLOCATABLE :: TWOe4o(:,:,:,:,:,:)
    COMPLEX(q), ALLOCATABLE :: POL_ADD(:,:,:,:)
    TYPE (wavespin) WHF
...
!calculate PIJKL
    ...
    ENDIF
       IF (IO%IU0>=0.AND.ASSOCIATED(WPOTH).AND. .NOT.ASSOCIATED(H)) THEN
           WRITE(*,*)
           IF ( PRESENT( PIJKL ) ) THEN
              WRITE(*,'(" Calculating WIJKL and PIJKL at w=",(2F10.4))')REAL(COMEGA),AIMAG(COMEGA)
           ELSE
              WRITE(*,'(" Calculating UIJKL at w=",(2F10.4))')REAL(COMEGA),AIMAG(COMEGA)
           ENDIF 
           WRITE(*,*)
       ENDIF
...
!       but GCHG23 is conjugated in the ZGEMM routine
!==========================================================================
          CHAM=0
          CALL ZGEMM('C','N',  NGLB*NGLB, NGLB*NGLB,  NP, (1._q,0._q), &
               GCHG23(1,1),  SIZE(GCHG23,1), GCHG14(1,1),  SIZE(GCHG14,1), &
               (0._q,0._q), CHAM(1,1,1,1), SIZE(CHAM,1)*SIZE(CHAM,2))
          NFLOAT=NFLOAT+8._q*NGLB*NGLB*NGLB*NGLB*NP
...
!==========================================================================
    ...
    CALL M_sum_d(WGW%COMM_INTER, NFLOAT, 1)
    CALL M_sum_d(WGW%COMM_INTER, NFFT  , 1)

    CALL M_sum_z(WGW%COMM_INTER, TWOe4o, SIZE(TWOe4o))

    IF (L2E4W_ALL) THEN
      CALL M_sum_z(WGW%COMM_INTER, TWOE_4WANNIER, SIZE(TWOE_4WANNIER) )
    ENDIF
    ...
    IF ( PRESENT (PIJKL) ) THEN
      CALL M_sum_z(WGW%COMM_INTER, POL_ADD, SIZE(POL_ADD))
...
! local variables
    ...
    COMPLEX(q)      , ALLOCATABLE :: CRHO14(:,:,:)  ! (1._q,0._q) center charge
    COMPLEX(q)      , ALLOCATABLE :: CRHO23(:,:,:)  ! (1._q,0._q) center charge
    COMPLEX(q), ALLOCATABLE :: CHAM(:,:,:,:,:),TWOe4o_ADD(:,:,:,:)
    LOGICAL :: LFULL
    REAL(q) :: POTFAK(GRIDHF%MPLWV)           ! 1/(G+dk)**2 (G)
    INTEGER :: ISP1, ISP2
    INTEGER :: R1, N1, N2, N3, N4
...
!==========================================================================
       DO K1=1,WDES%NKPTS
          CHAM=0
          IF (NQ>=0) THEN
          CALL ZGEMM('C','N',  NGLB*NGLB, NGLB*NGLB*WDES%NKPTS,  NP, (1._q,0._q), &
               GCHG23(1,1,K1),  SIZE(GCHG23,1), GCHG14(1,1,1),  SIZE(GCHG14,1), &
               (0._q,0._q), CHAM, SIZE(CHAM,1)*SIZE(CHAM,2))
          NFLOAT=NFLOAT+8._q*NGLB*NGLB*NGLB*NGLB*WDES%NKPTS*NP
!         it is straightforward and simple to calculate all elements of CHAM in (1._q,0._q) go
!         simply add (1._q,0._q) more (last) index to CHAM, but this might require huge amounts of memory
          
          IF (ASSOCIATED(H).AND..NOT.ASSOCIATED(WPOTH)) THEN
             CALL ZGEMM('C','N',  NGLB*NGLB, NGLB*NGLB*WDES%NKPTS, H%TOTAL_ENTRIES, (1._q,0._q), &
                  CRHO23(1,1,K1), SIZE(CRHO23,1), CRHO14(1,1,1), SIZE(CRHO14,1), &
                  (1._q,0._q) , CHAM, SIZE(CHAM,1)*SIZE(CHAM,2))
             NFLOAT=NFLOAT+8._q*NGLB*NGLB*NGLB*NGLB*WDES%NKPTS*H%TOTAL_ENTRIES
          ENDIF
...
!*********************************************************************
!
! based on CALCULATE_WIJKL_FOURCENTRE, this routine computes the
! frequency dependend screened coulomb potential in the basis defined
! by W and WDES
! WPOT_DIST ... distributed W_GG'(q,iw) among NOMEGAPAR goups
! WIJKL ....... matrix elements on exit
!*********************************************************************

  SUBROUTINE WIJKL_TWOCENTRE_DISTRIBUTED( WDES,W,KPOINTS,LATT_CUR,T_INFO,P,&
    WGW,NB_START,NB_STOP,LMAX,CRPA_WPOTH,WPOT_DIST,IO,WIJKL,PIJKL)
    ...
    TYPE (type_info) T_INFO
    TYPE (potcar) P(T_INFO%NTYP)
    TYPE (wpothandle),POINTER  :: CRPA_WPOTH 
    TYPE (responsefunction) WPOT_DIST(:)
    TYPE (in_struct) IO
    TYPE (wavedes) WGW           ! descriptor for basis set of response function
    INTEGER NB_START,NB_STOP     ! global band indeces
    INTEGER LMAX                 ! maximum L index for (1._q,0._q) center terms
...
! (1._q,0._q)-center terms
    ...
    IF ( .NOT. ASSOCIATED( CRPA_WPOTH ) ) THEN
       CALL vtutor%bug("internal ERROR in VASP: CRPA_WPOTH not associated", "dmft.F", 2602)
    ENDIF
 
! intialize map from NOMEGAPAR groups -> QPT blocking
    CALL MAP_NOMEGAPAR_TO_QPT
...
! loop over frequency groups
    ...
    WIJKL(:,:,:,:,:,:,NOMEGA_GLOBAL ) = 0 

! set current frequency point update in wpot handle
    COMEGA=CMPLX( 0._q, CRPA_WPOTH%OMEGA( NOMEGA_GLOBAL ),q ) 
    CRPA_WPOTH%ID_NOMEGAPAR = ID_NOMEGA
    CRPA_WPOTH%NOMEGA_LOCAL = NOMEGA_LOCAL
    CRPA_WPOTH%NOMEGA_GLOBAL = NOMEGA_GLOBAL 
...
130 FORMAT (5X, //, &
         &'----------------------------------------------------', &
         &'----------------------------------------------------'//)
    IF (IO%IU6>=0 .AND. CRPA_WPOTH%NOMEGA_GLOBAL == 1 ) THEN
       WRITE(IO%IU6,130)
       WRITE(IO%IU0,'(A,4I5)') 'allocating two-electron 4 orbital integral table', &
          NGLB,NGLB,NGLB,NGLB
    ENDIF
...
! these are going to be distributed over the nodes when
! executed in parallel
    CRPA_WPOTH%NQ_LOCAL = 0
    nqloop: DO NQ_BASE=1,((WDES%NKPTS+WDES%NB_PAR-1)/WDES%NB_PAR)*WDES%NB_PAR,WDES%NB_PAR
       CRPA_WPOTH%NQ_LOCAL = CRPA_WPOTH%NQ_LOCAL + 1 

       CRPA_WPOTH%NQ_IN_FULL = CRPA_WPOTH%NQ_DIST(CRPA_WPOTH%O%COMM%NODE_ME,CRPA_WPOTH%NQ_LOCAL)
# 2715
...
! k-point treated on current node
       ...
       IF (NQ>WDES%NKPTS) NQ=-1
       IF ( NQ < 0 ) THEN
! store this, to indicate that rank participates in collective communication only
          CRPA_WPOTH%NQ_IN_FULL = NQ
          NQ = 1 
       ELSE
          CALL SETWDES(WGW, WGWQ, NQ)
          NP =WGWQ%NGVECTOR
...
! gather distributed potential, all ranks must participate here
       CALL GATHER_DISTRIBUTED_WPOT( CRPA_WPOTH, WPOT_DIST, NQ, POTFAK)
! ranks not carrying data in CRPA_WPOTH now have NQ = -1
...
!=============================================================
             IF ( PRESENT(PIJKL) .AND. .NOT. LCRPA ) THEN
                CALL CALCULATE_PIJKL(WHF,WDES,LATT_CUR,KPOINTS%ISMEAR,&
                   KPOINTS%SIGMA,COMEGA,K1,K4,ISP1,NB_START,NB_STOP,POL_ADD,IO)
             ENDIF
          ENDIF
       ENDDO !  k1
...
!==========================================================================
! apply W potential on centre densities
          IF (CRPA_WPOTH%LFULL) THEN
             IF (CRPA_WPOTH%WPOT(1)%LREALSTORE) THEN
                CALL DGEMM('N','N', 2*NP, NGLB*NGLB, 2*NP, 1.0_q, &
                     CRPA_WPOTH%WPOT(1)%RESPONSEFUN(1,1,1), &
                     2*SIZE(CRPA_WPOTH%WPOT(1)%RESPONSEFUN,1), &
                     GCHG14(1,1), 2*SIZE(GCHG14,1), &
                     0.0_q, GCHG23(1,1), 2*SIZE(GCHG23,1) )
             ELSE
                CALL ZGEMM('T','N', NP, NGLB*NGLB, NP, (1.0_q,0.0_q), &
                     CRPA_WPOTH%WPOT(1)%RESPONSEFUN(1,1,1), &
                     SIZE(CRPA_WPOTH%WPOT(1)%RESPONSEFUN,1), &
...
!==========================================================================
    ...
    ENDIF
       ...
       IF (SIZE(WIJKL,1)>=NGLB.AND.SIZE(WIJKL,2)>=NGLB.AND. &
      &     SIZE(WIJKL,3)>=NGLB.AND.SIZE(WIJKL,4)>=NGLB) &
      &   WIJKL(1:NGLB,1:NGLB,1:NGLB,1:NGLB,1:WDES%ISPIN,1:WDES%ISPIN,NOMEGA_GLOBAL)=&
        TWOe4o(:,:,:,:,1:WDES%ISPIN,1:WDES%ISPIN)
...
# 3052
    ...
    CONTAINS 
!******************************************************************
! set up which mpi rank is responsible for which q-point in the
! full BZ
! also, setup communicator to
!******************************************************************
    SUBROUTINE  MAP_NOMEGAPAR_TO_QPT
       USE mpimy
       INTEGER :: IERROR 
       INTEGER :: I 


       NPOINTS_IN_ROOT_GROUP = CRPA_WPOTH%O%NPOINTS_IN_ROOT_GROUP
       NCPU = CRPA_WPOTH%O%COMM_BETWEEN_GROUPS%NCPU
       NPOINTS = CRPA_WPOTH%O%NPOINTS
! set distribution of q-points among CPUS here
       CRPA_WPOTH%NQ_DIST( :, : ) = 0
       I = 0
       DO NQ_BASE=1,((WDES%NKPTS+WDES%NB_PAR-1)/WDES%NB_PAR)*WDES%NB_PAR,WDES%NB_PAR
          I = I + 1
! k-point treated on current node
          NQ = NQ_BASE+WDES%NB_LOW - 1
          IF ( NQ > WDES%NKPTS ) NQ = -1
          IF ( NQ >= 0 ) CRPA_WPOTH%NQ_DIST( CRPA_WPOTH%O%COMM%NODE_ME, I ) = NQ
       ENDDO
       CALL M_sum_i( CRPA_WPOTH%O%COMM, CRPA_WPOTH%NQ_DIST, CRPA_WPOTH%O%COMM%NCPU*WDES%NKPTS )
# 3089

    END SUBROUTINE MAP_NOMEGAPAR_TO_QPT 

  END SUBROUTINE WIJKL_TWOCENTRE_DISTRIBUTED
...
! passed variables
    ...
    COMPLEX(q)       :: CRHO14(:,:)    ! (1._q,0._q) center charge
...
! local variables
    ...
    COMPLEX(q) :: GWORK( MAX(GRIDHF%MPLWV,WGW%GRID%MPLWV))
    COMPLEX(q) :: CPHASE(GRIDHF%MPLWV)
    COMPLEX(q), ALLOCATABLE :: CRHOLM(:), CRHO_TMP(:,:) ! augmentation occupancy matrix
    LOGICAL LPHASE
...
! using strips of size m for the second band
    ...
    IF (NP/= SIZE(GCHG14,1))THEN
       CALL vtutor%bug("internal error ONEELECTRON_CHARGE: NP is not correct " // str(NQ) // " " // &
          str(K1) // " " // str(K4) // " " // str(NP) // " " // str(SIZE(GCHG14,1)), "dmft.F", 3148)
    ENDIF
...
! 1 runs over occupied bands only. This has to be 1._q in the same way here.
!---------------------------------------------------------------------------
...
!skip if excitation energy goes below threshold
     DCELTOT = REAL(W%CELTOT(NB2, K4, ISP1)-W%CELTOT(NB1,K1,ISP1),q)
     ...
     DO N1=NB_START,NB_STOP  
        ...
        IF ( LFINITE_TEMPERATURE ) THEN
          IF ( DCELTOT >= 0 ) THEN
             CALL SET_PIJKL_WEIGHT(WEIGHT,-DCELTOT,SHIFT,COMEGA,ISMEAR,SIGMA)
             WEIGHT=W%WDES%RSPIN*W%WDES%WTKPT(K1)*&
                (1-W%FERTOT(NB2,K4,ISP1))*(W%FERTOT(NB1, K1, ISP1))*WEIGHT
          ENDIF
        ELSE
        ...
        WEIGHT=WEIGHT/W%WDES%NKPTS*UK1(NB1,N1)*UK4(NB2,N2)*&
           CONJG(UK1(NB1,N3))*CONJG(UK4(NB2,N4))
...
!***********************************************************************
 SUBROUTINE REMOVE_PIJKL_FROM_WIJKL(WDES,BANDS_TOT,P,W,U)
    ...
    COMPLEX(q) W(:,:,:,:,:,:)
    COMPLEX(q) U(:,:,:,:,:,:)
    COMPLEX(q),ALLOCATABLE :: UCOMP(:,:)
    COMPLEX(q),ALLOCATABLE :: WCOMP(:,:)
    COMPLEX(q),ALLOCATABLE :: PCOMP(:,:)
...
!define compound indices
 ALLOCATE(UCOMP(NWTOTSUB*NWTOTSUB,NWTOTSUB*NWTOTSUB));UCOMP=(0._q,0._q)
 ALLOCATE(WCOMP(NWTOTSUB*NWTOTSUB,NWTOTSUB*NWTOTSUB));WCOMP=(0._q,0._q)
 ALLOCATE(PCOMP(NWTOTSUB*NWTOTSUB,NWTOTSUB*NWTOTSUB));PCOMP=(0._q,0._q)
  ...
  DO K=1,NWTOTSUB
    DO J=1,NWTOTSUB
     DO I=1,NWTOTSUB
       ...
       J_=J
       I_=I
       IF ( NTARGET_MASK(I)==1 .OR. &
            NTARGET_MASK(J)==1 .OR. &
            NTARGET_MASK(K)==1 .OR. &
            NTARGET_MASK(L)==1 ) THEN
...
!LU decomposition of (1 + P.W)
      ...
      IF (INF/=0) THEN
         CALL vtutor%error("ZGETRF returns " // str(INF))
      ENDIF
...
!invert matrix (1 + W.P)
      ...
      IF (INF/=0) THEN
         CALL vtutor%error("error : ZGETRI returns " // str(INF))
      ENDIF
...
!***********************************************************************
  SUBROUTINE SET_TARGET_STATES(NKPTS_IRZ,IO)
     USE base
     USE reader_tags
...
!Calculate unscreened potential, true by default
     LVPOT=.TRUE.
     CALL PROCESS_INCAR(.FALSE., IO%IU0, IO%IU5, 'LVPOT', LVPOT, IERR, WRITEXMLINCAR)

!Calculate screened potential, true by default
     LWPOT=.TRUE.
     CALL PROCESS_INCAR(.FALSE., IO%IU0, IO%IU5, 'LWPOT', LWPOT, IERR, WRITEXMLINCAR)
! all WFULLXXXX.tmp files have to be present when reading WFULL files
     IF ( LWPOT .AND. ICHIREAL < 1 ) THEN
        DO NQ = 1, NKPTS_IRZ  
           WRITE (APP  , "(4I1)") MOD(NQ/1000,10),MOD(NQ/100,10),MOD(NQ/10,10), MOD(NQ,10)
           INQUIRE( FILE="WFULL"//APP//".tmp", EXIST=LEXIST ) 
           IF( .NOT. LEXIST ) THEN
              LWPOT = .FALSE. 
              IF( IO%IU0 >=0 ) THEN
                  WRITE(*,*)"File WFULL"//APP//".tmp not found, will not calculate WIJKL!" 
              ENDIF    
              EXIT 
           ENDIF
        ENDDO
     ...
     NWLOW=-1
     CALL PROCESS_INCAR(.FALSE., IO%IU0, IO%IU5, 'NWLOW', NWLOW, IERR, WRITEXMLINCAR)

     NWHIGH=-1
     CALL PROCESS_INCAR(.FALSE., IO%IU0, IO%IU5, 'NWHIGH', NWHIGH, IERR, WRITEXMLINCAR)

     VCUTOFF=-30
     CALL PROCESS_INCAR(.FALSE., IO%IU0, IO%IU5, 'VCUTOFF', VCUTOFF, IERR, WRITEXMLINCAR)

     CALL CLOSE_INCAR_IF_FOUND(IO%IU5)

! set NTARGET_MASK based on NTARGET_STATES
     IF ( .NOT. ALLOCATED( NTARGET_MASK ) ) THEN 
        CALL INIT_CRPA_STATES( CRPA_NUM_WAN, IO%IU0, IO%IU6 ) 
     ENDIF
!
! use all states, if none are selected in INCAR
     IF ( SUM( NTARGET_MASK( : ) ) == 0 ) THEN
           NTARGET_MASK = 1 
     ENDIF
     IF (CRPA_NUM_WAN<SIZE(NTARGET_MASK)) NTARGET_MASK(CRPA_NUM_WAN+1:)=0
...
! will not be calculated and the result for VIJKL = 0
     IF ( LCRPA .OR. L2E4W) THEN
       ...
       AEXX  = 1.0
     ENDIF
     IF( SHIFT < 0 .AND. L2E4W ) SHIFT = 0.1 
  
!set default values for NWLOW and NWHIGH
     NSTATES=SUM(NTARGET_MASK(1:CRPA_NUM_WAN))
...
!full RPA (all states)
        IF( NSTATES == 0 )THEN
           ...
           NWHIGH = CRPA_NUM_WAN
!crpa (all states)
        ELSEIF( NSTATES == CRPA_NUM_WAN)THEN
           NWLOW = 1
           NWHIGH = CRPA_NUM_WAN
!some states
        ELSEIF(NSTATES > 0 .AND. NSTATES < CRPA_NUM_WAN )THEN
     ...
     ELSEIF( NWLOW < 0 .AND. NWHIGH > 0 )THEN
        NWLOW = 1
        NWHIGH = MIN(CRPA_NUM_WAN, NWHIGH)
      
!wrong set set default
     ELSEIF( NWHIGH > NWLOW .AND. NWLOW >0 )THEN
        NWHIGH = MIN(CRPA_NUM_WAN, NWHIGH)
        IF (SUM(NTARGET_MASK(1:CRPA_NUM_WAN)) == 0 ) THEN
           NTARGET_MASK(1:NWLOW-1) = 0 
           NTARGET_MASK(NWHIGH:CRPA_NUM_WAN) = 0
           NTARGET_MASK(NWLOW:NWHIGH) = 1
        ENDIF
     ELSE
        IF( IO%IU0>=0) WRITE( IO%IU0,'(A/,3I4)')'Warning in DMFT_MATRIX_ELEMENTS,'//&
           ' choice of target bands incompatible',NWLOW,NWHIGH,CRPA_NUM_WAN
        NWLOW = 1 
        NWHIGH = CRPA_NUM_WAN
     ENDIF 
...
!number of Wannier bands considered
     ...
     IF ( IO%IU0>=0 ) THEN
        ...
        IF ( LUSEBLOCH ) THEN
           WRITE(IO%IU0,'(" for the Bloch bands: ",I4," - ",I4)') NWLOW, NWHIGH
        ELSE
           WRITE(IO%IU0,'(" for the Wannier states: ",I4," - ",I4)') NWLOW, NWHIGH
        ENDIF
        WRITE(IO%IU0,*)" " 
     ENDIF
     RETURN
...
!
! Format of *IJKL file is:
! i j k l   Re(*_ijkl)   Im(*_ijkl)
!
!
!***********************************************************************
  SUBROUTINE WRITE_DMFT_MATRIX_ELEMENTS(IO,ISPIN,LSCREENED,WIJKL,UIJKL,&
    PIJKL,COMEGA)
    USE base
    IMPLICIT NONE
    TYPE(in_struct) IO
    COMPLEX(q)                 :: WIJKL(:,:,:,:,:,:)  !either bare or screened
!interaction
    LOGICAL              :: LSCREENED           !write either screened or
!bare interaction
    INTEGER              :: ISPIN               !number of spin degrees
!of freedom
    COMPLEX(q), OPTIONAL       :: UIJKL(:,:,:,:,:,:)  !used for NLRPA
    COMPLEX(q), OPTIONAL       :: PIJKL(:,:,:,:)      !used for NLPRA
    COMPLEX(q), OPTIONAL :: COMEGA              !complex frequency
 
    INTEGER N1,N2,N3,N4
    INTEGER ISP1,ISP2
    INTEGER :: J,I, NWI,NWF,NWTOTSUB
    INTEGER,ALLOCATABLE :: WTARGET(:)
    CHARACTER(8) :: EXTENSION
    COMPLEX(q) :: CTMP1,CTMP2,CTMP3
    LOGICAL :: LFOCK
    INTEGER, SAVE :: NCOUNTER=0
    CHARACTER(8) :: COUNTER 

    IF( PRESENT( COMEGA ) .AND. LSCREENED ) THEN
       NCOUNTER=NCOUNTER+1
       WRITE(COUNTER,'(I4)')NCOUNTER
       EXTENSION="."//TRIM(ADJUSTL(COUNTER))
    ENDIF
...
!define extension
          IF ( ISP1 ==1 .AND. ISP2 == 1 ) THEN
             EXTENSION='up.up'
             IF( PRESENT( COMEGA ) .AND. LSCREENED )EXTENSION='up.up.'//TRIM(ADJUSTL(COUNTER))
          ELSEIF (ISP1==1 .AND. ISP2 == 2 ) THEN
             EXTENSION='up.dn'
             IF( PRESENT( COMEGA ) .AND. LSCREENED )EXTENSION='up.dn.'//TRIM(ADJUSTL(COUNTER))
          ELSEIF (ISP1==2 .AND. ISP2 == 2 ) THEN
             EXTENSION='dn.dn'
             IF( PRESENT( COMEGA ) .AND. LSCREENED )EXTENSION='dn.dn.'//TRIM(ADJUSTL(COUNTER))
          ENDIF
          ENDIF
...
# 3929
                ...
                ENDIF
                WRITE( 99 ,'(A)')'# File generated by VASP contains Coulomb matrix elements'
                WRITE( 99 ,'(A)')'# V_ijkl = [ij|kl] = < ik|jl> = \int \phi*_i(r1) \phi_j(r1) \phi*_k(r2) \phi_l(r2) / |r1-r2|'
                WRITE( 99 ,'(A,F20.12)')'#  I   J   K   L          RE(V_IJKL)          IM(V_IJKL)'
...
! Write WIJKL(I,I,J,J) to OUTCAR
                ...
                ELSE IF ( LUSELOCPROJ ) THEN
                   WRITE(IO%IU6,'(/A)') ' bare Coulomb repulsion V_iijj between LOCPROJs:'
                ELSE
...
! Write WIJKL(I,J,J,I) to OUTCAR
                ...
                ELSE IF ( LUSELOCPROJ ) THEN
                   WRITE(IO%IU6,'(/A)') ' bare Coulomb repulsion V_ijji between LOCPROJs:'
                ELSE
...
! Write WIJKL(I,J,I,J) to OUTCAR
                ...
                ELSE IF ( LUSELOCPROJ ) THEN
                   WRITE(IO%IU6,'(/A)') ' bare Coulomb repulsion V_ijij between LOCPROJs:'
                ELSE
...
!if cRPA we need to open UIJKL instead of WIJKL
                 IF ( ISPIN == 1 ) THEN
                    IF ( LCRPA ) THEN
                       OPEN(UNIT=99,FILE='UIJKL'//TRIM(ADJUSTL(EXTENSION)),STATUS='REPLACE')
                    ELSEIF ( LNLRPA .OR. L2E4W ) THEN 
                       OPEN(UNIT=99,FILE='WIJKL'//TRIM(ADJUSTL(EXTENSION)),STATUS='REPLACE')
                    ENDIF
                 ELSE
                    IF ( LCRPA ) THEN
                       OPEN(UNIT=99,FILE='UIJKL.'//TRIM(ADJUSTL(EXTENSION)),STATUS='REPLACE')
                    ...
                    ENDIF
                 ENDIF
                 WRITE( 99 ,'(A)')'# File generated by VASP contains (un)screened Coulomb potential'
                 WRITE( 99 ,'(A)')'# W_ijkl = [ij|kl] = < ik|W(r1,r2)|jl> '
                 IF( PRESENT( COMEGA ) ) WRITE(99, '(A,2F12.5)')'# omega = ',COMEGA
                 WRITE( 99 ,'(A,F20.12)')'#  I   J   K   L          RE(W_IJKL)          IM(W_IJKL)'
                 DO N4=1,NWTOT
                  DO N3=1,NWTOT
                    DO N2=1,NWTOT
                     DO N1=1,NWTOT
                 ...
                 WRITE(IO%IU6,130)
                 WRITE(IO%IU6,*)
                 IF( PRESENT( COMEGA ) ) THEN
                   WRITE(IO%IU6,'(" spin components:",2I3,", frequency:",2F10.4)') ISP1,ISP2,COMEGA
                 ELSE
                   WRITE(IO%IU6,'(" spin components:",2I3,", frequency:",2F10.4)') ISP1,ISP2,0.0,0.0
                 ENDIF
! Write WIJKL(I,I,J,J) or UIJKL to OUTCAR
                 IF ( LCRPA ) THEN
                    ...
                    ELSE IF ( LUSELOCPROJ ) THEN
                       WRITE(IO%IU6,'(/A)') ' screened Coulomb repulsion U_iijj between LOCPROJs:'
                    ELSE
                 ...
                 ELSE
                    ...
                    ELSE IF ( LUSELOCPROJ ) THEN
                       WRITE(IO%IU6,'(/A)') ' screened Coulomb repulsion W_iijj between LOCPROJs:'
                    ELSE
...
! Write WIJKL(I,J,J,I) to OUTCAR
                 IF ( LCRPA ) THEN
                    ...
                    ELSE IF ( LUSELOCPROJ ) THEN
                       WRITE(IO%IU6,'(/A)') ' screened Coulomb repulsion U_ijji between LOCPROJs:'
                    ELSE
                 ...
                 ELSE
                    ...
                    ELSE IF ( LUSELOCPROJ ) THEN
                       WRITE(IO%IU6,'(/A)') ' screened Coulomb repulsion W_ijji between LOCPROJs:'
                    ELSE
...
! Write WIJKL(I,J,I,J) to OUTCAR
                 IF ( LCRPA ) THEN
                    ...
                    ELSE IF ( LUSELOCPROJ ) THEN
                       WRITE(IO%IU6,'(/A)') ' screened Coulomb repulsion U_ijij between LOCPROJs:'
                    ELSE
                 ...
                 ELSE
                    ...
                    ELSE IF ( LUSELOCPROJ ) THEN
                       WRITE(IO%IU6,'(/A)') ' screened Coulomb repulsion W_ijij between LOCPROJs:'
                    ELSE
...
! Write PIJKL(I,I,J,J) to OUTCAR
                       ...
                       ELSE IF ( LUSELOCPROJ ) THEN
                          WRITE(IO%IU6,'(/A)') ' particle hole propagator P_iijj in LOCPROJ Basis:'
                       ELSE
...
! Write PIJKL(I,J,I,J) to OUTCAR
                       ...
                       ELSE IF ( LUSELOCPROJ ) THEN
                          WRITE(IO%IU6,'(/A)') ' particle hole propagator P_ijji in LOCPROJ Basis:'
                       ELSE
...
! Write PIJKL(I,J,I,J) to OUTCAR
                       ...
                       ELSE IF ( LUSELOCPROJ ) THEN
                          WRITE(IO%IU6,'(/A)') ' particle hole propagator P_ijij in LOCPROJ Basis:'
                       ELSE
...
! redistribute back over bands
      ...
      IF ( IU0>=0 ) WRITE(IU0,*)'orbitals projected onto localized basis set'
      
      RETURN
   END SUBROUTINE PROJECT_ORBITALS_NOSYMM

!******************** SUBROUTINE DUM_RHOIJ *****************************
!
! dumps (1._q,0._q)-particle density matrix in localized basis set
!
!***********************************************************************
   SUBROUTINE DUMP_ONE_ELECTRON_FCIINPUT( WDES, W, WDES_LOC, VIJKL, IO)
      USE dfast
      USE wave_high
      USE full_kpoints
      IMPLICIT NONE
      ...
      TYPE(wavespin)  :: W
      TYPE(wavedes)   :: WDES_LOC
      COMPLEX(q)            :: VIJKL(:,:,:,:,:,:)   !correction
      TYPE(in_struct) :: IO
! local variables
      INTEGER I,J,I_,J_
      INTEGER N1,N2,N1_,N2_
      INTEGER ISP1,ISP2,NK,NB
      COMPLEX(q), ALLOCATABLE :: U(:,:)
      COMPLEX(q), ALLOCATABLE :: RHO(:,:,:)
      COMPLEX(q), ALLOCATABLE :: HAM(:,:,:)

 
      IF ( .NOT. LFCIINPUT ) RETURN
      ...
      ALLOCATE( U( WDES%NB_TOT, WDES%NB_TOT ) )
      ALLOCATE( RHO( NWTOT, NWTOT, WDES%ISPIN ) )
      ALLOCATE( HAM( NWTOT, NWTOT, WDES%ISPIN ) )
      RHO = (0._q,0._q)
      HAM = (0._q,0._q)

      spin: DO ISP1=1,W%WDES%ISPIN
         kpoints: DO NK=1,W%WDES%NKPTS
            CALL DETERMINE_WANNIER_PROJECTOR( WDES, NK, ISP1, U, IO%IU0 ) 
            DO I = 1, CRPA_NUM_WAN
               IF ( I < NWLOW .OR. I > NWHIGH ) CYCLE 
               IF ( NTARGET_MASK(I)/=1) CYCLE
               I_ = I - NWLOW + 1
               DO J = 1, CRPA_NUM_WAN
                  IF ( J < NWLOW .OR. J > NWHIGH ) CYCLE 
                  IF ( NTARGET_MASK(J)/=1) CYCLE
                  J_ = J - NWLOW + 1
                  DO NB = 1 , WDES%NB_TOT
                     RHO(I_,J_,ISP1) = RHO(I_,J_,ISP1) + &
                       CONJG(U(NB,I))*REAL(W%FERTOT(NB,NK,ISP1),q)*U(NB,J)
                     HAM(I_,J_,ISP1) = HAM(I_,J_,ISP1) + &
                       CONJG(U(NB,I))*REAL(W%CELTOT(NB,NK,ISP1),q)*U(NB,J)
                  ENDDO
               ENDDO
            ENDDO
         ENDDO kpoints
...
!IF ( IO%IU0>=0 .AND. IO%NWRITE > 2 ) THEN
      IF ( IO%IU0>=0 ) THEN
         OPEN( 100, FILE='RHOIJ',STATUS='REPLACE') 
         WRITE( 100 ,'(A)')'# File generated by VASP contains one-electron density'
         WRITE( 100 ,'(A)')'# rho_ij = 1/N_k sum_{nk} U*_i(nk) f_(nk) U_j(nk)'
         DO ISP1 = 1, WDES%ISPIN
         WRITE( 100 ,'(A,I2)')'#   I    J          RE(RHO_IJ)          IM(RHO_IJ), ISPIN=',ISP1
            DO I = 1, NWTOT
               DO J = 1, NWTOT
                  WRITE( 100, '(2I5,2F20.12)') I, J, RHO( I, J, ISP1 )
               ENDDO
            ENDDO
         ENDDO
         CLOSE( 100 ) 
         WRITE(*,*)'rho_ij written to file'
      ENDIF

! subtract exchange and hartree contribution from TIJ
      DO I = 1, NWTOT
         DO J = 1, NWTOT
! subtract hartree and exchange terms from traget state
            DO N1 = 1, NWTOT
               DO N2 = 1, NWTOT
! subtract states in target space only
                   DO ISP1=1,W%WDES%ISPIN
                      HAM(I,J,ISP1) = HAM(I,J,ISP1) + RHO(N1,N2,ISP1)*VIJKL(I,N1,N2,J,ISP1,ISP1)
                      DO ISP2=1,W%WDES%ISPIN
                         HAM(I,J,ISP1) = HAM(I,J,ISP1) - RHO(N1,N2,ISP2)*&
                         W%WDES%RSPIN*VIJKL(I,J,N1,N2,ISP1,ISP2)
                      ENDDO
                   ENDDO
               ENDDO
            ENDDO
      ...
      IF ( IO%IU0>=0 ) THEN
         OPEN( 100, FILE='TIJ',STATUS='REPLACE') 
         WRITE( 100 ,'(A)')'# File generated by VASP contains DFT hamiltonian in localized basis'
         WRITE( 100 ,'(A)')'# t_ij = < i | (T + V_ext) | j >'
         DO ISP1 = 1, WDES%ISPIN
         WRITE( 100 ,'(A,I2)')'#   I    J            RE(T_IJ)           IM(T_IJ), ISPIN=',ISP1
            DO I = 1, NWTOT
               DO J = 1, NWTOT
! local index is
                  WRITE( 100, '(2I5,2F20.12)') I, J, HAM( I, J, ISP1 )
               ENDDO
            ENDDO
         ENDDO
         CLOSE( 100 ) 
         WRITE(*,*)'t_ij written to file'
      ENDIF
      DEALLOCATE(HAM)
      DEALLOCATE(RHO)
      DEALLOCATE(U)
   ...
   END SUBROUTINE DUMP_ONE_ELECTRON_FCIINPUT
