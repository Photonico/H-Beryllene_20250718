...
!> @ref openmp : each thread need a private copy (THREADPRIVATE)
  INTEGER, SAVE :: RS_GreensFunc_type=-1
!$OMP THREADPRIVATE(RS_GreensFunc_type)
...
!
!> @details @ref openmp :
!> ensure that in case SET_RSGF_ALL is called from within an OpenMP
!> parallel region that only the MASTER thread computes a new Greens
!> function (RS_GreensFunc is a shared data structure).
!
...
! local variables
    ...
    REAL(q) TERM_X1,TERM_X2,TERM_X3,TERM_X4

! unable to construct G_l_sr for L>LMAX
    IF (L>LMAX) THEN
       CALL vtutor%error("RS_COULOMB_GREEN_FUNC: L>Lmax: " // str(L) // " " // str(LMAX))
    END IF
! initialize to (0._q,0._q)
    G=0 
! start
!$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) SHARED(R, MU, L, G, LONGRANGE) &
!$OMP PRIVATE(IGRID, JGRID, X, Y, EXPX, ERFX, TWON, S, P, TERM_X1, TERM_X2, TERM_X3, TERM_X4, PHI, EXPP, EXPM, ERFP, ERFM, E, N, F)
    DO IGRID=1,R%NMAX
       X=MU*R%R(IGRID)
       EXPX=EXP(-X**2)
       ERFX=ERRFC(X)
       ...
       ENDDO
       TERM_X1=X**(L+1)
       TERM_X2=ERFX+S*(EXPX/SQRT(PI))
       TERM_X3=((2._q**(L+1))*(X**(2*L+1))*EXPX/FFACTOR(2*L-1)/(TWON+3._q)/SQRT(PI))
       TERM_X4=((2._q**L)*(2._q*X**2-TWON-3._q)*X**(2*L+1)*EXPX/FFACTOR(2*L-1)/(TWON+3._q)/(TWON+5._q)/SQRT(PI))
       DO JGRID=1,MIN(IGRID+1,R%NMAX)
          Y=MU*R%R(JGRID)
! expanded or full form
          IF (Y<0.1_q) THEN
! expanded form
             PHI=(Y**(L)/TERM_X1)*(TERM_X2+TERM_X3*(Y**2)+TERM_X4*(Y**4))
          ELSE
! full form
...
!> In double precision its use is limited to n=8.
  FUNCTION FACTPROD(N,P)
    ...
    REAL(q) FACTPROD
    IF (N>8) THEN
       CALL vtutor%error("FACTPROD: N too large (>8) for factorial expression: " // str(N))
    ENDIF
    IF (N<P) THEN
       CALL vtutor%error("FACTPROD: invalid call: N<PR: " // str(N) // " " // str(P))
    ENDIF
...
!>~~~
!>  @note S must be set to (0._q,0._q) before call, otherwise the results are added to S.
...
!$    INTEGER :: OMP_GET_NUM_THREADS
      ...
      IF (SIZE(S,1)<CHANNELS .OR. SIZE(S,2)<CHANNELS & 
         ...
         CALL vtutor_bug("internal ERROR in COLOUMB_4TERM: S is too small " // str(SIZE(S)), "pawfock.F", 461)
      ENDIF

! loop over k,l
!$OMP PARALLEL DO IF (CHANNELS>=OMP_GET_NUM_THREADS()) SCHEDULE(STATIC) DEFAULT(NONE) &
!$OMP PRIVATE(CH1, CH2, I, RHOAE, LL, LLP, LMIN, LMAX, M, LLOOP, CHI1, CHI2, SUM) &
!$OMP SHARED(CHANNELS, R, W, L, LONECENTERMAX, LDIM, S)
      DO CH1=1, CHANNELS
      DO CH2=1, CHANNELS
...
! calculate rho_kl(r)
         ...
         IF (LMAX/2>LDIM) THEN
            CALL vtutor_bug("internal ERROR in COLOUMB_4TERM: LDIM is not sufficiently large " // &
               str(LMAX/2) // " " // str(LDIM), "pawfock.F", 481)
         ENDIF
         M(1:R%NMAX) = 0
...
!$    INTEGER :: OMP_GET_NUM_THREADS
      ...
      IF (SIZE(S,1)<CHANNELS .OR. SIZE(S,2)<CHANNELS & 
           .OR. SIZE(S,3)<CHANNELS .OR. SIZE(S,4)<CHANNELS) THEN
         CALL vtutor_bug("internal ERROR in COLOUMB_4TERM_PS: S is too small " // str(SIZE(S)), "pawfock.F", 559)
      ENDIF
...
! loop over k,l
!$OMP PARALLEL DO IF (CHANNELS>=OMP_GET_NUM_THREADS()) &
!$OMP SCHEDULE(STATIC) DEFAULT(NONE) &
!$OMP PRIVATE(CH1, CH2, I, RHOPS, LL, LLP, LMIN, LMAX, M, LLOOP, CHI1, CHI2, SUM, RHOAUG, N, J) &
!$OMP SHARED(CHANNELS, R, W, L, LONECENTERMAX, LDIM, S, AUG, QPAW, LMAX_FOCKAE, NMAX_FOCKAE, AUG_FOCK, QPAW_FOCK)
      DO CH1=1, CHANNELS
      DO CH2=1, CHANNELS
...
! calculate rho_kl(r)
         ...
         LMIN = ABS(LL-LLP) ; LMAX = MIN(ABS(LL+LLP),LONECENTERMAX)
         IF (LMAX/2>LDIM) THEN
            CALL vtutor_bug("internal ERROR in COLOUMB_4TERM_PS: LDIM is not sufficiently large " &
               // str(LMAX/2) // " " // str(LDIM), "pawfock.F", 587)
         ENDIF
         IF (SIZE(AUG,2)-1<LMAX) THEN
            CALL vtutor_bug("internal ERROR in COLOUMB_4TERM_PS: SIZE(AUG) is too small " // &
               str(SIZE(AUG,2)) // " " // str(LMAX), "pawfock.F", 591)
         ENDIF
         M(1:R%NMAX) = 0
...
! local variables
      ...
      IF (LMAX/2>LDIM) THEN
         CALL vtutor%bug("internal ERROR in COLOUMB_2POT: LDIM is not sufficiently large " // &
            str(LMAX/2) // " " // str(LDIM), "pawfock.F", 909)
      ENDIF
...
! local variables
      ...
      LMIN = ABS(L1-L2) ; LMAX = ABS(L1+L2)
      IF (LMAX/2>LDIM) THEN
         CALL vtutor%bug("internal ERROR in COLOUMB_2POT_PS: LDIM is not sufficiently large " // &
            str(LMAX/2) // " " // str(LDIM), "pawfock.F", 952)
      ENDIF
...
!$OMP THREADPRIVATE(NTYP_SLATER,S)
  CONTAINS
