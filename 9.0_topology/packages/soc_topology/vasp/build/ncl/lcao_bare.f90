...
!> Initialize LCAO datastructure
    SUBROUTINE LCAO_INIT(P,XC,INFO,IU0,IU6)
      USE pseudo
      USE constant
      USE prec
      USE cl
...
!
         CALL EXTEND_ARRAYS(PP,XC,CATOM,IU0,IU6)
!
...
!
         ...
         CALL LCAO_SET_DIJ(CATOM,PP,XC,CRHODEIN,RHOLM,EDCH,EDCT1,EDCHXC,EDCT1XC,IU0, IU6)
...
!
            CATOM%POT = 0._q
            POT = 0._q
            CALL RAD_POT(CATOM%FR, XC, ISPIN, 0, 0, .FALSE.,  &
...
!              CATOM%POT(I) =-POT(I,1,1)/(2._q*SQRT(PI))-VADD
            ...
            IF (ITER>MINITER) CALL LCAO_SET_DIJ(CATOM,PP,XC,CRHODETMP,RHOLM,EDCH,EDCT1,EDCHXC,EDCT1XC,IU0, IU6)
...
!*******************************************************************
  ...
  SUBROUTINE EXTEND_ARRAYS(PP,XC,CATOM, IU0, IU6)
...
!   CALL PUSH_XC_TYPE(XC,XC_DATA_PP)
    CALL RAD_POT( PP%R, XC, 1, 1, 1, .FALSE.,  &
...
!
!*******************************************************************
    SUBROUTINE LCAO_SET_DIJ(CATOM,PP,XC,CRHODE,RHOLM,EDCH,EDCT1,EDCHXC,EDCT1XC,IU0, IU6)
...
!
      FPOT = 0._q
      POT=0._q
      CALL RAD_POT(PP%R, XC, 1, 0, 0, .FALSE., RHO, PP%RHOPS, FPOT, POT, EDCT1, EDCT1XC)
!
...
!
      FPOT = 0._q
      POT = 0._q
      CALL RAD_POT(PP%R, XC, 1, 0, 0, .FALSE., RHO, PP%RHOAE, FPOT, POT, EDCH, EDCHXC)
!
! shift has to be applied, because not the full charge
! is in the sphere
!
...
! A (0._q,0._q) can be found in a interval corresponding to MARGIN can be up to N0*NSAMPLING
...
! Define intervals in which to look for zeros
             IF (I==1) THEN
! Since the first two intervals are adjacent we use only (1._q,0._q)
...
!WRITE(*,*) '========'
!WRITE(*,*) I,XB1(K),XB2(K)
! Call bisection method to find the (0._q,0._q)
                   X1_TMP=XB1(K)
                   X2_TMP=XB2(K)
...
! We found a (0._q,0._q). Check if its really a (0._q,0._q),
...
# 1784
          ...
          CALL vtutor%bug('Found no zeros in interval '//str(X1)//':'//str(X2)//&
                          '.\nThis should not happen, please report indicating which POTCAR you are using.',"lcao_bare.F",1786)
       ENDIF
...
# 1799
          ...
          CALL vtutor%bug('Found no zeros in interval '//str(X1)//':'//str(X2)//&
                          '.\nThis should not happen, please report indicating which POTCAR you are using.',"lcao_bare.F",1801)
...
!-----------------------------------------------------
    ...
    call vh5_error(vh5_group_open_or_create(fileid,  trim(my_group), groupid),"lcao_bare.F",1891)
    call vh5_error(vh5_group_open_or_create(groupid, trim(my_subgroup), subgroupid),"lcao_bare.F",1892)

    do iatom=1,size(atom_lcao)
       element = atom_lcao(iatom)%element
! Write potential
! Write number of nodes in potcar
       call vh5_error(vh5_write(subgroupid,element//"/potcar_nodes",atom_lcao(iatom)%potcar_nodes(:)),"lcao_bare.F",1898)
! Write number of nodes in LCAO
       call vh5_error(vh5_write(subgroupid,element//"/lcao_nodes",atom_lcao(iatom)%lcao_nodes(:)),"lcao_bare.F",1900)
! Write atomic wavefunctions in the logarithmic grid
       call vh5_error(vh5_write(subgroupid,element//"/wfct_l",atom_lcao(iatom)%wfct_l(:,:3,:)),"lcao_bare.F",1902)
! Write potcar pseudo and ae wavefunctions in the logarithmic grid
       call vh5_error(vh5_write(subgroupid,element//"/potcar_r",atom_lcao(iatom)%fr%r(:)),"lcao_bare.F",1904)
       call vh5_error(vh5_write(subgroupid,element//"/potcar_wps",atom_lcao(iatom)%wps(:,:)),"lcao_bare.F",1905)
       call vh5_error(vh5_write(subgroupid,element//"/potcar_wae",atom_lcao(iatom)%wae(:,:)),"lcao_bare.F",1906)
! This is the mapping from LCAO to the POTCAR (because only occupied orbitals are used for LCAO)
       call vh5_error(vh5_write(subgroupid,element//"/lcao_l",atom_lcao(iatom)%l(:,:)),"lcao_bare.F",1908)
! Write atomic energies
       call vh5_error(vh5_write(subgroupid,element//"/lcao_energy",atom_lcao(iatom)%e(:)),"lcao_bare.F",1910)
! Write atomic energies from POTCAR
       call vh5_error(vh5_write(subgroupid,element//"/potcar_energy",atom_lcao(iatom)%potcar_e(:)),"lcao_bare.F",1912)
    enddo
!
! close/deallocate
!
    call vh5_error(vh5_group_close_writing(subgroupid),"lcao_bare.F",1917)
    call vh5_error(vh5_group_close_writing(groupid),"lcao_bare.F",1918)
    call vh5_error(vh5_file_flush(fileid),"lcao_bare.F",1919)
...
! QBFZERO=0.0_q ! Breaks umco_SiC (gnu), used uninitialized as a whole three lines below.
      DO I=1,100
         ...
         DO J=1,CATOM%NPLIN
            ...
            DO N=1,I+1
               ...
               IF (QBFZERO(N)**2._q*HSQDTM-ENMAX >ENMAX) THEN
                  CALL vtutor%bug("internal error in SETUP_LDEP_BESBASIS_LIN: exceeding bounds", "lcao_bare.F", 2459)
               ENDIF
