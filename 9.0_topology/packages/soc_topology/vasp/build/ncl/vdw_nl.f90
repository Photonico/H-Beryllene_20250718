...
!df/d(drho)/(drho)potential on output
    ...
    COMPLEX(q), allocatable :: CINP(:) !, THETA(:,:)
    REAL(q) ,allocatable :: k_k(:)
    COMPLEX(q)   DWORK(GRIDC%RL%NP), DWORKS(GRIDC%RL%NP)
    INTEGER IVDW_NL
...
!$ACC ENTER DATA COPYIN(d1,d2,IVDW_NL,gamma_vdw,alpha_vdw,dina,intgrid,nintg,dint1) 
!$ACC ENTER DATA CREATE(x_work,u_work) 

 !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
 !$OMP SHARED(d1,d2,IVDW_NL,gamma_vdw,alpha_vdw,dina,intgrid,nintg,dint1) &
 !$OMP PRIVATE(ia,ib,a,b,sa,sb,ca,cb,a2,b2,W,T,dinb,ders,x_work,u_work)
!$ACC PARALLEL LOOP PRESENT(d1,d2,IVDW_NL,gamma_vdw,alpha_vdw,dina,intgrid,nintg,dint1) &
!$ACC PRIVATE(ib,a,b,sa,sb,ca,cb,a2,b2,W,T,dinb,ders,x_work,u_work) 
    DO ia=1,nintg
!$ACC LOOP SEQ
      DO ib=1,nintg
        a=intgrid(ia)
    ...
    ENDDO
 !$OMP END PARALLEL DO
!$ACC EXIT DATA COPYOUT(dina) DELETE(d1,d2,IVDW_NL,gamma_vdw,alpha_vdw,intgrid,nintg,dint1,x_work,u_work) 
...
!some sensible value
  ...
  FUNCTION NY(ar,d,IVDW_NL,gamma_vdw,alpha_vdw)
    ...
    IF ((ar.GT.0._q).AND.(d.GT.0._q)) THEN
      IF (IVDW_NL.EQ.1) THEN
        y=ar/d
        ny = ar*ar*0.5_q/(1._q-exp(-gamma_vdw*y*y))
      ELSEIF ((IVDW_NL.EQ.3).OR.(IVDW_NL.EQ.4)) THEN
...
!can be 1._q once
    ...
    cell=latt_cur%B*AUTOA
!$ACC PARALLEL LOOP PRESENT(GRIDC,K_K) COPYIN(CELL) PRIVATE(N1,NC,N2,N3,K_X,K_Y,K_Z) 
    do i=1,gridc%rc%np
...
!$ACC ENTER DATA CREATE(q0,dq0,alp,pa,f_g,theta,f_k,dj_k,j_k) 
!$ACC ENTER DATA CREATE(k_x_pre) 
  cell=latt_cur%B*AUTOA
!$ACC ENTER DATA COPYIN(qs_vdw,spl,phik,d2phidk2,cell) 
  str11=0._q
  str21=0._q
  str31=0._q
  str12=0._q
...
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(DCHARG,DWORK4,q0) &
!$ACC& PRIVATE(ro,gr,s,nrs,rnrs,ex_LDA,ec_LDA) 
  DO I=1,NP
...
!$ACC PARALLEL PRESENT(q0,alp,dq0,qs_vdw) 
!$ACC LOOP GANG VECTOR PRIVATE(iofq)
  DO I=1,NP
...
!and Fourier transform it, this is stored in \Theta array
  DO alf=1,Nalp
!$ACC PARALLEL PRESENT(spl,alp,dq0,pa,DCHARG,dinp) 
!$ACC LOOP GANG VECTOR PRIVATE(coef,diff)
    DO I=1,NP
      coef(:)=spl(alp(i),alf,:)
      diff=dq0(i)
...
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(THETA,CINP) 
    DO I=1,GRIDC%RC%NP
      THETA(i,alf)=CINP(i)
    ENDDO
  ENDDO
...
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(dj_k,k_k,j_k) 
  DO i=1,GRIDC%RC%NP
  ...
  IF (ISIF .ge. 1) THEN
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(k_x_pre,gridc,cell) PRIVATE(n1,nc,n2,n3) 
     DO i=1,GRIDC%RC%NP
        n1=Mod((i-1),gridc%rc%nrow)+1
        nc=(i-1)/gridc%rc%nrow+1
        n2=gridc%rc%i2(nc)
...
!first we do the sum over \Theta_\beta * kernel
  DO alf=1,Nalp
!$ACC KERNELS PRESENT(f_K) 
    f_k=0._q
!$ACC END KERNELS
!$ACC PARALLEL LOOP COLLAPSE(2) GANG VECTOR PRESENT(k_k,phik,d2phidk2,f_k,theta,gridc,k_x_pre) &
!$ACC& PRIVATE(ik_k,phi,dphidk,a_k,b_k,a2_k,b2_k,a3_k,b3_k,n1,nc,n3,pref) &
!$ACC& REDUCTION(+:str11,str12,str13,str21,str22,str23,str31,str32,str33) 
    DO bet=1,Nalp
      DO i=1,GRIDC%RC%NP
        ik_k=k_k(i)/dk_k
        IF (ik_k.GE.Nr) THEN
...
!$ACC PARALLEL PRESENT(gridc,f_k,theta,CINP) 
!$ACC LOOP GANG VECTOR PRIVATE(nc,n1,n3,pref) REDUCTION(+:energ)
    DO i=1,GRIDC%RC%NP
...
!$ACC END PARALLEL
    CALL OPSYNC(DINP,CINP,GRIDC%NPLWV)
    CALL FFT3D(CINP,GRIDC,1)
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(dinp,f_g) 
    DO i=1,GRIDC%RL%NP
...
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(DCHARG,DWORK4,q0,spl,alp,dq0,f_g,DWORKG,DWORK) &
!$ACC& PRIVATE(ro,gr,nrs,rnrs,vld1,vld2,vld3,vld4,ec_LDA,vc_LDA,dq0dn,dq0dgr,dhdx, &
!$ACC& dtheadn,dtheadgr,coef,diff,pa_tm,dpadq0) 
  DO i=1,gridc%RL%NP
!calc the potential
...
!$ACC EXIT DATA DELETE(qs_vdw,spl,phik,d2phidk2,cell) 
!$ACC EXIT DATA DELETE(q0,dq0,alp,pa,f_g,theta,f_k,dj_k,j_k) 
!$ACC EXIT DATA DELETE(k_x_pre) 
...
!$ACC ROUTINE(SPLIT_CMPLX_ATOMIC_ADD_FROM_CMPLX) SEQ
  ...
  REAL(q) Zab_vdW
  REAL(q) EXC
!spin vdw mod these arrays have two instances, (1._q,0._q) for spin up, other for spin down
  REAL(q) DCHARG(GRIDC%RL%NP), DCHARGS(GRIDC%RL%NP) !charge density on input and output
  REAL(q) DWORKG(GRIDC%RL%NP), DWORKGS(GRIDC%RL%NP) !df/drho input, add the same non-local here
  REAL(q) DWORK4(GRIDC%RL%NP), DWORK4S(GRIDC%RL%NP) !gradient on input
!df/d(drho)/(drho)potential on output
...
!spin vdw mod (jk: not sure, needs to be checked)
  REAL(q) :: q0_temp
! fixing bad convergence for (1._q,0._q) spin density too low
! while the other OK
...
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(DCHARG,DCHARGS,DWORK4,DWORK4S,q0) &
!$ACC& PRIVATE(ro,ros,gr,grs,s,ss,nrs,rnrs,zeta,ex_LDA,ec_LDA,vc_LDA,LDO_UP,LDO_DN,qx_up,qx_dn,KBEC,KBVU,KBVD,k_F) 
!!!
  DO I=1,NP
!spin vdw mod, need to take care of noisy densities in (1._q,0._q) or both spins
    q0(i)=q0cut
...
!$ACC END PARALLEL
  ...
  DO alf=1,Nalp
!$ACC PARALLEL PRESENT(spl,alp,dq0,pa,DCHARG,DCHARGS,dinp) 
!$ACC LOOP GANG VECTOR PRIVATE(coef,diff)
    DO I=1,NP
      coef(:)=spl(alp(i),alf,:)
      diff=dq0(i)
...
!$ACC LOOP GANG VECTOR PRIVATE(nrs)
    DO I=1,NP
      nrs=0._q
...
!$ACC END PARALLEL
    CALL OPSYNC(DINP,CINP,GRIDC%NPLWV)
    CALL FFT3D(CINP,GRIDC,1)
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(dinp,f_g,gridc) 
    DO i=1,GRIDC%RL%NP
...
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(DCHARG,DCHARGS,DWORK4,DWORK4S,q0,spl,alp,dq0,f_g,DWORKG,DWORKGS,DWORK,DWORKS) &
!$ACC& PRIVATE(ro,ros,gr,grs,nrs,rnrs,vld1,vld2,vld3,vld4,ec_LDA,vc_LDA,dq0dn,dq0dns,dq0dgr,dhdx, &
!$ACC& dtheadn,dtheadns,dtheadgr,dtheadgrs,coef,diff,pa_tm,dpadq0,LDO_UP,LDO_DN,dqup_dq,dqdn_dq, &
!$ACC& qx_up,qx_dn,dq0dgrs,KBEC,KBVU,KBVD,k_F,s,DQZXDN,zeta) 
  DO i=1,gridc%RL%NP
!calc the potential
    LDO_UP=.TRUE.
    LDO_DN=.TRUE.
...
!$ACC PARALLEL LOOP PRESENT(GRIDC,K_K) COPYIN(CELL) PRIVATE(N1,N2,N3,NC,K_X,K_Y,K_Z) 
    do i=1,gridc%rc%np
      n1=Mod((i-1),gridc%rc%nrow)+1
      nc=(i-1)/gridc%rc%nrow+1
      n2=gridc%rc%i2(nc)
...
!  IF (IU0>=0) WRITE(IU0,*)  "MPLWV: ", GRIDC%MPLWV, GRIDC%MPLWV, GRIDC%RC%NP
  ALLOCATE(q0(NP),dq0(NP),alp(NP),pa(NP),f_g(NP,Nalp))
  ALLOCATE(THETA(GRIDC%MPLWV,Nalp),f_k(GRIDC%MPLWV))
!$ACC ENTER DATA CREATE(Q0,DQ0,ALP,PA,F_G,THETA,F_K) COPYIN(QS_VDW,SPL,PHIK,D2PHIDK2) 

  cell=latt_cur%B*AUTOA
  stress=0._q
!$ACC ENTER DATA COPYIN(CELL,STRESS) 
...
!calculate q0 in each point and it's saturated version (Soler's paper)
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(DCHARG,DWORK4,Q0) &
!$ACC& PRIVATE(RO,GR,OMEGA_P2,OMEGA_G2,KVAL,OMEGA_0,NRS) 
  DO I=1,NP
    IF (DCHARG(i).LT. 1e-11_q) THEN
      ro=1e-15_q
      gr=0._q
...
!that is which interpolating point we will be using and how far from it are we
  ...
  a_iofq=max(a_iofq,1e-12_q)
  b_iofq=(qs_vdw(2)-qs_vdw(1))/(exp(a_iofq)-1.0_q)
!$ACC PARALLEL PRESENT(Q0,QS_VDW,ALP,DQ0) 
!$ACC LOOP GANG VECTOR PRIVATE(IOFQ)
  DO I=1,NP
    iofq=1+log(1+(q0(i)-qs_vdw(1))/b_iofq)/a_iofq
    iofq=max(1,iofq)
...
!and Fourier transform it, this is stored in \Theta array
  DO alf=1,Nalp
!$ACC PARALLEL PRESENT(spl,alp,dq0,pa,DCHARG,dinp) 
!$ACC LOOP GANG VECTOR PRIVATE(diff,coef)
    DO I=1,NP
      coef(:)=spl(alp(i),alf,:)
      diff=dq0(i)
      pa(i)=coef(1)+diff*(coef(2)+diff*(coef(3)+diff*coef(4)))
    ENDDO
!$ACC LOOP GANG VECTOR PRIVATE(ro,kval)
    DO I=1,NP
      IF (DCHARG(i).ge.0._q) THEN
        ro=DCHARG(i)*AUTOA3
...
!$ACC END PARALLEL
    CALL OPSYNC(DINP,CINP,GRIDC%NPLWV)
    CALL FFT3D(CINP,GRIDC,-1)
!$ACC PARALLEL LOOP PRESENT(THETA,CINP) 
    DO I=1,GRIDC%RC%NP
      THETA(i,alf)=CINP(i)
    ENDDO
  ENDDO
...
!$ACC KERNELS PRESENT(f_k) 
    f_k=0._q
!$ACC END KERNELS
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(k_k,phik,d2phidk2,f_k,theta,gridc,cell,stress) &
!$ACC& PRIVATE(ik_k,phi,dphidk,a_k,b_k,a2_k,b2_k,a3_k,b3_k,n1,n2,n3,nc,k_x,k_a,pref) 
    DO bet=1,Nalp
      DO i=1,GRIDC%RC%NP
        ik_k=k_k(i)/dk_k
        IF (ik_k.GE.Nr) THEN
...
!$ACC PARALLEL LOOP PRESENT(gridc,f_k,theta) PRIVATE(nc,n1,n3,pref) REDUCTION(+:energ) 
    DO i=1,GRIDC%RC%NP
      pref=real(f_k(i)*conjg(theta(i,alf)),kind=q)
# 2802
...
!now we fourier transform the \Theta * kernel to real space to reuse it for potential
!$ACC PARALLEL LOOP PRESENT(CINP,f_k) 
    DO i=1,GRIDC%RC%NP
      CINP(I)=f_k(i)
    ENDDO
    CALL OPSYNC(DINP,CINP,GRIDC%NPLWV)
    CALL FFT3D(CINP,GRIDC,1)
!$ACC PARALLEL LOOP PRESENT(f_g,dinp) 
    DO i=1,GRIDC%RL%NP
      f_g(i,alf)=dinp(i)*RINPL
    ENDDO
  ENDDO

!$ACC KERNELS PRESENT(DCHARG) 
  DCHARGSUM=SUM(DCHARG)
!$ACC END KERNELS
...
!$ACC PARALLEL LOOP GANG VECTOR PRESENT(GRIDC,DCHARG,DWORK4,DWORKG,DWORK,q0,spl,alp,dq0,f_g) &
!$ACC& PRIVATE(ro,gr,dq0dn,dq0dgr,nrs,rnrs,dhdx,dtheadn,dtheadgr,coef,diff,pa_tm,dpadq0) 
  DO i=1,gridc%RL%NP
!calc the potential
    IF (DCHARG(i).LT. 1e-11_q) THEN
      ro=1e-15_q
...
!$ACC EXIT DATA DELETE(Q0,DQ0,ALP,PA,F_G,THETA,F_K,CELL,QS_VDW,SPL,PHIK,D2PHIDK2) COPYOUT(STRESS) IF(OFFLOAD_ON) WAIT(ACC_ASYNC_Q)
  DEALLOCATE(q0,dq0,alp,pa)
  ...
  CALL M_sum_d(GRIDC%COMM,E_corr,1)
  CALL M_sum_d(GRIDC%COMM,NELECT,1)
