...
! #define testfock
      ...
      PUBLIC :: DMATRIX_READER, DMATRIX_CALC, LDO_DMATRIX
...
! local variables
      ...
      LDO_DMATRIX=.FALSE.
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'LDMATRIX', LDO_DMATRIX, IERR, WRITEXMLINCAR)
...
!           GSQU=(DKX+GX)**2+(DKY+GY)**2+(DKZ+GZ)**2
            ...
            IF (GSQU<KPOINTS_FULL%VKPTMINDIST2/40) THEN
               POTFAK(NI,1:3)=0._q
            ELSE
               POTFAK(NI,1)=GX*GX/GSQU-1._q/3._q
               POTFAK(NI,2)=GY*GY/GSQU-1._q/3._q
               POTFAK(NI,3)=GZ*GZ/GSQU-1._q/3._q
               POTFAK(NI,4)=GX*GY/GSQU
               POTFAK(NI,5)=GX*GZ/GSQU
               POTFAK(NI,6)=GY*GZ/GSQU
            ENDIF

            IF (GRID%REAL2CPLX) POTFAK(NI,:)=POTFAK(NI,:)*GRID%FFTWEIGHT(NI)
...
!    &                0._q, SQPi15,   0._q,   0._q,   0._q  /
     ...
     &              SQPi15,mSQpi15,   0._q,   0._q,   0._q,   0._q /
      ...
      IF (LMAX<2) THEN
         CALL vtutor%error("DAB_POLY_POLAR: ERROR: LMAX<2, LMAX= " // str(LMAX))
      ENDIF
...
! local variables
      ...
      IF (LMAX<2) THEN
         CALL vtutor%error("DAB_HEAVYSIDE: ERROR: LMAX<2, LMAX= " // str(LMAX))
      ENDIF
...
! local variables
      ...
      IF (LLMAX/2>SIZE(S1,1).OR.LLMAX/2>SIZE(S2,1)) THEN
         CALL vtutor%error("CALCSL4AE: ERROR: internal inconsistency: " // str(LLMAX/2) // " " // &
            str(SIZE(S1,1)) // " " // str(SIZE(S2,1)) // " " // str(SIZE(S3,1)))
      ENDIF
...
! local variables
      ...
      IF (LLMAX/2>SIZE(S1,1).OR.LLMAX/2>SIZE(S2,1).OR.LLMAX/2>SIZE(S3,1)) THEN
         CALL vtutor%error("CALCSL4PS: ERROR: internal inconsistency: " // str(LLMAX/2) // " " // &
            str(SIZE(S1,1)) // " " // str(SIZE(S2,1)) // " " // str(SIZE(S3,1)))
      ENDIF
...
! local variables
      ...
      IF (SIZE(RHO)<R%NMAX) THEN
         CALL vtutor%error("RAD_POT_DIP: ERROR: RHO incorrectly dimensioned: " // str(R%NMAX) // " "&
             // str(SIZE(RHO)))
      ENDIF
      IF (SIZE(POT1)<R%NMAX.OR.SIZE(POT2)<R%NMAX) THEN
         CALL vtutor%error("RAD_POT_DIP: ERROR: POT incorrectly dimensioned: " // str(R%NMAX) // " "&
             // str(SIZE(POT1)) // " " // str(SIZE(POT2)))
      ENDIF
