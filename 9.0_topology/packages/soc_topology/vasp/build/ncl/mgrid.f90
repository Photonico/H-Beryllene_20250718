...
!>
!> for the x direction the reciprocal lattice
...
!     is that (1._q,0._q) in the sub grid
       ...
       IF (GRID_SUB%RC%NCOL >GRID%RC%NCOL) THEN
          CALL vtutor%bug("GEN_GRID_SUB: internal error " // str(GRID_SUB%RC%NCOL), "mgrid.F", 337)
       ENDIF
...
!
  SUBROUTINE SRCH_COL1(L2S,L3S,GRID,NC2)
    ...
    DO NC=1,NCOL
       ...
       ENDIF
    ENDDO
    CALL vtutor%bug("SRCH_COL1: internal error " // str(L2S) // " " // str(L3S) // "\n " // &
       str(GRID%LPCTY) // "\n " // str(GRID%LPCTZ), "mgrid.F", 404)
...
!***********************************************************************
  ...
  SUBROUTINE GEN_TRANS(GRID_SUB, GRID, TRANS)
    ...
    IF (NGX>GRID%NGX) THEN
       CALL vtutor%bug("internal error in GEN_TRANS: GRID_SUB is not a subgrid of GRID", "mgrid.F", 447)
    ENDIF
...
! now check that only (1._q,0._q) node contains all orbital data
       IF (GRID%COMM%NCPU/=1) THEN
          CALL vtutor%bug("internal ERROR in SET_RL_GRID: GRID contains more than one CPU " // &
             str(GRID%COMM%NCPU), "mgrid.F", 521)
       ENDIF
       ...
       DO N3=1,GRID%NGZ
          DO N2=1,GRID%NGY
             ...
             IF (GRID%RL%I2(IND)/=N2 .OR. GRID%RL%I3(IND)/=N3) THEN
                CALL vtutor%bug("internal ERROR in GEN_GRID_HF: orbital GRID not sequential " // &
                   str(IND) // " " // str(GRID%RL%I2(IND)) // " " // str(N2) // " " // &
                   str(GRID%RL%I3(IND)) // " " // str(N3), "mgrid.F", 531)
             ENDIF
          ENDDO
       ENDDO
# 541
...
! distribute columns among proc
    ...
    IF (GRID%RC%NCOL >NCOL_MAX) THEN
       CALL vtutor%bug("REC_STDLAY: internal error " // str(NCOL_MAX) // " " // str(GRID%RC%NCOL), "mgrid.F", 732)
    ENDIF
...
! distribute planes among proc
    ...
    IF (GRID%IN%NCOL >NCOL_MAX) THEN
       CALL vtutor%bug("INTER_STDLAY: internal error " // str(NCOL_MAX) // " " // str(GRID%IN%NCOL), "mgrid.F", 789)
    ENDIF
...
! distribute planes among proc
       ...
       IF (GRID%RL%NCOL >NCOL_MAX) THEN
          CALL vtutor%bug("REAL_STDLAY: internal error " // str(NCOL_MAX) // " " // str(GRID%RL%NCOL), "mgrid.F", 855)
       ENDIF
...
!
    IF (GRID_DEST%NGX /= GRID_SRC%NGX .OR. GRID_DEST%NGY /= GRID_SRC%NGY .OR. GRID_DEST%NGZ /= GRID_SRC%NGZ) THEN
       CALL vtutor%bug("internal ERROR in SET_RL_GRID: GRID_DEST is not conform to GRID_SRC", "mgrid.F", 930)
    ENDIF
!
...
! now check that only (1._q,0._q) node contains all data
       IF (GRID_SRC%COMM%NCPU/=1) THEN
          CALL vtutor%bug("internal ERROR in SET_RL_GRID: GRID_SRC contains more than one CPU " // &
             str(GRID_SRC%COMM%NCPU), "mgrid.F", 946)
       ENDIF
! now check that the data layout is strictly sequential in real space
       IND=0
       DO N3=1,GRID_SRC%NGZ
          DO N2=1,GRID_SRC%NGY
             ...
             IF (GRID_SRC%RL%I2(IND)/=N2 .OR. GRID_SRC%RL%I3(IND)/=N3) THEN
                CALL vtutor%bug("internal ERROR in SET_RL_GRID: GRID_SRC not sequential " // &
                   str(IND) // " " // str(GRID_SRC%RL%I2(IND)) // " " // str(N2) // " " // &
                   str(GRID_SRC%RL%I3(IND)) // " " // str(N3), "mgrid.F", 956)
             ENDIF
          ENDDO
       ENDDO
...
! allocate RL_FFT structure
    ...
    ELSE
       IF (GRID_SRC%RL%NFAST/=3) THEN
          CALL vtutor%bug("internal ERROR in SET_RL_GRID: GRID_SRC has a strange NFAST index " // &
             str(GRID_SRC%RL%NFAST), "mgrid.F", 968)
       ENDIF
    ENDIF
!
...
# 1011
  ...
  SUBROUTINE  SET_RL_GRID_OLD(GRID_DEST,GRID_SRC)
    ...
    DEALLOCATE(GRID_DEST%RL%I2,GRID_DEST%RL%I3,GRID_DEST%RL%INDEX)
    IF (GRID_DEST%NGX /= GRID_SRC%NGX .OR. GRID_DEST%NGY /= GRID_SRC%NGY .OR. GRID_DEST%NGZ /= GRID_SRC%NGZ) THEN
       CALL vtutor%error("ERROR in SET_RL_GRID: GRID_DEST is not conform to GRID_SRC")
    ENDIF
    IF (GRID_DEST%COMM%NODE_ME <= GRID_SRC%COMM%NCPU) THEN
       GRID_DEST%RL%NFAST= GRID_SRC%RL%NFAST
       GRID_DEST%RL%NCOL = GRID_SRC%RL%NCOL
...
!
!> set up a "metric" for evaluating scalar products
!> on a grid
!>
!> such as
!> ~~~
...
!*************************SUBROUTINE RC_FLIP ***************************
!
!> rearrange the storage mode for spin components of charge density:
!>
...
!! USE moffload_struct_def
    ...
    IF (ISPIN==2) THEN
       ...
       FAC=1._q
       IF (LBACK) FAC=0.5_q
!$ACC PARALLEL LOOP PRESENT(CHTOT) PRIVATE(CQU,CQD) 
       DO K=1,GRID%RC%NP
          CQU=CHTOT(K,1)
          CQD=CHTOT(K,2)
...
!$ACC PARALLEL LOOP PRESENT(CHTOT) PRIVATE(C00,C01,C10,C11) 
       DO K=1,GRID%RC%NP
          C00=CHTOT(K,1)
          C01=CHTOT(K,2)
          C10=CHTOT(K,3)
    ...
    ELSE IF ( ISPIN==4 .AND. LBACK) THEN
       FAC=0.5_q
!$ACC PARALLEL LOOP PRESENT(CHTOT) PRIVATE(C00,CX,CY,CZ) 
       DO K=1,GRID%RC%NP
          C00=CHTOT(K,1)
          CX =CHTOT(K,2)
          CY =CHTOT(K,3)
...
!
!> version of #RC_FLIP for potentials
...
!
!>  version of #RC_FLIP for operating in real space (possibly real arrays)
!
...
!! USE moffload_struct_def
    ...
    IF (ISPIN==2) THEN
       FAC=1._q
       IF (LBACK) FAC=0.5_q
!$ACC PARALLEL LOOP PRESENT(CHTOTR) PRIVATE(CQU,CQD) 
       DO K=1,GRID%RL%NP
          CQU=CHTOTR(K,1)
          CQD=CHTOTR(K,2)
       ...
       ENDDO
    ELSE IF ( ISPIN==4 .AND. .NOT. LBACK) THEN
!$ACC PARALLEL LOOP PRESENT(CHTOTR) PRIVATE(C00,C01,C10,C11) 
       DO K=1,GRID%RL%NP
          C00=CHTOTR(K,1)
          C01=CHTOTR(K,2)
          C10=CHTOTR(K,3)
    ...
    ELSE IF ( ISPIN==4 .AND. LBACK) THEN
       FAC=0.5_q
!$ACC PARALLEL LOOP PRESENT(CHTOTR) PRIVATE(C00,CX,CY,CZ) 
       DO K=1,GRID%RL%NP
          C00=CHTOTR(K,1)
          CX =CHTOTR(K,2)
          CY =CHTOTR(K,3)
...
!$ACC KERNELS PRESENT(C) 
    C=0
!$ACC END KERNELS
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(NX,NY) PRESENT(C,C_LOCAL,GRID) 
...
! and pick up data
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(NX,NY) PRESENT(C,C_LOCAL) 
    DO NC=1,GRID%RL%NCOL
  NX= GRID%RL%I2(NC)
  NY= GRID%RL%I3(NC)
       DO NZ=1,GRID%NGZ
...
!> ~~~
!> in REAL(q) SPACE
!>  depending on the mode this is 1._q using real or complex arrays
...
!$ACC PARALLEL LOOP PRESENT(C,A,B) 
       DO N=1,NPLWV
          C(N)=A(N)*B(N)
       ENDDO
!  )  else
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(GRID_SOFT,CSUM,SOFT_TO_C,CADD) 
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(GRID_SOFT,CDEST,SOFT_TO_C,CSRC) 
    col: DO NC=1,GRID_SOFT%RC%NCOL
       row: DO N1=1,GRID_SOFT%RC%NROW
...
!***********************************************************************
!
!>  Bring an array defined on the fine grid
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CDEST,CSRC,SOFT_TO_C) 
    col: DO NC=1,GRID_SOFT%RC%NCOL
       row: DO N1=1,GRID_SOFT%RC%NROW
...
!***********************************************************************
!
!>  Add an array defined on the fine grid
...
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(CDEST,CSRC,GRID_SOFT,SOFT_TO_C) 
    col: DO NC=1,GRID_SOFT%RC%NCOL
       row: DO N1=1,GRID_SOFT%RC%NROW
...
!*************************SUBROUTINE INIDAT ****************************
!>  Initialize a workarray to some non (0._q,0._q) data
!***********************************************************************
...
!>  parallel version currently not supported
!
!**********************************************************************
