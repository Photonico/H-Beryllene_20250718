...
!c convert to cartesian coordinates and calculate EKIN
         DO NI=1,NIONS
           VTMP(1)=   V(1,NI)
           VTMP(2)=   V(2,NI)
           VTMP(3)=   V(3,NI)
...
!c convert to cartesian coordinates and calculated EKIN
          EKIN=0
          DO NI=1,NIONS
            VTMP(1)=   V(1,NI)
            VTMP(2)=   V(2,NI)
...
!c normal distributed variable X with E(X)=MU and V(X)=SIGMA^2
       ...
       END FUNCTION boltzmann_velocity
...
!> The case GGA=PB (DFTmethod='bp') is not considered anymore since BP was removed.
   FUNCTION VERBALIZE_DFTMETHOD(XC) RESULT(DFTmethod)
