...
!------------------------------------------------------
  subroutine rotate_wave_spin_recip(wdes, smat, nk, spinflip, cw)
    ...
    call rotate_wave_spin_recip_low(wdes%lnoncollinear, smat, wdes%ngvector(nk), spinflip, cw)
...
!> Rotates spin-part of wfn in reciprocal space.
!------------------------------------------------------
  subroutine rotate_wave_spin_recip_low(lnoncollinear, smat, ngvector, spinflip, cw)
    use wave
    use tutor, only: vtutor
    implicit none
    logical :: lnoncollinear
    complex(q),intent(in)     :: smat(2,2)
    integer, intent(in)       :: ngvector
    integer, intent(in)       :: spinflip
    complex(q),intent(inout)  :: cw(:)
    ...
    call rot_wave(cw, smat, ngvector)

  end subroutine rotate_wave_spin_recip_low
...
# 356
    ...
    call ZCOPY(2*npl,tmp(1),1,wf(1),1)
...
!$acc enter data create(tmpproj) 
!$acc kernels present(tmpproj) 
    tmpproj=0
!$acc end kernels
# 403
    ...
    call ZCOPY(wdes1%npro,tmpproj(1),1,cproj(1),1)
!$acc exit data delete(tmpproj) 
    deallocate(tmpproj)
