...
MODULE david_inner
  USE hamil 
CONTAINS
!************************ SUBROUTINE EDDAV_INNER ************************
! RCS:  $Id: davidson.F,v 1.5 2003/06/27 13:22:15 kresse Exp kresse $
!
!> Performes a Davidson like optimsation of the
...
!>
!>  EINFO\%BREAK  absolut break condition
...
!> wavefunction currently added to subspace
...
!PK Just copy the recipe over from davidson.F
       CALL vtutor%error("EDDAV_INNER: KPAR>1 not implemented, sorry.")
    END IF
# 162
...
!=======================================================================
    ...
    IF (NSIM_LOCAL*NCPU /= NSIM) THEN
       CALL vtutor%bug("internal ERROR in EDDAV NSIM is not correct " // str(NSIM), "david_inner.F", 170)
    ENDIF
...
! stop if imaginary part /= 0
                IF(ABS(AIMAG(ALPHA(I)/BETA(I)))>1.E-6_q)THEN
                   CALL vtutor%error("complex eigenvalue( " // str(I) // ")= " // str(ALPHA(I)/BETA(I)))
                END IF
                R(I)=ALPHA(I)/BETA(I)
             END DO
             NDIM_=NDIM
...
# 817
             ...
             END DO
             IF (IFAIL/=0) THEN
                CALL vtutor%error("EDDAV LAPACK: Routine ZPOTRF failed! " // str(IFAIL))
             ENDIF
# 831
             ...
             IF (IFAIL/=0) THEN
                CALL vtutor%error("EDDAV LAPACK: Routine ZTRTRI failed! " // str(IFAIL))
             ENDIF
...
! subtract  |beta_k> W1%CPROJ(j,k)
             IF (INFO%LREAL) THEN
                ...
                DO ISPINOR=0,WDES1%NRSPINORS-1
                   CALL FFTEXT(WDES1%NGVECTOR,WDES1%NINDPW, W1(NP)%CR(1+ISPINOR*WDES1%GRID%MPLWV), W1(NP)%CPTWFP(1+ISPINOR*WDES1%NGVECTOR),GRID,.TRUE.)
                ENDDO
             ELSE
                DO ISPINOR=0,WDES1%NRSPINORS-1
...
# 1283
    ...
    IF (IFAIL/=0) THEN
       CALL vtutor%bug("internal ERROR in PROJOVL: Call to routine ZHEEV failed! Error code was " &
          // str(IFAIL), "david_inner.F", 1288)
    ENDIF
...
# 1327
    ...
    IF (IFAIL/=0) THEN
       CALL vtutor%bug("internal ERROR in PROJOVL: Call to routine ZHEGV failed! Error code was " &
          // str(IFAIL), "david_inner.F", 1333)
    ENDIF
