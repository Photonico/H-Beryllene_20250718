...
! the following matrices store are sub range of the full matrices AMAT, BMAT and VMAT
    COMPLEX(q), ALLOCATABLE :: TWOELECTRON3O(:,:,:,:)   ! part of the A matrix resonant-resonant coupling
    COMPLEX(q), ALLOCATABLE :: BTWOELECTRON3O(:,:,:,:)  ! part of the B matrix resonant-antiresonant coupling
    COMPLEX(q), ALLOCATABLE :: TWOE3O_HARTREE(:,:,:,:)  ! part of Coulomb kernel in four orbital representation (Hartree only no exchange)
    TYPE (wavespin) WHF
...
# 247
    ...
    COMPLEX(q), ALLOCATABLE :: AMAT(:,:), AMAT_SCALA(:)
    COMPLEX(q), ALLOCATABLE :: BMAT(:,:), BMAT_SCALA(:)
    COMPLEX(q), ALLOCATABLE :: VMAT(:,:), VMAT_SCALA(:)
    COMPLEX(q), ALLOCATABLE :: AVpW(:,:), AVpW_SCALA(:)
    COMPLEX(q), ALLOCATABLE :: BVpW(:,:), BVpW_SCALA(:)
...
! smaller values are dangerous and are usually not what the user desires
    ...
    IF (ISP_IN<0 .AND. WDES%ISPIN==1) THEN
       CALL vtutor%bug("internal error in CALCULATE_BSE: ISP is negative but ISPIN=1", "bse.F", 347)
    ENDIF

    IF (WDES%LGAMMA .AND. (.NOT. WGW%GRID%REAL2CPLX .OR. .NOT. WGW%GRID%LREAL)) THEN
       CALL vtutor%bug("internal error in CALCULATE_BSE: WGW must apply real to complex FFT", "bse.F", 351)
    ENDIF
!=======================================================================
! preparation
!=======================================================================
...
! QP calculation does not yet support 1
...
!==========================================================================
    IF ( IBSE==0) THEN
! if 1 is available initialize BSE_DESC
...
# 634
          ...
          IF (IO%IU0>=0) WRITE(IO%IU0,'(A,F8.3,A,I7)') ' BSE attempting extended potential matrix allocation of',8.E-9_q*2_qi8*NSIZE,' Gbyte  rank=',NAB
          IF (IO%IU6>=0) WRITE(IO%IU6,'(A,F8.3,A,I7)') ' BSE attempting extended potential matrix allocation of',8.E-9_q*2_qi8*NSIZE,' Gbyte  rank=',NAB
          CALL REGISTER_ALLOCATE(8._q*2_qi8* NSIZE , "pot")

       ENDIF

!==========================================================================
    ELSE ! initialisation for the non-1-aware case
!==========================================================================
       SIZE8A=NCV
       SIZE8B=NCV
...
# 672
       ...
       IF (IO%IU0>=0) WRITE(IO%IU0,'(A,F8.3,A,I7)') ' BSE attempting matrix allocation of',8.E-9_q*2_qi8*NSIZE,' Gbyte  rank=',NCV
       IF (IO%IU6>=0) WRITE(IO%IU6,'(A,F8.3,A,I7)') ' BSE attempting matrix allocation of',8.E-9_q*2_qi8*NSIZE,' Gbyte  rank=',NCV
       CALL REGISTER_ALLOCATE(8._q*2_qi8* NSIZE , "bse")
...
! "SIZE8" replaces expression "NSTRIPV*NCBD*NSTRIPV*NCBD4" of the original code
    ...
    IF (ALLOCATED(VMAT).OR. ALLOCATED(VMAT_SCALA)) THEN
       ...
       CALL REGISTER_ALLOCATE(8._q*2_qi8* SIZE(TWOE3O_HARTREE, KIND=qi8) , "bse")
    ENDIF
...
!     another few changes in the following lines ...
       CALL DEREGISTER_ALLOCATE(8._q*2_qi8* SIZE(TWOELECTRON3O, KIND=qi8) , "bse")
       DEALLOCATE(TWOELECTRON3O)
       IF (ANTIRES>=2) THEN
          CALL DEREGISTER_ALLOCATE(8._q*2_qi8* SIZE(BTWOELECTRON3O, KIND=qi8) , "bse")
          DEALLOCATE(BTWOELECTRON3O)
       ENDIF
       IF (ALLOCATED(TWOE3O_HARTREE)) THEN
          CALL DEREGISTER_ALLOCATE(8._q*2_qi8* SIZE(TWOE3O_HARTREE, KIND=qi8) , "bse")
...
! deallocation frees precious memory for the 1 calls
       IF (.NOT. LCORBSE .AND. (.NOT. LQP) .AND. NBSEEIG == 0)  THEN
...
# 1167
       ...
       CALL M_sum_z8(WGW%COMM_INTER, AMAT  , SIZE(AMAT,KIND=qi8))
       IF (ANTIRES>=2) THEN
          CALL M_sum_z8(WGW%COMM_INTER, BMAT  , SIZE(BMAT,KIND=qi8))
       ENDIF
       IF (ALLOCATED(VMAT)) THEN
          CALL M_sum_z8(WGW%COMM_INTER, VMAT, SIZE(VMAT,KIND=qi8))
       ENDIF
       IF (ALLOCATED(AVpW)) THEN
          CALL M_sum_z8(WGW%COMM_INTER, AVpW, SIZE(AVpW,KIND=qi8))
       ENDIF
       IF (ALLOCATED(BVpW)) THEN
          CALL M_sum_z8(WGW%COMM_INTER, BVpW, SIZE(BVpW,KIND=qi8))
       ENDIF
...
!========================================================================
! compute AC-correlation energy if there is more than (1._q,0._q) integration point
       IF(NALPHA > 1 ) THEN
         IF ( LscaLAPACKaware) THEN
...
!================================================================================
  ...
  IF (ANY(BSEHOLE /= -123456.0_q) .OR. ANY(BSEELECTRON /= -123456.0_q)) THEN
...
# 1513
          ...
          CALL DEREGISTER_ALLOCATE(8._q*2_qi8* NSIZE , "pot")
                ...
                IF (K1==K2 .AND. NB1_INTO_TOT==NB2_INTO_TOT .AND. NB3_INTO_TOT==NB4_INTO_TOT .AND.  ISP== ISP2) THEN
                   IF (NCV13/= NCV24) THEN
                      CALL vtutor%bug("internal error in TWOELECTRON4O_STORE: not a diagonal element " &
                         // str(NCV13) // " " // str(NCV24) // "\n " // str(N1) // " " // str(N2) &
                         // " " // str(N3) // " " // str(N4) // " " // str(K1) // " " // str(K2), "bse.F", 1714)
                   ENDIF
                   IF(IDIAG>0) THEN
...
! since a similar thing misses in TWOELECTRON4O_STORE
                IF (NCV13P/=0 .AND. NCV24P/=0 .AND. LTRIANGLE) THEN
                   AMAT(NCV13P,NCV24P)=CONJG(TWOELECTRON3O( N1, NB3 , N2, NB4))*WHF%WDES%WTKPT(1)*ALPHAPT
                ENDIF
             ENDDO
          ENDDO
       ENDDO
...
! test on NB4_INTO_TOT
                ...
                IF (NCV13 > NCV24) THEN
                   AMAT(IROW+(JCOL-1)*DESC(LLD_))= &
                        CONJG(TWOELECTRON3O_LOCAL(N1, NB3, N2, NB4 ))*WHF%WDES%WTKPT(1)*SCALE_PERT*ALPHAPT
                ENDIF
             ENDDO
          ENDDO
       ENDDO
...
!> when this is not stored in a 1 aware way
!
!**********************************************************************
       ...
       ELSE
          ...
          IF (LAMBDA /= DESC(NB_)*MYCOL+NPCOL*(J1-1)+J2) THEN
             CALL vtutor%bug("internal error in CALCULATE_BSE_OSZI_SCALA: " // str(LAMBDA) // " " &
                // str(DESC(NB_)) // " " // str(NPCOL*(J1-1)) // " " // str(J2), "bse.F", 2556)
          ENDIF
       ENDIF
...
! now sum CTRANS over all nodes for the current LAMBDA
       CALL M_sum_z(WHF%WDES%COMM_INTER, CTRANS, 3)
...
# 3036
    ...
    COMPLEX(q), INTENT(IN OUT)  :: AMAT(:,:)      !< in: matrix to be diagonalized
!< out: eigenvectors
    COMPLEX(q), INTENT(IN OUT), ALLOCATABLE :: AMAT_SCALA(:) !< in: matrix distributed according to DESC
...
# 3179
          ...
          CALL M_sum_z8(MY_COMM, AMAT  , SIZE(AMAT,KIND=qi8))
...
# 3290
    ...
    COMPLEX(q), INTENT(IN OUT)  :: A(:,:)         !< in: matrix A
!< out:  U R U+ A U R U+
    COMPLEX(q), INTENT(IN)      :: U(:,:)         !< eigenvectors of (A-B)
...
! TODO gK: we need (1._q,0._q) subroutine to handle negative eigenvalues in RB
...
!> Furthermore, (1._q,0._q) can show that
!> ~~~
!>  (X+Y) (X-Y)+  = 1
!> ~~~
!> and
...
!< out: X
    ...
    COMPLEX(q), INTENT(IN OUT)   :: U(:,:)      !< eigenvectors of A-B
!< out: Y
    COMPLEX(q),ALLOCATABLE :: T1(:,:),T2(:,:)   !< auxilary
...
!  U+ Z -> T2
    CALL PZGEMM( 'C','N', NCV, NCV, NCV, (1._q,0._q), &
         U(1), 1, 1, DESC, Z(1), 1, 1, DESC, (0._q,0._q), T2(1), 1, 1, DESC)
...
!  Z =(X+Y)= U T1 -> A
    CALL PZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), &
         U(1), 1, 1, DESC, T1(1), 1, 1, DESC, (0._q,0._q), Z(1), 1, 1, DESC)

!  T1=(X-Y)= U T2 -> A
    CALL PZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), &
         U(1), 1, 1, DESC, T2(1), 1, 1, DESC, (0._q,0._q), T1(1), 1, 1, DESC)

!  test multiply Z x U should be equal unity matrix
! U=T1
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!     Z(1), 1, 1, DESC, T1(1), 1, 1, DESC, (0._q,0._q), T2(1), 1, 1, DESC)
!DO I=1,NCV
!   WRITE(77,'(10E20.10)') T2(:,I)
!ENDDO
...
! test calculate X X+ - Y Y+  should be equal unity matrix
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!   Z(1), 1, 1, DESC, Z(1), 1, 1, DESC, (0._q,0._q), T1(1), 1, 1, DESC)
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, -(1._q,0._q), &
!   U(1), 1, 1, DESC, U(1), 1, 1, DESC, (1._q,0._q), T1(1), 1, 1, DESC)
! DO I=1,NCV*NCV
!    WRITE(77,'(10E20.10)') T1(I)
! ENDDO
...
# 4143
    ...
    COMPLEX(q)  :: X(:,:), Y(:,:), H(:,:)
    COMPLEX(q),ALLOCATABLE :: T1(:,:),T2(:,:),T3(:,:),TMP(:,:),EYE(:,:)
...
! test calculate X X+ - Y Y+  should be equal unity matrix
!  CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!    X(1,1), NCV, X(1,1), NCV, (0._q,0._q), T2(1,1), NCV)
!  CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, -(1._q,0._q), &
!    Y(1,1), NCV, Y(1,1), NCV, (1._q,0._q), T2(1,1), NCV)
!  DO I=1,NCV
!     WRITE(77,*) T2(:,I)
!  ENDDO
!  WRITE(77,*) '    '
...
! XX+ --> T1
      CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
        X(1,1), NCV, X(1,1), NCV, (0._q,0._q), T1(1,1), NCV)

! YY+ --> T2
      CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
        Y(1,1), NCV, Y(1,1), NCV, (0._q,0._q), T2(1,1), NCV)

! XY+ --> T3
      CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
        X(1,1), NCV, Y(1,1), NCV, (0._q,0._q), T3(1,1), NCV)

! test: according to Scuseria et al. (J. Chem. Phys. 2008, 129, 231101)
! Y+X = X+Y (where + indicates adjoint); check that XY+ = YX+
!  CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!    Y(1,1), NCV, X(1,1), NCV, -(1._q,0._q), T3(1,1), NCV)
!  DO I=1,NCV
!     WRITE(78,*) T3(:,I)
!  ENDDO
!  WRITE(78,*) '    '
...
! now contract T1 and T3 with the Hartree kernel:
     CALL ZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), &
       H(1,1), NCV, TMP(1,1), NCV, (0._q,0._q), T1(1,1), NCV)
     CALL ZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), &
       H(1,1), NCV, T3(1,1), NCV, (0._q,0._q), T2(1,1), NCV)
...
!**********************************************************************
!
!> In this 1-aware routine the density matrix P_alpha = |X,Y><X,Y| is
!> constructed and some orthogonality properties are tested.
!> The matrix is then contracted with the Coulomb superoperator
!> to yield the correlation energy using the plasmon equation.
!
...
# 4244
    ...
    COMPLEX(q)  :: X(:), Y(:), H(:)
    COMPLEX(q),ALLOCATABLE :: T1(:),T2(:),T3(:),TMP(:)
...
! XX+ --> T1
      CALL PZGEMM( 'N','C' , NCV, NCV, NCV, (1._q,0._q), &
         X(1), 1, 1, DESC, X(1), 1, 1, DESC, (0._q,0._q), T1(1), 1, 1, DESC)

! YY+ --> T2
      CALL PZGEMM( 'N','C' , NCV, NCV, NCV, (1._q,0._q), &
         Y(1), 1, 1, DESC, Y(1), 1, 1, DESC, (0._q,0._q), T2(1), 1, 1, DESC)

! test calculate X X+ - Y Y+ should be equal unity matrix
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!     X(1), 1, 1, DESC, X(1), 1, 1, DESC, (0._q,0._q), T1(1), 1, 1, DESC)
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, -(1._q,0._q), &
!     Y(1), 1, 1, DESC, Y(1), 1, 1, DESC, (1._q,0._q), T1(1), 1, 1, DESC)
!  JCOL=0
!  DO J1=1,NQ,DESC(NB_)
!     J1RES=MIN(DESC(NB_),NQ-J1+1)
!     DO J2=1,J1RES
...
! XY+ --> T3
      CALL PZGEMM( 'N','C' , NCV, NCV, NCV, (1._q,0._q), &
         X(1), 1, 1, DESC, Y(1), 1, 1, DESC, (0._q,0._q), T3(1), 1, 1, DESC)
...
! V*TMP --> T1 ; W*T3 --> T2
      CALL PZGEMM( 'N','N' , NCV, NCV, NCV, (1._q,0._q), &
         H(1), 1, 1, DESC, TMP(1), 1, 1, DESC, (0._q,0._q), T1(1), 1, 1, DESC)

      CALL PZGEMM( 'N','N', NCV, NCV, NCV, (1._q,0._q), &
         H(1), 1, 1, DESC, T3(1), 1, 1, DESC, (0._q,0._q), T2(1), 1, 1, DESC)
...
! 2 particle or 2 hole states
    ...
    INTEGER :: K1_IN_IRZ, K3_IN_IRZ ! momenta in the irreducible Brillouin (1._q,0._q)
    ...
    COMPLEX(q)    :: CDER_BETWEEN_STATE(3), AHEAD
...
! to K3 yielding K3_IN_IRZ
         ...
         IF (K1_IN_IRZ > SIZE(SE_SPECTR,3)) THEN
            CALL vtutor%bug("internal error in SELFEN_2ndO: K1_IN_IRZ exceeds SE_SPECTR " // &
               str(K1_IN_IRZ) // " " // str(SIZE(SE_SPECTR,3)), "bse.F", 4479)
         ENDIF
...
! if state 3 is occupied, same sign as 2
               ...
               SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)=SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)- V(IV,J)*CONJG(IRRVB(IV,J))*P_PLUS*S2E%WTKPT(K1_IN_IRZ, K3_IN_IRZ)
               NPIN=INT((-ERES2-OMEGAMIN)/((OMEGAMAX-OMEGAMIN)/REAL(NOMEGA-1))+1.5)
               NPIN=MIN(MAX(1,NPIN),NOMEGA)
               SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)=SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)+ V(IV,J)*CONJG(IRRVA(IV,J))*P_MINUS*S2E%WTKPT(K1_IN_IRZ, K3_IN_IRZ)
            ENDDO
         ELSE
            DO J = 1, NCV
               N2  = BSE_INDEX%N1(J)
...
! if state 3 is occupied, same sign as 2
               ...
               NPIN=INT((ERES1-OMEGAMIN)/((OMEGAMAX-OMEGAMIN)/REAL(NOMEGA-1))+1.5)
               NPIN=MIN(MAX(1,NPIN),NOMEGA)
               SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)=SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)- V(IV,J)*CONJG(V(IV,J))*P_PLUS*S2E%WTKPT(K1_IN_IRZ, K3_IN_IRZ)
               NPIN=INT((-ERES2-OMEGAMIN)/((OMEGAMAX-OMEGAMIN)/REAL(NOMEGA-1))+1.5)
               NPIN=MIN(MAX(1,NPIN),NOMEGA)
               SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)=SE_SPECTR(NPIN,N1,K1_IN_IRZ,ISP)+ V(IV,J)*CONJG(V(IV,J))*P_MINUS*S2E%WTKPT(K1_IN_IRZ, K3_IN_IRZ)
            ENDDO ! over J
         ENDIF
      ENDDO ! over I
...
! 3b) knowing NQ obtain |n4, k4, ISP2 >
            ...
            AHEAD=(CDER_BETWEEN_STATE(1)*CONJG(CDER_BETWEEN_STATE(1)))*(EDEPS/LATT_CUR%OMEGA)*W%WDES%RSPIN*W%WDES%RSPIN*W%WDES%WTKPT(K2)
       ...
       ELSE
          IF (J /= DESC(NB_)*MYCOL+NPCOL*(J1-1)+J2) THEN
             CALL vtutor%bug("internal error in SELFEN_2ndO_SCALA: " // str(J) // " " // &
                str(DESC(NB_)) // " " // str(NPCOL*(J1-1)) // " " // str(J2), "bse.F", 4774)
          ENDIF
       ENDIF
       IF (JCOL<0) THEN
          CTMP=0._q
...
! 2a) knowing NQ obtain |n3, k3, ISP >
          ...
          IF(JV/=NCV13) THEN
            CALL vtutor%bug("internal error1 in SE2ndORD_SCALA: " // str(NCV13) // " " // str(JV) &
               // " " // str(DESC(NB_)) // " " // str(NPCOL*(J1-1)), "bse.F", 4795)
          ENDIF
! if there is no potential matrix element calculated, CYCLE
          IF( JV==0) CYCLE
...
!WRITE(*,*) 'internal error3 in SE2ndORD_SCALA:',NCV24,IV,DESCSTD(MB_)*MYROW,NPROW*(I1-1)+I2
                CALL vtutor%bug("internal error3 in SE2ndORD_SCALA: " // str(NCV24) // " " // &
                   ...
                   str(N4) // " " // str(K2) // " " // str(ISP2) // " " // str(IB), "bse.F", 4853)
                ENDIF
! if the index does not identify a particle-hole pair then  CYCLE
                IF(NCV24==0) CYCLE
                IF( IB==0) CYCLE
...
! if state 3 is empty same sign as 4
                ...
                CTMP(NPIN_RES)=CTMP(NPIN_RES)-VMAT(IROW+(JCOL-1)*DESC(LLD_))*CONJG(VMAT(IROW+(JCOL-1)*DESC(LLD_)))*P_PLUS
...
! if state 3 is occupied, same sign as 2
                ...
                CTMP(NPIN_ANT)=CTMP(NPIN_ANT)+VMAT(IROW+(JCOL-1)*DESC(LLD_))*CONJG(VMAT(IROW+(JCOL-1)*DESC(LLD_)))*P_MINUS
...
! now sum CTMP over all nodes for the current J
       CALL M_sum_z(W%WDES%COMM_INTER, CTMP, NOMEGA)
...
# 4958
    ...
    COMPLEX(q) :: X(:,:)       !< eigenvectors of BSE contributions X
    COMPLEX(q) :: Y(:,:)       !< eigenvectors of BSE contributions Y
    COMPLEX(q) :: V(:,:)       !< Coulomb kernel spans also particle-particle manyfold
    COMPLEX(q) :: IRRVA(:,:)   !< A matrix of BSE spans also particle-particle manyfold
    COMPLEX(q) :: IRRVB(:,:)   !< B matrix of BSE spans also particle-particle manyfold
...
! 2 particle or 2 hole states
    ...
    INTEGER :: NCV      ! number of e-h pairs in the BSE matrix
    INTEGER :: K1, K2, K3, K4  ! momentum counters
    INTEGER :: K1_IN_IRZ, K3_IN_IRZ ! momenta in the irred Brillouin (1._q,0._q)
    INTEGER :: N1, N2, N3, N4  ! band counters
    INTEGER :: ISP, ISP2       ! spin variables
    INTEGER :: ILEN, IBLOCK    ! for strip-wise matrix multiplication
    COMPLEX(q), ALLOCATABLE :: AHEAD(:), TMPHEAD(:)
    COMPLEX(q)    :: CDER_BETWEEN_STATE(3)
    INTEGER :: K2_IN_FULL_ORIG
...
# 5019
    ...
    COMPLEX(q), ALLOCATABLE :: V_SAVE(:,:), IRRVA_SAVE(:,:), IRRVB_SAVE(:,:)
...
! preliminary sanity tests
    ...
    IF ( .NOT. ASSOCIATED(SEFROMBSE%REAL_PART)) THEN
       CALL vtutor%bug("internal error in SELF_FULL: SELFENERGY data structure not allocated", "bse.F", 5042)
    ENDIF
...
! manifold along (1._q,0._q) dimension.
!=============================================================================
...
! set V_lambda = V (X_lambda+Y_lambda)
      CALL ZGEMM( 'N', 'N', ILEN, NCV, NCV, (1._q,0._q), &
         V(IBLOCK+1,1), POT_INDEX%NCV, X(1,1), NCV, (0._q,0._q), VR(1,1), ILEN )

      CALL ZGEMM( 'N', 'N', ILEN, NCV, NCV, (1._q,0._q), &
         V(IBLOCK+1,1), POT_INDEX%NCV, Y(1,1), NCV, (1._q,0._q), VR(1,1), ILEN )
...
! VN = IRRVA*X + IRRVB*Y --> IRRVB
         CALL ZGEMM( 'N', 'N', ILEN, NCV, NCV, (1._q,0._q), &
            IRRVA(IBLOCK+1,1), POT_INDEX%NCV, X(1,1), NCV, (0._q,0._q), VR(1,1), ILEN )

         CALL ZGEMM( 'N', 'N', ILEN, NCV, NCV, (1._q,0._q), &
            IRRVA(IBLOCK+1,1), POT_INDEX%NCV, Y(1,1), NCV, (0._q,0._q), VN(1,1), ILEN )

         CALL ZGEMM( 'N', 'N', ILEN, NCV, NCV, (1._q,0._q), &
            IRRVB(IBLOCK+1,1), POT_INDEX%NCV, X(1,1), NCV, (1._q,0._q), VN(1,1), ILEN )

         CALL ZGEMM( 'N', 'N', ILEN, NCV, NCV, (1._q,0._q), &
            IRRVB(IBLOCK+1,1), POT_INDEX%NCV, Y(1,1), NCV, (1._q,0._q), VR(1,1), ILEN )
...
!!$    IF (MOD(I-1,OMP_GET_NUM_THREADS())/=OMP_GET_THREAD_NUM()) CYCLE
       ...
       K1  = POT_INDEX%NK(I)
       ISP = POT_INDEX%ISP(I)
! skip those that will have (0._q,0._q) weights below (WTKPT)
...
! to K3 yielding K3_IN_IRZ
       ...
       IF (K1_IN_IRZ > SIZE(SE_SPECTR,3)) THEN
          CALL vtutor%bug("internal error in SELFEN_FULL: K1_IN_IRZ exceeds SE_SPECTR " // &
             str(K1_IN_IRZ) // " " // str(SIZE(SE_SPECTR,3)), "bse.F", 5214)
       ENDIF
       K3_IN_IRZ=S2E%K2_IN_IRZ(NQ, K1)
...
! if state 3 is occupied, same sign as 2
    ...
    CALL M_sum_d(W%WDES%COMM_INTER, SE_SPECTR, SIZE(SE_SPECTR))
...
! and twice the 2nd order direct diagram G (I P_0 V) (instead of once)
    DO ISP=1,SEFROMBSE%ISPIN
       DO K1=1,SEFROMBSE%NKPTS
          DO N1=1,SEFROMBSE%NBANDS
             SEFROMBSE%REAL_PART(:,N1,K1,ISP) = REAL(SE_SPECTR(:,N1,K1,ISP),q)*(1.0_q/AHARTREE)
! the imaginary part should be (0._q,0._q), not quite obvious for beyond RPA
...
!> return the (1._q,0._q) with the largest Z factor
!
!**********************************************************************
...
! local<s
     ...
     NOMEGA=SIZE(OMEGA)
! search all nodes and select (1._q,0._q) closest to EVAL
       ...
       ELSE
          IF (LAMBDA /= DESC(NB_)*MYCOL+NPCOL*(J1-1)+J2) THEN
             CALL vtutor%bug("internal error in WRITE_BSE_SCALA: " // str(LAMBDA) // " " // &
                str(DESC(NB_)) // " " // str(NPCOL*(J1-1)) // " " // str(J2), "bse.F", 5898)
          ENDIF
       ENDIF
             ...
             DO I2=1,I1RES
                ...
                IF (NCV13/=BSE_INDEX%INDEX(NB1_INTO_TOT-BD(ISP)%VBMIN+1, NB3_INTO_TOT-BD(ISP)%CBMIN+1, K1, ISP -ISP_LOW+1)) THEN
                   CALL vtutor%bug("internal error in WRITE_BSE_SCALA: " // str(NCV13) // " " // &
                      ...
                      NB3_INTO_TOT-BD(ISP)%CBMIN+1,K1,ISP -ISP_LOW+1)), "bse.F", 5927)
                ENDIF
...
!    CALL M_sum_z(WHF%WDES%COMM, EIGVECT, NBSEEIG*NCV)
    CALL M_sum_z(WHF%WDES%COMM_INTER, EIGVECT, NBSEEIG*NCV)
...
! init contex for gathering eigenvectors
...
! find position corresponding to the grid point
    ...
    IF(WHF%WDES%GRID%RL%NFAST /= 1)  THEN
      CALL vtutor%bug("The order of PW indices is incompatible with this routine", "bse.F", 6082)
    ENDIF
