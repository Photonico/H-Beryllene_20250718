...
!> grid for potentials/charge
      TYPE (grid_3d)     GRIDC
!> temporary grid in us.F
      TYPE (grid_3d)     GRIDUS
!> Broyden grid
      TYPE (grid_3d)     GRIDB
!> index table between GRIDB and GRIDC
      TYPE (transit)     B_TO_C
!> index table between GRIDC and GRIDUS
...
! local
      TYPE (energy) E
      REAL(q) :: TOTENL=0
      REAL(q) :: EDIFF,DESUM1,DESUM(INFO%NELM),ECONV
...
! local l-projected wavefunction characters (not really used here)
      ...
      REAL(q) RMS, RMST, WEIGHT, RMSC, RMSP, RMST_FIRST, EXHF_DUMMY, DE2H
      LOGICAL LDELAY,LFIRST,LLAST

      INTEGER NK,N1,N2,NELM
...
! distribute over bands, no matter what we had originally
      CALL REDIS_PW_OVER_BANDS(W_ORIG%WDES, W_ORIG)

      CALL ZCOPY(SIZE(W%CPTWFP), W_ORIG%CPTWFP(1,1,1,1),1, W%CPTWFP(1,1,1,1),1)
      CALL ZCOPY(SIZE(W%CPROJ), W_ORIG%CPROJ(1,1,1,1),1, W%CPROJ(1,1,1,1),1)

      W%CELTOT   =W_ORIG%CELTOT
      W%FERTOT   =W_ORIG%FERTOT
      W%OVER_BAND=W_ORIG%OVER_BAND

!$ACC UPDATE DEVICE(W%FERWE) 


      IF (LHFCALC_SAVE.AND.W%WDES%COMM_KINTER%NCPU.GT.1) THEN
         CALL KPAR_SYNC_WAVEFUNCTIONS(W%WDES,W)
!!    
         CALL MRG_FERWE(W%WDES,W)
!!    
      END IF
      ...
      CALL SET_KINEDEN(GRID2,GRID_SOFT2,GRIDC,SOFT2_TO_C,LATT_CUR,SYMM, &
           XC%LTAU,T_INFO%NIONS,W,W%WDES,KINEDEN)

      CALL POTLOK(KINEDEN,GRID2,GRIDC,GRID_SOFT2, W%WDES%COMM_INTER, W%WDES, &
           INFO,XC,P,T_INFO,E,LATT_CUR, &
           CHDEN,CHTOT,CSTRF,CVTOT,DENCOR,SV,HAMILTONIAN%MUTOT,HAMILTONIAN%MU,SOFT2_TO_C,XCSIF)

      CALL SETDIJ(W%WDES,GRIDC,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO,INFO%LOVERL, &
           LMDIM,CDIJ,CQIJ,CVTOT,IRDMAA,IRDMAX)

      CALL SET_DD_PAW(W%WDES, P, XC, T_INFO, INFO%LOVERL, &
           W%WDES%NCDIJ, LMDIM, CDIJ(1,1,1,1),  RHOLM, CRHODE(1,1,1,1), &
           E, LASPH=(INFO%LASPH.AND.XC%LDOMETAGGA), LCOREL= .FALSE.  )
...
! return the updated wavefunctions, und eigenvalues
         CALL ZCOPY(SIZE(W_ORIG%CPTWFP), W%CPTWFP(1,1,1,1),1, W_ORIG%CPTWFP(1,1,1,1),1)
         CALL ZCOPY(SIZE(W_ORIG%CPROJ), W%CPROJ(1,1,1,1),1, W_ORIG%CPROJ(1,1,1,1),1)
      ...
      IF (OVER_BAND) THEN
         CALL REDIS_PW_ALL(W_ORIG%WDES, W_ORIG)
      ENDIF
...
!     IF (LINIT_SUBROT_SCF_DONE.OR.(.NOT.INFO%LDIAG).OR.(.NOT.INFO%LONESW)) RETURN
      ...
      ENCUT_SUBROT_SCF=-1
      CALL PROCESS_INCAR(LOPEN, IU0, IU5, 'ENCUTSUBROTSCF', ENCUT_SUBROT_SCF, IERR, WRITEXMLINCAR)
