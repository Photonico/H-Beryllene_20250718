...
!  B. Ramberger, T. Schäfer, G. Kresse, G. (2017).
...
!
  COMPLEX(q), ALLOCATABLE :: CRHODE_RPA(:,:,:,:)! first order change of PAW occupancies
...
!
  ...
  REAL(q), ALLOCATABLE :: DE_RPA_ONEC(:,:)       ! energy change from (1._q,0._q)-center terms upon displacement RPA_DISPLACEMENT
...
! arrays to store first order change of self-consistent Kohn-Sham potential
  ...
  COMPLEX(q),ALLOCATABLE :: CDIJ_RPA(:,:,:,:) ! derivative of non local strength
  REAL(q)             :: FORCE_ADD_RPA     ! additional component to force from (1._q,0._q)-centre terms
...
!***********************************************************************
  ...
  SUBROUTINE STORE_RPA_DENSITY_MATRIX(W, WDES, CHAM_MAT, COMM_INTAU, DESC, SIGMA_MAT )
    ...
    TYPE (wavespin)    W
    TYPE (wavedes)     WDES
    COMPLEX(q), POINTER :: CHAM_MAT(:,:,:,:)     ! density matrix rho^(1) in KS orbital basis
    COMPLEX(q), POINTER, OPTIONAL :: SIGMA_MAT(:,:,:,:) ! overlap density matrix gamma^(1) in KS orbital basis
...
! local
    ...
    IF (SIZE(W_RPA%CPTWFP,1) /= SIZE(W%CPTWFP,1)) THEN
       CALL vtutor%bug("internal error in STORE_RPA_DENSITY_MATRIX: number of coefficients not " // &
          "correct " // str(SIZE(W_RPA%CPTWFP,1)) // " " // str(SIZE(W%CPTWFP,1)), "rpa_force.F", 273)
    ENDIF

    IF (SIZE(W_RPA%CPTWFP,2) /= WDES%NBANDS) THEN
       CALL vtutor%bug("internal error in STORE_RPA_DENSITY_MATRIX: number of bands not correct " &
          // str(SIZE(W_RPA%CPTWFP,2)) // " " // str(WDES%NBANDS), "rpa_force.F", 278)
    ENDIF
...
!  back-redistribution over plane wave coefficients
          CALL REDISTRIBUTE_PROJ( WA)
          CALL REDISTRIBUTE_PW( WA)
          W_RPA%CPTWFP   (:, 1:WDES%NBANDS, NK, ISP)=WA%CPTWFP   (:, 1:WDES%NBANDS)
...
!  back-redistribution over plane wave coefficients
          CALL REDISTRIBUTE_PROJ( WA)
          CALL REDISTRIBUTE_PW( WA)
          W_S%CPTWFP   (:, 1:WDES%NBANDS, NK, ISP)=WA%CPTWFP   (:, 1:WDES%NBANDS)
...
! here, however, we need to symmetrize the densities and (1._q,0._q)-centre occupancies
...
! symmetrize CRHODE_RPA (currently only 1._q by DEPLE_ADD, alas...)
...
! but this will be 1._q later, and will change only little in terms of
! performance
...
! symmetrise total pseudo charge density CHTOT as well
    ...
    IF (NONLR_S%LREAL) THEN
       IF (ASSOCIATED(W_S)) THEN
          CALL vtutor%bug("internal error in VASP: RPA_DENSITY does not yet support W_S for real space", "rpa_force.F", 506)
       ENDIF
...
!***********************************************************************
  ...
  SUBROUTINE RPA_ZERO_POT( CVTOT1, CDIJ1)
    ...
    COMPLEX(q)    :: CDIJ1(:,:,:,:)! derivative of non local strength
  ...
  SUBROUTINE RPA_ADD_TO_POT( CVTOT1, CDIJ1, FORCE, WEIGHT)
    ...
    IF (.NOT. ALLOCATED(CVTOT_RPA)) THEN
       CALL vtutor%bug("internal error in VASP: RPA_ADD_TO_POT, CVTOT_RPA is not allocated", "rpa_force.F", 570)
    ENDIF
    IF (.NOT. ALLOCATED(CDIJ_RPA)) THEN
       CALL vtutor%bug("internal error in VASP: RPA_ADD_TO_POT, CDIJ_RPA is not allocated", "rpa_force.F", 573)
    ENDIF
...
! loop over reciprocal lattice vectors
         DO NG=1,GRIDC%RC%NP
           ...
           FOR=FOR+CVTOT_RPA(NG,ISP)* CONJG(CHTOT_RPA(NG,ISP))
        ENDDO
     ENDDO
! sum all cores
     CALL M_sum_z(GRIDC%COMM, FOR, 1)

! now contract RPA (1._q,0._q)-center density matrix against change of (1._q,0._q)-centre strenght
     FOR_NL=0
     IF (WDES%LOVERL) THEN
...
!OCL NOVREC
            DO LP=1,LMMAXC
               FOR_NL=FOR_NL+ CRHODE_RPA(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)*CONJG(CDIJ_RPA(LP,L,NI,ISP+ISPINOR_+2*ISPINOR))
            ENDDO
         ENDDO
...
! sum all cores
     CALL M_sum_z(WDES%COMM_INB, FOR_NL, 1)

     RPA_NDISPLACEMENTS(ION)=RPA_NDISPLACEMENTS(ION)+1
     IF (RPA_NDISPLACEMENTS(ION)>3) THEN
        CALL vtutor%bug("internal error in VASP: RPA_FORCE_CHARGE displacements larger 3 " // str(ION), "rpa_force.F", 677)
     ENDIF
...
!=======================================================================
    ...
    IF (IFAIL/=0) THEN
       CALL vtutor%bug("internal ERROR in MUL_INV_SECOND_DERIV Call to routine DSYEV failed! Error " &
          // "code was " // str(IFAIL), "rpa_force.F", 895)
    ENDIF
!=======================================================================
...
! allocate required work space
    COMPLEX(q),ALLOCATABLE,TARGET :: CPROW1(:,:,:,:), CPROW0(:,:,:,:)
...
510                      NIS = NIS+WDES%NITYP(NT)
    ...
    CALL M_sum_d(WDES%COMM, FRNL(1,1),NONLR_S%NIONS*3)
...
! work arrays
    ...
    COMPLEX(q) :: CM
    COMPLEX(q),ALLOCATABLE :: CX1(:) ,CY1(:) ,CZ1(:), C1(:)
    COMPLEX(q),ALLOCATABLE :: CXL1(:),CYL1(:),CZL1(:),CL1(:)
    COMPLEX(q),ALLOCATABLE :: CX2(:) ,CY2(:) ,CZ2(:), C2(:)
    COMPLEX(q),ALLOCATABLE :: CXL2(:),CYL2(:),CZL2(:),CL2(:)
...
!=======================================================================
    ...
    CALL M_sum_d(WDES%COMM, EINLGAMMA(1,1),NONL_S%NIONS*3)
...
!OCL NOVREC
        DO LP=1,LMMAXC
           CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
                WEIGHT*W1%CPROJ(L+LMBASE,N,NK,ISP)*CONJG(W0%CPROJ(LP+LMBASE_,N,NK,ISP))+ &
                WEIGHT*W0%CPROJ(L+LMBASE,N,NK,ISP)*CONJG(W1%CPROJ(LP+LMBASE_,N,NK,ISP))
        ENDDO
        ENDDO
