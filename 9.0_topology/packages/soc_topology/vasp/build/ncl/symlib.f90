...
!
! if you need some other precision specify it in the INCAR file
!
MODULE sym_prec
  USE prec
  REAL(q) :: TINY=1.E-5_q
!$ACC DECLARE CREATE(TINY)
END MODULE
!********************* Symmetry package ********************************
!                                                                      *
...
!>   package.
!>
!>   The function of SETGRP is to set up all possible
...
!>   package.
!>
!>   The function of GETGRP is to return all possible space
...
!>   some nontrivial translation) or not.
!>
!>   The procedure is in principle
...
!>   symmetry operation.
!>
!>   The test is 1._q by symmetrizing the vectors
...
!>   symmetry operation.
!>
!>   The test is 1._q by checking atomic magnetic
...
!>   located at different atomic positions.
!>
...
! these coordinates are transformed to -0.5!
            IF ((ABS(TAU(IA,1,ISP)+0.5_q))<0.5_q*TINY) TAU(IA,1,ISP)=-0.5_q
            IF ((ABS(TAU(IA,2,ISP)+0.5_q))<0.5_q*TINY) TAU(IA,2,ISP)=-0.5_q
            IF ((ABS(TAU(IA,3,ISP)+0.5_q))<0.5_q*TINY) TAU(IA,3,ISP)=-0.5_q
            IF ((ABS(TAU(IA,1,ISP)-0.5_q))<0.5_q*TINY) TAU(IA,1,ISP)=-0.5_q
            IF ((ABS(TAU(IA,2,ISP)-0.5_q))<0.5_q*TINY) TAU(IA,2,ISP)=-0.5_q
            IF ((ABS(TAU(IA,3,ISP)-0.5_q))<0.5_q*TINY) TAU(IA,3,ISP)=-0.5_q
...
! Impose periodic boundary conditions on the rotated coordinates:
                  ...
                  IF ((ABS(TR(IA,1)+0.5_q))<0.5_q*TINY) TR(IA,1)=-0.5_q
                  IF ((ABS(TR(IA,2)+0.5_q))<0.5_q*TINY) TR(IA,2)=-0.5_q
                  IF ((ABS(TR(IA,3)+0.5_q))<0.5_q*TINY) TR(IA,3)=-0.5_q
                  IF ((ABS(TR(IA,1)-0.5_q))<0.5_q*TINY) TR(IA,1)=-0.5_q
                  IF ((ABS(TR(IA,2)-0.5_q))<0.5_q*TINY) TR(IA,2)=-0.5_q
                  IF ((ABS(TR(IA,3)-0.5_q))<0.5_q*TINY) TR(IA,3)=-0.5_q
...
! if already mapped, cycle
                    IF (MAP(JA,ISP,IROT,ITRANS)/=-1) CYCLE
! take the difference between TR and TAU
                    TAUDIF(1)=TR(IA,1)-TAU(JA,1,ISP)
                    TAUDIF(2)=TR(IA,2)-TAU(JA,2,ISP)
                    TAUDIF(3)=TR(IA,3)-TAU(JA,3,ISP)
! Bring the difference vector into the interval [-0.5, 0.5)
                    TAUDIF(1)=MOD((TAUDIF(1)+100.5_q),1._q)-0.5_q
                    TAUDIF(2)=MOD((TAUDIF(2)+100.5_q),1._q)-0.5_q
                    TAUDIF(3)=MOD((TAUDIF(3)+100.5_q),1._q)-0.5_q
! If all components of the difference vector are smaller than TINY in an
! absolute sense, then spacegoup operation (IROT, ITRANS) brings the atom
! at position JA to position IA: store in MAP and exit loop JA
                    IF (ABS(TAUDIF(1))<TINY.AND.ABS(TAUDIF(2))<TINY.AND.ABS(TAUDIF(3))<TINY) THEN
                       MAP(JA,ISP,IROT,ITRANS)=IA ; EXIT
                    ENDIF
                  ENDDO
               ENDDO
    5       CONTINUE
            ...
            DO IROT=1,NROT
               DO ITRANS=1,NP
                  IF (MAP(IA,ISP,IROT,ITRANS)==-1) THEN
                      CALL ERROR(' POSMAP', ' symmetry equivalent atom not found, you might try &
                         &decreasing or increasing SYMPREC by an order of magnitude.', IA)
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
...
! Bring the difference vector into the interval [-0.5, 0.5)
                 ...
                 TAUDIF(3)=MOD((TAUDIF(3)+100.5_q),1._q)-0.5_q
! If all components of the difference vector are smaller than TINY in an
! absolute sense, then symmetry operation IROT brings the atom
! at position JA to position IA: store in MAP
                 IF (ALL(ABS(TAUDIF)<TINY)) MAP(JA,ISP,IROT)=IA
               ENDDO
            ENDDO
    5    CONTINUE
! Search the classes ... :
...
! Calculate cell volumes of the cells defined by both sets of vectors
      CALL CELVOL(A1,A2,A3,OMEGAA)
      CALL CELVOL(B1,B2,B3,OMEGAB)
! We will scale the symmetry tolerance by the following factor
      SCALE=ABS((OMEGAB/OMEGAA)**(1._q/3._q))
...
! Error check: We must end up with INTEGER matrices ... !
         IF (ABS(R(1,1)-SB(1,1,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(2,1)-SB(2,1,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(3,1)-SB(3,1,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(1,2)-SB(1,2,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(2,2)-SB(2,2,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(3,2)-SB(3,2,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(1,3)-SB(1,3,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(2,3)-SB(2,3,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
         IF (ABS(R(3,3)-SB(3,3,I))*SCALE>TINY) CALL ERROR('SGRCON', &
     &           'Found some non-integer element in rotation matrix',I)
  100 CONTINUE
! Good bye ... :
      RETURN
...
!>   -----------------
!>
!>      V(3,NVEC) contains the input vectors (basis A1,A2,A3).
!>      NVEC gives the number of vectors in V.
!>
!>      A1(3),A2(3),A3(3) contain the first set of basis vectors.
!>      B1(3),B2(3),B3(3) contain the second set of basis vectors.
!>
...
!>   ------------------
!>
!>      V(3,NVEC) contains the converted vectors (basis B1,B2,B3).
!
!***********************************************************************

      SUBROUTINE VECCON(V,NVEC,A1,A2,A3,B1,B2,B3)
      USE prec
      ...
      DIMENSION BI1(3),BI2(3),BI3(3),VTMP(3),R(3)
...
! Perform the transformation:
         R(1)=A1(1)*VTMP(1)+A2(1)*VTMP(2)+A3(1)*VTMP(3)
         R(2)=A1(2)*VTMP(1)+A2(2)*VTMP(2)+A3(2)*VTMP(3)
         R(3)=A1(3)*VTMP(1)+A2(3)*VTMP(2)+A3(3)*VTMP(3)
         VTMP(1)=BI1(1)*R(1)+BI1(2)*R(2)+BI1(3)*R(3)
         VTMP(2)=BI2(1)*R(1)+BI2(2)*R(2)+BI2(3)*R(3)
         VTMP(3)=BI3(1)*R(1)+BI3(2)*R(2)+BI3(3)*R(3)
! Store the result:
         V(1,I)=VTMP(1)
         V(2,I)=VTMP(2)
         V(3,I)=VTMP(3)
...
!>   space group operators (INTEGER rotation matrices and nontrivial
!>   translations given in crystal coordinates) of a lattice with
!>   some arbitrary basis (atomic arrangement).
!>
!>   This will be 1._q by
!>   first setting up the point group operators of the pure bravais
!>   lattice without basis (empty supercell) and checking which of the
!>   operations can reproduce the lattice with basis (to be 1._q in
!>   routine GETGRP - for further details see comments on GETGRP).
...
!***********************************************************************
      ...
      DIMENSION S(3,3),GTRANS(3),NA(NSP),TAU(NAX,3,NSX)
      DIMENSION TAUROT(NAX,3,NSX),WRK(NAX,3),INDEX(NAX),TAUSAV(3),TRA(3)
...
! and translated coordinates, the result should be (0.,0.,0.) as well.
               DO IC=1,NC
                  VECDIF(1,IC)=VEC(IA,1,ISP,IC)-VECROT(IAP,1,ISP,IC)
                  VECDIF(2,IC)=VEC(IA,2,ISP,IC)-VECROT(IAP,2,ISP,IC)
                  VECDIF(3,IC)=VEC(IA,3,ISP,IC)-VECROT(IAP,3,ISP,IC)
               ENDDO
! SPINDIFM is the difference between the spins at the original and the
! translated coordinates.
               SPINDIFM=SPIN(IA,ISP)-SPINROT(IAP,ISP)
...
! spinflips.
               SPINDIFP=SPIN(IA,ISP)+SPINROT(IAP,ISP)
! If VECDIF is not equal (0.,0.,0.) TNTRAN=TNTRAN.AND.FALSE
! and so it remains FALSE forever ... (if only (1._q,0._q) instance is not reproduced!).
! Only if all VECDIFs are (0._q,0._q) TNTRAN (starting with the value TRUE) will
! remain TRUE (that means we found an allowed symmetry operation).
               DO IC=1,NC
