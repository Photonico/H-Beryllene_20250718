...
! Early exit if possible
      IF (((.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_PBE))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_RPBE))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_PBESOL))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_REVPBE))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_OPTPBE))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_OPTB88PBE))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_OPTB86BPBE))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_PW86RPBE))) .AND. &
        &  (.NOT.((XC%NXC==1).AND.(XC%ID(1)==ID_XC_CX)))) .OR. &
...
!LS chose between PBE (with EP exchange hole) and PBEsol (HJS exchange hole)
               IF ((XC%NXC==1).AND.(XC%ID(1)==ID_XC_PBESOL)) THEN
...
!$ACC ROUTINE SEQ
      ...
      REAL(q) YX2,FX
!$ACC ROUTINE (WPBE_SPLIN2) SEQ
!$ACC ROUTINE (LOGRHO) SEQ
...
! Find bracketing indices at the rho axis
      ...
      IF ((LOGRHO(JLO).GT.X1).OR.(LOGRHO(JHI).LT.X1)) THEN
    CALL vtutor%error('WPBE_SPLINE: ERROR(2), LOG(RHO) out of bounds: ' // &
       str(X1) // ' ' // str(LOGRHO(JLO)) // ' ' // str(LOGRHO(JHI)) // &
       ' ' // str(JLO) // ' ' // str(JHI) // ' ' // str(LOGRHO0))
      ENDIF
...
! Find bracketing indices at the S axis
    ...
    CALL vtutor%error('WPBE_SPLINE: ERROR(2), S out of bounds: ' // str(X2) // ' ' // &
       str(S(KLO)) // ' ' // str(S(KHI)) // ' ' // str(KLO) // ' ' // str(KHI))
         IF (S(KLO).GT.X2) THEN
            KLO=NS-4
            KHI=NS-3
         ENDIF
