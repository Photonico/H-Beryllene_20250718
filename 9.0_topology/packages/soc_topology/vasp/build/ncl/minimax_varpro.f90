...
# 2 "minimax_varpro.F" 2 

MODULE minimax_varpro
...
!   REAL(qd), PARAMETER :: 1.E-28_qd = REAL(100.,KIND=qd) * EPSILON(1.0_qd)



CONTAINS
SUBROUTINE VARPRO (L, NL, N, NMAX, LPP2, IV, T, Y, W,  A,  &
                   IPRINT, ALF, BETA, ITMAX, QUADRATURE, IERR,  &
                   INU, NWRITE)
...
!     ..................................................................
   ...
   REAL(qd), INTENT(IN)        :: t(:,:)       ! t(nmax,iv)
   REAL(qd), INTENT(IN)        :: y(:)
   REAL(qd), INTENT(IN OUT)    :: w(:)
   REAL(qd), INTENT(OUT)       :: a(:,:)       ! a(nmax,lpp2)
   INTEGER, INTENT(IN)        :: iprint
   REAL(qd), INTENT(IN OUT)    :: alf(amax)       ! alf(nl)
   REAL(qd), INTENT(IN OUT)    :: beta(amax)
...
! Local variables
   ...
   REAL(qd) :: acum, gnstep, nu, prjres, r, rnew
   INTEGER   :: b1, isel, isub, iter, iterin, j, jsub, k, ksub, lnl2, lp1,  &
                modit, nlp1, i 
   LOGICAL   :: skip
# 363

   REAL(qd), PARAMETER  :: nu_max = 100._qd
...
10 CONTINUE
   CALL vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, alf, ierr,  &
           iprint, a, beta, a(:,lp1), r, quadrature, inu, nwrite, .false.)
   gnstep = 1
   iterin = 0
...
20 CONTINUE
   CALL vpfac1 (nlp1, n, l, iprint, a(:,b1:), prjres, ierr, inu,nwrite)
   IF (ierr < 0) GO TO 180
   ierr = 2
   IF (nu == 0) GO TO 40
...
40 CONTINUE
   CALL vpbsol (nl, a(:,b1:), a(:,lnl2))
   do i = 1, nl
      a(i,b1) = alf(i) + a(i,lnl2)
   enddo
...
60 CONTINUE
   CALL vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, a(:,b1),  ierr,  &
            iprint, a, beta, a(:,lp1), rnew, quadrature, inu,nwrite, .false.)
   IF (ierr /= 2) GO TO 180
   iter = iter + 1
   iterin = iterin + 1
   ...
   if (inu>=0 .and. nwrite >2) WRITE (inu,240) (a(i,b1), i=1,nl)
...
! 1000       FORMAT(15X,'IERR AT 45 =',I7)
   GO TO 170
80 CONTINUE 
   IF (rnew-r < 1.E-28_qd*(r+1)) GO TO 130

!              RETRACT THE STEP JUST TAKEN

   IF (nu /= 0) GO TO 100
!                                             GAUSS-NEWTON OPTION ONLY
   gnstep = REAL(0.5,KIND=qd)*gnstep
   IF (gnstep < 1.E-28_qd) GO TO 170
   DO  k=1,nl
...
100 CONTINUE 
   nu = REAL(1.5,KIND=qd)*nu
   IF (.NOT.skip) then
     if (inu>=0 .and. nwrite >2) WRITE (inu,210) nu
   endif
   IF (nu <= nu_max) GO TO 110
   ierr = -2
   CALL vperr (iprint, ierr, 1, inu,nwrite)
!       WRITE(6,1000)IERR
...
!           IF ITERIN IS > 1, A STEP WAS RETRACTED DURING THIS OUTER ITERATION.
   ...
   IF (iterin == 1) nu = REAL(0.5,KIND=qd)*nu
   IF (skip) GO TO 150
   if(inu>=0 .and. nwrite >2) WRITE (inu,190) nu
...
150 CONTINUE
   ierr = 3
   IF (prjres > 1.E-28_qd*(r+1)) GO TO 10
...
170 CONTINUE
   IF (nl > 0) THEN
      isel = 4
      CALL vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, alf, isel, iprint, a,  &
                 beta, a(:,lp1), r, quadrature, inu, nwrite, .false. )
   END IF
   CALL vppost (l, nl, n, lnl2, 1.E-28_qd, r, iprint, alf, w, a, a(:,lp1), beta, inu, nwrite)
180 CONTINUE

! clean SAVE variables for a clean exit
   CALL vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, alf, isel, iprint, a,  &
              beta, a(:,lp1), r, quadrature, inu, nwrite, .true. )
...
INTEGER, INTENT(IN)        :: l
INTEGER, INTENT(IN)        :: iprint
REAL(qd), INTENT(IN OUT)    :: b(:,:)     ! b(nmax,nlp1)
REAL(qd), INTENT(OUT)       :: prjres
...
!                                OF B AND TO RESIDUAL VECTOR.
  10 kp1 = k+1
  DO  j = kp1,nlp1
    acum = DOT_PRODUCT_MPR( b(lpk:n,k), b(lpk:n,j) )
    acum = acum/beta
    DO  i=lpk,n
...
INTEGER, INTENT(IN)     :: nlp1
REAL(qd), INTENT(IN)     :: nu
REAL(qd), INTENT(OUT)    :: b(:,:)     ! b(nmax,nlp1)
...
SUBROUTINE vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, alf,  &
          isel, iprint, a, u, r, rnorm, quadrature, inu,nwrite, lclean)
...
REAL(qd), INTENT(IN)         :: y(:)
REAL(qd), INTENT(IN OUT)     :: w(:)
REAL(qd), INTENT(IN OUT)     :: alf(:)
INTEGER, INTENT(IN OUT)     :: isel
INTEGER, INTENT(IN)         :: iprint
REAL(qd), INTENT(IN OUT)     :: a(:,:)    ! a(nmax,lpp2)
REAL(qd), INTENT(IN OUT)     :: u(:)
REAL(qd), INTENT(OUT)        :: r(:)
REAL(qd), INTENT(OUT)        :: rnorm
INTEGER, INTENT(IN)         :: inu
INTEGER, INTENT(IN)         :: nwrite
LOGICAL, INTENT(IN)         :: lclean
TYPE(quadrature_handle)    :: quadrature

! Local variables

REAL(qd) :: acum, alpha, beta
INTEGER   :: i, j, k
INTEGER   :: ksub=0
INTEGER   :: m=0
INTEGER   :: kp1=0
LOGICAL         :: ljumped=.false.
INTEGER         :: inc(amax,bmax)=0
LOGICAL, SAVE   :: nowate, philp1
REAL(qd), SAVE   :: saved
LOGICAL, SAVE   :: lfirst=.true.
INTEGER, SAVE   :: firstc=0
INTEGER, SAVE   :: firstr=0
INTEGER, SAVE   :: lastc=0
INTEGER, SAVE   :: lnl2=0
INTEGER, SAVE   :: lp1=0
INTEGER, SAVE   :: lp2=0
INTEGER, SAVE   :: lpp1=0
INTEGER, SAVE   :: ncon=0
INTEGER, SAVE   :: nconp1=0
...
CALL vpinit (l, nl, n, nmax, lpp2, iv, t, w, alf, isel,  &
             iprint, a, inc, ncon, nconp1, philp1, nowate,quadrature, inu,nwrite)
IF (isel /= 1) GO TO 180
GO TO 30

10 CONTINUE
CALL ada (lp1, nl, n, nmax, lpp2, iv, a, inc, t, alf, MIN(isel,3), quadrature)
IF (isel == 2) GO TO 20
!                                                 ISEL = 3 OR 4
firstc = lp2
lastc = lpp1
...
100 beta = -a(k,k)*u(k)
  DO  j=firstc,lastc
    acum = u(k)*a(k,j) + DOT_PRODUCT_MPR( a(kp1:n,k), a(kp1:n,j) )
    acum = acum/beta
...
SUBROUTINE vpinit (l, nl, n, nmax, lpp2, iv, t, w, alf,  isel,  &
                   iprint, a, inc, ncon, nconp1, philp1, nowate, quadrature, inu,nwrite)
...
REAL(qd), INTENT(IN OUT)    :: a(:,:)   ! a(nmax,lpp2)
TYPE(quadrature_handle)   :: quadrature
INTEGER, INTENT(IN OUT)    :: inc(amax,bmax)
...
30 CONTINUE
CALL ada(lp1, nl, n, nmax, lpp2, iv, a, inc, t, alf, isel, quadrature)
nowate = .true.
DO  i=1,n
!write(*,*)'loop wi',i,n, size(w)
  nowate = nowate .AND. (w(i) == 1)
  IF (w(i) >= 0) GO TO 40
!                                                ERROR IN WEIGHTS
  isel = -6
...
REAL(qd), INTENT(IN)        :: eps
REAL(qd), INTENT(IN)        :: rnorm
INTEGER, INTENT(IN)        :: iprint
INTEGER, INTENT(IN)        :: inu
INTEGER, INTENT(IN)        :: nwrite
REAL(qd), INTENT(IN OUT)    :: alf(:)
REAL(qd), INTENT(IN OUT)    :: w(:)
REAL(qd), INTENT(IN OUT)    :: a(:,:)
REAL(qd), INTENT(IN OUT)    :: r(:)
REAL(qd), INTENT(IN OUT)    :: u(:)

! Local variables

REAL(qd)   :: acum, prjres, SAVE, tmp
...
DO  kback=1,l
  k = lp1 - kback
  kp1 = k + 1
  acum = DOT_PRODUCT_MPR( a(kp1:n,k), r(kp1:n) )
  SAVE = r(k)
  r(k) = acum/a(k,k)
  acum = -acum/(u(k)*a(k,k))
  u(k) = SAVE
...
! last check: positive weights are demanded for finite temperature grids
do k = 1, l 
  if ( u(k) < 0 ) then
     isel = -6
     CALL vperr (iprint, isel, k, inu,nwrite)
...
IF (l > 0 .and. nl > 0 .and. nl==l ) then
   ...
   DO i=1, nl
      if ( inu>=0 .and. nwrite >2) WRITE(inu,'(2F40.28)')u(i),alf(i)
   ENDDO 
else
   if ( inu>=0 .and. nwrite >2) write(*,*)'error in vppost nl/=l',nl,l
endif

IF ( INU>=0 .AND. NWRITE >2) THEN
    WRITE (inu,120) rnorm, SAVE, acum
ENDIF
...
INTEGER, INTENT(IN)   :: n
REAL(qd), INTENT(IN)   :: sigma2
REAL(qd), INTENT(INOUT):: a(:,:)
...
DO  i=1,nm1
  ...
  DO  j=ip1,n
    jm1 = j - 1
    sum = DOT_PRODUCT_MPR( a(i,i:jm1), a(i:jm1,j) )
    a(i,j) = -sum*a(j,j)
  END DO
END DO
...
40 DO  i=1,n
  DO  j=i,n
    sum = DOT_PRODUCT_MPR( a(i,j:n), a(j,j:n) )
    sum = sum*sigma2
    a(i,j) = sum
    a(j,i) = sum
  END DO
...
!****************************************************************************
! sorts coefficients C small size N (<10000) in ascending order (pick sort)
!****************************************************************************
...
!****************************************************************************
   ...
   SUBROUTINE ada(LP1, NL, N, NMAX, LPP2, IV, A, INC, T, ALF, ISEL, QUADRATURE)
      IMPLICIT NONE
      ...
      INTEGER, INTENT(IN)     :: IV
      REAL(qd), INTENT(IN OUT) :: A(:,:)     ! A(NMAX,LPP2)
      INTEGER, INTENT(IN OUT) :: INC(:,:)    ! INC(12,LP1)
      REAL(qd), INTENT(IN)     :: T(:,:)        ! T(NMAX)
      REAL(qd), INTENT(IN)     :: ALF(:)        ! ALF(NL)
...
!exp basis functin is default
      DO L = 1, IV
         DO J=1,NL
            DO  I=1,N
               A(I,J) = QUADRATURE%PHI2( T(I,L), ALF(J) )
            ENDDO
         END DO
      END DO
...
50    CONTINUE
      DO L = 1, IV
         DO  I=1,N
            DO J=NL+2,2*NL+1
               K=J-NL-1
               A(I,J) = QUADRATURE%D_PHI2_DL( T(I,L), ALF(K) )
            ENDDO
         END DO
      END DO
