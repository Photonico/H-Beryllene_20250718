...
! local
      ...
      ELSE
         IF (SIZE(FA,1)/=SIZE(X) .OR. SIZE(FA,2)/=R%N) THEN
            CALL vtutor%bug("internal error in CALC_RATPOL: FA is not properly dimensioned " // &
               str(SIZE(FA,1)) // " " // str(SIZE(X)) // " " // str(SIZE(FA,2)) // " " // str(R%N), "ratpol.F", 195)
         ENDIF
         IF (SIZE(FB,1)/=SIZE(X) .OR. SIZE(FB,2)/=R%N) THEN
            CALL vtutor%bug("internal error in CALC_RATPOL: FB is not properly dimensioned " // &
               str(SIZE(FB,1)) // " " // str(SIZE(X)) // " " // str(SIZE(FB,2)) // " " // str(R%N), "ratpol.F", 199)
         ENDIF
