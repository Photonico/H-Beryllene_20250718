...
!***********************************************************************************
  SUBROUTINE OVERLAP_DISTRIBUTE_G(WGWQ, IU0, IU6)
    ...
    CALL M_sum_i(COMM_OVERLAP, G_OVERRUN, 1)
...
! local
    ...
    IF ( NI > SIZE(OVERLAP_DENS,2) .OR. NA > SIZE(OVERLAP_DENS,3) .OR. K2 > SIZE(OVERLAP_DENS,4)) THEN
       CALL vtutor%bug("internal error in OVERLAP_STORE: bounds exceeded " // str(NI) // " " // &
          ...
          str(K2) // " " // str(SIZE(OVERLAP_DENS,4)), "lt_mp2.F", 135)
    ENDIF
...
! note that we DON NOT redistribute the occupied orbitals WOCC%CPTWFP and WOCC%CPROJ
      DO I = 1, W%WDES%ISPIN
        DO NK = 1, W%WDES%NKPTS
...
! collect results from each core
    CALL M_sum_d(W%WDES%COMM_KINTER, EXCHANGE_TOTAL, N_MP2_ENCUTS)
    CALL M_sum_d(W%WDES%COMM_KINTER, DIRECT_TOTAL, N_MP2_ENCUTS)
...
!CALL M_sum_d(W%WDES%COMM,NFLOAT,1)
!CALL M_sum_d(W%WDES%COMM,NFFT,1)
...
!> 4 pi e^2 / G^2
!> ~~~
!> for (1._q,0._q) given G multiplied by a smooth cosinus cutoff between ENCUTSOFT and ENCUT
!
!********************************************************************************
...
! note: each core in WHF%WDES%COMM_KINTER needs ALL unoccupied bands for the decomposition later
              CALL M_sum_z(WHF%WDES%COMM_KINTER, OVERLAP_I(1,1), SIZE(OVERLAP_I))
...
! local
    ...
    COMPLEX(q), ALLOCATABLE :: CL(:,:)       ! matrix with coefficients (overlaps) for Schmidt decomposition
...
! note that CL(a,i) for i > LAST_FILLED will be set to (0._q,0._q), hence WT for i > LAST_FILLED
! will be (0._q,0._q)
...
! gather results from all cores
        CALL M_sum_z(W%WDES%COMM_KIN, CL(1,1), SIZE(CL))
...
! matrix mulitplication: WTA%CW_RED = WA%CW_RED * CL
        CALL ZGEMM('N', 'N',  WDESK2%NPL_RED, WDESOCC%NB_TOT, WDESK2%NB_TOTK(ISP), (1._q,0._q), WA%CW_RED(1,1), &
                    WDESK2%NRPLWV_RED, CL, SIZE(CL,1), (0._q,0._q), WTA%CW_RED(1,1),  WDESK2%NRPLWV_RED)
        NFLOAT = NFLOAT +  WDESK2%NPL_RED * WDESOCC%NB_TOT * WDESK2%NB_TOTK(ISP) * 2 * 2

        CALL ZGEMM('N', 'N', WDESK2%NPRO_RED, WDESOCC%NB_TOT, WDESK2%NB_TOTK(ISP), (1._q,0._q), WA%CPROJ_RED(1,1), &
                   WDESK2%NPROD_RED, CL, SIZE(CL,1), (0._q,0._q), WTA%CPROJ_RED(1,1), WDESK2%NPROD_RED)
        NFLOAT = NFLOAT + WDESK2%NPRO_RED * WDESOCC%NB_TOT * WDESK2%NB_TOTK(ISP) * 2 * 2
...
! local variables
    ...
    COMPLEX(q),      ALLOCATABLE :: GWORK(:)         ! work array for overlap densities
    COMPLEX(q),      ALLOCATABLE :: GWORKSUM(:)      ! work array for overlap densities
    COMPLEX(q),      ALLOCATABLE :: CRHOLM(:)        ! augmentation occupancy matrix
...
! go back to reciprocal space
    ...
    CALL M_sum_z(WOCCHF%WDES%COMM_KIN, GWORKSUM , SIZE(GWORKSUM))

    IF (MCALPHA/=0) THEN
! with finite size corrections:
      CALL vtutor%bug("internal error in VASP: LAPLACE_TRANSFORMED_MP2 does not support MCALPHA/=0", "lt_mp2.F", 1125)
    ELSE
...
! local
    ...
    REAL(q) :: FSG                                    ! singularity correction setting V(G=0) to a finite value
    TYPE(rotation_handle), POINTER :: ROT_HANDLE
    COMPLEX(q), ALLOCATABLE :: GWORK1(:,:)                  ! buffer for < phi*_i,k1 | exp(-iG'r | w_j,T(k2-q) >*
    COMPLEX(q), ALLOCATABLE :: GWORK2(:,:)                  ! buffer for < phi*_j,k2 | exp(-iG'r | w_i,T(k1-q) >
    COMPLEX(q), ALLOCATABLE :: GWORK_SUM(:)  
    COMPLEX(q), ALLOCATABLE :: CRHOLM(:,:)                  ! augmentation occupancy matrix
...
! OMP parallelization is activated
    ...
    ALLOCATE( GWORK1( GRIDHF%MPLWV, NBLK),                       &
              GWORK2( GRIDHF%MPLWV, NBLK),                       &
              GWORK_SUM( GRIDHF%MPLWV),                       &
...
!FSG = FSG_STORE(1)
    FSG = 0 ! set to (0._q,0._q) for the time beeing
...
! apply coulomb potential for each cutoff
    ...
    CALL M_sum_d(WOCCHF%WDES%COMM_KIN, E_EXCH, N_MP2_ENCUTS)
...
!>  potential kernel POTFAKS. This is 1._q for different cutoffs stored in ENCUTS.
!
!*********************************************************************************
  SUBROUTINE APPLY_GFAC_2_ENCUTS(GRID, CWORK, CWORKG, POTFAKS, ENERGIES, ENCUTS, N_MP2_ENCUTS)
    ...
    REAL(q) :: ENCUTS(N_MP2_ENCUTS)
    REAL(q), PARAMETER :: THRESHOLD = 1e-12_q
...
! LOCAL
    ...
    IF (GRID%REAL2CPLX) THEN
      IF (.NOT. ASSOCIATED(GRID%FFTWEIGHT)) THEN
        CALL vtutor%bug("internal error in APPLY_GFAC_2_ENCUTS: FFTWEIGHT is not associated", "lt_mp2.F", 1612)
      ENDIF
...
! check if POTFAKSS are equal for different cutoffs (if so treat all POTFAKSS equally)
          IF ( ABS(POTFAKS(NP,I) - POTFAKS(NP,1)) < THRESHOLD ) THEN
...
!>  kernel POTFAKS. This is 1._q for different cutoffs stored in ENCUTS.
!
!*********************************************************************************
...
! LOCAL
    ...
    IF (GRID%REAL2CPLX) THEN
      IF (.NOT. ASSOCIATED(GRID%FFTWEIGHT)) THEN
        CALL vtutor%bug("internal error in APPLY_GFAC_ENCUTS: FFTWEIGHT is not associated", "lt_mp2.F", 1679)
      ENDIF
!$OMP PARALLEL DO DEFAULT(SHARED) &
!$OMP PRIVATE(NP) &
!$OMP REDUCTION(+:ENERGIES)
...
!>
!> This function is used to overcome a COMPLEX(q)/COMPLEX problem
!
!*********************************************************************************
