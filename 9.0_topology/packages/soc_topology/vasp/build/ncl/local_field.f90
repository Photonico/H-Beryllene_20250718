...
!> takes care of the weights in the routines CONSTRUCT_KAPPA.
...
!> the routines usually CYCLES and shortcuts on elements
!> of the kernels that are (0._q,0._q) because of Fermi-occupancy rules
!> if the full Coulob kernel is needed (for GW)
!> this fast CYCLE needs to be switched off

  LOGICAL :: FAST_CYCLE=.TRUE.
...
!**********************************************************************
  SUBROUTINE CALCULATE_LOCAL_FIELD_FOCK( &
    ...
    USE mlr_optic
    USE ini
    USE tutor, ONLY: vtutor, isAlert, AEXXzero
    IMPLICIT NONE
! structures
...
!=======================================================================
    ...
    IF (.NOT. LDIRECT .AND. ANTIRES ==0) THEN
       CALL vtutor%bug("internal error in CALCULATE_LOCAL_FIELD_FOCK: neither the direct nor the &
          &exchange contribution is selected \n the routine will do nothing, I think it is safer to " &
          // "stop right here !!", "local_field.F", 309)
    ENDIF

    IF (.NOT. ASSOCIATED(TBSE%RESPONSEFUN)) THEN
       CALL vtutor%bug("internal error in CALCULATE_LOCAL_FIELD_FOCK: TBSE not allocated", "local_field.F", 313)
    ENDIF
    IF (.NOT. ASSOCIATED(TBSEA%RESPONSEFUN) .AND. ANTIRES>=2) THEN
       CALL vtutor%bug("internal error in CALCULATE_LOCAL_FIELD_FOCK: TBSEA not allocated", "local_field.F", 316)
    ENDIF
...
! the blocking in the routines  CONSTRUCT_KAPPA
    IF (MOD(NBANDSV, NGLB)/=0) THEN
       CALL vtutor%bug("internal error in CALCULATE_LOCAL_FIELD: NGLB does not factorize NBANDSV", "local_field.F", 489)
    ENDIF
...
! G=0 component is direction dependent store the three components
    ...
    IF (.NOT. LGWLF .AND. AEXX==0.0) THEN
       CALL vtutor%write(isAlert, AEXXzero)
    ENDIF
...
10  FORMAT(" BLAS level 3 operations / number of FFT's:"/ &
           ...
           " number of operations in update of kappa        ",F14.2," Gflops, ",F13.0," fft"/ &
           " number of operations in construction of GG W GG",F14.2," Gflops, ",F13.0," fft")
    ...
    ENDIF
    CALL DUMP_RESPONSE( CHI, WGWQ, NOMEGA, ID_XI_LOCAL_FIELD, IO%IU6, 3, .FALSE.)
    CALL DUMP_RESPONSE( CHI0, WGWQ, NOMEGA, ID_XI_GW, IO%IU6, 3, .FALSE.)
    CALL DUMP_RESPONSE( TBSE, WGWQ, NOMEGA, ID_GG, IO%IU6, 3, .FALSE.)
...
! without this copy the contributions are way overestimated !
    ...
    IF (LINVXI) THEN
       ...
       CALL DUMP_RESPONSE( TBSE, WGWQ, NOMEGA, ID_FXC, IO%IU6, 3, .FALSE.)
    ...
    IF (ANTIRES>=2) THEN
       CALL DUMP_RESPONSE( TBSEA, WGWQ, NOMEGA, ID_GG_ANTIRESONANT, IO%IU6, 3, .FALSE.)
       ...
       IF (LINVXI) THEN
          ...
          CALL DUMP_RESPONSE( TBSEA, WGWQ, NOMEGA, ID_FXC_ANTIRESONANT, IO%IU6, 3, .FALSE.)
...
!***********************************************************************
!
!> Determines whether a specific q point (usually k1-k2) should
...
!*********************************************************************
!
!> performs a simple global summation
...
! checking of DETERMINE_FXC_FROM_TBSE_IDIR
    IF (FREQUENCY_DEPENDENT_TBSE .AND. IDIR_MAX>1) THEN
       CALL vtutor%bug("internal error in CALCULATE_LOCAL_FIELD_PREPARE: frequency dependent non " &
          // "isotropic fxc is presently not implemented", "local_field.F", 1139)
    ENDIF
    IF (IDIR_MAX==3) THEN
       NDIR=4
    ELSE
...
!**********************************************************************
!
!> determines  the exchange correlation kernel
!>
...
!**********************************************************************
  ...
  SUBROUTINE DETERMINE_FXC_FROM_TBSE(CHI, TBSE, WGW, LATT_CUR, IU0, IU6 )
    ...
    USE wave
    USE full_kpoints
    USE tutor, ONLY: vtutor, isError, XiSingular
...
!         WORK, SIZE(CHI%RESPONSEFUN,NOMEGA)*NWORK, INFO )
    ...
    IF (INFO>0) THEN
       CALL vtutor%write(isError, XiSingular)
    ENDIF
...
!**********************************************************************
!
!> determines  the exchange correlation kernel using block inversion
!>
!> ~~~
!>    -1        -1
!>   Xi   TBSE Xi  -> TBSE
...
!**********************************************************************
!
!> sets diagonal components of response function beyond a certain
...
!**********************************************************************
!
!> determines local field kernel f_xc on the local density functional
!> level
...
!**********************************************************************
  ...
  SUBROUTINE CALCULATE_LOCAL_FIELD_DFT( &
          GRIDC, WDES, LATT_CUR, XC, CHTOT, DENCOR, &
          WGW, NQ, ISP, TVXC)
    USE mgrid
    USE full_kpoints
    USE ldalib, only : fexcp
    ...
    TYPE (xc_info)     XC           !< information about the XC functional
...
! local
    ...
    IF (ISP/=1) THEN
       CALL vtutor%bug("internal error in CALCULATE_LOCAL_FIELD_DFT: ISP=2 is not yet implemented", "local_field.F", 1739)
    ENDIF
...
!=======================================================================
    CALL ZHEEV &
         ...
         &           W,  IFAIL)
    IF (IFAIL/=0) THEN
       CALL vtutor%bug("internal ERROR in ROTINV: Call to routine ZHEEV failed! Error code was " // &
          str(IFAIL), "local_field.F", 2150)
    ENDIF
...
!************************ SUBROUTINE ROTHALF   *************************
!
!> calculates a matrix to the power -1/2
!> but kills the very low frequency components
!
!***********************************************************************
...
!=======================================================================
    CALL ZHEEV &
         ...
         &           W,  IFAIL)
    IF (IFAIL/=0) THEN
       CALL vtutor%bug("internal ERROR in ROTHALF: Call to routine ZHEEV failed! Error code was " &
          // str(IFAIL), "local_field.F", 2202)
    ENDIF
    IF (IU6>=0) WRITE(IU6,'(10F10.6)') HFEIG
!=======================================================================
! set up the unitary transformation CEIDB for wavefunctions
...
! using strips of size m for the second band
    IF (NSTRIP1 /= SIZE(TWOELECTRON3O,1)) THEN
       CALL vtutor%bug("internal error in CONSTRUCT_KAPPA: NSTRIP1 must be equal first dimension of " &
          // "TWOELECTRON3O " // str(NSTRIP1) // " " // str(SIZE(TWOELECTRON3O,1)), "local_field.F", 2292)
    ENDIF
    IF (SIZE(TWOELECTRON3O,3)*SIZE(TWOELECTRON3O,4)/= SIZE(WKAPPA(1)%CPTWFP,2)) THEN
       CALL vtutor%bug("internal error in CONSTRUCT_KAPPA: size problem with WKAPPA " // &
          str(SIZE(TWOELECTRON3O,3)*SIZE(TWOELECTRON3O,4)) // " " // str(SIZE(WKAPPA(1)%CPTWFP,2)), "local_field.F", 2296)
    ENDIF

    IF (TBSE%SHIFT/=0 .AND. WGWQ%LGAMMA) THEN
       CALL vtutor%bug("internal error in CONSTRUCT_KAPPA: Gamma point only version does not support " &
          // "complex shift", "local_field.F", 2301)
    ENDIF
...
!     contract sum_n1,n3 <v_k1,n1| cos(G-q)r / sin(G-q)r  | c_k1+q,n3> F(n1,n3,n2,n4) = kappa_n2,n4 (G)
          ...
          NFLOAT=NFLOAT+4._q*NP*SIZE(WKAPPA(NOMEGA)%CPTWFP,2)*SIZE(TWOELECTRON3O,1)*NSTRIP3
       ELSE
!     contract sum_n1,n3 <v_k1,n1| e-i(G-q)r | c_k1+q,n3> F(n1,n3,n2,n4)  = kappa_n2,n4 (G)
          ...
          NFLOAT=NFLOAT+8._q*NP*SIZE(WKAPPA(NOMEGA)%CPTWFP,2)*SIZE(TWOELECTRON3O,1)*NSTRIP3
       ENDIF
    ENDDO
    ENDDO
...
! local
    ...
    IF (NSTRIP2 /= SIZE(TWOELECTRON3O,3)) THEN
       CALL vtutor%bug("internal error in CONSTRUCT_KAPPA: NSTRIP2 must be equal to third dimension " &
          // "of TWOELECTRON3O " // str(NSTRIP2) // " " // str(SIZE(TWOELECTRON3O,3)), "local_field.F", 2524)
    ENDIF
    IF (SIZE(TWOELECTRON3O,3)*SIZE(TWOELECTRON3O,4)/= SIZE(WKAPPA(1)%CPTWFP,2)) THEN
       CALL vtutor%bug("internal error in CONSTRUCT_KAPPA: size problem with WKAPPA " // &
          str(SIZE(TWOELECTRON3O,3)*SIZE(TWOELECTRON3O,4)) // " " // str(SIZE(WKAPPA(1)%CPTWFP,2)), "local_field.F", 2528)
    ENDIF

    IF (TBSE%SHIFT/=0 .AND. WGWQ%LGAMMA) THEN
       CALL vtutor%bug("internal error in CONSTRUCT_KAPPA: Gamma point only version does not support " &
          // "complex shift", "local_field.F", 2533)
    ENDIF
...
! for gamma point only the complex array is interpreted as a real array
          ...
          NFLOAT=NFLOAT+8._q*NSTRIP4*SIZE(TWOELECTRON3O,3)*NP*NP
...
!****************** SUBROUTINE  TWOE4O_ACC_DIRECT ***************
!> calculates direct terms for BSE
!>
...
!**********************************************************************
 ...
 SUBROUTINE TWOE4O_ACC_DIRECT(WHF, P, LATT_CUR, ISP, WGW, FSG, &
...
! passed variables
    ...
    INTEGER K1, K2, K3, K4, NPOS1, NSTRIP1, NPOS2, NSTRIP2, NPOS3, NSTRIP3, NPOS4, NSTRIP4
    COMPLEX(q) :: TWOELECTRON3O(:,:,:,:)
    INTEGER :: CBMIN, CBMAX, CBMIN4, CBMAX4, VBMIN
    REAL(q) :: NFLOAT, NFFT
    INTEGER :: NSTRIP
...
! local variables
    ...
    INTEGER NP, NB1, NB2, NB3, NB4
    TYPE (wavedes1)    WGWQ
    INTEGER NQ, NQ_
    COMPLEX(q), ALLOCATABLE :: GCHG(:,:,:)    ! charge
    COMPLEX(q), ALLOCATABLE :: GCHG12(:,:,:)    ! charge
    COMPLEX(q), ALLOCATABLE :: GCHG34(:,:,:)    ! charge
    COMPLEX(q), TARGET  :: CWORK12(MAX(GRIDHF%MPLWV,WGW%GRID%MPLWV),MAX(NSTRIP1,NSTRIP2))
    COMPLEX(q), TARGET  :: CWORK34(MAX(GRIDHF%MPLWV,WGW%GRID%MPLWV),MAX(NSTRIP3,NSTRIP4))
    COMPLEX(q),       POINTER :: GWORK12(:,:), GWORK34(:,:)
    COMPLEX(q) :: CPHASE(GRIDHF%MPLWV)
    COMPLEX(q) :: CRHOLM(AUG_DES%NPRO*WHF%WDES%NRSPINORS)        ! augmentation occupancy matrix
    COMPLEX(q) :: CRHOLM12(AUG_DES%NPROD*WHF%WDES%NRSPINORS,MAX(NSTRIP1,NSTRIP2)) ! augmentation occupancy matrix
    COMPLEX(q) :: CRHOLM34(AUG_DES%NPROD*WHF%WDES%NRSPINORS,MAX(NSTRIP3,NSTRIP4)) ! augmentation occupancy matrix
...
# 2826
    ...
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOE4O_ACC_DIRECT: q-point changed " // str(NQ) // &
          " " // str(NQ_), "local_field.F", 2831)
    ENDIF
...
! using strips of size m for the second band
    ...
    ALLOCATE( &
         ...
         GCHG34(NP, NSTRIP3 , NSTRIP4), &
         GCHG(NP, NSTRIP1, NSTRIP2), &
         CHAM(NSTRIP1, NSTRIP2, NSTRIP3, NSTRIP4))
...
!$ACC ENTER DATA CREATE(GWORK12,GWORK34,CWORK12,CWORK34,CRHOLM12,CRHOLM34,GCHG,GCHG12,GCHG34,CHAM,CPHASE) 
!$ACC ENTER DATA COPYIN(POTFAK) 
!$ACC ENTER DATA COPYIN(WPOTH%WPOT(NQ)%RESPONSEFUN) IF(LFULL)
...
!                              e^(-i Gr)
!==================================================================================
!$ACC KERNELS PRESENT(GCHG12) 
   GCHG12=0
!$ACC END KERNELS
! !$OMP PARALLEL PRIVATE(CWORK12,GWORK12,CRHOLM12,N2,NB2,NB2_INTO_TOT)
   CALL c_f_pointer(c_loc(CWORK12),GWORK12,[ SIZE(CWORK12,1),SIZE(CWORK12,2)])
! !$OMP DO REDUCTION(+:NFFT)
   DO N2=1,NSTRIP2
      NB2_INTO_TOT=NPOS2-1+N2
      NB2         =NPOS2-VBMIN+N2
      IF (FAST_CYCLE .AND. EMPTY_XI_ORBITAL(WHF%FERTOT(NB2_INTO_TOT,K2,ISP))) CYCLE

       CALL FOCK_CHARGE_MU(W1(1:NSTRIP1), W2(N2), GWORK12, CRHOLM12)

! apply phase factor to shift by reciprocal lattice vector
       IF (LPHASE) CALL APPLY_PHASE_MU(GRIDHF, CPHASE(1), CWORK12, CWORK12, NSTRIP1)

! now we have v'*_k2,n2(r'), v_k1,n1(r')
! extract using wave function FFT
       CALL FFTEXT_MU(WGWQ%NGVECTOR,NSTRIP1,WGWQ%NINDPW(1), &
                      CWORK12(1,1),SIZE(CWORK12,1),         &
                      GCHG12(1,1,N2),SIZE(GCHG12,1),        &
                      WGWQ%GRID,.FALSE.)
       NFFT=NFFT+NSTRIP1

       IF (MULTIPLY_FRACTIONAL_OCC) THEN
!$ACC PARALLEL LOOP PRESENT(GCHG12,WHF,WHF%FERTOT) 
         DO N1=1,NSTRIP1
           NB1_INTO_TOT=NPOS1-1+N1
           NB1         =NPOS1-VBMIN+N1
           GCHG12(1:NP,N1,N2)=GCHG12(1:NP,N1,N2)*SQRT(ABS(WHF%FERTOT(NB2_INTO_TOT,K2,ISP)))*SQRT(ABS(WHF%FERTOT(NB1_INTO_TOT,K1,ISP)))
         ENDDO
       ENDIF

       IF (.NOT. LFULL) THEN
          CALL APPLY_GFAC_WAVEFUN_MU(WGWQ, GCHG12(1,1,N2), POTFAK, NSTRIP1)
!$ACC KERNELS PRESENT(GCHG12) 
          GCHG12(1:NP,1:NSTRIP1,N2)=-CONJG(GCHG12(1:NP,1:NSTRIP1,N2))
!$ACC END KERNELS
       ENDIF
...
! !$OMP END PARALLEL
    IF (LFULL) THEN
          IF (WPOTH%WPOT(NQ)%LREALSTORE) THEN
             CALL DGEMM('N','N', 2*NP, NSTRIP1*NSTRIP2, 2*NP, 1.0_q, &
                  WPOTH%WPOT(NQ)%RESPONSEFUN(1,1,1), 2*SIZE(WPOTH%WPOT(NQ)%RESPONSEFUN,1), GCHG12(1,1,1), 2*SIZE(GCHG12,1), &
                  0.0_q, GCHG(1,1,1), 2*SIZE(GCHG,1) )
          ELSE
             
             CALL ZGEMM('T','N', NP, NSTRIP1*NSTRIP2, NP, (1.0_q,0.0_q), &
                  WPOTH%WPOT(NQ)%RESPONSEFUN(1,1,1), SIZE(WPOTH%WPOT(NQ)%RESPONSEFUN,1), GCHG12(1,1,1), SIZE(GCHG12,1), &
                  (0.0_q,0.0_q), GCHG(1,1,1), SIZE(GCHG,1) )
             
          ENDIF
       CALL ZCOPY( SIZE(GCHG,1)*NSTRIP1*NSTRIP2,  GCHG(1,1,1), 1, GCHG12(1,1,1), 1)
...
!                              e^-(i G'r')
!==========================================================================
!$ACC KERNELS PRESENT(GCHG34) 
          GCHG34=0
!$ACC END KERNELS

! !$OMP PARALLEL PRIVATE(CWORK34,GWORK34,CRHOLM34,N4,NB4,NB4_INTO_TOT)
   CALL c_f_pointer(c_loc(CWORK34),GWORK34,[ SIZE(CWORK34,1),SIZE(CWORK34,2)])
! !$OMP DO REDUCTION(+:NFFT)
          DO N4=1,NSTRIP4
             NB4_INTO_TOT=NPOS4-1+N4
             NB4         =NPOS4-CBMIN+N4
             IF (FAST_CYCLE .AND. FILLED_XI_ORBITAL(WHF%FERTOT(NB4_INTO_TOT,K4,ISP))) CYCLE

             CALL FOCK_CHARGE_MU(W3(1:NSTRIP3), W4(N4), GWORK34, CRHOLM34)

! apply phase factor to shift by reciprocal lattice vector
             IF (LPHASE) CALL APPLY_PHASE_MU( GRIDHF, CPHASE(1), CWORK34, CWORK34, NSTRIP3)

! now we have c_n3,k3(r')  c*_n4,k4(r')
! extract using FFT
             CALL FFTEXT_MU(WGWQ%NGVECTOR,NSTRIP3,WGWQ%NINDPW(1), &
                            CWORK34(1,1),SIZE(CWORK34,1),         &
                            GCHG34(1,1,N4),SIZE(GCHG34,1),      &
                            WGWQ%GRID,.FALSE.)
             NFFT=NFFT+NSTRIP3

! conjugate to get: c*_n3,k3(r')  c_n4,k4(r')
!$ACC KERNELS PRESENT(GCHG34) 
             GCHG34(1:NP,1:NSTRIP3,N4)=CONJG(GCHG34(1:NP,1:NSTRIP3,N4))*(1.0_q/GRIDHF%NPLWV)
!$ACC END KERNELS

             IF (MULTIPLY_FRACTIONAL_OCC) THEN
!$ACC PARALLEL LOOP PRESENT(GCHG34,WHF,WHF%FERTOT) 
                DO N3=1,NSTRIP3
                   NB3_INTO_TOT=NPOS3-1+N3
                   NB3         =NPOS3-CBMIN+N3
                   GCHG34(1:NP,N3,N4)= GCHG34(1:NP,N3,N4)*SQRT(ABS(1-WHF%FERTOT(NB3_INTO_TOT,K3,ISP)))*SQRT(ABS(1-WHF%FERTOT(NB4_INTO_TOT,K4,ISP)))
                ENDDO
             ENDIF
...
!$ACC END KERNELS
          ...
          CALL ZGEMM('C','N',  NSTRIP1*NSTRIP2 , SIZE(CHAM,3)*NSTRIP4,  NP, (1._q,0._q), &
               GCHG12(1,1,1),  SIZE(GCHG12,1), &
               GCHG34(1,1,1),  SIZE(GCHG34,1), &
               (0._q,0._q), CHAM(1,1,1,1), SIZE(CHAM,1)*SIZE(CHAM,2))
          
          NFLOAT=NFLOAT+8._q*NSTRIP1*NSTRIP2*SIZE(CHAM,3)*NSTRIP4*NP

!==========================================================================
!   store in TWOELECTRON3O
!==========================================================================
! !$OMP PARALLEL DO PRIVATE(N1,N2,NB1_INTO_TOT,NB2_INTO_TOT,N4,NB4_INTO_TOT,NB4,N3,NB3_INTO_TOT,NB3)
!$ACC PARALLEL LOOP COLLAPSE(4) PRESENT(CHAM,TWOELECTRON3O) 
          DO N1=1,NSTRIP1
             DO N2=1,NSTRIP2
           NB1_INTO_TOT=NPOS1-1+N1
           NB2_INTO_TOT=NPOS2-1+N2
...
!                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_DIRECT: out of bounds 4 " &
!                         // str(NB4), "local_field.F", 3015)
!                   ENDIF
!#endif
                   DO N3=1,NSTRIP3
!!                 NB1_INTO_TOT=NPOS1-1+N1
...
!                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_DIRECT: out of bounds 2 " &
!                            // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 3026)
!                      ENDIF
!#endif
                      TWOELECTRON3O( N1, N3 , N2, N4)=TWOELECTRON3O( N1, N3 , N2, N4)+CHAM( N1, N2, N3, N4)
# 3034
...
!$ACC EXIT DATA DELETE(CRHOLM12,CRHOLM34,GWORK12,GWORK34,CWORK12,CWORK34,POTFAK) 
!$ACC EXIT DATA DELETE(GCHG,GCHG12,GCHG34,CHAM,CPHASE) 
!$ACC EXIT DATA DELETE(WPOTH%WPOT(NQ)%RESPONSEFUN) IF(LFULL)

# 3049


    DEALLOCATE( GCHG12, GCHG34, GCHG, CHAM)

    

  END SUBROUTINE TWOE4O_ACC_DIRECT
...
! is transformed corresponding to a wavevector q
    ...
    NQ_=KPOINT_IN_FULL_GRID(WHF%WDES%VKPT(:,K3)-WHF%WDES%VKPT(:,K4),KPOINTS_FULL)
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_DIRECT: q-point changed " // str(NQ) // &
          " " // str(NQ_), "local_field.F", 3123)
    ENDIF
...
!==================================================================================
    GCHG12=0
!$OMP PARALLEL DO   PRIVATE(N1,NB1,NB1_INTO_TOT,N2,NB2_INTO_TOT,NB2,GWORK,CRHOLM) &
!$OMP REDUCTION(+:NFFT)
    DO N1=1,NSTRIP1
       NB1_INTO_TOT=NPOS1-1+N1
       NB1         =NPOS1-VBMIN+N1
       IF (FAST_CYCLE .AND. EMPTY_XI_ORBITAL(WHF%FERTOT(NB1_INTO_TOT,K1,ISP))) CYCLE
...
!$OMP PARALLEL DO   PRIVATE(N4,NB4,NB4_INTO_TOT,N3,NB3_INTO_TOT,NB3,GWORK,CRHOLM) &
!$OMP REDUCTION(+:NFFT)
          DO N4=1,NSTRIP4
             NB4_INTO_TOT=NPOS4-1+N4
             NB4         =NPOS4-CBMIN+N4
             ...
             DO N3=1,NSTRIP3
                NB3_INTO_TOT=NPOS3-1+N3
                NB3         =NPOS3-CBMIN+N3
                IF (FAST_CYCLE .AND. FILLED_XI_ORBITAL(WHF%FERTOT(NB3_INTO_TOT,K3,ISP))) CYCLE
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   DO N3=1,NSTRIP3
                      ...
                      IF ( NB3> SIZE(TWOELECTRON3O,2)) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_DIRECT: out of bounds 2 " &
                            // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 3291)
                      ENDIF
...
!$OMP END PARALLEL DO
    ...
    DEALLOCATE( GCHG12, GCHG34, CHAM)
!    WRITE(*,'(16F10.4)') REAL(TWOELECTRON3O)

  END SUBROUTINE TWOELECTRON4O_ACC_DIRECT

!****************** SUBROUTINE  TWOE4O_ACC_EX  ******************
!> calculates reasonant anti-resonant coupling exchange term for BSE
!
...
!**********************************************************************
 ...
 SUBROUTINE TWOE4O_ACC_EX(WHF, P, LATT_CUR, ISP, WGW, FSG, &
               W1, K1, NPOS1, NSTRIP1, W2, K2, NPOS2, NSTRIP2,&
               W3, K3, NPOS3, NSTRIP3, W4, K4, NPOS4, NSTRIP4,&
               TWOELECTRON3O, CBMIN, CBMAX, CBMIN4, CBMAX4, VBMIN, NFLOAT, NFFT, NSTRIP, &
               ENCUTGW, ENCUTGWSOFT)
...
! local variables
    ...
    COMPLEX(q), TARGET  :: CWORK(MAX(GRIDHF%MPLWV,WGW%GRID%MPLWV),MAX(NSTRIP1,NSTRIP2))
    COMPLEX(q),       POINTER :: GWORK(:,:)
    COMPLEX(q) :: CPHASE(GRIDHF%MPLWV)
    COMPLEX(q) :: CRHOLM(AUG_DES%NPROD*WHF%WDES%NRSPINORS,MAX(NSTRIP1,NSTRIP2)) ! augmentation occupancy matrix
    COMPLEX(q), ALLOCATABLE :: CHAM(:,:,:,:)    ! hamilton matrix
    LOGICAL LPHASE
    REAL(q) :: FSG                        ! singularity correction
    REAL(q) :: POTFAK(GRIDHF%MPLWV)       ! 1/(G+dk)**2 (G)
...
# 3392
    ...
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOE4O_ACC_EX: q-point changed " // str(NQ) // " " &
          // str(NQ_), "local_field.F", 3397)
    ENDIF
...
! using strips of size m for the second band
    ...
    ALLOCATE( &
         GCHG(NP, NSTRIP1, NSTRIP4), &
         GCHG14(NP, NSTRIP1, NSTRIP4), &
         GCHG23(NP, NSTRIP2, NSTRIP3), &
         CHAM(NSTRIP1, NSTRIP4, NSTRIP2,  NSTRIP3))
...
!$ACC ENTER DATA CREATE(GWORK,CWORK,CRHOLM,GCHG,GCHG14,GCHG23,CHAM,CPHASE) 
!$ACC ENTER DATA COPYIN(POTFAK) 
!$ACC ENTER DATA COPYIN(WPOTH%WPOT(NQ)%RESPONSEFUN) IF(LFULL)
!==========================================================================
...
!$ACC KERNELS PRESENT(GCHG14) 
       GCHG14=0
!$ACC END KERNELS


! !$OMP PARALLEL PRIVATE(CWORK,GWORK,CRHOLM,N4,NB4,NB4_INTO_TOT)
...
! !$OMP DO REDUCTION(+:NFFT)
       DO N4=1,NSTRIP4
          ...
          CALL FOCK_CHARGE_NO_CONJG_MU(W1(1:NSTRIP1), W4(N4), GWORK, CRHOLM)

! apply phase factor e^iGr first (since potential is read for NQ and not K1 +K4)
          IF (LPHASE) CALL APPLY_PHASE_MU( GRIDHF, CPHASE(1), CWORK(1,1), CWORK(1,1), NSTRIP1 )

! extract using FFT
          CALL FFTEXT_MU(WGWQ%NGVECTOR,NSTRIP1,WGWQ%NINDPW(1), &
                         CWORK(1,1),SIZE(CWORK,1),            &
                         GCHG14(1,1,N4),SIZE(GCHG14,1),       &
                         WGWQ%GRID,.FALSE.)
          NFFT=NFFT+NSTRIP1


          IF (MULTIPLY_FRACTIONAL_OCC) THEN
!$ACC PARALLEL LOOP PRESENT(GCHG14,WHF,WHF%FERTOT) 
             DO N1=1,NSTRIP1
                NB1_INTO_TOT=NPOS1-1+N1
                NB1         =NPOS1-VBMIN+N1
                GCHG14(1:NP,N1,N4)=GCHG14(1:NP,N1,N4)*SQRT(ABS(1-WHF%FERTOT(NB4_INTO_TOT,K4,ISP)))*SQRT(ABS(WHF%FERTOT(NB1_INTO_TOT,K1,ISP)))
             ENDDO
          ENDIF

          IF (.NOT. LFULL) THEN
! multiply with potential factor for simple diagonal kernel such as 4 pi e^2/ G^2
             CALL APPLY_GFAC_WAVEFUN_MU(WGWQ, GCHG14(1,1,N4), POTFAK, NSTRIP1)
!$ACC KERNELS PRESENT(GCHG14) 
             GCHG14(1:NP,1:NSTRIP1,N4)=-CONJG(GCHG14(1:NP,1:NSTRIP1,N4))
!$ACC END KERNELS
          ENDIF
       ENDDO
! !$OMP END DO
...
!$ACC KERNELS PRESENT(GCHG23) 
          GCHG23=0
!$ACC END KERNELS

! !$OMP PARALLEL PRIVATE(CWORK,GWORK,CRHOLM,N3,NB3,NB3_INTO_TOT)
   CALL c_f_pointer(c_loc(CWORK),GWORK,[ SIZE(CWORK,1),SIZE(CWORK,2)])
! !$OMP DO REDUCTION(+:NFFT)
          DO N3=1,NSTRIP3
             NB3_INTO_TOT=NPOS3-1+N3
             ...
             CALL FOCK_CHARGE_NO_CONJG_MU(W2(1:NSTRIP2), W3(N3), GWORK, CRHOLM)

! apply phase factor e^iGr if required
             IF (LPHASE) CALL APPLY_PHASE_MU( GRIDHF, CPHASE(1), CWORK, CWORK, NSTRIP2)

! now we have c_n3,k3(r')  c*_n4,k4(r')
! extract using FFT
             CALL FFTEXT_MU(WGWQ%NGVECTOR,NSTRIP2,WGWQ%NINDPW(1), &
                            CWORK(1,1),SIZE(CWORK,1),         &
                            GCHG23(1,1,N3),SIZE(GCHG23,1),      &
                            WGWQ%GRID,.FALSE.)
             NFFT=NFFT+NSTRIP2

! conjugate now to get: c*_k3,n3(r') v'*_k2,n2(r')
!$ACC KERNELS PRESENT(GCHG23) 
             GCHG23(1:NP,1:NSTRIP2,N3)=CONJG(GCHG23(1:NP,1:NSTRIP2,N3))*(1.0_q/GRIDHF%NPLWV)
!$ACC END KERNELS

             IF (MULTIPLY_FRACTIONAL_OCC) THEN
!$ACC PARALLEL LOOP PRESENT(GCHG23,WHF,WHF%FERTOT) 
                DO N2=1,NSTRIP2
                   NB2_INTO_TOT=NPOS2-1+N2
                   NB2         =NPOS2-VBMIN+N2
                   GCHG23(1:NP,N2,N3)=GCHG23(1:NP,N2,N3)*SQRT(ABS(1-WHF%FERTOT(NB3_INTO_TOT,K3,ISP)))*SQRT(ABS(WHF%FERTOT(NB2_INTO_TOT,K2,ISP)))
                ENDDO
             ENDIF
          ENDDO
! !$OMP END DO
...
!$ACC END KERNELS
          ...
          CALL ZGEMM('C' ,'N',  NSTRIP1*NSTRIP4, NSTRIP2*NSTRIP3,  NP, (1._q,0._q), &
               GCHG14(1,1,1),  SIZE(GCHG14,1), &
               GCHG23(1,1,1),  SIZE(GCHG23,1), &
               (0._q,0._q), CHAM(1,1,1,1), SIZE(CHAM,1)*SIZE(CHAM,2))
          NFLOAT=NFLOAT+8._q*NSTRIP1*NSTRIP2*NSTRIP3*NSTRIP4* NP
...
!#ifndef ACC_OFFLOAD
!                   IF (NB4 > SIZE(TWOELECTRON3O,4)) THEN
!                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX: out of bounds 4 " // &
!                         str(NB4), "local_field.F", 3581)
!                   ENDIF
!#endif
                   DO N3=1,NSTRIP3
!!                 NB1_INTO_TOT=NPOS1-1+N1
...
!#ifndef ACC_OFFLOAD
!                      IF ( NB3> SIZE(TWOELECTRON3O,2)) THEN
!                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX: out of bounds 2 " &
!                            // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 3592)
!                      ENDIF
!#endif
                      TWOELECTRON3O( N1, N3 , N2, N4)=TWOELECTRON3O( N1, N3 , N2, N4)+CHAM( N1, N4, N2, N3)
                   ENDDO
                ENDDO
             ENDDO
          ENDDO
...
!ENDDO
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!$ACC EXIT DATA DELETE(CRHOLM,GWORK,CWORK,POTFAK) 
!$ACC EXIT DATA DELETE(GCHG,GCHG14,GCHG23,CHAM,CPHASE) 
!$ACC EXIT DATA DELETE(WPOTH%WPOT(NQ)%RESPONSEFUN) IF(LFULL)

# 3610


    DEALLOCATE(GCHG14, GCHG23, CHAM )

    

  END SUBROUTINE TWOE4O_ACC_EX
...
! is transformed corresponding to a wavevector q
    ...
    NQ_=KPOINT_IN_FULL_GRID(WHF%WDES%VKPT(:,K3)+WHF%WDES%VKPT(:,K2),KPOINTS_FULL)
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX: q-point changed " // str(NQ) // " " &
          // str(NQ_), "local_field.F", 3691)
    ENDIF
...
! to cure the problem
       ...
       GCHG14=0
!$OMP PARALLEL DO   PRIVATE(N4,NB4,NB4_INTO_TOT,N1,NB1_INTO_TOT,NB1,GWORK,CRHOLM) &
!$OMP REDUCTION(+:NFFT)
       DO N4=1,NSTRIP4
          NB4_INTO_TOT=NPOS4-1+N4
          NB4         =NPOS4-CBMIN+N4
...
!==========================================================================
       DO NPOS3=CBMIN, CBMAX, NSTRIP
          ...
          GCHG23=0
!$OMP PARALLEL DO   PRIVATE(N3,NB3,NB3_INTO_TOT,N2,NB2_INTO_TOT,NB2,GWORK,CRHOLM) &
!$OMP REDUCTION(+:NFFT)
          DO N3=1,NSTRIP3
             NB3_INTO_TOT=NPOS3-1+N3
             NB3         =NPOS3-CBMIN+N3
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   DO N3=1,NSTRIP3
                      ...
                      IF ( NB3> SIZE(TWOELECTRON3O,2)) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX: out of bounds 2 " &
                            // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 3852)
                      ENDIF
...
!> calculates (see comments in TWOELECTRON4O_ACC_EX)
!>
!> int_d3r d3r' c*_k3,n3(r') v'*_k2,n2(r') c'_k4,n4(r) v_k1,n1(r)  W(r',r)
!>
!> see comments above
...
! is transformed corresponding to a wavevector q
    ...
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX_INTER: q-point changed " // str(NQ) &
          // " " // str(NQ_), "local_field.F", 3940)
    ENDIF
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   DO N3=1,NSTRIP3
                      ...
                      IF ( NB3> SIZE(TWOELECTRON3O,2)) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX: out of bounds 2 " &
                            // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 4108)
                      ENDIF
                      TWOELECTRON3O( N1, NB3 , N2, NB4)=TWOELECTRON3O( N1, NB3 , N2, NB4)+CHAM( N1, N4, N2, N3)
# 4118
...
!====================KPOINT_IN_FULL_CLOSEST============================
!>
!> this function is similar to KPOINT_IN_FULL_GRID
...
! we need to fall back to conventional k-points
       ...
       IF (NKFOUND == -1) THEN
          CALL vtutor%bug("internal error in KPOINT_IN_FULL_CLOSEST: no k-point found " // &
             str(KPOINTS_F%NKPTS) // " " // str(B), "local_field.F", 4199)
       ENDIF
    ENDIF
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   IF (NB4 > SIZE(TWOELECTRON3O,4) .OR. NB4 <= 0) THEN
                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX2: out of bounds 4 " &
                         // str(NB4), "local_field.F", 4424)
                   ENDIF
                   ...
                   DO N3=1,NSTRIP3
                      ...
                      IF ( NB3> SIZE(TWOELECTRON3O,2) .OR. NB3<=0) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX2: out of bounds 2 " &
                            // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 4432)
                      ENDIF
                      TWOELECTRON3O( N1, NB3 , N2, NB4)=TWOELECTRON3O( N1, NB3 , N2, NB4)+CHAM( N1, N4, N2, N3)
# 4442
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   DO N3=1,NSTRIP3
                      ...
                      IF ( NB3> SIZE(TWOELECTRON3O,2) .OR. NB3<=0) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_EX2: out of bounds 2 " &
                            // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 4691)
                      ENDIF
                      TWOELECTRON3O( N1, NB3 , N2, NB4)=TWOELECTRON3O( N1, NB3 , N2, NB4)+CHAM( N1, N4, N2, N3)
# 4701
...
!****************** SUBROUTINE  TWOE4O_ACC_HARTREE  ************
!> calculates Hartree terms for BSE
!>
...
!**********************************************************************
 ...
 SUBROUTINE TWOE4O_ACC_HARTREE(WHF, P, LATT_CUR, ISP, ISP2, WGW, &
               FEX, LCONJG, FMUL,  &
               W1, K1, NPOS1, NSTRIP1, W2, K2, NPOS2, NSTRIP2, &
               W3, K3, NPOS3, NSTRIP3, W4, K4, NPOS4, NSTRIP4, &
               TWOELECTRON3O, CBMIN, CBMAX, CBMIN4, CBMAX4, VBMIN, NFLOAT, NFFT, NSTRIP)
...
! local variables
    ...
    COMPLEX(q), TARGET  :: CWORK(MAX(GRIDHF%MPLWV,WGW%GRID%MPLWV),MAX(NSTRIP1,NSTRIP2))
    COMPLEX(q), TARGET  :: CGCHG(MAX(GRIDHF%MPLWV,WGW%GRID%MPLWV),MAX(NSTRIP1,NSTRIP2))
    COMPLEX(q),       POINTER :: GWORK(:,:) => NULL()
    COMPLEX(q),       POINTER :: GCHG(:,:) => NULL()
...
! set the descriptor for charge at the point k2-k4 = q'
    ...
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOE4O_ACC_HARTREE: q-point changed " // str(NQ) &
          // " " // str(NQ_), "local_field.F", 4814)
    ENDIF
...
! using strips of size m for the second band
    ...
    ALLOCATE( &
         GCHG13(NP, NSTRIP1, NSTRIP3), &
         GCHG24(NP, NSTRIP2, NSTRIP4), &
         CHAM(NSTRIP1, NSTRIP3, NSTRIP2,  NSTRIP4))

!$ACC ENTER DATA CREATE(CRHOLM,GWORK,CWORK,GCHG,CGCHG,GCHG13,GCHG24,CHAM,POTFAK,CPHASE) 
...
!$ACC KERNELS PRESENT(GCHG13) 
       GCHG13=0
!$ACC END KERNELS

! !$OMP PARALLEL PRIVATE(CWORK,GWORK,GCHG,CGCHG,CRHOLM,N3,NB3,NB3_INTO_TOT)
   CALL c_f_pointer(c_loc(CWORK),GWORK,[ SIZE(CWORK,1),SIZE(CWORK,2)])
   CALL c_f_pointer(c_loc(CGCHG),GCHG,[ SIZE(CGCHG,1), SIZE(CGCHG,2)])
! !$OMP DO REDUCTION(+:NFFT)
       DO N3=1,NSTRIP3
          NB3_INTO_TOT=NPOS3-1+N3
          NB3         =NPOS3-CBMIN+N3
          ...
          CALL FOCK_CHARGE_MU(W1(1:NSTRIP1), W3(N3), GWORK, CRHOLM)

          CALL ZCOPY(SIZE(GWORK), GWORK(1,1),1, GCHG(1,1),1)

! fft to reciprocal space
          CALL FFT3D_MU(NSTRIP1,CWORK(1,1),SIZE(CWORK,1),WGWQ%GRID,-1)
          NFFT=NFFT+NSTRIP1

! multiply by 4 pi e^2/G^2 and divide by # of gridpoints to obtain potential
          CALL APPLY_GFAC_MU(GRIDHF, CWORK(1,1), SIZE(CWORK,1),POTFAK, NSTRIP1)
...
! this is exactly (0._q,0._q) if LGWLF is set
          IF (FEX) THEN
!$ACC ENTER DATA COPYIN(FXCR) 
             CALL LF_ADD_MUL_MU(GWORK(1,1), GCHG(1,1), FXCR(1), GRIDHF%RL%NP, NSTRIP1)
!$ACC EXIT DATA DELETE(FXCR) 
          ENDIF
...
! extract using FFT
          CALL FFTEXT_MU(WGWQ%NGVECTOR,NSTRIP1,WGWQ%NINDPW(1), &
                         CWORK(1,1),SIZE(CWORK,1),            &
                         GCHG13(1,1,N3),SIZE(GCHG13,1),       &
                         WGWQ%GRID,.FALSE.)
          NFFT=NFFT+NSTRIP1

!$ACC KERNELS PRESENT(GCHG13) 
          GCHG13(1:NP,1:NSTRIP1,N3)=CONJG(GCHG13(1:NP,1:NSTRIP1,N3))*(1.0_q/GRIDHF%NPLWV)
!$ACC END KERNELS

          IF (MULTIPLY_FRACTIONAL_OCC) THEN
!$ACC PARALLEL LOOP PRESENT(GCHG13,WHF,WHF%FERTOT) 
            DO N1=1,NSTRIP1
              NB1_INTO_TOT=NPOS1-1+N1
              NB1         =NPOS1-VBMIN+N1
              GCHG13(1:NP,N1,N3)=GCHG13(1:NP,N1,N3)*SQRT(ABS(1-WHF%FERTOT(NB3_INTO_TOT,K3,ISP)))*SQRT(ABS(WHF%FERTOT(NB1_INTO_TOT,K1,ISP)))
            ENDDO
          ENDIF
       ENDDO
! !$OMP END DO
...
! !$OMP DO REDUCTION(+:NFFT)
       DO N4=1,NSTRIP4
          NB4_INTO_TOT=NPOS4-1+N4
          NB4         =NPOS4-CBMIN+N4
          IF (FAST_CYCLE .AND. FILLED_XI_ORBITAL(WHF%FERTOT(NB4_INTO_TOT,K4,ISP2))) CYCLE

          CALL FOCK_CHARGE_MU(W2(1:NSTRIP2), W4(N4), GWORK, CRHOLM)

! apply phase factor e^iGr if required
          IF (LPHASE) CALL APPLY_PHASE_MU( GRIDHF, CPHASE(1), CWORK(1,1), CWORK(1,1), NSTRIP2)

! now we have v'_k2,n2(r') c'*_k4,n4(r')
! extract using FFT
          CALL FFTEXT_MU(WGWQ%NGVECTOR,NSTRIP2,WGWQ%NINDPW(1), &
                         CWORK(1,1),SIZE(CWORK,1),            &
                         GCHG24(1,1,N4),SIZE(GCHG24,1),       &
                         WGWQ%GRID,.FALSE.)
          NFFT=NFFT+NSTRIP1

!  conjugate c'_k4,n4(r') v'*_k2,n2(r')
!$ACC KERNELS PRESENT(GCHG24) 
          GCHG24(1:NP,1:NSTRIP2,N4)=CONJG(GCHG24(1:NP,1:NSTRIP2,N4))*(1.0_q/GRIDHF%NPLWV)
!$ACC END KERNELS

          IF (MULTIPLY_FRACTIONAL_OCC) THEN
!$ACC PARALLEL LOOP PRESENT(GCHG24,WHF,WHF%FERTOT) 
             DO N2=1,NSTRIP2
               NB2_INTO_TOT=NPOS2-1+N2
               NB2         =NPOS2-VBMIN+N2
               GCHG24(1:NP,N2,N4)=GCHG24(1:NP,N2,N4)*SQRT(ABS(1-WHF%FERTOT(NB4_INTO_TOT,K4,ISP)))*SQRT(ABS(WHF%FERTOT(NB2_INTO_TOT,K2,ISP)))
             ENDDO
          ENDIF
...
!$ACC END KERNELS
          ...
          NFLOAT=NFLOAT+8._q*NSTRIP1*NSTRIP3*NSTRIP2*NSTRIP4*NP
!==========================================================================
!   store in TWOELECTRON3O
!==========================================================================
!$ACC PARALLEL LOOP COLLAPSE(4) PRESENT(CHAM,TWOELECTRON3O) 
! !$OMP PARALLEL DO PRIVATE(N1,N2,NB1_INTO_TOT,NB2_INTO_TOT,N4,NB4_INTO_TOT,NB4,N3,NB3_INTO_TOT,NB3)
          DO N1=1,NSTRIP1
             DO N2=1,NSTRIP2
                DO N4=1,NSTRIP4
                   DO N3=1,NSTRIP3
                      TWOELECTRON3O( N1, N3 , N2, N4)=TWOELECTRON3O( N1, N3 , N2, N4)+CHAM( N1, N3, N2, N4)*FMUL
                   ENDDO
                ENDDO
             ENDDO
          ENDDO
...
!ENDDO
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!$ACC EXIT DATA DELETE(CRHOLM,CWORK,GWORK,POTFAK,CGCHG,GCHG,GCHG13,GCHG24,CHAM,CPHASE) 


# 5012


    DEALLOCATE(GCHG13, GCHG24, CHAM )
!  restore semilocal exchange correlation
  ...
  END SUBROUTINE TWOE4O_ACC_HARTREE
...
! set the descriptor for charge at the point k2-k4 = q'
    NQ_=KPOINT_IN_FULL_GRID(WHF%WDES%VKPT(:,K2)-WHF%WDES%VKPT(:,K4),KPOINTS_FULL)
    IF (NQ/=NQ_) THEN
       CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HARTREE: q-point changed " // str(NQ) &
          // " " // str(NQ_), "local_field.F", 5114)
    ENDIF
...
! to cure the problem
       ...
       GCHG13=0
!$OMP PARALLEL DO   PRIVATE(N3,NB3,NB3_INTO_TOT,N1,NB1_INTO_TOT,NB1,GWORK,GCHG,CRHOLM) &
!$OMP REDUCTION(+:NFFT)
       DO N3=1,NSTRIP3
          NB3_INTO_TOT=NPOS3-1+N3
          NB3         =NPOS3-CBMIN+N3
...
!$OMP PARALLEL DO   PRIVATE(N4,NB4,NB4_INTO_TOT,N2,NB2_INTO_TOT,NB2,GWORK,CRHOLM) &
!$OMP REDUCTION(+:NFFT)
          DO N4=1,NSTRIP4
             NB4_INTO_TOT=NPOS4-1+N4
             NB4         =NPOS4-CBMIN+N4
             ...
             DO N2=1,NSTRIP2
                NB2_INTO_TOT=NPOS2-1+N2
                NB2         =NPOS2-VBMIN+N2
                IF (FAST_CYCLE .AND. EMPTY_XI_ORBITAL(WHF%FERTOT(NB2_INTO_TOT,K2,ISP2))) CYCLE
          ...
          DO N1=1,NSTRIP1
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   IF (NB4 > SIZE(TWOELECTRON3O,4)) THEN
                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HARTREE: out of bounds 4 " &
                         // str(NB4) // "\n when it should be max: " // str(SIZE(TWOELECTRON3O,4)), "local_field.F", 5268)
                   ENDIF
...
!
!> calculates the head (singular contribution) related to the
!> Hartree-term
...
!**********************************************************************
 ...
 SUBROUTINE TWOELECTRON4O_ACC_HEAD(WHF, P, LATT_CUR, ISP, ISP2, WGW, &
               FMUL,  &
               W1, K1, NPOS1, NSTRIP1, W2, K2, NPOS2, NSTRIP2, W3, K3, W4, K4, &
               TWOELECTRON3O, CBMIN, CBMAX, CBMIN4, CBMAX4, NSTRIP)
...
! passed variables
    ...
    INTEGER K1, K2, K3, K4, NPOS1, NSTRIP1, NPOS2, NSTRIP2
    COMPLEX(q) :: TWOELECTRON3O(:,:,:,:)
    INTEGER :: CBMIN, CBMAX, CBMIN4, CBMAX4
    INTEGER :: NSTRIP
...
! local variables
    ...
    COMPLEX(q) :: CDER_BETWEEN_STATE13(3),CDER_BETWEEN_STATE24(3)
...
! is transformed corresponding to a wavevector q
    ...
    IF (K1_IN_FULL_ORIG==0) RETURN
    K2_IN_FULL_ORIG=KPOINT_IN_FULL_GRID(WHF%WDES%VKPT(:,K2),KPOINTS_FULL_ORIG)
    IF (K2_IN_FULL_ORIG==0) RETURN

! TODO: maybe remove outer blocking, (1._q,0._q) loop N3 from CBMIN to CBMAX
    DO NPOS3=CBMIN, CBMAX, NSTRIP
       NSTRIP3=MIN(CBMAX+1-NPOS3,NSTRIP)

! TODO: maybe remove outer blocking, (1._q,0._q) loop N4 from CBMIN4 to CBMAX4
       DO NPOS4=CBMIN4, CBMAX4, NSTRIP
          NSTRIP4=MIN(CBMAX4+1-NPOS4,NSTRIP)
          ...
          DO N1=1,NSTRIP1
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   IF (NB4 > SIZE(TWOELECTRON3O,4)) THEN
                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HEAD: out of bounds 4 "&
                          // str(NB4) // "\n when it should be max: " // str(SIZE(TWOELECTRON3O,4)), "local_field.F", 5373)
                   ENDIF
                   IF (K2_IN_FULL_ORIG /=0) THEN
                      CALL  CDER_BETWEEN_STATES_ROTATED_NOERR( &
                           CDER_BETWEEN_STATE24,LATT_CUR, K2_IN_FULL_ORIG, ISP2, NB2_INTO_TOT , NB4_INTO_TOT )
                   ENDIF
                   ...
                   DO N3=1,NSTRIP3
                      ...
                      NB3         =NPOS3-CBMIN+N3
                      IF (NB3 > SIZE(TWOELECTRON3O,2)) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HEAD: out of bounds 2 " &
                            // str(NB3) // "\n when it should be max: " // str(SIZE(TWOELECTRON3O,2)), "local_field.F", 5385)
                      ENDIF
...
!
!> calculates the transition element
!>~~~
...
! local variables
    ...
    INTEGER N3, NB1, NB3
    INTEGER NQ
    COMPLEX(q) :: GWORK(MAX(GRIDHF%MPLWV,WGW%GRID%MPLWV),VBMAX-VBMIN+1)
    COMPLEX(q) :: CPHASE(GRIDHF%MPLWV)
    COMPLEX(q) :: CRHOLM(AUG_DES%NPROD*WHF%WDES%NRSPINORS,VBMAX-VBMIN+1)        ! augmentation occupancy matrix
    LOGICAL LPHASE
    COMPLEX(q), EXTERNAL :: CRHO0
    COMPLEX(q) :: CDER(VBMAX-VBMIN+1)
    INTEGER I1,I2,I3,NC
...
!$ACC ENTER DATA CREATE(CRHOLM,GWORK,CPHASE,CDER) 
...
!==========================================================================
    DO N3=CBMIN, CBMAX
       ...
       CALL FOCK_CHARGE_MU(W1(1:VBMAX-VBMIN+1), W3(NB3), GWORK, CRHOLM)

! apply phase factor e^iGr if required
       IF (LPHASE) CALL APPLY_PHASE_MU( GRIDHF, CPHASE(1), GWORK(1,1), GWORK(1,1), VBMAX-VBMIN+1)

       CALL FFT3D_MU(VBMAX-VBMIN+1,GWORK(1,1),SIZE(GWORK,1),GRIDHF,-1)
       NFFT=NFFT+VBMAX-VBMIN+1

!$ACC KERNELS  
       CDER=0
!$ACC END KERNELS

! copy of CRHO0 function
!$ACC PARALLEL LOOP PRIVATE(N1,NC) 
       DO N1=VBMIN,VBMAX
          NB1=N1-VBMIN+1
          DO NC=1,GRIDHF%RC%NCOL
            IF (GRIDHF%RC%I2(NC)==1 .AND. GRIDHF%RC%I3(NC)==1) THEN
              CDER(NB1)=GWORK(NC,NB1)*(1.0_q/GRIDHF%NPLWV)
            ENDIF
          ENDDO
       ENDDO

       CALL M_sum_z( GRIDHF%COMM, CDER, SIZE(CDER))

!$ACC PARALLEL LOOP PRESENT(CDER_BETWEEN_STATES_Q,CDER) 
       DO N1=VBMIN,VBMAX
          NB1_INTO_TOT=N1
          NB1         =N1-VBMIN+1 ! local index into W1
          CDER_BETWEEN_STATES_Q(NB3_INTO_TOT, NB1_INTO_TOT, NK1_IN_KPOINTS_FULL_ORIG, ISP)=CDER(NB1)
       ENDDO

    ENDDO
!$ACC EXIT DATA DELETE(CRHOLM,GWORK,CPHASE,CDER) 


  END SUBROUTINE ONEELECTRON_TRANS

!****************** SUBROUTINE  TWOE4O_ACC_HARTREE_SFLIP *******
!
!> calculates Hartree terms allowing for spin flips
...
!**********************************************************************
 ...
 SUBROUTINE TWOE4O_ACC_HARTREE_SFLIP(WHF, P, LATT_CUR, ISP, ISP2, WGW, &
               FEX, LCONJG, FMUL,  &
               W1, K1, NPOS1, NSTRIP1, W2, K2, NPOS2, NSTRIP2, &
               W3, K3, NPOS3, NSTRIP3, W4, K4, NPOS4, NSTRIP4, &
               TWOELECTRON3O, CBMIN, CBMAX, VBMIN, &
...
! !$OMP PARALLEL PRIVATE(CWORK,GWORK,CGCHG,GCHG,CRHOLM,N3,NB3,NB3_INTO_TOT)
   CALL c_f_pointer(c_loc(CWORK),GWORK,[ SIZE(CWORK,1),SIZE(CWORK,2)])
   CALL c_f_pointer(c_loc(CGCHG),GCHG,[ SIZE(CGCHG,1), SIZE(CGCHG,2)])
! !$OMP DO REDUCTION(+:NFFT)
       DO N3=1,NSTRIP3
          ...
          CALL ZCOPY(SIZE(CWORK), GWORK(1,1),1, GCHG(1,1),1)
...
! !$OMP PARALLEL PRIVATE(CWORK,GWORK,CGCHG,GCHG,CRHOLM,N4,NB4,NB4_INTO_TOT)
   CALL c_f_pointer(c_loc(CWORK),GWORK,[ SIZE(CWORK,1),SIZE(CWORK,2)])
! !$OMP DO REDUCTION(+:NFFT)
          DO N4=1,NSTRIP4
             NB4_INTO_TOT=NPOS4-1+N4
...
!$ACC PARALLEL LOOP PRESENT(GCHG24,WHF,WHF%FERTOT) 
                DO N2=1,NSTRIP2
                  NB2_INTO_TOT=NPOS2-1+N2
                  NB2         =NPOS2-VBMIN+N2
                  GCHG24(1:NP,N2,N4)=GCHG24(1:NP,N2,N4)*SQRT(ABS(1-WHF%FERTOT(NB4_INTO_TOT,K4,ISP2)))*SQRT(ABS(WHF%FERTOT(NB2_INTO_TOT,K2,ISP2)))
                ENDDO
             ENDIF
...
!#ifndef ACC_OFFLOAD
!                   IF (NB4 > SIZE(TWOELECTRON3O,4)) THEN
!                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HARTREE_SF: out of bounds 4 " &
!                         // str(NB4) // "\n when it should be max: " // str(SIZE(TWOELECTRON3O,4)), "local_field.F", 5789)
!                   ENDIF
!#endif
                   DO N3=1,NSTRIP3
!!                 NB1_INTO_TOT=NPOS1-1+N1
...
!#ifndef ACC_OFFLOAD
!                      IF ( NB3> SIZE(TWOELECTRON3O,2)) THEN
!                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HARTREE_SF: out of " &
!                            // "bounds 2 " // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 5800)
!                      ENDIF
!#endif
                      TWOELECTRON3O( N1, N3 , N2, N4)=TWOELECTRON3O( N1, N3 , N2, N4)+CHAM( N1, N3, N2, N4)*FMUL
!#if defined (dump_twoe)  && ! defined (ACC_OFFLOAD)
...
!ENDDO
!$ACC WAIT(ACC_ASYNC_Q) IF(OFFLOAD_ON)
!$ACC EXIT DATA DELETE(CRHOLM,CWORK,GWORK,POTFAK,GCHG,CGCHG,GCHG13,GCHG24,CHAM,CPHASE) 
...
!  restore semilocal exchange correlation
  ...
  END SUBROUTINE TWOE4O_ACC_HARTREE_SFLIP
          ...
          DO N1=1,NSTRIP1
             ...
             DO N2=1,NSTRIP2
                ...
                DO N4=1,NSTRIP4
                   ...
                   IF (NB4 > SIZE(TWOELECTRON3O,4)) THEN
                      CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HARTREE_SF: out of bounds 4 " &
                         // str(NB4) // "\n when it should be max: " // str(SIZE(TWOELECTRON3O,4)), "local_field.F", 6077)
                   ENDIF
                   DO N3=1,NSTRIP3
                      NB3_INTO_TOT=NPOS3-1+N3
                      NB3         =NPOS3-CBMIN+N3
                      IF ( NB3> SIZE(TWOELECTRON3O,2)) THEN
                         CALL vtutor%bug("internal error in TWOELECTRON4O_ACC_HARTREE_SF: out of " &
                            // "bounds 2 " // str(NB3_INTO_TOT) // " " // str(NB3), "local_field.F", 6084)
                      ENDIF
                      TWOELECTRON3O( N1, NB3 , N2, NB4)=TWOELECTRON3O( N1, NB3 , N2, NB4)+CHAM( N1, N3, N2, N4)*FMUL
# 6094
...
!****************** SUBROUTINE  LF_ADD_MUL_MU ********************************
!
! batched version of LF_ADD_MUL
!
!**********************************************************************

SUBROUTINE LF_ADD_MUL_MU( GWORK, GCHG, FXCR, N, NB)
...
!!  USE moffload_struct_def
  ...
  IMPLICIT NONE
  INTEGER :: N,NB
  COMPLEX(q) :: GWORK( N,NB), GCHG( N,NB), FXCR(N)

  INTEGER :: I,J

  

!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(GWORK,FXCR,GCHG) 
  DO J=1,NB
    DO I=1,N
       GWORK(I,J)=GWORK(I,J)+GCHG(I,J)*FXCR(I)
    ENDDO
  ENDDO
  

END SUBROUTINE LF_ADD_MUL_MU
