...
def _normalise_dielectric_component_boundary(boundary):
    """
    Convert one component boundary specification to a (start, end) tuple.

    Accepted forms
    --------------
    None
        No boundary restriction: (None, None).
    number
        Range from zero to that number: (0, number).
    (number,) or ((start, end),)
        One-item wrappers are unwrapped recursively.
    (start, end)
        Explicit range. Either endpoint may be None.
    """
    if boundary is None:
        return (None, None)

    if isinstance(boundary, (int, float, np.number)) and not isinstance(
        boundary, (bool, np.bool_)
    ):
        return (0, boundary)
    ...
    if isinstance(boundary, (tuple, list)):
        ...
        if len(boundary) == 1:
            return _normalise_dielectric_component_boundary(boundary[0])

        if len(boundary) == 2:
            start, end = boundary
            valid_endpoint = lambda value: (
                value is None
                or (
            ...
            if valid_endpoint(start) and valid_endpoint(end):
                return (start, end)

    raise TypeError(
        "Each component boundary must be None, a number, a one-item "
...
def _expand_dielectric_component_boundaries(boundary, component_count):
    ...
    For multiple components, a top-level tuple/list whose length equals
    component_count is interpreted component by component. Otherwise, the
    supplied boundary is treated as one shared boundary.

    Examples for two components
    ---------------------------
    8
        [(0, 8), (0, 8)]
    (8,)
    ...
    if (
        ...
        and len(boundary) == component_count
        and component_count > 1
    ):
        return [
            _normalise_dielectric_component_boundary(component_boundary)
            for component_boundary in boundary
        ]

    shared_boundary = _normalise_dielectric_component_boundary(boundary)
    return [shared_boundary for _ in range(component_count)]

def plot_dielectric_function_componentwise(suptitle, systems=None, components=None,
                             layout="horizontal", expansion_label=True,
                             unit=None, x_boundary=(None, None), y_boundary=(None, None),
                             spectrum_flag=None, figure_size = (None,None)):
    ## Help information
    ...
    ## mono-component flag
    if isinstance(components, str) or isinstance(components, dict):
        mono_x_boundary = _expand_dielectric_component_boundaries(x_boundary, 1)[0]
        return plot_dielectric_monocomp(
            suptitle, systems, components, layout, expansion_label, unit,
            mono_x_boundary, spectrum_flag, figure_size
        )
    elif isinstance(components, list) and len(components) == 1:
        mono_x_boundary = _expand_dielectric_component_boundaries(x_boundary, 1)[0]
        return plot_dielectric_monocomp(
    ...
    ## one x-axis boundary for each component
    component_x_boundaries = _expand_dielectric_component_boundaries(
        x_boundary, len(components)
    )
    ...
    if expansion_flag is True:
        for subplot_index in range(2*len(components)):
            ...
            # current component index, label, and x-axis boundary
            component_index = subplot_index // 2
            current_component = comp_labels[component_index].lower()
            photon_start, photon_end = component_x_boundaries[component_index]
